...
Процедура СоздатьРКО() Экспорт
	...
	Если НетДаты Или НетНомера или НетКассы Тогда
		ТекстСообщения = "";
		...
		КонецЕсли;
		Если НетКассы Тогда
			ТекстСообщения = ТекстСообщения + ?(НетНомера или НетДаты,Символы.ПС,"") + "Не указана касса!";
		КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из РКО Цикл
		...
		Если СтрокаТаблицы.Отметка И НЕ ЗначениеЗаполнено(СтрокаТаблицы.РКО) Тогда
			...
			ДокументРКО.РасчетныйДокумент	= ПлатежнаяВедомость;
			...
			ДокументРКО.УстановитьПриложение(ПлатежнаяВедомость);
			...
			//АБС+++ Гончаров 21.05.2013
			ДокументРКО.абс_СчетКассы = ПланыСчетов.Хозрасчетный.КассаОрганизации;
			//\\АБС--- Гончаров 21.05.2013
...
Функция ПровестиРКО() Экспорт
	...
	Для Каждого СтрокаТаблицы Из РКО Цикл
		Если СтрокаТаблицы.Отметка и ЗначениеЗаполнено(СтрокаТаблицы.РКО) Тогда
			...
			Исключение
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Не удалось провести документ: " + Строка(ДокументРКО),Отказ);
			КонецПопытки;
