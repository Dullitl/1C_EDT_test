...
Процедура ЗаполнитьНачальныеНастройки(НачальноеЗаполнение = Истина) Экспорт
	...
	Иначе
		...
		ТекстЗапроса = ФормированиеСпискаРаботниковПереопределяемый.ПолучитьТекстЗапросаНачальныеНастройкиДругиеРегистры(ЭтотОбъект, СтруктураПредставлениеПолей, СтруктураФорматаПолей, мСоответствиеНазначений, мСтруктураДляОтбораПоКатегориям, ТаблицаПолей, НоваяСтрока);
	...
	СтруктураПредставлениеПолей.Вставить("ОбособленноеПодразделение",	ОбщегоНазначения.ПреобразоватьСтрокуИнтерфейса("Организация")); 
...
Функция ПолучитьСписокРаботников() Экспорт
	...
	Если ИсполнятьЗапрос Тогда
		...
		Если РезультатОтбора.Пустой() Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не обнаружено сотрудников, удовлетворяющих указанным условиям!");
		КонецЕсли;
