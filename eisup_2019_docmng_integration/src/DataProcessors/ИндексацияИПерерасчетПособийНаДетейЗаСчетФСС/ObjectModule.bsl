...
Процедура ПолучитьДанныеПоНачисленнымПособиям() Экспорт
	...
	|			ИНАЧЕ 100
	|		КОНЕЦ / 100 * (ВЫРАЗИТЬ(ДЕНЬГОДА(КОНЕЦПЕРИОДА(ОсновныеНачисленияРаботниковОрганизаций.ПериодДействия, ГОД)) / 12 КАК ЧИСЛО(15, 1))) КАК ЧИСЛО(15, 2)) КАК МесячноеПособие,
