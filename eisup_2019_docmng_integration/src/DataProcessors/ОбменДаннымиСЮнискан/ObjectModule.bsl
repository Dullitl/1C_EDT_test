...
Функция ПолучитьДеревоИзФайла(ФайлДляЧтения) Экспорт
	...
	Исключение
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Импорт из файла """ + ФайлДляЧтения + """ : В файле нарушена структура XML или это не XML файл!");
	...
	Исключение
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Возникла ошибка при чтении файла: " + ОписаниеОшибки());
		Возврат Неопределено;
	КонецПопытки;
...
Функция ОтправитьДеревоПоHTTP(Дерево, ИмяФайлаДляЗаписиЗапроса, ИмяФайлаДляЗаписиОтвета, ИмяСервера, АдресОтправки) Экспорт
	...
	Исключение
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Возникла ошибка при отправке данных: " + ОписаниеОшибки());
		Возврат Неопределено;
	КонецПопытки;
	...
	Если Не Файл.Существует() Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не найден файл ответа : " + ИмяФайлаДляЗаписиОтвета);
		Возврат Неопределено;
	КонецЕсли;
	
	// может файл пустой
	Если Файл.Размер() = 0 Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Файл ответа пустой: " + ИмяФайлаДляЗаписиОтвета);
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПолучитьGLNПредприятия() Экспорт
	...
	Если мGLNПредприятия = Неопределено Тогда
		...
		Если НЕ ЗначениеЗаполнено(мGLNПредприятия) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не заполнена константа ""GLN предприятия""");
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьПарольПредприятия() Экспорт 
	...
	Если мПарольПредприятия = Неопределено Тогда
		...
		Если НЕ ЗначениеЗаполнено(мПарольПредприятия) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не заполнена константа ""Пароль предприятия""");
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьПрефиксПредприятия() Экспорт 
	...
	Если мПрефиксПредприятия = Неопределено Тогда
		...
		Если НЕ ЗначениеЗаполнено(мПрефиксПредприятия) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не заполнена константа ""Префикс предприятия""");
		КонецЕсли;
	КонецЕсли;
