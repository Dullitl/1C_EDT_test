...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбластиПоискаДублей.НайтиПоЗначению(ОбластьПоискаДублей) = Неопределено Тогда
		ОбластьПоискаДублей = "";
	КонецЕсли;
	...
	ОтображениеСостояния = Элементы.ДублиУспешноУдалены.ОтображениеСостояния;
	ОтображениеСостояния.Видимость = Истина;
	ОтображениеСостояния.Текст = НСтр("ru = 'Дубли успешно удалены'");
	ОтображениеСостояния.Картинка = Элементы.Информация32.Картинка;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИмяФормыИсточника = "ОбластьПоискаДублей" Тогда
		ПредставлениеОбластиПоискаДублей = ОбластиПоискаДублей.НайтиПоЗначению(ВыбранноеЗначение).Представление;
...
Процедура ОбновитьМестаИспользованияНеобработанныхДубли(Знач СтрокаДанных)
	...
	Если ДанныеСтроки.Количество = 0 Тогда
		Элементы.ОписаниеТекущейГруппыДублей.Заголовок = ФорматСтроки(НСтр("ru = 'Элемент ""%1"" успешно обработан'"), 
			ДанныеСтроки.Наименование
		);
...
&НаКлиенте
Процедура ПравилаОтбора(Команда)
...
&НаКлиенте
Процедура ПравилаПоиска(Команда)
	...
	ПараметрыФормы.Вставить("ПредставлениеОбластиОтбора", Элементы.ПредставлениеОбластиПоискаДублей.СписокВыбора[0].Представление);
...
&НаКлиенте
Процедура РазвернутьГруппуДублей(Команда)
	
	РазвернутьГруппуДублейИерархически( Элементы.НайденныеДубли.ТекущаяСтрока );
	
КонецПроцедуры

&НаКлиенте
Процедура СвернутьГруппуДублей(Команда)
	
	СвернутьГруппуДублейИерархически( Элементы.НайденныеДубли.ТекущаяСтрока );
...
Функция ЗаполнитьРезультатыУдаленияДублей(Знач ТаблицаОшибок)
	...
	Если ЗавершеноБезОшибок Тогда
		...
		ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(Элементы.Информация16.Картинка, " ", ФорматСтроки(
			НСтр("ru = 'Указанные дубли (%1) объединены в элементы (%2)'"),
	...
	Пока ПозицияРодителя >= 0 Цикл
		...
		Пока ПозицияПотомка >= 0 Цикл
			...
			ИначеЕсли ТаблицаОшибок.Найти("Ссылка", Потомок.Ссылка) = Неопределено Тогда
		...
		Если Потомки.Количество() = 1 И Потомки[0].Основной Тогда
			Родители.Удалить(Родитель);
		Иначе
			Родитель.Количество = Потомки.Количество();
			Родитель.Наименование = ОсновнойПотомок.Наименование + " (" + Родитель.Количество + ")";
		КонецЕсли;
...
Процедура ИнициализироватьКомпоновщикОтбораИПравила()
	...
	КомпоновщикПредварительногоОтбора.Инициализировать( Новый ИсточникДоступныхНастроекКомпоновкиДанных(АдресСхемыКомпоновки) );
...
Процедура ШагУспешноеУдалениеПриАктивации(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	Элементы.ДублиУспешноУдалены.ОтображениеСостояния.Текст = ОписаниеСостоянияНайденныхДублей;
	Элементы.Заголовок.Доступность = Ложь;
...
// Завершение вопроса перед удалением дублей
&НаКлиенте
Процедура ЗавершениеПодтвержденияУдаленияБезПрикладныхПравил(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	Если РезультатВопроса <> КодВозвратаДиалога.Да Тогда
		// Отказ от удаления дублей
		Возврат;
	КонецЕсли;
...
Функция СостояниеДлительнойОперацииМастера()
	...
	ИначеЕсли ТипЗнч(НастройкиМастера.ИдентификаторФоновогоЗадания) <> Тип("УникальныйИдентификатор") Тогда
		...
		НакопленныеСообщения = ПолучитьСообщенияПользователю(Истина);
