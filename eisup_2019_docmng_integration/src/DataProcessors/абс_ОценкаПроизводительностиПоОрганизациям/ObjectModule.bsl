...
Функция ПолучитьAPDEX(ПараметрыЗапроса)
	...
	|ПОМЕСТИТЬ ПользователиОрганизации
	|ИЗ
	|	Справочник.Пользователи КАК Пользователи
	|ГДЕ
	|	Пользователи.абс_Сотрудник.Организация = &Организация";
	Запрос.Выполнить();
	...
	|ГДЕ
	|	НЕ КлючевыеОперации.КлючеваяОперация = &КОИтого
	|	И ЗамерыВремени.ИмяПользователя В
	|			(ВЫБРАТЬ
	|				ПользователиОрганизации.ИмяПользователя
	|			ИЗ
	|				ПользователиОрганизации)
	|
	|СГРУППИРОВАТЬ ПО
	|	КлючевыеОперации.КлючеваяОперация,
	|	КлючевыеОперации.НомерСтроки,
