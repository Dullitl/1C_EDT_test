...
Функция ПоказателиПроизводительности() Экспорт
	...
	КлючеваяОперацияИтого = Справочники.абс_БСПКлючевыеОперации.ОбщаяПроизводительностьСистемы;
...
Функция ВычислитьAPDEX(ПараметрыВычисления) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.абс_БСПЗамерыВремени КАК ЗамерыВремени
	...
	Иначе
		...
		|								КОГДА ЗамерыВремени.ДатаНачалаЗамера >= &НачалоПериода%Номер%
		|										И ЗамерыВремени.ДатаНачалаЗамера <= &КонецПериода%Номер%
		|									ТОГДА ЗамерыВремени.ВремяВыполнения
		|								ИНАЧЕ 0
		|							КОНЕЦ)  / СУММА(ВЫБОР
		|								КОГДА ЗамерыВремени.ДатаНачалаЗамера >= &НачалоПериода%Номер%
		|										И ЗамерыВремени.ДатаНачалаЗамера <= &КонецПериода%Номер%
