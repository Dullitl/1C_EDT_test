...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Попытка
		ЗагружаемаяНастройка = ЗагрузитьКлючевыеОперации();
	...
	СписокВыбора = Элементы.Отображение.СписокВыбора;
	СписокВыбора.Добавить("APDEX", "Коэффициент APDEX");
	СписокВыбора.Добавить("Среднее", "Среднее время выполнения");
	
	Объект.Отображение = "APDEX";
КонецПроцедуры
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Обработка.абс_БСПОценкаПроизводительности.Форма.ФормаОтбора") Тогда
		
		Если ВыбранноеЗначение <> Неопределено Тогда
			ОбновитьПоказатели(ВыбранноеЗначение);
		КонецЕсли;
		
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Справочник.абс_БСПКлючевыеОперации.Форма.ФормаВыбора") Тогда
		...
		ПредопределенныйЭлемент = ПредопределенноеЗначение("Справочник.абс_БСПКлючевыеОперации.ОбщаяПроизводительностьСистемы");
		ТекущаяСтрока.КлючеваяОперация = ВыбранноеЗначение.КлючеваяОперация;
		ТекущаяСтрока.ЦелевоеВремя = ВыбранноеЗначение.ЦелевоеВремя;
		ТекущаяСтрока.Приоритет = ВыбранноеЗначение.Приоритет;
...
Процедура ПроизводительностьВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Лев(Поле.Имя, 18) <> "Производительность" Или
		СтрокаТЧ.КлючеваяОперация = ПредопределенноеЗначение("Справочник.абс_БСПКлючевыеОперации.ОбщаяПроизводительностьСистемы")
		Тогда
			Возврат;
	КонецЕсли;
	СтандартнаяОбработка = Ложь;
	...
	ОткрытьФорму("Обработка.абс_БСПОценкаПроизводительности.Форма.ИсторияВыполнения", ПараметрыОткрытия, ЭтаФорма);
...
Процедура ПереместитьВверх(Команда)
	...
	Если Врем.Количество() <= 1 ИЛИ 
		ТекущийИндекс = 0 ИЛИ 
		ТД.КлючеваяОперация = ПредопределенноеЗначение("Справочник.абс_БСПКлючевыеОперации.ОбщаяПроизводительностьСистемы")
		Тогда
			Возврат;
	КонецЕсли;
...
Процедура ПереместитьВниз(Команда)
	...
	Если Врем.Количество() <= 1 Или
		...
		Врем[ТекущийИндекс + 1].КлючеваяОперация = ПредопределенноеЗначение("Справочник.абс_БСПКлючевыеОперации.ОбщаяПроизводительностьСистемы") Или
		ТД.КлючеваяОперация = ПредопределенноеЗначение("Справочник.абс_БСПКлючевыеОперации.ОбщаяПроизводительностьСистемы")
		Тогда
			Возврат;
	КонецЕсли;
...
Процедура ЭкспортДанных(Команда)
	...
	ИмяФайла = "";
	РезультатВызова = абс_БСПОценкаПроизводительностиКлиент.ВызватьДиалогРаботыСФайлами(
...
Процедура Настройка(Команда)
	...
	ОткрытьФорму("Обработка.абс_БСПОценкаПроизводительности.Форма.АвтоматическийЭкспортЗамеровПроизводительности", , ЭтаФорма);
...
Процедура УстановитьОтбор(Команда)
	...
	ОткрытьФорму("Обработка.абс_БСПОценкаПроизводительности.Форма.ФормаОтбора", , ЭтаФорма);
...
Функция РассчитатьЦелевоеВремя(КлючеваяОперация, ЗначениеAPDEX, ИндексПериода)
	...
	ПараметрыВычисления = ЭтаОбработка.СтруктураПараметровДляРасчётаАпдекса();
...
Процедура ОбработатьРеквизитыОбъекта(КолонкиТаблицаКлючевыхОпераций)
	...
	Если УдаляемыеРеквизиты.Количество() <> (КолонкиТаблицаКлючевыхОпераций.Количество() - КоличествоПредопределенныхКолонок) Тогда
		...
		УстановитьУсловноеОформлениеТЧ(ПоляОтбора, ПоляОформления, УсловноеОформление, Объект.Отображение);
...
//
&НаСервереБезКонтекста
Процедура УстановитьУсловноеОформлениеТЧ(ПоляОтбора, ПоляОформления, УсловноеОформление, Отображение);
	...
	ЭлементОтбора.ПравоеЗначение = Справочники.абс_БСПКлючевыеОперации.ОбщаяПроизводительностьСистемы;
	// Оформляемое поле
	...
	Если Отображение = "APDEX" Тогда
		
		// Оформление для показателей производительности
		Соответствие = СоответсвиеЦветовУровнюАпдекса();
...
Функция ПодготовитьПараметрыЭкспорта()
	...
	ОбщаяПроизводительность = ПредопределенноеЗначение("Справочник.абс_БСПКлючевыеОперации.ОбщаяПроизводительностьСистемы");
	...
	ПараметрыЭкспорта = Новый Структура("ДатаНачала, ДатаОкончания, Шаг, МассивКлючевыхОпераций");
...
Процедура Экспортировать(АдресВХранилище, ПараметрыЭкспорта)
	...
	Пока НачалоИнтервала < Конец Цикл
		...
		ЭкспортироватьИнтервал(ИмяВременногоФайла, НачалоИнтервала, КонецИнтервала, МассивКлючевыхОпераций);
	...
	ЗаполнитьОписание(ИмяФайлаОписания, Начало, Конец, МассивКлючевыхОпераций);
...
//
&НаСервереБезКонтекста
Процедура ЭкспортироватьИнтервал(ИмяВременногоФайла, Начало, Конец, МассивКлючевыхОпераций)
	
	НаборЗаписейЗамеры = НаборЗаписейЗамерыВремени(Начало, Конец, МассивКлючевыхОпераций);
...
//
&НаСервереБезКонтекста
Функция НаборЗаписейЗамерыВремени(Начало, Конец, МассивКлючевыхОпераций)
	
	МетаданныеРегистра = Метаданные.РегистрыСведений.абс_БСПЗамерыВремени;
...
//
&НаСервереБезКонтекста
Процедура ЗаполнитьОписание(ИмяФайлаОписания, Начало, Конец, МассивКлючевыхОпераций)
	...
	Для а = 0 По МассивКлючевыхОпераций.Количество() - 1 Цикл
		ЗаписьXML.ЗаписатьНачалоЭлемента("KeyOperation");
		ЗаписьXML.ЗаписатьАтрибут("name", Строка(МассивКлючевыхОпераций[а]));
...
Процедура ОткрытьФормуВыбора()
	...
	ОткрытьФорму("Справочник.абс_БСПКлючевыеОперации.ФормаВыбора", ПараметрыФормы, ЭтаФорма);
...
Функция ТаблицаКлючевыхОперацийДляВычисленияAPDEX()
	...
	ТаблицаКлючевыхОпераций.Колонки.Добавить(
		"КлючеваяОперация", 
		Новый ОписаниеТипов("СправочникСсылка.абс_БСПКлючевыеОперации"));
	ТаблицаКлючевыхОпераций.Колонки.Добавить(
		"Приоритет", 
...
//
&НаСервереБезКонтекста
Функция СоответсвиеЦветовУровнюАпдекса()
...
Функция НастройкаВыполнена()
	...
	Для Каждого СтрокаТЧ Из Объект.Производительность Цикл
		...
		Если СтрокаТЧ.ЦелевоеВремя = 0 
			И СтрокаТЧ.КлючеваяОперация <> Справочники.абс_БСПКлючевыеОперации.ОбщаяПроизводительностьСистемы 
		Тогда
...
//
&НаСервереБезКонтекста
Функция ЗагрузитьКлючевыеОперации()
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ОбщаяПроизводительностьСистемы", Справочники.абс_БСПКлючевыеОперации.ОбщаяПроизводительностьСистемы);
	
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	КлючевыеОперации.Ссылка КАК КлючеваяОперация,
	|	ВЫБОР
	|		КОГДА КлючевыеОперации.Ссылка = &ОбщаяПроизводительностьСистемы
	|			ТОГДА ""я""
	|		ИНАЧЕ КлючевыеОперации.Приоритет
	|	КОНЕЦ КАК Приоритет,
	|	КлючевыеОперации.ЦелевоеВремя КАК ЦелевоеВремя
	|ИЗ
	|	Справочник.абс_БСПКлючевыеОперации КАК КлючевыеОперации
	|
	|УПОРЯДОЧИТЬ ПО
	|	Приоритет
	|АВТОУПОРЯДОЧИВАНИЕ";
