...
Процедура ПодготовитьДанныеАвтоматическогоЗаполненияНаСервере()
	...
	Документы.УведомлениеОКонтролируемыхСделках.ПодготовитьДанныеАвтоматическогоЗаполнения(Объект.Уведомление,СписокОрганизаций);
...
Процедура ЗаполнитьУведомлениеНаСервере()
	...
	Документы.УведомлениеОКонтролируемыхСделках.СформироватьКонтролируемыеСделкиУведомления(Объект.Уведомление,СписокОрганизаций);
...
Процедура УправлениеФормой(Форма)
	...
	Форма.Элементы.ПереключитьАлгоритм.Доступность   						= ДоступностьГиперссылок;
...
Процедура СоздатьУведомление()
	...
	Если ЗначениеЗаполнено(Объект.Организация)
		...
		СписокФилиалов.Добавить(Объект.Организация);
		Документы.УведомлениеОКонтролируемыхСделках.ПодготовитьДанныеАвтоматическогоЗаполнения(НовоеУведомление.Ссылка,СписокФилиалов);
...
Функция ПолучитьПутьВыгрузки(ПутьДляВыгрузки = Неопределено) Экспорт
	...
	ПараметрыФормы.Вставить("Документ", Объект.Уведомление);	
...
Процедура ДекорацияКонтрагентыОфшорыНажатие(Элемент)
	...
	ОбщийСписокОрганизаций.Добавить(Объект.Организация);
	Для каждого Филиал  Из СписокОрганизаций Цикл
		ОбщийСписокОрганизаций.Добавить(Филиал.Значение);
	КонецЦикла;
	
	ПараметрыОткрытия = Новый Структура("Организация, ОтчетныйГод", ОбщийСписокОрганизаций, Объект.ОтчетныйГод);
...
Процедура ДекорацияСоставУведомленияНажатие(Элемент, СтандартнаяОбработка)
	...
	// {{ Киселева
	// убрала нерабочий код
	СтандартнаяОбработка = Ложь;
		
	ПараметрыФормы=Новый Структура; 
	ПараметрыФормы.Вставить("Организация", Объект.Организация);
	ПараметрыФормы.Вставить("ОтчетныйГод", Объект.ОтчетныйГод);
	// {{ТТК Сладков А. Заявка № 31.05.2016 начало
	
	ПараметрыФормы.Вставить("ТипУведомления", ЭтаФорма.ТипУведомления);

	ОткрытьФорму(ПолучитьПолноеИмяФормы("ФормаУведомления"), ПараметрыФормы, , Объект.Уведомление);
	// }}ТТК Сладков А. Заявка № 31.05.2016 окончание
	
	//ОткрытьФорму("Обработка.ПомощникПодготовкиУведомленияОКонтролируемыхСделках.Форма.ФормаУведомления", ПараметрыФормы, , Объект.Уведомление);
	
	// кусок кода из типовой УПП
	//СтандартнаяОбработка = Ложь;
	//	
	//ПараметрыФормы=Новый Структура; 
	//ПараметрыФормы.Вставить("Отбор", Новый Структура("УведомлениеОКонтролируемойСделке", Объект.Уведомление));
...
Процедура КомандаПечатьУведомленияОКонтролируемыхСделках(Команда)
	...
	ПараметрыПечати.Вставить("СохранитьВФайл",Ложь);
	ПараметрыПечати.Вставить("ПутьВКаталог","");
...
&НаСервере
Функция ПолучитьЗначениеАлгоритма()
	НовыйАлгоритм = глЗначениеПеременной("абс_ТЦОЗаполнениеПоНовомуАлгоритму");
	Возврат НовыйАлгоритм;	
КонецФункции 

&НаСервере
Процедура УстановитьЗначениеАлгоритма()
	НовыйАлгоритм = ПолучитьЗначениеАлгоритма();
	Константы.абс_ТЦОЗаполнениеПоНовомуАлгоритму.Установить(Не НовыйАлгоритм);
КонецПроцедуры

&НаКлиенте
Процедура ПереключитьАлгоритмНажатие(Элемент)
	УстановитьЗначениеАлгоритма();
	УстановитьИмяАлгоритма();
КонецПроцедуры

&НаКлиенте
Процедура УстановитьИмяАлгоритма()
	НовыйАлгоритм = ПолучитьЗначениеАлгоритма();
	Если  НовыйАлгоритм Тогда
		Элементы.ПереключитьАлгоритм.Заголовок = "Переключить на старый алгоритм";
	Иначе
		Элементы.ПереключитьАлгоритм.Заголовок = "Переключить на новый алгоритм";
	КонецЕсли;
КонецПроцедуры
