			...
			Для каждого СтрокаДокумента Из ВыборкаДокументов Цикл
				...
				Иначе
					...
					Если ПроводитьДокументыПоОперативнымРегистрам Тогда
						ДокументОбработан = ПровестиНеПлатежныйДокументПоРегистрамНДС(СтрокаДокумента.Ссылка);
					...
					Если ПроводитьДокументыПоРегистрамПартий И СтрокаДокумента.СписаниеПартий Тогда
						
						ТаблицаДокументовСписанияПартий[0].Регистратор = СтрокаДокумента.Ссылка;
						УправлениеЗапасамиПартионныйУчетФормированиеПроводокРегл.ДвижениеНДС(ТаблицаДокументовСписанияПартий, 
							УправлениеЗапасами.ИспользуетсяРасширеннаяАналитикаУчета(СтрокаДокумента.Дата));
					КонецЕсли;
...
Функция ПолучитьТаблицуДокументов(ДатаНачала, СсылкаНач, ДатаКонца, БылПоследнийДокумент, МассивОрганизаций)
	...
	ТекстЗапроса = 
		...
		|		ПО Источник.Организация = УчетнаяПолитика.Организация
		...
		|				ГДЕ
		|					Источник.Период >= УчетнаяПолитика.Период
		|					И Источник.Организация = УчетнаяПолитика.Организация))
		|ГДЕ
		|	Источник.Период >= &ДатаПоследнего
		|	И (ЕСТЬNULL(УчетнаяПолитика.СложныйУчетНДС, ЛОЖЬ)
	...
	Если ПроводитьДокументыПоОперативнымРегистрам Или ОтразитьСуммовыеИКурсовыеРазницыВРегистрахНДС Тогда
		...
		Для каждого ВидДокументов Из ДополнительныеДокументы Цикл
			...
			Если ВидДокументов.Ключ = "СчетФактураВыданный" Тогда
				ТекстЗапроса = ТекстЗапроса+" И Источник.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.НДСВидСчетаФактуры.НаАванс) И Не Источник.СформированПриВводеНачальныхОстатковНДС";
			ИначеЕсли ВидДокументов.Ключ = "РеализацияТоваровУслуг" Тогда
				ТекстЗапроса = ТекстЗапроса+" И (Источник.ВидОперации <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРеализацияТоваров.ОтгрузкаБезПереходаПраваСобственности) ИЛИ Источник.Организация В (&МассивОрганизацийНачислениеНДСПоОтгрузке))";
...
Функция СформироватьТаблицуОплаты(ДокументСсылка, ТаблицаОплат, ТаблицаСуммовыхРазниц, РасчетыПоПриобретению = Истина)
	...
	|	ВЫБОР
	|		КОГДА ДанныеРегСведений.ДоговорКонтрагента.ВестиПоДокументамРасчетовСКонтрагентом
	|			ТОГДА ДанныеРегСведений.РублеваяСумма
	...
	|					ИНАЧЕ ДанныеРегНакопления.РублеваяСумма
	|				КОНЕЦ
...
//
Процедура ОчиститьДвиженияРегистровНДС(ДокументОбъект, ДокументСсылка, ТолькоОперативныеДвижения = Ложь)
		 ...
		 Тогда
			...
			Если НаборЗаписей.Количество() > 0 Тогда
				Если ТолькоОперативныеДвижения И НаличиеРеквизитаСписаниеПартийВРегистре[ТипЗнч(Движение)] Тогда
					...
					Пока Инд > 0 Цикл
						Инд = Инд - 1;
						Удалять = Не НаборЗаписей[Инд].СписаниеПартий;
						Если Удалять Тогда
...
//
Функция ПровестиНеПлатежныйДокументПоРегистрамНДС(ДокументВыборка)
	
	Перем Заголовок, СтруктураШапкиДокумента, СтруктураШапкиДокументаВалюта, СтруктураШапкиДокументаРубли;
	Перем ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоОборудованию, ТаблицаПоОбъектамСтроительства;
	...
	ОчиститьДвиженияРегистровНДС(ДокументОбъект, ДокументВыборка, ТипЗнч(ДокументВыборка) = Тип("ДокументСсылка.ОтчетПроизводстваЗаСмену"));
	...
	ИначеЕсли ТипЗнч(ДокументВыборка) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах") Тогда
		...
		ДокументОбъект.ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоУслугам, ТаблицаПоТоварам, Неопределено);
		
		ДокументОбъект.ДвиженияРегистровПодсистемыНДСПоступление(СтруктураШапкиДокумента, ТаблицаПоУслугам, Отказ);
	...
	ИначеЕсли ТипЗнч(ДокументВыборка) = Тип("ДокументСсылка.ОтчетПроизводстваЗаСмену") Тогда
	...
	ИначеЕсли ТипЗнч(ДокументВыборка) = Тип("ДокументСсылка.РеализацияУслугПоПереработке") Тогда
		...
		ВыручкаПоТоварам = ОбщегоНазначения.СформироватьТаблицуЗначений(ТаблицаПоТоварам,СтруктураТаблицыВыручки, , Истина);
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ВыручкаПоТоварам, ТаблицаВыручки);
	...
	ИначеЕсли ТипЗнч(ДокументВыборка) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
		...
		Если Не ДокументВыборка.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаАванс Тогда
			Возврат Ложь;
		КонецЕсли;
		...
		УчетНДСФормированиеДвижений.СформироватьДвиженияНДС_СчетФактураВыданный(СтруктураШапкиДокумента, ТаблицаПоУслугам);
...
Процедура ПровестиПлатежныйДокументПоРегистрамНДС(ДокументСсылка, ОчищатьДвижения = Истина)
	...
	Заголовок = ttk_ОбщегоНазначения.ПредставлениеДокументаПриПроведении(ДокументСсылка);
	...
	Иначе
		...
		Если НаличиеДвиженийКРегистрации Тогда
			Если не ТаблицаСуммовыхРазниц = неопределено 
				...
				и не ЭтоВозврат
				Тогда
					УчетНДСФормированиеДвижений.КорректировкаРегистровНДСПоСуммовымРазницам_Приобретение(СтруктураШапкиДокумента, ТаблицаСуммовыхРазниц);
			КонецЕсли; 
		КонецЕсли; 
	КонецЕсли;
...
Процедура ОтразитьКорректировкиПоРасчетамВУЕпоРегистрамНДС(ДокументСсылка, ТаблицаДокумента, Колонки)
	...
	Если Покупки Тогда
		Если СуммоваяРазница <> 0 Тогда
			УчетНДСФормированиеДвижений.КорректировкаРегистровНДСПоСуммовымРазницам_Приобретение(СтруктураПараметров, СуммоваяРазницаПоДокументам, Истина);
		КонецЕсли;
	Иначе
		Если СуммоваяРазница <> 0 Тогда
			УчетНДСФормированиеДвижений.КорректировкаРегистровНДСПоСуммовымРазницам_Реализация(СтруктураПараметров, СуммоваяРазницаПоДокументам, Истина);
		КонецЕсли;
	КонецЕсли;
