...
#Если Клиент Тогда
	...
	Функция СоздатьКонтрагента(СтрокаКонтрагент = Неопределено, ОткрыватьФорму=Истина) Экспорт //или СтрокаКонтрагент - строка, или ТаблицаКонтрагентов - ТЗ
		...
		Если ТипЗнч(СтрокаКонтрагент.Реквизит)<>Тип("СправочникСсылка.Контрагенты") Тогда
			...
			//АБС
			НовыйЭлемент.абс_СтатусКонтрагента = Перечисления.абсСтатусыКонтрагентов.Подготовка;
			НовыйЭлемент.абс_Ответственный = ГлЗначениеПеременной("ГлТекущийПользователь");
			...
			Если НовыйЭлемент.абс_СтатусКонтрагента = Перечисления.абсСтатусыКонтрагентов.Подготовка Тогда
				БПСогласование = НайтиБПСогласование(НовыйЭлемент.Ссылка);
				
				Если Не ЗначениеЗаполнено(БПСогласование) Тогда
					
					абс_БизнесПроцессы.ЗапуститьБПСогласованияКонтрагента(НовыйЭлемент.Ссылка);
			...
			НаборЗаписей.Отбор.Контрагент.Установить(НовыйЭлемент.Ссылка);
			НаборЗаписей.Прочитать();
			
			Запись = НаборЗаписей.Добавить();
			Запись.Период = ТекущаяДата();
			
			Запись.Контрагент			= НовыйЭлемент.Ссылка;
			Запись.Пользователь 		= глЗначениеПеременной("глТекущийПользователь");	
			Запись.СтатусКонтрагента	= Перечисления.абсСтатусыКонтрагентов.Подготовка;
		...
		Для каждого Строка из СтрокаКонтрагент.Строки Цикл
			...
			Если Строка.Представление = "Договор" и  Строка.Пометка Тогда
				...
				Попытка
					...
					НайденныйДоговорОбъект.абс_Ответственный =  ГлЗначениеПеременной("ГлТекущийПользователь");
					
					//АБС
					НайденныйДоговорОбъект.Записать();
					
					Если  НайденныйДоговорОбъект.абс_СтатусДоговора = Перечисления.абсСтатусыДоговоров.Подготовка Тогда
						
						БПСогласование = НайтиБПСогласованиеДоговоров(НайденныйДоговорОбъект.Ссылка);
						
						Если Не ЗначениеЗаполнено(БПСогласование) Тогда
							абс_БизнесПроцессы.ЗапуститьБПСогласованияДоговора(НайденныйДоговорОбъект.Ссылка);	
						КонецЕсли;	
						НаборЗаписей = РегистрыСведений.абс_ИзменениеСтатусовДоговоров.СоздатьНаборЗаписей();
						
						НаборЗаписей.Отбор.Договор.Установить(НайденныйДоговорОбъект.Ссылка);
						НаборЗаписей.Прочитать();
						
						Запись = НаборЗаписей.Добавить();
						Запись.Период = ТекущаяДата();
						
						Запись.Договор				= НайденныйДоговорОбъект.Ссылка;
						Запись.Исполнитель	 		= глЗначениеПеременной("глТекущийПользователь");	
						Запись.СтатусДоговора		= Перечисления.абсСтатусыДоговоров.Подготовка;
						НаборЗаписей.Записать();
	...
	Функция ПолучитьНомерВФорматеОбмена(НомерДокумента, Знач ПрефиксОрганизации)
		...
		ttk_ОбщегоНазначения.ДобавитьПрефиксУзла(ПрефиксОрганизации);
		...
		//АБС ВСТАВКА 36866  25.12.2013 14:40:52  Стрельцов
		//удаление ведущих букв
		абс_БуквеннаяСтрока = "q,w,e,r,t,y,u,i,o,p,a,s,d,f,g,h,j,k,l,z,x,c,v,b,n,m
							  |й,ц,у,к,е,н,г,ш,щ,з,х,ъ,ф,ы,в,а,п,р,о,л,д,ж,э,я,ч,с,м,и,т,ь,б,ю";
		Пока Найти(абс_БуквеннаяСтрока, Нрег(Лев(Номер, 1))) > 0 Цикл
			Номер = Сред(Номер, 2);
		КонецЦикла;						 
		//\\АБС ВСТАВКА 36866 КОНЕЦ
	...
	Функция ПолучитьТекстЗапросаПоПлатежноеТребованиеВыставленное()
		...
		|	"""" КАК СрокДействия,
		|	Выбор Когда ПлатежноеТребованиеВыставленное.УсловиеОплаты = Значение(Перечисление.УсловияОплатыРасчетныхДокументов.БезАкцепта) Тогда ""Без акцепта, ""+ПОДСТРОКА(ПлатежноеТребованиеВыставленное.ОснованиеДляБезакцептногоСписания, 1, 200) Иначе ""С акцептом"" Конец КАК УсловиеОплаты1,
...
КонецПроцедуры
	...
	Процедура Выгрузить(ДокументыНаЭкспорт, ВерсияФормата = "1.02") Экспорт
		...
		//Проверка документа заявка 1445
		Для Каждого СтрокаТЧ Из ДокументыНаЭкспорт.Значение Цикл
			Если  ЗначениеЗаполнено(СтрокаТЧ.Документ) Тогда
				Если Не СтрокаТЧ.Документ.Проведен И  СтрокаТЧ.Выгружать Тогда
					Предупреждение("Документ "+СтрокаТЧ.Документ+" не проведен!!!
					|Выгрузка невозможна.");
					Возврат;
				КонецЕсли;
				//АБС Коломиец 000019988+
				Если СтрокаТЧ.Документ.Статус = Перечисления.абсСтатусыПлатежей.Отправлено И СтрокаТЧ.Выгружать Тогда
					Предупреждение("Документ "+СтрокаТЧ.Документ+" находится в статусе Отправлено!!!
					|Повторная выгрузка запрещена.");
					Возврат;
				КонецЕсли;
				//АБС Коломиец 000019988-
			КонецЕсли;
		...
		Для каждого СтрокаСекции из ДокументыНаЭкспорт.Значение Цикл
			...
			КонецЕсли;
			//АБС Коломиец 000019988+
			Если СтрокаСекции.Документ.ВыгруженаВКБ Тогда
				ТекстВопроса = Строка(СтрокаСекции.Документ) + " ранее был выгружен в Банк-клиент. Выгрузить повторно?";
				Ответ = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет, 0);
				Если Ответ = КодВозвратаДиалога.Нет Тогда
					СтрокаСекции.Выгружать = Ложь;
					Продолжить;	 
				КонецЕсли;
			КонецЕсли;		
			//АБС Коломиец 000019988-				
		...
		//Установка статуса "Выгружен в банк" выгрузки в Платежных поручениях исходящих
		ЕстьОшибки = Ложь;
		НачатьТранзакцию();

			Для Каждого ДокументПлатеж Из ДокументыНаЭкспорт.Значение Цикл
				Если	ДокументПлатеж.Выгружать Тогда
					Если ТипЗнч(ДокументПлатеж.Документ.Ссылка) = Тип("ДокументСсылка.ПлатежноеПоручениеИсходящее") Тогда
						ОбъектПлатеж = ДокументПлатеж.Документ.Ссылка.ПолучитьОбъект();
						ОбъектПлатеж.ВыгруженаВКБ = Истина;
						ОбъектПлатеж.Статус = Перечисления.абсСтатусыПлатежей.Отправлено;
						
						ОбъектПлатеж.Записать();
						Для Каждого СтрокаРасшифровки Из ОбъектПлатеж.РасшифровкаПлатежа Цикл
							Если ЗначениеЗаполнено(СтрокаРасшифровки.ДокументПланированияПлатежа) Тогда
								Для Каждого СтрокаОплат Из СтрокаРасшифровки.ДокументПланированияПлатежа.Оплаты Цикл
									Если ЗначениеЗаполнено(СтрокаОплат.СчетНаОплату) Тогда
										ОбъектСчет = СтрокаОплат.СчетНаОплату.ПолучитьОбъект();
										СтрокаОплаты = ОбъектСчет.Оплаты.Найти(СтрокаОплат.Оплата,"Оплата");
										Если СтрокаОплаты<>Неопределено Тогда
											СтрокаОплаты.СтатусОплаты = ПЕречисления.абсСтатусыСчетов.ОтправленВБанк;
			...
			Если ЕстьОшибки Тогда
				ОтменитьТранзакцию();
				Сообщить("Данные требуется выгрузить повторно");
	...
	Процедура РаспознатьДанныеВСтрокеДокумента(СтрокаДокумента)
		...
		Если ВсеРеквизитыПоискаЕсть Тогда
			...
			Пока Результат.Следующий() Цикл
				...
				Если ВыборкаНомер = НомерДляПоискаДок И (Не ЗначениеЗаполнено(Результат.НомерСчета) ИЛИ Результат.НомерСчета = СчетДляПоискаДок) Тогда
			...
			ИначеЕсли Лев(Врег(СокрЛП(Стр)), 10)="КОНЕЦФАЙЛА" Тогда
				...
				Для каждого СтрокаДокумента Из ДокументыДляИмпорта Цикл
					...
					Если СтрокаДокумента.ПлательщикСчет = БанковскийСчет.НомерСчета
						...
						//АБС
						Состояние("Обрабатывается "+НомерСтроки+" из "+ДокументыДляИмпорта.Количество());
						//АБС
	...
	Процедура ЗаполнитьРеквизитПлатежногоПорученияИсходящего(ОбъектДокумента, ИсходныеДанные)
		...
		Если ЗначениеЗаполнено(ИсходныеДанные.СтатусСоставителя) Тогда
			...
			ИначеЕсли ИсходныеДанные.СтатусСоставителя = "06" 
				ИЛИ ((ИсходныеДанные.СтатусСоставителя >= "16")и(ИсходныеДанные.СтатусСоставителя <= "20")) Тогда
		...
		Для каждого СтрокаСекции из ДокументыКИмпорту Цикл
			Если СтрокаСекции.Загружать Тогда
				...
				Если ПустаяСтрока(РезультатПроверки) Тогда
					...
					УстановитьСвойство(ОбъектДокумента, "ОтражатьВНалоговомУчете", ОтражатьВНалоговомУчете);
					УстановитьСвойство(ОбъектДокумента, "ОтражатьВБухгалтерскомУчете", ОтражатьВБухгалтерскомУчете);
					...
					Если НайденнаяСтрока <> неопределено И ОбъектДокумента.РеквизитыКонтрагента.Количество()=0 Тогда
						...
						ВидыДоговоров = УправлениеДенежнымиСредствами.ОпределитьВидДоговораСКонтрагентом(СтрокаСекции.ВидОперации);
						Если ВидыДоговоров.Количество()<>0 Тогда
							ВидДоговора = Строка(ВидыДоговоров[0]);
							Для каждого Строка из НайденнаяСтрока.Строки Цикл
	...
	КонецПроцедуры
  	...
  	Функция ПроверитьЗаполнениеНалоговыхРеквизитов_НеИсп(СтрокаЭкспорта) //теперь процедура
	...
	//АБС Вспомогательные функции
	
	// Функции поиска бизнесс процессов по ссылке на контрагента.
	//  Используются для 
	// 		1. отображения ссылки на БП в форме 
	//		2. поиска задач
	Функция НайтиБПСогласование(Контрагент)
		Возврат абс_БизнесПроцессы.НайтиБизнесПроцессПоКонтрагенту(Контрагент, "абсСогласованиеКонтрагентов");
	КонецФункции
	
	// Функции поиска бизнесс процессов по ссылке 
	Функция НайтиБПСогласованиеДоговоров(Договор)
		
		Возврат абс_БизнесПроцессы.НайтиБизнесПроцессПоДоговору(Договор.Ссылка, "абсСогласованиеДоговоров");
	...
	//АБС
	//
	Процедура АБС_РаспознованиеПлатежа(ТаблицаДок) Экспорт
		
		Для Каждого СтрокаТаблицы Из ТаблицаДок Цикл
			
			СтрокаТаблицы.НомерДоговора = 	РазобратьСтрокуНазначения(СтрокаТаблицы.НазначениеПлатежа).НомерДоговора;
			СтрокаТаблицы.НомерСчета 	= 	РазобратьСтрокуНазначения(СтрокаТаблицы.НазначениеПлатежа).НомерСчета;
	...
	Функция РазобратьСтрокуНазначения(НазначениеПлатежаПолное)
		...
		СтруктураВозврата = Новый Структура("НомерДоговора,НомерСчета");
		...
		//СтрокаПоиска1 "по счету N";
		//СтрокаПоиска2 "по счету №";
		//ПозицияСч1 = Найти(НазначениеПлатежа,СтрокаПоиска1);
		//Если ПозицияСч1>0 Тогда
		//	ПозицияПробела = Найти(Сред(НазначениеПлатежа,ПозицияСч1+10)," ");
		//	Если ПозицияПробела>0  Тогда
		//   	 
		//	КонецЕсли;
		//	
	...
	Процедура АБС_ОпределениеДоговора(ТаблицаДоговоров,ТаблицаДок) Экспорт
		
		Если ТаблицаДоговоров.Количество()>0 Тогда	
			Для Каждого СтрокаТаблицы Из ТаблицаДок Цикл
				ОбработкаПрерыванияПользователя();
				Если ЗначениеЗаполнено(СтрокаТаблицы.Контрагент) И ТипЗнч(СтрокаТаблицы.Контрагент) = Тип("СправочникСсылка.Контрагенты")   Тогда
					СтрокаТаблицы.Договор = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
					СтруктураОтбора = Новый Структура("Контрагент");
					СтруктураОтбора.Вставить("Контрагент",СтрокаТаблицы.Контрагент);
					МассивСтрок = ТаблицаДоговоров.НайтиСтроки(СтруктураОтбора);
					Если МассивСтрок.Количество()>0 Тогда
						
						Для Каждого СтрокаДоговора Из МассивСтрок Цикл
							Назначение = СтрокаТаблицы.НазначениеПлатежа;
							Если Найти(Назначение,СокрЛП(СтрокаДоговора.НомерДоговора))>0 Тогда
								СтрокаТаблицы.Договор = СтрокаДоговора.Договор;
								Прервать;
							КонецЕсли;	
						КонецЦикла;	
						
						Если ЗначениеЗаполнено(СтрокаТаблицы.НомерДоговора) Тогда 
							Для Каждого СтрокаДоговора Из МассивСтрок Цикл
								Если СокрЛП(СтрокаДоговора.НомерДоговора) = СокрЛП(СтрокаТаблицы.НомерДоговора) Тогда
									СтрокаТаблицы.Договор = СтрокаДоговора.Договор;
									Прервать;
								КонецЕсли;	
							КонецЦикла;	
							
						ИначеЕсли ЗначениеЗаполнено(СтрокаТаблицы.НомерСчета) Тогда
							//Поиск по номеру счета номер и сам договор среди договоров этого контрагента
							Для Каждого СтрокаДоговора Из МассивСтрок Цикл
								Если Найти(СокрЛП(СтрокаТаблицы.НомерСчета),СокрЛП(СтрокаДоговора.НомерДоговора))>0 Тогда
									СтрокаТаблицы.Договор = СтрокаДоговора.Договор;
									Прервать;
								КонецЕсли;	
							КонецЦикла;	
						...
						Иначе 
							Для Каждого СтрокаДоговора Из МассивСтрок Цикл
								Назначение = Лев(СтрокаТаблицы.НазначениеПлатежа,150);
								Если Найти(Назначение,СокрЛП(СтрокаДоговора.НомерДоговора))>0 Тогда
									СтрокаТаблицы.Договор = СтрокаДоговора.Договор;
									Прервать;
								КонецЕсли;	
	...
	Функция АБС_СформироватьТаблицуКонтрагентовДоговоров(ТаблицаДок) Экспорт
		...
		Если МассивКонтрагентов.Количество()>0 Тогда
			...
			|	Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
			|ГДЕ
			|	ДоговорыКонтрагентов.Владелец В(&МассивКонтрагентов)";
