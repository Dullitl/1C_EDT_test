...
Процедура ЗаписатьРегистрируемыеОбъекты(НаборЗаписей, СтрокаОбъекта)
	...
	Для Каждого Стр Из СтрокаОбъекта.Строки Цикл
		...
		Если Стр.СодержитЭлементы И Стр.Использование <> 1 И НЕ (Стр.ЭтоОбъект И Стр.Использование = 0) Тогда
			ЗаписатьРегистрируемыеОбъекты(НаборЗаписей, Стр);
		КонецЕсли;
...
Процедура ЗаписатьНастройки()
	...
	Если РегистрируемыеОбъекты.ПолучитьЭлементы().Количество() <> 0 Тогда
		...
		ЗаписатьРегистрируемыеОбъекты(НаборЗаписей, Дерево);
...
Процедура ОбновитьФлагиРегистрируемыхОбъектов()
	...
	ТЗНеРегистрируемые = внЖурналРегистрации.ПолучитьНеРегистрируемыеОбъекты();
	НачальнаяПометкаКоллекций(ТЗНеРегистрируемые, Дерево);
...
//
&НаСервере
Процедура НачальнаяПометкаКоллекций(ТЗНеРегистрируемые, ТекСтрока)
	...
	Для каждого Строки Из ТекСтрока.Строки Цикл
		...
		НайденныеСтроки = ТЗНеРегистрируемые.НайтиСтроки(ПараметрыОтбора);				
		...
		Если Строки.Строки.Количество() > 0 Тогда
			НачальнаяПометкаКоллекций(ТЗНеРегистрируемые, Строки)
		КонецЕсли;
