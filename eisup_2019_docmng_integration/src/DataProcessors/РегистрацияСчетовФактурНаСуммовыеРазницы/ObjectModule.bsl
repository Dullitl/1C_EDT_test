...
Процедура ЗаполнитьСтрокиДокумента(ТаблицаРезультатов) Экспорт
	...
	Если ТаблицаРезультатов.Количество()>0 Тогда
		Запрос.Текст = 
			...
			|	И СчетФактураВыданный.Ссылка.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.НДСВидСчетаФактуры.НаСуммовуюРазницу)";
		
		Запрос.УстановитьПараметр("ДокументыОснования", ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(ТаблицаРезультатов.ВыгрузитьКолонку("ДокументОснование"), Истина));
...
Процедура СформироватьСчетаФактурыНаСуммовуюРазницу(НеиспользуемыеСчетаФактуры) Экспорт
	...
	Для каждого СтрокаТП Из Список Цикл
		...
		Исключение
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не выписан счет-фактура на суммовую разницу по строке "+Строка(СтрокаТП.НомерСтроки));
