...
Функция ПечатьПеречисленияЗарплаты(СсылкаНаДокумент) Экспорт
	...
	Для каждого СтрокаПлатежа ИЗ ОбъектДокумента.ПеречислениеЗаработнойПлаты Цикл
		...
		Обл.Параметры.Сумма = ВыборкаФизЛиц.Сумма + ВыборкаФизЛиц.КомпенсацияЗаЗадержкуЗарплаты;
...
Функция ДобавитьОткрытиеСчета(ФизЛицо, ВидВклада, СтрокаЗаявки) Экспорт
	...
	Если ЛицевойСчет <> Неопределено Тогда
		ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки("Формирование данных для экспорта:", "Для " + ФизЛицо.Наименование + " нельзя открыть лицевой счет - он уже открыт!");
		Возврат Ложь;
	КонецЕсли;
	...
	СтрокаТЧ.РабочийТелефон = ПолучитьТелефонСтрокой(ФизЛицо, "ТелефонСлужебный");
...
Функция ДобавитьЗачислениеЗарплаты(ФизЛицо, Сумма, ЛицевойСчет = Неопределено) Экспорт
	...
	Если ЛицевойСчет = Неопределено Тогда
		ЛицевойСчет = ПолучитьЛицевойСчет(ФизЛицо, Организация, Банк);
		Если ЛицевойСчет = Неопределено Тогда
			ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки("Формирование данных для экспорта:", "Не найден лицевой счет для " + ФизЛицо.Наименование);
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция ДобавитьОткрытиеСчетаСотрудника(Ветка, СтруктураОткрытиеСчетов) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(СтруктураОткрытиеСчетов.Сотрудник) Тогда 
		ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки("Формирование данных на открытие счетов", "Не указано физ лицо для открытия счета");
		Возврат Неопределено;
	...
	Если ФорматФайлаобмена = ФорматФайла3 Тогда
		Если ЗначениеЗаполнено(СтруктураОткрытиеСчетов.Должность) Тогда
			ДОбавитьЭлемент(УзелОС, "Должность", СтруктураОткрытиеСчетов.Должность);
		Иначе
			...
			ЗапросДолжность.УстановитьПараметр("ДатаСреза", ДатаФормирования);
			ЗапросДолжность.УстановитьПараметр("ФизЛицо", СтруктураОткрытиеСчетов.Сотрудник);
			ВыборкаДолжности = ЗапросДолжность.Выполнить().Выбрать();
			// Должность сотрудника
			Если ВыборкаДолжности.Следующий() Тогда
				ДобавитьЭлемент(УзелОС, "Должность", ВыборкаДолжности.Должность);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	// Место рождения
	ДобавитьМестоРожденияВДеревоЭкспорта(УзелОС, ДанныеФизЛица.МестоРождения);
	
	// Адрес прописки
	Если НЕ ПустаяСтрока(СтрЗаменить(СтруктураОткрытиеСчетов.АдресПрописки, Символы.ПС, "")) Тогда
		АдресПрописки = СтруктураОткрытиеСчетов.АдресПрописки;
	...
	// Адрес проживания
	Если НЕ ПустаяСтрока(СтрЗаменить(СтруктураОткрытиеСчетов.АдресПроживания, Символы.ПС, "")) Тогда
		АдресПроживания = СтруктураОткрытиеСчетов.АдресПроживания;
	...
	// Рабочий телефон
	Если НЕ ПустаяСтрока(СтруктураОткрытиеСчетов.РабочийТелефон) Тогда
		ДобавитьЭлемент(УзелОС, "РабочийТелефон", Лев(СокрЛП(СтруктураОткрытиеСчетов.РабочийТелефон),60));
	КонецЕсли;
	
	// Домашний телефон
	Если НЕ ПустаяСтрока(СтруктураОткрытиеСчетов.ДомашнийТелефон) Тогда
		ДобавитьЭлемент(УзелОС, "ДомашнийТелефон", Лев(СокрЛП(СтруктураОткрытиеСчетов.ДомашнийТелефон), 30));
	КонецЕсли;
	...
	Если ФорматФайлаобмена = ФорматФайла3 Тогда
		...
		Если ЗначениеЗаполнено(СтруктураОткрытиеСчетов.ПризнакЗарплатный) Тогда
			ДобавитьЭлемент(УзелОС, "ПризнакЗарплатный", СтруктураОткрытиеСчетов.ПризнакЗарплатный);
		КонецЕсли;
		
		ЗапросГражданство = Новый Запрос;
		ЗапросГражданство.Текст = "ВЫБРАТЬ
		...
		ЗапросГражданство.УстановитьПараметр("ФизЛицо", СтруктураОткрытиеСчетов.Сотрудник);
		ЗапросГражданство.УстановитьПараметр("ДатаСреза", ДатаФормирования);
		Выборка = ЗапросГражданство.Выполнить().Выбрать();
		// Резидент
		Если Выборка.Следующий() Тогда
			
			ДобавитьЭлемент(УзелОС, "Резидент", ?(Выборка.Резидент, "true", "false"));
			
			Гражданство = Выборка.Страна;
		Иначе
			Гражданство = "";
			ДобавитьЭлемент(УзелОС, "Резидент",  "true");
		КонецЕсли;
		
			
		// Гражданство
		Если ЗначениеЗаполнено(Гражданство) Тогда
			ДобавитьЭлемент(УзелОС, "Гражданство", Гражданство);
		ИначеЕсли ЗначениеЗаполнено(СтруктураОткрытиеСчетов.Гражданство) Тогда
			ДобавитьЭлемент(УзелОС, "Гражданство", СтруктураОткрытиеСчетов.Гражданство);
		КонецЕсли;
		
		// Категория населения
		ДобавитьЭлемент(УзелОС, "КатегорияНаселения", СтруктураОткрытиеСчетов.КатегорияНаселения);
		
		// Бонус участника
		Если ЗначениеЗаполнено(СтруктураОткрытиеСчетов.БонусУчастника) Тогда
			ДобавитьЭлемент(УзелОС, "БонусУчастника", СтруктураОткрытиеСчетов.БонусУчастника);
		КонецЕсли;
		
		// Тариф следующий год
		Если ЗначениеЗаполнено(СтруктураОткрытиеСчетов.ТарифСледующийГод) Тогда
			ДобавитьЭлемент(УзелОС, "ТарифСледующийГод", СтруктураОткрытиеСчетов.ТарифСледующийГод);
		КонецЕсли;
		
		// Тариф текущий год
		Если ЗначениеЗаполнено(СтруктураОткрытиеСчетов.ТарифТекущийГод) Тогда
			ДобавитьЭлемент(УзелОС, "ТарифТекущийГод", СтруктураОткрытиеСчетов.ТарифТекущийГод);
		КонецЕсли;
		
		// Бонус программа
		Если ЗначениеЗаполнено(СтруктураОткрытиеСчетов.БонусПрограмма) Тогда
			ДобавитьЭлемент(УзелОС, "БонусПрограмма", СтруктураОткрытиеСчетов.БонусПрограмма);
		КонецЕсли;
		
		// Признак рассылки
		Если ЗначениеЗаполнено(СтруктураОткрытиеСчетов.ПризнакРассылки) Тогда
			ДобавитьЭлемент(УзелОС, "ПризнакРассылки", СтруктураОткрытиеСчетов.ПризнакРассылки);
		КонецЕсли;
		
		// Интернет адрес
		Если ЗначениеЗаполнено(СтруктураОткрытиеСчетов.ИнтернетАдрес) Тогда
			ДобавитьЭлемент(УзелОС, "ИнтернетАдрес", СтруктураОткрытиеСчетов.ИнтернетАдрес);
		КонецЕсли;
		
		// Счет дебета
		Если ЗначениеЗаполнено(СтруктураОткрытиеСчетов.СчетДебета) Тогда
			ДобавитьЭлемент(УзелОС, "СчетДебета", СтруктураОткрытиеСчетов.СчетДебета);
		КонецЕсли;
		
		// Мобильный телефон
		Если ЗначениеЗаполнено(СтруктураОткрытиеСчетов.МобильныйТелефон) Тогда
			ДобавитьЭлемент(УзелОС, "МобильныйТелефон", СтруктураОткрытиеСчетов.МобильныйТелефон);
		КонецЕсли;
		
		// Оператор Связи
		Если ЗначениеЗаполнено(СтруктураОткрытиеСчетов.ОператорСвязи) Тогда
			ДобавитьЭлемент(УзелОС, "ОператорСвязи", СтруктураОткрытиеСчетов.ОператорСвязи);
		КонецЕсли;
		
		// Мобильный банк
		Если ЗначениеЗаполнено(СтруктураОткрытиеСчетов.МобильныйБанк) Тогда
			ДобавитьЭлемент(УзелОС, "МобильныйБанк", СтруктураОткрытиеСчетов.МобильныйБанк);
		КонецЕсли;
		
		// Передача БКИ
		ДобавитьЭлемент(УзелОС, "ПередачаБКИ", ?(СтруктураОткрытиеСчетов.ПередачаБКИ, "true", "false"));
	КонецЕсли;
		
	// КонтрольнаяИнформация
	Если ФорматФайлаобмена = ФорматФайла3 Тогда
		ДобавитьЭлемент(УзелОС, "КонтрольнаяИнформация", СтруктураОткрытиеСчетов.КонтрольнаяИнформация);
	КонецЕсли;
КонецФункции
...
Функция ДобавитьЗачислениеЗарплатыСотрудника(Ветка, СтруктураЗачислениеЗарплаты) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(СтруктураЗачислениеЗарплаты.Сотрудник) Тогда 
		ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки("Формирование данных на перечисление ЗП", "Не указано физ лицо для перечисления ЗП");
		Возврат Неопределено;
...
Функция СоздатьНовыйПотокXMLСчетаПК() Экспорт
	...
	Если ПустаяСтрока(НомерДоговора) Тогда
		ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки(СтадияОбработки, "Не указан номер договора для работы с банком!");
		Возврат Неопределено;
	КонецЕсли;
	
	Если ПустаяСтрока(НаименованиеОрганизации) Тогда
		ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки(СтадияОбработки, ОбщегоНазначения.ПреобразоватьСтрокуИнтерфейса("Не указано наименование организации!"));
		Возврат Неопределено;
	КонецЕсли;
	
	Если ПустаяСтрока(ИНН) Тогда
		ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки(СтадияОбработки, ОбщегоНазначения.ПреобразоватьСтрокуИнтерфейса("Не указан ИНН организации!"));
		Возврат Неопределено;
	КонецЕсли;
	
	Если ПустаяСтрока(РасчетныйСчетОрганизации) Тогда
		ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки(СтадияОбработки, "В документе " + Документ + "не указан зарплатный счет или указан зарплатный счет с незаполненным номером счета!");
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПолучитьДеревоИзФайла(ФайлДляЧтения, Сообщать = Истина) Экспорт
	...
	Если ТекстXML = Неопределено Тогда
		Если Сообщать Тогда
			ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки(СтадияОбработки, "В файле отсутствуют данные!");
		КонецЕсли;
		Возврат Неопределено;
	КонецЕсли;
	...
	Исключение
		Если Сообщать Тогда
			ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки(СтадияОбработки, "В файле нарушена структура XML или это не XML файл!");
		КонецЕсли;
		Возврат Неопределено;
	КонецПопытки;
...
Функция ПолучитьСчетаПК(ДеревоОбработки, ФайлДляЧтения, Сообщать = Истина) Экспорт
	...
	Если ДеревоОбработки.Строки.Количество() = 0 Тогда
		Если Сообщать Тогда
			ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки(СтадияОбработки, "Отсутствуют элементы!");
		КонецЕсли;
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ДеревоОбработки.Строки.Количество() > 1 Тогда
		Если Сообщать Тогда
			ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки(СтадияОбработки, "Более одного элемента верхнего уровня!");
		КонецЕсли;
		Возврат Неопределено;
	КонецЕсли;
	...
	Если СчетаПК.Имя <> "СчетаПК" Тогда
		Если Сообщать Тогда
			ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки(СтадияОбработки, "Некорректное имя элемента верхнего уровня!");
		КонецЕсли;
		Возврат Неопределено;
	КонецЕсли;
...
//	ШаблонЭлементаВерхнегоУровня.Вставить("РасчетныйСчетОрганизации", Неопределено);
	...
	Для каждого ЭлементШаблона Из ШаблонЭлементаВерхнегоУровня Цикл
		Если ЭлементШаблона.Значение = Неопределено Тогда
			Если Сообщать Тогда
				ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки(СтадияОбработки, "Некорректная структура элемента верхнего уровня!");
			КонецЕсли;
			Возврат Неопределено;
		КонецЕсли;
	КонецЦикла;
			...
			Для каждого СтрокаВетки из Ветка.Строки Цикл
				Если ИмяЭлементаИмпорта = "РезультатОткрытияСчетов" ИЛИ ИмяЭлементаИмпорта = "РезультатЗачисленияЗарплаты" Тогда
					...
					Для каждого ЭлементШаблона Из ШаблонОперации Цикл
						Если ЭлементШаблона.Значение = Неопределено Тогда
							ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки("Анализ операции", "Некорректная структура элемента " + ИмяЭлементаИмпорта + "!");
							Продолжить;
						КонецЕсли;
					КонецЦикла;
