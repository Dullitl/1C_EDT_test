	...
	Для каждого КонечныйПлан из КонечныеПланы Цикл
		...
		Для каждого Документ из ТаблицаДокументов Цикл
			...
			Если мДокумент = Неопределено Тогда
				...
				ЗаполняемыйДокумент.ВалютаДокумента = мВалюта;
			Иначе
...
Процедура ДобавитьСтрокуВТабличнуюЧасть(ТабличнаяЧасть, ДобавляемаяСтрока, КонечныйПлан, ИндексСтроки, Период = '00010101')
	...
	Если КонечныйПлан.Сценарий.ДетализацияПланирования = Перечисления.ДетализацияПланирования.НоменклатурныеГруппы И ТипЗнч(ДобавляемаяСтрока.Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
		...
		Если ДобавляемаяСтрока.Номенклатура.НоменклатурнаяГруппа.Пустая() Тогда
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Для номенклатуры """ + ДобавляемаяСтрока.Номенклатура.Наименование + """ не определена номенклатурная группа.");
			Возврат;
...
Функция КорректностьЗаполненияДанных() Экспорт
	...
	Если КонечныеПланы.Количество() > 0 Тогда
		...
		Для каждого КонечныйПлан из КонечныеПланы Цикл
			...
			Если СтрДлина(Текст) > 0 Тогда
				...
				ttk_ОбщегоНазначения.СообщитьОбОшибке(Текст,, "Конечные планы: Строка " + КонечныйПлан.НомерСтроки + ".");
	...
	Иначе
		...
		ДанныеКорректны = Ложь;
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не заполнены.",, "Конечные планы.");
	...
	Для каждого СтратегияРасчета из СтратегииРасчета Цикл
		...
		Если СтрДлина(Текст) > 0 Тогда
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке(Текст,, "Стратегии расчета количества: " + СтратегияРасчета.НаименованиеСтратегии + ".");
...
Функция КорректностьЗаполненияДокумента() Экспорт
	...
	Если СтрДлина(Текст) > 0 Тогда
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке(Текст,, "В документе: " + Строка(мДокумент));
