...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Для Каждого Запись Из ЭтотОбъект Цикл
		...
		Если Запись.Подразделение.Пустая() Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru = 'Не заполнено значение реквизита ""Подразделение""'"), Отказ, ЗаголовокСообщения);
		КонецЕсли;
			
		Если Запись.ГраницаСмены = '0001-01-01' Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru = 'Не выбрана смена'"), Отказ, ЗаголовокСообщения);
		КонецЕсли;
