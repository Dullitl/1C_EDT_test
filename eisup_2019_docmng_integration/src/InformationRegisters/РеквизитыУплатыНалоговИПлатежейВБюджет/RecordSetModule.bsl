...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Для каждого Запись Из ЭтотОбъект Цикл
		Если НЕ ЗначениеЗаполнено(Запись.ВидПлатежа) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не заполнен вид платежа", Отказ, Заголовок);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Запись.ВидПеречисленияВБюджет) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не заполнен вид перечисления", Отказ, Заголовок);
		КонецЕсли;
	КонецЦикла;
