...
Процедура СформироватьОтчетПоНаличиюСчетовФактур(ДокументРезультат, ТаблицаРезультат) Экспорт
	...
	ЗаголовокОтчета.Параметры.НаименованиеОрганизации = СведенияОбОрганизации.ПолноеНаименование;
	...
	Для Каждого СтрокаРезультат Из ТаблицаРезультат Цикл
		...
		//АБС ВСТАВКА 56921
		Если ЗначениеЗаполнено(СтрокаРезультат.ДокументОснование) Тогда
			Строка.Параметры.Контрагент = СтрокаРезультат.ДокументОснование.Контрагент;
			Строка.Параметры.Договор = СтрокаРезультат.ДокументОснование.ДоговорКонтрагента;
			Строка.Параметры.Ответственный = СтрокаРезультат.ДокументОснование.Ответственный;
		КонецЕсли;
		//\\АБС ВСТАВКА 56921 КОНЕЦ
