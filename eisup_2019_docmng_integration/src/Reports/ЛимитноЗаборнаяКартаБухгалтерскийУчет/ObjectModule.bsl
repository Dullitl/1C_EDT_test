...
Процедура СформироватьЛимитноЗаборнуюКарту(ДокументРезультат, ОбходПоГруппировке)
	...
	Пока Обход.Следующий() Цикл
		...
		Если Не ШапкаВыведена Тогда
			...
			ОбластьМакетаШапка.Параметры.ТекстЦена = "Цена, руб.коп.";
			ОбластьМакетаШапка.Параметры.ТекстСумма = "Сумма без учета НДС, руб.коп.";
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ДатаКон) Тогда
		ДатаКонца = ОбщегоНазначенияЗК.ПолучитьРабочуюДату();
	Иначе
