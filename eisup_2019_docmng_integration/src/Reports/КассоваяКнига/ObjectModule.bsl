...
Процедура СформироватьОтчет(ДокументРезультат) Экспорт
	...
	//АБС ИЗМЕНЕНИЕ 66555
	//Изменения, которые внесены по задаче №66555 временные. После того, как будет 
	//обновлена платформа на релизе выше 8.2.13.202 данные изменения необходимо убрать.
	//\\АБС ИЗМЕНЕНИЕ 66555 КОНЕЦ
	...
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.ОперационнаяКасса);
	...
	//АБС ИЗМЕНЕНИЕ 66555
	Итог.УстановитьПараметр("ПустаяСсылка",Справочники.Валюты.ПустаяСсылка());
	//\\АБС ИЗМЕНЕНИЕ 66555 КОНЕЦ
	...
	|	ВЫБОР
    |		КОГДА КассовыеДокументы.Документ.ВалютаДокумента ЕСТЬ NULL 
	|				ИЛИ КассовыеДокументы.Документ.ВалютаДокумента = НЕОПРЕДЕЛЕНО
	|			ТОГДА &ПустаяСсылка
	|		ИНАЧЕ КассовыеДокументы.Документ.ВалютаДокумента
	|	КОНЕЦ КАК Валюта,
	//\\АБС ИЗМЕНЕНИЕ 66555 КОНЕЦ
	...
	//АБС ИЗМЕНЕНИЕ 66555
	Запрос.УстановитьПараметр("ПустаяСсылка",Справочники.Валюты.ПустаяСсылка());
	//\\АБС ИЗМЕНЕНИЕ 66555 КОНЕЦ
