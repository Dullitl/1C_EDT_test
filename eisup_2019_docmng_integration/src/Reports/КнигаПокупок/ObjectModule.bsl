...
Процедура СформироватьКнигуПокупок(Организация,НачалоПериода, КонецПериода, ЭлементыФормы) Экспорт
	...
	УчетнаяПолитикаНУ = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(КонецПериода, Организация);
	...
	Для Каждого ИтогПоПериодам ИЗ ДеревоЗаписей.Строки Цикл;
		...
		Для Каждого ИтогПоПериодамКорректировки ИЗ ИтогПоПериодам.Строки Цикл;
			...
			Иначе
				...
				ИтогЗаПериод.Вставить("ВсегоПокупок", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
...
Функция ОпределитьДатуИНомерСФ(ЗаписьКниги, Секция = Неопределено)
	...
	Если ДатаНомер = ", №" Или ДатаНомер = "" Тогда
		...
		Если (ЗаписьКниги.НаАванс
			И ЗначениеЗаполнено(ЗаписьКниги.ДоговорАванса)) Тогда
			Запрос.УстановитьПараметр("ДоговорКонтрагента",ЗаписьКниги.ДоговорАванса);
			УсловиеПоДоговору = " ДоговорКонтрагента = &ДоговорКонтрагента ";
		КонецЕсли;
		Запрос.УстановитьПараметр("ДокументОснование", ЗаписьКниги.СчетФактураДокумент); 
		
		ЗапросТекст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
		               |	ВложенныйЗапрос.Номер,
		               |	ВложенныйЗапрос.Дата,
		               |	ВложенныйЗапрос.Ссылка
		               |ИЗ
		               |	(ВЫБРАТЬ ПЕРВЫЕ 1
		               |		СчетФактураПолученныйДокументыОснования.Ссылка.НомерВходящегоДокумента КАК Номер,
		               |		СчетФактураПолученныйДокументыОснования.Ссылка.ДатаВходящегоДокумента КАК Дата,
		               |		СчетФактураПолученныйДокументыОснования.Ссылка КАК Ссылка,
		               |		1 КАК Приоритет
		               |	ИЗ
		               |		РегистрНакопления.НДСЗаписиКнигиПокупок.Обороты(, , Регистратор, "+УсловиеПоДоговору+") КАК НДСЗаписиКнигиПокупокОбороты
		               |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураПолученный.ДокументыОснования КАК СчетФактураПолученныйДокументыОснования
		               |			ПО НДСЗаписиКнигиПокупокОбороты.СчетФактура = СчетФактураПолученныйДокументыОснования.ДокументОснование
		               |				И НДСЗаписиКнигиПокупокОбороты.ДоговорКонтрагента = СчетФактураПолученныйДокументыОснования.Ссылка.ДоговорКонтрагента
		               |	ГДЕ
		               |		НДСЗаписиКнигиПокупокОбороты.СчетФактура = &ДокументОснование
		               |	
		               |	ОБЪЕДИНИТЬ ВСЕ
		               |	
		               |	ВЫБРАТЬ ПЕРВЫЕ 1
		               ...
		               |			ПО НДСЗаписиКнигиПокупокОбороты.СчетФактура = СчетФактураВыданныйДокументыОснования.ДокументОснование
		               |				И НДСЗаписиКнигиПокупокОбороты.ДоговорКонтрагента = СчетФактураВыданныйДокументыОснования.Ссылка.ДоговорКонтрагента
		               |	ГДЕ
		               |		НДСЗаписиКнигиПокупокОбороты.СчетФактура = &ДокументОснование
		               |	
		               |	ОБЪЕДИНИТЬ ВСЕ
		               ...
		               |		НДСЗаписиКнигиПокупокОбороты.СчетФактура.ДатаВходящегоДокумента,
		               |		НДСЗаписиКнигиПокупокОбороты.СчетФактура.Ссылка,
		               |		2
		               |	ИЗ
		               |		РегистрНакопления.НДСЗаписиКнигиПокупок.Обороты(, , Регистратор, "+УсловиеПоДоговору+") КАК НДСЗаписиКнигиПокупокОбороты
		               |	ГДЕ
		               |		НДСЗаписиКнигиПокупокОбороты.СчетФактура = &ДокументОснование) КАК ВложенныйЗапрос
		               |
		               |УПОРЯДОЧИТЬ ПО
...
Функция ПолучитьЗаписиКнигиПокупок(Организация, НачалоПериода, КонецПериода, СписокСчетовФактур)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		ПО (ВЫБОР
	               |				КОГДА ЗаписиКнигиПокупок.ВидЦенности В (&ВидыЦенностей_НалоговыйАгент)
	               |						И (НЕ ЗаписиКнигиПокупок.ДоговорАванса = НЕОПРЕДЕЛЕНО)
	               |					ТОГДА ЗаписиКнигиПокупок.ДокументОплаты = Приоритеты.СчетФактура
	               |				ИНАЧЕ ЗаписиКнигиПокупок.СчетФактура = Приоритеты.СчетФактура
	               |			КОНЕЦ)
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	               |			ТаблицаСчетаФактурыДокументы.СчетФактура КАК СчетФактура,
	               |			ТаблицаСчетаФактурыДокументы.Приоритет КАК Приоритет,
	               |			МАКСИМУМ(ТаблицаСчетаФактурыДокументы.ДоговорАванса) КАК ДоговорАванса,
	               |			МАКСИМУМ(ТаблицаСчетаФактурыДокументы.СтавкаНДСАванса) КАК СтавкаНДСАванса,
	               |			МАКСИМУМ(ТаблицаСчетаФактурыДокументы.СчетФактураДокумент) КАК СчетФактураДокумент,
	               |			МАКСИМУМ(ТаблицаСчетаФактурыДокументы.НомерСчетаФактуры) КАК НомерСчетаФактуры,
	               |			МАКСИМУМ(ТаблицаСчетаФактурыДокументы.ДатаСчетаФактуры) КАК ДатаСчетаФактуры,
	               |			ТаблицаСчетаФактурыДокументы.УдалятьЛидирующиеНули КАК УдалятьЛидирующиеНули
	               |		ИЗ
	               |			ТаблицаСчетаФактурыДокументы КАК ТаблицаСчетаФактурыДокументы
	               |		
	               |		СГРУППИРОВАТЬ ПО
	               ...
	               |			ТаблицаСчетаФактурыДокументы.Приоритет) КАК ТаблицаСчетаФактурыДокументы
...
Функция ПолучитьДополнительнуюИнформациюПоСФ(Организация, КонецПериода, СписокСчетовФактур, ЗаписьДополнительногоЛиста = Ложь)
	...
	Для Каждого СтрокаИнформации Из ДополнительнаяИнформацияПоСФ.Строки Цикл
		...
		КонецЕсли; 
	КонецЦикла; 
	ДополнительнаяИнформацияПоСФ.Колонки.Добавить("ГТДиСтрана", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(0)));
	
	// Дополенние по ГТД и стране происхождения
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ТаблицаТоваров.Ссылка КАК СчетФактура,
	|	ТаблицаТоваров.СерияНоменклатуры.СтранаПроисхождения КАК СтранаПроисхождения,
