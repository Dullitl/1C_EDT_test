...
Процедура ВыводКорСубконто(Знач ВыборкаОбороты, ПравилоКорСчета, Знач Инд, СтруктураПараметров, ОтборДляРасшифровки)
	...
	Пока ВыборкаОбороты.Следующий() Цикл
		...
		ВыборкаОборотыУровень = ВыборкаОбороты.Уровень();
		
		Если ВыборкаОборотыУровень > 7 Тогда
			ВыборкаОборотыУровень = 7;
		КонецЕсли;
		// Вывод оборота                       
		...
		СтруктураПараметров.ДокументРезультат.Вывести(ВыводимаяОбласть, ВыборкаОборотыУровень);
