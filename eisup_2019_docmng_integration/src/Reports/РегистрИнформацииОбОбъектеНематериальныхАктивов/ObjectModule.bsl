...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Организация) или НЕ ЗначениеЗаполнено(НематериальныйАктив) тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Для формирования регистра информации необходимо указать организацию и объект НМА.");
		Возврат;
	КонецЕсли;	
	...
	Если Состояние.Количество()=0 тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Объект """+НематериальныйАктив+""" не был отражен в налоговом учете в организации """+Организация+""".");
		Возврат;
	КонецЕсли;
