...
// {{KM WARE Лазаревский К.В. Заявка №000024624 04.08.2015 начало
// По просьбе Саакьянц Марина Сергеевна - дополнены поля
	Если мНазваниеОтчета = "Льготы по НДС" Тогда
		ПолеДокРегистратор = ПостроительОтчета.ВыбранныеПоля.Добавить("Регистратор", "Регистратор");
		ПолеДокРегистратор.Представление = "Регистратор"; 
		ПолеДокРегистратор = ПостроительОтчета.ВыбранныеПоля.Добавить("Регистратор.абс_ПериодОтражения", "Регистраторабс_ПериодОтражения");
		ПолеДокРегистратор.Представление = "Период отражения НДС"; 
	КонецЕсли;
// }}KM WARE Лазаревский К.В. Заявка №000024624 04.08.2015 окончание	
