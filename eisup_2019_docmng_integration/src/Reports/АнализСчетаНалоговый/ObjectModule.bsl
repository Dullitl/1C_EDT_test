...
Функция СформироватьЗаголовок() Экспорт
	...
	Если НЕ ПоСубсчетамИСубконто И ПостроительОтчета.ИзмеренияСтроки.Количество()=0 И Не ЗначениеЗаполнено(Период) Тогда
	...
	СтрОтбор = УправлениеОтчетами.СформироватьСтрокуОтборов(ПостроительОтчета.Отбор);
   	СтрОтбор = СтрОтбор + ?(Не ПустаяСтрока(СтрОтбор), ", ", "") + ?(ЗначениеЗаполнено(ВидУчета), "Вид учета = "+ВидУчета, "");
...
Процедура ЗаполнитьПараметрыРасшифровки(Область, Выборка, ОтборСубконто = Неопределено)
	...
	ПараметрыКарточкиСчета.Вставить("ВидУчета", ВидУчета);
	...
	ИначеЕсли Выборка.Группировка() = "КорСчет" Тогда
		...
		Если ЗначениеЗаполнено(ВидУчета) Тогда
			ПараметрыРасшифровкиДт["Отбор"].Вставить("ВидУчетаДт", ВидУчета);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ВидУчета) Тогда
			ПараметрыРасшифровкиДт["Отбор"].Вставить("ВидУчетаКт", ВидУчета);
		КонецЕсли;
...
Процедура СформироватьОтчет(ДокументРезультат,  ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	Если ЗначениеЗаполнено(ВидУчета) Тогда
		ТекстОтбор = ТекстОтбор + " И ВидУчета = &ВидУчета";
	КонецЕсли;
	...
	Если абс_ВключаяПодчиненныеОрганизации Тогда
		...
		ЗапросОбороты.Текст = СтрЗаменить(ЗапросОбороты.Текст, " = &Организация", " В ИЕРАРХИИ (&Организация)");
		ЗапросОстатки.Текст = СтрЗаменить(ЗапросОстатки.Текст, " = &Организация", " В ИЕРАРХИИ (&Организация)");
...
Процедура ОбработкаРасшифровкиСтандартногоОтчета(Расшифровка) Экспорт
	...
	Если СпособРасшифровки="Отчет" Тогда
		...
		Если Расшифровка.Отбор["Период"] <> Неопределено Тогда
			Расшифровка.ДатаНач = Расшифровка.Отбор["Период"];
			Расшифровка.ДатаКон = БухгалтерскиеОтчеты.ПолучитьДатуОкончанияПериода(Расшифровка.ДатаНач, Расшифровка.Период);
		КонецЕсли;
