			...
			Для каждого СтрокаФизЛицо из СтрокаПериодаРегистрации.Строки Цикл
				...
				Пока Сч <= МаксКолвоСтрок Цикл
					...
					Если Сч <= КолВоСтрокНачислений тогда
						...
						СтрокаНачисленияУдержания.ГоловнаяОрганизация = ?(Начисления.Строки[Сч].Организация = NULL или Начисления.Строки[Сч].Организация.ГоловнаяОрганизация.Пустая(), Начисления.Строки[Сч].Организация, Начисления.Строки[Сч].Организация.ГоловнаяОрганизация);
						...
						Если ВидРасчетногоЛистка = "Подробно" И (ТипЗнч(Начисления.Строки[Сч].ВидРасчета) = Тип("ПланВидовРасчетаСсылка.ОсновныеНачисленияОрганизаций") 
							...
							СведенияОВидеРасчета = РаботаСДиалогамиПереопределяемый.ПолучитьСведенияОВидеРасчетаСхемыМотивации(СведенияОВидахРасчета, ВидРасчета, "ПриРасчете");
							ПервыйРаз = истина;
					...
					Если Сч <= КолВоСтрокУдержаний тогда
						...
						СтрокаНачисленияУдержания.ГоловнаяОрганизация             = ?(Удержания.Строки[Сч].Организация = NULL или Удержания.Строки[Сч].Организация.ГоловнаяОрганизация.Пустая(), Удержания.Строки[Сч].Организация, Удержания.Строки[Сч].Организация.ГоловнаяОрганизация);
				...
				Если МаксКолвоСтрок = -1 И Макс(КолВоСтрокДоходыВНатуральнойФорме, КолВоСтрокВыплачено) = -1 тогда
					...
					СтрокаНачисленияУдержания.ГоловнаяОрганизация = ?(СтрокаФизЛицо.Организация = NULL или СтрокаФизЛицо.Организация.ГоловнаяОрганизация.Пустая() , СтрокаФизЛицо.Организация, СтрокаФизЛицо.Организация.ГоловнаяОрганизация);
				...
				Для Сч = 0 по МаксКолвоСтрок Цикл
					...
					Если Сч <= КолВоСтрокДоходыВНатуральнойФорме тогда
						...
						СтрокаНачисленияУдержания.ГоловнаяОрганизация = ?(ДоходыВНатуральнойФорме.Строки[Сч].Организация = NULL или ДоходыВНатуральнойФорме.Строки[Сч].Организация.ГоловнаяОрганизация.Пустая(), ДоходыВНатуральнойФорме.Строки[Сч].Организация, ДоходыВНатуральнойФорме.Строки[Сч].Организация.ГоловнаяОрганизация);
					...
					Если Сч <= КолВоСтрокВыплачено тогда
						...
						СтрокаНачисленияУдержания.ГоловнаяОрганизация             = ?(Выплачено.Строки[Сч].Организация = NULL или Выплачено.Строки[Сч].Организация.ГоловнаяОрганизация.Пустая(), Выплачено.Строки[Сч].Организация, Выплачено.Строки[Сч].Организация.ГоловнаяОрганизация);
...
Процедура НастроитьПостроительЗапросов(ПостроительОтчетов, ВидРасчетногоЛистка) Экспорт
	...
	Если ВидРасчетногоЛистка = "Кратко" тогда
		...
		|				ТОГДА ""А""
		|			ИНАЧЕ ""Б""
		|		КОНЕЦ + ОсновныеНачисленияРаботниковОрганизации.ВидРасчета.КатегорияРасчета.Код + ОсновныеНачисленияРаботниковОрганизации.ВидРасчета.Код КАК ПорядокВидаРасчета,
		...
		|				ТОГДА ""А""
		|			ИНАЧЕ ""Б""
		|		КОНЕЦ + ОсновныеНачисленияРаботниковОрганизации.ВидРасчета.КатегорияРасчета.Код + ОсновныеНачисленияРаботниковОрганизации.ВидРасчета.Код,
	...
	Иначе
		...
		|	НачисленияУдержания.Организация.ГоловнаяОрганизация,
		|	НачисленияУдержания.ДатаВедомости
		|ИЗ
		|	(ВЫБРАТЬ
		|		НачисленияУдержания.ВидДвижения КАК ВидДвижения,
		|		НачисленияУдержания.ФизЛицо КАК ФизЛицо,
		...
		|		НачисленияУдержания.СальдоНачальноеНДФЛ КАК СальдоНачальноеНДФЛ,
		|		НачисленияУдержания.СальдоКонечноеНДФЛ КАК СальдоКонечноеНДФЛ,
		|		НачисленияУдержания.ДатаВедомости КАК ДатаВедомости
		|	ИЗ
		|		(ВЫБРАТЬ
		|			ВЫБОР
		|				КОГДА ОсновныеНачисленияРаботниковОрганизации.ВидРасчета.ЯвляетсяДоходомВНатуральнойФорме
		...
		|			Сальдо.Физлицо,
		|			Периоды.ПериодРегистрации,
		|			Сальдо.Организация.ГоловнаяОрганизация) КАК НачисленияУдержания) КАК НачисленияУдержания
		|{ГДЕ
		|	НачисленияУдержания.ФизЛицо,
	...
	ТекстПостроителя = УправлениеОтчетами.СформироватьСписокМесяцев(ТекстПостроителя, ДатаНач, ДатаКон);
