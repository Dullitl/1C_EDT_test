			...
			Пока ИндексПоВерсиям >= 1 Цикл
				...
				Если НайденнаяСтрока <> Неопределено Тогда
					...
					Для Каждого Колонка Из ТекущаяТЧВерсии.Колонки Цикл
						...
						Если СсылкаНаОбъект.Метаданные().ТабличныеЧасти.Найти(ИмяТабличнойЧасти) <> Неопределено Тогда 
						...
						Иначе
							Представление = Значение;//ОпределитьПредставление(Значение, Колонка.Имя, Колонка.Имя,ТипЗнч(Колонка.ТипЗначения.ПривестиЗначение()));
						КонецЕсли;
...
Функция РассчитатьИзменения(НомерВерсии,
	...
	Для каждого Строка Из РезультатРазбораВерсии_1.ДвиженияДокумента Цикл
		ТабличныеЧасти_1.Вставить(Строка.ИмяТаблицы,Строка.Таблица);	
	КонецЦикла;
	///////////////////////////////////////////////////////////////////////////////
...
Функция РазборВерсии(НомерВерсии, Ссылка)
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ АвторВерсии, ДатаВерсии, ВерсияОбъекта,ДвиженияДокумента 
	...
	Иначе
		...
		Текст.ДобавитьСтроку(Выборка.ДвиженияДокумента);
		Текст.Записать(ИмяВременногоФайла);
		ТЗ = ЗначениеИзФайла(ИмяВременногоФайла); 
		УдалитьФайлы(ИмяВременногоФайла);
		...
		Результат.Вставить("ДвиженияДокумента", ТЗ);
		Возврат Результат;
	КонецЕсли;
...
Функция СчитатьНачальныеЗначенияРеквизитовИТабличныхЧастей(ТаблицаРеквизитов,
	...
	Для каждого Строка Из ВерсияОбъекта.ДвиженияДокумента Цикл
		ТабличныеЧасти.Вставить(Строка.ИмяТаблицы,Строка.Таблица);	
	КонецЦикла;	
...
//
Функция ОпределитьПредставление(ЗначениеРеквизита, НаименованиеПоля, ОписаниеРеквизита,Тип = Неопределено)
...
// Основная экспортная функция модуля, отвечающая за восстановление объектов.
//
Процедура ВосстановитьВерсиюОбъекта(ВерсияОбъекта) Экспорт
	
	ЭлементОбъект = СсылкаНаОбъект.ПолучитьОбъект();
	
	ОписаниеВерсии    = ПолучитьОписаниеПоВерсии(ВерсияОбъекта);
	ДвиженияДокумента = ПолучитьДвиженияДокумента(ВерсияОбъекта);

	Объект = ПолучитьОбъектИзXML(ПолучитьСтрокуXMLПоНомеруВерсии(ВерсияОбъекта));
	
	ВерсияОбъекта = РазборВерсии(ВерсияОбъекта, СсылкаНаОбъект);
	...
	Для Каждого ЭлементРеквизит Из ВерсияОбъекта.Реквизиты Цикл
		...
		Если НаименованиеРеквизита = "Ссылка" Или ОписаниеРеквизита = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ЭлементОбъект[ЭлементРеквизит.НаименованиеРеквизита] = Объект[ЭлементРеквизит.НаименованиеРеквизита];
		
	КонецЦикла;
	
	
	//Восстановим табличные части
	Для Каждого ТабличнаяЧасть Из ВерсияОбъекта.ТабличныеЧасти Цикл
		
		ТЧОбъекта			= ЭлементОбъект[ТабличнаяЧасть.Ключ];
		ТЧВерсииОбъекта		= Объект[ТабличнаяЧасть.Ключ]; 
		
		ТЧОбъекта.Очистить();
		
		Для Каждого СтрТЧВерсииОбъекта Из ТЧВерсииОбъекта Цикл
			
			СтрТЧОбъекта = ТЧОбъекта.Добавить();
			ЗаполнитьЗначенияСвойств(СтрТЧОбъекта, СтрТЧВерсииОбъекта);
			
		КонецЦикла;
		
	КонецЦикла;
	
	////Восстановим движения
	Для каждого Строка Из ДвиженияДокумента Цикл
		ЭлементОбъект.Движения[Строка.ИмяТаблицы].Загрузить(Строка.Таблица);
	    ЭлементОбъект.Движения[Строка.ИмяТаблицы].Записать(Истина);
	КонецЦикла;	
		
	ЭлементОбъект.Записать();

	
КонецПроцедуры //ВосстановитьВерсиюОбъекта

Функция ПолучитьДвиженияДокумента(НомерВерсии) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ ДвиженияДокумента
	                |ИЗ РегистрСведений.ВерсииОбъектов
	                |ГДЕ Объект = &Ссылка
	                |И НомерВерсии = &НомВер";
	Запрос.УстановитьПараметр("Ссылка", СсылкаНаОбъект);
	...
	НеТЗ = ТЗ.СкопироватьКолонки();
	Для каждого Строка Из ТЗ Цикл
		Если Строка.Таблица.Количество()>0 Тогда
			ЗаполнитьЗначенияСвойств(НеТЗ.добавить(),Строка);	
		КонецЕсли;		
	КонецЦикла;	
...
Процедура СохранитьТекущиюКопию() Экспорт
	
	ОбъектДок = СсылкаНаОбъект.ПолучитьОбъект();
	
	ЧислоВерсийОбъекта = ВерсионированиеОбъектовПривилегированный.ПолучитьЧислоВерсийОбъекта(СсылкаНаОбъект);
	...
	ЗаписатьXML(ЗаписьXML, ОбъектДок, НазначениеТипаXML.Явное);
	ЗаписьXML.Закрыть();
	...
	Для Каждого ТекРегистрДвижений Из ОбъектДок.Метаданные().Движения Цикл
		ОбъектДок.Движения[ТекРегистрДвижений.Имя].Прочитать();
		Строка = ДвиженияДокумента.Добавить();
		Строка.ИмяТаблицы = ТекРегистрДвижений.Имя;
		Строка.Таблица = ОбъектДок.Движения[ТекРегистрДвижений.Имя].Выгрузить();	
	КонецЦикла;
	ДвиженияДокумента = ВерсионированиеОбъектов.абс_ПреобразоватьТаблицуВСторку(ДвиженияДокумента);
                            	
	ВерсионированиеОбъектовПривилегированный.ЗаписатьВерсиюОбъекта(СсылкаНаОбъект, ЧислоВерсийОбъекта, ХранилищеДанных,ДвиженияДокумента);
