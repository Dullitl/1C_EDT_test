...
Функция ПроверитьНаименованияПоказателей()
	...
	Если МассивДублей.Количество() <> 0 Тогда
		ЕстьОшибки = Истина;
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Обнаружены ошибки в структуре отчета.");
	КонецЕсли; 
	Для каждого ЭлКоллекции Из МассивДублей Цикл
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Показатель с именем """ + ЭлКоллекции + """ уже существует.",,, СтатусСообщения.Информация);
	КонецЦикла; 
...
Функция ПроверитьНастройки(ЗаголовокДействия)
	...
	Если Показатели.Строки.НайтиСтроки(Новый Структура("ЭтоГруппа", Ложь), Истина).Количество() = 0 Тогда
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстОшибки,ЕстьОшибки, ЗаголовокДействия);
	КонецЕсли; 
