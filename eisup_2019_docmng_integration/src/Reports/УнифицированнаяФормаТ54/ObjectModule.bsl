...
Перем ДанныеОФизЛицах, ТаблицаКадрыОтпускаУдержания, Отпуска, Удержания, РезультатНачиселений;
...
Процедура СформироватьМакет(ДокументРезультат, Работник, Уровень) Экспорт
	...
	ВыборкаПоФизЛицам = РезультатНачиселений.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
...
Функция СформироватьОтчет(Результат = Неопределено, ДанныеРасшифровки = Неопределено, ВыводВФормуОтчета = Истина) Экспорт
	...
	|		КОГДА РаботникиОрганизацииСрезПоследних.ПериодЗавершения <= &ДатаАктуальности
	|				И РаботникиОрганизацииСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|			ТОГДА ВЫРАЗИТЬ(РаботникиОрганизацииСрезПоследних.ОбособленноеПодразделениеЗавершения.НаименованиеПолное КАК СТРОКА(300))
	|		ИНАЧЕ ВЫРАЗИТЬ(РаботникиОрганизацииСрезПоследних.ОбособленноеПодразделение.НаименованиеПолное КАК СТРОКА(300))
...
//	Запрос.Текст =
	...
	РезультатНачиселений = Запрос.Выполнить();
