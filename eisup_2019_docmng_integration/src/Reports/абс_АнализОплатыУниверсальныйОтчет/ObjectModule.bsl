...
Процедура УстановитьНачальныеНастройки(ДополнительныеПараметры = Неопределено) Экспорт
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	РеализацияТоваровУслуг.абс_СчетНаОплатуПокупателю КАК СчетНаОплату,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыПоРеализацииВУсловныхЕдиницахОрганизации.Обороты(, , Регистратор, ) КАК РасчетыПоРеализацииВУсловныхЕдиницахОрганизацииОбороты
	|		ПО (РасчетыПоРеализацииВУсловныхЕдиницахОрганизацииОбороты.Регистратор = РеализацияТоваровУслуг.Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетНаОплатуПокупателю КАК СчетНаОплатуПокупателю
	|		ПО РеализацияТоваровУслуг.абс_СчетНаОплатуПокупателю = СчетНаОплатуПокупателю.Ссылка
	|ГДЕ
	|	(НЕ СчетНаОплатуПокупателю.Ссылка ЕСТЬ NULL )
	|	И СчетНаОплатуПокупателю.Дата МЕЖДУ &ДатаНачала И &ДатаКонца
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			Документ В
	|				(ВЫБРАТЬ
	|					ВТРеализации.Документ
	|				ИЗ
	|					ВТРеализации)) КАК РасчетыПоРеализацииВУсловныхЕдиницахОрганизацииОбороты
	|ГДЕ
	|	РасчетыПоРеализацииВУсловныхЕдиницахОрганизацииОбороты.СуммаВзаиморасчетовПриход > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА (НЕ ВТОплаты.Документ ЕСТЬ NULL )
	|			ТОГДА ВЫБОР
	|					КОГДА ЕСТЬNULL(ВТОплаты.СуммаОплаты, 0) >= ЕСТЬNULL(ВТРеализации.СуммаДокумента, 0)
	|						ТОГДА ВТОплаты.ДатаОплаты
	|					ИНАЧЕ ""Не оплачен""
	|				КОНЕЦ
	|		ИНАЧЕ ""Не оплачен""
	|	КОНЕЦ КАК Статус
	...
	|			ПО ВТРеализации.Документ = ВТОплаты.Документ
	|		ПО (ВТРеализации.СчетНаОплату = СчетНаОплатуПокупателю.Ссылка)
	|ГДЕ
	|	СчетНаОплатуПокупателю.Дата МЕЖДУ &ДатаНачала И &ДатаКонца
	|{ГДЕ
	...
	//УниверсальныйОтчет.ДобавитьПоказатель("Сумма", "Сумма (руб.)", Истина, "", "","");
	...
	//
	УниверсальныйОтчет.ДобавитьПоказатель("Статус", "Статус", Истина, "Статус");
	...
	// Необходимо вызывать для каждой добавляемой группировки колонки.
	 //УниверсальныйОтчет.ДобавитьИзмерениеКолонки("Статус");
