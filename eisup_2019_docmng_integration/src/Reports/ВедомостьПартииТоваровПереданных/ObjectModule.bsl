...
Процедура УстановитьНачальныеНастройкиУпр(ДополнительныеПараметры = Неопределено)
	...
	|	ИсточникДанных.абс_ЦФО КАК ЦФО,
	|	ПРЕДСТАВЛЕНИЕ(ИсточникДанных.абс_ЦФО) КАК ЦФОПредставление,
	|	ИсточникДанных.абс_НазначениеПлатежа КАК НазначениеПлатежа,
	|	ПРЕДСТАВЛЕНИЕ(ИсточникДанных.абс_НазначениеПлатежа) КАК НазначениеПлатежаПредставление,
	...
	|	ИсточникДанных.абс_ЗаказПроект.Проект КАК Проект
	|	//ПОЛЯ_СВОЙСТВА
	|	//ПОЛЯ_КАТЕГОРИИ
	|{ВЫБРАТЬ
	|	Номенклатура.*,
	...
	|}
	|ИЗ
	|	РегистрНакопления.ПартииТоваровПереданные.ОстаткиИОбороты(&ДатаНач, &ДатаКон, Регистратор {(&Периодичность)}, , {(Номенклатура).* КАК Номенклатура, (Организация).* КАК Организация, (СтатусПередачи).* КАК СтатусПередачи, (ДоговорКонтрагента.Владелец).* КАК ДоговорКонтрагентаВладелец, (ДоговорКонтрагента).* КАК ДоговорКонтрагента, (ДокументПередачи).* КАК ДокументПередачи, (ХарактеристикаНоменклатуры).* КАК ХарактеристикаНоменклатуры, (ДокументОприходования).* КАК ДокументОприходования, (СтатусПартии).* КАК СтатусПартии}) КАК ИсточникДанных
	|//СОЕДИНЕНИЯ
	|{ГДЕ
	|	ИсточникДанных.КоличествоНачальныйОстаток КАК КоличествоНачальныйОстаток,
	|	(ИсточникДанных.КоличествоНачальныйОстаток * ИсточникДанных.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент / ИсточникДанных.Номенклатура.ЕдиницаДляОтчетов.Коэффициент) КАК КоличествоНачальныйОстатокВЕдиницахДляОтчетов,
	...
	|	ИсточникДанных.абс_НазначениеПлатежа.* КАК НазначениеПлатежа,
	|	ИсточникДанных.СерияНоменклатуры.*,
	|	ИсточникДанных.абс_ЗаказПроект.Проект.*
	|	//УСЛОВИЯ_СВОЙСТВА
	|	//УСЛОВИЯ_КАТЕГОРИИ
	|}
	|{УПОРЯДОЧИТЬ ПО
	...
	//// АБС Вставка 59238 
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("абс_ЦФО", "ЦФО");
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("абс_НазначениеПлатежа", "Назначение использования");
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("СерияНоменклатуры", "Серия номенклатуры");
	////\\ АБС Вставка 59238
	                      
	//АБС НАЧАЛО Задача №08759
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("абс_ЗаказПроектПроект", "Проект");
	...
	УниверсальныйОтчет.ДобавитьОтбор("НазначениеПлатежа");
...
Процедура УстановитьНачальныеНастройкиБух(ДополнительныеПараметры = Неопределено)
	...
	|}
	|ИЗ
	|	РегистрНакопления.ПартииТоваровПереданныеБухгалтерскийУчет.ОстаткиИОбороты(&ДатаНач, &ДатаКон, Регистратор {(&Периодичность)}, , {(Номенклатура).* КАК Номенклатура, (СчетУчета).*, (Организация).*, (ДоговорКонтрагента.Владелец).* КАК ДоговорКонтрагентаВладелец, (ДоговорКонтрагента).* КАК ДоговорКонтрагента, (ДокументПередачи).* КАК ДокументПередачи, (ХарактеристикаНоменклатуры).* КАК ХарактеристикаНоменклатуры, (ДокументОприходования).* КАК ДокументОприходования}) КАК ИсточникДанных
	|//СОЕДИНЕНИЯ
	|{ГДЕ
	|	ИсточникДанных.КоличествоНачальныйОстаток КАК КоличествоНачальныйОстаток,
	|	(ИсточникДанных.КоличествоНачальныйОстаток * ИсточникДанных.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент / ИсточникДанных.Номенклатура.ЕдиницаДляОтчетов.Коэффициент) КАК КоличествоНачальныйОстатокВЕдиницахДляОтчетов,
...
Процедура УстановитьНачальныеНастройкиНал(ДополнительныеПараметры = Неопределено)
	...
	|}
	|ИЗ
	|	РегистрНакопления.ПартииТоваровПереданныеНалоговыйУчет.ОстаткиИОбороты(&ДатаНач, &ДатаКон, Регистратор {(&Периодичность)}, , {(Номенклатура).* КАК Номенклатура, (СчетУчета).*, (Организация).*, (ДоговорКонтрагента.Владелец).* КАК ДоговорКонтрагентаВладелец, (ДоговорКонтрагента).* КАК ДоговорКонтрагента, (ДокументПередачи).* КАК ДокументПередачи, (ХарактеристикаНоменклатуры).* КАК ХарактеристикаНоменклатуры, (ДокументОприходования).* КАК ДокументОприходования}) КАК ИсточникДанных
	|//СОЕДИНЕНИЯ
	|{ГДЕ
	|	ИсточникДанных.КоличествоНачальныйОстаток КАК КоличествоНачальныйОстаток,
	|	(ИсточникДанных.КоличествоНачальныйОстаток * ИсточникДанных.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент / ИсточникДанных.Номенклатура.ЕдиницаДляОтчетов.Коэффициент) КАК КоличествоНачальныйОстатокВЕдиницахДляОтчетов,
	...
	УниверсальныйОтчет.ДобавитьПоказатель("КоличествоНачальныйОстатокВБазовыхЕдиницах", "Количество (в базовых ед.)", Ложь, "ЧЦ=15; ЧДЦ=3", "НачальныйОстаток", "Начальный остаток");
	УниверсальныйОтчет.ДобавитьПоказатель("СтоимостьНачальныйОстаток", "Стоимость", Истина, "ЧЦ=15; ЧДЦ=2", "НачальныйОстаток", "Начальный остаток");
	УниверсальныйОтчет.ДобавитьПоказатель("ПостояннаяРазницаНачальныйОстаток", "Постоянная разница", Истина, "ЧЦ=15; ЧДЦ=2", "НачальныйОстаток", "Начальный остаток");
	УниверсальныйОтчет.ДобавитьПоказатель("ВременнаяРазницаНачальныйОстаток", "Временная разница", Истина, "ЧЦ=15; ЧДЦ=2", "НачальныйОстаток", "Начальный остаток");
	
	УниверсальныйОтчет.ДобавитьПоказатель("КоличествоПриход", "Количество", Истина, "ЧЦ=15; ЧДЦ=3", "Приход", "Приход");
	УниверсальныйОтчет.ДобавитьПоказатель("КоличествоПриходВЕдиницахДляОтчетов", "Количество (в ед. отчетов)", Ложь, "ЧЦ=15; ЧДЦ=3", "Приход", "Приход");
	УниверсальныйОтчет.ДобавитьПоказатель("КоличествоПриходВБазовыхЕдиницах", "Количество (в базовых ед.)", Ложь, "ЧЦ=15; ЧДЦ=3", "Приход", "Приход");
	УниверсальныйОтчет.ДобавитьПоказатель("СтоимостьПриход", "Стоимость", Истина, "ЧЦ=15; ЧДЦ=2", "Приход", "Приход");
	УниверсальныйОтчет.ДобавитьПоказатель("ПостояннаяРазницаПриход", "Постоянная разница", Истина, "ЧЦ=15; ЧДЦ=2", "Приход", "Приход");
	УниверсальныйОтчет.ДобавитьПоказатель("ВременнаяРазницаПриход", "Временная разница", Истина, "ЧЦ=15; ЧДЦ=2", "Приход", "Приход");
	
	УниверсальныйОтчет.ДобавитьПоказатель("КоличествоРасход", "Количество", Истина, "ЧЦ=15; ЧДЦ=3", "Расход", "Расход");
	УниверсальныйОтчет.ДобавитьПоказатель("КоличествоРасходВЕдиницахДляОтчетов", "Количество (в ед. отчетов)", Ложь, "ЧЦ=15; ЧДЦ=3", "Расход", "Расход");
	УниверсальныйОтчет.ДобавитьПоказатель("КоличествоРасходВБазовыхЕдиницах", "Количество (в базовых ед.)", Ложь, "ЧЦ=15; ЧДЦ=3", "Расход", "Расход");
	УниверсальныйОтчет.ДобавитьПоказатель("СтоимостьРасход", "Стоимость", Истина, "ЧЦ=15; ЧДЦ=2", "Расход", "Расход");
	УниверсальныйОтчет.ДобавитьПоказатель("ПостояннаяРазницаРасход", "Постоянная разница", Истина, "ЧЦ=15; ЧДЦ=2", "Расход", "Расход");
	УниверсальныйОтчет.ДобавитьПоказатель("ВременнаяРазницаРасход", "Временная разница", Истина, "ЧЦ=15; ЧДЦ=2", "Расход", "Расход");
	
	УниверсальныйОтчет.ДобавитьПоказатель("КоличествоКонечныйОстаток", "Количество", Истина, "ЧЦ=15; ЧДЦ=3", "КонечныйОстаток", "Конечный остаток");
	УниверсальныйОтчет.ДобавитьПоказатель("КоличествоКонечныйОстатокВЕдиницахДляОтчетов", "Количество (в ед. отчетов)", Ложь, "ЧЦ=15; ЧДЦ=3", "КонечныйОстаток", "Конечный остаток");
	УниверсальныйОтчет.ДобавитьПоказатель("КоличествоКонечныйОстатокВБазовыхЕдиницах", "Количество (в базовых ед.)", Ложь, "ЧЦ=15; ЧДЦ=3", "КонечныйОстаток", "Конечный остаток");
	УниверсальныйОтчет.ДобавитьПоказатель("СтоимостьКонечныйОстаток", "Стоимость", Истина, "ЧЦ=15; ЧДЦ=2", "КонечныйОстаток", "Конечный остаток");
	УниверсальныйОтчет.ДобавитьПоказатель("ПостояннаяРазницаКонечныйОстаток", "Постоянная разница", Истина, "ЧЦ=15; ЧДЦ=2", "КонечныйОстаток", "Конечный остаток");
	УниверсальныйОтчет.ДобавитьПоказатель("ВременнаяРазницаКонечныйОстаток", "Временная разница", Истина, "ЧЦ=15; ЧДЦ=2", "КонечныйОстаток", "Конечный остаток");
