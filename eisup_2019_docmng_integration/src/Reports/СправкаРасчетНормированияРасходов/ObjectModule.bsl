#Если Клиент Тогда
	...
	Функция СформироватьТекстЗапроса()
		...
		Текст	= "ВЫБРАТЬ
				  ...
				  //АБС ВСТАВКА 47730  08.10.2014 16:42:07  Шамов
		     	  //|	И РасчетНормированияРасходов.Организация = &Организация
		     	  |	И РасчетНормированияРасходов.Организация В (&Организация)
	...
	КонецЕсли;
		...
		Пока ВыборкаПериод.Следующий() Цикл
			...
			//АБС ИЗМЕНЕНИЕ 47730  08.10.2014 16:42:46  Шамов
			//ЗаголовокОтчета.Параметры.НазваниеОрганизации = "" + Организация.НаименованиеПолное;
			ВремНазваниеОрганизации = "";
			Для Каждого Элемент Из Организация Цикл
				ВремНазваниеОрганизации = ВремНазваниеОрганизации + Элемент.Значение.НаименованиеПолное + "; ";
			КонецЦикла;
			ЗаголовокОтчета.Параметры.НазваниеОрганизации = ВремНазваниеОрганизации;
			//АБС ИЗМЕНЕНИЕ 47730 КОНЕЦ			
			...
			//АБС ИЗМЕНЕНИЕ 47730  08.10.2014 16:59:58  Шамов
			...
			ВыборкаОперации = ВыборкаПериод.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			Пока ВыборкаОперации.Следующий() Цикл
				ВидОперации = ВыборкаОперации.ВидОперации;
			//АБС ИЗМЕНЕНИЕ 47730 КОНЕЦ 
				...
				ИначеЕсли ВидОперации = Перечисления.ВидыРасходовНУ.ДобровольноеЛичноеСтрахование Тогда	
					...
					Если ДатаКон < Дата("20090101") Тогда
						БазаНормирования = "не более 3% от суммы расходов по оплате труда (" + ВыборкаОперации.СуммаБазы + " руб.) (п.16 ст.255)";
