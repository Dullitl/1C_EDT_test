...
Процедура СформироватьКнигуПродаж(ЭлементыФормы) Экспорт
	...
	УчетнаяПолитикаНУ = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(НачалоПериода, Организация);
...
КонецПроцедуры // СформироватьОсновнойРаздел()

Процедура ЗаполнитьСтрокуДополнительногоЛиста(Секция, ЗаписьКниги, ЕстьЗаписиПоКолонке20)
		
	ДатаИНомерСФ = ОпределитьДатуИНомерСФ(ЗаписьКниги.Строки[0]);
...
Процедура СформироватьДополнительныеЛисты(Организация, НачалоПериода, КонецПериода, ЭлементыФормы, УчетнаяПолитикаНУ = Неопределено)
	...
	СписокСчетовФактур = Неопределено;
	Результат = ПолучитьЗаписиДополнительныхЛистов(Организация, НачалоПериода, КонецПериода, УчетнаяПолитикаНУ.МоментОпределенияНалоговойБазыНДС, СписокСчетовФактур);
...
//------------------------
		...
		Для Каждого ИтогПоПериодамКорректировки ИЗ ИтогПоПериодам.Строки Цикл;
			...
			Иначе
				...
				ИтогЗаПериод.Вставить("ВсегоПродаж", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
				ИтогЗаПериод.Вставить("СуммаБезНДС10", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
				ИтогЗаПериод.Вставить("НДС10", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
				ИтогЗаПериод.Вставить("СуммаБезНДС18", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
				ИтогЗаПериод.Вставить("НДС18", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
				ИтогЗаПериод.Вставить("НДС0", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
				ИтогЗаПериод.Вставить("СуммаБезНДС20", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
				ИтогЗаПериод.Вставить("НДС20", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
				ИтогЗаПериод.Вставить("СуммаСовсемБезНДС", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
			КонецЕсли;
		...
		Если СкрытьКолонкиПоСтавке20 Тогда
			Если не МожноСкрытьКолонкиПоСтавке20 Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("В дополнительном листе книге продаж организации <"+Организация+"> за период "+ПредставлениеПериода(ИтогПоПериодамКорректировки.НалоговыйПериод, КонецДня(ИтогПоПериодамКорректировки.КонецНалоговогоПериода))+" есть записи по ставке НДС 20%!
				|Отображение соотвествующих колонок не может быть отключено.");
			Иначе
...
КонецПроцедуры // СформироватьДополнительныеЛисты()
...
Функция ОпределитьДатуИНомерСФ(ЗаписьКниги, Секция = Неопределено)
	...
	Если ЗначениеЗаполнено(ЗаписьКниги.СчетФактураДокумент) Тогда
		...
		ИначеЕсли ТипЗнч(ЗаписьКниги.СчетФактураДокумент) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда	
			ДатаНомер = "" + Формат(?(ЗначениеЗаполнено(ЗаписьКниги.СчетФактураДокумент.абс_ДатаСФ), ЗаписьКниги.СчетФактураДокумент.абс_ДатаСФ, ЗаписьКниги.ДатаСчетаФактуры), "ДФ=dd.MM.yyyy") + ", №" + ОбщегоНазначения.ПолучитьНомерНаПечать(Новый Структура("Дата, Номер, Организация", ЗаписьКниги.ДатаСчетаФактуры, ЗаписьКниги.НомерСчетаФактуры, Организация), ПрефиксыРИБиОрганизации);		
			//АБС Коломиец-	
		Иначе
	...
	ИначеЕсли ТипЗнч(ЗаписьКниги.СчетФактура) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
		ДатаНомер = "" + Формат(?(ЗначениеЗаполнено(ЗаписьКниги.СчетФактура.абс_ДатаСФ), ЗаписьКниги.СчетФактура.абс_ДатаСФ, ЗаписьКниги.СчетФактура.Дата), "ДФ=dd.MM.yyyy") + ", №" + ОбщегоНазначения.ПолучитьНомерНаПечать(ЗаписьКниги.СчетФактура,ПрефиксыРИБиОрганизации);	//АБС Коломиец		
	...
	//АБС Коломиец+
	Если ДатаНомер = ", №" Или ДатаНомер = "" Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
		               ...
		               |		1 КАК Приоритет,
		               |		СчетФактураВыданныйДокументыОснования.Ссылка.абс_ДатаСФ КАК ДатаСФ
		               |	ИЗ
		               |		РегистрНакопления.НДСЗаписиКнигиПродаж.Обороты(, , Регистратор, ) КАК НДСЗаписиКнигиПродажОбороты
		               ...
		               |	ГДЕ
		               |		НДСЗаписиКнигиПродажОбороты.СчетФактура = &ДокументОснование
		               ...
		               |	
		               |	ВЫБРАТЬ ПЕРВЫЕ 1
		               |		НДСЗаписиКнигиПродажОбороты.СчетФактура.НомерВходящегоДокумента,
		               |		НДСЗаписиКнигиПродажОбороты.СчетФактура.ДатаВходящегоДокумента,
		               |		НДСЗаписиКнигиПродажОбороты.СчетФактура.Ссылка,
		               |		2,
		               |		НДСЗаписиКнигиПродажОбороты.СчетФактура.абс_ДатаСФ
		               |	ИЗ
		               |		РегистрНакопления.НДСЗаписиКнигиПродаж.Обороты(, , Регистратор, ) КАК НДСЗаписиКнигиПродажОбороты
		               |	ГДЕ
		               |		НДСЗаписиКнигиПродажОбороты.СчетФактура = &ДокументОснование
		               |		И НДСЗаписиКнигиПродажОбороты.СчетФактура.Проведен = ИСТИНА
		               |	
		               |	ОБЪЕДИНИТЬ ВСЕ
		               |	
		               |	ВЫБРАТЬ ПЕРВЫЕ 1
		               |		НДСЗаписиКнигиПродажОбороты.Регистратор.НомерВходящегоДокумента,
		               |		НДСЗаписиКнигиПродажОбороты.Регистратор.ДатаВходящегоДокумента,
		               ...
		               |		РегистрНакопления.НДСЗаписиКнигиПродаж.Обороты(, , Регистратор, ) КАК НДСЗаписиКнигиПродажОбороты
		               |	ГДЕ
		               |		НДСЗаписиКнигиПродажОбороты.СчетФактура = &ДокументОснование) КАК ВложенныйЗапрос
		               |
		               |УПОРЯДОЧИТЬ ПО
		...
		Если Рез.Следующий() Тогда
  			...
  			Если ТипЗнч(Рез.Ссылка) = Тип("ДокументСсылка.СчетФактураПолученный") Тогда
				ДатаНомер = "" + Формат(Рез.Дата, "ДФ=dd.MM.yyyy") + ", №" + СокрЛП(Рез.Номер);
			Иначе
				// {{KM WARE Агапов Н.А. Заявка №34379 21.09.2015 начало
				Если ПроверкаДаты(Рез.ДатаСФ) = истина тогда
					ДатаНомер = "" + Формат(?(ЗначениеЗаполнено(Рез.ДатаСФ), Рез.ДатаСФ, Рез.Дата), "ДФ=dd.MM.yyyy") + ", №" + ОбщегоНазначения.ПолучитьНомерНаПечать(Новый Структура("Дата, Номер, Организация", Рез.Дата, Рез.Номер, Организация), ПрефиксыРИБиОрганизации);						
				Иначе
					ДатаНомер = "" + Формат(Рез.Дата, "ДФ=dd.MM.yyyy") + ", №" + ОбщегоНазначения.ПолучитьНомерНаПечать(Новый Структура("Дата, Номер, Организация", Рез.Дата, Рез.Номер, Организация), ПрефиксыРИБиОрганизации);	
				КонецЕсли;
				// }}KM WARE Агапов Н.А. Заявка №34379 21.09.2015 окончание
			КонецЕсли;	
...
// 	Результат - ссылка на результаты выполнения запроса к данным регистра "НДСПродажи"
//  МоментОпределенияНалоговойБазыНДС
Функция ПолучитьЗаписиДополнительныхЛистов(Организация, НачалоПериода, КонецПериода, МоментОпределенияНалоговойБазыНДС, СписокСчетовФактур)
	...
	Запрос.Текст =	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	              	|	ЗаписиКнигиПродаж.Организация КАК Организация,
	              	|	УчетнаяПолитикаОрганизаций.НДСНалоговыйПериод КАК НалПер,
	              	|	ВЫБОР
	              	...
	              	|	ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.СчетФактураДокумент, ЗаписиКнигиПродаж.СчетФактура) КАК СчетФактураДокумент,
					///АБС ВСТАВКА №5554 НАЧАЛО
					//|	ТаблицаСчетаФактурыДокументы.НомерСчетаФактуры КАК НомерСчетаФактуры,
					|	ВЫБОР КОГДА ТаблицаСчетаФактурыДокументы.СчетФактураДокумент.абс_НомерСФНаЛьготуПоНДС <> """" ТОГДА
					|		ТаблицаСчетаФактурыДокументы.СчетФактураДокумент.абс_НомерСФНаЛьготуПоНДС
					|	ИНАЧЕ
					|		ТаблицаСчетаФактурыДокументы.НомерСчетаФактуры
					|  	КОНЕЦ КАК НомерСчетаФактуры, 					
					//\\АБС ВСТАВКА №5554 КОНЕЦ
	              	|	ТаблицаСчетаФактурыДокументы.ДатаСчетаФактуры КАК ДатаСчетаФактуры,
	              	|	ЗаписиКнигиПродаж.СтавкаНДС_Аванс КАК СтавкаНДС_Аванс,
	              	|	ЗаписиКнигиПродаж.ДоговорАванса КАК ДоговорАванса,
	              	|	ЗаписиКнигиПродаж.Контрагент,
	              	...
	              	|					РегистрСведений.УчетнаяПолитикаОрганизаций КАК УчетнаяПолитикаОрганизаций
	              	|				ГДЕ
	              	|					ЗаписиКнигиПродаж.КорректируемыйПериод >= УчетнаяПолитикаОрганизаций.Период
	              	|					И УчетнаяПолитикаОрганизаций.Организация = ЗаписиКнигиПродаж.Организация))
	              	|
	              	|УПОРЯДОЧИТЬ ПО
	              	|	НалоговыйПериод,
...
Функция ПолучитьДополнительнуюИнформациюПоСФ(Организация, КонецПериода, СписокСчетовФактур, ЗаписьДополнительногоЛиста = Ложь)
	...
	ДополнительнаяИнформацияПоСФ.Колонки.Добавить("IDСчетФактура", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(0)));
