...
Функция ЗаголовокОтчета() Экспорт
	Возврат "Оборотно-сальдовая ведомость по счету " + ?(ЗначениеЗаполнено(Счет), Счет, " не выбран счет") + " (налоговый)" + " (с детализацией по ДЗО)";	
...
Функция абс_СформироватьЗапрос(СтруктураПараметров) Экспорт
	...
	Для Каждого Показатель Из СтруктураПараметров.МассивПоказателей Цикл
		...
		КонецЦикла;
	        ...
	        |
	        |СГРУППИРОВАТЬ ПО
	        |	ОстаткиОбороты.Регистратор.абс_ОрганизацияИсточник"+?(ПоСубсчетам,", ОстаткиОбороты.Счет","") + "" +"
	        |	" + ТекстПоляГруппировки +"
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////";
...
Процедура СформироватьОтчет(ДокументРезультат,  ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	СтруктураПараметровОграничений = БухгалтерскиеОтчеты.СформироватьСтруктуруОграниченийДляОборотноСальдовойВедомостиПоСчету(ЭтотОбъект, Ложь, ВидУчета);
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	ПостроительОтчета.Параметры.Вставить("ВидУчета", ВидУчета);
