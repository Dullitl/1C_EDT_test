...
Процедура СформироватьОтчет(ДокументРезультат, ЕстьОшибки = Неопределено, ВнешниеНаборыДанных = Неопределено) Экспорт
	...
	|	ВЫРАЗИТЬ(РаботникиОрганизацииСрезПоследних.ОбособленноеПодразделение.НаименованиеПолное КАК СТРОКА(300)) КАК НаименованиеОрганизации,
...
Процедура СформироватьМакет(ДокументРезультат, Работник, ЕстьОшибки, Уровень) Экспорт
	...
	Если КадровыеДанные.НайтиСледующий(СтруктураПоискаСотрудник) тогда
		...
		Если КадровыеДанные.ВидДокумента <> Null Тогда
			Если	КадровыеДанные.ВидДокументаСсылка <> Справочники.ДокументыУдостоверяющиеЛичность.ИМНС21 И
				КадровыеДанные.ВидДокументаСсылка <> Справочники.ДокументыУдостоверяющиеЛичность.ИМНС01 Тогда
	...
	ЧетвертаяСтраницаТ2.Параметры["ФИОКадровика"]       = УправлениеОтчетами.ФамилияИнициалыОтветсвенногоЛица(КадровыйРаботник.ФизЛицо);
	ВтораяСтраницаТ2.Параметры["ДолжностьКадровика"]    = КадровыйРаботник.ТекущаяДолжностьОрганизации;
	ВтораяСтраницаТ2.Параметры["ФИОКадровика"]          = УправлениеОтчетами.ФамилияИнициалыОтветсвенногоЛица(КадровыйРаботник.ФизЛицо);
