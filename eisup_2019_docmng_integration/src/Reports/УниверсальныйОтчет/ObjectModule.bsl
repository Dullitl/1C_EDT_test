...
Процедура СформироватьОтчет(ТабличныйДокумент, Очищать = Истина, ФиксироватьЗаголовок = Истина, Отчет = Неопределено) Экспорт
	...
	Если ВыводВозможен() Тогда
		...
		//Бобылев А.А.
		//Если ПостроительОтчета.ВыбранныеПоля.Найти("НоменклатураАртикул") <> Неопределено И Отчет.УниверсальныйОтчет.мНазваниеОтчета = "Ведомость по партиям товаров на складах (бухгалтерский учет)" Тогда
		//	Текст = ПостроительОтчета.Текст;
		//	Текст = СтрЗаменить(Текст, "ПРЕДСТАВЛЕНИЕ(ИсточникДанных.Номенклатура)", "1 Как ПредставлениеНоменклатура");
		//	ПостроительОтчета.Текст = Текст;
		//	//ПостроительОтчета.ВыбранныеПоля.Удалить(ПостроительОтчета.ВыбранныеПоля.Найти("НоменклатураАртикул"));
		...
		//	//		Если Артикул <> "" Тогда
		//	//		    Элемент.НоменклатураАртикул = "";
		//	//			Элемент.НоменклатураПредставление = Строка(Элемент.Номенклатура) + " [" + Артикул + "]";
		//	//		КонецЕсли;
		//	//КонецЦикла; 
		//	//ПостроительОтчета.Текст = ТекстОтчета;
		//	//Запрос = Новый Запрос;
		...
		//	//ПостроительОтчета.Результат = ВыполнениеЗапроса;
		//	//ПостроительОтчета.ВыбранныеПоля.Удалить(ПостроительОтчета.ВыбранныеПоля.Найти("НоменклатураАртикул"));
		//КонецЕсли;
