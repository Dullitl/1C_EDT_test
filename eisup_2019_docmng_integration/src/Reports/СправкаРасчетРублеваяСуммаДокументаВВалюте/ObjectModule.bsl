...
Функция СформироватьОтчет(ДокументРезультат) Экспорт
	...
	Если НЕ Документ.Проведен Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Справка-расчет формируется только по проведенным документам.");
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ СтруктураШапкиДокумента.Свойство("ОтражатьВБухгалтерскомУчете") Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("В документе не найден реквизит ""Отражать в бухгалтерском учете""");
		Возврат Ложь;
	ИначеЕсли НЕ СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Справка-расчет формируется только по документам, отраженным в бухгалтерском учете");
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ (НаправлениеДвижения = "Поступление" ИЛИ НаправлениеДвижения = "Выбытие") Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Справка-расчет формируется только по документам, отражающим операции приобретения или реализации");
		Возврат Ложь;
	КонецЕсли;
	
	Если НЕ СтруктураШапкиДокумента.Свойство("ВалютаДокумента") Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("В документе не найден реквизит ""Валюта документа""");
		Возврат Ложь;
	ИначеЕсли НЕ ЗначениеЗаполнено(СтруктураШапкиДокумента.ВалютаДокумента) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("В документе не заполнен реквизит ""Валюта документа""");
		Возврат Ложь;
	ИначеЕсли СтруктураШапкиДокумента.ВалютаДокумента = мВалютаРегламентированногоУчета Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Справка-расчет формируется только по документам в иностранной валюте");
		Возврат Ложь;
	КонецЕсли;
	...
	Если ВидДокумента = "РеализацияТоваровУслуг" Тогда
		...
		Если ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ОтгрузкаБезПереходаПраваСобственности 
			...
			Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Справка-расчет формируется по операции реализации покупателю.");
			Возврат Ложь;
		КонецЕсли;
	...
	ИначеЕсли ВидДокумента = "ПоступлениеТоваровУслуг" Тогда
		...
		Если ДоговорКонтрагента.ВидДоговора <> Перечисления.ВидыДоговоровКонтрагентов.СПоставщиком 
			Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Справка-расчет формируется по операции поступления от поставщика.");
			Возврат Ложь;
		КонецЕсли;
	...
	Если НЕ СтруктураШапкиДокумента.Свойство("СчетУчетаРасчетовСКонтрагентом") Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("В документе не найден реквизит ""Счет расчетов с контрагентом""");
		Возврат Ложь;
	ИначеЕсли НЕ ЗначениеЗаполнено(СтруктураШапкиДокумента.СчетУчетаРасчетовСКонтрагентом) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("В документе не заполнен реквизит ""Счет расчетов с контрагентом""");
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Выборка.Следующий();
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Для формирования справки-расчета необходимо выполнить восстановление последовательностей расчетов по организации """ 
	...
	Если НаправлениеДвижения = "Поступление" Тогда
		...
		Если НарушенаПоследовательность Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Граница последовательности по приобретению находится ранее документа, данные справки-расчета могут быть неверными.",
	...
	Иначе
		НарушенаПоследовательность = НЕ Последовательности.РасчетыПоРеализацииОрганизации.Проверить(
			Документ.МоментВремени(), Новый Структура("Организация", СтруктураШапкиДокумента.Организация));
		Если НарушенаПоследовательность Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Граница последовательности по реализации находится ранее документа, данные справки-расчета могут быть неверными.",
				, , СтатусСообщения.Внимание);
		КонецЕсли;
	КонецЕсли;
