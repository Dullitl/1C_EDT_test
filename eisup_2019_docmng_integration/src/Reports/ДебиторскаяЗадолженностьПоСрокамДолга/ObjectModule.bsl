...
Процедура УстановитьТекстЗапроса()
	...
	|ИЗ
	|	РегистрНакопления.ВзаиморасчетыСКонтрагентамиПоДокументамРасчетов.Остатки(&ДатаОстатков, 
	//АБС ВСТАВКА 35379  27.11.2013 9:53:02  Шамов
	|	(ДоговорКонтрагента.ВестиПоДокументамРасчетовСКонтрагентом)
	//АБС ВСТАВКА 35379 КОНЕЦ Шамов
	...
	|ИЗ
	|	РегистрНакопления.ВзаиморасчетыСКонтрагентамиПоДокументамРасчетов.Обороты(&НачалоИнтервала1, &КонецИнтервала1, , 
	//АБС ВСТАВКА 35379  27.11.2013 9:53:02  Шамов
	|	(ДоговорКонтрагента.ВестиПоДокументамРасчетовСКонтрагентом)
	//\\
	...
	|	КОНЕЦ
	|ИЗ
	|	РегистрНакопления.ВзаиморасчетыСКонтрагентами.Обороты(&НачалоИнтервала1, &КонецИнтервала1, , (НЕ ДоговорКонтрагента.ВестиПоДокументамРасчетовСКонтрагентом) {(Организация).* КАК Организация, (Контрагент).* КАК Контрагент, (ДоговорКонтрагента).* КАК ДоговорКонтрагента, (Сделка).* КАК Сделка}) КАК ВзаиморасчетыБезДокументовОбороты";
