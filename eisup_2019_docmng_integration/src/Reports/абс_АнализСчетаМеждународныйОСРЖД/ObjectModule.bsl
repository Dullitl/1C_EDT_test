...
Функция ЗаголовокОтчета() Экспорт
	Возврат  "Анализ счета " + ?(ЗначениеЗаполнено(Счет), Счет, " не выбран счет") + " (международный учет ОС РЖД)";
КонецФункции
...
Процедура ЗаполнитьПараметрыРасшифровки(Область, Выборка, ОтборСубконто = Неопределено)
	...
	//АБС ЗАМЕНА 34336 25.11.13 Вихорев
	//ПараметрыРасшифровки.Вставить("ИмяОбъекта", "ОтчетПоПроводкам"+ИмяРегистраБухгалтерии);
	ПараметрыРасшифровки.Вставить("ИмяОбъекта", "абс_ОтчетПоПроводкамМеждународныйОСРЖД");
	//АБС ЗАМЕНА 34336 КОНЕЦ
...
Процедура СформироватьОтчет(ДокументРезультат,  ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	// Start КТТК Ермолов Е.Л.  16.10.2015 000033874
	Если ЗначениеЗаполнено(Организация) Тогда
		Если абс_ВключаяПодчиненныеОрганизации Тогда
			ТекстОтбор = ТекстОтбор + " И Организация В ИЕРАРХИИ (&Организация)";
		Иначе 
			ТекстОтбор = ТекстОтбор + " И Организация = &Организация";
		КонецЕсли;
	КонецЕсли;
	// Stop КТТК Ермолов Е.Л.  16.10.2015
...
ИмяРегистраБухгалтерии = "абс_МеждународныйОСРЖД";
