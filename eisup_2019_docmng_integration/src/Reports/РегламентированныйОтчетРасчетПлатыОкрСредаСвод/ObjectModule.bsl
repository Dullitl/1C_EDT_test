...
Функция ПолучитьЭлектронныеДокументы(ВыгружаемыеДокументы, УникальныйИдентификатор = Неопределено) Экспорт
	...
	ЭтоБП = (ИдКонфигурации = "БП" ИЛИ ИдКонфигурации = "БПКОРП");
	ЭтоУППКА = (ИдКонфигурации = "УПП" ИЛИ ИдКонфигурации = "КА");
	
	Результат = Новый Соответствие;
	
	Для Каждого ВыгружаемыйДокумент Из ВыгружаемыеДокументы Цикл
		
		Если ЭтоБП ИЛИ ЭтоУППКА Тогда
			...
			Если ЭтоДокументРегистраУчетаНДС Тогда
				...
				ТекстыВыгрузки = ОбъектВыгрузки.ЭлектронноеПредставление();
				
				МассивВыгрузки = Новый Массив;
				Для Каждого ТекстВыгрузки Из ТекстыВыгрузки Цикл
					
					// Формирование бинарного файла и помещение его в хранилище
					ИмяВременногоФайла = ПолучитьИмяВременногоФайла();
					СохраняемыйФайл = Новый ТекстовыйДокумент;
					СохраняемыйФайл.УстановитьТекст(ТекстВыгрузки.ТекстФайла);
					СохраняемыйФайл.Записать(ИмяВременногоФайла, ТекстВыгрузки.КодировкаТекста);
					...
					ЭлементВыгрузки.Вставить("ИмяФайла", ТекстВыгрузки.ИмяФайла);
					ЭлементВыгрузки.Вставить("АдресВременногоХранилища", СсылкаНаДвоичныеДанныеФайла);
...
КонецФункции // ПолучитьЭлектронныеДокументы()

// Возвращает только цифровые символы из строки.
//
// Параметры
//  ИсходнаяСтрока - строка - строка из которой необходимо извлеч цифровые символы.
//
// Возвращаемое значение:
//   Строка - строки извлеченных цифровых символов.
//
Функция ТолькоЦифрыИзСтроки(Знач ИсходнаяСтрока) Экспорт
...
//
// Параметры:
//   СохраненныйРегламентированныйОтчет - ДокументСсылка.РегламентированныйОтчет - ссылка на сохраненный
...
//
Функция ПредставлениеПачекДокументов(СохраненныйРегламентированныйОтчет) Экспорт
	
	Возврат ПроцедурыПерсонифицированногоУчета.ПечатьРазделов25РасчетаРСВ1(СохраненныйРегламентированныйОтчет, Истина)
	
КонецФункции

// Возвращает комплект табличных документов представляющих пачки документов для печати РСВ-1.
//
// Параметры:
//   СохраненныйРегламентированныйОтчет - ДокументСсылка.РегламентированныйОтчет - ссылка на сохраненный
//                                        документ, содержащий сведения отчета.
...
// Возвращаемое значение:
//   * Структура - представления и описания пачек документов.
//      ** Пачки - Таблица значений:
//         *** Представление - Табличный документ - представление пачки
//         *** Описание - Строка - описание пачки для навигации по листам
//      ** Предупреждения - Таблица значений:
//      ** НомерПоследнейСтраницы - Число - номер страницы на последнем листе представлений пачек документов
//
Функция ПредставлениеПачекДокументовДляПечати(СохраненныйРегламентированныйОтчет, НомерСтраницы) Экспорт
	
	Возврат ПроцедурыПерсонифицированногоУчета.ПечатьРазделов25РасчетаРСВ1(СохраненныйРегламентированныйОтчет, Ложь, НомерСтраницы)
	
КонецФункции

// Возвращает комплект табличных документов содержащих сведения по физическим лицам для отображения в РСВ-1.
//
// Параметры:
//   СохраненныйРегламентированныйОтчет - ДокументСсылка.РегламентированныйОтчет - ссылка на сохраненный
//                                        документ, содержащий сведения отчета.
...
//
Функция ПредставлениеСведенийПоФизлицам(СохраненныйРегламентированныйОтчет) Экспорт
	
	Возврат ПроцедурыПерсонифицированногоУчета.ПечатьРазделов6РасчетаРСВ1(СохраненныйРегламентированныйОтчет, Истина)
...
//         *** Описание - Строка - описание сведений для навигации по листам
//      ** Предупреждения - Таблица значений:
//      ** НомерПоследнейСтраницы - Число - номер страницы на последнем листе представлений пачек сведений
//
Функция ПредставлениеСведенийПоФизлицамДляПечати(СохраненныйРегламентированныйОтчет, НомерСтраницы) Экспорт
	
	Возврат ПроцедурыПерсонифицированногоУчета.ПечатьРазделов6РасчетаРСВ1(СохраненныйРегламентированныйОтчет, Ложь, НомерСтраницы)
...
//
Функция ЭлектронноеПредставлениеПачекДокументов(СохраненныйРегламентированныйОтчет) Экспорт
	
	Возврат ПроцедурыПерсонифицированногоУчета.СведенияРаздела25РасчетаРСВ1(СохраненныйРегламентированныйОтчет)
	
КонецФункции

// Возвращает комплект текстов документов содержащих сведения по физическим лицам для электронного представления.
//
// Параметры:
//  СохраненныйРегламентированныйОтчет - ДокументСсылка.РегламентированныйОтчет - ссылка на сохраненный
//                                       документ, содержащий сведения отчета.
...
//        *** Документ - ДокументСсылка - ссылка на документ информационной базы, содержащий ошибку
//
Функция ЭлектронноеПредставлениеСведенийПоФизлицам(СохраненныйРегламентированныйОтчет) Экспорт
	
	Возврат ПроцедурыПерсонифицированногоУчета.ФайлыРазделов6РасчетаРСВ1(СохраненныйРегламентированныйОтчет)
	
КонецФункции

// Открывает форму рабочего места первоначального заполнения отчета РСВ-1
//
// Параметры:
//   Организация - СправочникСсылка.Организации - организация, по которой создается отчет;
//   НаДату - Дата - любая дата в периоде за который создается отчет.
//
Процедура ОткрытьРабочееМестоЗаполненияРСВ1(Организация, НаДату) Экспорт
	
	ПроцедурыПерсонифицированногоУчета.ОткрытьРабочееМесто(Организация, НаДату);
	
КонецПроцедуры

#Если ТолстыйКлиентОбычноеПриложение Тогда

// Возвращает ссылку на сохраненный экземпляр отчета РСВ-1.
//
// Параметры:
//   Организация - СправочникСсылка.Организации - организация, по которой создается отчет;
//   НаДату - Дата - любая дата в периоде за который создается отчет.
//
// Возвращаемое значение:
//   ДокументСсылка.РегламентированныйОтчет - ссылка на созданный регламентированный отчет с заполненными
//                                            сведениями по умолчанию.
//
Функция СоздатьЭкземплярРСВ1(Организация, НаДату) Экспорт
	
	ИдОтчета = "РегламентированныйОтчетРСВ1";
	
	ОбъектОтчета = РеглОтчеты(ИдОтчета);
	Если НЕ РегламентированнаяОтчетность.ЭтоВнешнийОтчет(ИдОтчета) Тогда
		ОбъектОтчета = ОбъектОтчета.Создать();
	КонецЕсли;
	
	ОсновнаяФормаОтчета = ОбъектОтчета.ПолучитьФорму("ОсновнаяФорма");
	
	ИмяДействующейФормы = ОсновнаяФормаОтчета.ПолучитьФормуДляПериода(НаДату);
	
	ДействующаяФорма = ОбъектОтчета.ПолучитьФорму(ИмяДействующейФормы);
	ДействующаяФорма.Организация = Организация;
	ДействующаяФорма.мДатаКонцаПериодаОтчета = КонецКвартала(НаДату);
	ДействующаяФорма.мДатаНачалаПериодаОтчета = НачалоГода(ДействующаяФорма.мДатаКонцаПериодаОтчета);
	ДействующаяФорма.ДатаПодписи = РабочаяДата;
	
	ДействующаяФорма.Инициализация(Истина);
	ДействующаяФорма.Сохранить();
	
	Возврат ДействующаяФорма.мСохраненныйДок;
	
КонецФункции

// Заполняет сохраненный в информационной базе отчет РСВ-1 по сведениям информационной базы.
//
// Параметры:
//   ЗаполняемыйОтчет - ДокументСсылка.РегламентированныйОтчет - ссылка на сохраненный отчет.
//
Процедура ЗаполнитьЭкземплярРСВ1(ЗаполняемыйОтчет) Экспорт
	
	ЗаполняемаяФорма = ИнициализацияОтчета(ЗаполняемыйОтчет);
	
	ЗаполняемаяФорма.Инициализация(Истина);
	ЗаполняемаяФорма.ЗаполнитьАвто("ЗаполнятьБезПодтверждения");
	ЗаполняемаяФорма.Сохранить();
	
КонецПроцедуры

// Распечатывает сохраненный в информационной базе отчет РСВ-1.
//
// Параметры:
//   СохраненныйРегламентированныйОтчет - ДокументСсылка.РегламентированныйОтчет - ссылка на сохраненный отчет.
//   НаПринтер - Булево - Истина - Печать отчета на принтер
//                        Ложь - показ отчета в форме предварительного просмотра
//
Процедура РаспечататьЭкземплярРСВ1(СохраненныйРегламентированныйОтчет, НаПринтер) Экспорт
	
	Если НаПринтер Тогда
		ВидПечати = "ПечататьСразу";
	Иначе
		ВидПечати = "ПоказатьБланк";
	КонецЕсли;
	
	ЗаполняемаяФорма = ИнициализацияОтчета(СохраненныйРегламентированныйОтчет);
	
	ЗаполняемаяФорма.Инициализация(Истина);
	ЗаполняемаяФорма.Печать(ВидПечати);
	
КонецПроцедуры

// Выгружает электронное представление сохраненного в информационной базе отчета РСВ-1.
//
// Параметры:
//   СохраненныйРегламентированныйОтчет - ДокументСсылка.РегламентированныйОтчет - ссылка на сохраненный отчет.
//
Процедура ВыгрузитьЭкземплярРСВ1(СохраненныйРегламентированныйОтчет) Экспорт
	
	СписокДокументов = Новый СписокЗначений;
	СписокДокументов.Добавить(СохраненныйРегламентированныйОтчет);
	
	ВыгрузитьОтчеты(СписокДокументов);
	
КонецПроцедуры

// Проверяет возможность выгрузки электронного представления сохраненного в информационной базе отчета РСВ-1.
//
// Параметры:
//   СохраненныйРегламентированныйОтчет - ДокументСсылка.РегламентированныйОтчет - ссылка на сохраненный отчет.
//
Процедура ПроверитьВозможностьВыгрузкиЭкземпляраРСВ1(СохраненныйРегламентированныйОтчет) Экспорт
	
	СписокДокументов = Новый СписокЗначений;
	СписокДокументов.Добавить(СохраненныйРегламентированныйОтчет);
	
	РезультатПроверки = ПроверитьРегламентированныеОтчеты(СписокДокументов);
...
Функция ЭтоПБОЮЛ(Организация) Экспорт
	...
	Если ИдКонф = "ББУ" ИЛИ ИдКонф = "БГУ" ИЛИ ИдКонф = "ЗБУ" ИЛИ ЭтоИностраннаяОрганизация(Организация) Тогда
		Возврат Ложь;
	Иначе
...
Функция ЭтоИностраннаяОрганизация(Организация) Экспорт
	...
	Если Организация.Метаданные().Реквизиты.Найти("ИностраннаяОрганизация") <> Неопределено Тогда
		ЭтоИностраннаяОрганизация = Организация.ИностраннаяОрганизация;
	КонецЕсли;
...
Функция ПолучитьСУчетомКэша(Кэш, Параметр, ФизЛицо = Неопределено)
	...
	ИначеЕсли Параметр = "УчетнаяПолитикаНалоговыйУчет" Тогда
		...
		ИначеЕсли ИдКонф = "БГУ" Тогда
			Результат = РегистрыСведений.УчетнаяПолитикаУчреждений.СрезПоследних(Кэш.ДатаЗначения, Новый Структура("Учреждение", Кэш.Организация.Ссылка));
...
Функция ПолучитьСведенияОбОрганизации(Знач Организация, Знач ДатаЗначения = Неопределено, Знач СписокПоказателей = Неопределено) Экспорт
	...
	Если СписокПоказателей = Неопределено Тогда
		...
		СписокПоказателей.Добавить("ЭлектроннаяПочта");
	...
	Для Каждого Показатель Из СписокПоказателей Цикл
		...
		ИначеЕсли ИмяПоказателя = "ТипНП" Тогда
			...
			ИначеЕсли ЭтоИностраннаяОрганизация(Организация) Тогда
				Значение = 5;
			Иначе
		...
		ИначеЕсли ИмяПоказателя = "ОКАТО" Тогда
			...
			Значение = ОКАТО(Организация);
			
		ИначеЕсли ИмяПоказателя = "ОКТМО" Тогда
			
			Значение = ОКТМО(Организация);
		...
		ИначеЕсли ИмяПоказателя = "КодПодчФСС" Тогда
		...
		ИначеЕсли ИмяПоказателя = "КодОрганаФСГС" Тогда
			...
			Если Метаданные.Справочники.Организации.Реквизиты.Найти("КодОрганаФСГС") <> Неопределено Тогда
		...
		ИначеЕсли ИмяПоказателя = "НаимОИОПол" Тогда
			...
			Если ИДКонф = "ББУ" ИЛИ ИДКонф = "БГУ" ИЛИ ИДКонф = "ЗБУ" ИЛИ НЕ ЭтоИностраннаяОрганизация(Организация) Тогда
				Значение = "";
			Иначе
				Значение = Организация.НаименованиеПолное;
			КонецЕсли;
		...
		ИначеЕсли ИмяПоказателя = "КодИОСтрРег" Тогда
			...
			Если ИДКонф <> "ББУ" И ИДКонф <> "БГУ" И ИДКонф <> "ЗБУ" И ЭтоИностраннаяОрганизация(Организация) Тогда
		...
		ИначеЕсли ИмяПоказателя = "ДатаПереходаНаУСН" Тогда
			...
			Если ИДКонф = "ББУ" ИЛИ ИДКонф = "ЗУП" ИЛИ ИДКонф = "ЗБУ" Тогда
				Значение = ПустаяДата;
			Иначе
		...
		ИначеЕсли ИмяПоказателя = "ДолжнИсп" Тогда
			...
			Если ИдКонф = "ББУ" ИЛИ ИдКонф = "БГУ" Тогда
				Если Результат <> Неопределено И Результат.Количество() > 0 Тогда
					СтруктураОтбора = Новый Структура("Сотрудник", Результат[0].ОтветственноеЛицо);
					ДанныеСотрудника = РегистрыСведений.ДанныеСотрудников.СрезПоследних(Кэш.ДатаЗначения, СтруктураОтбора);
					Если ДанныеСотрудника <> Неопределено И ДанныеСотрудника.Количество() > 0 Тогда
						Значение = ДанныеСотрудника[0].Должность;
					Иначе
						Значение = "";
					КонецЕсли;
				Иначе
		...
		ИначеЕсли ИмяПоказателя = "ЭлектроннаяПочта" Тогда
			ЭтоПБОЮЛ = ЭтоПБОЮЛ(Организация);
			Если ИдКонф = "ББУ" ИЛИ ИдКонф = "БГУ" Тогда
				ВидКонтактнойИнформации = Справочники.ВидыКонтактнойИнформации.EmailУчреждения;
				Данные = Новый Структура("Объект, Тип, Вид", Организация.Ссылка, Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты, ВидКонтактнойИнформации);
				Результат2 = РегистрыСведений.КонтактнаяИнформация.Получить(Данные);
				Если Результат2 <> Неопределено Тогда
					Значение = Результат2.Представление;
				Иначе
			...
			Иначе
				Если ЭтоПБОЮЛ Тогда
					ОбъектСведений = Организация.ИндивидуальныйПредприниматель;
					ВидКонтактнойИнформации = Справочники.ВидыКонтактнойИнформации.EmailФизЛица;
				Иначе
					ОбъектСведений = Организация.Ссылка;
					ВидКонтактнойИнформации = Справочники.ВидыКонтактнойИнформации.EmailОрганизации;
				КонецЕсли;
				Данные = Новый Структура("Объект, Тип, Вид", ОбъектСведений, Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты, ВидКонтактнойИнформации);
				Результат2 = РегистрыСведений.КонтактнаяИнформация.Получить(Данные);
				Если Результат2 <> Неопределено Тогда
					Значение = Результат2.Представление;
				Иначе
...
Функция ОКАТО(ИсточникСведений)
	
	Значение = "";
	
	Если ИсточникСведений.Метаданные().Реквизиты.Найти("КодОКАТО") <> Неопределено Тогда
		МетаданныеРеквизит = ИсточникСведений.Метаданные().Реквизиты.Найти("КодОКАТО");
		
		Если МетаданныеРеквизит.Тип.СодержитТип(Тип("Строка")) Тогда
			Значение = СокрЛП(ИсточникСведений.КодОКАТО);
		Иначе
			Значение = СокрЛП(ИсточникСведений.КодОКАТО.Код);
		КонецЕсли;
		
	ИначеЕсли Метаданные.Справочники.Организации.Реквизиты.Найти("КодПоОКАТО") <> Неопределено Тогда
		МетаданныеРеквизит = ИсточникСведений.Метаданные().Реквизиты.Найти("КодПоОКАТО");
		
		Если МетаданныеРеквизит.Тип.СодержитТип(Тип("Строка")) Тогда
			Значение = СокрЛП(ИсточникСведений.КодПоОКАТО);
		Иначе
			Значение = СокрЛП(ИсточникСведений.КодПоОКАТО.Код);
		КонецЕсли;
...
Функция ОКТМО(ИсточникСведений)
	
	МетаданныеРеквизит = ИсточникСведений.Метаданные().Реквизиты.Найти("КодПоОКТМО");
	
	Если МетаданныеРеквизит = Неопределено Тогда
		МетаданныеРеквизит = ИсточникСведений.Метаданные().Реквизиты.Найти("КодОКТМО");
	КонецЕсли;
	
	Если МетаданныеРеквизит = Неопределено Тогда
		Значение = "";
	ИначеЕсли МетаданныеРеквизит.Тип.СодержитТип(Тип("Строка")) Тогда
		Значение = СокрЛП(ИсточникСведений[МетаданныеРеквизит.Имя]);
	Иначе
		Значение = СокрЛП(ИсточникСведений[МетаданныеРеквизит.Имя].Код);
	КонецЕсли;
...
Функция ПолучитьТекстФайла(ДокументОбъект,Отказ) Экспорт
	...
	Если ДокументОбъект.Проведен тогда
		//Для проведенного документа берем сохраненный ранее текст файла 
		Отказ = Ложь;
...
// Возвращает значение счетчика выгруженных файлов отчетности для ПФР.
// Параметры:
//   Организация - СправочникСсылка.Организации - Организация, по которой возвращается значение счетчика
//   Год - Число - год, за который возвращается значение счетчика
// Возвращаемое значение:
//   Число - текущее значение счетчика выгруженных файлов
...
// Устанавливает счетчик выгруженных файлов отчетности для ПФР.
// Параметры:
//   Организация - СправочникСсылка.Организации - Организация, по которой устанавливается счетчик
//   Год - Число - год, за который устанавливается счетчик выгруженных файлов
//   СчФайлов - Число - значение счетчика выгруженных файлов
...
Функция ФИОНаписаноВерно(Знач СтрокаПараметр, ПроверятьТолькоРусскиеБуквы = Ложь) Экспорт
	...
	СписокДопустимыхЗначений.Вставить(КодСимвола("."), ".");
	СписокДопустимыхЗначений.Вставить(КодСимвола("-"), "-");
	СписокДопустимыхЗначений.Вставить(КодСимвола("ё"), "ё"); // отход от стандарта обусловлен читаемостью кода
	СписокДопустимыхЗначений.Вставить(КодСимвола("Ё"), "Ё"); // отход от стандарта обусловлен читаемостью кода
...
Процедура роОткрытьФормуСпискаДопСтраниц(Форма, ИмяСтраницы = Неопределено) Экспорт
	...
	// В процедуру внесены изменения, если на основной панели, есть еще панель
	...
	Если ВыбранноеЗначение <> Неопределено Тогда
		...
		// Если в табличном поле есть многострочность, то ее очистим, а в процедуре
...
Функция роОчистить(Форма, ЗадаватьВопросПередОчисткой = Истина, Знач ТекстВопроса = "") Экспорт
	...
	Иначе
		// Если Вопрос не задается, тогда очистка выполнится однозначно, признак возведем.
		ОчисткаПроизведена = Истина;
	КонецЕсли;
...
Процедура роПроставитьСведенияОПредставителеОрганизации(Форма, ФлагТолькоФЛ, СхемаПолученияКодИФНС = "0") Экспорт
	...
	Если НЕ ((ПустаяСтрока(СтруктураПредставителя.ПредставительЮр)) И (ПустаяСтрока(СтруктураПредставителя.ПредставительФЛ)) И (ПустаяСтрока(СтруктураПредставителя.ДокументПредставителя))) Тогда
		...
		Если (НЕ Форма.мПредставительФизЛицо) И (ФлагТолькоФЛ) Тогда
			...
			// В случае, если в РС все-таки есть данные по представителю, тогда выведем их.
...
// если 1 - то вернем все к ЭТАЛОНу
...
// Возвращает сведения о представителе организации в указанном налоговом органе
//
// Параметры
//  Организация - СправочникСсылка.Организации - организация, по которой необходимо
//    получить сведения.
//  КодНО - Строка - код налогового органа.
//  КПП - Строка - код причины постановки на учет.
// 
// Возвращаемое значение:
//   Структура - сведения о подписанте.
//
Функция ПолучитьПоКодамСведенияОПредставителе(Организация, КодНО, КПП = Неопределено) Экспорт
	...
	КодТиповогоРешения = ИДКонфигурации();
	Если КодТиповогоРешения = "БП" ИЛИ КодТиповогоРешения = "БПКОРП"
	   И ЗначениеЗаполнено(Организация.ГоловнаяОрганизация) Тогда
...
Функция СведенияОРегистрацииВНалоговыхОрганах(Организация) Экспорт
...
Процедура ВыбратьИУстановитьТерриториальныйОрганФНС(Форма, СтандартнаяОбработка, ИмяТитульногоЛиста) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(СправочникИФНС, ТаблИнспекций);

	мТитульный = Форма.ЭлементыФормы["ПолеТабличногоДокумента" + ИмяТитульногоЛиста];
	
	КодИМНС = роВернутьЗначениеПараметраСЛиста(Форма, ИмяТитульногоЛиста, "КодИМНС");
	КПП = роВернутьЗначениеПараметраСЛиста(Форма, ИмяТитульногоЛиста, "КПП1_"); 
...
Процедура ПроверитьНапоминанияПользователяКалендарьБухгалтераСобытия(ВыбПользователь) Экспорт
	...
	Если КолВоДней = 0 Тогда
		Возврат;
	КонецЕсли;
	ТекРабочаяДата = ОбщегоНазначенияЗК.ПолучитьРабочуюДату();
...
Функция ПолучитьСсылкуНаОтчетФормы(Форма) Экспорт
...
Процедура ПередОткрытиемОсновнойФормыРегламентиованногоОтчета(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Форма.ВладелецФормы <> Неопределено Тогда
		Если Не Форма.ВладелецФормы.ЭтоНовый() Тогда
			...
			Если Форма.ВладелецФормы.ИсточникОтчета = "РегламентированныйОтчетСтатистикаПрочиеФормы" Тогда
				ВыбФорма = ?(Найти(ВыбраннаяФорма, "_") = 0, ВыбраннаяФорма, Лев(ВыбраннаяФорма, Найти(ВыбраннаяФорма, "_") - 1));
			Иначе
				ВыбФорма = ВыбраннаяФорма;
			КонецЕсли;
			
			//УжеОткрытаяФорма = Отчеты[ЭтотОбъект.Метаданные().Имя].ПолучитьФорму(?(ПустаяСтрока(ВыбФорма), ПолучитьФормуДляПериода(мСохраненныйДок.ДатаОкончания), СокрЛП(ВыбФорма)), , мСохраненныйДок.Ссылка);
			УжеОткрытаяФорма = ФормаРеглОтчета(Форма.ЭтотОбъект.Метаданные().Имя, СокрЛП(ВыбФорма), , Форма.мСохраненныйДок.Ссылка);
			...
			Если Форма.ЭтотОбъект.Метаданные().Формы.Найти(СокрП(ВыбФорма)) <> Неопределено Тогда
				ВыбФормаОтчета = Форма.ПолучитьФорму(СокрП(ВыбФорма), , Форма.мСохраненныйДок.Ссылка);
			Иначе
...
Процедура ПослеЗаписиРегламентированногоОтчета(Форма, КОПолучатель = Неопределено) Экспорт
	...
	Попытка
		...
		Если КонтекстЭДО <> Неопределено Тогда
			...
			ИначеЕсли КОПолучатель = "ФСГС" Тогда
				КонтекстЭДО.ОбновитьСтатусОтправкиВРегламентированномОтчетеФСГС(Форма);
			Иначе
...
Функция роПолучитьОбщуюФорму(ИмяФормы, Владелец = Неопределено, КлючУникальности = Неопределено) Экспорт
	...
	Обработка = Метаданные.Обработки.ОбщиеОбъектыРегламентированнойОтчетности;
	Если Обработка.Формы.Найти(ИмяФормы) <> Неопределено Тогда
		Если ПравоДоступа("Использование", Обработка) Тогда 
...
Функция ИнициализацияОтчета(Док, Форма = Неопределено)
	...
	// Для совместимости с универсальным отчетом статистики
	Если Док.ИсточникОтчета = "РегламентированныйОтчетСтатистикаПрочиеФормы" Тогда
		ВыбраннаяФорма = ?(Найти(ВыбраннаяФорма, "_") = 0, ВыбраннаяФорма, Лев(ВыбраннаяФорма, Найти(ВыбраннаяФорма, "_") - 1));
	КонецЕсли;
...
Функция ВыбратьФормуОтчетаИзДействующегоСписка(Форма) Экспорт
	...
	// Если в таблице есть колонка нарастающий итог, то удалим ее
	...
	// Если в таблице есть колонка НаименованиеОтчета, то удалим ее
	// используется в декларациях по алкогольной продукции.
...
Функция ВывестиМашиночитаемуюФорму(Форма, ИмяКнопки) Экспорт
	...
	Если СвойствоОпределено(Форма, "мПроверятьСоотношенияПриПечатиИВыгрузки")
		И Форма.мПроверятьСоотношенияПриПечатиИВыгрузки <> Неопределено И Форма.мПроверятьСоотношенияПриПечатиИВыгрузки Тогда
		ПроверитьКонтрольныеСоотношенияВОтчете(Форма, Форма.мСохраненныйДок, Форма.мБезОткрытияФормы);
	КонецЕсли;
...
//  2 - "новый"
//      (формат ПО Росстата «Заполнение форм статистической отчётности», версии 2.4.4);
//  1 - "старый"
...
Процедура ПоказатьФормуНастроекФорматаВыгрузкиСтатистическойОтчетностиВЭлектронномВиде(Организация, ОрганизацияДоступна = Истина) Экспорт
	
	ФормаВыбора = Обработки.ОбщиеОбъектыРегламентированнойОтчетности.ПолучитьФорму("НастройкиФорматаВыгрузкиОтчетовСтатистики");
	ФормаВыбора.Организация = Организация;
	ФормаВыбора.ОрганизацияДоступна = ОрганизацияДоступна;
...
Функция ПолучитьМассивФормСтатистикиТолькоДляИП()
	Результат = Новый Массив;
	Результат.Добавить("РегламентированныйОтчетСтатистикаФорма1ИП");
...
Функция ВсеЮрФизЛица(ЮрЛицо) Экспорт 
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Если Не (РегламентированнаяОтчетность.ЭтоПБОЮЛ(Выборка.Ссылка) = ЮрЛицо) Тогда 
...
Процедура УстановитьОтборВФормеВыбораОтчета(Форма) Экспорт 
	ИмяОтчета = Строка(Форма.ОбработкаОбъект);
	Если Найти(ИмяОтчета, "Отчет.РегламентированныйОтчетСтатистика") Тогда 
		Возврат;
	КонецЕсли;
	
	ИмяОтчета = Сред(ИмяОтчета, Найти(ИмяОтчета, ".") + 1);
	...
	Если ПолучитьМассивФормСтатистикиТолькоДляОрганизации().Найти(ИмяОтчета) <> Неопределено Тогда
		...
		ПолеОрганизация = Форма.ЭлементыФормы.Организация;
		ПолеОрганизация.РежимВыбораИзСписка = Истина;
		ПолеОрганизация.КнопкаВыбора = Ложь;
		ПолеОрганизация.СписокВыбора.Очистить();
		Для Каждого Элт Из МассивОтбора Цикл
			ПолеОрганизация.СписокВыбора.Добавить(Элт);
		КонецЦикла;
		
		Если МассивОтбора.Найти(Форма.Организация) = Неопределено Тогда
			Форма.Организация = Неопределено;
			Форма.ЭлементыФормы.Организация.Значение = Неопределено;
		КонецЕсли;
...
Процедура ПоказатьФормуПредложениеОформитьЗаявлениеНаПодключение(Организация = Неопределено) Экспорт
	...
	ФормаИнформации = Обработки.ОбщиеОбъектыРегламентированнойОтчетности.ПолучитьФорму("ПредложениеОформитьЗаявлениеНаПодключение");
	
	ФормаИнформации.Организация = Организация;
	
	ФормаИнформации.ОткрытьМодально();
...
Процедура УстановкаСтатусаПриОткрытииРегламентированногоОтчета(Форма) Экспорт
	...
	Если Форма.ЭлементыФормы.Найти("Статус") <> Неопределено И Форма.ЭлементыФормы.Статус.Значение = Перечисления.СтатусыРегламентированныхОтчетов.ПустаяСсылка() Тогда
		...
		Если СсылкаНаОтчет <> Неопределено Тогда
			Форма.ЭлементыФормы.Статус.Значение = ПолучитьСтатусРегламентированногоОтчета(СсылкаНаОтчет);
		Иначе
...
Процедура ПриИнициализацииФормыРегламентированногоОтчета(Форма, КОПолучатель = Неопределено, Действие = Неопределено, ЭлектронныеПредставления = Ложь) Экспорт
	
	Если КОПолучатель = "ПФР" Тогда
		
		УправлениеВидимостьюЭлементовОтправкиРегламентированногоОтчетаПФР(Форма, Неопределено, ЭлектронныеПредставления);
	...
	ИначеЕсли КОПолучатель = "ФСГС" Тогда
		...
		УправлениеВидимостьюЭлементовОтправкиРегламентированногоОтчетаФСГС(Форма, Неопределено, ЭлектронныеПредставления);
		УстановкаСтатусаПриОткрытииРегламентированногоОтчета(Форма);
	...
	ИначеЕсли КОПолучатель = "ФСС" Тогда
		
		УстановкаСтатусаПриОткрытииРегламентированногоОтчета(Форма);
		УправлениеВидимостьюЭлементовОтправкиРегламентированногоОтчетаФСС(Форма, ЭлектронныеПредставления);
		
		Если КонтекстЭДОЛокальный() <> Неопределено Тогда
			КонтекстЭДОЛокальный().ОбновитьСтатусОтправкиВРегламентированномОтчетеФСС(Форма);
			КонтекстЭДОЛокальный().ОбновитьМенюОтправкиФСС(Форма, Действие);
		КонецЕсли;
		
	ИначеЕсли КОПолучатель = "ФСРАР" Тогда
		
		УстановкаСтатусаПриОткрытииРегламентированногоОтчета(Форма);
		УправлениеВидимостьюЭлементовОтправкиРегламентированногоОтчетаФСРАР(Форма, ЭлектронныеПредставления);
		
		Если КонтекстЭДОЛокальный() <> Неопределено Тогда
			КонтекстЭДОЛокальный().ОбновитьСтатусОтправкиВРегламентированномОтчетеФСРАР(Форма);
			КонтекстЭДОЛокальный().ОбновитьМенюОтправкиФСРАР(Форма, Действие);
		Иначе
			СкрытьПодменюОтправкиВФормеРегламентированногоОтчетаФСРАР(Форма);
			СкрытьПанельОтправкиВФормеРегламентированногоОтчетаФСРАР(Форма);
		КонецЕсли;
		
	ИначеЕсли КОПолучатель = "РПН" Тогда
		
		УстановкаСтатусаПриОткрытииРегламентированногоОтчета(Форма);
		УправлениеВидимостьюЭлементовОтправкиРегламентированногоОтчетаРПН(Форма, ЭлектронныеПредставления);
		
		Если КонтекстЭДОЛокальный() <> Неопределено Тогда
			КонтекстЭДОЛокальный().ОбновитьСтатусОтправкиВРегламентированномОтчетеРПН(Форма);
			КонтекстЭДОЛокальный().ОбновитьМенюОтправкиРПН(Форма, Действие);
		Иначе
			СкрытьПодменюОтправкиВФормеРегламентированногоОтчетаРПН(Форма);
			СкрытьПанельОтправкиВФормеРегламентированногоОтчетаРПН(Форма);
		КонецЕсли;
		
	Иначе
		
		УправлениеВидимостьюЭлементовОтправкиРегламентированногоОтчета(Форма, ЭлектронныеПредставления);
		УстановкаСтатусаПриОткрытииРегламентированногоОтчета(Форма);
...
Процедура ОбновитьСтатусОтправкиВРегламентированномОтчетеФСС(Форма) Экспорт
	...
	СсылкаНаОтчет = РегламентированнаяОтчетность.ПолучитьСсылкуНаОтчетФормы(Форма);
	
	РеквизитыОтправки = Неопределено;
	СтатусТекущейОтправки = Неопределено;
	Если ЗначениеЗаполнено(СсылкаНаОтчет) Тогда
		РеквизитыОтправки = ПолучитьРеквизитыОтправкиФСС(СсылкаНаОтчет);
	...
	Если СтатусТекущейОтправки = Неопределено Тогда
		ТекстПанели = "Отчет не отправлялся.";
	Иначе
		ТекстПанели = "Отчет отправлен " + Формат(РеквизитыОтправки.ДатаОтправки, "ДЛФ=DDT") + ".";
		
		
		Если СтатусТекущейОтправки = Перечисления.СтатусыОтправки.НеПринят Тогда
			ИмеютсяОшибки = Истина;
			ТекстПанели = ТекстПанели + " Отчет не принят: получен протокол ошибок.";
			ПоказыватьКнопкуПротокола = Истина;
	...
	Если ЗначениеЗаполнено(ТекстПанели) Тогда
		...
		КнопкаОткрытьПротоколОтправки = Форма.ЭлементыФормы.Найти("КнопкаОткрытьПротоколОтправки");
		Если КнопкаОткрытьПротоколОтправки <> Неопределено Тогда
			КнопкаОткрытьПротоколОтправки.ЦветТекстаКнопки = ЦветШрифта;
		КонецЕсли;
		ПанельКнопкиОткрытьПротоколОтправки = Форма.ЭлементыФормы.Найти("ПанельКнопкиОткрытьПротоколОтправки");
		Если ПанельКнопкиОткрытьПротоколОтправки <> Неопределено Тогда
			Если ПоказыватьКнопкуПротокола Тогда
				ПанельКнопкиОткрытьПротоколОтправки.Свертка = РежимСверткиЭлементаУправления.Нет
			Иначе
				ПанельКнопкиОткрытьПротоколОтправки.Свертка = РежимСверткиЭлементаУправления.Право;
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьРеквизитыОтправкиФСС(ОбъектФСС, Сданными = Ложь) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               ...
	               |ГДЕ
	               |	ОтправкиФСС.ОтчетСсылка = &ОбъектФСС
	               |	И ОтправкиФСС.ПометкаУдаления = ЛОЖЬ
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ОтправкиФСС.ДатаОтправки УБЫВ";
	Запрос.Параметры.Вставить("ОбъектФСС", ОбъектФСС);
...
Процедура ПриНажатииНаКнопкуОтправкиВРегламентированномОтчете(Форма, Кнопка) Экспорт
	...
	Если Организация.ВидОбменаСКонтролирующимиОрганами <> Перечисления.ВидыОбменаСКонтролирующимиОрганами.ОбменВУниверсальномФормате Тогда
		Если ИмяКнопки = "Отправить" Тогда
			РегламентированнаяОтчетность.ПоказатьФормуПредложениеОформитьЗаявлениеНаПодключение(Организация);
			Возврат;	
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриНажатииНаКнопкуОтправкиВОбъектеОтправкиВФНС(Форма, Кнопка) Экспорт
	...
	// определяем действие
	Если ИмяКнопки = "ПоказатьЦиклыОбмена" Тогда
		КонтекстЭДО.ПоказатьЦиклыОбменаОбъектаОтправкиВФНС(Форма.Ссылка);
		Возврат;
	ИначеЕсли ИмяКнопки = "Подготовить" Тогда
		КодДействия = 0;
	ИначеЕсли ИмяКнопки = "ПодготовитьИПодписать" Тогда
...
Процедура ОбработкаОповещенияОбИзмененииСтатусаРегламентированногоОтчета(Форма, ИмяСобытия, Параметр, Источник, КОПолучатель = Неопределено) Экспорт
	...
	Если ИмяСобытия = "Изменение статуса регламентированного отчета" И ЗначениеЗаполнено(Параметр) И Источник = ПолучитьСсылкуНаОтчетФормы(Форма) Тогда
	...
	Если КОПолучатель = "ФСС" Тогда
		Если КонтекстЭДОЛокальный() <> Неопределено Тогда
			КонтекстЭДОЛокальный().ОбновитьСтатусОтправкиВРегламентированномОтчетеФСС(Форма);
			КонтекстЭДОЛокальный().ОбновитьМенюОтправкиФСС(Форма);
		КонецЕсли;
	ИначеЕсли КОПолучатель = "ФСРАР" Тогда
		Если КонтекстЭДОЛокальный() <> Неопределено Тогда
			КонтекстЭДОЛокальный().ОбновитьСтатусОтправкиВРегламентированномОтчетеФСРАР(Форма);
			КонтекстЭДОЛокальный().ОбновитьМенюОтправкиФСРАР(Форма);
		КонецЕсли;
	ИначеЕсли КОПолучатель = "РПН" Тогда
		Если КонтекстЭДОЛокальный() <> Неопределено Тогда
			КонтекстЭДОЛокальный().ОбновитьСтатусОтправкиВРегламентированномОтчетеРПН(Форма);
			КонтекстЭДОЛокальный().ОбновитьМенюОтправкиРПН(Форма);
		КонецЕсли;
	ИначеЕсли КонтекстЭДО <> Неопределено Тогда
		...
		ИначеЕсли КОПолучатель = "ФСГС" Тогда
			КонтекстЭДО.ОбработкаОповещенияФормыРегламентированногоОтчетаФСГС(Форма, ИмяСобытия, Параметр, Источник);
		Иначе
...
Процедура УправлениеВидимостьюЭлементовОтправкиРегламентированногоОтчета(Форма, ЭлектронныеПредставления = Ложь)
	...
	КоманднаяПанельФормы = Форма.ЭлементыФормы.Найти("КоманднаяПанельФормы");
	Если КоманднаяПанельФормы = Неопределено Тогда
		//Если ЭлектронныеПредставления Тогда
			КоманднаяПанельФормы = Форма.ЭлементыФормы.Найти("ДействияФормы");
		//КонецЕсли;
		Если КоманднаяПанельФормы = Неопределено Тогда
			Возврат;
		КонецЕсли;
	...
	Если ПодменюОтправка = Неопределено И НЕ ЭлектронныеПредставления Тогда
		Возврат;
	КонецЕсли;
	
	//решено скрыть кнопки "Подготовить" и "ПодготовитьИПодписать"
	КнопкаПодготовитьИПодписать = ПодменюОтправка.Кнопки.Найти("ПодготовитьИПодписать");
	Если КнопкаПодготовитьИПодписать <> Неопределено Тогда
		ПодменюОтправка.Кнопки.Удалить(КнопкаПодготовитьИПодписать);
	КонецЕсли;
	
	КнопкаПодготовить = ПодменюОтправка.Кнопки.Найти("Подготовить");
	Если КнопкаПодготовить <> Неопределено Тогда
		ПодменюОтправка.Кнопки.Удалить(КнопкаПодготовить);
	КонецЕсли;
	
	РазделительПодготовить = ПодменюОтправка.Кнопки.Найти("Разделитель");
	Если РазделительПодготовить <> Неопределено Тогда
		ПодменюОтправка.Кнопки.Удалить(РазделительПодготовить);
	КонецЕсли;
	...
	Иначе
		Попытка
			...
			ДоступКУчетнойЗаписиЕсть = (ОбъектУчетнаяЗапись <> Неопределено);
		Исключение
			ДоступКУчетнойЗаписиЕсть = Ложь;
		КонецПопытки;
	КонецЕсли;
	...
	ИЛИ НЕ УчетнаяЗаписьПредназначенаДляДокументооборотаСФНС(УчетнаяЗаписьОбмена) Тогда
		...
		Если ПодменюОтправка <> Неопределено Тогда
			// удаляем подменю "Отправка" (удаляем все пункты кроме "Отправить)
			Если ПодменюОтправка <> Неопределено Тогда
				
				МассивУдаляемыхКнопок = Новый Массив;
				
				Для каждого КнопкаПодменю Из ПодменюОтправка.Кнопки Цикл
					Если КнопкаПодменю.Имя <> "Отправить" Тогда
						МассивУдаляемыхКнопок.Добавить(КнопкаПодменю);
					КонецЕсли;
				КонецЦикла; 
				
				Для каждого УдаляемаяКнопка Из МассивУдаляемыхКнопок Цикл
					ПодменюОтправка.Кнопки.Удалить(УдаляемаяКнопка);
				КонецЦикла;
	...
	Иначе
		...
		//Если ЭлектронныеПредставления Тогда
		//	ПоказатьПодменюОтправкаВЭлектронномПредставлении(Форма, ПодменюОтправка);
		//КонецЕсли;
		
		Если НЕ ПравоДоступа("Чтение", Метаданные.Справочники.УчетныеЗаписиДокументооборота) ИЛИ НЕ ЕстьДоступКУчетнойЗаписи(УчетнаяЗаписьОбмена) ИЛИ УчетнаяЗаписьОбмена.ОбменНапрямую ИЛИ НЕ УчетнаяЗаписьОбмена.ИспользоватьСервисОнлайнПроверкиОтчетов
			ИЛИ (УчетнаяЗаписьОбмена.СпецоператорСвязи <> Перечисления.СпецоператорыСвязи.Такском И (КонтекстЭДО = Неопределено
			ИЛИ Найти(";" + ВРег(СокрЛП(КонтекстЭДО.ПолучитьПараметрСпецоператора(УчетнаяЗаписьОбмена.СпецоператорСвязи, "ОнлайнПроверкаКонтролирующиеОрганы"))) + ";", ";ФНС;") = 0)) Тогда
...
Процедура ПоказатьПанельОтправкиВФормеРегламентированногоОтчета(Форма)
...
Процедура ПоказатьПодменюОтправкаВЭлектронномПредставлении(Форма, ПодменюОтправка = Неопределено)
	...
	Если ПодменюОтправка = Неопределено Тогда
		...
		Иначе
			...
			ДействияФормы.Кнопки.Вставить(ИндексКнопкиПерейти + 1, "РазделительОтправка", ТипКнопкиКоманднойПанели.Разделитель);
			ПодменюОтправка = ДействияФормы.Кнопки.Вставить(ИндексКнопкиПерейти + 2, "Отправка", ТипКнопкиКоманднойПанели.Подменю, "Отправка");
			ПодменюОтправка.Картинка = БиблиотекаКартинок.ДокументооборотСКонтролирующимиОрганамиТемныйФон;
			ПодменюОтправка.Отображение = ОтображениеКнопкиКоманднойПанели.НадписьКартинка;
		КонецЕсли;
...
Процедура ПриОткрытииФормыРегламентированногоОтчета2НДФЛ(Форма, Действие = Неопределено, ЭлектронныеПредставления = Ложь) Экспорт
	
	УправлениеВидимостьюЭлементовОтправкиРегламентированногоОтчета2НДФЛ(Форма, Действие, ЭлектронныеПредставления);
	
КонецПроцедуры

Процедура УправлениеВидимостьюЭлементовОтправкиРегламентированногоОтчета2НДФЛ(Форма, Действие = Неопределено, ЭлектронныеПредставления = Ложь)
	...
	ИЛИ НЕ УчетнаяЗаписьПредназначенаДляДокументооборотаСФНС(УчетнаяЗаписьОбмена) Тогда
	    ПрорисоватьПодменюОтправкаВРегламентированномОтчете2НДФЛ(Форма, Действие, ЭлектронныеПредставления);
	...
	Иначе
		ПрорисоватьПодменюОтправкаВРегламентированномОтчете2НДФЛ(Форма, Действие, ЭлектронныеПредставления);
		Если ЭлектронныеПредставления Тогда
			ПоказатьПанельОтправкиВФормеРегламентированногоОтчета(Форма);
		КонецЕсли;
...
Процедура ПрорисоватьПодменюОтправкаВРегламентированномОтчете2НДФЛ(Форма, Действие = Неопределено, ЭлектронныеПредставления = Ложь)
	...
	Если ПравоДоступа("Чтение", Метаданные.Справочники.УчетныеЗаписиДокументооборота) И ЕстьДоступКУчетнойЗаписи(УчетнаяЗаписьОбмена) И НЕ УчетнаяЗаписьОбмена.ОбменНапрямую И УчетнаяЗаписьОбмена.ИспользоватьСервисОнлайнПроверкиОтчетов
		И (УчетнаяЗаписьОбмена.СпецоператорСвязи = Перечисления.СпецоператорыСвязи.Такском ИЛИ (КонтекстЭДО <> Неопределено
		И Найти(";" + ВРег(СокрЛП(КонтекстЭДО.ПолучитьПараметрСпецоператора(УчетнаяЗаписьОбмена.СпецоператорСвязи, "ОнлайнПроверкаКонтролирующиеОрганы"))) + ";", ";ФНС;") <> 0)) Тогда
...
Процедура ПриНажатииНаКнопкуОтправкиВУведомленииОКонтролируемыхСделках(Форма, ИмяКоманды = "") Экспорт
	
	// записываем при необходимости
	Если Форма.Параметры.Ключ.Пустая() ИЛИ Форма.Модифицированность Тогда
		
		Если ИмяКоманды = "ПроверитьВИнтернете" Тогда
			Предупреждение("Перед проверкой необходимо записать уведомление.");
		Иначе
			Предупреждение("Перед отправкой необходимо записать уведомление.");
		КонецЕсли;
		Возврат;
		
	КонецЕсли;
	
	УведомлениеСсылка = Форма.Объект.Ссылка;
	
	Если ИмяКоманды <> "ПроверитьВИнтернете" Тогда
		
		Организация = УведомлениеСсылка.Организация;
	...
	Если ИмяКоманды = "ПроверитьВИнтернете" Тогда
		КонтекстЭДО.ПроверитьОтчетСИспользованиемСервисаОнлайнПроверки(УведомлениеСсылка);
	Иначе
		// отправляем
		КонтекстЭДО.ОтправкаУведомленияОКонтролируемыхСделкахВФНС(УведомлениеСсылка);
	КонецЕсли;
	
КонецПроцедуры

Функция ОбновитьПанельСостоянияОтправкиВУведомленииОКонтролируемыхСделках(Форма) Экспорт
	
	УведомлениеСсылка = Форма.Объект.Ссылка;
	Организация = Форма.Объект.Организация;
	УчетнаяЗаписьДокументооборота = Организация.УчетнаяЗаписьОбмена;
	ПредназначенаДляДокументооборотаСКонтролирующимОрганом = Истина;
	
	Если Организация.ВидОбменаСКонтролирующимиОрганами <> ПредопределенноеЗначение("Перечисление.ВидыОбменаСКонтролирующимиОрганами.ОбменВУниверсальномФормате")
		ИЛИ НЕ ЗначениеЗаполнено(УчетнаяЗаписьДокументооборота) ИЛИ УчетнаяЗаписьПредназначенаДляДокументооборотаСФНС(УчетнаяЗаписьДокументооборота) <> Истина Тогда
		ПредназначенаДляДокументооборотаСКонтролирующимОрганом = Ложь;
	КонецЕсли;
	
	ПараметрыПрорисовки = ПолучитьПараметрыПрорисовкиПанелиОтправкиВУведомленииОКонтролируемыхСделках(УведомлениеСсылка, Организация, УчетнаяЗаписьДокументооборота, ПредназначенаДляДокументооборотаСКонтролирующимОрганом);
	ПрименитьПараметрыПрорисовкиПанелиОтправкиВУведомленииОКонтролируемыхСделках(Форма, ПараметрыПрорисовки);
	
	// управление видимостью кнопки ПроверитьВИнтернете
	ПодменюОтправка = Форма.Элементы.Найти("Отправка");
	Если ПодменюОтправка <> Неопределено Тогда
		ГруппаКнопокОтправкаВКонтролирующийОрган = ПодменюОтправка.ПодчиненныеЭлементы.Найти("ГруппаОтправкаВКонтролирующийОрган");
		Если ГруппаКнопокОтправкаВКонтролирующийОрган <> Неопределено Тогда
			КнопкаПроверитьВИнтернете = ГруппаКнопокОтправкаВКонтролирующийОрган.ПодчиненныеЭлементы.Найти("ПроверитьВИнтернете");
			
			Если КнопкаПроверитьВИнтернете <> Неопределено Тогда
				
				КнопкаПроверитьВИнтернете.Видимость = ПредназначенаДляДокументооборотаСКонтролирующимОрганом И УчетнаяЗаписьДокументооборота.ИспользоватьСервисОнлайнПроверкиОтчетов
					И (УчетнаяЗаписьДокументооборота.СпецоператорСвязи = Перечисления.СпецоператорыСвязи.Такском ИЛИ (КонтекстЭДО <> Неопределено
					И Найти(";" + ВРег(СокрЛП(КонтекстЭДО.ПолучитьПараметрСпецоператора(УчетнаяЗаписьДокументооборота.СпецоператорСвязи, "ОнлайнПроверкаКонтролирующиеОрганы"))) + ";", ";ФНС;") > 0));
...
Функция ПолучитьПараметрыПрорисовкиПанелиОтправкиВУведомленииОКонтролируемыхСделках(УведомлениеСсылка, Организация, УчетнаяЗаписьДокументооборота, ПредназначенаДляДокументооборотаСКонтролирующимОрганом)
	...
	Возврат КонтекстЭДО.ПолучитьПараметрыПрорисовкиПанелиОтправкиВУведомленииОКонтролируемыхСделках(УведомлениеСсылка, Организация, УчетнаяЗаписьДокументооборота, ПредназначенаДляДокументооборотаСКонтролирующимОрганом);

КонецФункции

Процедура ПрименитьПараметрыПрорисовкиПанелиОтправкиВУведомленииОКонтролируемыхСделках(Форма, ПараметрыПрорисовки) 
	...
	// находим надпись
	НадписьСостояниеОтправки = ГруппаПанельОтправки.ПодчиненныеЭлементы.Найти("НадписьСостояниеОтправки");
	Если НадписьСостояниеОтправки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	// находим кнопку открытия протокола
	КнопкаОткрытьПротоколОтправки = ГруппаПанельОтправки.ПодчиненныеЭлементы.Найти("КнопкаОткрытьПротоколОтправки");

	// извлекаем свойства из структуры параметров прорисовки
	ВидимостьПанели = ?(ТипЗнч(ПараметрыПрорисовки.ВидимостьПанели) = Тип("Булево"), ПараметрыПрорисовки.ВидимостьПанели, Ложь);
	ВидимостьКнопкиПротокола = ?(ТипЗнч(ПараметрыПрорисовки.ВидимостьКнопкиПротокола) = Тип("Булево"), ПараметрыПрорисовки.ВидимостьКнопкиПротокола, Ложь);
	ТекстНадписи = ?(ТипЗнч(ПараметрыПрорисовки.ТекстНадписи) = Тип("Строка"), ПараметрыПрорисовки.ТекстНадписи, "");
	ЦветТекстаНадписи = ?(ТипЗнч(ПараметрыПрорисовки.ЦветТекстаНадписи) = Тип("Цвет"), ПараметрыПрорисовки.ЦветТекстаНадписи, Новый Цвет);
	
	// присваиваем значения из параметров
	ГруппаПанельОтправки.Видимость = ВидимостьПанели;
	НадписьСостояниеОтправки.Заголовок = ТекстНадписи;
	НадписьСостояниеОтправки.ЦветТекста = ЦветТекстаНадписи;
	Если КнопкаОткрытьПротоколОтправки <> Неопределено Тогда
		КнопкаОткрытьПротоколОтправки.Видимость = ВидимостьКнопкиПротокола;
		КнопкаОткрытьПротоколОтправки.ЦветТекста = ЦветТекстаНадписи;
	КонецЕсли;
...
Процедура УправлениеВидимостьюЭлементовОтправкиРегламентированногоОтчетаПФР(Форма, Действие = Неопределено, ЭлектронныеПредставления = Ложь)
	...
	ИЛИ НЕ УчетнаяЗаписьПредназначенаДляДокументооборотаСПФР(УчетнаяЗаписьОбмена) Тогда
	    ПрорисоватьПодменюОтправкаВРегламентированномОтчетеПФР(Форма, Действие, ЭлектронныеПредставления);
...
Функция УчетнаяЗаписьПредназначенаДляДокументооборотаСПФР(УчетнаяЗапись) Экспорт
	...
	Исключение
		УчетнаяЗаписьОбъект = Неопределено;
	КонецПопытки;
	Если УчетнаяЗаписьОбъект = Неопределено Тогда // к объекту нет доступа или объект удален
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПрорисоватьПодменюОтправкаВРегламентированномОтчетеПФР(Форма, Действие = Неопределено, ЭлектронныеПредставления = Ложь)
	...
	Если НЕ СвойствоОпределено(Форма, "мСохраненныйДок") Тогда
		...
		Если ПравоДоступа("Чтение", Метаданные.Справочники.УчетныеЗаписиДокументооборота) И ЕстьДоступКУчетнойЗаписи(УчетнаяЗаписьОбмена) И НЕ УчетнаяЗаписьОбмена.ОбменНапрямую И УчетнаяЗаписьОбмена.ИспользоватьСервисОнлайнПроверкиОтчетов
			И (УчетнаяЗаписьОбмена.СпецоператорСвязи = Перечисления.СпецоператорыСвязи.Такском ИЛИ (КонтекстЭДО <> Неопределено
			И Найти(";" + ВРег(СокрЛП(КонтекстЭДО.ПолучитьПараметрСпецоператора(УчетнаяЗаписьОбмена.СпецоператорСвязи, "ОнлайнПроверкаКонтролирующиеОрганы"))) + ";", ";ПФР;") <> 0)) Тогда
			Если ПодменюОтправка.Кнопки.Найти("Проверить") = Неопределено Тогда
				КнопкаПроверить = ПодменюОтправка.Кнопки.Вставить(0, "Проверить", ТипКнопкиКоманднойПанели.Действие, "Проверить в Интернете", ДействиеОтправка);
				КнопкаПроверить.Картинка = БиблиотекаКартинок.СинтаксическийКонтроль;
			КонецЕсли;
		...
		Если Форма.Метаданные().Реквизиты.Найти("РасчетРСВ1") <> Неопределено Тогда
			...
			КонецЕсли;
			ПодменюОтправка.Кнопки.ПоказатьРСВ.Доступность = ЗначениеЗаполнено(Форма.РасчетРСВ1);
	...
	Иначе
		...
		// регулируем видимость кнопки Проверить
		Если ПравоДоступа("Чтение", Метаданные.Справочники.УчетныеЗаписиДокументооборота) 
			И ЕстьДоступКУчетнойЗаписи(УчетнаяЗаписьОбмена)
			И УчетнаяЗаписьОбмена.ИспользоватьСервисОнлайнПроверкиОтчетов Тогда
			Если ПодменюОтправка.Кнопки.Найти("Проверить") = Неопределено Тогда
				КнопкаПроверить = ПодменюОтправка.Кнопки.Вставить(0, "Проверить", ТипКнопкиКоманднойПанели.Действие, "Проверить в Интернете", ДействиеОтправка);
				КнопкаПроверить.Картинка = БиблиотекаКартинок.СинтаксическийКонтроль;
			КонецЕсли;
...
Функция ЕстьДоступКУчетнойЗаписи(УчетнаяЗапись) Экспорт
	
	Если Метаданные.РегистрыСведений.Найти("ПользователиУчетныхЗаписейДокументооборота") = Неопределено Тогда
	...
	Иначе
		...
		Если ПравоДоступа("Чтение", Метаданные.РегистрыСведений.ПользователиУчетныхЗаписейДокументооборота) Тогда
			...
			|	И ПользователиУчетныхЗаписейДокументооборота.УчетнаяЗапись = &УчетнаяЗапись");
			...
			ЕстьДоступ = Выборка.Количество() > 0;
...
Процедура СкрытьПанельОтправкиВФормеРегламентированногоОтчетаФСГС(Форма)
	
	СкрытьПанельОтправкиВФормеРегламентированногоОтчета(Форма); // !!!
	
КонецПроцедуры

Процедура УправлениеВидимостьюЭлементовОтправкиРегламентированногоОтчетаФСГС(Форма, Действие = Неопределено, ЭлектронныеПредставления = Ложь)
	...
	Если НЕ ДоступКУчетнойЗаписиЕсть
	ИЛИ НЕ ОбменЗадействован
	ИЛИ НЕ УчетнаяЗаписьПредназначенаДляДокументооборотаСФСГС(УчетнаяЗаписьОбмена) Тогда
	    ПрорисоватьПодменюОтправкаВРегламентированномОтчетеФСГС(Форма, Действие, ЭлектронныеПредставления);
...
Функция УчетнаяЗаписьПредназначенаДляДокументооборотаСФСГС(УчетнаяЗапись) Экспорт
	...
	Возврат УчетнаяЗаписьОбъект.ПредназначенаДляДокументооборотаСФСГС;
	
КонецФункции

Процедура СкрытьПодменюОтправкиВФормеРегламентированногоОтчетаФСГС(Форма)
...
Процедура ПрорисоватьПодменюОтправкаВРегламентированномОтчетеФСГС(Форма, Действие = Неопределено, ЭлектронныеПредставления = Ложь)
	...
	// регулируем видимость кнопки Проверить
	Если ПравоДоступа("Чтение", Метаданные.Справочники.УчетныеЗаписиДокументооборота) И ЕстьДоступКУчетнойЗаписи(УчетнаяЗаписьОбмена) И НЕ УчетнаяЗаписьОбмена.ОбменНапрямую И УчетнаяЗаписьОбмена.ИспользоватьСервисОнлайнПроверкиОтчетов
		И УчетнаяЗаписьОбмена.СпецоператорСвязи <> Перечисления.СпецоператорыСвязи.Такском И КонтекстЭДО <> Неопределено
		И Найти(";" + ВРег(СокрЛП(КонтекстЭДО.ПолучитьПараметрСпецоператора(УчетнаяЗаписьОбмена.СпецоператорСвязи, "ОнлайнПроверкаКонтролирующиеОрганы"))) + ";", ";ФСГС;") <> 0 Тогда //для спецоператора Такском для статотчетности сервис не используем, пока
		Если ПодменюОтправка.Кнопки.Найти("Проверить") = Неопределено Тогда
			КнопкаПроверить = ПодменюОтправка.Кнопки.Вставить(0, "Проверить", ТипКнопкиКоманднойПанели.Действие, "Проверить в Интернете", ДействиеОтправка);
			КнопкаПроверить.Картинка = БиблиотекаКартинок.СинтаксическийКонтроль;
		КонецЕсли;
...
Процедура ПриНажатииНаКнопкуОтправкиВРегламентированномОтчетеФСГС(Форма, Кнопка) Экспорт
	...
	Если Форма.Модифицированность Тогда
		Если Вопрос("Для продолжения необходимо сохранить отчет." + Символы.ПС + "Продолжить?",РежимДиалогаВопрос.ДаНет) <> КодВозвратаДиалога.Да ИЛИ НЕ Форма.Сохранить() Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	Если ИмяКнопки = "Проверить" Тогда
		КонтекстЭДО.ПроверитьОтчетСИспользованиемСервисаОнлайнПроверки(Форма, "ФСГС");
	Иначе
		КонтекстЭДО.ОтправкаРегламентированногоОтчетаВФСГС(Форма.мСохраненныйДок.Ссылка, , Форма);
	КонецЕсли;
...
Процедура СкрытьПанельОтправкиВФормеРегламентированногоОтчетаФСС(Форма)
	
	СкрытьПанельОтправкиВФормеРегламентированногоОтчета(Форма); // !!!
	
КонецПроцедуры

Процедура СкрытьПодменюОтправкиВФормеРегламентированногоОтчетаФСС(Форма)
...
Процедура УправлениеВидимостьюЭлементовОтправкиРегламентированногоОтчетаФСС(Форма, ЭлектронныеПредставления = Ложь)
	
	ОбменЗадействован = Ложь;
	ОрганизацияОтчета = Форма.Организация;
	...
	Если НЕ ОбменЗадействован Тогда
	...
	Иначе
		Если ЭлектронныеПредставления Тогда
			ПоказатьПодменюОтправкаВЭлектронномПредставлении(Форма);
			ПоказатьПанельОтправкиВФормеРегламентированногоОтчета(Форма);
		КонецЕсли;
		
		// регулируем видимость кнопки Проверить
		УправлениеВидимостьюКнопкиПроверить(Форма, "ФСС", ЭлектронныеПредставления);
	КонецЕсли;
	
КонецПроцедуры

Процедура УправлениеВидимостьюКнопкиПроверить(Форма, КонтролирующийОрган, ЭлектронныеПредставления)
	...
	Если ДействияФормы <> Неопределено Тогда
		ПодменюОтправка = ДействияФормы.Кнопки.Найти("Отправка");
		Если ЭлектронныеПредставления Тогда
			ДействиеДляПроверки = Новый Действие("ДействияФормыПроверить");
		Иначе
			ДействиеДляПроверки = Новый Действие("КоманднаяПанельФормыОтправить");
		КонецЕсли;
		
		Если ПодменюОтправка <> Неопределено Тогда
			ОрганизацияОтчета = Форма.Организация;
			
			Если ПравоДоступа("Чтение", Метаданные.Справочники.УчетныеЗаписиДокументооборота) И НЕ ОрганизацияОтчета.УчетнаяЗаписьОбмена.ОбменНапрямую И ОрганизацияОтчета.УчетнаяЗаписьОбмена.ИспользоватьСервисОнлайнПроверкиОтчетов
				И ОрганизацияОтчета.УчетнаяЗаписьОбмена.СпецоператорСвязи <> Перечисления.СпецоператорыСвязи.Такском И КонтекстЭДОЛокальный() <> Неопределено
				И Найти(";" + ВРег(СокрЛП(КонтекстЭДОЛокальный().ПолучитьПараметрСпецоператора(ОрганизацияОтчета.УчетнаяЗаписьОбмена.СпецоператорСвязи, "ОнлайнПроверкаКонтролирующиеОрганы"))) + ";", ";" + КонтролирующийОрган + ";") <> 0 Тогда
				Если ПодменюОтправка.Кнопки.Найти("Проверить") = Неопределено Тогда
					КнопкаПроверить = ПодменюОтправка.Кнопки.Вставить(0, "Проверить", ТипКнопкиКоманднойПанели.Действие, "Проверить в Интернете", ДействиеДляПроверки);
					КнопкаПроверить.Картинка = БиблиотекаКартинок.СинтаксическийКонтроль;
				КонецЕсли;
...
Процедура ПриИзмененииОрганизацииВРегламентированномОтчетеФСС(Форма, Действие, ЭлектронныеПредставления = Ложь) Экспорт
...
Функция РазобратьСтрокуСубъекта(СтрокаСубъект) Экспорт
...
Функция ПолучитьСертификатыОбщие() Экспорт
...
Процедура ЗаменитьМаркерВЗапросеДвоичнымиДанными(ИмяФайлаОтправки, ИмяФайлаВыгрузки) Экспорт
...
Функция ПолучитьСвойстваКомпоненты()
	
	МакетПолучен = Ложь;
	
	Если КонтекстЭДО <> Неопределено Тогда
		Попытка
			МакетКомпоненты = КонтекстЭДО.Метаданные().Макеты["КомпонентаОбмена"];
			ДанныеКомпонентаОбмена = КонтекстЭДО.ПолучитьМакет("КомпонентаОбмена");
			МакетПолучен = Истина;
		Исключение
		КонецПопытки; 
	КонецЕсли;
	...
	Если НЕ МакетПолучен Тогда
		Попытка
			Отчет4ФСС = РеглОтчеты("РегламентированныйОтчет4ФСС");
			Если ТипЗнч(Отчет4ФСС) = Тип("ОтчетМенеджер.РегламентированныйОтчет4ФСС") Тогда
				МакетКомпоненты = Метаданные.Отчеты.РегламентированныйОтчет4ФСС.Макеты["КомпонентаОбмена"];
			Иначе
				МакетКомпоненты = Отчет4ФСС.Метаданные().Макеты["КомпонентаОбмена"];
			КонецЕсли;
			
			ДанныеКомпонентаОбмена = Отчет4ФСС.ПолучитьМакет("КомпонентаОбмена");
		Исключение
			Сообщить("Не удалось найти внешнюю компоненту ExtIntgr.dll в конфигурации:
			|" + ИнформацияОбОшибке().Описание, СтатусСообщения.Важное);
			Возврат Неопределено;
		КонецПопытки;
	КонецЕсли;
	...
	Попытка
		...
		СтруктураСвойств.Вставить("Версия", МакетКомпоненты.Синоним);
		СтруктураСвойств.Вставить("Адрес", ПоместитьВоВременноеХранилище(ДанныеКомпонентаОбмена));
...
Функция СоздатьОбъектКомпонентыПеребираяВерсии(АдресКомпоненты, ProgID, СтрокаНомерВерсии)
	ЧислоВерсия = Число(СтрокаНомерВерсии);
	Объект = Неопределено;
	
	Попытка
		//попытка создания указанной версии
		СтрокаВерсия = Формат(ЧислоВерсия, "ЧЦ=3; ЧВН=");
		Объект = Новый("Addin.ЭДОCom." + ProgID + СтрокаВерсия);
	Исключение
		Попытка
			//попытка подключения и создания указанной версии
			ПодключитьВнешнююКомпоненту(АдресКомпоненты, "ЭДОCom", ТипВнешнейКомпоненты.COM);
			Объект = Новый("Addin.ЭДОCom." + ProgID + СтрокаВерсия);
		Исключение
			//производим 6 попыток создания объекта: версии +3, +2, +1, (0 - пропускаем), -1, -2, -3
			ЧислоВерсияНачальнаяПопытка = ЧислоВерсия + 4;
			Для НомерПопытки = 1 По 7 Цикл
				Если НомерПопытки = 4 Тогда
					//(0 - пропускаем)
					Продолжить;
				КонецЕсли;
				
				Если ЧислоВерсияНачальнаяПопытка - НомерПопытки < 1 Тогда
					// номер версии - положительное число
					Прервать
				КонецЕсли;
				
				СтрокаВерсия = Формат(ЧислоВерсияНачальнаяПопытка - НомерПопытки, "ЧЦ=3; ЧВН=");
				Попытка
					Объект = Новый("Addin.ЭДОCom." + ProgID + СтрокаВерсия);
					Прервать;
				Исключение
...
Функция ИнициализироватьВнешнююКомпоненту() Экспорт
	...
	СвойстваКомпоненты = ПолучитьСвойстваКомпоненты();
	Если СвойстваКомпоненты = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ВерсияКомпоненты = СвойстваКомпоненты.Версия;
	АдресКомпоненты = СвойстваКомпоненты.Адрес;
	
	Крипт = СоздатьОбъектКомпонентыПеребираяВерсии(АдресКомпоненты, "CryptS", ВерсияКомпоненты);
...
Функция СоздатьОбъектДляРаботыСКриптографией() Экспорт
...
Функция СоздатьОбъектДляРаботыСДвоичнымиДанными() Экспорт
	...
	ДвДанные = СоздатьОбъектКомпонентыПеребираяВерсии(АдресКомпоненты, "BinaryDataS", ВерсияКомпоненты);
...
Функция ПолучитьПараметрыКриптографии(Криптопровайдеры) 
...
Функция ПолучитьПараметрыКриптографииПоУмолчанию(Криптопровайдеры) 
...
Функция ВопросПредупреждение(ОсновнойТекст, ТекстПереключатель1, ТекстПереключатель2, ПакетныйРежим = Ложь) Экспорт
...
Процедура ПоказатьПротоколОбработки(HTMLТекст, Заголовок, ПечатьВозможна = Истина) Экспорт
	...
	ФормаОтображенияHTML.ПечатьВозможна = ПечатьВозможна;
...
Процедура ЗаполнитьНовыеПоляОтправкиФСС() Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ОтправкиФСС.ДатаНачалаПериода = &ПустаяДата";
...
//o	НомерДоговора, тип: Строка 
//o	ДатаДоговора, тип: Дата 
...
Функция ПолучитьНомерДатаДоговораДокументов(МассивСсылок) Экспорт
	...
	Если ИдКонфигурации = "БГУ" Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		|	УслугиОказанные.Договор.НомерДоговора КАК НомерДоговора,
		|	УслугиОказанные.Договор.ДатаДоговора КАК ДатаДоговора,
		|	УслугиОказанные.Ссылка
		|ИЗ
		|	Документ.УслугиОказанные КАК УслугиОказанные
		|ГДЕ
		|	УслугиОказанные.Ссылка В(&МассивСсылок)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	УслугиСтороннихОрганизаций.Договор.НомерДоговора КАК НомерДоговора,
		|	УслугиСтороннихОрганизаций.Договор.ДатаДоговора КАК ДатаДоговора,
		|	УслугиСтороннихОрганизаций.Ссылка
		|ИЗ
		|	Документ.УслугиСтороннихОрганизаций КАК УслугиСтороннихОрганизаций
		|ГДЕ
		|	УслугиСтороннихОрганизаций.Ссылка В(&МассивСсылок)";
		
	Иначе
		Если ИдКонфигурации = "УПП" Тогда
			ДополнительныйТекст = "ВЫБРАТЬ
			|	АктОбОказанииПроизводственныхУслуг.ДоговорКонтрагента.Номер КАК НомерДоговора,
			|	АктОбОказанииПроизводственныхУслуг.ДоговорКонтрагента.Дата КАК ДатаДоговора,
			...
			|ГДЕ
			|	АктОбОказанииПроизводственныхУслуг.Ссылка В(&МассивСсылок)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|";
		...
		Запрос.Текст = ДополнительныйТекст + "ВЫБРАТЬ
...
Процедура ЗаполнитьПредопределенныеВидыОтправляемыхДокументов(СправочникЦеликом = Ложь) Экспорт
	...
	Иначе
		...
		Пока ВыборкаРезультата.Следующий() Цикл
			ИмяЭлемента = Справочники.ВидыОтправляемыхДокументов.ПолучитьИмяПредопределенного(ВыборкаРезультата.Ссылка);
...
Процедура ЗаменитьРеквизитВидОтчета(ИмяСправочника)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(ВидОтчетаСтарый) = Тип("СправочникСсылка.ВидыОтправляемыхДокументов") Тогда
		...
		Иначе
			...
			Если ВидОтчетаНовый = Справочники.ВидыОтправляемыхДокументов.ПустаяСсылка() Тогда
				//подходящий элемент нового справочника не найден, создадим новый элемент
...
// Выполняется при обновлении ИБ, 
// заменяет в справочниках ОтправкиФСС и ЭлектронныеПредставленияРегламентированныхОтчетов
// ссылки удаляемых предопределенных элементов 
// УдалитьРеестрСведенийВФССОЕжемесячныхПособияхПоУходу
// УдалитьРеестрСведенийВФССОПособияхПоНетрудоспособности
// УдалитьРеестрСведенийВФССОПособияхПриРожденииРебенка
// на новый предопределенный элемент РеестрСведенийВФСС

Процедура ЗаменитьСсылкиУдаляемыхЭлементовВидыОтправляемыхДокументов() Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	Справочник.ОтправкиФСС КАК ОтправкиФСС
	               |ГДЕ
	               |	(ОтправкиФСС.ВидОтчета = ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.УдалитьРеестрСведенийВФССОЕжемесячныхПособияхПоУходу)
	               |			ИЛИ ОтправкиФСС.ВидОтчета = ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.УдалитьРеестрСведенийВФССОПособияхПоНетрудоспособности)
	               |			ИЛИ ОтправкиФСС.ВидОтчета = ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.УдалитьРеестрСведенийВФССОПособияхПриРожденииРебенка))";
				   
	Результат = Запрос.Выполнить();
	ВыборкаДетальныеЗаписи = Результат.Выбрать();
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		
		ОтправкаФССОбъект = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
		ОтправкаФССОбъект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.РеестрСведенийВФСС;
		
		ОтправкаФССОбъект.ОбменДанными.Загрузка = Истина;
		ОтправкаФССОбъект.Записать();
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |ГДЕ
	               |	(ЭлектронныеПредставленияРегламентированныхОтчетов.ВидОтчета = ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.УдалитьРеестрСведенийВФССОЕжемесячныхПособияхПоУходу)
	               |			ИЛИ ЭлектронныеПредставленияРегламентированныхОтчетов.ВидОтчета = ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.УдалитьРеестрСведенийВФССОПособияхПоНетрудоспособности)
	               |			ИЛИ ЭлектронныеПредставленияРегламентированныхОтчетов.ВидОтчета = ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.УдалитьРеестрСведенийВФССОПособияхПриРожденииРебенка))";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ЭлектронныеПредставленияРегламентированныхОтчетовОбъект = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
		ЭлектронныеПредставленияРегламентированныхОтчетовОбъект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.РеестрСведенийВФСС;
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ СТЫКОВКИ С ПОДСИСТЕМОЙ ЗАЩИЩЕННОГО ЭЛЕКТРОННОГО
// ДОКУМЕНТООБОРОТА С ФСРАР
//

Функция КонтекстЭДОЛокальный() Экспорт
	
	//Если КонтекстЭДО <> Неопределено Тогда
	//	Возврат КонтекстЭДО;
	//	
	//Иначе
		//Возврат РегламентированнаяОтчетностьКлиентПовтИсп.КонтекстДокументооборотаСНалоговымиОрганами();
		
		ДвоичныеДанныеОбработки = ПолучитьМакет("Макет");
...
Процедура СкрытьПанельОтправкиВФормеРегламентированногоОтчетаФСРАР(Форма)
	
	СкрытьПанельОтправкиВФормеРегламентированногоОтчета(Форма); // !!!
	
КонецПроцедуры

Процедура СкрытьПодменюОтправкиВФормеРегламентированногоОтчетаФСРАР(Форма)
...
Процедура УправлениеВидимостьюЭлементовОтправкиРегламентированногоОтчетаФСРАР(Форма, ЭлектронныеПредставления = Ложь)
	...
	Иначе
		...
		// регулируем видимость кнопки Проверить
		УправлениеВидимостьюКнопкиПроверить(Форма, "ФСРАР", ЭлектронныеПредставления);
	КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ СТЫКОВКИ С ПОДСИСТЕМОЙ ЗАЩИЩЕННОГО ЭЛЕКТРОННОГО
// ДОКУМЕНТООБОРОТА С РПН
//

Процедура СкрытьПанельОтправкиВФормеРегламентированногоОтчетаРПН(Форма)
	
	СкрытьПанельОтправкиВФормеРегламентированногоОтчета(Форма); // !
	
КонецПроцедуры

Процедура СкрытьПодменюОтправкиВФормеРегламентированногоОтчетаРПН(Форма)
...
Процедура УправлениеВидимостьюЭлементовОтправкиРегламентированногоОтчетаРПН(Форма, ЭлектронныеПредставления = Ложь)
	...
	Иначе
		...
		// регулируем видимость кнопки Проверить
		УправлениеВидимостьюКнопкиПроверить(Форма, "РПН", ЭлектронныеПредставления);
	КонецЕсли;
...
// Возвращает документ РегламентированныйОтчет за предыдущий налоговый период
//
Функция ПолучитьДокументПНП(ТекущийДокумент, ОтчетныйПериод = Неопределено, НарастающийИтог = Ложь) Экспорт 
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |		ПО СтатусыРегламентированныхОтчетов.Отчет = РегламентированныйОтчет.Ссылка
	               |ГДЕ
	               |	РегламентированныйОтчет.ИсточникОтчета = &ИсточникОтчета
	               |	И РегламентированныйОтчет.Организация = &Организация
...
// Выполняется при обновлении информационной базы.
// Производит назначение номеров пачек в сохраненной отчетности для ПФР
// либо по номеру выгрузки, либо по счетчику выгрузок ПФР.
Процедура НазначитьНомераПачекОтчетовПФР() Экспорт
	...
	ЗапросПоследнихНомеров.Текст = "ВЫБРАТЬ
	                               |	ЕСТЬNULL(СчетчикВыгрузокПФР.Организация, СчетчикПоДокументам.Организация) КАК Организация,
	                               |	ЕСТЬNULL(СчетчикВыгрузокПФР.Год, СчетчикПоДокументам.Год) КАК Год,
	                               |	ВЫБОР
	                               |		КОГДА ЕСТЬNULL(СчетчикВыгрузокПФР.Значение, 0) >= ЕСТЬNULL(СчетчикПоДокументам.НомерПачки, 0)
	                               |			ТОГДА ЕСТЬNULL(СчетчикВыгрузокПФР.Значение, 0)
	                               ...
	                               |		ПО СчетчикВыгрузокПФР.Организация = СчетчикПоДокументам.Организация
	                               |			И СчетчикВыгрузокПФР.Год = СчетчикПоДокументам.Год
	                               |
	                               |УПОРЯДОЧИТЬ ПО
	                               |	Организация,
	                               |	Год";
	...
	Пока ВыборкаОтчетов.Следующий() Цикл
		...
		Попытка
			...
			ОтчетОбъект.НомерПачки = НомерПачки;
			ОтчетОбъект.Записать();
			РегламентированнаяОтчетность.УстановитьСчетчикВыгруженныхФайловПФР(ВыборкаОтчетов.Организация, ВыборкаОтчетов.Год, НомерПачки);
...
Процедура ОпределитьНомерПачкиПоИмениФайла(НомерПачки, ИмяФайла)
	...
	Если ПозицияПервогоТокенаDCK <> 0 Тогда
		...
		Если ОбщегоНазначения.ТолькоЦифрыВСтроке(НомерПачкиОрганизации) Тогда
			...
			Если НомерПачкиВыгрузки = 0 Тогда
				...
				Если ОбщегоНазначения.ТолькоЦифрыВСтроке(НомерПачкиПодразделения) Тогда
...
// Выполняется при обновлении информационной базы.
// Устанавливает флажок ИспользоватьСервисОнлайнПроверкиОтчетов для учетных записей спецоператоров "Калуга Астрал" и "Форус"
...
// Открывает или активирует форму элемента справочника "Организации"
// с возможностью установки активного элемента формы.
//
// Параметры
//  Владелец - Форма - Ссылка на форму из которой производится открытие формы справочника.
//  Организация - Ссылка на справочник Организации - указатель на элемент справочника
//                Организации, форму которого необходимо открыть.
//  ИмяЭлемента - Строка - Имя реквизита или элемента формы, на который устанавливается фокус ввода.
//
Процедура ОткрытьФормуЭлементаСправочникаОрганизаций(Владелец, Организация, ИмяЭлемента = "") Экспорт
	
	КлючУникальностиФормы = "a8d3bd14-efca-47da-8f9a-a242dcd22419";
	
	ФормаЭлемента = Организация.ПолучитьФорму("ФормаЭлемента", Владелец, КлючУникальностиФормы);
	ФормаЭлемента.ЗакрыватьПриЗакрытииВладельца = Истина;
	
	Если ЗначениеЗаполнено(ИмяЭлемента) Тогда
		Если ИмяЭлемента = "ОсновнаяПанель.КонтактнаяИнформация" Тогда
			СтраницаПанели = ФормаЭлемента.ЭлементыФормы.ОсновнаяПанель.Страницы.Найти("КонтактнаяИнформация");
			Если СтраницаПанели = Неопределено Тогда
				// Альтернативное наименование в семействе ЗУП.
				СтраницаПанели = ФормаЭлемента.ЭлементыФормы.ОсновнаяПанель.Страницы.Найти("АдресаИТелефоны");
			КонецЕсли;
			ФормаЭлемента.ЭлементыФормы.ОсновнаяПанель.ТекущаяСтраница = СтраницаПанели;
		Иначе
			ФормаЭлемента.ТекущийЭлемент = ФормаЭлемента.ЭлементыФормы[ИмяЭлементаФормыДляРеквизита(ИмяЭлемента)];
		КонецЕсли;
	КонецЕсли;
...
Процедура ОткрытьФормуЭлементаСправочникаРегистрацииВИФНС(Владелец, РегистрацияВИФНС) Экспорт
	
	КлючУникальностиФормы = "a3b380ed-4d48-4e33-a9d2-67e92b3c1946";
	
	ФормаЭлемента = РегистрацияВИФНС.ПолучитьФорму("ФормаЭлемента", Владелец, КлючУникальностиФормы);
	ФормаЭлемента.ЗакрыватьПриЗакрытииВладельца = Истина;
...
Процедура ОткрытьФормуСпискаОтветственныхЛиц(Владелец, Организация) Экспорт
	
	КлючУникальностиФормы = "7b5cab16-016e-4c8e-9db0-513c544b3c9e";
	
	Если ИДКонфигурации() = "БГУ" Тогда
		ИмяРегистра = "ОтветственныеЛицаУчреждений";
	Иначе
		ИмяРегистра = "ОтветственныеЛицаОрганизаций";
	КонецЕсли;
	
	ФормаРегистра = РегистрыСведений[ИмяРегистра].ПолучитьФорму("ФормаСписка", Владелец, КлючУникальностиФормы);
	ФормаРегистра.ЗакрыватьПриЗакрытииВладельца = Истина;
	
	РаботаСДиалогами.УстановитьОтборПоОрганизации(ФормаРегистра, Ложь, Организация, "РегистрСведенийСписок", Ложь, "СтруктурнаяЕдиница");
...
Функция ИмяЭлементаФормыДляРеквизита(ИмяРеквизита)
	
	ИДКонфигурации = ИДКонфигурации();
	
	ИмяЭлементаФормы = ИмяРеквизита;
	
	Если ИмяРеквизита = "КодОКВЭД" И ИДКонфигурации <> "БГУ" Тогда
		
		ИмяЭлементаФормы = "КодОКОПФ2";
		
	ИначеЕсли ИмяРеквизита = "НаименованиеОКВЭД"  И ИДКонфигурации <> "БГУ" Тогда
		
		ИмяЭлементаФормы = "НаименованиеОКОПФ2";
		
	ИначеЕсли ИмяРеквизита = "КодПоОКАТО" И ИДКонфигурации = "БГУ" Тогда
		
		ИмяЭлементаФормы = "КодОКАТОППО";
		
	ИначеЕсли ИмяРеквизита = "КодОКОПФ" И ИДКонфигурации = "БГУ" Тогда
		
		ИмяЭлементаФормы = "КодКОПФ";
		
	ИначеЕсли ИмяРеквизита = "НаименованиеОКОПФ" И ИДКонфигурации = "БГУ" Тогда
		
		ИмяЭлементаФормы = "КодКОПФ";
		
	ИначеЕсли ИмяРеквизита = "КодОКФС" Тогда
		
		Если ИДКонфигурации = "БГУ" Тогда
			ИмяЭлементаФормы = "КодКФС";
		Иначе
			ИмяЭлементаФормы = "КодОКОПФ1";
		КонецЕсли;
		
	ИначеЕсли ИмяРеквизита = "НаименованиеОКФС" Тогда
		
		Если ИДКонфигурации = "БГУ" Тогда
			ИмяЭлементаФормы = "НаименованиеКФС";
		Иначе
			ИмяЭлементаФормы = "НаименованиеОКОПФ1";
		КонецЕсли;
		
	ИначеЕсли ИмяРеквизита = "КодИМНС" Тогда
		
		Если ИДКонфигурации = "БГУ" Тогда
			ИмяЭлементаФормы = "КодИФНС";
		ИначеЕсли Метаданные.Справочники.Организации.Реквизиты.Найти("РегистрацияВИФНС") <> Неопределено Тогда
			ИмяЭлементаФормы = "РегистрацияВИФНС";
		КонецЕсли;
		
	ИначеЕсли ИмяРеквизита = "НаименованиеИМНС"
	        И Метаданные.Справочники.Организации.Реквизиты.Найти("РегистрацияВИФНС") <> Неопределено Тогда
		ИмяЭлементаФормы = "РегистрацияВИФНС";
...
// определение форм
Форма20060901 = ОпределитьФормуВДеревеФормИФорматов(мФормыИФорматы, "1151235", '20060523', "459",	"ФормаОтчета2006Кв3");
Форма20070901 = ОпределитьФормуВДеревеФормИФорматов(мФормыИФорматы, "1151235", '20070405', "204",	"ФормаОтчета2007Кв3");
Форма20080601 = ОпределитьФормуВДеревеФормИФорматов(мФормыИФорматы, "1151235", '20080328', "182",	"ФормаОтчета2008Кв2");
...
НоваяФорма.ОписаниеОтчета     = "Прил. № 1 к Приказу Федеральной службы по экологическому, технологическому и атомному надзору от 23.05.2006 № 459 (сводная форма)";
НоваяФорма.ДатаНачалоДействия = '20060701';
НоваяФорма.ДатаКонецДействия  = '20070630';

НоваяФорма = мТаблицаФормОтчета.Добавить();
НоваяФорма.ФормаОтчета        = "ФормаОтчета2007Кв3";
НоваяФорма.ОписаниеОтчета     = "Прил. № 1 к Приказу Федеральной службы по экологическому, технологическому и атомному надзору от 05.04.2007 № 204";
НоваяФорма.ДатаНачалоДействия = '20070701';
НоваяФорма.ДатаКонецДействия  = '20080531';
