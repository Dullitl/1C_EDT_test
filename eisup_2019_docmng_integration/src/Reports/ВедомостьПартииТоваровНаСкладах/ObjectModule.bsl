...
Процедура УстановитьНачальныеНастройкиУпр(ДополнительныеПараметры = Неопределено)
	...
	|	ИсточникДанных.Номенклатура.Наименование + ""["" + ИсточникДанных.Номенклатура.Артикул + ""]"" КАК НоменклатураСАртикулом
	|{ВЫБРАТЬ
	|	Номенклатура.*,
	|	НоменклатураСАртикулом,
	|	Организация.*,
	|	Склад.*,
	...
	УниверсальныйОтчет.ДобавитьИзмерениеСтроки("НоменклатураСАртикулом");
