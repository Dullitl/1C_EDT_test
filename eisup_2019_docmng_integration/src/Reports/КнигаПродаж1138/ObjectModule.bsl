...
Процедура СформироватьКнигуПродаж(ЭлементыФормы) Экспорт
	...
	СтруктураПараметров.Вставить("ВыводитьПродавцовПоАвансам", ВыводитьПродавцовПоАвансам);
	...
	СтруктураПараметров.Вставить("ЗаписьДополнительногоЛиста");
	...
	СтруктураПараметров.Вставить("СоответствиеСтрокиДопИнформацииПоСчетуФактуре", СоответствиеСтрокиДопИнформацииПоСчетуФактуре);
...
Процедура СформироватьОсновнойРаздел(ЭлементыФормы, СтруктураПараметров, ТабличныйДокумент, Результат, СписокСчетовФактур);
	...
	Иначе
		Макет = ПолучитьМакет("КнигаПродаж1137");
	КонецЕсли;
	...
	Для Каждого ИтогПоПериодам ИЗ ДеревоЗаписей.Строки Цикл
		...
		Для Каждого ИтогПоПериодамКорректировки ИЗ ИтогПоПериодам.Строки Цикл;
			...
			Если ВерсияПостановленияНДС1137 = 1 Тогда
				Макет = ПолучитьМакет("ДополнительныйЛистКнигиПродаж1137");
			ИначеЕсли ВерсияПостановленияНДС1137 = 2 Тогда
				Макет = ПолучитьМакет("ДополнительныйЛистКнигиПродаж952");
			Иначе
				Макет = ПолучитьМакет("ДополнительныйЛистКнигиПродаж735");
			КонецЕсли;
			...
			ВывестиШапкуДопЛиста(ТабличныйДокумент, Макет, ИтогПоПериодамКорректировки, НомерЛиста);
			...
			Если ВерсияПостановленияНДС1137 < 3
				И МожноСкрытьКолонкиПоСтавке20 Тогда
...
Процедура абс_ЗаполнитьТаблицуВалют(ПараметрыЗаполнения,ЗаписьКниги,СоставСтроки)
	...
	Если ЗначениеЗаполнено(ПараметрыЗаполнения.Валюта) 
		...
		СтрокаПоВалюте.Сумма  = СоставСтроки.ВсегоПродажВВалюте;
	КонецЕсли;	
...
Процедура ВывестиПодвал(ТабличныйДокумент, Макет)
	...
	Секция.Параметры.ИмяРук = мСведенияОбОрганизации.ФИОРук;
...
Процедура ПолучитьСчетаФактурыДокументыКнигаПродаж(Запрос)
	...
	//АБС
	//|	ЕСТЬNULL(ВТ_РегистрацияСчетовФактур.НомерСчетаФактуры, ВТ_ТаблицаСчетаФактурыДокументы.НомерСчетаФактуры) КАК НомерСчетаФактуры,
	|	ЕСТЬNULL(ВТ_ТаблицаСчетаФактурыДокументы.НомерСчетаФактуры, ВТ_РегистрацияСчетовФактур.НомерСчетаФактуры) КАК НомерСчетаФактуры,
	//АБС
	|	ЕСТЬNULL(ВТ_РегистрацияСчетовФактур.СчетФактура, ВТ_ТаблицаСчетаФактурыДокументы.СчетФактураДокумент) КАК СчетФактураДокумент,
...
Функция ПолучитьЗаписиКнигиПродаж(СписокСчетовФактур, СтруктураПараметров) Экспорт
	...
	|	КОНЕЦ КАК СчетФактура,
	|	ВЫБОР
	|		КОГДА НДСЗаписиКнигиПродаж.НДС = 0
	|			ТОГДА ДАТАВРЕМЯ(1, 1, 1)
	|		ИНАЧЕ НДСЗаписиКнигиПродаж.ДатаОплаты
	|	КОНЕЦ КАК ДатаОплаты,
	|	ВЫБОР
	|		КОГДА НДСЗаписиКнигиПродаж.НДС = 0
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ НДСЗаписиКнигиПродаж.ДокументОплаты
	...
	|			ТОГДА ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.КодВидаОперации, ""01"")
	...
	Иначе
		Для Каждого СтрокаЗаписи Из ЗаписьКниги.Строки Цикл
			Если ЗначениеЗаполнено(СтрокаЗаписи.ДатаОплаты) Тогда
				...
				КонецЕсли;
			КонецЕсли;
			абс_ОпределитьНомерДатуДокументаОплаты(СтрокаЗаписи, ЭтоДетальнаяЗапись);
...
Функция Удалить_ОпределитьДатуИНомерСФ(ЗаписьКниги, СтруктураПараметров)
	...
	Иначе
		...
		Если ТипЗнч(ЗаписьКниги.СчетФактура) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
			ДатаНомер = "" + Формат(ЗаписьКниги.СчетФактура.Дата, "ДФ=dd.MM.yyyy") + ";" 
				+ ПолучитьНомерНаПечать(ЗаписьКниги.СчетФактура, СтруктураПараметров.ПрефиксыРИБиОрганизации);
		ИначеЕсли ТипЗнч(ЗаписьКниги.СчетФактураДокумент) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
			ДатаНомер = "" + Формат(ЗаписьКниги.СчетФактураДокумент.Дата, "ДФ=dd.MM.yyyy") + ";" 
				+ ПолучитьНомерНаПечать(ЗаписьКниги.СчетФактураДокумент, СтруктураПараметров.ПрефиксыРИБиОрганизации);
		КонецЕсли;
...
Функция ОпределитьДатуИНомерСФ(ЗаписьКниги, СтруктураПараметров, НомерВпереди = Ложь)
	...
	// {{KM WARE Агапов Н.А. Заявка №34379 23.09.2015 начало
	// Необходимо убрать отбор по Юр. и Физ. лицу
	// Необходимо добавить отбор по дате
	Если ТипЗнч(ЗаписьКниги.СчетФактура) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
		Если Регистрысведений.kwr_УстановкаДатыСФ.ПроверкаДаты(ЗаписьКниги.СчетФактура.абс_ДатаСФ,ЗаписьКниги.Организация) = истина тогда
			ДатаСФ = "" + Формат(ЗаписьКниги.СчетФактура.абс_ДатаСФ, "ДФ=dd.MM.yyyy");
		иначе
			ДатаСФ = "" + Формат(ЗаписьКниги.СчетФактура.Дата, "ДФ=dd.MM.yyyy");
		КонецЕсли;
	ИначеЕсли ТипЗнч(ЗаписьКниги.СчетФактураДокумент) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
		ДатаСФ = "" + Формат(ЗаписьКниги.СчетФактураДокумент.абс_ДатаСФ, "ДФ=dd.MM.yyyy");
	КонецЕсли;
	...
	//
	//Если ТипЗнч(ЗаписьКниги.СчетФактура) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
	//	Если Найти(ЗаписьКниги.Покупатель,"Юридическое лицо")>0 или Найти(ЗаписьКниги.Покупатель,"Физическое лицо")>0 Тогда
	//		ДатаСФ = "" + Формат(ЗаписьКниги.СчетФактура.абс_ДатаСФ, "ДФ=dd.MM.yyyy");
	//	КонецЕсли;
	//ИначеЕсли ТипЗнч(ЗаписьКниги.СчетФактураДокумент) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
	//	Если Найти(ЗаписьКниги.Покупатель,"Юридическое лицо")>0 или Найти(ЗаписьКниги.Покупатель,"Физическое лицо")>0 Тогда
	...
	//	Возврат "" + ДатаСФ + ?(ЗначениеЗаполнено(ДатаСФ), ";", "") + НомерСФ;		
	//КонецЕсли;	
	// }}KM WARE Агапов Н.А. Заявка № 23.09.2015 окончание	
КонецФункции
//\\АБС ВСТАВКА №000028978 КОНЕЦ
