...
Функция СформироватьЗапрос(СтруктураПараметров) Экспорт
	...
	Для каждого Измерение Из ПостроительОтчета.ИзмеренияСтроки Цикл
		...
		ТекстПоля = ТекстПоля + ", ПРЕДСТАВЛЕНИЕ(" + Измерение.ПутьКДанным + ".Код) КАК " + Измерение.Имя + "ПредставлениеКод";
...
Процедура СформироватьОтчет(ДокументРезультат,  ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	ДокументРезультат.ИмяПараметровПечати = "v2_ОборотноСальдоваяВедомостьПоСчету " + ИмяРегистраБухгалтерии;
...
Процедура ВывестиПоказателиОборотноСальдовойВедомостиПоСчету(Выборка, СтруктураПараметров, Знач Счет, 
	...
	Для Каждого ИмяПоказателя Из СтруктураПараметров.МассивПоказателей Цикл
		...
		Уровень = ?(Выборка.Уровень()>3,3,Выборка.Уровень());
		ЦветФона = врМакет.ПолучитьОбласть("Цвет"+Уровень).Область(1,1,1,1).ЦветФона;
		Шрифт = врМакет.ПолучитьОбласть("Цвет"+Уровень).Область(1,1,1,1).Шрифт;
		
		Для н = 1 по 6 Цикл
			Область.Область(1,н,1,н).ЦветФона = ЦветФона;
			Область.Область(1,н,1,н).Шрифт    = Шрифт;
		КонецЦикла;
...
Процедура ВывестиГруппировкуОборотноСальдовойВедомостиПоСчету(ОбъектОтчета, Выборка, Знач ИндексТекущейГруппировки, СтруктураПараметров) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Лев(ИмяГруппировки, 8) = "Субконто" Тогда
			...
			ВыводимаяОбласть.Параметры.СубконтоПредставлениеКод = СокрЛП(Выборка[ИмяГруппировки + "ПредставлениеКод"]);
		...
		Для н = 2 по 3 Цикл
			ВыводимаяОбласть.Область(1,н,1,н).ЦветФона = ЦветФона;
			ВыводимаяОбласть.Область(1,н,1,н).Шрифт    = Шрифт;
		КонецЦикла;
