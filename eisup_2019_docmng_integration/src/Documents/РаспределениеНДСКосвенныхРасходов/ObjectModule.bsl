					...
					Пока ВыборкаПоСчетуУчетаНДС.Следующий() Цикл
						...
						Если РассчитатьСуммуКОтражению Тогда
							Если не СуммаСНДСкРаспределению = 0 Тогда
								СуммаСНДСРезультатРаспределения = ttk_ОбщегоНазначения.РаспределитьПропорционально(СуммаСНДСкРаспределению,БазисРаспределенияСНДС);
							КонецЕсли; 
							Если не НДСКРаспределению = 0 Тогда
								НДСРезультатРаспределения = ttk_ОбщегоНазначения.РаспределитьПропорционально(НДСкРаспределению,БазисРаспределенияНДС);
							КонецЕсли; 
...
Функция ПолучитьВыборкуИзРегистраНДСКосвенныеРасходы(Организация, НачалоПериода, КонецПериода) Экспорт
	...
	|	ВЫБОР КОГДА СчетаУчетаПоДеятельностиЕНВД.Счет ЕСТЬ NULL ТОГДА
	|		ЛОЖЬ
	|	ИНАЧЕ
	|		ИСТИНА
	|	КОНЕЦ КАК ЕНВД,
	...
	|		ГДЕ
	|			НДСКосвенныеРасходы.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	...
	|			НДСКосвенныеРасходы.СубконтоНУ1,
	|			НДСКосвенныеРасходы.СубконтоНУ2,
	|			НДСКосвенныеРасходы.СубконтоНУ3) КАК НДСКосвенныеРасходы
	...
	|	ПО
	|		НДСКосвенныеРасходы.СчетЗатрат = СчетаУчетаПоДеятельностиЕНВД.Счет
	|
	|ГДЕ
	|	(НЕ(ЕСТЬNULL(НДСКосвенныеРасходыОбороты.СуммаБезНДСОборот, 0) = 0
	|				И ЕСТЬNULL(НДСКосвенныеРасходыОбороты.НДСОборот, 0) = 0))
	|	И (СчетаУчетаПоДеятельностиЕНВД.Счет ЕСТЬ NULL ИЛИ СчетаУчетаПоДеятельностиЕНВД.ПодлежитРаспределению)
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ПолучитьСписокКонтрагентовПоСчетамФактурам(Организация, СписокСчетовФактур)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Организация",  Организация);
	Запрос.УстановитьПараметр("СчетаФактуры",       ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(СписокСчетовФактур.ВыгрузитьКолонку("СчетФактура")));
...
Процедура ПроверитьСоответствиеДатыПериоду(Отказ, Заголовок)
	...
	Если (Дата > КонецПериода) ИЛИ (Дата < НачалоПериода) Тогда
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Дата документа не входит в указанный налоговый период", Отказ, Заголовок);
...
Процедура ПроверитьЗаполнениеТабличнойЧастиСоставКосвенныхРасходов(ТаблицаКосвенныхРасходов, Отказ, Заголовок)
	...
	Если Не Отказ Тогда
		...
		Если ВыручкаНДС <> ВыручкаНДСПоРегистру Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Сумма выручки по обычным ставкам НДС (" + ВыручкаНДС + " " + ВалютаУчета + ") не совпадает с информацией в регистрах учета (" + ВыручкаНДСПоРегистру + " " + ВалютаУчета + ")!", , Заголовок,СтатусСообщения.Внимание);
		КонецЕсли;
		Если ВыручкаЕНВД <> ВыручкаЕНВДПоРегистру Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Сумма выручки облагаемой ЕНВД (" + ВыручкаЕНВД + " " + ВалютаУчета + ") не совпадает с информацией в регистрах учета (" + ВыручкаЕНВДПоРегистру + " " + ВалютаУчета + ")!", , Заголовок,СтатусСообщения.Внимание);
		КонецЕсли;
		Если ВыручкаБезНДС <> ВыручкаБезНДСПоРегистру Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Сумма выручки без НДС (" + ВыручкаБезНДС + " " + ВалютаУчета + ") не совпадает с информацией в регистрах учета (" + ВыручкаБезНДСПоРегистру + " " + ВалютаУчета + ")!", , Заголовок,СтатусСообщения.Внимание);
		КонецЕсли;
		Если ВыручкаНДС0 <> ВыручкаНДС0ПоРегистру Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Сумма выручки облагаемой НДС по ставке 0% (" + ВыручкаНДС0 + " " + ВалютаУчета + ") не совпадает с информацией в регистрах учета (" + ВыручкаНДС0ПоРегистру + " " + ВалютаУчета + ")!", , Заголовок,СтатусСообщения.Внимание);
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого Строка Из СоставКосвенныхРасходов Цикл
		Если Строка.СуммаВсего <> (Строка.БезНДССумма + Строка.НДС0Сумма + Строка.ЕНВДСумма + Строка.НДССумма) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Для строки """ + Строка.НомерСтроки + """ общая сумма в колонке ""Сумма по счету"" не совпадает с итогом распределения этой суммы в других колонках!", Отказ, Заголовок);
		КонецЕсли;
		
		Если Строка.НДСВсего <> (Строка.БезНДС + Строка.НДС0 + Строка.ЕНВДНДС + Строка.НДС) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Для строки """ + Строка.НомерСтроки + """ общая сумма в колонке ""НДС по счету"" не совпадает с итогом распределения этой суммы в других колонках!", Отказ, Заголовок);
		КонецЕсли;
		...
		Если (Не Строка.НДСВключенВСтоимость И (Строка.БезНДС <> 0 или Строка.ЕНВДНДС <> 0)) Или 
			...
			Если СтрокиСчетовУчета.Количество() > 0 Тогда
				...
				Если СчетчикСуммы = 0 Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Для строки """ + Строка.НомерСтроки + """ на закладке ""Счета учета расходов"" не указаны суммы, использующиеся для распределения по движениям (при включении(исключении) НДС в стоимость)!", Отказ, Заголовок);
				КонецЕсли;
				
			Иначе
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Для строки """ + Строка.НомерСтроки + """ на закладке ""Счета учета расходов"" не указаны счета и суммы относимых затрат. Невозможно выполнить включение НДС в стоимость затрат!", Отказ, Заголовок);
			КонецЕсли;
		...
		Если не (Строка.НДСВсего = 0  и Строка.БезНДС =0 и Строка.НДС0 =0 и Строка.ЕНВДНДС =0 и Строка.НДС =0) и НЕ ЗначениеЗаполнено(Строка.СчетУчетаНДС) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("В строке номер """ + Строка.НомерСтроки + """ табличной части ""Состав косвенных расходов"": Не заполнено значение реквизита ""Счет учета НДС""!", Отказ, Заголовок);
		КонецЕсли; 
...
Процедура ДвиженияПоРегистрамПодсистемыНДС(СтруктураШапкиДокумента, РаспределениеКосвенныхРасходов, Отказ, Заголовок);
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаДляРаспределения, ТаблицаДвиженийНДСКосвенныеРасходы);
...
Процедура ДвиженияПоРегиструНДСПредъявленныйРеализация0(СтруктураШапкиДокумента, РаспределениеКосвенныхРасходов, Отказ, Заголовок)
	...
	СписокДокументовОтгрузки = ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(БазоваяТаблицаРаспределения.ВыгрузитьКолонку("ДокументОтгрузки"), Истина);
	...
	Для Каждого СтрокаРасхода Из ТаблицаДвижений Цикл
		...
		МассивСумм = ttk_ОбщегоНазначения.РаспределитьПропорционально(СтрокаРасхода.СуммаБезНДС, МассивКоэффициентов);
		Если МассивСумм <> Неопределено Тогда
			СтрокаРасхода.СуммыДвижений.ЗагрузитьКолонку(МассивСумм, "СуммаБезНДС");
		КонецЕсли;
		МассивСумм = ttk_ОбщегоНазначения.РаспределитьПропорционально(СтрокаРасхода.НДС, МассивКоэффициентов);
		Если МассивСумм <> Неопределено Тогда
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаДвижений, ТаблицаДвиженийНДСПредъявленныйРеализация0);
...
Процедура ПроверитьСписаниеПоРегиструНДСКосвенныеРасходы(СтруктураШапкиДокумента, Заголовок, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения, , Заголовок, СтатусСообщения.Внимание); 
...
Процедура ОбработкаПроведения(Отказ, Режим) Экспорт
	...
	//АБС - добавлено условие
	Если НДСРаспределять Тогда
		ДвиженияПоВключениюНДСВСтоимость();
		Возврат;
	КонецЕсли;
	//АБС\\
		
	Если НЕ ЗначениеЗаполнено(НачалоПериода) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не установлен период документа!", Отказ, Заголовок);
	КонецЕсли; 
...
//АБС
Процедура ДвиженияПоВключениюНДСВСтоимость()
	//Движение = Движения.НДСПредъявленный;
	//Для Каждого СтрРасходов Из СоставКосвенныхРасходов Цикл
	...
	ТаблицаПартий.Колонки.Добавить("СтатусПартии");
	ТаблицаПартий.Колонки.Добавить("Организация");
	ТаблицаПартий.Колонки.Добавить("Качество");
	ТаблицаПартий.Колонки.Добавить("СписаниеПартий");
	ТаблицаПартий.Колонки.Добавить("Стоимость");			
	Для Каждого Стр Из  СчетаУчетаРасходов Цикл
		Если СоставКосвенныхРасходов.Найти(Стр.КлючСтроки, "КлючСтроки") <> Неопределено Тогда
			СтрРасходов = СоставКосвенныхРасходов.Найти(Стр.КлючСтроки, "КлючСтроки");
			...
			Проводка.Содержание              = "Включение в стоимость";
			
			Проводка.СчетДт                  = Стр.СчетЗатрат;
			БухгалтерскийУчет.УстановитьСубконто( Проводка.СчетДт, Проводка.СубконтоДт, "Номенклатура", Стр.Субконто1);
			БухгалтерскийУчет.УстановитьСубконто( Проводка.СчетДт, Проводка.СубконтоДт, "Склады", Стр.Субконто2);
			
			
			
			Проводка.СчетКт                  = СтрРасходов.СчетУчетаНДС;
			БухгалтерскийУчет.УстановитьСубконто( Проводка.СчетКт, Проводка.СубконтоКт, "Контрагенты", СтрРасходов.поставщик);
			БухгалтерскийУчет.УстановитьСубконто( Проводка.СчетКт, Проводка.СубконтоКт, "СФПолученные", СтрРасходов.СчетФактура);
			
			Проводка.Сумма = Стр.НДС;
			НоваяСтрока = ТаблицаПартий.Добавить();
			НоваяСтрока.Номенклатура = Стр.Субконто1;
			НоваяСтрока.Склад = Стр.Субконто2;
			НоваяСтрока.ДокументОприходования = СтрРасходов.СчетФактура;
			НоваяСтрока.СтатусПартии = Перечисления.СтатусыПартийТоваров.Купленный;
			НоваяСтрока.Качество = Справочники.Качество.Новый;
			НоваяСтрока.СписаниеПартий = Ложь;
			НоваяСтрока.Организация = Организация;
			НоваяСтрока.Стоимость = Стр.НДС;
			
			// предназначено для отчет по НДС включенному в стоимость
			Движения.НДСВключенныйВСтоимость.Очистить();
			Движение = Движения.НДСВключенныйВСтоимость.Добавить();
			Движение.Период		  = Дата;
			Движение.Организация  = Организация;
			Движение.СчетФактура  = СтрРасходов.СчетФактура;
			Движение.ВидЦенности  = СтрРасходов.ВидЦенности;
			Движение.СтавкаНДС    = СтрРасходов.СтавкаНДС;
			Движение.СчетУчетаНДС = СтрРасходов.СчетУчетаНДС;
			Движение.Поставщик    = СтрРасходов.Поставщик;
			Движение.СуммаБезНДС  = Стр.СуммаБезНДС;
			Движение.НДС 		  = Стр.НДС;
			
		КонецЕсли;
	КонецЦикла;
	
	ТаблицаДВ = Новый ТаблицаЗначений;
	ТаблицаДВ.Колонки.Добавить("ВидЦенности");
	ТаблицаДВ.Колонки.Добавить("Поставщик");
	ТаблицаДВ.Колонки.Добавить("СчетФактура");
	ТаблицаДВ.Колонки.Добавить("СтавкаНДС");
	ТаблицаДВ.Колонки.Добавить("СчетУчетаНДС");
	ТаблицаДВ.Колонки.Добавить("Организация");
	ТаблицаДВ.Колонки.Добавить("СуммаБезНДС");
	ТаблицаДВ.Колонки.Добавить("НДС");
	Для Каждого Стр ИЗ СоставКосвенныхРасходов Цикл
		НоваяСтрока = ТаблицаДВ.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока,Стр);
		НоваяСтрока.СуммаБезНДС = Стр.СуммаВсего;
		НоваяСтрока.НДС = Стр.НДСВсего;
		НоваяСтрока.Организация = Организация;
	КонецЦикла;
	Движения.НДСПредъявленный.Очистить();
	Движения.НДСПредъявленный.мТаблицаДвижений = ТаблицаДВ;
	Движения.НДСПредъявленный.мПериод = Дата;
	Движения.НДСПредъявленный.ВыполнитьРасход();
	
	Движения.ПартииТоваровНаСкладах.мПериод = Дата;
	Движения.ПартииТоваровНаСкладах.мТаблицаДвижений =ТаблицаПартий;
	Движения.ПартииТоваровНаСкладах.ВыполнитьПриход();
	
	Движения.ПартииТоваровНаСкладахБухгалтерскийУчет.мПериод= Дата;
	Движения.ПартииТоваровНаСкладахБухгалтерскийУчет.мТаблицаДвижений =ТаблицаПартий;
	Движения.ПартииТоваровНаСкладахБухгалтерскийУчет.ВыполнитьПриход();
	
	Движения.ПартииТоваровНаСкладахНалоговыйУчет.мПериод = Дата;
	Движения.ПартииТоваровНаСкладахНалоговыйУчет.мТаблицаДвижений =ТаблицаПартий;
