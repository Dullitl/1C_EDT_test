...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "БланкСогласования") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "БланкСогласования", "Бланк согласования акта агента", ПечатьБланкаСогласования(МассивОбъектов, ОбъектыПечати));
	КонецЕсли;	
	// АБС ВСТАВКА КОНЕЦ
...
Функция ПечатьПоступлениеТоваров(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого Ссылка Из МассивОбъектов Цикл
		...
		ОбластьМакета.Параметры.ТекстЗаголовка = ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, "Накладная");
		...
		Для Каждого ВыборкаСтрокТовары Из ВыборкаСтрок Цикл
			...
			ОбластьМакета.Параметры.Товар       = абс_СерверныеФункции.ПредставлениеCOMCODE(ВыборкаСтрокТовары.Номенклатура) +    //АБС Коломиец 12077
			СокрЛП(ВыборкаСтрокТовары.Товар) + ФормированиеПечатныхФормСервер.ПредставлениеСерий(ВыборкаСтрокТовары)
...
Функция ПечатьТОРГ12ЗаПоставщика(МассивОбъектов, ОбъектыПечати, БезУслуг = Ложь)
	...
	Для каждого Ссылка Из МассивОбъектов Цикл
		...
		Для Каждого ВыборкаСтрокТовары Из ВыборкаСтрок Цикл
			...
			абс_СерверныеФункции.ПредставлениеCOMCODE(ВыборкаСтрокТовары.Номенклатура) +    //АБС Коломиец 12077
...
Функция ПечатьТОРГ4(МассивОбъектов, ОбъектыПечати)
	...
	|	КратностьВзаиморасчетов КАК Кратность
	|ИЗ
	|	Документ.абс_НеотфактурованнаяПоставка КАК ПоступлениеТоваровУслуг
	|ГДЕ
	|	ПоступлениеТоваровУслуг.Ссылка = &ТекущийДокумент";
	...
	|		0                                                  КАК Метка
	|	ИЗ
	|		Документ.абс_НеотфактурованнаяПоставка.Товары КАК ПоступлениеТоваровУслуг
	|
	|	ГДЕ
	|		ПоступлениеТоваровУслуг.Ссылка = &ТекущийДокумент
	|
	...
	|	2                                   КАК Метка
	|ИЗ
	|	Документ.абс_НеотфактурованнаяПоставка.ВозвратнаяТара КАК ПоступлениеТоваровУслуг
	|
	|ГДЕ
	|	ПоступлениеТоваровУслуг.Ссылка = &ТекущийДокумент
	|
	...
	|	-1 КАК Метка
	|ИЗ
	|	Документ.абс_НеотфактурованнаяПоставка.Оборудование КАК ПоступлениеТоваровУслугОборудование
	|
	|ГДЕ
	|	ПоступлениеТоваровУслугОборудование.Ссылка = &ТекущийДокумент
	|
...
// АБС ВСТАВКА
Функция ПечатьБланкаСогласования(МассивОбъектов, ОбъектыПечати) 
	...
	|	абс_ИзменениеСтатусовПервичныхДокументов.Комментарий КАК Комментарий,
	|	абс_ИзменениеСтатусовПервичныхДокументов.СтатусДокумента КАК Статус,
	|	абс_ИзменениеСтатусовПервичныхДокументов.Пользователь.абс_Сотрудник КАК Исполнитель
	|ИЗ
	|	Документ.ПоступлениеТоваровУслуг КАК ПоступлениеТоваровУслуг
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.абс_ИзменениеСтатусовПервичныхДокументов КАК абс_ИзменениеСтатусовПервичныхДокументов
	|		ПО ПоступлениеТоваровУслуг.Ссылка = абс_ИзменениеСтатусовПервичныхДокументов.ПервичныйДокумент
	|ГДЕ
	|	ПоступлениеТоваровУслуг.Ссылка В(&МассивОбъектов)
	|	И абс_ИзменениеСтатусовПервичныхДокументов.ПервичныйДокумент В(&МассивОбъектов)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка,
	|	Период
...
Процедура ЗаполнитьПараметрыДоговора(Параметры,Договор)
	Попытка
		Параметры.Подразделение = Договор.Ответственный.абс_Сотрудник.ТекущееПодразделениеОрганизации;
		Параметры.СуммаДокумента = Договор.СуммаДокумента;
		Параметры.Ответственный = Договор.Ответственный.абс_Сотрудник;
