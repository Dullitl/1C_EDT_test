...
Процедура ПроверитьДублиНМА(Отказ, Заголовок)
	...
	Если ЕстьДубли Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя дублировать нематериальный актив!", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, 
	...
	СтруктураПараметровЗачетАванса = БухгалтерскийУчетРасчетовСКонтрагентами.ПодготовкаСтруктурыПараметровДляЗачетаАванса(Ссылка, мВалютаРегламентированногоУчета, Заголовок,СтруктураШапкиДокумента.СуммаДокумента);
