...
Процедура Автозаполнение(ВременнаяТаблица = Неопределено, Реквизиты = Неопределено, ОсновноеНачисление = Истина, ДатаАктуальности, ПарамНомерСтроки = Неопределено, КорректироватьДатыНачалаОкончания = Ложь) Экспорт
	...
	Запрос.УстановитьПараметр("ПарамПериодРегистрации",ДатаАктуальности);
	...
	Если КорректироватьДатыНачалаОкончания Тогда
		ТекстЗапроса = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВременнаяТаблица.Сотрудник КАК Сотрудник,
		|	ВременнаяТаблица.ПодразделениеОрганизации КАК Подразделение,
		|	ВременнаяТаблица.ДатаПриема,
		|	ВременнаяТаблица.ДатаУвольнения
	...
	Если КорректироватьДатыНачалаОкончания И ОсновноеНачисление Тогда
		ТекстЗапроса = ТекстЗапроса + "
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВременнаяТаблица.Сотрудник КАК Сотрудник,
		|	ВременнаяТаблица.Подразделение КАК Подразделение,
		|	НачисленияОрганизацийПоказатели.Показатель,
		|	ВЫБОР КОГДА ВременнаяТаблица.ДатаПриема > &ПарамДатаНачала ТОГДА ВременнаяТаблица.ДатаПриема ИНАЧЕ &ПарамДатаНачала КОНЕЦ КАК ДатаНачала,
		|	ВЫБОР КОГДА ВременнаяТаблица.ДатаУвольнения < &ПарамДатаОкончания ТОГДА ВременнаяТаблица.ДатаУвольнения ИНАЧЕ &ПарамДатаОкончания КОНЕЦ КАК ДатаОкончания,
		...
		|	И ВременнаяТаблица.ДатаПриема <= &ПарамДатаОкончания
		|	И ВЫБОР КОГДА ВременнаяТаблица.ДатаУвольнения <> ДАТАВРЕМЯ(1,1,1,0,0,0) ТОГДА ВременнаяТаблица.ДатаУвольнения >= &ПарамДатаНачала ИНАЧЕ ИСТИНА КОНЕЦ
		|;
		|";
	Иначе
		ТекстЗапроса = ТекстЗапроса + "
	...
	|	ВЫБОР 
	|		КОГДА (ЗначенияПоказателейСхемМотивации.Показатель.ВидПоказателя = ЗНАЧЕНИЕ(Перечисление.ВидыПоказателейСхемМотивации.Общий) Или ЗначенияПоказателейСхемМотивации.Показатель.ВидПоказателя = ЗНАЧЕНИЕ(Перечисление.ВидыПоказателейСхемМотивации.ДляВсехОрганизаций))
	...
	|			ВЫБОР
	|				КОГДА (ПериодическиеЗначенияПоказателейСхемМотивацииСрезПоследних.Показатель.ВидПоказателя = ЗНАЧЕНИЕ(Перечисление.ВидыПоказателейСхемМотивации.Общий) Или ПериодическиеЗначенияПоказателейСхемМотивацииСрезПоследних.Показатель.ВидПоказателя = ЗНАЧЕНИЕ(Перечисление.ВидыПоказателейСхемМотивации.ДляВсехОрганизаций))
	|					ТОГДА Значение(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)
	|					ИНАЧЕ ПериодическиеЗначенияПоказателейСхемМотивацииСрезПоследних.Подразделение
	|			КОНЕЦ КАК Подразделение,
	...
	|			NULL
	|		ИЗ
	|			РегистрСведений.ПериодическиеЗначенияПоказателейСхемМотивации.СрезПоследних(&ПарамДатаОкончания, Организация = &парамОрганизация ИЛИ Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)) КАК ПериодическиеЗначенияПоказателейСхемМотивацииСрезПоследних) КАК ЗначенияПоказателейСхемМотивации
	|		
	...
	|				И ОсновныеНачисленияОрганизацийПоказатели.Показатель.ВозможностьИзменения = ЗНАЧЕНИЕ(Перечисление.ИзменениеПоказателейСхемМотивации.НеИзменяется)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	...
	|	Показатели" + Сч +".Показатель КАК ППоказатель"+Сч +",
	|	ВЫБОР
	|		КОГДА Показатели" + Сч +".Показатель.ТипПоказателя = ЗНАЧЕНИЕ(Перечисление.ТипыПоказателейСхемМотивации.Денежный)
	|			ТОГДА ВЫБОР
	|					КОГДА Начисления.Валюта" + Сч +" В (&парамВалютаРегламентУчета, ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка))
	|						ТОГДА 
	|			ВЫБОР
	|				КОГДА Начисления.ПериодЗавершения <= &ПарамПериодРегистрации
	|						И Начисления.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|					ТОГДА Начисления.Показатель" + Сч +"Завершения
	|				ИНАЧЕ Начисления.Показатель" + Сч +"
	|			КОНЕЦ
	|					ИНАЧЕ 
	|						ВЫБОР
	|							КОГДА Начисления.ПериодЗавершения <= &ПарамПериодРегистрации
	...
	|								ТОГДА Начисления.Показатель" + Сч +"Завершения
	|							ИНАЧЕ Начисления.Показатель" + Сч +"
	|						КОНЕЦ * ЕСТЬNULL(Валюты" + Сч +".Курс / Валюты" + Сч +".Кратность, 0)
	|				КОНЕЦ";
	Если Сч = 1 Тогда
		Запрос.Текст = Запрос.Текст + "
	|		КОГДА Показатели" + Сч +".Показатель.ТипПоказателя = ЗНАЧЕНИЕ(Перечисление.ТипыПоказателейСхемМотивации.ТарифныйРазряд) ИЛИ Начисления.ВидРасчета.ТребуетВводаТарифногоРазряда
	|			ТОГДА ТарифныеРазряды" + Сч +".Размер";
	Иначе
		Запрос.Текст = Запрос.Текст + "
	|		КОГДА Показатели" + Сч +".Показатель.ТипПоказателя = ЗНАЧЕНИЕ(Перечисление.ТипыПоказателейСхемМотивации.ТарифныйРазряд)
	|			ТОГДА ТарифныеРазряды" + Сч +".Размер";
	КонецЕсли;
	Запрос.Текст = Запрос.Текст + "
	...
	Для Сч = 1 По 6 Цикл
		Запрос.Текст = Запрос.Текст + "
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалютДляРасчетовСПерсоналом КАК Валюты" + Сч +"
	|			ПО (Валюты" + Сч +".Период = &ПарамДатаОкончания)
	|			И Начисления.Валюта" + Сч +" = Валюты" + Сч +".Валюта
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РазмерТарифныхСтавок.СрезПоследних(&ПарамДатаОкончания, ) КАК ТарифныеРазряды" + Сч +"
	|		ПО	ВЫБОР
	|				КОГДА Начисления.ПериодЗавершения <= &ПарамДатаОкончания
	|						И Начисления.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|					ТОГДА Начисления.ТарифныйРазряд" + Сч +"Завершения
	|				ИНАЧЕ Начисления.ТарифныйРазряд" + Сч +"
	|			КОНЕЦ = ТарифныеРазряды" + Сч +".ТарифныйРазряд
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПоказателиИзПВР КАК Показатели" + Сч +"
	|			ПО Показатели" + Сч +".Ссылка = Начисления.ВидРасчета
	|			И Показатели" + Сч +".НомерСтроки = " + Сч;
	КонецЦикла;
	Запрос.Текст = Запрос.Текст + "
	|ГДЕ
	...
	|	ИЛИ Начисления.Показатель1Завершения  <> 0 ИЛИ Начисления.Показатель2Завершения  <> 0 ИЛИ Начисления.Показатель3Завершения <> 0  ИЛИ Начисления.Показатель4Завершения <> 0  ИЛИ Начисления.Показатель5Завершения <> 0  ИЛИ Начисления.Показатель6Завершения <> 0 
	|	ИЛИ Начисления.ТарифныйРазряд1 ЕСТЬ НЕ NULL ИЛИ Начисления.ТарифныйРазряд2 ЕСТЬ НЕ NULL ИЛИ Начисления.ТарифныйРазряд3 ЕСТЬ НЕ NULL ИЛИ Начисления.ТарифныйРазряд4 ЕСТЬ НЕ NULL ИЛИ Начисления.ТарифныйРазряд5 ЕСТЬ НЕ NULL ИЛИ Начисления.ТарифныйРазряд6 ЕСТЬ НЕ NULL
	|	ИЛИ Начисления.ТарифныйРазряд1Завершения ЕСТЬ НЕ NULL ИЛИ Начисления.ТарифныйРазряд2Завершения ЕСТЬ НЕ NULL ИЛИ Начисления.ТарифныйРазряд3Завершения ЕСТЬ НЕ NULL ИЛИ Начисления.ТарифныйРазряд4Завершения ЕСТЬ НЕ NULL ИЛИ Начисления.ТарифныйРазряд5Завершения ЕСТЬ НЕ NULL ИЛИ Начисления.ТарифныйРазряд6Завершения ЕСТЬ НЕ NULL
	|";
	...
	|	Показатели.ДатаНачала,
	|	Показатели.ДатаНачала КАК ДатаНачалаСобытия,
	|	Показатели.ДатаОкончания КАК ДатаОкончания,
	...
	|			ИЛИ Показатели.Показатель.ВидПоказателя = Значение(Перечисление.ВидыПоказателейСхемМотивации.ДляВсехОрганизаций))
	...
	|	Показатели.ДатаНачала,
	|	Показатели.ДатаОкончания";
	
	Если ПарамНомерСтроки = Неопределено Тогда
	...
	Иначе
		...
		Если Выборка.Следующий() Тогда
			ПарамНомерСтроки = ПарамНомерСтроки - 1;
			Если ОсновноеНачисление Тогда
				ОсновныеНачисления[ПарамНомерСтроки].ВидРасчета = Выборка.ВидРасчета;
				ОсновныеНачисления[ПарамНомерСтроки].ДатаНачала = Выборка.ДатаНачала;
				ОсновныеНачисления[ПарамНомерСтроки].ДатаНачалаСобытия = Выборка.ДатаНачалаСобытия;
				ОсновныеНачисления[ПарамНомерСтроки].ДатаОкончания = Выборка.ДатаОкончания;
				ОсновныеНачисления[ПарамНомерСтроки].Авторасчет = Выборка.Авторасчет;
				ОсновныеНачисления[ПарамНомерСтроки].Сотрудник = Выборка.Сотрудник;
				ОсновныеНачисления[ПарамНомерСтроки].ПодразделениеОрганизации = Выборка.ПодразделениеОрганизации;
				Для СчКолонки = 1 По 6 Цикл
					ОсновныеНачисления[ПарамНомерСтроки]["Показатель"+СчКолонки] = Выборка["Показатель"+СчКолонки];
				КонецЦикла;
				
			Иначе
				ДополнительныеНачисления[ПарамНомерСтроки].ВидРасчета = Выборка.ВидРасчета;
				ДополнительныеНачисления[ПарамНомерСтроки].ДатаНачала = Выборка.ДатаНачала;
				ДополнительныеНачисления[ПарамНомерСтроки].ДатаОкончания = Выборка.ДатаОкончания;
				ДополнительныеНачисления[ПарамНомерСтроки].Авторасчет = Выборка.Авторасчет;
				ДополнительныеНачисления[ПарамНомерСтроки].Сотрудник = Выборка.Сотрудник;
				ДополнительныеНачисления[ПарамНомерСтроки].ПодразделениеОрганизации = Выборка.ПодразделениеОрганизации;
				ДополнительныеНачисления[ПарамНомерСтроки].КодВычета = Выборка.КодВычета;
				Для СчКолонки = 1 По 6 Цикл
					ДополнительныеНачисления[ПарамНомерСтроки]["Показатель"+СчКолонки] = Выборка["Показатель"+СчКолонки];
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция СформироватьЗапросПоДопНачисления()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НДФЛВычетыПоДоходам.СрезПоследних(&ДатаРегистрации, КодДохода = ЗНАЧЕНИЕ(Справочник.ДоходыНДФЛ.МатпомощьПриРожденииРебенка)) КАК ВычетПоМатпомощиПриРождении
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок = "")
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.Организация) Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке(ОбщегоНазначения.ПреобразоватьСтрокуИнтерфейса("Не указана организация, по которой выполняются начисления!"), Отказ, Заголовок);
	КонецЕсли;
	
	//  ПериодРегистрации
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ПериодРегистрации) Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке("Не указан месяц, в котором выполняются начисления!", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиНачисления(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Сотрудник) Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не задан Сотрудник!", Отказ, Заголовок);
	КонецЕсли;
	...
	ИначеЕсли Не ВыборкаПоСтрокамДокумента.СтрокаИсправления Тогда
		Если ВыборкаПоШапкеДокумента.РегистрируютсяПочасовыеОтклонения <> ВыборкаПоСтрокамДокумента.ЯвляетсяПочасовымОтклонением Тогда
			ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "указанный вид расчета не соответствует способу регистрации времени в документе!", Отказ, Заголовок);
		КонецЕсли;
	...
	Если ВыборкаПоШапкеДокумента.РегистрируютсяПочасовыеОтклонения И ВыборкаПоСтрокамДокумента.ЯвляетсяПочасовымОтклонением Тогда
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ВидИспользованияРабочегоВремени) Тогда
			ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указан учет времени для вида расчета!", Отказ, Заголовок);
		КонецЕсли;
		
		// ОплачиватьЧасов
		Если ВыборкаПоСтрокамДокумента.ОплачиватьЧасов <= 0 Или ВыборкаПоСтрокамДокумента.ОплачиватьЧасов > 24 Тогда
			ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "количество оплачиваемых часов должно быть положительным и не превышать 24!", Отказ, Заголовок);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ПериодДействияНачало) Тогда
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ГрафикРаботы) Тогда
			ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "по состоянию на " + Формат(ВыборкаПоСтрокамДокумента.ПериодДействияНачало,"ДЛФ=DD") + " не задан рабочий график сотрудника, либо он еще не принят на работу!", Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиДополнительныеНачисления(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок = "")
	...
	ИначеЕсли ВыборкаПоСтрокамДокумента.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.Процентом
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.БазовыйПериодНачало) Тогда
			ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указана дата начала базового периода начисления!", Отказ, Заголовок);
		КонецЕсли;
		
		// Дата окончания
		Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.БазовыйПериодКонец) Тогда
			ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указана дата окончания базового периода начисления!", Отказ, Заголовок);
		КонецЕсли;
...
Процедура СформироватьДоходыПоКодамНДФЛ(ВыборкаПоШапкеДокумента, НаборЗаписей)
	...
	|		ВЫБОР
	|			КОГДА Дополнительные.КодВычета = ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.МатпомощьПриРожденииРебенка)
	|				ТОГДА ВЫБОР
	|						КОГДА Дополнительные.Сторно
	|							ТОГДА ВЫБОР
	|									КОГДА -Дополнительные.Результат < Дополнительные.СуммаВычета * ВычетПоМатпомощиПриРождении.НормативЗатрат
	|										ТОГДА Дополнительные.Результат
	|									ИНАЧЕ -Дополнительные.СуммаВычета * ВычетПоМатпомощиПриРождении.НормативЗатрат
	|								КОНЕЦ
	|						КОГДА Дополнительные.Результат < Дополнительные.СуммаВычета * ВычетПоМатпомощиПриРождении.НормативЗатрат
	|							ТОГДА Дополнительные.Результат
	|						ИНАЧЕ Дополнительные.СуммаВычета * ВычетПоМатпомощиПриРождении.НормативЗатрат
	...
	|		ВЫБОР
	|			КОГДА Дополнительные.КодВычета = ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.МатпомощьПриРожденииРебенка)
	|				ТОГДА Дополнительные.СуммаВычета
	|			ИНАЧЕ 0
	|		КОНЕЦ,
	...
	|			ПО Дополнительные.ВидРасчета.КодДоходаНДФЛ = ВычетПоМатпомощиПриРождении.КодДохода
	|	ГДЕ
	...
	Пока ДоходыПоКодам.Следующий() Цикл
		...
		ЗаполнитьЗначенияСвойств(Движение,ДоходыПоКодам); // Период, Физлицо, КодДохода, СуммаДохода, СуммаВычета, КодВычета, ПодразделениеОрганизации, КоличествоДетей, ВидРасчета
		ЗаполнитьЗначенияСвойств(Движение,ВыборкаПоШапкеДокумента); // ГоловнаяОрганизация, ОбособленноеПодразделение
