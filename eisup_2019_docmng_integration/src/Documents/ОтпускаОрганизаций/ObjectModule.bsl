...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	ИначеЕсли ИмяМакета = "Отпуска" тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,Метаданные().Синоним + " (форма Т-7)"));
		Иначе
...
Процедура Автозаполнение(ДатаНачала, ДатаОкончания) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Организация) Тогда
		Сообщить(ОбщегоНазначения.ПреобразоватьСтрокуИнтерфейса("Не указана организация!"));
		Возврат
	КонецЕсли;
	...
	Для Каждого Работник Из РаботникиОрганизации Цикл
		...
		ЗаполнитьЗначенияСвойств(Строка, Работник);
	...
	Пока ВыборкаПоОтпускам.Следующий() Цикл
		Строка = РаботникиОрганизации.Найти(ВыборкаПоОтпускам.НомерСтроки, "НомерСтроки");
		
		Строка.РабочийГодС	= ВыборкаПоОтпускам.ДатаНачалаРабочегоГода;
		Строка.РабочийГодПо= ВыборкаПоОтпускам.ДатаОкончанияРабочегоГода;
		...
		Если ВыборкаПоВидамОтпусков.Следующий() Тогда
			...
			Пока ВыборкаПоОстаткам.Следующий() Цикл
				...
				Если ДнейОтпуска <= 0 Тогда
					Строка.РабочийГодПо= ВыборкаПоОстаткам.ДатаОкончанияРабочегоГода;
					Прервать;
				КонецЕсли;
			КонецЦикла;
...
Функция ПечатьТ6_2004()
	...
	Пока ВыборкаРаботники.Следующий() Цикл
		...
		//ОбластьМакета.Параметры.Работник = ОбщегоНазначения.
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.Организация) Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке(ОбщегоНазначения.ПреобразоватьСтрокуИнтерфейса("Не указана организация, сотрудники которой отправляются в отпуск!"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиРаботникаОрганизации(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если НетВидаОтпуска Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указан вид отпуска!", Отказ, Заголовок);
	...
	Иначе
		...
		Если ПричинаОтсутствия <> Перечисления.ПричиныОтсутствияНаРаботеВОрганизации.ОтпускЕжегодный
			...
			ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "вместо отпуска указана иная причина отсутствия!", Отказ, Заголовок);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ВидДополнительногоОтпуска) 
		...
		ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "неверно указана дата начала дополнительного отпуска!", Отказ, Заголовок);
	КонецЕсли;
