...
//
Функция ДобавитьРаздел(абс_НомерГТД = Неопределено,абс_ЗакупочныйЗаказТЧ = Неопределено, СтавкаНДС = Неопределено) Экспорт
	...
	НовыйРаздел.СтавкаНДС      = ?(СтавкаНДС = Неопределено,УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(глЗначениеПеременной("глТекущийПользователь"), "ОсновнаяСтавкаНДС"),СтавкаНДС);
	...
	НовыйРаздел.абс_НомерГТД = абс_НомерГТД;
	НовыйРаздел.абс_ЗакупочныйЗаказТЧ = абс_ЗакупочныйЗаказТЧ;
...
//
Процедура ПосчитатьИтогиПоРазделу(НомерРаздела, ВсегоСтоимость, ВсегоПошлина, ВсегоНДС, ВсегоСтоимость1 = 0, ВсегоПошлина1 = 0, ВсегоНДС1 = 0)  Экспорт
	...
	Для каждого ЭлементМассива Из Разделы Цикл
		ВсегоСтоимость1 = ВсегоСтоимость1 + ЭлементМассива.ТаможеннаяСтоимость;
		ВсегоПошлина1   = ВсегоПошлина1   + ЭлементМассива.СуммаПошлины;
		ВсегоНДС1       = ВсегоНДС1       + ЭлементМассива.СуммаНДС;
	КонецЦикла;	
...
//
Процедура ЗаполнитьПоПоступлению(ДокументПоступления, НомерРаздела, врТаблица = Неопределено) Экспорт
...
Процедура ПроверитьЗаполнениеТабличнойЧастиРазделы(ТаблицаПоРазделам, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Для каждого Раздел Из ТаблицаПоРазделам Цикл
		...
		Если ВсегоПошлина <> Раздел.СуммаПошлины Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("По разделу """ + СокрЛП(НомерРаздела) + """ сумма пошлины не совпадает с итогом по товарам раздела", Отказ, Заголовок);
		КонецЕсли;

		Если ВсегоНДС <> Раздел.СуммаНДС Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("По разделу """ + СокрЛП(НомерРаздела) + """ сумма НДС не совпадает с итогом по товарам раздела", Отказ, Заголовок);
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеТабличнойЧастиТовары(ТаблицаПоТоварам, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	//		ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + СтрокаСообщения);
	//	КонецЕсли;
	//КонецЦикла;
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
		...
		Если СтруктураШапкиДокумента.ВидДоговора<> Перечисления.ВидыДоговоровКонтрагентов.Прочее Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Договор для отражения валютного депозита должен иметь вид ""Прочее"".", Отказ, Заголовок);
		КонецЕсли;

		Если СтруктураШапкиДокумента.ВедениеВзаиморасчетов <> Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоДоговоруВЦелом Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Договор для отражения валютного депозита может вестись только по договору в целом.", Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
	
	Если СтруктураШапкиДокумента.ВидДоговораРегл <> Перечисления.ВидыДоговоровКонтрагентов.Прочее Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Договор для отражения рублевого депозита должен иметь вид ""Прочее"".", Отказ, Заголовок);
	КонецЕсли;
	...
	Если СтруктураШапкиДокумента.ВедениеВзаиморасчетовРегл <> Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоДоговоруВЦелом Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Договор для отражения рублевого депозита может вестись только по договору в целом.", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ВыполнитьРаспределениеПоСкладам(ТаблицаРаспределяемая, масКолонкиРаспределяемые)
	...
	|			)
	|СГРУППИРОВАТЬ ПО Ссылка, Номенклатура, ХарактеристикаНоменклатуры, СерияНоменклатуры, Склад
	|ИТОГИ Сумма(Количество) ПО ДокументОприходования, Номенклатура, ХарактеристикаНоменклатуры, СерияНоменклатуры";
...
Процедура ПодготовитьТаблицуТоваровУпр(ТаблицаТоваров, СтруктураШапкиДокумента)
	...
	МассивТаможенныйСбор    = ttk_ОбщегоНазначения.РаспределитьПропорционально(ТаможенныйСбор,    МассивКоэф, 2);
	МассивТаможенныйСборВал = ttk_ОбщегоНазначения.РаспределитьПропорционально(ТаможенныйСборВал, МассивКоэф, 2);
...
Процедура ПодготовитьТаблицуТоваровРегл(ТаблицаТоваров, СтруктураШапкиДокумента)
	...
	Если ТамСборРегл > 0 Тогда
		...
		МассивСумм = ttk_ОбщегоНазначения.РаспределитьПропорционально(ТамСборРегл, МассивКоэф, 2);
...
Процедура ДвиженияПоРегистрамУпр(СтруктураШапкиДокумента, ТаблицаПоТоварам, Отказ)
	...
	Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		...
		Иначе		
			...
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоТоварамЗакупки, ТаблицаДвижений);
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ПоступлениеТоваров
	 ИЛИ ПоступлениеТоваровНТТ Тогда
		...
		Если ПоступлениеТоваровНТТ Тогда
		...
		ИначеЕсли ПоступлениеТоваров Тогда
		
			Запрос = новый Запрос;
			Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
			               |	ПоступлениеТоваровУслугТовары.СерияНоменклатуры.НомерГТД КАК СерияНоменклатурыНомерГТД,
			               |	ПоступлениеТоваровУслугТовары.абс_ЗакупочныйЗаказТЧ КАК абс_ЗакупочныйЗаказТЧ,
			               ...
			               |	СерияНоменклатурыНомерГТД,
			               |	СтавкаНДС
			               |;
			               |
			               |////////////////////////////////////////////////////////////////////////////////
			               |ВЫБРАТЬ РАЗРЕШЕННЫЕ
			               |	КОЛИЧЕСТВО(вр1.СерияНоменклатурыНомерГТД) КАК НомерРаздела,
			               |	вр.СерияНоменклатурыНомерГТД КАК абс_НомерГТД,
			               |	вр.абс_ЗакупочныйЗаказТЧ,
			               |	вр.СтавкаНДС
			               |ИЗ
			               |	вр КАК вр
			               |		ЛЕВОЕ СОЕДИНЕНИЕ вр КАК вр1
			               |		ПО вр.СерияНоменклатурыНомерГТД >= вр1.СерияНоменклатурыНомерГТД
			               |
			               |СГРУППИРОВАТЬ ПО
			               |	вр.СерияНоменклатурыНомерГТД,
			               |	вр.абс_ЗакупочныйЗаказТЧ,
			               |	вр.СтавкаНДС
			               |
			               |УПОРЯДОЧИТЬ ПО
			...
			Пока Результат.Следующий() Цикл
				...
				НомерРаздела = Результат.НомерРаздела;
				
				// Заполним Табличную часть
				Раздел    = ДобавитьРаздел(Результат.абс_НомерГТД, Результат.абс_ЗакупочныйЗаказТЧ, Результат.СтавкаНДС);
				ЗаполнитьПоГТДиЗЗ(Основание, НомерРаздела, Результат.абс_НомерГТД, Результат.абс_ЗакупочныйЗаказТЧ);
...
Процедура ЗаполнитьПоГТДиЗЗ(Основание, НомерРаздела, абс_НомерГТД, абс_ЗакупочныйЗаказТЧ)
	
	НомерРаздела = ?(НомерРаздела = 0,1,НомерРаздела);
	
	ТоварыБулево = Основание.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ПокупкаКомиссия;
	
	Запрос = новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
					|	*
					|ИЗ
					|	Документ.ПоступлениеТоваровУслуг."+?(ТоварыБулево,"Товары","Оборудование")+" КАК ТЧ
					|ГДЕ
					|	ТЧ.Ссылка = &Ссылка
					//|	И ТЧ.абс_ЗакупочныйЗаказТЧ = &абс_ЗакупочныйЗаказТЧ
					|	И выбор когда &абс_НомерГТД_Отбор Тогда ТЧ.СерияНоменклатуры.НомерГТД = &абс_НомерГТД иначе истина конец";
	
	запрос.УстановитьПараметр("Ссылка",Основание);
	запрос.УстановитьПараметр("абс_ЗакупочныйЗаказТЧ",абс_ЗакупочныйЗаказТЧ);
	запрос.УстановитьПараметр("абс_НомерГТД",абс_НомерГТД);
	запрос.УстановитьПараметр("абс_НомерГТД_Отбор",ЗначениеЗаполнено(абс_НомерГТД));
	
	ТаблицаЗначенийТовары =  Запрос.Выполнить().Выгрузить();
	     
	Если НЕ ТоварыБулево Тогда
		ТаблицаЗначенийТовары = Основание.Оборудование.Выгрузить();
		ТаблицаЗначенийТовары.Колонки.Добавить("СтатусПартии");
		ТаблицаЗначенийТовары.ЗаполнитьЗначения(Перечисления.СтатусыПартийТоваров.Оборудование,"СтатусПартии");
	...
	Для каждого СтрокаТаблицыЗначений Из ТаблицаЗначенийТовары Цикл
		...
		// Реквизиты, которых нет в оборудовании.
		Если ТоварыБулево Тогда
			НоваяСтрока.ЗаказПокупателя = СтрокаТаблицыЗначений.абс_ЗакупочныйЗаказТЧ;
		КонецЕсли;

		НоваяСтрока.ФактурнаяСтоимость = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СтрокаТаблицыЗначений.Сумма, Основание.ВалютаДокумента,
		                                 ?(ВВалютеРеглУчета, мВалютаРегламентированногоУчета, ВалютаДокумента),
		                                 ЗаполнениеДокументов.КурсДокумента(Основание, мВалютаРегламентированногоУчета),
		                                 ?(ВВалютеРеглУчета, КурсРеглУчета, КурсДокумента),
		                                 ЗаполнениеДокументов.КратностьДокумента(Основание, мВалютаРегламентированногоУчета),
		                                 ?(ВВалютеРеглУчета, КратностьРеглУчета, КратностьДокумента));
...
Процедура СформироватьТаблицыПоДокументу(СтруктураШапкиДокумента, ТоварыВВалюте, ТоварыВРублях)
	...
	Если не СтруктураШапкиДокумента.ТаможенныйСборВал = 0 и не ТоварыВВалюте.Итог("ФактурнаяСтоимость") = 0  тогда
        МассивРезультата = ttk_ОбщегоНазначения.РаспределитьПропорционально(СтруктураШапкиДокумента.ТаможенныйСборВал,ТоварыВВалюте.ВыгрузитьКолонку("ФактурнаяСтоимость"));
	...
	Если не СтруктураШапкиДокумента.ТаможенныйСбор = 0 и не ТоварыВРублях.Итог("ФактурнаяСтоимость") = 0  тогда
        МассивРезультата = ttk_ОбщегоНазначения.РаспределитьПропорционально(СтруктураШапкиДокумента.ТаможенныйСбор,ТоварыВРублях.ВыгрузитьКолонку("ФактурнаяСтоимость"));
