	...
	Если ВидСтатьиЗатрат = "Нематериальные" Тогда
		Если СтатьяЗатрат.ВидЗатрат = Перечисления.ВидыЗатрат.Материальные
		   И СтатьяЗатрат.ХарактерЗатрат = Перечисления.ХарактерЗатрат.ПроизводственныеРасходы Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Укажите статью затрат с нематериальным видом затрат!", Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
...
Функция ПроверкаРеквизитов(Отказ, Заголовок) Экспорт
	...
	ПроверитьЗаполнениеСубконтоТабличнойЧастиМатериалыРегл(Отказ, Заголовок);		
...
//
Процедура ПроверитьЗаполнениеСубконтоТабличнойЧастиМатериалыРегл(Отказ, Заголовок)
	...
	Если ОтражатьВБухгалтерскомУчете И Не ПроизводственныеРасходы Тогда
		...
		Если ОтражатьВНалоговомУчете И Не ПроизводственныеРасходыНУ Тогда
			СтруктураПараметров = Новый Структура("СчетДтНУ, СубконтоДтНУ1, СубконтоДтНУ2, СубконтоДтНУ3",
				"Не выбран счет дебета (нал. учет)",
				"Не указано субконто 1 дебета счета (нал. учет)",
				"Не указано субконто 2 дебета счета (нал. учет)",
				"Не указано субконто 3 дебета счета (нал. учет)");
			УправлениеЗатратами.ПроверитьЗаполнениеСубконто(ЭтотОбъект, СтруктураПараметров,, Отказ, Заголовок);
		КонецЕсли;
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоМатериалам, Отказ, Заголовок)
	...
	КонецЕсли; //АБС Коломиец 18232, 18726	
	//Иначе
...
Процедура абсДвиженияПоРегистрамПартийМатериаловВЭксплуатации(СтруктураШапкиДокумента, ТаблицаПоМатериалам, Отказ, Заголовок)
	...
	Для Каждого РазделУчета Из СтруктураРазделовУчета Цикл
		...
		//|	СписаниеМатериаловИзЭксплуатацииМатериалы.ДокументПередачи,
		//|	СписаниеМатериаловИзЭксплуатацииМатериалы.ФизЛицо,
		//|	СписаниеМатериаловИзЭксплуатацииМатериалы.НазначениеИспользования,
		//|	СписаниеМатериаловИзЭксплуатацииМатериалы.НазначениеИспользования.СпособПогашенияСтоимости КАК СпособПогашенияСтоимости,
		//|	СписаниеМатериаловИзЭксплуатацииМатериалы.НазначениеИспользования.СрокПолезногоИспользования КАК СрокПолезногоИспользования,
		//|	СписаниеМатериаловИзЭксплуатацииМатериалы.Количество,
		//|	СписаниеМатериаловИзЭксплуатацииМатериалы.Сумма,
		//|	СписаниеМатериаловИзЭксплуатацииМатериалы.НомерСтроки,
		//|	СписаниеМатериаловИзЭксплуатацииМатериалы.Ссылка.Организация,
		//|	СписаниеМатериаловИзЭксплуатацииМатериалы.СчетПередачиБУ КАК СчетУчета
		//|ПОМЕСТИТЬ ВТ_Материалы
		//|ИЗ
		//|	Документ.СписаниеМатериаловИзЭксплуатации.Материалы КАК СписаниеМатериаловИзЭксплуатацииМатериалы
		//|ГДЕ
		//|	СписаниеМатериаловИзЭксплуатацииМатериалы.Ссылка = &Ссылка
		//|;
		//|
		//|////////////////////////////////////////////////////////////////////////////////
		//|ВЫБРАТЬ
		...
		//|	ВЫБОР
		//|		КОГДА ЕСТЬNULL(ПартииМатериаловВЭксплуатацииОстатки.КоличествоОстаток, 0) < ВТ_МатериалыСвернуто.Количество
		//|			ТОГДА ИСТИНА
		//|		ИНАЧЕ ЛОЖЬ
		//|	КОНЕЦ КАК НедостаточноКоличестваПоРегистру,
		//|	&Подразделение
		//|ИЗ
		...
		//".Остатки(&Период, Подразделение = &Подразделение И Организация = &Организация) КАК ПартииМатериаловВЭксплуатацииОстатки
		...
		//АБС ВСТАВКА 35878  10.12.2013 13:25:00  Шамов
		Если ЭтоУУ Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ВТ_МатериалыСвернуто.СчетУчета = ПартииМатериаловВЭксплуатацииОстатки.СчетУчета", "И ИСТИНА");
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТ_МатериалыСвернуто.Организация = ПартииМатериаловВЭксплуатацииОстатки.Организация", "ИСТИНА");
		...
		//ПараметрыОтбора.Вставить("НедостаточноКоличестваПоРегистру", Истина);
		//
		//НайденныеСтроки = ОстаткиПоРегистру.НайтиСтроки(ПараметрыОтбора);
		//
...
Процедура ОбработкаЗаполнения(Основание)
	...
	// АБС ВСТАВКА 66268		
	// Продажа малоценки осуществляется на закладке "Услуги" документа РТУ, 
	// номенклатура с видом "Основные средства до 20 тыс. рублей" 
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		абс_Основание				= Основание.Ссылка;
		ТипСтоимости 				= Перечисления.ВидыНормативнойСтоимостиПроизводства.Рассчитывается;
		
		ВидНоменклатурыМалоценка = Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Основные средства менее 20 тыс.руб.", Истина);
		
		Для Каждого СтрокаУслуги Из Основание.Услуги Цикл
			
			Если НЕ СтрокаУслуги.Номенклатура.ВидНоменклатуры = ВидНоменклатурыМалоценка Тогда
				Продолжить;
			КонецЕсли;
			...
			НоваяСтрока.ЕдиницаИзмерения           = СтрокаУслуги.Номенклатура.ЕдиницаХраненияОстатков;
			НоваяСтрока.ЕдиницаИзмеренияМест       = СтрокаУслуги.Номенклатура.ЕдиницаХраненияОстатков;
			НоваяСтрока.Количество                 = СтрокаУслуги.Количество;
			//НоваяСтрока.КоличествоМест             = ТекСтрокаМатериалы.КоличествоМест;
			//НоваяСтрока.Коэффициент                = ТекСтрокаМатериалы.Коэффициент;
			НоваяСтрока.Коэффициент				   = НоваяСтрока.ЕдиницаИзмерения.Коэффициент;
			
			НоваяСтрока.СчетПередачиБУ             = ПланыСчетов.Хозрасчетный.НайтиПоКоду("10.12");
			НоваяСтрока.СчетПередачиНУ             = ПланыСчетов.Налоговый.НайтиПоКоду("10.12");
