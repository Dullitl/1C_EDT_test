...
Процедура ЗаполнитьДокумент(Основание) Экспорт
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ПриемНаРаботуВОрганизацию") Тогда
		...
		"ВЫБРАТЬ
		|	ПринятыеРаботники.Сотрудник,
		|	ПринятыеРаботники.ДатаПриема КАК ДатаИзменения
		|ИЗ
		|	Документ.ПриемНаРаботуВОрганизацию.РаботникиОрганизации КАК ПринятыеРаботники
		|ГДЕ
		|	ПринятыеРаботники.Ссылка = &ДокументСсылка";
		
		РаботникиОрганизации.Загрузить(ЗапросДляЗаполнения.Выполнить().Выгрузить());
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.КадровоеПеремещениеОрганизаций") Тогда
		...
		|	КадровоеПеремещение.ДатаНачала КАК ДатаИзменения
		|ИЗ
...
Функция СформироватьЗапросПоРаботникиОрганизации(ВыборкаПоШапкеДокумента)
	...
	|	ПоследующиеДвижение.Регистратор,
	...
	|			УчетОсновногоЗаработкаРаботниковОрганизацииВРегламентированномУчете.Регистратор КАК Регистратор,
	|			ПРЕДСТАВЛЕНИЕ(УчетОсновногоЗаработкаРаботниковОрганизацииВРегламентированномУчете.Регистратор) КАК РегистраторПредставление
	|		ИЗ
	...
	|		
	|		СГРУППИРОВАТЬ ПО
	|			УчетОсновногоЗаработкаРаботниковОрганизацииВРегламентированномУчете.Регистратор,
	|			ПРЕДСТАВЛЕНИЕ(УчетОсновногоЗаработкаРаботниковОрганизацииВРегламентированномУчете.Регистратор),
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ОбособленноеПодразделение) Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке(ОбщегоНазначения.ПреобразоватьСтрокуИнтерфейса("Не указана организация, для которой ведется учет!"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиРаботникаОрганизации(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Период) Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указана дата изменения учета сотрудника!", Отказ, Заголовок);
	КонецЕсли;
	
	//// СпособОтраженияВБухучете
	//Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.СпособОтраженияВБухучете)  Тогда
	//	//ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указан способ отражения заработка работника в бухгалтерском учете!", Отказ);
	...
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.КонфликтнаяСтрокаНомер)  Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "по сотруднику " + ВыборкаПоСтрокамДокумента.СотрудникНаименование + " найдена повторяющаяся строка № " + ВыборкаПоСтрокамДокумента.КонфликтнаяСтрокаНомер + "!", Отказ, Заголовок);
	КонецЕсли;
	
	// повторяющиеся строки
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.РегистраторПредставление)  Тогда
		Расшифровки = Новый Массив;
		Расшифровки.Добавить(Новый Структура("Представление, Расшифровка", ВыборкаПоСтрокамДокумента.РегистраторПредставление, ВыборкаПоСтрокамДокумента.Регистратор));
		ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "способ учета основного заработка сотрудника " + ВыборкаПоСтрокамДокумента.СотрудникНаименование + " уже зарегистрирован ранее документом ", Отказ, Заголовок, , Расшифровки);
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если	ТипЗнч(Основание) = Тип("ДокументСсылка.КадровоеПеремещениеОрганизаций")
		ИЛИ	ТипЗнч(Основание) = Тип("ДокументСсылка.ПриемНаРаботуВОрганизацию") Тогда
		...
		Если Основание.Проведен Тогда
			...
			ОтражаемыйДокумент = Основание;
			ЭтоКадровоеПеремещение = Ложь;
			Если ТипЗнч(Основание) = Тип("ДокументСсылка.ПриемНаРаботуВОрганизацию") Тогда
				РеквизитДатаС	= "ДатаПриема";
				
			ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.КадровоеПеремещениеОрганизаций") Тогда
				ЭтоКадровоеПеремещение = Истина;
			...
			Для Каждого  СтрокаОснования Из Основание.РаботникиОрганизации Цикл
				...
				// Для даты окончания
				Если ЭтоКадровоеПеремещение И Не СтрокаОснования.НапомнитьПоЗавершении И ЗначениеЗаполнено(СтрокаОснования.ДатаОкончания) Тогда
					НоваяСтрока = РаботникиОрганизации.Добавить();
					НоваяСтрока.Сотрудник		= СтрокаОснования.Сотрудник;
					НоваяСтрока.ФизЛицо			= СтрокаОснования.ФизЛицо;
					НоваяСтрока.ДатаИзменения	= СтрокаОснования.ДатаОкончания;
				КонецЕсли;
