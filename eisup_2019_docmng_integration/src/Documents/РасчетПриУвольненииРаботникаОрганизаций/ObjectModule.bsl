...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ИмяМакета = "РасчетСреднегоЗаработка" Тогда
		Если ДнейЧасовВыходногоПособия = 0 И ДнейЧасовСохраненияСреднегоЗаработка = 0 Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("По документу не выплачивается сохраняемый средний заработок!");
			Возврат Неопределено;
		КонецЕсли;
	...
	ИначеЕсли ИмяМакета = "РасчетКомпенсацииОтпуска" Тогда
		Если ДнейЧасовКомпенсацииУдержанияОтпуска = 0 Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("По документу не выплачивается компенсация отпуска!");	
			Возврат Неопределено;
		КонецЕсли;
		...
		Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,Метаданные().Синоним) + " (расчет среднего заработка " + ?(ПризнакКомпенсацииОтпуска,"компенсации отпуска","удержания за отпуск") + ")");
	...
	ИначеЕсли ЗначениеЗаполнено(ИмяМакета) Тогда
		...
		Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,Метаданные().Синоним) + РасчетПриУвольненииРаботникаОрганизацийПереопределяемый.ОписаниеРегламентированнойПечатнойФормы(ЭтотОбъект, ИмяМакета));
	КонецЕсли;
...
Процедура Рассчитать(РассчитываемыеТаблицы, РежимПерерасчета = Ложь, ВыборкаПоШапкеДокумента = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДатаУвольнения) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указана дата увольнения сотрудника!", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ОпределитьРасчетныйПериод() Экспорт
	...
	Если ЗначениеЗаполнено(ДатаУвольнения) Тогда 
		...
		ПериодРасчетаСреднегоЗаработкаНачало = ДобавитьМесяц(НачалоМесяца(ДатаУвольнения), - МесяцевРасчетногоПериода);
...
Функция СформироватьЗапросПоШапке() Экспорт
	...
	|	ВЫБОР
	|		КОГДА ПараметрыВидаРасчетаВыходноеПособие.ПорядокОпределенияРасчетногоПериодаСреднегоЗаработка = ЗНАЧЕНИЕ(Перечисление.ПорядокОпределенияРасчетногоПериодаСреднегоЗаработка.ПоКолдоговору)
	|			ТОГДА ПараметрыВидаРасчетаВыходноеПособие.ЧислоМесяцев
	|		ИНАЧЕ 12
	|	КОНЕЦ КАК ПериодРасчетаСреднегоЗаработкаВыходноеПособие,
	|	ВЫБОР
	|		КОГДА ПараметрыВидаРасчетаСохраняемыйЗаработокНаВремяТрудоустройства.ПорядокОпределенияРасчетногоПериодаСреднегоЗаработка = ЗНАЧЕНИЕ(Перечисление.ПорядокОпределенияРасчетногоПериодаСреднегоЗаработка.ПоКолдоговору)
	|			ТОГДА ПараметрыВидаРасчетаСохраняемыйЗаработокНаВремяТрудоустройства.ЧислоМесяцев
	|		ИНАЧЕ 12
	|	КОНЕЦ КАК ПериодРасчетаСреднегоЗаработкаСохраняемыйЗаработокНаВремяТрудоустройства,
	|	ВЫБОР
	|		КОГДА ВЫБОР
	|				КОГДА РасчетПриУвольненииРаботникаОрганизации.ПризнакКомпенсацииОтпуска
	|					ТОГДА ПараметрыВидаРасчетаКомпенсация.ПорядокОпределенияРасчетногоПериодаСреднегоЗаработка
	|				КОГДА ЕСТЬNULL(ВариантыВстроенныхАлгоритмовРасчетаЗарплаты.УдержаниеЗаОтпускУменьшаетНалоги, ЛОЖЬ)
	|					ТОГДА ПараметрыВидаРасчетаКомпенсация.ПорядокОпределенияРасчетногоПериодаСреднегоЗаработка
	|				ИНАЧЕ ПараметрыВидаРасчетаУдержание.ПорядокОпределенияРасчетногоПериодаСреднегоЗаработка
	|			КОНЕЦ = ЗНАЧЕНИЕ(Перечисление.ПорядокОпределенияРасчетногоПериодаСреднегоЗаработка.ПоКолдоговору)
	|			ТОГДА ВЫБОР
	|					КОГДА РасчетПриУвольненииРаботникаОрганизации.ПризнакКомпенсацииОтпуска
	...
	|	КОНЕЦ КАК ПериодРасчетаСреднегоЗаработкаОтпуск,
	|	ВЫБОР
	|		КОГДА РасчетПриУвольненииРаботникаОрганизации.ПризнакКомпенсацииОтпуска
	|			ТОГДА ВЫБОР
...
Функция СформироватьЗапросПоНачислениям(ВыборкаПоШапкеДокумента) Экспорт
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Удержание,
	|	ЕСТЬNULL(Начисление.ВидРасчета.ВидЕжегодногоОтпуска, ПараметрыВидаРасчетаКомпенсация.ВидЕжегодногоОтпуска) КАК ВидЕжегодногоОтпуска
	|ИЗ
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок = "")
	...
	Если Не ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ДатаУвольнения) Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке("Не указана дата увольнения!", Отказ, Заголовок);
	КонецЕсли;
	...
	Если ВыборкаПоШапкеДокумента.ДнейЧасовКомпенсацииУдержанияОтпуска <> 0 И НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ПорядокРасчетаОтпуска) Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке("Не  указан порядок расчета отпуска!", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ДобавитьСтрокуВДвиженияПоКомпенсации(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента)
	...
	Движение.ВидЕжегодногоОтпуска	= ВыборкаПоСтрокамДокумента.ВидЕжегодногоОтпуска;
	...
	Движение.ДатаРегистрации		= ВыборкаПоШапкеДокумента.Дата;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если НЕ ЗначениеЗаполнено(ДатаУвольнения) Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке("Не указана дата увольнения работника!", Отказ, Заголовок);
	КонецЕсли;
	...
	Если НЕ Отказ Тогда
		...
		Пока ВыборкаПоНачислениям.Следующий() Цикл 
			...
			Если НЕ Отказ Тогда
				...
				Если Не ВыборкаПоНачислениям.ВидЕжегодногоОтпуска.Пустая() Тогда
