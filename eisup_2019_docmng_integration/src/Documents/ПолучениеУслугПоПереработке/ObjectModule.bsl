...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ЗначениеЗаполнено(СтатьяЗатрат) Тогда
		Если СтатьяЗатрат.ВидЗатрат = Перечисления.ВидыЗатрат.Материальные Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Укажите нематериальную статью затрат (закладка ""Учет затрат"")!", Отказ, Заголовок);
		КонецЕсли;
...
Процедура СформироватьДвижениеПоРегиструСписанныеТовары(
	...
	Если абс_ОтветствХранение Тогда   //АБС Коломиец 11415
		НоваяСтрока.ДопустимыйСтатус4 = Перечисления.СтатусыПартийТоваров.ОтветственноеХранение;
	Иначе
		// АБС ВСТАВКА 50947
	...
	// АБС ВСТАВКА Болоболин 68937
	ИначеЕсли СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете И СтрокаТабличнойЧасти.СчетЗатрат.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ОбщепроизводственныеРасходы) Тогда
		НоваяСтрока.КорСубконтоБУ1 = СтруктураШапкиДокумента.ПодразделениеОрганизации;
		НоваяСтрока.КорСубконтоБУ2 = СтрокаТабличнойЧасти.абс_ВидДеятельности;
		НоваяСтрока.КорСубконтоБУ3 = СтрокаТабличнойЧасти.СтатьяЗатрат;
	ИначеЕсли СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете И СтрокаТабличнойЧасти.СчетЗатрат.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ПрочиеДоходыИРасходы) Тогда
		НоваяСтрока.КорСубконтоБУ1 = СтруктураШапкиДокумента.ПодразделениеОрганизации;
		НоваяСтрока.КорСубконтоБУ2 = СтрокаТабличнойЧасти.абс_ПрочиеДоходыРасходы;
		НоваяСтрока.КорСубконтоБУ3 = СтрокаТабличнойЧасти.СтатьяЗатрат;
		//\\ АБС Болоболин 68937 ВСТАВКА
		//АБС ВСТАВКА №000021832 НАЧАЛО
		Если ЗначениеЗаполнено(НоваяСтрока.абс_СубконтоБУ2) Тогда
			НоваяСтрока.КорСубконтоБУ3 = НоваяСтрока.абс_СубконтоБУ2;	
		КонецЕсли;
		//\\АБС ВСТАВКА №000021832 КОНЕЦ
	Иначе
		НоваяСтрока.КорСубконтоБУ1 = СтруктураШапкиДокумента.ПодразделениеОрганизации;
	...
	НоваяСтрока.абс_ЦФО 		  		= СтрокаТабличнойЧасти.абс_ЦФО;
	НоваяСтрока.абс_НазначениеПлатежа 	= СтрокаТабличнойЧасти.абс_НазначениеПлатежа;
	// АБС ВСТАВКА КОНЕЦ	
...
//	НоваяСтрока.абс_СубконтоБУ2 = СтрокаТабличнойЧасти.абс_СубконтоБУ2;   //АБС Коломиец
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, 
	...
	ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, 
							  ...
							  ТаблицаПоВзаиморасчетам, Отказ, Заголовок,ТаблицаПоМатериалам); //Родин Лимитный контроль
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, 
	...
	Если мЕстьЗаказ Тогда
		...
		Если Не Отказ Тогда
			...
			ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "ЗаказПоставщику",    Заказ);
			ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "СтатусПартии",       ОпределитьСтатусПартииПриходаУпр());
	...
	ttk_ОбщегоНазначения.ПереименоватьКолонкуТаблицыЗначений(ТаблицаПоТоварам, ТабИмен, "НДС");
	ttk_ОбщегоНазначения.ПереименоватьКолонкуТаблицыЗначений(ТаблицаПоТоварам, ТабИмен, "НДСУпр", "НДС");
	
	ТабИменУслуги = Неопределено;
	ttk_ОбщегоНазначения.ПереименоватьКолонкуТаблицыЗначений(ТаблицаПоУслугам, ТабИменУслуги, "НДС");
	ttk_ОбщегоНазначения.ПереименоватьКолонкуТаблицыЗначений(ТаблицаПоУслугам, ТабИменУслуги, "НДСУпр", "НДС");
	...
	ttk_ОбщегоНазначения.ЗаписатьТаблицыДокументаВРегистр(НаборДвижений, Неопределено, ТаблицыДанныхДокумента, Дата);
	...
	Если ТаблицаПоМатериалам.Количество() > 0 Тогда
		...
		Если НЕ Отказ Тогда
			...
			ТаблицыДанныхДокумента = ttk_ОбщегоНазначения.ЗагрузитьТаблицыДокументаВСтруктуру(НаборДвиженийТовары, СтруктураТаблицДокумента);
			...
			ttk_ОбщегоНазначения.ЗаписатьТаблицыДокументаВРегистр(НаборДвиженийТовары, ВидДвиженияНакопления.Расход, ТаблицыДанныхДокумента, Дата);
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам, 
	...
	//		СтруктураТаблиц.Вставить("ТаблицаПоМатериалам",ТаблицаПоМатериалам);
	//		
	...
	// Прочитаем проводки после записи для формирования проводок по бюджету
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗначения = Тип("ДокументСсылка.ЗаказПоставщику")
	 ...
	 ИЛИ ТипЗначения = Тип("ДокументСсылка.РеализацияУслугПоПереработке") Тогда
	...
	Если ТипЗначения = Тип("ДокументСсылка.ПередачаТоваров") Тогда
		
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, ЗаполнитьСтруктуруСчетовУчетаШапки());
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Основание);
		Для каждого Строка Из Основание.Товары Цикл
			СтрокаТЧ = ЭтотОбъект.ИспользованныеМатериалы.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТЧ, Основание);
			ЗаполнитьЗначенияСвойств(СтрокаТЧ, Строка);
			СтрокаТЧ.СчетУчетаБУ = Строка.СчетПередачиБУ;
			СтрокаТЧ.СчетУчетаНУ = Строка.СчетПередачиНУ;
		КонецЦикла;
		//АБС ВСТАВКА №20442 НАЧАЛО
		Для каждого Строка Из Основание.абс_Оборудование Цикл
			СтрокаТЧ = ЭтотОбъект.ИспользованныеМатериалы.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТЧ, Основание);
			ЗаполнитьЗначенияСвойств(СтрокаТЧ, Строка);
			СтрокаТЧ.СчетУчетаБУ = Строка.СчетПередачиБУ;
			СтрокаТЧ.СчетУчетаНУ = Строка.СчетПередачиНУ;
		КонецЦикла;
		//\\АБС ВСТАВКА №20442 КОНЕЦ
		ЭтотОбъект.Комментарий = "Введен на основании: "+Основание;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//Крамаренко Д.М.
	//НФС 2018
	Для каждого Строка из ИспользованныеМатериалы Цикл
		
		СтрокаНачалаСообщенияОбОшибке = Нстр("ru = 'В строке номер """+ СокрЛП(Строка.НомерСтроки) +
		""" табличной части "" Использованные материалы "": '");
		...
		//надо переделать на реквизиты табличной части
		Если СокрЛП(Строка.СтатьяОборотов.Код) = "КВпоПост" Тогда
			Если НЕ ЗначениеЗаполнено(абс_ТЭО) ИЛИ НЕ ЗначениеЗаполнено(абс_КВ) Тогда
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// АБС Фролов Проведение по ЦФО и Назначению платежа
	СтруктураПолей.Вставить("абс_ЦФО"					, "Ссылка.абс__ЦФО");
	СтруктураПолей.Вставить("абс_НазначениеПлатежа"		, "Ссылка.абс_НазначениеПлатежа");
	СтруктураПолей.Вставить("СерияНоменклатуры"			, "СерияНоменклатуры");
	// АБС Фролов Проведение по ЦФО и Назначению платежа КОНЕЦ
	// АБС Болоболин Проведение по 25, 91 счетам
	СтруктураПолей.Вставить("абс_ВидДеятельности"		, "абс_ВидДеятельности");
	СтруктураПолей.Вставить("абс_ПрочиеДоходыРасходы"	, "абс_ПрочиеДоходыРасходы");
	СтруктураПолей.Вставить("абс_СубконтоБУ2"	        , "абс_СубконтоБУ2");
	// АБС Болоболин Проведение по 25, 91 счетам КОНЕЦ
	...
	абс_СписанныеМатериалыНаОСДвижения(СтруктураШапкиДокумента,ТаблицаПоМатериалам,Отказ);
	//\\АБС ВСТАВКА №2080 КОНЕЦ
	
	//АБС ВСТАВКА №39158 НАЧАЛО «24 февраля 2014 г.», Пополитов
	КорректируемПроводкиПоДаннымПрошлогоПериода(Отказ);	
	//\\АБС ВСТАВКА №39158 КОНЕЦ
...
//АБС заявка 1970
Процедура абс_ПроводкиПо76()
	
	Если ЗначениеЗаполнено(абс_Контрагент) И ЗначениеЗаполнено(абс_ДоговорКонтрагента) Тогда
		...
		Для Каждого Проводка Из НаборЗаписейБУ Цикл
			Если Проводка.СчетДт = ПланыСчетов.Хозрасчетный.РасчетыПоПретензиям Тогда
				...
				Для каждого ВидСубконто Из Проводка.СчетДт.ВидыСубконто Цикл
					...
					Если ВидСубконто.ВидСубконто.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ДоговорыКонтрагентов")) Тогда
						Проводка.СубконтоДт.Вставить(ВидСубконто.ВидСубконто, абс_ДоговорКонтрагента);
					ИначеЕсли ВидСубконто.ВидСубконто.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Контрагенты")) Тогда
						Проводка.СубконтоДт.Вставить(ВидСубконто.ВидСубконто, абс_Контрагент);
					КонецЕсли;
				КонецЦикла;
...
Процедура абс_СписанныеМатериалыНаОСДвижения(СтруктураШапкиДокумента,ТаблицаПоМатериалам,Отказ) Экспорт 
	...
	Если НЕ Отказ Тогда
		...
		НаборЗаписей = РегистрыНакопления.ПартииТоваровПереданныеБухгалтерскийУчет.СоздатьНаборЗаписей();
...
Процедура КорректируемПроводкиПоДаннымПрошлогоПериода(Отказ)  
	...
	Счет_20_01_1    = ПланыСчетов.Налоговый.НайтиПоКоду("20.01.1");
	...
	Для каждого Проводка Из ТЗ Цикл
		//ОбработкаПрерыванияПользователя();
		Если Проводка.СчетДт = Счет_20_01_1 Тогда 
			Строка = Движения.Налоговый.Добавить();
			Строка.Сумма       		= -Проводка.Сумма;
			Строка.Активность  		= Проводка.Активность;
			Строка.Период      		= Проводка.Период;	
