...
Процедура ПроверитьЗаполнениеРеквизитовДокумента(ТаблицаДиапазонов, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ТаблицаДиапазонов.Количество() = 0 Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не заполнена шкала диапазонов.", Отказ, Заголовок);
	КонецЕсли;
	...
	Для Тмп = 0 По КоличествоСтрок Цикл
		...
		Если Тмп > 0 Тогда
			Если ТекущаяСтрокаТЗ.ВерхняяГраница = ПредыдущаяСтрокаТЗ.ВерхняяГраница Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + " продублирована верхняя граница.", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
...
Процедура ДвиженияПоРегистрам(СтруктураШапкиДокумента, ТаблицаПоДиапазонам, Отказ, Заголовок)
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоДиапазонам, ТаблицаДвижений);
