...
Процедура ПодготовитьСтруктуруШапкиДокумента(
	...
	СтруктураШапкиДокумента.Вставить("абс_ПерепроводитьДокументыСписания", абс_ПерепроводитьДокументыСписания);    //АБС Коломиец 15198
...
Процедура ОбработкаПроведения(Отказ)
	...
	//АБС ВСТАВКА 35878  10.12.2013 16:55:45  Шамов
	КонтрольСписанияПартийМВЭ_УУ();
...
//АБС ВСТАВКА 35878  10.12.2013 16:55:45  Шамов
Процедура КонтрольСписанияПартийМВЭ_УУ()
	
	НаборДвижений = РегистрыНакопления.ПартииМатериаловВЭксплуатации.СоздатьНаборЗаписей();
	НаборДвижений.Отбор.Регистратор.Установить(Ссылка);
	НаборДвижений.Прочитать();
	
	мУдаляемыеЗаписи = Новый Массив;
	
	Для Каждого Движение Из НаборДвижений Цикл
		Если НЕ ЗначениеЗаполнено(Движение.ДокументПередачи) ТОгда
			Продолжить;
		КонецЕсли;
		//АБС ИЗМЕНЕНИЕ 38121 04.02.2014 18:24:43  Шамов
		//Попытка
		//	Если Движение.ДокументПередачи.Организация = Организация ТОгда
		//		Продолжить;
		//	КонецЕсли;
		//	мУдаляемыеЗаписи.Добавить(Движение);	
		//Исключение
		//	мУдаляемыеЗаписи.Добавить(Движение);	
		//КонецПопытки;
		Если НЕ ПолныеПрава.ДокументПоДоступнойОрганизации(Движение.ДокументПередачи, Организация) Тогда
			мУдаляемыеЗаписи.Добавить(Движение);	
		КонецЕсли;
		//АБС ИЗМЕНЕНИЕ 38121 КОНЕЦ
	КонецЦикла;
	
	Для Каждого Движение Из мУдаляемыеЗаписи Цикл
		НаборДвижений.Удалить(Движение);
	КонецЦикла;
