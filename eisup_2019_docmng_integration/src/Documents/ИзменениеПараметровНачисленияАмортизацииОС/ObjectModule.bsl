	...
	Для Каждого СтрокаТЧ Из ОС Цикл
		Если ОтражатьВУправленческомУчете Тогда
			...
			Если НЕ (СтруктПараметров["Дата"] = Дата(1,1,1)) Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Упр. учет: Основное средство """ + СтрокаТЧ.ОсновноеСредство + """ в строке № " + СтрокаТЧ.НомерСтроки + " снято с учета!", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
