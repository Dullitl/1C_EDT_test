...
Функция ПечатьВводОстатков()
	...
	Область.Параметры.ТекстЗаголовок = ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента( Ссылка);
...
Процедура ДвиженияПоРегистрамУпр(СтруктураШапкиДокумента, ТаблицаПоМатериалам)
	...
	Если НЕ УправлениеЗапасами.ИспользуетсяРасширеннаяАналитикаУчета(Дата) Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу( ТаблицаПоМатериалам, ТабПартии);
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу( ТаблицаПоМатериалам, ТабПартииМат);
...
Процедура ДвиженияПоРегистрамРегл(СтруктураШапкиДокумента, ТаблицаПоМатериалам)
	...
	Если НЕ УправлениеЗапасами.ИспользуетсяРасширеннаяАналитикаУчета(Дата) Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоМатериаламРегл, ТаблицаДвиженийПартииБухгалтерскийУчет);
		...
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоМатериаламРегл, ТаблицаДвиженийПартииНалоговыйУчет);
...
Процедура ФормироватьПроводкиБух( СтруктураШапкиДокумента, ТаблицаПоМатериалам)
	...
	Для Каждого СтрокаДок Из ТаблицаПоМатериалам Цикл
		...
		Если СтрокаДок.СчетУчетаБУ <> ПланыСчетов.Хозрасчетный.ИнвентарьИХозяйственныеПринадлежности и СтрокаДок.СчетУчетаБУ.Код<>"10.12" Тогда
...
Процедура ФормироватьПроводкиНал( СтруктураШапкиДокумента, ТаблицаПоМатериалам)
	...
	Для Каждого СтрокаДок Из ТаблицаПоМатериалам Цикл
		...
		Если СтрокаДок.СчетУчетаНУ <> ПланыСчетов.Налоговый.ИнвентарьИХозяйственныеПринадлежности и СтрокаДок.СчетУчетаБУ.Код<>"10.12" Тогда
