...
Функция ПечатьЗаказа()
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, "Заказ на производство");
...
Процедура ЗаполнитьМатериалыПоСпецификации(ТабличнаяЧасть, ИмяТабличнойЧасти) Экспорт
	...
	Для Каждого СтрокаТабличнойЧасти Из Продукция Цикл
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ИсходныеКомплектующие, ТаблицаИсходныеКомплектующие);
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, 
	...
	Если ТаблицаПоПродукции.Количество() > 0 Тогда
		...
		ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "ЗаказНаПроизводство", Ссылка);
