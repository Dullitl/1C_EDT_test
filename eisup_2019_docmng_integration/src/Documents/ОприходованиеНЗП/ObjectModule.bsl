...
Процедура ЗаполнитьСписокРеквизитовЗависимыхОтТиповУчетаРегл()
	...
	мСтрокаРеквизитыНалУчета = "СчетКтНУ, НадписьСчетКтНУ,
		|СубконтоКтНУ1, НадписьСубконтоКтНУ1,
		|СубконтоКтНУ2, НадписьСубконтоКтНУ2,
		|СубконтоКтНУ3, НадписьСубконтоКтНУ3,
		|Материалы.СчетЗатратНУ";
...
Процедура ПроверкаРеквизитов(Отказ, Заголовок, СтруктураШапкиДокумента) Экспорт
	...
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
		РеквизитыТЧ = РеквизитыТЧ + ", СчетЗатратНУ";
	КонецЕсли;
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		Если Не СписСчетовБух.НайтиПоЗначению(СчетКт) = Неопределено Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя указывать счет " + СчетКт + " в качестве корреспонденции! (счет бух. учета)", Отказ, Заголовок);
		КонецЕсли;
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			СписСчетовНал = УправлениеПроизводством.ПолучитьЗатратныеСчета("Нал");
			Если Не СписСчетовНал.НайтиПоЗначению(СчетКтНУ) = Неопределено Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя указывать счет " + СчетКтНУ + " в качестве корреспонденции! (счет нал. учета)", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСубконтоТабличнойЧастиСоставРегл(Отказ, Заголовок, СтруктураШапкиДокумента)
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			СтруктураПараметров = Новый Структура("СчетКтНУ, СубконтоКтНУ1, СубконтоКтНУ2, СубконтоКтНУ3",
				"Не выбран счет кредита (нал. учет)",
				"Не указано субконто 1 кредита счета (нал. учет)",
				"Не указано субконто 2 кредита счета (нал. учет)",
				"Не указано субконто 3 кредита счета (нал. учет)");
			УправлениеЗатратами.ПроверитьЗаполнениеСубконто(ЭтотОбъект, СтруктураПараметров,, Отказ, Заголовок);
		КонецЕсли;
