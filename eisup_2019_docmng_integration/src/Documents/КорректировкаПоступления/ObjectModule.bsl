...
// Функция возвращает номенклатурную группу и статью оборотов
Функция ОпределитьИспользуемыйОбъект(НоменклатурнаяГруппа, ТипКонтрагента = Неопределено) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ttk_СоответствиеНеИспользуемыхОбъектовКИспользуемым.НеИспользуемыйОбъект,
	...
	Если ТипКонтрагента = Неопределено Тогда
		Запрос.Текст = Запрос.Текст +
			" И ttk_СоответствиеНеИспользуемыхОбъектовКИспользуемым.ТипКонтрагента <> ЗНАЧЕНИЕ(Справочник.абс_ТипыКонтрагентов.ФизическиеЛица)";
	Иначе
		Запрос.Текст = Запрос.Текст +
			" И ttk_СоответствиеНеИспользуемыхОбъектовКИспользуемым.ТипКонтрагента = &ТипКонтрагента";
	...
	Запрос.Текст = Запрос.Текст +
		...
		|	ttk_СоответствиеНеИспользуемыхОбъектовКИспользуемым.НеИспользуемыйОбъект,
		|	ttk_СоответствиеНеИспользуемыхОбъектовКИспользуемым.ИспользуемыйОбъект";
	Запрос.УстановитьПараметр("НеИспользуемыйОбъект", НоменклатурнаяГруппа);
	...
	Пока Результат.Следующий() Цикл
		СтруктураВозврата.Вставить("НеИспользуемыйОбъект", Результат.НеИспользуемыйОбъект);
...
Функция ДоступнаКорректировкаБУиНУ() Экспорт
	...
	// Для документов ручного учета тоже должна быть возможность корректировать регламентный учет
	ИЛИ ТипЗнч(ДокументПоступления) = Тип("ДокументСсылка.ДокументРасчетовСКонтрагентом")
...
Процедура ЗаполнитьСвойстваШапки(ЗаполнятьРедактируемыеРеквизиты = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(мДокументПоступленияСсылка) Тогда
		...
		// Гущина ВСТАВКА 7734134  2016.06.01     когда корректируют поступления старых организаций
		// было
		...
		Если ТипЗнч(мДокументПоступленияСсылка) = Тип("ДокументСсылка.ДокументРасчетовСКонтрагентом") 
			и   ЗначениеЗаполнено(мДокументПоступленияСсылка.абс_ДокументОснование) тогда 
			МетаданныеДокументаСтаройОрганизации = мДокументПоступленияСсылка.абс_ДокументОснование.Метаданные();
// Казаков Е.  07.11.2017 <<
			Если НЕ МетаданныеДокументаСтаройОрганизации.Реквизиты.Найти("СчетУчетаРасчетовПоАвансам") = Неопределено Тогда
				СчетУчетаРасчетовПоАвансам = мДокументПоступленияСсылка.абс_ДокументОснование.СчетУчетаРасчетовПоАвансам;
			КонецЕсли;	
			Если НЕ МетаданныеДокументаСтаройОрганизации.Реквизиты.Найти("СчетУчетаРасчетовСКонтрагентом") = Неопределено Тогда
				СчетУчетаРасчетовСКонтрагентом = мДокументПоступленияСсылка.абс_ДокументОснование.СчетУчетаРасчетовСКонтрагентом;
			КонецЕсли;	
// Казаков Е.  07.11.2017 >>
			Если МетаданныеДокументаСтаройОрганизации.Реквизиты.Найти("СуммаВключаетНДС") = Неопределено Тогда
				СуммаВключаетНДС = Истина;
// +++ввв 02.08.2016 г.
			Иначе
				СуммаВключаетНДС = мДокументПоступленияСсылка.абс_ДокументОснование.СуммаВключаетНДС;
// ---ввв 02.08.2016 г.
			КонецЕсли;
// +++ввв 02.08.2016 г.
			Если МетаданныеДокументаСтаройОрганизации.Реквизиты.Найти("УчитыватьНДС") = Неопределено Тогда
				УчитыватьНДС = Истина;
			Иначе
				УчитыватьНДС = мДокументПоступленияСсылка.абс_ДокументОснование.УчитыватьНДС;
			КонецЕсли;
// ---ввв 02.08.2016 г.
		Иначе
// +++ввв 02.08.2016 г.
			...
			Если МетаданныеДокументаОснования.Реквизиты.Найти("УчитыватьНДС") = Неопределено Тогда
				УчитыватьНДС = Истина;
			Иначе
				УчитыватьНДС = мДокументПоступленияСсылка.УчитыватьНДС;
			КонецЕсли;
// ---ввв 02.08.2016 г.
			Если МетаданныеДокументаОснования.Реквизиты.Найти("СуммаВключаетНДС") = Неопределено Тогда
				СуммаВключаетНДС = Истина;
// +++ввв 02.08.2016 г.
			Иначе
				СуммаВключаетНДС = мДокументПоступленияСсылка.СуммаВключаетНДС;
// ---ввв 02.08.2016 г.
			КонецЕсли;
		КонецЕсли;
		// ВСТАВКА 7734134  конец      
...
Процедура ЗаполнитьПоПоступлению()
	...
	Если ИмяВидаДокумента <> "КорректировкаПоступления" Тогда
		...
		//+ Машута А. 12.10.2018 задача 765
		|	ПоступлениеТоваровУслугТовары.ttk_ОбъектБюджетирования,
		|	ПоступлениеТоваровУслугТовары.абс_КВ КАК ttk_КВ,
		|	ПоступлениеТоваровУслугТовары.абс_ТЭО КАК ttk_ТЭО,
		|	ПоступлениеТоваровУслугТовары.ttk_Город,
		//- Машута А. 12.10.2018 задача 765
	...
	Если ИмяВидаДокумента <> "КорректировкаПоступления" Тогда
		...
		|	ПоступлениеТоваровУслугУслуги.абс_КВ КАК ttk_КВ,
		|	ПоступлениеТоваровУслугУслуги.абс_ТЭО КАК ttk_ТЭО,
		|	ПоступлениеТоваровУслугУслуги.ttk_Город,
		//- Машута А. 12.10.2018 задача 765
...
Процедура ТоварыИТараПоРегистрамОстатковИПартийУпр(РежимПроведения, ТаблицаПоТоварам, ТаблицаПоУслугам, Отказ, Заголовок, СтруктураШапкиДокумента)
	...
	//Бобылев А.А. 21.05.2018 
	Попытка
		ТЗ = ТаблицаТоварыОпт.Скопировать();
		ТЗ.Колонки.СчетУчетаБУ.Имя = "СчетУчета";
		ИзмСтруктТаблицДокументаОпт = Новый Структура;
		ИзмСтруктТаблицДокументаОпт.Вставить("ТаблицаПоТоварам", ТЗ);
		ТаблицыДанныхДокументаОпт     = ttk_ОбщегоНазначения.ЗагрузитьТаблицыДокументаВСтруктуру(Движения.ТоварыНаСкладах, ИзмСтруктТаблицДокументаОпт);
	Исключение
	...
	Попытка
		ТЗ2 = ТаблицаТоварыРозница.Скопировать();
		ТЗ2.Колонки.СчетУчетаБУ.Имя = "СчетУчета";
		ИзмСтруктТаблицДокументаРозница = Новый Структура;
		ИзмСтруктТаблицДокументаРозница.Вставить("ТаблицаПоТоварам", ТЗ2);
		ТаблицыДанныхДокументаРозница = ttk_ОбщегоНазначения.ЗагрузитьТаблицыДокументаВСтруктуру(Движения.ТоварыВРознице, ИзмСтруктТаблицДокументаРозница);
	Исключение
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоВзаиморасчетам, ТаблицаПоРасчетам, 
	...
	// В постановке задачи явным образом указано, что во всех статусах кроме указанных ниже должны осуществлять движения
	// по РН "КонтролируемыеЗначенияБюджетов" и "абс_ГрафикПоставкиЗакупочногоЗаказа"
	Если НЕ (абс_СтатусДокумента = Перечисления.абс_СтатусыПервичныхДокументов.Подготовка ИЛИ
		 абс_СтатусДокумента = Перечисления.абс_СтатусыПервичныхДокументов.Отмена ИЛИ 
		 абс_СтатусДокумента = Перечисления.абс_СтатусыПервичныхДокументов.Отказ) Тогда
	...
	// Разницы по ПБУ18/02
// {{KM WARE Семенов И.С. Заявка № 000032879 15.07.2015 начало
// 
	...
	//КонецЕсли;
// }}KM WARE Семенов И.С. Заявка № 32879 15.07.2015 окончание
...
Процедура ДвижениеПоЛимитамИГрафикуПоставок(СтруктураШапкиДокумента,Отказ) Экспорт
	...
	//+ Машута А. 09.10.2018 http://sd.ttk.ru/sd/operator/#uuid:task$2280309
	//вынес в отдельную процедуру
	ПериодПоставки = НачалоМесяца(Дата);
	//Для совместимости с предшествовашим функционалом. Значение период поставки в дальнейшем
	//алгоритме использовалось из последней итерации по перебору массива движений.
	СоответствиеДанныеДляОбработки = Документы.КорректировкаПоступления.ДанныеДляСторноДвижений_абс_ГрафикПоставок_ОборотыБюджета(ДокументПТУ, Ссылка, ПериодПоставки);
	СторноДвижений_абс_ГрафикПоставокЗакупочногоЗаказа(ДокументПТУ, ПериодПоставки, ТекКурс, СоответствиеДанныеДляОбработки["абс_ГрафикПлатежейПоЗакупочномуЗаказу"]);
	СписатьЛимиты_КонтролируемыеЗначенияБюджетов(ТекКурс, ПериодПоставки, СоответствиеДанныеДляОбработки["КонтролируемыеЗначенияБюджетов"]);
	//- Машута А. 09.10.2018
	...
	//+ Машута А. 15.10.2018 задача 765
	#Область СтарыйЗапрос_КонтрольОстаткаГрафикаПоставокЗЗ
	//Запрос = Новый Запрос;
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	КорректировкаПоступленияТовары.абс_ЗакупочныйЗаказТЧ КАК ЗакупочныйЗаказ,
	|	КорректировкаПоступленияТовары.СтатьяОборотов,
	|	КорректировкаПоступленияТовары.ttk_КВ КАК КВ,
	|	КорректировкаПоступленияТовары.ttk_ТЭО КАК ТЭО,
	|	КорректировкаПоступленияТовары.абс_ЦФУ КАК ЦФУ,
	|	КорректировкаПоступленияТовары.ttk_ОбъектБюджетирования,
	|	ВЫБОР КОГДА КорректировкаПоступления.СуммаВключаетНДС 
	|		ТОГДА КорректировкаПоступленияТовары.Сумма - КорректировкаПоступленияТовары.СуммаДоИзменения - (КорректировкаПоступленияТовары.СуммаНДС - КорректировкаПоступленияТовары.СуммаНДСДоИзменения)
	|		ИНАЧЕ КорректировкаПоступленияТовары.Сумма - КорректировкаПоступленияТовары.СуммаДоИзменения КОНЕЦ КАК Сумма
	|ПОМЕСТИТЬ ВТ_ТЧ
	|ИЗ
	...
	|		ПО (КорректировкаПоступления.Ссылка = КорректировкаПоступленияТовары.Ссылка)
	...
	|	ВЫБОР КОГДА КорректировкаПоступления.СуммаВключаетНДС 
	|		ТОГДА КорректировкаПоступленияУслуги.Сумма - КорректировкаПоступленияУслуги.СуммаДоИзменения - (КорректировкаПоступленияУслуги.СуммаНДС - КорректировкаПоступленияУслуги.СуммаНДСДоИзменения)
	|		ИНАЧЕ КорректировкаПоступленияУслуги.Сумма - КорректировкаПоступленияУслуги.СуммаДоИзменения КОНЕЦ
	|ИЗ
	...
	|			И (КорректировкаПоступленияУслуги.Ссылка = &Ссылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТЧ.ЗакупочныйЗаказ,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВТ_АналитикиПроверки.ЗакупочныйЗаказ,
	|	ВТ_АналитикиПроверки.СтатьяОборотов,
	|	ВТ_АналитикиПроверки.КВ,
	|	ВТ_АналитикиПроверки.ТЭО,
	|	ВТ_АналитикиПроверки.ЦФУ,
	|	ВТ_АналитикиПроверки.ttk_ОбъектБюджетирования,
	|	ВТ_АналитикиПроверки.Сумма КАК СуммаКорректировки,
	...
	|		ПО ВТ_АналитикиПроверки.ЗакупочныйЗаказ = абс_ГрафикПоставкиЗакупочногоЗаказаОстатки.ЗакупочныйЗаказ
	|			И ВТ_АналитикиПроверки.СтатьяОборотов = абс_ГрафикПоставкиЗакупочногоЗаказаОстатки.СтатьяОборотов
	|			И ВТ_АналитикиПроверки.КВ = абс_ГрафикПоставкиЗакупочногоЗаказаОстатки.абс_КВ
	|			И ВТ_АналитикиПроверки.ТЭО = абс_ГрафикПоставкиЗакупочногоЗаказаОстатки.абс_ТЭО
	|			И ВТ_АналитикиПроверки.ЦФУ = абс_ГрафикПоставкиЗакупочногоЗаказаОстатки.абс_ЦФУ
	...
	Иначе   		
		Пока Выборка.Следующий() Цикл 				
			Если Выборка.ЗакупочныйЗаказ.НефиксированнаяСумма Тогда
				Если Не КонтрольЛимитов(Выборка.СуммаКорректировки * ТекКурс,Выборка.ЗакупочныйЗаказ, НачалоМесяца(Дата), 
					Новый Структура("ЦФУ, КВ, ТЭО, СтатьяОборотов", Выборка.ЦФУ, Выборка.КВ, Выборка.ТЭО, Выборка.СтатьяОборотов)) Тогда	  //АБС Коломиец 12392
					Отказ = истина;
...
//+ Машута А. 09.10.2018 задача 720
Процедура СторноДвижений_абс_ГрафикПоставокЗакупочногоЗаказа(ДокументПТУ, ПериодПоставки, ТекКурс, ДанныедляОбработки)
		
	Для каждого СтрокаДанных Из ДанныедляОбработки Цикл
		
		Если СтрокаДанных.СуммаКорректировки = 0 Тогда
			Продолжить;	
		КонецЕсли;
			
		СтрокаРегистра = Движения.абс_ГрафикПоставкиЗакупочногоЗаказа.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаРегистра,СтрокаДанных,, "ПериодПоставки");
		СтрокаРегистра.Сумма = СтрокаДанных.СуммаКорректировки;
		СтрокаРегистра.Период = Дата;
		СтрокаРегистра.ПериодПоставки = НачалоМесяца(Дата);
		СтрокаРегистра.ВидДвижения = ВидДвиженияНакопления.Расход;
		
		Если СтрокаДанных.НефиксированнаяСумма Тогда
			
			СтрокаРегистраПриход = Движения.абс_ГрафикПоставкиЗакупочногоЗаказа.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаРегистраПриход,СтрокаРегистра);
			СтрокаРегистраПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
...
//+ Машута А. 11.10.2018 задача 720
#Область СтарыйФункционал_СписатьЛимиты
...
//КонецПроцедуры
#КонецОбласти
Процедура СписатьЛимиты_КонтролируемыеЗначенияБюджетов(ТекКурс, ПериодПоставки, ДанныеЗаполнения)
	
	Для каждого СтрокаДанныеЗаполнения ИЗ ДанныеЗаполнения Цикл
		
		Если ЗначениеЗаполнено(СтрокаДанныеЗаполнения.ЗакупочныйЗаказ) И СтрокаДанныеЗаполнения.НефиксированнаяСумма = Истина Тогда
			
			Движение = Движения.КонтролируемыеЗначенияБюджетов.Добавить();
			Движение.Период 					= Дата;
			Движение.Сценарий					= СтрокаДанныеЗаполнения.Сценарий;
			Движение.ttk_ОбъектБюджетирования	= СтрокаДанныеЗаполнения.ttk_ОбъектБюджетирования;
			Движение.СтатьяОборотов 			= СтрокаДанныеЗаполнения.СтатьяОборотов;
			Движение.ЦФО 						= СтрокаДанныеЗаполнения.ЦФО;
			Движение.абс_ТипКонтрагента 		= СтрокаДанныеЗаполнения.ТипКонтрагента;
			Движение.абс_ТипСети 				= СтрокаДанныеЗаполнения.ТипСети;
			Движение.абс_КВ 					= СтрокаДанныеЗаполнения.КВ;
			Движение.абс_ТЭО 					= СтрокаДанныеЗаполнения.ТЭО;
			Движение.абс_ЦФУ 					= СтрокаДанныеЗаполнения.ЦФУ;
			Движение.абс_ТипРасхода 			= СтрокаДанныеЗаполнения.ТипРасхода;
			Движение.Организация 				= Организация;
			Движение.СуммаСценарияИсполнение	= СтрокаДанныеЗаполнения.СуммаПотрЛимитов * ТекКурс;
			Движение.СуммаСценарияИсполнениеВал = СтрокаДанныеЗаполнения.СуммаПотрЛимитов;
			Движение.Валюта						= ВалютаДокумента;
...
Функция КонтрольЛимитов(СуммаПроверка,ЗакупочныйЗаказ, ПериодПоставки, Параметры = Неопределено)
	...
	СтруктураАналитик.Вставить("абс_ЦФУ", ?(Параметры = Неопределено, ЗакупочныйЗаказ.ЦФУ, Параметры.ЦФУ));
	СтруктураАналитик.Вставить("ЦФО", ЗакупочныйЗаказ.ЦФО);
	...
	СтруктураАналитик.Вставить("абс_КВ",?(Параметры = Неопределено, ЗакупочныйЗаказ.КВ, Параметры.КВ));
	СтруктураАналитик.Вставить("абс_ТЭО", ?(Параметры = Неопределено, ЗакупочныйЗаказ.ТЭО, Параметры.ТЭО));
	СтруктураАналитик.Вставить("СтатьяОборотов", ?(Параметры = Неопределено, ЗакупочныйЗаказ.БюджетнаяСтатья, Параметры.СтатьяОборотов));
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоВзаиморасчетам, 
	...
	Если глЗначениеПеременной("абс_ИспользоватьКонтрольЛимитовПоДоговорам") Тогда
		Если Не Отказ и ДоговорКонтрагента.абс_ОсобыйПорядкокКонтроляЛимитов Тогда
			...
			// Start КТТК Ермолов Е.Л.  17.06.2015 000033133
			НаборДвижений = ЭтотОбъект.Движения.абс_ЗатратыПоДоговорамСКонтролемЛимитов;
			ТаблицаДвижений = НаборДвижений.Выгрузить();
			ТаблицаДвижений.Очистить();			
			
			абс_Бюджетирование.ФормированиеДвиженийПоКонтролируемымЗатратам(ЭтотОбъект,Отказ,СообщениеПриОтказе,СтруктураТаблиц,Ложь,ТаблицаДвижений,НаборДвижений );
			...
			// Stop КТТК Ермолов Е.Л.  17.06.2015
...
Процедура ОбработкаЗаполнения(Основание)
	...
	// {{ТТК Сладков А. Заявка №7737710  27.07.2016 начало
	ИзменитьСтатьюЗатратДляКорректировокПрошлогоПериода();	
	// }}ТТК Сладков А. Заявка №7737710  27.07.2016 окончание
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Для Каждого Услуга Из Услуги Цикл
		Если ЗначениеЗаполнено(Услуга.НоменклатурнаяГруппа) И (Услуга.НоменклатурнаяГруппа.абс_СтатусНоменклатурнойГруппы = Перечисления.абс_СтатусыНоменклатуры.Архив) Тогда
			НеИспользуемыйОбъект = Услуга.НоменклатурнаяГруппа;
			ИспользуемыйОбъект = ОпределитьИспользуемыйОбъект(НеИспользуемыйОбъект);
			Если ИспользуемыйОбъект.Количество() > 0 Тогда
				#Если Клиент Тогда
					Ответ = Вопрос("В строке " + Строка(Услуга.НомерСтроки) + " найдена номенклатурная группа в статусе 'Архив'.
						|Заменить номенклатурную группу на новую '" + Строка(ИспользуемыйОбъект.ИспользуемыйОбъект) + "'?", РежимДиалогаВопрос.ДаНет);
					Если Ответ = КодВозвратаДиалога.Да Тогда
						Услуга.НоменклатурнаяГруппа = ИспользуемыйОбъект.ИспользуемыйОбъект;
					КонецЕсли;
				#КонецЕсли
			Иначе
				Сообщить("Для архивной номенклатурной группы '" + Строка(Услуга.НоменклатурнаяГруппа) + "' не найдено ни одной используемой номенклатурной группы.");
				Отказ = Истина;
				Возврат;
			КонецЕсли;
		КонецЕсли;
...
Процедура СформироватьПроводки(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам, Отказ, Заголовок) Экспорт
		...
		//Если К3 <> 0 И К2 <> 1 Тогда 
			...
			Если СтрСуммаБУ < 0 Тогда //Доходы
				...
				Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
					...
					//Сторчевой А.Н 77159438 10.08.2017 {
	...
	Для Каждого СтрокаУслуги Из ТаблицаПоУслугам Цикл
		...
		Если Год(СтруктураШапкиДокумента.ДокументПоступленияДата) < Год(СтруктураШапкиДокумента.Дата)
			//Сторчевой А.Н. 15.29.2017 77120531 {                                   
			и НЕ СтруктураШапкиДокумента.абс_ПриПроведенииПрошлогоПериодаСчетаИзТабличнойЧасти
			// } Сторчевой А.Н. 15.29.2017 77120531 
			Тогда
			...
			Если СтрСуммаБУ < 0 Тогда //Доходы
				...
				Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
					...
					// {{ТТК Сладков А. Заявка №7737710  06.07.2016 начало
					//Если СтруктураШапкиДокумента.СтатьяПрочихДоходовИРасходов.ПринятиеКНалоговомуУчету = Истина Тогда
					...
					//Сторчевой А.Н 77159494 10.08.2017 {
					//Закомментировано(
	//				СтатьяИзменениеСтоимостиРаботУслуг = Проводка.СубконтоКт.ПрочиеДоходыИРасходы;
	...
	//
	//				КонецЕсли;
	//				// }}ТТК Сладков А. Заявка №7737710  06.07.2016 окончание
					//)Закомментировано
			...
			ИначеЕсли СтрСуммаБУ > 0 Тогда
				...
				Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
					...
					//Сторчевой А.Н 77159494 10.08.2017 {
					//Закомментировано(
 //   				СтатьяИзменениеСтоимостиРаботУслуг = Проводка.СубконтоДт.ПрочиеДоходыИРасходы;
						...
						Если СчетНУПоСоответствию <> СтрокаУслуги.СчетЗатратНУ 
							...
							Если ПроизводственныеРасходыНУ Тогда
								...
								Если Найти(ПроводкаПР.СчетДт.Код, "25") > 0 Тогда
									ИмяКолонкиСубконто = "СубконтоНУ"; //km-ware Лазаревский Исправлено с Субконто на СубконтоНУ
...
// {{ТТК Сладков А. Заявка №7737710  27.07.2016 начало
Процедура	ИзменитьСтатьюЗатратДляКорректировокПрошлогоПериода() Экспорт
	
	
Если ВидОперации=Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение 
	и ЭтотОбъект.ДокументПоступления<>Неопределено Тогда
	
	Если  Год(ЭтотОбъект.ДокументПоступления.Дата)<Год(ЭтотОбъект.Дата) Тогда

		ЭтотОбъект.СтатьяПрочихДоходовИРасходов =  ПолучитьСтатьюИзменениеСтоимостиРаботУслуг();
		
		Сообщить("обнаружена корректировка прошлого периода - статья прочих доходов и расходов изменена на статью " + ЭтотОбъект.СтатьяПрочихДоходовИРасходов.Наименование);
...
Функция ПолучитьСтатьюИзменениеСтоимостиРаботУслуг() Экспорт
	 ...
	 // в сторону уменьшения или увелечения	 
