...
Процедура ЗаполнитьСтроки_СМРхозспособом(УдалятьНезаполненные = Ложь) Экспорт
	...
	Если не ?(НЕ ЗначениеЗаполнено(мУчетнаяПолитикаНУ), Ложь, мУчетнаяПолитикаНУ.Свойство("НДСНалоговыйПериод")) тогда
		мУчетнаяПолитикаНУ = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(Дата, Организация);
	...
	Если УдалятьНезаполненные Тогда
		УдалитьНезаполненныеСтроки(ТаблицаРезультатов, "ОбъектСтроительства, СтавкаНДС");
	КонецЕсли;
...
Функция ЗаполнитьСМРхозспособомПоУказаномуПериоду(НачалоПериода, КонецПериода)
	...
	Запрос = Новый Запрос;
	//АБС ИЗМЕНЕНИЕ 32776
	//\\УДАЛЕНО условие СуммаОборотДт>0 чтобы сумма заполнения полностью совпадала с ОСВ по 08.3
	//\\/АБС ИЗМЕНЕНИЕ 32776 КОНЕЦ
	...
	//|	СУММА(СтроительствоХозспособом.Сумма * &СтавкаНДС_Значение / 100) КАК НДС     //АБС ИЗМЕНЕНИЕ 000020026  31.01.2014 10:47:50  Коломиец																					 
	|	СУММА(ВЫРАЗИТЬ(СтроительствоХозспособом.Сумма * &СтавкаНДС_Значение / 100 КАК ЧИСЛО(15, 2))) КАК НДС
	//АБС ИЗМЕНЕНИЕ 000020026 КОНЕЦ
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|	ГДЕ
	|		НачислениеНДСпоСМРхозспособомСМРхозспособом.Ссылка.Организация = &Организация
	|		И НачислениеНДСпоСМРхозспособомСМРхозспособом.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|		И НачислениеНДСпоСМРхозспособомСМРхозспособом.Ссылка.Проведен
	|		И НачислениеНДСпоСМРхозспособомСМРхозспособом.Ссылка.Ссылка <> &Ссылка) КАК СтроительствоХозспособом
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если СтруктураШапкиДокумента.Дата<'20051231' Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Документ не может быть проведен с датой менее 31 декабря 2005 года!",Отказ, Заголовок);
	КонецЕсли; 
...
Процедура ПроверитьСуществованиеДругихДокументовВТекущемПериоде(СтруктураШапкиДокумента, Отказ,Заголовок)
	...
	Если НЕ ПустаяСтрока(СтрокаДокументовПересечений) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке(("Найдены документы по начислению НДС по СМР хозспособом, которые действуют в выбранном периоде("+ПредставлениеПериода(НачалоДня(НачалоПериода), КонецДня(КонецПериода), "ФП = Истина")+"):" + Символы.ПС + СтрокаДокументовПересечений), Отказ, Заголовок);
	КонецЕсли; 
...
Процедура ДвиженияПоРегистрамНДС_НачислениеСМРхозспособом(СтруктураШапкиДокумента, ТаблицаПоСМРхозспособом, Отказ, Заголовок)
	...
	Если мВестиУчетНДС Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоСМРхозспособом,ТаблицаДвижений_НДСНачисленный);
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоСМРхозспособом,ТаблицаДвижений_НДСПредъявленный);
...
мУчетнаяПолитикаНУ = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(Ссылка.Дата, Ссылка.Организация);
