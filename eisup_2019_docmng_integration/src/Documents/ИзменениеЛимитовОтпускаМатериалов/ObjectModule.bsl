...
Функция ПечатьЛимитыОтпускаМатериалов()
	...
	Область.Параметры.ТекстЗаголовка = ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(Ссылка, "Лимиты отпуска материалов");
...
Функция ПечатьЛимитноЗаборнаякарта()
	...
	Пока Обход.Следующий() Цикл
		...
		ОбластьМакетаШапка.Параметры.ТекстЦена = "Цена (" + глЗначениеПеременной("ВалютаУправленческогоУчета") + ")";
		ОбластьМакетаШапка.Параметры.ТекстСумма = "Сумма (" + глЗначениеПеременной("ВалютаУправленческогоУчета") + ")";
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаБазыРаспределения, Отказ, Заголовок)
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаБазыРаспределения, ТаблицаДвижений);
