	...
	Для Каждого СтрокаТЧ Из ТаблицаПосредническаяДеятельность Цикл
		...
		Если СтруктураШапкиДокумента.Корректировка Тогда
			Если СтрокаТЧ.СуммаПоСчетуФактуреРазницаУменьшение = 0
				...
				ttk_ОбщегоНазначения.СообщитьОбОшибке( "Не заполнены суммовые показатели корректировочного счета-фактуры. (строка № " + СтрокаТЧ.НомерСтроки + ")." , Отказ, Заголовок);
		...
		Иначе
			Если СтрокаТЧ.СуммаПоСчетуФактуре = 0
				...
				ttk_ОбщегоНазначения.СообщитьОбОшибке( "Не заполнены суммовые показатели счета-фактуры. (строка № " + СтрокаТЧ.НомерСтроки + ")." , Отказ, Заголовок);
...
Процедура УстановитьПризнакВыставлен() Экспорт
	...
	ДатаВыставления = ?(Дата = '00010101', ОбщегоНазначенияЗК.ПолучитьРабочуюДату(), Дата);
