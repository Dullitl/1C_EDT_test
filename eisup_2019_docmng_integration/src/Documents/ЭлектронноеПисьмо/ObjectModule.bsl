...
Функция ЗахватитьТекст() Экспорт
	...
	Исключение
		ttk_ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, "Не удалось сохранить файл на диск!");
		Возврат Ложь;
	КонецПопытки;
...
Процедура ОсвободитьТекст() Экспорт
	...
	Исключение
		ttk_ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, "Файл редактирования не удалось открыть.");
		Возврат;
	КонецПопытки;
...
Процедура ОтправитьПисьмо(ФормаПисьма = Неопределено) Экспорт
	...
	Если ПустаяСтрока(Кому) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указан ни один получатель.",, ШапкаСообщения);
		Возврат;
	КонецЕсли;
	
	Если ПометкаУдаления Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя отправлять помеченное на удаление письмо.",, ШапкаСообщения);
		Возврат;
	КонецЕсли;
	...
	Если ФормаПисьма <> Неопределено Тогда
		Попытка
			...
			ФормаПисьма.Закрыть();
		Исключение
			ttk_ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, ШапкаСообщения);
			Возврат;
		КонецПопытки;
	Иначе
		Попытка
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
		...
		Иначе
			Если Ответственный <> Ссылка.Ответственный Тогда
				Если НЕ НеВыдаватьСообщенияПриЗаписиОбъекта Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя менять ответственного в отправленном электронном письме.",, ШапкаСообщения);
				КонецЕсли; 
				Отказ = Истина;
			КонецЕсли; 
		КонецЕсли;
	...
	Если ГруппаУчетнойЗаписи = ГруппаУдаленные Тогда
		Если НЕ НеВыдаватьСообщенияПриЗаписиОбъекта Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(("Нельзя выбирать в качестве группы письма группу """ + ГруппаУдаленные + """, потому что она предназначена только для отображения удаленных писем."),, ШапкаСообщения);
		КонецЕсли; 
		Отказ = Истина;
	КонецЕсли;
	...
	Если ГруппаУчетнойЗаписи = ГруппаЧерновики Тогда
		Если НЕ НеВыдаватьСообщенияПриЗаписиОбъекта Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(("Нельзя выбирать в качестве группы письма группу """ + ГруппаЧерновики + """, потому что она предназначена только для отображения черновиков писем."),, ШапкаСообщения);
		КонецЕсли; 
		Отказ = Истина;
	КонецЕсли; 
