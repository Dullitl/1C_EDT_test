...
Процедура УстановитьОграничениеТипаСделки()
	Если ДоговорПоСчетам Тогда  
		//АБС НАЧАЛО №7047 (пункт 5) НАЧАЛО
		//ОписаниеТипаСделки = Новый ОписаниеТипов("ДокументСсылка.СчетНаОплатуПоставщика");
		ОписаниеТипаСделки = Новый ОписаниеТипов("ДокументСсылка.абс_СчетНаОплату");
		//\\АБС НАЧАЛО №7047 (пункт 5) КОНЕЦ
	Иначе
...
Функция ИзменениеНоменклатурыСервер(ДанныеДляЗаполнения)	
	...
	Если ДанныеДляЗаполнения <> Неопределено Тогда		
		...
		КонецЕсли;
	КонецЕсли;
	// АБС ВСТАВКА Не разрешаем выбирать номенклатуру, которая не используется КОНЕЦ
	
	ЗначенияДляЗаполнения = РаботаСДиалогамиСервер.ИзменениеНоменклатуры(ДанныеДляЗаполнения);
	
	// АБС ВСТАВКА Не разрешаем выбирать номенклатуру, которая не используется
	Если НЕ ЗначениеЗаполнено(ДанныеДляЗаполнения.Номенклатура) Тогда
...
Функция ИзменениеНоменклатурыУслугиСервер(ДанныеДляЗаполнения)
	...
	Если ДанныеДляЗаполнения <> Неопределено Тогда		
		...
		Если ДанныеДляЗаполнения.Номенклатура.абс_НеИспользовать Тогда
			...
			ЗначенияДляЗаполнения.Номенклатура = Справочники.Номенклатура.ПустаяСсылка();
...
Процедура УстановитьРежимПросмотраДокумента()
	...
	Элементы.ФормаЗаписать.Доступность 			= РазрешеноЗаписатьДокумент;
