...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,"Увольнение из организации "));
...
Функция СформироватьЗапросПоРаботникиОрганизации(ВыборкаПоШапкеДокумента)
	...
	|ВЫБРАТЬ
	|	СписокДат.НомерСтроки КАК НомерСтроки,
	|	СотрудникиОсвободившиеСтавкиВОрганизациях.Период,
	|	СотрудникиОсвободившиеСтавкиВОрганизациях.ПериодЗавершения,
	|	СотрудникиОсвободившиеСтавкиВОрганизациях.ОсвобождатьСтавку,
	|	СотрудникиОсвободившиеСтавкиВОрганизациях.ОсвобождатьСтавкуЗавершения
	|ПОМЕСТИТЬ ВТОсвобожденныеСтавки
	|ИЗ
	|	(ВЫБРАТЬ
	|		Док.Сотрудник КАК Сотрудник,
	|		МАКСИМУМ(СотрудникиОсвободившиеСтавкиВОрганизациях.Период) КАК ДатаПоследнегоИзменения,
	|		Док.НомерСтроки КАК НомерСтроки
	|	ИЗ
	|		РегистрСведений.СотрудникиОсвободившиеСтавкиВОрганизациях КАК СотрудникиОсвободившиеСтавкиВОрганизациях
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДанныеДокумента КАК Док
	|			ПО СотрудникиОсвободившиеСтавкиВОрганизациях.Период <= Док.ДатаУвольнения
	|				И СотрудникиОсвободившиеСтавкиВОрганизациях.Сотрудник = Док.Сотрудник
	|	
	|	СГРУППИРОВАТЬ ПО
	|		Док.Сотрудник,
	|		Док.НомерСтроки) КАК СписокДат
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СотрудникиОсвободившиеСтавкиВОрганизациях КАК СотрудникиОсвободившиеСтавкиВОрганизациях
	|		ПО СписокДат.ДатаПоследнегоИзменения = СотрудникиОсвободившиеСтавкиВОрганизациях.Период
	|			И СписокДат.Сотрудник = СотрудникиОсвободившиеСтавкиВОрганизациях.Сотрудник
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	НомерСтроки
	|;
	...
	|		И ДвиженияРаботниковОрганизации.Сотрудник В
	|				(ВЫБРАТЬ
	|					ТЧРаботникиОрганизации.Сотрудник
	|				ИЗ
	|					ВТДанныеДокумента КАК ТЧРаботникиОрганизации)
	...
	|	ЕСТЬNULL((НЕ ВЫБОР
	|				КОГДА ОсвобожденныеСтавки.ПериодЗавершения <= ТЧРаботникиОрганизации.ДатаУвольнения
	|						И ОсвобожденныеСтавки.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|					ТОГДА ОсвобожденныеСтавки.ОсвобождатьСтавкуЗавершения
	|				ИНАЧЕ ОсвобожденныеСтавки.ОсвобождатьСтавку
	|			КОНЕЦ), ИСТИНА) КАК ОсвобождатьСтавку,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОсвобожденныеСтавки КАК ОсвобожденныеСтавки
	|		ПО ТЧРаботникиОрганизации.НомерСтроки = ОсвобожденныеСтавки.НомерСтроки
...
Процедура ПроверитьЗаполнениеСтрокиРаботникаОрганизации(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если ВыборкаПоСтрокамДокумента.ВидСтрокиЗапроса = "ДанныеДляДвиженийКадров" Тогда
		...
		Если НетДатыУвольнения Тогда
			ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указана дата увольнения!", Отказ, Заголовок);
		КонецЕсли;
...
Процедура ДобавитьСтрокуВДвиженияПоРегистрамНакопления(ВыборкаПоШапкеДокумента, ВыборкаПоРаботникиОрганизации)
	...
	Если ВыборкаПоРаботникиОрганизации.ВидСтрокиЗапроса = "ДанныеДляДвиженийКадров" И ВыборкаПоРаботникиОрганизации.ОсвобождатьСтавку Тогда
...
мОкруглятьДниОтпускаПриКомпенсации = Неопределено;
