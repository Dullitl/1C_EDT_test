...
Функция ПечатьОтчетаКомитенту()
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, "Отчет комитенту");
...
Функция ПечатьАкта()
	...
	ОбластьМакета.Параметры.ТекстЗаголовка      = ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, "Акт об оказании услуг");
...
Процедура ЗаполнитьРеализованнымиУпр(ДокументПоставки = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если КоличествоРеализованных < 0 Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Обнаружено отрицательное количество реализованных товаров " + Выборка.Номенклатура + " " + Выборка.ХарактеристикаНоменклатуры + " " + Выборка.СерияНоменклатуры);
			Продолжить;
		КонецЕсли;	
		КоличествоПолученных    = ?(Выборка.КоличествоПолученных = NULL, 0, Выборка.КоличествоПолученных);
		Если КоличествоПолученных < 0 Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Обнаружено отрицательное количество полученных на комиссию товаров " + Выборка.Номенклатура + " " + Выборка.ХарактеристикаНоменклатуры + " " + Выборка.СерияНоменклатуры);
			Продолжить;
		КонецЕсли;
		Выручка                 = ?(Выборка.Выручка = NULL, 0, Выборка.Выручка);
		Количество              = Мин(КоличествоПолученных, КоличествоРеализованных);
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам,
	...
	Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		...
		Если НЕ Отказ Тогда
			...
			Если УправлениеЗапасами.ИспользуетсяРасширеннаяАналитикаУчета(Дата) Тогда
				ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "ДокументПоставки", Неопределено);
				ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "ДоговорКонтрагента", Неопределено);
				ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "Сделка", Неопределено);
				ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "Организация", Организация);
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, 
	...
	// Проводки по зачету аванса
	
	СуммаПоУслугам = ТаблицаПоУслугам.Итог("СуммаБезНДСВал") + ТаблицаПоУслугам.Итог("НДСВал");
	СтруктураПараметровЗачетАванса = ПодготовкаСтруктурыПараметровДляЗачетаАвансаПоУслугам(
		СтруктураШапкиДокумента, Заголовок, СуммаПоУслугам);
	
	Если СтруктураПараметровЗачетАванса = Ложь Тогда
	   ТаблицаАвансов = 0;
	Иначе
		ДвиженияДляРегистраРасчетовПоРеализации = ПодготовитьТаблицуДвиженийДляРегистраРасчетовПоРеализации(
			СтруктураПараметровЗачетАванса, СтруктураШапкиДокумента, ТаблицаПоУслугам);
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
		...
		Если Не ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомитентом Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Договор с контрагентом должен иметь вид договора с комитентом!");
			Возврат;
		КонецЕсли;
