...
Перем мСписокСчетовНал Экспорт;
...
Процедура ЗаполнитьСчетаУчетаВТабЧасти(ТабличнаяЧасть, ИмяТабЧасти, ЗаполнятьБУ, ЗаполнятьНУ) Экспорт
	...
	СчетаУчетаВДокументах.ЗаполнитьСчетаУчетаТабличнойЧасти(ИмяТабЧасти, ТабличнаяЧасть, ЭтотОбъект, ЗаполнятьБУ, ЗаполнятьНУ,, Ложь);
...
Процедура ПроверкаРеквизитов(Отказ, Заголовок, СтруктураШапкиДокумента, ТаблицаЗатрат)
	...
	Если ЗначениеЗаполнено(Номенклатура) И НЕ Номенклатура.Услуга Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Указана номенклатура """ + Номенклатура + """, которая не является услугой", Отказ, Заголовок);
	КонецЕсли;
	...
	// Проверим, что пользователь указал корректные счета учета
	Для Каждого СтрокаТЧ Из ТаблицаЗатрат Цикл
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете И ЗначениеЗаполнено(СтрокаТЧ.Счет) И мСписокСчетовБух.НайтиПоЗначению(СтрокаТЧ.Счет) = Неопределено Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Указан Счет, который НЕ является счетом учета затрат (строка № " + СтрокаТЧ.НомерСтроки + " табличной части ""Получатели"")", Отказ, Заголовок);
		КонецЕсли;
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете И ЗначениеЗаполнено(СтрокаТЧ.СчетНУ) И НЕ СтруктураШапкиДокумента.ОрганизацияПрименяетУСН И мСписокСчетовНал.НайтиПоЗначению(СтрокаТЧ.СчетНУ) = Неопределено Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Указан Счет (НУ), который НЕ является счетом учета затрат (строка № " + СтрокаТЧ.НомерСтроки + " табличной части ""Получатели"")", Отказ, Заголовок);
		КонецЕсли;
	КонецЦикла;
...
мСписокСчетовБух = БухгалтерскийУчет.ПолучитьМассивСчетов("Хозрасчетный", "20, 23, 25, 26, 28, 29, 44", Ложь);
мСписокСчетовНал = БухгалтерскийУчет.ПолучитьМассивСчетов("Налоговый",    "20, 23, 25, 26, 28, 29, 44", Ложь);
