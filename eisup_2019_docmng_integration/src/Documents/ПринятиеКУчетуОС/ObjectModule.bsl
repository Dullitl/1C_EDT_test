...
#Если Клиент Тогда
	...
	Функция ПечатьОС1(ПечатьПоДаннымУпрУчета = Истина)
		...
		//АБС+ 6183
		|	ПринятиеКУчетуОС.СпособНачисленияАмортизацииБУ КАК ПарСпособ,
		|	ПринятиеКУчетуОС.ГодоваяНормаАмортизацииБУ КАК ПарНорма,
		//АБС-
		...
		Если ВыборкаПоОС.Количество() = 1 Тогда
			...
			Если ОсновныеСредства.Количество()>0 Тогда
				ОбластьМакета.Параметры.ДоговорПередачи = "Приказ";
				//АБС Заявка 2651
				Если ЗначениеЗаполнено(Комиссия) Тогда
					ОбластьМакета.Параметры.НомерДоговора = СокрЛП(Комиссия.Номер);
					ДатаПриказа = Комиссия.Дата;
					Если ЗначениеЗаполнено(ДатаПриказа) Тогда
						ОбластьМакета.Параметры.ДатаДоговора = ДатаПриказа;
					КонецЕсли;
				...
				Иначе
					...
					ОбластьМакета.Параметры.НомерДоговора = СокрЛП(ОсновныеСредства[0].ОсновноеСредство.НомерПриказа.Номер);
					ДатаПриказа = ОсновныеСредства[0].ОсновноеСредство.НомерПриказа.Дата;
					Если ЗначениеЗаполнено(ДатаПриказа) Тогда
						ОбластьМакета.Параметры.ДатаДоговора = ДатаПриказа;
					КонецЕсли;
	...
	Функция НапечататьСоставКомиссии()	
		//Если ОтразитьВСоставеМПЗ тогда
		//		Сообщить("Актив переквалифицирован в МПЗ. Печать формы ""ОС-1"" невозможна.");
		//		Возврат Неопределено;
		//	КонецЕсли;
		//АБС
		//ПечатьПоДаннымУпрУчета = Истина;
		ПечатьПоДаннымУпрУчета = Ложь;
		//\\
		ЗапросПоШапке = Новый Запрос();
		ЗапросПоШапке.УстановитьПараметр("Ссылка", Ссылка);
		ЗапросПоШапке.УстановитьПараметр("ПечатьПоДаннымУпрУчета", ПечатьПоДаннымУпрУчета);
		...
		ТекстЗапросаПоШапке = "ВЫБРАТЬ
		|	ПринятиеКУчетуОС.Подразделение КАК Подразделение,
		...
		|	ПринятиеКУчетуОС.МОЛУУ КАК МОЛ,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ПринятиеКУчетуОС.СрокПолезногоИспользованияУУ, 0) <> 0
		|			ТОГДА 100 / ПринятиеКУчетуОС.СрокПолезногоИспользованияУУ
		|		ИНАЧЕ 0
		...
		Если ВыборкаПоОС.Количество() = 1 Тогда
			...
			ТабДокумент   = Новый ТабличныйДокумент();
			ПредставлДаты = СокрЛП(Формат(Дата,"ДЛФ=dd"));
			СимволДня = Найти(ПредставлДаты," ");
			ДлинаСтроки = СтрДлина(ПредставлДаты);
			День = Лев(ПредставлДаты,СимволДня);
			СимволовНаМесяц = ДлинаСтроки-СимволДня-7;
			Месяц = Сред(ПредставлДаты,СимволДня+1,СимволовНаМесяц);
			Год = прав(ПредставлДаты,7);
			...
			Если НЕ(ВыборкаПоОС.ГруппаОС = Перечисления.ГруппыОС.Здания ИЛИ
				...
				ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ПринятиеКУчетуОС_ОС1";
				//Макет         = ПолучитьМакет("ОС1бухК");
				Макет         = ПолучитьМакет("ОС1_РаскрыватьСостав");
				
				ОбластьМакета = Макет.ПолучитьОбласть("Заголовок");
				ОбластьМакета.Параметры.ДатаПолуч = День;
				ОбластьМакета.Параметры.МесяцПолуч = Месяц;
				ОбластьМакета.Параметры.ГодПолуч = Год;
			Иначе
				Макет 		   = ПолучитьМакет("ОС1К");
				ОбластьМакета  = Макет.ПолучитьОбласть("Заголовок");
				ОбластьМакета.Параметры.кВидуУчетаПеренос 	= кВидуУчетаПеренос;
			...
			//Если Не ЗначениеЗаполнено(ОбъектСтроительства) Тогда
			Если ОсновныеСредства.Количество()>0 Тогда
				ОбластьМакета.Параметры.ОбъектСтроительства = "Приказ";
				//АБС Заявка 2651
				Если ЗначениеЗаполнено(Комиссия) Тогда
					ОбластьМакета.Параметры.НомерДоговора = СокрЛП(Комиссия.Номер);
					ДатаПриказа = Комиссия.Дата;
			...
			Если НЕ(ВыборкаПоОС.ГруппаОС = Перечисления.ГруппыОС.Здания ИЛИ
				ВыборкаПоОС.ГруппаОС = Перечисления.ГруппыОС.Сооружения) Тогда
				ОбластьМакета.Параметры.ГодВыпуска =Формат(Год(Дата),"ЧГ=0");
			КонецЕсли;
			...
			ТабДокумент.Вывести(ОбластьМакета);
			//АБС
			ОбластьСтрока = Макет.ПолучитьОбласть("Раздел3");
			ОбластьСтрока.Параметры.НаименованиеОС = ВыборкаПоОС.НаименованиеОС;
			ОбластьСтрока.Параметры.Количество = 1;
			ТабДокумент.Вывести(ОбластьСтрока);
			//Если РаскрыватьСоставОС Тогда
			//Если ВыборкаПоОС.ОсновноеСредство.СоставОС.Количество()>0 Тогда
			//	Для Каждого Строка Из ВыборкаПоОС.ОсновноеСредство.СоставОС Цикл
			//		ОбластьСтрока.Параметры.НаименованиеОС = Строка.Наименование;
			//		ОбластьСтрока.Параметры.Количество = Строка.Колво;
			//		ТабДокумент.Вывести(ОбластьСтрока);
			//	КонецЦикла;
			//КонецЕсли;
			//КонецЕсли;
			ОбластьРазде3_Подвал = Макет.ПолучитьОбласть("Раздел3_Подвал");
			Если НЕ(ВыборкаПоОС.ГруппаОС = Перечисления.ГруппыОС.Здания ИЛИ ВыборкаПоОС.ГруппаОС = Перечисления.ГруппыОС.Сооружения) Тогда
				ОбластьРазде3_Подвал.Параметры.ДатаПолуч = День;
				ОбластьРазде3_Подвал.Параметры.МесяцПолуч = Месяц;
				ОбластьРазде3_Подвал.Параметры.ГодПолуч = Год;
			КонецЕсли;
			ТабДокумент.Вывести(ОбластьРазде3_Подвал);
			//\\АБС
			...
			Пока Выборка.Следующий() Цикл
				...
				конецесли;
			КонецЦикла;
			ТабПодвал = Макет.ПолучитьОбласть("Подвал");
			ТабПодвал.Параметры.ДатаПолуч = День;
			ТабПодвал.Параметры.МесяцПолуч	 = Месяц;
			ТабПодвал.Параметры.ГодПолуч   =   Год;
			ТабПодвал.Параметры.ИнвНомер   = ВыборкаПоОС.ИнвНомерУпр;
			ТабПодвал.Параметры.ДатаСоставления = Формат(Дата,"ДФ=dd.MM.yyyy");									
			Если Не ПечатьПоДаннымУпрУчета тогда
				ТабПодвал.Параметры.МОЛПолучателя= МОЛБУ;
			Иначе 
				ТабПодвал.Параметры.МОЛПолучателя= МОЛУУ;
			КонецЕсли;
			Запрос = Новый Запрос;
			Запрос.Текст =
			"ВЫБРАТЬ
			...
			|ИЗ
			|	РегистрСведений.ОтветственныеЛицаОрганизаций.СрезПоследних(&КонДата, ) КАК ОтветственныеЛицаОрганизацийСрезПоследних
			|ГДЕ
			|	ОтветственныеЛицаОрганизацийСрезПоследних.ОтветственноеЛицо = &Бухгалтер
			...
			ФиоБух = Запрос.Выполнить().Выбрать();
			Пока ФиоБух.Следующий() Цикл
				Бух = ФиоБух.ФизическоеЛицо;	 
			КонецЦикла;
			ТабПодвал.Параметры.ГлавБухПолучателя=Бух;
			Запрос = Новый Запрос;
			Запрос.Текст =
			"ВЫБРАТЬ
			|	РаботникиОрганизацийСрезПоследних.Должность
			...
			Запрос.УстановитьПараметр("Организация",Организация); 
			Если Не ПечатьПоДаннымУпрУчета тогда
				Запрос.УстановитьПараметр("ФизЛицо",МОЛБУ);
			Иначе Запрос.УстановитьПараметр("ФизЛицо",МОЛУУ);
			КонецЕсли;
			Долж = Запрос.Выполнить().Выбрать();
			Пока Долж.Следующий() Цикл
				Должность = Долж.Должность; 
			КонецЦикла;
			ТабПодвал.Параметры.ДолжностьПолучателя=Должность;
		...
		Иначе // Объектов много - групповая печать.
			...
			Если ОсновныеСредства.Количество()>0 Тогда
				//АБС Заявка 2651
				Если ЗначениеЗаполнено(Комиссия) Тогда
					Шапка2ОС1б.Параметры.ОбъектСтроительства =  ""+Комиссия;
					Шапка2ОС1б.Параметры.НомерДоговора = СокрЛП(Комиссия.Номер);
					ДатаПриказа = Комиссия.Дата;
					Если ЗначениеЗаполнено(ДатаПриказа) Тогда
						Шапка2ОС1б.Параметры.ДатаДоговора = ДатаПриказа;
					КонецЕсли;
					
				Иначе
					//\\АБс Заявка 2651
					
					Шапка2ОС1б.Параметры.ОбъектСтроительства = ОсновныеСредства[0].ОсновноеСредство.НомерПриказа;
					Шапка2ОС1б.Параметры.НомерДоговора = СокрЛП(ОсновныеСредства[0].ОсновноеСредство.НомерПриказа.Номер);
					ДатаПриказа = ОсновныеСредства[0].ОсновноеСредство.НомерПриказа.Дата;
					Если ЗначениеЗаполнено(ДатаПриказа) Тогда
						Шапка2ОС1б.Параметры.ДатаДоговора = ДатаПриказа;
					КонецЕсли;
			...
			РаботаСДиалогами.НапечататьДокумент(ОС1б_Страница4, , , "Форма ОС-1б. Страница 4" );
			РаботаСДиалогами.НапечататьДокумент(ОС1б_Страница3, , , "Форма ОС-1б. Страница 3");
			РаботаСДиалогами.НапечататьДокумент(ОС1б_Страница2, , , "Форма ОС-1б. Страница 2");
			РаботаСДиалогами.НапечататьДокумент(ОС1б_Страница1, , , "Форма ОС-1б. Страница 1");
	...
	Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
		...
		ИначеЕсли ИмяМакета = "ОС1бухСЗаполнениемКомиссии" тогда
			//АБС ВСТАВКА 54544 НАЧАЛО 
			ТабДокумент = НапечататьСоставКомиссии();
			//\\АБС ВСТАВКА 54544 КОНЕЦ 
		...
		УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект, "Принятие к учету основных средств"), Ссылка);
	...
	Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
		...
		СтруктураМакетов.Вставить("ОС1бухСЗаполнениемКомиссии", "Форма ОС-1(бух. учет) с заполнением полей ");
...
Процедура ДополнитьОбязательныеРеквизитыШапкиУпр(Реквизиты,СтруктураШапкиДокумента)
	...
	Если (СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.ОбъектыСтроительства 
		ИЛИ СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.Прочее) и не абс_СуммыВТабличнойЧасти Тогда
...
Процедура ДополнитьОбязательныеРеквизитыШапкиРегл(Реквизиты,СтруктураШапкиДокумента)
	...
	Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.ОбъектыСтроительства и не абс_СуммыВТабличнойЧасти Тогда
	...
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.ОбъектыСтроительства и абс_СуммыВТабличнойЧасти Тогда
	...
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
		...
		Если СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ВключениеВРасходыПриПринятииКУчету
			или СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат Тогда
	...
	Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.ОбъектыСтроительства Тогда
		...
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			...
			Если СтруктураШапкиДокумента.ПоддержкаПБУ18 Тогда
				Если (СтруктураШапкиДокумента.СтоимостьНУ + СтруктураШапкиДокумента.СтоимостьПР + СтруктураШапкиДокумента.СтоимостьВР) = 0 и НЕ абс_СуммыВТабличнойЧасти Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не заполнены значения реквизитов ""Стоимость (НУ)"", ""Стоимость (ПР)"", ""Стоимость (ВР)""!", Отказ, Заголовок);
				Конецесли;
			КонецЕсли;
...
Процедура ПроверкаРеквизитовТЧ(РежимПроведения,ТаблицаОС,СтруктураШапкиДокумента,Отказ, Заголовок) Экспорт
	...
	Если ТаблицаОС.Количество()= 0 тогда
		Отказ = Истина;
		ttk_ОбщегоНазначения.СообщитьОбОшибке( "Табличная часть ""Основные средства"" пуста" , Отказ, Заголовок);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьКолонкиРегистраСписанныеТовары(ТаблицаДвижений, СтруктураШапкиДокумента)
	...
	Если СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат тогда
		ТаблицаДвижений.ЗаполнитьЗначения(СубконтоНУ1,"КорСубконтоНУ1");
		ТаблицаДвижений.ЗаполнитьЗначения(СубконтоНУ2,"КорСубконтоНУ2");
		ТаблицаДвижений.ЗаполнитьЗначения(СубконтоНУ3,"КорСубконтоНУ3");
	КонецЕсли;	
	
	//АБС_Стрельцов+ добавлено: 13.12.2012
	//------------------------------------
	ТаблицаДвижений.ЗаполнитьЗначения(Константы.абс_ПроведениеПоМеханизмуПроектногоУчета.Получить(), "СписыватьТолькоПоЗаказу");
	ТаблицаДвижений.ЗаполнитьЗначения(СтруктураШапкиДокумента.абс_ЗаказПокупателя, "ЗаказПартии");
	//------------------------------------
	//АБС_Стрельцов-
...
Процедура ДвиженияПоРегиструСписанныеТовары(СтруктураШапкиДокумента, ТаблицаПоОС);
	...
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
		...
		Если СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = ПорядокВключенияСтоимости.ВключениеВРасходыПриПринятииКУчету
			или СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат тогда
			ТаблицаДвижений.ЗаполнитьЗначения(Перечисления.КодыОперацийПартииТоваров.ПринятиеКУчетуОССоСписаниемНаЗатраты,   "КодОперацииПартииТоваров");
		Иначе
			ТаблицаДвижений.ЗаполнитьЗначения(Перечисления.КодыОперацийПартииТоваров.ПринятиеКУчетуОС,   "КодОперацииПартииТоваров");
...
Процедура ФормированиеДвиженийУпр(РежимПроведения,СтруктураШапкиДокумента, ТаблицаПоОС, Отказ, Заголовок)
	...
	Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.Оборудование Тогда
		...
		//Горелик Р.В. 03.10.2018 [SC13186]+++
		//Бобылев А.А. 21.05.2018 СППР00-00000088
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
			СтрокаДвижений.СчетУчета = СтруктураШапкиДокумента.СчетУчетаБУВнеоборотногоАктива;
		КонецЕсли;
		//Бобылев А.А.---------------------------
		//Горелик Р.В. 03.10.2018 [SC13186]---
	...
	Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.ОбъектыСтроительства Тогда
		...
		Движение.Сумма               = ?(абс_СуммыВТабличнойЧасти,ОсновныеСредства.Итог("СуммаОС_УУ"),СтруктураШапкиДокумента.Стоимость);
	...
	Если НЕ СтруктураШапкиДокумента.ОтразитьВСоставеМПЗ Тогда	
		...
		//АБС
		ДвижениеМестоЭксплуатацииПоСооружению = Движения.МестоЭксплуатацииОСПоСооружению;	
		//АБС
		...
		Для Каждого Строка Из ОсновныеСредства Цикл
			...
			Если УказаноМестонахождение Тогда
				...
				//АБС Тупиков начало 13042012
				Движение.Обособленное 	 	 		 = СтруктураШапкиДокумента.абс_Обособленное;
				//\\АБС Тупиков конец
			...
			КонецЕсли;
			//АБС ВСТАВКА 76582
			//Движение по регистру МестоЭксплуатацииОСПоСооружению
			Движение = ДвижениеМестоЭксплуатацииПоСооружению.Добавить();
			Движение.Период						 = ДатаДока;
			Движение.ОсновноеСредство			 = ТекОС;
			Движение.МестоЭксплуатацииПоСооружению = СтруктураШапкиДокумента.МестоЭксплуатацииПоСооружению;
			Движение.ФункциональнаяПринадлежность = СтруктураШапкиДокумента.ФункциональнаяПринадлежность;
			//\\АБС
			
			//АБС Перезаполнение МестаЭксплуатацииПоСооружению
			Если Не ТекОС.МестоЭксплуатацииПоСооружению = СтруктураШапкиДокумента.МестоЭксплуатацииПоСооружению Тогда 
				РедактируемоеОС = ТекОС.ПолучитьОбъект();
				РедактируемоеОС.МестоЭксплуатацииПоСооружению = СтруктураШапкиДокумента.МестоЭксплуатацииПоСооружению;
				Попытка 
					РедактируемоеОС.Записать();
				Исключение
				КонецПопытки;
			КонецЕсли;
			//\\
			//\\АБС ВСТАВКА 76582 КОНЕЦ
...
Процедура ДвиженияПоОтражениюАктиваВСоставеМПЗ(СтруктураШапкиДокумента, ТаблицаПоОС, Отказ, Заголовок)
	...
	Если НЕ СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.Оборудование Тогда
		...
		Если НЕ СуммаПроводки = 0 Тогда
			...
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "СтатьиЗатрат", СтруктураШапкиДокумента.абс_СтатьяЗатрат);
...
Процедура ФормированиеДвиженийРегл(РежимПроведения,СтруктураШапкиДокумента, ТаблицаПоОС, ТаблицаОплат, Отказ, Заголовок)
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда 
		...
		Иначе
			...
			Для Каждого Строка Из ОсновныеСредства Цикл
				...
				Движение.НачислятьАмортизациюВТекущемМесяце = СтруктураШапкиДокумента.абс_НачислятьАмортизациюВТекущемМесяцеБУ;
				...
				Если Не СписыватьПриПринятииКУчету Тогда
					...
					Движение.ПрименитьВТекущемМесяце = СтруктураШапкиДокумента.абс_НачислятьАмортизациюВТекущемМесяцеБУ;
				КонецЕсли;
	...
	ТЗДанныеОС.Колонки.Добавить("Сумма", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	ТЗДанныеОС.Колонки.Добавить("СуммаХоз", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	ТЗДанныеОС.Колонки.Добавить("СуммаПР", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	ТЗДанныеОС.Колонки.Добавить("СуммаПРХоз", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	ТЗДанныеОС.Колонки.Добавить("СуммаВР", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	ТЗДанныеОС.Колонки.Добавить("СуммаВРХоз", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	ТЗДанныеОС.Колонки.Добавить("СуммаВРМинус", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	ТЗДанныеОС.Колонки.Добавить("СуммаКапитальныхВложений", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	СписатьВБУИВНУ = ((СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ВключениеВРасходыПриПринятииКУчету 
							или СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат)
	...
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда 
		...
		//абс н
		Если (СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.Оборудование)
			//или СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.ОбъектыСтроительства)
			//абс к
			Тогда
			...
			// {{ТТК Лапин А. Заявка № 7719595 03.03.2016 начало
			Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.Оборудование 
				И (СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат 
					И СтруктураШапкиДокумента.Дата >= '20160101') Тогда
			...
			Иначе
				Запрос.УстановитьПараметр("Вид",         ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства);  			
			КонецЕсли;
			// }}ТТК Лапин А. Заявка № 7719595 03.03.2016 окончание
			//Иначе
			//	Запрос.УстановитьПараметр("Вид",         ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ОбъектыСтроительства);
			//КонецЕсли;
			...
			Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.Оборудование 
				И (СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат 
					И СтруктураШапкиДокумента.Дата >= '20160101') Тогда
				Запрос.Текст = СтрЗаменить(Запрос.Текст,"ВЫБРАТЬ", "ВЫБРАТЬ ПЕРВЫЕ " + Строка(ТаблицаПоОС.Количество())); //ТаблицаПоОС.количество()
			КонецЕсли;
			// }}ТТК Лапин А. Заявка № 7719595 01.03.2016 окончание
			
			ТЗДанныеОС = Запрос.Выполнить().Выгрузить();
			
			// {{ТТК Лапин А. Заявка № 7719595 01.03.2016 начало
			...
			Если СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ВключениеВРасходыПриПринятииКУчету 
				ИЛИ (СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат 
					И СтруктураШапкиДокумента.Дата >= '20160101' )
				Тогда
			// }}ТТК Лапин А. Заявка № 7719595 01.03.2016 окончание
				...
				//Если СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат Тогда
				//	ТЗДанныеОС.ЗаполнитьЗначения(СчетУчетаНУВнеоборотногоАктива,"СчетАмортизацииНУ");
				//Иначе	
				...
				// Распредение стоимости по направлениям начисления амортизации. ЗДЕСЬ ПРОВОДКИ НУ
				...
				вр_ТЗДанныеОС = ТЗДанныеОС.Скопировать();
				вр_ТЗДанныеОС.Свернуть("ОбъектУчета");
				
				Для Каждого ТЗДанныеОССтрока ИЗ вр_ТЗДанныеОС Цикл
		...
		Иначе // не оборудование
			...
			Для Каждого Строка Из ОсновныеСредства Цикл
				...
				Если СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ВключениеВРасходыПриПринятииКУчету
					или СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат Тогда
					//АБС
					НоваяСтрока.СуммаВРМинус =-СтрокаТабОС.СуммаОС_НУ;
				Иначе
			...
			//АБС ИЗМЕНЕНИЕ	26712 
			//Для списания на расходы или затраты, стоимость НУ не регистрировать.
			//Т.к. проводок по 01 НУ быть не должно.
			Если СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.НачислениеАмортизации 
				или СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.СтоимостьНеВключаетсяВРасходы  Тогда
				Движения.СтоимостьОСНалоговыйУчет.ВыполнитьПриход();
			КонецЕсли;    			
			//Движения.СтоимостьОСНалоговыйУчет.ВыполнитьПриход();
			//\\АБС ИЗМЕНЕНИЕ 26712 КОНЕЦ
		...
		Если СтруктураШапкиДокумента.СписыватьПриПринятииКУчету 
			...
			И Не (СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ВключениеВРасходыПриПринятииКУчету 
			       или СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат) Тогда              			
			Запрос = Новый Запрос;  			
			Запрос.УстановитьПараметр("Регистратор", Ссылка);
			Запрос.УстановитьПараметр("Вид",         ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства);
		...
		Для Каждого Строка Из ОсновныеСредства Цикл
			...
			Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.ОбъектыСтроительства Тогда
				...
				Если (СтрокаТабОС.СуммаОС_НУ - СтрокаТабОС.СуммаОС_НУ_Хоз) > 0 Тогда
					...
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 1, СубконтоНУ1);
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 2, СубконтоНУ2);
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 3, СубконтоНУ3);					
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "ОсновныеСредства", ТекОС);   					
			...
			//++ Задача № 9721 Логинчев А.С. 17.05.2012 10:58:54
			Если абс_ИспользоватьНакопленныеЗатратыПоКапитализацииПроцентов Тогда
				//Если СтрокаТабОС.СуммаОС_БУ - СтрокаТабОС.СуммаОС_НУ > 0 Тогда
					//Проводка = ПроводкиНУ.Добавить();
					//Проводка.Период 		= ДатаДока;
					//Проводка.Организация 	= СтруктураШапкиДокумента.Организация;
					//Проводка.Сумма       	= СтрокаТабОС.СуммаОС_БУ - СтрокаТабОС.СуммаОС_НУ;
					//Проводка.Содержание  	= "Принят к учету объект ОС";
					//	
					//Проводка.СчетДт      = СтруктураШапкиДокумента.СчетУчетаНУ;
					//БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "ОсновныеСредства", ТекОС);
					...
					//	Запрос.УстановитьПараметр("ОсновноеСредство",СтрокаТабОС.ОсновноеСредство);
					//	Запрос.Текст =
					//	"ВЫБРАТЬ
					...
					//	Выборка.Следующий();
					//	Попытка
					//		СпособОтражения = Выборка.СпособыОтраженияРасходовПоАмортизации.Способы[0];
					//	Исключение
					//		СпособОтражения = Неопределено;
					//	КонецПопытки;
					//	Если НЕ СпособОтражения = Неопределено Тогда
					//		Проводка = ПроводкиНУ.Добавить();
					//		Проводка.Период 		= ДатаДока;
					//		Проводка.Организация 	= СтруктураШапкиДокумента.Организация;
					//		Проводка.Сумма       	= СтрокаТабОС.СуммаОС_НУ - СтрокаТабОС.СуммаОС_БУ;
					//		Проводка.Содержание  	= "Принят к учету объект ОС";
					//		
					//		Проводка.СчетДт      = СпособОтражения.СчетЗатратНУ;
					//		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 1, СпособОтражения.СубконтоНУ1);
					//		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 2, СпособОтражения.СубконтоНУ2);
					//		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 3, СпособОтражения.СубконтоНУ3);
					//		Проводка.ВидУчетаДт = Перечисления.ВидыУчетаПоПБУ18.ВР;
					//	КонецЕсли;
				//КонецЕсли;
				...
				Запрос.Текст = 
				"ВЫБРАТЬ
				|	СУММА(абс_КапитализацияПроцентовПоОСОстатки.СуммаОстаток) КАК СуммаОстаток,
				|	абс_КапитализацияПроцентовПоОСОстатки.ОбъектСтроительства,
				...
				|				И Организация = &Организация) КАК абс_КапитализацияПроцентовПоОСОстатки
				|
				|СГРУППИРОВАТЬ ПО
				|	абс_КапитализацияПроцентовПоОСОстатки.ОбъектСтроительства,
				|	абс_КапитализацияПроцентовПоОСОстатки.СтатьяДоходовРасходов";
				...
				Если Выборка.Следующий() Тогда
					...
					Если Выборка.СуммаОстаток < абс_СуммаЗатратКапитализированныхПроцентов Тогда
						
						ttk_ОбщегоНазначения.СообщитьОбОшибке("Сумма затрат по капитализируемым процентам превышает остаток по объекту строительства. 
						|Сумма затрат: " + Формат(абс_СуммаЗатратКапитализированныхПроцентов, "ЧЦ=15; ЧДЦ=2") + ", остаток по объекту: " + Формат(Выборка.СуммаОстаток, "ЧЦ=15; ЧДЦ=2") + ".",Отказ);
						
					КонецЕсли;
					
					// Кт 91.02.7 - Сумма капитализированных процентов 
					Проводка = ПроводкиНУ.Добавить();
					
					Проводка.Период 		= ДатаДока;
					Проводка.Организация 	= СтруктураШапкиДокумента.Организация;
					Проводка.Сумма       	= - абс_СуммаЗатратКапитализированныхПроцентов;
					Проводка.Содержание  	= "Принят к учету объект ОС";
						
					Проводка.СчетКт      	= ПланыСчетов.Налоговый.ВнереализационныеРасходы;
					Проводка.ВидУчетаКт 	= Перечисления.ВидыУчетаПоПБУ18.ВР;
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "ПрочиеДоходыИРасходы", Выборка.СтатьяДоходовРасходов);
					...
					Запись.ОбъектСтроительства 		= ОбъектСтроительства;
					Запись.СтатьяДоходовРасходов 	= Выборка.СтатьяДоходовРасходов;  					
					Запись.Сумма 					= абс_СуммаЗатратКапитализированныхПроцентов;
					                                                              					
					Запись = Движения.абс_КапитализацияПроцентовПоОС.ДобавитьПриход(); 					
					Запись.ВидДвижения				= ВидДвиженияНакопления.Приход;
					Запись.Период					= Дата; 					
					Запись.Организация 				= СтруктураШапкиДокумента.Организация;
					Запись.ОсновноеСредство 		= СтрокаТабОС.ОсновноеСредство;
					Запись.СтатьяДоходовРасходов 	= Выборка.СтатьяДоходовРасходов;
					
					Запись.Сумма 					= абс_СуммаЗатратКапитализированныхПроцентов;
				КонецЕсли;
			КонецЕсли;
			//-- Задача № 9721 Логинчев А.С.
		...
		Для Каждого Строка Из ТЗДанныеОС Цикл
			...
			Если ОтражатьВРегистрахНУ 
				...
				Если СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.НачислениеАмортизации Тогда
					...
					Движение.ПрименитьВТекущемМесяце = СтруктураШапкиДокумента.абс_НачислятьАмортизациюВТекущемМесяцеНУ;
					...
					Движение.НачислятьАмортизациюВТекущемМесяце = СтруктураШапкиДокумента.абс_НачислятьАмортизациюВТекущемМесяцеНУ;
		...
		Если СписатьВБУИВНУ Тогда		
			...
			Для Каждого Строка ИЗ ТЗДанныеОС Цикл
				...
				Строка.ВидУчета = "Нал";
			КонецЦикла;
			ТЗДанныеОС.Свернуть("НаправлениеАмортизации,ОбъектУчета,СчетАмортизации,СчетАмортизацииНУ,ИмяСубконто,ВидУчета","Сумма,СуммаБУ,СуммаПР,СуммаВР");
			// далее нужно распределить суммы по направлениям начисления амортизации
			ТабЗатрат = УправлениеВнеоборотнымиАктивами.ПолучитьРаспределениеАмортизацииПоНаправлениямРегл(ЭтотОбъект, Отказ, Заголовок, ТЗДанныеОС, СтруктураШапкиДокумента, "Нал", "ОС", "Списание стоимости объекта ОС");
		КонецЕсли;
...
Процедура ДвиженияРегистровПодсистемыНДС(СтруктураШапкиДокумента, ТаблицаПоОС, Отказ, Заголовок, ФормироватьПроводки = Истина) Экспорт
	...
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.ОбъектыСтроительства	Тогда
		...
		Если (?(абс_СуммыВТабличнойЧасти,ОсновныеСредства.Итог("СуммаОС_БУ"),СтруктураШапкиДокумента.СтоимостьБУ) - ?(абс_СуммыВТабличнойЧасти,ОсновныеСредства.Итог("СуммаОС_БУ_Хоз"),СтруктураШапкиДокумента.СтоимостьБУХоз)) > 0 Тогда
			...
			ИначеЕсли (СуммаОстатка < 0) Тогда
				...
				КоэффициентРаспределения = 1;
			Иначе
				КоэффициентРаспределения = (?(абс_СуммыВТабличнойЧасти,ОсновныеСредства.Итог("СуммаОС_БУ"),СтруктураШапкиДокумента.СтоимостьБУ) - ?(абс_СуммыВТабличнойЧасти,ОсновныеСредства.Итог("СуммаОС_БУ_Хоз"),СтруктураШапкиДокумента.СтоимостьБУХоз))/СуммаОстатка;
			КонецЕсли;
			...
			Если абс_СуммыВТабличнойЧасти и ТаблицаДвижений_НДСОСиНМА <> Неопределено Тогда
				ТаблицаДвижений_НДСОСиНМА.Колонки.Добавить("СуммаОстатка");
				ТаблицаДвижений_НДСОСиНМА.ЗаполнитьЗначения(СуммаОстатка,"СуммаОстатка");
			КонецЕсли;	
		...
		Если СтруктураШапкиДокумента.Дата >= '20060101' Тогда
			...
			ИначеЕсли (СуммаОстатка_Хозспособ < 0) Тогда
				...
				КоэффициентРаспределения = 1;
			Иначе
				КоэффициентРаспределения = (?(абс_СуммыВТабличнойЧасти,ОсновныеСредства.Итог("СуммаОС_БУ_Хоз"),СтруктураШапкиДокумента.СтоимостьБУХоз))/СуммаОстатка_Хозспособ;
			КонецЕсли;
			...
			СуммаБезНДСПоХозСпособу = ?(абс_СуммыВТабличнойЧасти,ОсновныеСредства.Итог("СуммаОС_БУ_Хоз"),СтруктураШапкиДокумента.СтоимостьБУХоз) - СуммаБезНДСПоХозСпособу_НачисленоРанее;
			СуммаНДСПоХозСпособу = ?(абс_СуммыВТабличнойЧасти,ОсновныеСредства.Итог("СуммаОС_НДС"),СтруктураШапкиДокумента.СуммаНДС) - НДСПоХозСпособу_НачисленоРанее;
			...
			Если абс_СуммыВТабличнойЧасти и ТаблицаДвижений_НДСОСиНМА <> Неопределено Тогда
				ТаблицаДвижений_НДСОСиНМА.Колонки.Добавить("СуммаОстатка_Хозспособ");
				ТаблицаДвижений_НДСОСиНМА.ЗаполнитьЗначения(СуммаОстатка_Хозспособ,"СуммаОстатка_Хозспособ");
			КонецЕсли;			
		...
		Если НЕ абс_СуммыВТабличнойЧасти Тогда
		...
		Иначе	
			УчетНДСФормированиеДвижений.ПоТабличнойЧастиСформироватьДвиженияПоРегиструНДСОСиНМА_ПринятиеКУчетуОС_Модернизация(СтруктураШапкиДокумента,ТаблицаДвижений_НДСПредъявленный,ТаблицаДвижений_НДСОСиНМА,ТаблицаПоОС);
		КонецЕсли;	
...
Процедура ПодготовитьТаблицаОС( СтруктураШапкиДокумента, ТаблицаПоОС)
	...
	Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		МассивСуммУУ     = ttk_ОбщегоНазначения.РаспределитьПропорционально( СтруктураШапкиДокумента.Стоимость,      МассивКоэф, 2);
	КонецЕсли;
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		МассивСуммБУ     = ttk_ОбщегоНазначения.РаспределитьПропорционально( СтруктураШапкиДокумента.СтоимостьБУ,    МассивКоэф, 2);
		МассивСуммБУ_Хоз = ttk_ОбщегоНазначения.РаспределитьПропорционально( СтруктураШапкиДокумента.СтоимостьБУХоз, МассивКоэф, 2);
		МассивСуммБУ_НДС = ttk_ОбщегоНазначения.РаспределитьПропорционально( СтруктураШапкиДокумента.СуммаНДС,       МассивКоэф, 2);
	КонецЕсли;
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
		МассивСуммНУ     = ttk_ОбщегоНазначения.РаспределитьПропорционально( СтруктураШапкиДокумента.СтоимостьНУ,    МассивКоэф, 2);
		МассивСуммНУ_Хоз = ttk_ОбщегоНазначения.РаспределитьПропорционально( СтруктураШапкиДокумента.СтоимостьНУХоз, МассивКоэф, 2);
		МассивСуммПР     = ttk_ОбщегоНазначения.РаспределитьПропорционально( СтруктураШапкиДокумента.СтоимостьПР,    МассивКоэф, 2);
		МассивСуммПР_Хоз = ttk_ОбщегоНазначения.РаспределитьПропорционально( СтруктураШапкиДокумента.СтоимостьПРХоз, МассивКоэф, 2);
		МассивСуммВР     = ttk_ОбщегоНазначения.РаспределитьПропорционально( СтруктураШапкиДокумента.СтоимостьВР,    МассивКоэф, 2);
		МассивСуммВР_Хоз = ttk_ОбщегоНазначения.РаспределитьПропорционально( СтруктураШапкиДокумента.СтоимостьвРХоз, МассивКоэф, 2);
	КонецЕсли;
...
Процедура ПодготовитьПараметрыУчетнойПолитики(Отказ, СтруктураШапкиДокумента)
	...
	Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		мУчетнаяПолитикаУпр = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиУпр(Дата);
...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоОС, Отказ = Ложь) Экспорт
	...
	//АБС ВСТАВКА №10528 НАЧАЛО
	Если абс_СуммыВТабличнойЧасти Тогда
		Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
			СтруктураПолей.Вставить("СуммаОС_УУ",     "СуммаОС_УУ");
		КонецЕсли;
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
			СтруктураПолей.Вставить("СуммаОС_БУ",     "СуммаОС_БУ");
			СтруктураПолей.Вставить("СуммаОС_БУ_Хоз", "СуммаОС_БУ_Хоз");
			СтруктураПолей.Вставить("СуммаОС_НДС",    "СуммаОС_НДС");
		КонецЕсли;
		СтруктураПолей.Вставить("СуммаОС_ПР",     "СуммаОС_ПР");
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			СтруктураПолей.Вставить("СуммаОС_НУ",     "СуммаОС_НУ");
			СтруктураПолей.Вставить("СуммаОС_НУ_Хоз", "СуммаОС_НУ_Хоз");
			СтруктураПолей.Вставить("СуммаОС_ПР_Хоз", "СуммаОС_ПР_Хоз");
			СтруктураПолей.Вставить("СуммаОС_ВР",     "СуммаОС_ВР");
			СтруктураПолей.Вставить("СуммаОС_ВР_Хоз", "СуммаОС_ВР_Хоз");
		КонецЕсли;  
	КонецЕсли;
	//\\АБС ВСТАВКА №10528 КОНЕЦ
	...
	Если НЕ Отказ и НЕ абс_СуммыВТабличнойЧасти Тогда
...
Процедура ОбработкаПроведения(Отказ,РежимПроведения)
	...
	//Горелик Р.В. 03.10.2018 [SC13186]---
	//Исаенко С.П 11.09.2018 427
	//СтруктураШапкиДокумента.СчетУчетаБУ = СтруктураШапкиДокумента.СчетУчетаБУВнеоборотногоАктива;
	//СтруктураШапкиДокумента.СчетУчетаНУ = СтруктураШапкиДокумента.СчетУчетаНУВнеоборотногоАктива;
	//Исаенко С.П
	//Горелик Р.В. 03.10.2018 [SC13186]---
	...
	//АБС ВСТАВКА №7104 НАЧАЛО
	СтруктураШапкиДокумента.Вставить("абс_СтатьяЗатрат",абс_СтатьяЗатрат);
	//\\АБС ВСТАВКА №7104 КОНЕЦ
	...
	Если НЕ Отказ Тогда
		...
		//АБС ВСТАВКА Навценя 11.02.2015
		абс_ВнеоборотныеАктивыВызовСервера.ЗаполнитьОбособленноеПодразделениеОС(СтруктураШапкиДокумента);
		//АБС ВСТАВКА КОНЕЦ	
		// +++ T#77111954  2017.05.17 Гущина
		Если ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.Оборудование
			И (СчетУчетаБУВнеоборотногоАктива = ПланыСчетов.Хозрасчетный.ПриобретениеОбъектовОсновныхСредств
			   или СчетУчетаБУВнеоборотногоАктива = ПланыСчетов.Хозрасчетный.НайтиПоКоду("08.09"))тогда 
			ЗаполнитьСоставОС(Истина); 
		КонецЕсли;
		// --- T#77111954  2017.05.17 Гущина
	КонецЕсли;
...
Процедура КорректировкаПроводок(СтруктураШапкиДокумента,Отказ)
	...
	Если Отказ
		или не (СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат 
				или СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ВключениеВРасходыПриПринятииКУчету) Тогда
		Возврат;	
	Иначе	
		врСчетДт = БухгалтерскийУчет.ПреобразоватьСчетаБУвСчетНУ(Новый Структура("СчетБУ", СчетУчетаБУ), Ложь); 
	КонецЕсли;
	...
	Для каждого Проводка Из Движения.Налоговый Цикл 
		// {{ТТК Лапин А. Заявка № 7719595 03.03.2016 начало
		//Если СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат
		//	и (не СчетУчетаНУВнеоборотногоАктива = Проводка.СчетКт и СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.Оборудование)
		//	или (не СчетУчетаНУ = Проводка.СчетКт и СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.ОбъектыСтроительства) Тогда   				
		Если (не СчетУчетаНУ = Проводка.СчетКт и СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.ОбъектыСтроительства) Тогда   					
			
		// }}ТТК Лапин А. Заявка № 7719595 03.03.2016 окончание	
			Проводка.СчетДт = врСчетДт;	
			...
			КонецЦикла;	
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "ОсновныеСредства", ОсновныеСредства[0].ОсновноеСредство);
			Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.ОбъектыСтроительства Тогда
				Проводка.ВидУчетаДт = Перечисления.ВидыУчетаПоПБУ18.ВР;
				Проводка.ВидУчетаКт = Перечисления.ВидыУчетаПоПБУ18.ВР;			
			КонецЕсли;
		// {{ТТК Лапин А. Заявка № 7719595 03.03.2016 начало
		ИначеЕсли СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат
			И СтруктураШапкиДокумента.Дата >= '20160101'
			И СчетУчетаНУВнеоборотногоАктива = Проводка.СчетКт 
			И СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.Оборудование	Тогда   				
		...
		ИначеЕсли СчетУчетаНУ = Проводка.СчетКт Тогда	
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 1, СубконтоНУ1);
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 2, СубконтоНУ2);
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 3, СубконтоНУ3);
		КонецЕсли;		
	КонецЦикла;	
	
	Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.ОбъектыСтроительства
		и СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат Тогда
		
		ТЗ = Движения.Налоговый.Выгрузить();
		Для каждого Проводка Из ТЗ Цикл  			
			Если СчетУчетаНУ = Проводка.СчетКт Тогда 
				...
				КонецЦикла;	
				БухгалтерскийУчет.УстановитьСубконто(Строка.СчетДт, Строка.СубконтоДт, 1, Проводка.СубконтоКт1);
				БухгалтерскийУчет.УстановитьСубконто(Строка.СчетДт, Строка.СубконтоДт, 2, Проводка.СубконтоКт2);
				БухгалтерскийУчет.УстановитьСубконто(Строка.СчетДт, Строка.СубконтоДт, 3, Проводка.СубконтоКт3);
				
				Строка.СчетКт = СчетУчетаНУВнеоборотногоАктива;	
				...
				НайденнаяСтрока = ТЗ.Найти(СчетУчетаНУВнеоборотногоАктива, "СчетКт");
				Если не НайденнаяСтрока = Неопределено Тогда
					// {{ТТК Лапин А. Заявка № 7720033 31.03.2016 начало
					//БухгалтерскийУчет.УстановитьСубконто(Строка.СчетКт, Строка.СубконтоКт, 1, НайденнаяСтрока.СубконтоКт1);
					//БухгалтерскийУчет.УстановитьСубконто(Строка.СчетКт, Строка.СубконтоКт, 2, НайденнаяСтрока.СубконтоКт2);
					//БухгалтерскийУчет.УстановитьСубконто(Строка.СчетКт, Строка.СубконтоКт, 3, НайденнаяСтрока.СубконтоКт3);
					Если Строка.СчетКт = ПланыСчетов.Налоговый.НайтиПоКоду("08.03") Тогда
						БухгалтерскийУчет.УстановитьСубконто(Строка.СчетКт, Строка.СубконтоКт, "ОбъектыСтроительства", НайденнаяСтрока.СубконтоКт1);
						БухгалтерскийУчет.УстановитьСубконто(Строка.СчетКт, Строка.СубконтоКт, "СпособыСтроительства", НайденнаяСтрока.СубконтоКт2);
						БухгалтерскийУчет.УстановитьСубконто(Строка.СчетКт, Строка.СубконтоКт, "СтатьиЗатрат",         НайденнаяСтрока.СубконтоКт3);
					Иначе
						БухгалтерскийУчет.УстановитьСубконто(Строка.СчетКт, Строка.СубконтоКт, 1, НайденнаяСтрока.СубконтоКт1);
						БухгалтерскийУчет.УстановитьСубконто(Строка.СчетКт, Строка.СубконтоКт, 2, НайденнаяСтрока.СубконтоКт2);
						БухгалтерскийУчет.УстановитьСубконто(Строка.СчетКт, Строка.СубконтоКт, 3, НайденнаяСтрока.СубконтоКт3);
					КонецЕсли;
					// так пришлось делать так как Субконто 2 и 3 меняются местами после  
					// Движения.Налоговый.Прочитать();  и ТЗ = Движения.Налоговый.Выгрузить();
					
					// }}ТТК Лапин А. Заявка № 7720033 31.03.2016 окончание
				КонецЕсли;
				
				//+ Романова Н.Г. #7765619  18.05.2017				
				Если СтоимостьНУХоз > 0 Тогда 
					
					Если Строка.ВидУчетаДт = Перечисления.ВидыУчетаПоПБУ18.ВР Тогда  
						Строка.Сумма = -(СтоимостьНУ - СтоимостьНУХоз);
						СуммаНУХоз = -СтоимостьНУХоз;
					Иначе
					    Строка.Сумма = СтоимостьНУ - СтоимостьНУХоз;
						СуммаНУХоз = СтоимостьНУХоз;
					КонецЕсли;
					...
					Если не НайденнаяСтрока = Неопределено Тогда
						Если Строка.СчетКт = ПланыСчетов.Налоговый.НайтиПоКоду("08.03") Тогда
							БухгалтерскийУчет.УстановитьСубконто(Строка.СчетКт, Строка.СубконтоКт, "ОбъектыСтроительства", НайденнаяСтрока.СубконтоКт1);
							БухгалтерскийУчет.УстановитьСубконто(Строка.СчетКт, Строка.СубконтоКт, "СпособыСтроительства", Перечисления.СпособыСтроительства.Хозспособ);
							БухгалтерскийУчет.УстановитьСубконто(Строка.СчетКт, Строка.СубконтоКт, "СтатьиЗатрат",         НайденнаяСтрока.СубконтоКт3);
						Иначе
							БухгалтерскийУчет.УстановитьСубконто(Строка.СчетКт, Строка.СубконтоКт, 1, НайденнаяСтрока.СубконтоКт1);
							БухгалтерскийУчет.УстановитьСубконто(Строка.СчетКт, Строка.СубконтоКт, 2, НайденнаяСтрока.СубконтоКт2);
	...
	ИначеЕсли СтруктураШапкиДокумента.Дата >= '20160101' 
		И СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.Оборудование
		И СтруктураШапкиДокумента.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат
		//И СтруктураШапкиДокумента.СпособыОтраженияРасходовПоАмортизации = Справочники.СпособыОтраженияРасходовПоАмортизации.НайтиПоКоду("000000012"))
		Тогда
		...
		Для каждого Проводка Из ТЗ Цикл  			
			Если СчетУчетаНУ = Проводка.СчетКт Тогда 
				...
				Если не НайденнаяСтрока = Неопределено Тогда
					...
					//Если Строка.СчетДт.Количественный = Истина Тогда
					//    Строка.КоличествоДт = НайденнаяСтрока.КоличествоКт;
					//КонецЕсли;	
					Если Строка.СчетКт.Количественный = Истина Тогда
						Если Строка.ВидУчетаДт = Перечисления.ВидыУчетаПоПБУ18.ВР Тогда
							Строка.КоличествоКт = -1 * НайденнаяСтрока.КоличествоКт;
						Иначе
							Строка.КоличествоКт = НайденнаяСтрока.КоличествоКт;
						КонецЕсли;
					Конецесли;
				КонецЕсли;
...
КонецПроцедуры //КорректировкаПроводок(СтруктураШапкиДокумента,Отказ)	
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//Бобылев А.А. 07.09.2018 CHG149
	Сч = 0;
	Для каждого Элемент Из ОсновныеСредства Цикл
		Сч = Сч + 1;
		Если Элемент.ОсновноеСредство.Автотранспорт Тогда
			Если НЕ ПроверкаРегистраТС(Элемент.ОсновноеСредство) Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Нет данных о регистрации выбранного ТС, находящегося в строке " + Сч,Отказ);	
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	//Бобылев А.А.--------------------
	...
	//Если ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат
	//	и не ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.Оборудование Тогда
	//	ttk_ОбщегоНазначения.СообщитьОбОшибке("Только вид операции “Оборудование” можно указывать с порядком вкл. стоимости в состав расходов “прямое списание на затраты”.",Отказ);
	//КонецЕсли;
	Если ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПрямоеСписаниеНаСчетЗатрат
		и (ОсновныеСредства.Количество() > 1 или ОсновныеСредства.Количество() = 0) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Можно указывать только 1 (одно) ОС в тч ""Основные средства"" с порядком вкл. стоимости в состав расходов “прямое списание на затраты”.",Отказ);
	КонецЕсли;	
    // {{KM WARE Лазаревский К.В. Заявка №33016 27.07.2015 начало
	// Контроль ошибок при заполнении карточек ОС
	ПараметрСписокОС = ОсновныеСредства.ВыгрузитьКолонку("ОсновноеСредство");
	РезультатПроверки = ЗаполнениеДокументовУП.КонтрольОСНаПравильностьЗаполнения(ПараметрСписокОС);
	Если РезультатПроверки.Отказ Тогда
		Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
			Отказ = Истина;
		КонецЕсли;
		Сообщить("Ошибки в заполнении карточки ОС" + символы.ПС + РезультатПроверки.ПричинаОтказа, СтатусСообщения.Важное);
	КонецЕсли;
	// }}KM WARE Лазаревский К.В. Заявка №33016 27.07.2015 окончание              	
	
	// {{ТТК Лапин А. Заявка 35645 № 24.12.2015 начало
	// проверка заполнения колонокк ТЧ "ОсновныеСредства"  "СуммаОС_БУ" и "СуммаОС_НУ". Заполняются по кнопке "Заполнить / По стоимости"
	Если ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.Оборудование Тогда 
		Если ОсновныеСредства.Количество() > 0 Тогда
			ИтогоБУ = ОсновныеСредства.Итог("СуммаОС_БУ");
			ИтогоНУ = ОсновныеСредства.Итог("СуммаОС_НУ");
			Если ИтогоБУ = 0 ИЛИ ИтогоНУ = 0 Тогда
				Сообщить("Не заполнены значения сумм БУ и НУ в табличной части ""ОсновныеСредства"". Заполняются по кнопке ""Заполнить / По стоимости"" ");
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
		
		// +++ T#77111954  2017.05.17 Гущина
		Если СчетУчетаБУВнеоборотногоАктива = ПланыСчетов.Хозрасчетный.НайтиПоКоду("08.09") 
			И ttk_Комплектация.Количество()= 0  И РежимЗаписи = РежимЗаписиДокумента.Проведение тогда
			Сообщить("Не указан докумен комплектации номенклатуры!");
			Отказ = Истина;
		КонецЕсли;
		// --- T#77111954  2017.05.17 Гущина
		
	КонецЕсли;
	// }}ТТК Лапин А. Заявка 35645 № 24.12.2015 окончание
	
	// +++ T#77111954  2017.05.17 Гущина
	ЗаполнитьСоставОС(Ложь); // удалим состав, а добавим при проведении. Т.к. могут перепроводить и менять вид операции
	// --- T#77111954  2017.05.17 Гущина
...
Функция ПроверкаРегистраТС(ОС)
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
	Если Выборка.ВидЗаписи = Перечисления.ВидЗаписиОРегистрации.Регистрация Тогда
		Возврат Истина;
	КонецЕсли;
	Возврат Ложь;
КонецФункции
...
// +++ T#77111954  2017.05.18 Гущина
Процедура ЗаполнитьСоставОС(ДобавитьСостав) 
	
	Для Каждого Строка Из ЭтотОбъект.ОсновныеСредства Цикл
		ТекОС = Строка.ОсновноеСредство.Ссылка.ПолучитьОбъект();
		ТекОС_Состав_Удаление = ТекОС.абс_СоставОС;
		
		СтруктураПоиска = Новый Структура;
		СтруктураПоиска.Вставить("ttk_ДокументПоступления", ЭтотОбъект.Ссылка);
		СтруктураПоиска.Вставить("ttk_АвтоЗаполнениеДокументом", Истина);
		
		СтрокиТЧ =  ТекОС_Состав_Удаление.НайтиСтроки(СтруктураПоиска);
		Для Каждого ТекСтрока Из СтрокиТЧ Цикл
			Индекс = ТекОС_Состав_Удаление.Индекс(ТекСтрока);
			ТекОС_Состав_Удаление.Удалить(Индекс);
		КонецЦикла;
		
		Если ДобавитьСостав тогда
			Если СчетУчетаБУВнеоборотногоАктива = ПланыСчетов.Хозрасчетный.ПриобретениеОбъектовОсновныхСредств тогда//08.04
				ТекОС_Состав =  ТекОС.абс_СоставОС.Добавить();
				ТекОС_Состав.Номенклатура                 = Номенклатура;
				ТекОС_Состав.ХарактеристикаНоменклатуры   = ХарактеристикаНоменклатуры;
				ТекОС_Состав.СерияНоменклатуры            = СерияНоменклатуры;
				ТекОС_Состав.Количество                   = 1; 
				ТекОС_Состав.Сумма                        = Строка.СуммаОС_БУ; 
				ТекОС_Состав.ttk_ДокументПоступления      = ЭтотОбъект.Ссылка; 
				ТекОС_Состав.ttk_Организация              = Организация;
				ТекОС_Состав.Комментарий                  = Комментарий;
				ТекОС_Состав.ttk_АвтоЗаполнениеДокументом = Истина;
			Иначе
				МассивКомплектаций = Новый Массив;
				МассивКомплектаций = ЭтотОбъект.ttk_Комплектация.ВыгрузитьКолонку("ДокументКомплектации");
				
				ЗапросПоКомплектации = Новый Запрос;
				ЗапросПоКомплектации.Текст = 
				"ВЫБРАТЬ
				...
				|ГДЕ
				|	ПартииТоваровНаСкладахБухгалтерскийУчет.Регистратор В(&Регистратор)
				...
				ЗапросПоКомплектации.УстановитьПараметр("Регистратор",МассивКомплектаций);
				ЗапросПоКомплектации.УстановитьПараметр("ВидДвижения",ВидДвиженияНакопления.Расход);
				Результат = ЗапросПоКомплектации.Выполнить().Выбрать();
				Пока Результат.Следующий() цикл
					ТекОС_Состав =  ТекОС.абс_СоставОС.Добавить();
					ТекОС_Состав.Номенклатура                 = Результат.Номенклатура;
					ТекОС_Состав.ХарактеристикаНоменклатуры   = Результат.ХарактеристикаНоменклатуры;
					ТекОС_Состав.СерияНоменклатуры            = Результат.СерияНоменклатуры;
					ТекОС_Состав.Количество                   = Результат.Количество; 
					ТекОС_Состав.Сумма                        = Результат.Стоимость; 
					ТекОС_Состав.ttk_ДокументПоступления      = ЭтотОбъект.Ссылка; 
					ТекОС_Состав.ttk_Организация              = Организация;
					ТекОС_Состав.Комментарий                  = Комментарий;
					ТекОС_Состав.ttk_АвтоЗаполнениеДокументом = Истина;
