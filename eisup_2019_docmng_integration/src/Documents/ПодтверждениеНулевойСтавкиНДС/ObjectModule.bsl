...
Процедура ЗаполнитьСтрокиДокумента()
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РезультатЗапроса,ТаблицаРезультата);
	...
	Для Каждого СтрокаТЧ из Состав Цикл
		Если СтрокаТЧ.Событие = Перечисления.СобытияПоНДСПродажи.НеПодтвержденаСтавка0 Тогда
			Если НЕ ЗначениеЗаполнено(СтрокаТЧ.СуммаНДС) Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("В строке номер """+ СокрЛП(СтрокаТЧ.НомерСтроки) +
...
Процедура ДвиженияПоРегистрам(СтруктураШапкиДокумента, ТаблицаПоДокументам, Отказ, Заголовок);
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоДокументам,ТаблицаДвижений_НДСРеализация0);
	...
	Для каждого ТекСтрокаСостав Из ТаблицаПоДокументам Цикл
		...
		Если мЗаконодательство2006 
			И Не ТекСтрокаСостав.Событие = Перечисления.СобытияПоНДСПродажи.НеПодтвержденаСтавка0 Тогда
	...
	СписокСчетовФактур = ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(ТаблицаДвижений_НДСРеализация0.ВыгрузитьКолонку("СчетФактура"),Истина);
...
Процедура СформироватьДвижениеНДСПредъявленныйРеализация0_ОтражениеПодтверждения0(СтруктураШапкиДокумента, ТаблицаДокументовРеализации, ДанныеПоРеализации0, Отказ,Заголовок)
	...
	Для каждого СтрокаПодтверждения Из ТаблицаПодтверждений Цикл
		...
		Если СтрокаРеализации = Неопределено Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("По документу <"+Строка(СтрокаПодтверждения.СчетФактура)+"> не обнаружена реализация по ставке НДС 0%.", Отказ, Заголовок);
	...
	ДокументыРеализации = ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(ТаблицаДокументовРеализации.ВыгрузитьКолонку("СчетФактура"), Истина);
