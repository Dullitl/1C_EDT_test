...
Функция СформироватьЗапросДляПечати(Режим)
	...
	ИначеЕсли Режим = "ПоТабличнойЧастиДокумента" Тогда
		...
		|				И (ПриемНаРаботуВОрганизациюОсновныеНачисления.ВидРасчета.ЗачетОтработанногоВремени)
		|				И ПриемНаРаботуВОрганизациюРаботники.Сторно = ПриемНаРаботуВОрганизациюОсновныеНачисления.Сторно
		|	ГДЕ
		|		ПриемНаРаботуВОрганизациюРаботники.Ссылка = &ДокументСсылка
		|		И (НЕ ПриемНаРаботуВОрганизациюРаботники.Сторно)) КАК СтрокиДокумента
...
Функция ПечатьТ1а(ИмяМакета)
	...
	Пока ВыборкаРаботники.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВложеннаяВыборка.ВалютаТарифнойСтавки) Тогда
			...
			Иначе
				ОбластьМакета.Параметры.ОкладТарифнаяСтавка = "" + Формат(ОкладВПечатнойФорме,"ЧЦ=15; ЧДЦ=2");
			КонецЕсли;
		Иначе
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если ВыборкаПоШапкеДокумента.Следующий() Тогда
		...
		Если НЕ Отказ Тогда
			...
			ВыборкаПоСтрокамДокумента = ПриемНаРаботуВОрганизациюПереопределяемый.СформироватьЗапросПоНачисления(ПроверкаШтатногоРасписания, ЭтотОбъект).Выбрать();
