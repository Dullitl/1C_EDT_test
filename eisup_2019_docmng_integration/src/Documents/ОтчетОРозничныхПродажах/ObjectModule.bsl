...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если Не СтруктураШапкиДокумента.СкладВТабличнойЧасти Тогда
		...
		Если ЗначениеЗаполнено(СтруктураШапкиДокумента.Склад) Тогда
			Если ВидОперации = Перечисления.ВидыОперацийОтчетОРозничныхПродажах.ОтчетНТТОПродажах Тогда
				Если СтруктураШапкиДокумента.ВидСклада <> Перечисления.ВидыСкладов.НТТ Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("При данном виде операции в качестве склада может быть выбрана только НТТ!", Отказ, Заголовок);
				КонецЕсли;
			Иначе
				Если СтруктураШапкиДокумента.ВидСклада = Перечисления.ВидыСкладов.НТТ Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("При данном виде операции в качестве склада не может быть выбрана НТТ!", Отказ, Заголовок);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОплатаПлатежнымиКартами.Количество() > 0 Тогда
		...
		Если НЕ ДоговорВзаиморасчетовЭквайрера.Пустая() И ДоговорВзаиморасчетовЭквайрера.Организация <> Организация Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Выбран договор эквайринга с другой организацией!", Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
	...
	Если СтруктураШапкиДокумента.УчитыватьНДС И Не СтруктураШапкиДокумента.СуммаВключаетНДС Тогда
		Для каждого СтрокаТаблицы ИЗ ТаблицаПоТоварам Цикл
			Если СтрокаТаблицы.ВидСклада = Перечисления.ВидыСкладов.НТТ Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Документ не может осуществлять реализацию с НТТ (строка " + СтрокаТаблицы.НомерСтроки + "), 
...
Процедура ПроверитьЗаполнениеТабличнойЧастиОплатаБанковскимиКредитами(ТаблицаПоБанковскимКредитам, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Для каждого СтрокаКредита Из ТаблицаПоБанковскимКредитам Цикл
		...
		Если ЗначениеЗаполнено(СтрокаКредита.ДоговорВзаиморасчетовБанкаКредитора) 
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + СтрокаОкончанияСообщенияОбОшибке,
		                    Отказ, Заголовок);
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоСкидкам,
	...
	Если ТаблицаНаСкладах.Количество() > 0 Тогда
		...
		Если Не Отказ Тогда
			...
			//+ Машута А. 15.10.2018 задача 829
			// Поскольку имя колонки в ТЧ и имя измерения отличаются, то дабы не перебирать и не дозаполнять движения по регистру,
			// переименуем колонку на момент заполнения и потом вернем обратно.
			КолонкаСчетУчета = ТаблицаНаСкладах.Колонки.Найти("СчетУчетаБУ");
			
			Если НЕ КолонкаСчетУчета = Неопределено Тогда
				КолонкаСчетУчета.Имя = "СчетУчета";		
			КонецЕсли;
			//- Машута А. 15.10.2018 задача 829
			
			// Заполним таблицу движений.
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаНаСкладах, ТаблицаДвижений);
			
			ТаблицаДвижений.ЗаполнитьЗначения(Справочники.Качество.Новый, "Качество");
			
			//+ Машута А. 15.10.2018 задача 829
			Если НЕ КолонкаСчетУчета = Неопределено Тогда
				КолонкаСчетУчета.Имя = "СчетУчетаБУ";		
			КонецЕсли;
			//- Машута А. 15.10.2018 задача 829
	...
	Если ТаблицаВРознице.Количество() > 0 Тогда
		...
		Если Не Отказ Тогда
			...
			// Заполним таблицу движений.
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаВРознице, ТаблицаДвижений);
	...
	Если ТаблицаВНТТ.Количество() > 0 Тогда
		...
		Если Не Отказ Тогда
			...
			// Заполним таблицу движений.
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаВНТТ, ТаблицаДвижений);
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоТоварамПродажи, ТаблицаДвижений);
	КонецЕсли;
	...
	// Заполним таблицу движений.
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоПродажамПоДисконтнымКартам, ТаблицаДвижений);
...
Процедура ДвиженияПоТоварамОрганизаций(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварамБезУслуг, Отказ, Заголовок)
	...
	// Заполним таблицу движений.
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоТоварамБезУслуг, ТаблицаДвижений);
...
Процедура ДвиженияПоРегиструСписанныеТовары(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварамБезУслуг, Отказ, Заголовок)
	...
	// Заполним таблицу движений.
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(УправлениеЗапасами.ПолучитьТаблицуСобственныхТоваров(СтруктураШапкиДокумента, ТаблицаПоТоварамБезУслуг), ТаблицаДвижений);
	...
	Иначе
		ТаблицаДвижений.ЗаполнитьЗначения(абс_СчетКассы,"КорСчетЗадолженностиБУ");//ПланыСчетов.Хозрасчетный.КассаОрганизации
		Если абс_СчетКассы = ПланыСчетов.Хозрасчетный.НайтиПоКоду("50.02") Тогда
			ТаблицаДвижений.ЗаполнитьЗначения(КассаККМ,"КорСубконтоЗадолженностиБУ2");
		КонецЕсли;
...
Процедура ПроводкиПоВыручкеОтРеализации(СтруктураШапкиДокумента, ТаблицаПоТоварам)
	...
	Для Каждого СтрокаТЧ Из ТаблицаВыручкиДляБУ Цикл
		...
		Если абс_СчетКассы = ПланыСчетов.Хозрасчетный.НайтиПоКоду("50.02") Тогда
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,2,КассаККМ); 	
		КонецЕсли;
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, 
	...
	Если СуммаОстаткаВыручки > 0 Тогда // т.е. есть ПКО не закрытые Отчетами ККМ
		...
		Для каждого СтрокаТЧ из ТаблицаОстатков Цикл
			...
			Проводка.СчетДт       = абс_СчетКассы;//ПланыСчетов.Хозрасчетный.КассаОрганизации;
...
Процедура ДвиженияПоРаспределениюОплат(СтруктураШапкиДокумента, ТаблицаПоПлатежнымКартам, ТаблицаПоБанковскимКредитам, Отказ, Заголовок)
	...
	СчетНаличные = абс_СчетКассы;//ПланыСчетов.Хозрасчетный.КассаОрганизации;
	ДатаДока     = Дата;
	...
	Если СуммаПродажЗаНаличные < 0 Тогда
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Сумма оплаты платежными картами и банковскими кредитами больше общей суммы продаж по отчету!", Отказ, Заголовок);
	...
	ИначеЕсли СуммаПродажЗаНаличные > 0 Тогда
		...
		// Лапин Бубович Заявка №7740974  27.07.2016 + 
		Если абс_СчетКассы = ПланыСчетов.Хозрасчетный.НайтиПоКоду("50.02") Тогда
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,2,КассаККМ);  
		КонецЕсли;
		// Лапин Бубович Заявка №7740974   27.07.2016 -
