...
Функция ПечатьОС2(ПечатьПоДаннымУпрУчета = Истина)
	...
	СотрудникПолучилДанные = ПолучитьДанныеСотрудника(?(ЗначениеЗаполнено(МОЛОрганизации), МОЛОрганизации, МОЛ));
	
	Если СотрудникПолучилДанные <> Неопределено тогда 
		ОбластьПодвал.Параметры.ПринялДолжность = СотрудникПолучилДанные.Должность;
	КонецЕсли;
	
	ФамилияИмяОтчествоФизЛица             = ФормированиеПечатныхФормСервер.ФамилияИмяОтчество(?(ЗначениеЗаполнено(МОЛОрганизации), МОЛОрганизации, МОЛ), Дата);
	...
	ОбластьПодвал.Параметры.ПринялФИО = ФИООтветственный;
	...
	Если  ОС.Количество() <> 0 Тогда
		...
		Если Результат.Количество() <> 0 Тогда
			...
			ФизЛицоСдал = Результат[0].МОЛ;
			СотрудникСдалДанные = ПолучитьДанныеСотрудника(ФизЛицоСдал);
			Если СотрудникСдалДанные <> Неопределено тогда 
				ОбластьПодвал.Параметры.СдалДолжность = СотрудникСдалДанные.Должность;
			КонецЕсли;
			ФамилияИмяОтчествоФизЛица             = ФормированиеПечатныхФормСервер.ФамилияИмяОтчество(ФизЛицоСдал, Дата);
			ФамилияИмяОтчествоОтветственного = ФамилияИмяОтчествоФизЛица.Фамилия + " " + ФамилияИмяОтчествоФизЛица.Имя + " " + ФамилияИмяОтчествоФизЛица.Отчество;
			ФИООтветственный                                     = ОбщегоНазначения.ФамилияИнициалыФизЛица(ФамилияИмяОтчествоОтветственного);
			ОбластьПодвал.Параметры.СдалФИО = ФИООтветственный;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеШапкиРегл(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Иначе
		Если ЗначениеЗаполнено(СтруктураШапкиДокумента.ПодразделениеОрганизации) 
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Выбранное подразделение организации не соответствует указанной организации", Отказ,Заголовок);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	// {{KM WARE Малыгин П.К. Заявка № 34118 09.12.2015 начало
	// Проверка еще одного реквизита по заявке
	ОбязательныеРеквизитыШапки = ОбязательныеРеквизитыШапки + ?(ПустаяСтрока(ОбязательныеРеквизитыШапки), "", ", ") + "абс_Обособленное";
	// }}KM WARE Малыгин П.К. Заявка № 34118 09.12.2015 окончание
...
Процедура ПроверкаРеквизитовТЧРегл(ТаблицаОС,СтруктураШапкиДокумента,Отказ, Заголовок)
	...
	Если Выборка.Количество() > 0 Тогда
		...
		Пока Выборка.Следующий() Цикл
			ttk_ОбщегоНазначения.СообщитьОбОшибке(("Бух.учет: Несоответствие организаций ОС """ + СокрЛП(Выборка.ОсновноеСредствоПредставление) + """ инв.№ " + СокрЛП(Выборка.Инв) + " и организации указанной в документе."),, Заголовок);
		КонецЦикла; 
	КонецЕсли; 
...
Процедура ДвиженияПоРегистрамУпр(СтруктураШапкиДокумента, ТаблицаПоОС, Отказ, Заголовок)
	...
	Для каждого СтрокаТЧ Из ТаблицаПоОС Цикл
		...
		//АБС
		Если  ЗначениеЗаполнено(СтруктураШапкиДокумента.МОЛ) Тогда
		Движение.МОЛ              = СтруктураШапкиДокумента.МОЛ;
		Иначе
		Движение.МОЛ              = СтрокаТЧ.МОЛ;
		КонецЕсли;
		//АБС
	...
	//Движение по регистру сведений "Место эксплуатации по сооружению"
	//АБС
	Для Каждого ТекСтрокаОС Из ОС Цикл
		Движение = Движения.МестоЭксплуатацииОСПоСооружению.Добавить();
		Движение.Период = Дата;
		Движение.ОсновноеСредство = ТекСтрокаОС.ОсновноеСредство;
		Движение.МестоЭксплуатацииПоСооружению = ТекСтрокаОС.МестоЭксплуатацииПоСооружению;
		Движение.ФункциональнаяПринадлежность = ТекСтрокаОС.ФункциональнаяПринадлежность;
	КонецЦикла;
	Движения.МестоЭксплуатацииОСПоСооружению.Записать();
	//АБС\\

КонецПроцедуры
//АБС
Процедура абс_ЗаполнитьМол(ТаблицаПоОС)
	
	Если ТаблицаПоОС.Колонки.Найти("МОЛ") = Неопределено Тогда
		ТаблицаПоОС.Колонки.Добавить("МОЛ");	
	КонецЕсли;
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	Документ.ПеремещениеОС.ОС КАК ПеремещениеОСОС
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МестонахождениеОСБухгалтерскийУчет.СрезПоследних(&Дата, ) КАК МестонахождениеОССрезПоследних
	               |		ПО ПеремещениеОСОС.ОсновноеСредство = МестонахождениеОССрезПоследних.ОсновноеСредство
	               |ГДЕ
	               |	ПеремещениеОСОС.Ссылка = &Ссылка";
	...
	Для Каждого Стр Из Результат Цикл
		СтрокаОС = ТаблицаПоОС.Найти(Стр.ОсновноеСредство,"ОсновноеСредство");
		Если СтрокаОС<>Неопределено Тогда
			СтрокаОС.МОЛ = Стр.Мол;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДвиженияПоРегистрамРегл(СтруктураШапкиДокумента, ТаблицаПоОС, Отказ, Заголовок)
	...
	Для каждого СтрокаТЧ Из ТаблицаПоОС Цикл
		...
		//АБС
		Если  ЗначениеЗаполнено(СтруктураШапкиДокумента.МОЛОрганизации) Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//АБС+
	ТабЧастьОС=ОС;
	Для Каждого ЭлементОС из ТабЧастьОС	Цикл
		ОбъектОС=ЭлементОС.ОсновноеСредство.ПолучитьОбъект();
		Если ОбъектОС.МестоЭксплуатацииПоСооружению <> ЭлементОС.МестоЭксплуатацииПоСооружению Тогда
			ОбъектОС.МестоЭксплуатацииПоСооружению = ЭлементОС.МестоЭксплуатацииПоСооружению;
			ОбъектОС.ОбменДанными.Загрузка = Истина;
			Попытка 
				ОбъектОС.Записать();
			Исключение
				Сообщить("ОС : МестоЭксплуатацииПоСооружению "+Строка(ОбъектОС) + "не изменен");
			КонецПопытки;
		КонецЕсли;
	КонецЦикла;	
	//АБС-	
...
Функция ПолучитьДанныеСотрудника(ФизЛицо)
	...
	Запрос.УстановитьПараметр("парамДатаСобытия", Дата);
