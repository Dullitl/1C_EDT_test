	...
	Если ИспользоватьМатериалы И РаспределятьПропорциональноКоличеству И Услуги.Количество() > 1 Тогда
		...
		Для К = 2 По Услуги.Количество() Цикл
			Если Не ЕдИзм = Услуги[К - 1].Номенклатура.БазоваяЕдиницаИзмерения Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя выбирать услуги измеряемые в разных единицах измерения в случае использования распределения пропорционально количеству.", Отказ, Заголовок);
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, 
	...
	СтруктураШапкиДокумента.ИспользоватьЗаказы = Истина; //АБС Коломиец 16205	
	...
	Если ЗначениеЗаполнено(Сделка) 
	   ...
	   И ТипЗнч(Сделка) <> Тип("ДокументСсылка.СчетНаОплатуПоставщика") Тогда
		...
		Если Не Отказ Тогда
			...
			ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "ЗаказПокупателя",    Сделка);
			ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "ЕдиницаИзмерения",    неопределено); //для услуг в регистр Заказы единица измерения не записывается
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, 
	...
	СтруктураПараметровЗачетАванса = БухгалтерскийУчетРасчетовСКонтрагентами.ПодготовкаСтруктурыПараметровДляЗачетаАванса(Ссылка, мВалютаРегламентированногоУчета, Заголовок,СуммаУслугСНДС, ТаблицаПоДокументамРасчетовСКонтрагентом);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// АБС ВСТАВКА Фролов												
	ПроверитьЗаполнениеТабличннойЧастиУслугиТТК(Заголовок, Отказ);
	// АБС ВСТАВКА Фролов КОНЕЦ
...
Процедура ПриЗаписи(Отказ)
	...
	Попытка
		...
		Если абс_Статус = Перечисления.абс_СтатусыПервичныхДокументов.ОбработкаБухгалтером Тогда
...
Процедура ПроверитьЗаполнениеТабличннойЧастиУслугиТТК(Заголовок, Отказ)
	...
	СтруктураОбязательныхПолей.Вставить("абс_СтатьяОборотов");
	//АБС
	//АБС Коломиец+
	Если ОтражатьВБухгалтерскомУчете Тогда
		СтруктураОбязательныхПолей.Вставить("СубконтоБУ");
	КонецЕсли;
	Если ОтражатьВНалоговомУчете Тогда
		СтруктураОбязательныхПолей.Вставить("СубконтоНУ");
	КонецЕсли;	
	//АБС Коломиец-
	...
	Если ОтражатьВБухгалтерскомУчете И ОтражатьВНалоговомУчете Тогда
		...
		Для Каждого Стр Из Услуги Цикл
			...
			Если Стр.СубконтоБУ <> Стр.СубконтоНУ Тогда
				
				СтрокаСообщенияОбОшибке = "В строке номер """+ СокрЛП(Стр.НомерСтроки) +
				""" табличной части ""Услуги"": разные значения СубконтоБУ и СубконтоНУ";
