...
Функция ПроверкаРеквизитов(Отказ, Заголовок) Экспорт
	...
	РеквизитыТЧ = "Номенклатура, ЕдиницаИзмерения, СтатьяЗатрат, СчетЗатрат";
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		Если Не СписСчетовБух.НайтиПоЗначению(СчетДт) = Неопределено Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя указывать счет " + СчетДт + " в качестве корреспонденции! (счет бух. учета)", Отказ, Заголовок);
		КонецЕсли;

		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			СписСчетовНал = УправлениеПроизводством.ПолучитьЗатратныеСчета("Нал");
			Если Не СписСчетовНал.НайтиПоЗначению(СчетДтНУ) = Неопределено Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя указывать счет " + СчетДтНУ + " в качестве корреспонденции!  (счет нал. учета)", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
