...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок)
	...
	Если НетДатыНачала Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке("Не указана дата начала отсутствия на рабочем месте!", Отказ, Заголовок);
	КонецЕсли;
	
	// ДатаОкончания
	НетДатыНачала = НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ДатаОкончания);
	Если НетДатыНачала Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке("Не указана дата окончания отсутствия на рабочем месте!", Отказ, Заголовок);
	КонецЕсли;
	
	// Не целодневный отпуск
	Если ВыборкаПоШапкеДокумента.НецелодневныйОтпуск Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке("Отпуск может быть только целодневным!", Отказ, Заголовок);
	КонецЕсли;
	...
	Если ВыборкаПоШапкеДокумента.ДатаНачала > ВыборкаПоШапкеДокумента.ДатаОкончания Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке("Дата начала отсутствия не может превышать дату окончания!", Отказ, Заголовок);
	КонецЕсли;
	
	Если ВыборкаПоШапкеДокумента.ПроверяемоеЗначение = "Нельзя" Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке("На указанный период документом " + ВыборкаПоШапкеДокумента.Регистратор + " уже была запланирована другая занятость!", , Заголовок);
	КонецЕсли;
	
	Если ВыборкаПоШапкеДокумента.СотрудникНеРаботает Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке("Сотрудник " + ВыборкаПоШапкеДокумента.Сотрудник + " не работает на предприятии!", , Заголовок);
	КонецЕсли;
	
	// Проверка: в регистре уже есть такое движение
	Если ВыборкаПоШапкеДокумента.КонфликтноеСостояние <> NULL Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке("Сотрудник уже переведен в состояние """ + ВыборкаПоШапкеДокумента.КонфликтноеСостояние + """ документом " + ВыборкаПоШапкеДокумента.КонфликтныйДокумент + "!", Отказ, Заголовок);
	КонецЕсли;
