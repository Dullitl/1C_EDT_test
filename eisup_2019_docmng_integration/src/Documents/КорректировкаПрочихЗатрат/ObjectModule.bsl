...
Процедура ПроверитьСтатьиЗатратВТЧ(ИмяРеквСтатьяЗатрат, ИмяРеквСчетЗатрат, СтруктураШапкиДокумента, Отказ, Заголовок)
	Для Каждого СтрокаТЧ Из Затраты Цикл
        ...
        Если СтатЗатрат.ВидЗатрат = Перечисления.ВидыЗатрат.Материальные Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Укажите статью затрат с нематериальным видом затрат (таб.часть ""Затраты"" строка № " + СтрокаТЧ.НомерСтроки + ", реквизит """+?(ИмяРеквСтатьяЗатрат="СтатьяЗатрат","Статья затрат","Статья затрат - получатель")+""")", Отказ, Заголовок);
		КонецЕсли;
	КонецЦикла;
