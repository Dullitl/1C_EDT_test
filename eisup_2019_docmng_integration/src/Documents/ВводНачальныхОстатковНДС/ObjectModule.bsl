...
Процедура ЗаполнитьРасчетыСКонтрагентами() Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ПолучитьТаблицуРасчетов(), РасчетыСКонтрагентами);
...
Процедура ПроверитьЗаполнениеТабличнойЧастиДополнительныеСведения(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Для Каждого СтрокаТЧ Из ДополнительныеСведения Цикл
		Если ((СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийВводНачОстатковНДС.НДСПоПриобретеннымЦенностям) И
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке("В строке """ + СтрокаТЧ.НомерСтроки + """ должна быть заполнена колонка " + 
		...
		Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийВводНачОстатковНДС.НДСпоАвансамПолученным И
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке("В строке """ + СтрокаТЧ.НомерСтроки + """ должна быть заполнена колонка ""Документ отгрузки""!", , Заголовок);
		КонецЕсли;
		Если (СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийВводНачОстатковНДС.НДСПоПриобретеннымЦенностям И
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке("В строке """ + СтрокаТЧ.НомерСтроки + """ должна быть заполнена колонка ""Счет учета НДС""!", , Заголовок);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СтрокаТЧ Из РасчетыСКонтрагентами Цикл
		...
		Если СтруктураШапкиДокумента.ВидОперации <> Перечисления.ВидыОперацийВводНачОстатковНДС.НДСпоСМРдляСобственногоПотребления И
		   НЕ ЗначениеЗаполнено(СтрокаТЧ.Контрагент) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не заполнен реквизит ""Контрагент""!", Отказ, Заголовок);
		КонецЕсли;
		Если СтруктураШапкиДокумента.ВидОперации <> Перечисления.ВидыОперацийВводНачОстатковНДС.НДСпоСМРдляСобственногоПотребления И
		   НЕ ЗначениеЗаполнено(СтрокаТЧ.ДоговорКонтрагента) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не заполнен реквизит ""Договор контрагента""!", Отказ, Заголовок);
		КонецЕсли;
		Если СтрокаТЧ.Аванс Тогда
			Если НЕ ЗначениеЗаполнено(СтрокаТЧ.ДокументОплаты) Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не заполнен реквизит ""Документ оплаты""!", Отказ, Заголовок);
			КонецЕсли;
		Иначе
			Если НЕ ЗначениеЗаполнено(СтрокаТЧ.СчетФактура) Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не заполнен реквизит ""Счет-фактура""!", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
		...
		Если Не СтрокаТЧ.РасчетыСБюджетом Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрокаТЧ.ВалютаВзаиморасчетов) Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не заполнен реквизит ""Валюта взаиморасчетов""!", Отказ, Заголовок);
			Иначе
				Если НЕ ЗначениеЗаполнено(СтрокаТЧ.КурсВзаиморасчетов) Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не заполнен реквизит ""Курс взаиморасчетов""!", Отказ, Заголовок);
				КонецЕсли;
		...
		Если ОтразитьРасчетыСКонтрагентами Тогда
			Если НЕ ЗначениеЗаполнено(СтрокаТЧ.СчетУчетаРасчетовСКонтрагентом) И (НЕ СтрокаТЧ.РасчетыСБюджетом) и
				...
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не заполнен реквизит ""Счет учета расчетов с контрагентом""!", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;			
...
Функция ПолучитьПараметрыУчетнойПолитикиЛокально(КонДата, Отказ, ОбрабатываемаяОрганизация, СчетФактура = Неопределено)
	...
	Если УчетнаяПолитика = Неопределено Тогда
		УчетнаяПолитика = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(КонДата, ОбрабатываемаяОрганизация);
		Если Не ЗначениеЗаполнено(УчетнаяПолитика) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("При формировании движений по НДС " + ?(Не ЗначениеЗаполнено(СчетФактура), "на " + Формат(КонДата, "ДФ=dd.MM.yyyy"), "для документа <" + Строка(СчетФактура)+">")+ " 
...
Процедура СформироватьДвиженияВводОстатковПоРегиструНДСПредъявленный(СтруктураШапкиДокумента, ТаблицаПоДвижениям, Отказ, Заголовок)
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоДвижениям, ТаблицаДвиженийНДСпредъявленный);
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаКВычету, ТаблицаДвиженийНДСПредъявленный);
...
Процедура СформироватьДвиженияВводОстатковПоРегиструНДСВключенныйВСтоимость(СтруктураШапкиДокумента, ТаблицаПоДвижениям, Отказ,Заголовок)
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоДвижениям, ТаблицаДвиженийНДСВключенныйВСтоимость);
...
Процедура СформироватьДвиженияВводОстатковПоРегиструНДСЗаписиКнигиПокупок(СтруктураШапкиДокумента, ТаблицаПоДвижениям, Отказ,Заголовок)
	...
	Если не СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийВводНачОстатковНДС.НДСпоАвансамПолученным Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаЗаписей, ТаблицаДвиженийНДСЗаписиКнигиПокупок);
		Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийВводНачОстатковНДС.НДСпоСМРдляСобственногоПотребления Тогда
...
Процедура СформироватьДвиженияВводОстатковПоРегиструНДСЗаписиКнигиПродаж(СтруктураШапкиДокумента, ТаблицаПоДвижениям, Отказ,Заголовок)
	...
	Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийВводНачОстатковНДС.НДСпоАвансамПолученным Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоДвижениям, ТаблицаДвиженийНДСЗаписиКнигиПродаж);
...
Процедура СформироватьДвиженияВводОстатковПоРегиструНДСНачисленный(СтруктураШапкиДокумента, ТаблицаПоДвижениям, Отказ,Заголовок)
	...
	Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийВводНачОстатковНДС.НДСпоАвансамПолученным
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаДвижений, ТаблицаДвиженийНДСНачисленный);
	...
	Иначе
		// Продажа
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоДвижениям, ТаблицаДвиженийНДСНачисленный);
...
Процедура СформироватьДвиженияВводОстатковПоРегиструНДСРеализация0(СтруктураШапкиДокумента, ТаблицаПоДвижениям, Отказ,Заголовок)
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаДвижений, ТаблицаДвиженийНДСРеализация0);
...
Процедура СформироватьДвиженияВводОстатковПоРегиструНДСсАвансов(СтруктураШапкиДокумента, ТаблицаПоДвижениям, Отказ,Заголовок)
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоДвижениям, ТаблицаДвиженийНДСсАвансов);
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаДвижений, ТаблицаДвиженийНДСсАвансов);
...
Процедура СформироватьСчетаФактуры(СоздаватьПодчиненныеДокументыОснования = Истина, ОбновитьСодержаниеПодчиненныхДокументов = Истина) Экспорт
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	&ДанныеПоСФ КАК ВНО_НДСДанныеПоСФ // Документ.ВводНачальныхОстатковНДС.ДанныеПоСФ
		|";
	Запрос.Выполнить();
	...
	СокращеннаяТаблица.Свернуть("Контрагент, ДоговорКонтрагента, ПредъявленСФ, ДатаСФ, НомерСФ, ДатаВходящего, НомерВходящего","");
	...
	Для каждого СтрокаПараметров Из СокращеннаяТаблица Цикл
		...
		СтрокаПараметров.МассивОснований = ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(МассивОснований,Истина);
			...
			Если СтрокиСФ.Количество() = 0 Тогда
				...
				ИначеЕсли СтрокаПараметров.МассивОснований.Количество()>0 Тогда
					...
					Если ВидОперации = Перечисления.ВидыОперацийВводНачОстатковНДС.НДСПоПриобретеннымЦенностям
						...
						Или ВидОперации = Перечисления.ВидыОперацийВводНачОстатковНДС.НДСПоАвансамВыданным
						Тогда
						ОснованиеСФ = СоздатьДокументСчетФактураПолученный(СтрокаПараметров, Ссылка);
						ОснованиеСФ.Записать(РежимЗаписиДокумента.Запись);
					Иначе
						...
						Исключение
							Попытка
								Если не ОснованиеСФ.Номер = СтрокаПараметров.НомерСФ Тогда
									...
									ОснованиеСФ.Записать(РежимЗаписиДокумента.Запись);
								Иначе
									ttk_ОбщегоНазначения.СообщитьОбОшибке("Не удалось выписать выданный счет-фактуру "+СтрокаПараметров.НомерСФ+" от "+СтрокаПараметров.ДатаСФ+". Вероятно счет-фактура с таким номером уже записан в информационной базе.");
	...
	ИспользуемыеПодчиненныеДокументы.ЗагрузитьЗначения(ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(ПараметрыДокументов.ВыгрузитьКолонку("НовоеОснование"),Истина));
	
	ИспользуемыеСчетаФактуры = Новый списокЗначений();
	ИспользуемыеСчетаФактуры.ЗагрузитьЗначения(ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(ПараметрыДокументов.ВыгрузитьКолонку("НовыйСчетФактура"),Истина));
	
	ВсеПодчиненныеДокументы = ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(ПодчиненныеДокументы.ВыгрузитьКолонку("ДокументОснование"),Истина);
	...
	ВсеПодчиненныеСчетаФактуры = ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(ПодчиненныеДокументы.ВыгрузитьКолонку("СчетФактура"),Истина);
...
Процедура ОбновитьСодержаниеПодчиненныхДокументов(ПараметрыДокументов, СоздаватьПодчиненныеДокументыОснования) Экспорт
	...
	Для каждого СтрокаДокументаОснования Из ДеревоДокументов.Строки Цикл
		...
		Исключение
			Для каждого СтрокаТабличнойЧасти Из СтрокаДокументаОснования.Строки Цикл
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Для строки №" + СтрокаТабличнойЧасти.НомерСтроки + " не удалось обновить параметры документа-основания счета-фактуры.");
			КонецЦикла; 
		Конецпопытки;
	...
	ПараметрыДокументов.Свернуть("НовыйСчетФактура,Контрагент, ДоговорКонтрагента, ПредъявленСФ, ДатаСФ, НомерСФ,НовоеОснование, ВалютаДокумента,НомерСтроки, КлючСтроки, ДатаВходящего, НомерВходящего", "Сумма, СуммаНДС, СуммаДокумента");
	...
	Для каждого СтрокаДокументаОснования Из ДеревоДокументов.Строки Цикл
		...
		Иначе
			...
			ОснованиеОбъект.НаАванс = ВидОперации = Перечисления.ВидыОперацийВводНачОстатковНДС.НДСПоАвансамВыданным;
...
Функция ПолучитьТаблицуИспользуемыхПодчиненныхДокументов() экспорт
	...
	Запрос.Текст = 
		...
		|						ТОГДА ВЫБОР
		|								КОГДА &ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВводНачОстатковНДС.НДСпоАвансамПолученным)
		|										И СчетаФактуры.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.НДСВидСчетаФактуры.НаАванс)
		|									ТОГДА ИСТИНА
		|								КОГДА (НЕ &ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВводНачОстатковНДС.НДСпоАвансамПолученным))
		|										И (НЕ СчетаФактуры.Ссылка.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.НДСВидСчетаФактуры.НаАванс))
		|									ТОГДА ИСТИНА
		|								ИНАЧЕ ЛОЖЬ
		|							КОНЕЦ
		|					ИНАЧЕ ЛОЖЬ
...
//
Функция СоздатьДокументСчетФактураПолученный(СтрокаПараметров, ДокументОснование)
...
Функция СоздатьДокументСчетФактураВыданный(СтрокаПараметров, ДокументОснование, ПрефиксОрганизации)
	...
	НовыйДокумент.ВидСчетаФактуры = ?(ДокументОснование.ВидОперации = Перечисления.ВидыОперацийВводНачОстатковНДС.НДСпоАвансамПолученным
													 				, Перечисления.НДСВидСчетаФактуры.НаАванс
																	, Перечисления.НДСВидСчетаФактуры.НаРеализацию);
