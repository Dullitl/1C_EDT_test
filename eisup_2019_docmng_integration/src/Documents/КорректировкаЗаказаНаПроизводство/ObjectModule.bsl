...
Функция ПечатьЗаказа(Тип)
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект);
	...
	Для Каждого СтрокаТЧ Из ТаблицаПоМатериалам Цикл
		Если ЗначениеЗаполнено(СтрокаТЧ.Подразделение)
		   ...
		   И СтрокаТЧ.Подразделение.ВидПодразделения<>Перечисления.ВидыПодразделений.ВспомогательноеПроизводство Тогда
		    ...
		    ttk_ОбщегоНазначения.СообщитьОбОшибке("В строке № " + СтрокаТЧ.НомерСтроки + " табличной части ""Материалы"" должно быть указано подразделение с видом ""Основное производство"" или ""Вспомогательное производство""!", Отказ, Заголовок);
		КонецЕсли;
		...
		Если ВидОперации = Перечисления.ВидыОперацийКорректировкаЗаказаНаПроизводство.ЗакрытиеПотребностейЗаказаНаПроизводство И СтрокаТЧ.Количество<0 Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("В строке № " + СтрокаТЧ.НомерСтроки + " табличной части ""Материалы"" указано отрицательное количество. " + Символы.ПС + "Нельзя указывать отрицательное количество при виде операции ""Закрытие потребностей заказа на производство"".", Отказ, Заголовок);
		КонецЕсли;
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, 
	...
	Если ВидОперации = Перечисления.ВидыОперацийКорректировкаЗаказаНаПроизводство.КорректировкаЗаказаНаПроизводство Тогда
		...
		ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "ЗаказНаПроизводство", СтруктураШапкиДокумента.ЗаказНаПроизводство);
		ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "Подразделение",   Подразделение, "ТаблицаПоПродукции");
		...
		Если не Отказ и (ТаблицаПоПродукцииРазмещение.Количество()>0 или ТаблицаПоМатериаламРазмещение.Количество() > 0)
			...
			ТаблицыДанныхДокумента = ttk_ОбщегоНазначения.ЗагрузитьТаблицыДокументаВСтруктуру(НаборДвижений, СтруктТаблицДокумента);
			Если ТаблицаПоПродукцииРазмещение.Количество()>0 Тогда
				ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "ЗаказПоставщику", СтруктураШапкиДокумента.ЗаказНаПроизводство, "ТаблицаПоПродукции");
			КонецЕсли;
			Если ТаблицаПоМатериаламРазмещение.Количество()>0 Тогда
				ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "ЗаказПокупателя", СтруктураШапкиДокумента.ЗаказНаПроизводство, "ТаблицаПоМатериалам");
			КонецЕсли;
				
			ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "ТоварТара",       Перечисления.ТоварТара.Товар);
		...
		Если ТаблицаПоМатериалам.Количество() > 0 Тогда
			...
			Если ТаблицаПоТоварамРезервирование.Количество() > 0 Тогда
				...
				Если НЕ Отказ Тогда
					...
					ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "ДокументРезерва", СтруктураШапкиДокумента.ЗаказНаПроизводство);
	...
	Если мИспользоватьПотребностиЗаказовНаПроизводство Тогда
		...
		ТаблицыДанныхДокумента = ttk_ОбщегоНазначения.ЗагрузитьТаблицыДокументаВСтруктуру(НаборДвижений, СтруктТаблицДокумента);
		ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "ЗаказНаПроизводство", ЗаказНаПроизводство);
		...
		ttk_ОбщегоНазначения.ЗаписатьТаблицыДокументаВРегистр(НаборДвижений, ВидДвижения, ТаблицыДанныхДокумента, Дата);
