...
Процедура ЗаполнитьЗависимымиОборотами(ТекСтатья,
	...
	//АБС ИЗМЕНЕНИЕ 56710 
	ПустойЦФО=Новый(Тип("СправочникСсылка.ПодразделенияОрганизаций"));
	//АБС ИЗМЕНЕНИЕ 56710 КОНЕЦ
...
Функция ПроверкаКонтролируемыхЗначений(СтруктураДанныхШапки, СтрокаОборотовДляКонтроля)
	...
	//АБС ИЗМЕНЕНИЕ 56710
	Запрос.УстановитьПараметр("ПустойЦФО",Новый(Тип("СправочникСсылка.ПодразделенияОрганизаций")));	
	//АБС ИЗМЕНЕНИЕ 56710 КОНЕЦ
...
Функция СформироватьЗапросПоДаннымДокумента(СтруктураДанныхШапки)
	...
	ТекстЗапроса="ВЫБРАТЬ
				...
				//АБС ВСТАВКА 56710
				|БюджетнаяОперация.абс_ТипКонтрагента,
				|БюджетнаяОперация.абс_ТипСети,
				|БюджетнаяОперация.абс_КВ,
				|БюджетнаяОперация.абс_ТЭО,
				|БюджетнаяОперация.абс_ЦФУ, 
				|БюджетнаяОперация.Организация КАК Организация 
                //АБС ВСТАВКА 56710 КОНЕЦ
	            |ИЗ
	            |	(ВЫБРАТЬ
				...
				//АБС ВСТАВКА 56710
				|		БюджетнаяОперацияШапка.абс_ТипКонтрагента КАК абс_ТипКонтрагента,
				|		БюджетнаяОперацияШапка.абс_ТипСети КАК абс_ТипСети,
				|		БюджетнаяОперацияШапка.абс_КВ КАК абс_КВ,
				|		БюджетнаяОперацияШапка.абс_ТЭО КАК абс_ТЭО,
				|		БюджетнаяОперацияШапка.абс_ЦФУ КАК абс_ЦФУ,
				|		БюджетнаяОперацияШапка.абс_Организация КАК Организация
              	//АБС ВСТАВКА 56710 КОНЕЦ
	            |	ИЗ
				...
				//АБС ВСТАВКА 56710
				|		БюджетнаяОперацияЗависимыеОбороты.абс_ТипКонтрагента,
				|		БюджетнаяОперацияЗависимыеОбороты.абс_ТипСети,
				|		БюджетнаяОперацияЗависимыеОбороты.абс_КВ,
				|		БюджетнаяОперацияЗависимыеОбороты.абс_ТЭО,
				|		БюджетнаяОперацияЗависимыеОбороты.абс_ЦФУ,
				|		БюджетнаяОперацияЗависимыеОбороты.абс_Организация КАК Организация
				//АБС ВСТАВКА 56710 КОНЕЦ
	            |	ИЗ
	...
	Для Каждого СтрокаОборотовДляКонтроля Из ТабОборотовДокумента Цикл
		...
		Иначе
			...
			Если ТабКонтролируемыхЗначений.Количество()>0 Тогда
				...
				ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТабКонтролируемыхЗначений,ТабКонтролируемыхДвижений)
