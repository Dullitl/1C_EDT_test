...
Функция СформироватьЗапросПоШапке(Режим)
	...
	|	Док.Ссылка
	|ИЗ
	|	Документ.УдалитьВозвратНаРаботу КАК Док
	|ГДЕ
...
Функция СформироватьЗапросПоРаботники(Режим)
	...
	|ИЗ
	|	Документ.УдалитьВозвратНаРаботу.Работники КАК ТЧРаботники
	...
	|		ИЗ
	|			Документ.УдалитьВозвратНаРаботу.Работники КАК ТЧРаботники
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.УдалитьВозвратНаРаботу.Работники КАК ТЧРаботники2
...
Процедура ПроверитьЗаполнениеСтрокиРаботника(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если НетДатыВозврата Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указана дата возврата на работу!", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипОснования = Тип("ДокументСсылка.УдалитьОтсутствиеНаРаботе") Тогда
