...
Процедура ДвиженияПоРегистрам(СтруктураШапкиДокумента, Отказ)
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда           		
		...
		Если ЗначениеЗаполнено(абс_ГруппаРБП) Тогда
			БухгалтерскийУчет.РасчетИСписаниеРПБ(СтруктураШапкиДокумента, Отказ, Массив, "БУ", мПроводкиБУ, ЗаписьРасчетаРБП, абс_ГруппаРБП);
		Иначе
			БухгалтерскийУчет.РасчетИСписаниеРПБ(СтруктураШапкиДокумента, Отказ, Массив, "БУ", мПроводкиБУ, ЗаписьРасчетаРБП);
		КонецЕсли;
		// АБС ИЗМЕНЕНО Списание РБП по группам КОНЕЦ
	...
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
		...
		// АБС ИЗМЕНЕНО Списание РБП по группам
		Если ЗначениеЗаполнено(абс_ГруппаРБП) Тогда
			БухгалтерскийУчет.РасчетИСписаниеРПБ(СтруктураШапкиДокумента, Отказ, Массив, "НУ", мПроводкиНУ, ЗаписьРасчетаРБП, абс_ГруппаРБП);
		Иначе
			БухгалтерскийУчет.РасчетИСписаниеРПБ(СтруктураШапкиДокумента, Отказ, Массив, "НУ", мПроводкиНУ, ЗаписьРасчетаРБП);
		КонецЕсли;
		// АБС ИЗМЕНЕНО Списание РБП по группам КОНЕЦ
...
Процедура ОбработкаПроведения(Отказ)
	...
	#Если Клиент Тогда
		абс_БСПОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени(Справочники.абс_БСПКлючевыеОперации.РасходыБудущихПериодов_Проведение);
	#Иначе
	...
	#Иначе
		абс_БСПОценкаПроизводительностиКлиентСервер.ЗакончитьЗамерВремени(Справочники.абс_БСПКлючевыеОперации.РасходыБудущихПериодов_Проведение, ВремяНачала);
	#КонецЕсли
	//АБС ВСТАВКА 38320 КОНЕЦ 
