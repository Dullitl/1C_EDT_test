...
Процедура ПодготовитьДанныеАвтоматическогоЗаполнения(Уведомление,СписокФилиалов) Экспорт
	
	ОбщийСписокОрганизаций = Новый Массив;
	ОбщийСписокОрганизаций.Добавить(Уведомление.Организация);
	Для каждого Филиал  Из СписокФилиалов Цикл
		ОбщийСписокОрганизаций.Добавить(Филиал.Значение);
	КонецЦикла;
	
	МенеджерВременныхТаблиц = ПолучитьВременныеТаблицыДляЗаполненияСделок(Уведомление,ОбщийСписокОрганизаций);
	ТаблицаСделок = ПолучитьТаблицуСделок();
	ТаблицаСделок.Очистить();
	
	
	СписокСтранОффшоров = Новый СписокЗначений;
	МакетСписка = Документы.УведомлениеОКонтролируемыхСделках.ПолучитьМакет("СписокСтранОффшоров");
	ТекОбласть = МакетСписка.Области.СписокСтранОфшоров;
	
	Для Ном = ТекОбласть.Верх По ТекОбласть.Низ Цикл
		КодСтраны = СокрП(МакетСписка.Область(Ном, 1).Текст);
		текСтранаСсылка = Справочники.КлассификаторСтранМира.НайтиПоКоду(КодСтраны);
		Если текСтранаСсылка <> Неопределено И текСтранаСсылка <> Справочники.КлассификаторСтранМира.ПустаяСсылка() Тогда
			СписокСтранОффшоров.Добавить(текСтранаСсылка);
		КонецЕсли;
	КонецЦикла;
	
	СписокСчетовВзаиморасчетовРасходов = Новый СписокЗначений;
	МакетСписка = Документы.УведомлениеОКонтролируемыхСделках.ПолучитьМакет("МакетСписокСчетовВзаиморасчетовРасход");
	ТекОбласть = МакетСписка.Области.ОбластьСчетов;
	
	Для Ном = ТекОбласть.Верх По ТекОбласть.Низ Цикл
		КодСчета = СокрП(МакетСписка.Область(Ном, 1).Текст);
		текСчетСсылка = ПланыСчетов.Хозрасчетный.НайтиПоКоду(КодСчета);
		Если текСчетСсылка <> Неопределено И текСчетСсылка <> ПланыСчетов.Хозрасчетный.ПустаяСсылка() Тогда
			СписокСчетовВзаиморасчетовРасходов.Добавить(текСчетСсылка);
		КонецЕсли;
	КонецЦикла;	
	
	СписокКорСчетовРасходИсключаемые = Новый СписокЗначений;
	МакетСпискаСчетов = Документы.УведомлениеОКонтролируемыхСделках.ПолучитьМакет("МакетСписокКорСчетовРасходИсключаемые");
	ОбластьСчетовИсключаемые = МакетСпискаСчетов.Области.ОбластьСчетовИсключаемые;
	Для Ном = ОбластьСчетовИсключаемые.Верх По ОбластьСчетовИсключаемые.Низ Цикл
		КодСчета = СокрП(МакетСпискаСчетов.Область(Ном,1).Текст);
		текСчетСсылка = ПланыСчетов.Хозрасчетный.НайтиПоКоду(КодСчета);
		Если текСчетСсылка <> Неопределено И текСчетСсылка <> ПланыСчетов.Хозрасчетный.ПустаяСсылка() Тогда
			СписокКорСчетовРасходИсключаемые.Добавить(текСчетСсылка);
		КонецЕсли;
	КонецЦикла;
	
	СписокЗначенийСвойств = Новый СписокЗначений;
	СписокЗначенийСвойств.Добавить(Справочники.ЗначенияСвойствОбъектов.НайтиПоКоду("000005372"));
	СписокЗначенийСвойств.Добавить(Справочники.ЗначенияСвойствОбъектов.НайтиПоКоду("000005373"));
	СписокЗначенийСвойств.Добавить(Справочники.ЗначенияСвойствОбъектов.НайтиПоКоду("000005374"));
	
	СписокСчетовДоход = Новый СписокЗначений;
	СписокСчетовДоход.Добавить(ПланыСчетов.Хозрасчетный.РасчетыСПокупателямиИЗаказчиками);
	//СписокСчетовДоход.Добавить(ПланыСчетов.Хозрасчетный.РасчетыСРазнымиДебиторамиИКредиторами);
	
	СписокКорСчетовДоход = Новый СписокЗначений;
	СписокКорСчетовДоход.Добавить(ПланыСчетов.Хозрасчетный.Продажи);
	СписокКорСчетовДоход.Добавить(ПланыСчетов.Хозрасчетный.ПрочиеДоходы);
	
	СписокКорСчетовРасход = Новый СписокЗначений;
	СписокКорСчетовРасход.Добавить(ПланыСчетов.Хозрасчетный.ОборудованиеКУстановке);
	СписокКорСчетовРасход.Добавить(ПланыСчетов.Хозрасчетный.ВложенияВоВнеоборотныеАктивы);
	СписокКорСчетовРасход.Добавить(ПланыСчетов.Хозрасчетный.Материалы);
	СписокКорСчетовРасход.Добавить(ПланыСчетов.Хозрасчетный.ЗаготовлениеИПриобретениеМЦ);
	СписокКорСчетовРасход.Добавить(ПланыСчетов.Хозрасчетный.НДСпоПриобретеннымЦенностям);
	СписокКорСчетовРасход.Добавить(ПланыСчетов.Хозрасчетный.ОсновноеПроизводство_);
	СписокКорСчетовРасход.Добавить(ПланыСчетов.Хозрасчетный.ВспомогательныеПроизводства);
	СписокКорСчетовРасход.Добавить(ПланыСчетов.Хозрасчетный.ОбщепроизводственныеРасходы);
	СписокКорСчетовРасход.Добавить(ПланыСчетов.Хозрасчетный.ОбщехозяйственныеРасходы);
	СписокКорСчетовРасход.Добавить(ПланыСчетов.Хозрасчетный.Товары);
	СписокКорСчетовРасход.Добавить(ПланыСчетов.Хозрасчетный.РасходыНаПродажу);
	СписокКорСчетовРасход.Добавить(ПланыСчетов.Хозрасчетный.ПрочиеРасходы);
	
	СчетаНДСДохода = Новый СписокЗначений;
	СчетаНДСДохода.Добавить(ПланыСчетов.Хозрасчетный.НДС);
	СчетаНДСДохода.Добавить(ПланыСчетов.Хозрасчетный.ОтрицательныеКорректировкиНДС);
	СчетаНДСДохода.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоНДСотложенномуДляУплатыВБюджет);
	СчетаНДСДохода.Добавить(ПланыСчетов.Хозрасчетный.РасчетыНДСНалоговогоАгента);
	СчетаНДСДохода.Добавить(ПланыСчетов.Хозрасчетный.НДСНачисленныйПоОтгрузке);
	
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               |	ВзаимозависимыеЛицаСрезПоследних.Контрагент КАК Контрагент,
	               |	&ВидЗависимостиСтраны КАК Значение
	               |ПОМЕСТИТЬ втЗависимыеКонтрагенты
	               |ИЗ
	               |	РегистрСведений.ВзаимозависимыеЛица.СрезПоследних(&КонецПериода, Организация В (&Организация)) КАК ВзаимозависимыеЛицаСрезПоследних
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	ХозрасчетныйОбороты.СуммаОборот КАК РасходСумма,
	               |	ВЫБОР
	               |		КОГДА ХозрасчетныйОбороты.КорСчет В ИЕРАРХИИ (&Счет19)
	               |			ТОГДА ХозрасчетныйОбороты.СуммаОборот
	               |		ИНАЧЕ 0
	               |	КОНЕЦ КАК РасходСуммаНДС,
	               |	втЗависимыеКонтрагенты.Значение,
	               ...
	               |			&КонецПериода,
	               |			Регистратор,
	               |			Счет В ИЕРАРХИИ (&СписокСчетовРасход)
	               |				И НЕ Счет.Код = ""62.02"",
	               |			,
	               |			Организация В (&Организация)
	               |				И Субконто1 В
	               |					(ВЫБРАТЬ
	               |						втЗависимыеКонтрагенты.Контрагент
	               |					ИЗ
	               |						втЗависимыеКонтрагенты),
	               |			КорСчет В ИЕРАРХИИ (&СписокКорСчетовРасход)
	               |				И НЕ КорСчет В ИЕРАРХИИ (&СписокКорСчетовРасходИсключаемые),
	               |			) КАК ХозрасчетныйОбороты
	               |		ЛЕВОЕ СОЕДИНЕНИЕ втЗависимыеКонтрагенты КАК втЗависимыеКонтрагенты
	               |		ПО ХозрасчетныйОбороты.Субконто1 = втЗависимыеКонтрагенты.Контрагент
	               |ГДЕ
	               |	НЕ ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ПереоценкаВалютныхСредств
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	втРасходы.Договор,
	               |	СУММА(-втРасходы.РасходСумма) КАК РасходСумма,
	               |	СУММА(-втРасходы.РасходСуммаНДС) КАК РасходСуммаНДС,
	               |	втРасходы.Значение,
	               |	втРасходы.Счет,
	               |	втРасходы.Период,
	               ...
	               |	ХозрасчетныйОбороты.Субконто1 КАК Контрагент,
	               |	ХозрасчетныйОбороты.Субконто2 КАК Договор,
	               |	ХозрасчетныйОбороты.СуммаОборот КАК ДоходСумма,
	               |	втЗависимыеКонтрагенты.Значение,
	               |	ХозрасчетныйОбороты.Регистратор,
	               |	ХозрасчетныйОбороты.Счет,
	               |	ХозрасчетныйОбороты.Период,
	               ...
	               |			&КонецПериода,
	               |			Регистратор,
	               |			Счет В ИЕРАРХИИ (&СписокСчетовДоход)
	               |				И НЕ Счет В ИЕРАРХИИ (&СчетаНДСДохода)
	               |				И НЕ Счет.Код = ""76.11""
	               |				И НЕ Счет.Код = ""62.02""
	               |				И НЕ Счет.Код = ""76.02"",
	               |			,
	               ...
	               |					ИЗ
	               |						втЗависимыеКонтрагенты),
	               |			КорСчет В ИЕРАРХИИ (&СписокКорСчетовДоход)
	               |				И НЕ КорСчет В ИЕРАРХИИ (&СписокСчетовДоход)
	               |				И НЕ КорСчет.Код = ""76.02"",
	               |			) КАК ХозрасчетныйОбороты
	               |		ЛЕВОЕ СОЕДИНЕНИЕ втЗависимыеКонтрагенты КАК втЗависимыеКонтрагенты
	               |		ПО ХозрасчетныйОбороты.Субконто1 = втЗависимыеКонтрагенты.Контрагент
	               |ГДЕ
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	ХозрасчетныйОбороты.СуммаОборот КАК НДСОборот,
	               |	ХозрасчетныйОбороты.Регистратор,
	               |	ХозрасчетныйОбороты.Период
	               |ПОМЕСТИТЬ втНДСДоходы
	               |ИЗ
	               |	РегистрБухгалтерии.Хозрасчетный.Обороты(
	               |			&НачалоПериода,
	               |			&КонецПериода,
	               |			Регистратор,
	               |			Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Продажи_НДС)
	               |				ИЛИ Счет.Код = ""91.03"",
	               |			,
	               |			Организация В (&Организация),
	               |			КорСчет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДС),
	               |			) КАК ХозрасчетныйОбороты
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               ...
	               |	втДоходы.Контрагент,
	               |	втДоходы.Договор,
	               |	СУММА(втДоходы.ДоходСумма) КАК ДоходСумма,
	               |	СУММА(ЕСТЬNULL(втНДСДоходы.НДСОборот, 0)) КАК ДоходНДССумма,
	               |	втДоходы.Значение,
	               |	втДоходы.Счет,
	               |	втДоходы.Период,
	               ...
	               |ПОМЕСТИТЬ втДоходыНДС
	               |ИЗ
	               |	втДоходы КАК втДоходы
	               |		ЛЕВОЕ СОЕДИНЕНИЕ втНДСДоходы КАК втНДСДоходы
	               |		ПО втДоходы.Регистратор = втНДСДоходы.Регистратор
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	втДоходы.Значение,
	               |	втДоходы.Контрагент,
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	ЕСТЬNULL(втДоходыНДС.Значение, втРасходыНДС.Значение) КАК Раздел,
	               |	ЕСТЬNULL(втДоходыНДС.Контрагент, втРасходыНДС.Контрагент) КАК Контрагент,
	               |	ЕСТЬNULL(втДоходыНДС.Договор, втРасходыНДС.Договор) КАК ДоговорКонтрагента,
	               |	ЕСТЬNULL(втДоходыНДС.Счет, втРасходыНДС.Счет) КАК СчетУчетаРасчетов,
	               |	СУММА(ЕСТЬNULL(втДоходыНДС.ДоходСумма, 0)) КАК ДоходСумма,
	               |	СУММА(ЕСТЬNULL(втДоходыНДС.ДоходСумма, 0) - ЕСТЬNULL(втДоходыНДС.ДоходНДССумма, 0)) КАК ДоходСуммаБезНДС,
	               |	СУММА(ЕСТЬNULL(втРасходыНДС.РасходСумма, 0)) КАК РасходСумма,
	               |	СУММА(ЕСТЬNULL(втРасходыНДС.РасходСумма, 0) - ЕСТЬNULL(втРасходыНДС.РасходСуммаНДС, 0)) КАК РасходСуммаБезНДС,
	               |	ЕСТЬNULL(втРасходыНДС.РасходСумма, 0) - ЕСТЬNULL(втРасходыНДС.РасходСуммаНДС, 0) + (ЕСТЬNULL(втДоходыНДС.ДоходСумма, 0) - ЕСТЬNULL(втДоходыНДС.ДоходНДССумма, 0)) КАК ВсегоДоходовРасходов,
	               |	ЕСТЬNULL(втДоходыНДС.Договор.абс_ТипПредметаСделки, втРасходыНДС.Договор.абс_ТипПредметаСделки) КАК ТипПредметаСделки,
	               |	ЕСТЬNULL(втДоходыНДС.Договор.абс_НаименованиеПредметаСделки, втРасходыНДС.Договор.абс_НаименованиеПредметаСделки) КАК НаименованиеПредметаСделки,
	               |	ЕСТЬNULL(втДоходыНДС.Организация, втРасходыНДС.Организация) КАК Организация,
	               |	ВЫБОР
	               |		КОГДА втДоходыНДС.ДоходСумма > 0
	               ...
	               |	ЕСТЬNULL(втДоходыНДС.Договор.абс_КодСтраныПроисхожденияПредметаСделкиОКСМ, втРасходыНДС.Договор.абс_КодСтраныПроисхожденияПредметаСделкиОКСМ) КАК СтранаПроисхожденияПредметаСделки,
	               |	ЕСТЬNULL(втДоходыНДС.Валюта, втРасходыНДС.Валюта) КАК Валюта,
	               |	ЕСТЬNULL(втРасходыНДС.РасходСумма, 0) - ЕСТЬNULL(втРасходыНДС.РасходСуммаНДС, 0) + (ЕСТЬNULL(втДоходыНДС.ДоходСумма, 0) - ЕСТЬNULL(втДоходыНДС.ДоходНДССумма, 0)) КАК СуммаБезНДСВВалютеРасчетов,
	               |	ЕСТЬNULL(втРасходыНДС.РасходСумма, 0) - ЕСТЬNULL(втРасходыНДС.РасходСуммаНДС, 0) + (ЕСТЬNULL(втДоходыНДС.ДоходСумма, 0) - ЕСТЬNULL(втДоходыНДС.ДоходНДССумма, 0)) КАК СуммаБезНДСВРублях,
	               |	ЕСТЬNULL(втДоходыНДС.ДоходНДССумма, втРасходыНДС.РасходСуммаНДС) КАК СуммаНДСВВалютеРасчетов,
	               |	ЕСТЬNULL(втДоходыНДС.ДоходНДССумма, втРасходыНДС.РасходСуммаНДС) КАК СуммаНДСВРублях,
	               |	ЕСТЬNULL(втДоходыНДС.Период, втРасходыНДС.Период) КАК Период,
	               |	ЕСТЬNULL(втДоходыНДС.Регистратор, втРасходыНДС.Регистратор) КАК РасчетныйДокумент,
	               |	втДоходыНДС.ДоходСумма КАК ДоходСумма1,
	               |	втДоходыНДС.ДоходНДССумма,
	               |	втРасходыНДС.РасходСумма КАК РасходСумма1,
	               |	втРасходыНДС.РасходСуммаНДС
	               |ИЗ
	               |	втДоходыНДС КАК втДоходыНДС
	               |		ПОЛНОЕ СОЕДИНЕНИЕ втРасходыНДС КАК втРасходыНДС
	               |		ПО втДоходыНДС.Контрагент = втРасходыНДС.Контрагент
	               |			И втДоходыНДС.Договор = втРасходыНДС.Договор
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ЕСТЬNULL(втДоходыНДС.Значение, втРасходыНДС.Значение),
	               |	ЕСТЬNULL(втДоходыНДС.Контрагент, втРасходыНДС.Контрагент),
	               |	ЕСТЬNULL(втДоходыНДС.Договор, втРасходыНДС.Договор),
	               |	ЕСТЬNULL(втДоходыНДС.Счет, втРасходыНДС.Счет),
	               |	ЕСТЬNULL(втРасходыНДС.РасходСумма, 0) - ЕСТЬNULL(втРасходыНДС.РасходСуммаНДС, 0) + (ЕСТЬNULL(втДоходыНДС.ДоходСумма, 0) - ЕСТЬNULL(втДоходыНДС.ДоходНДССумма, 0)),
	               |	ЕСТЬNULL(втДоходыНДС.Договор.абс_ТипПредметаСделки, втРасходыНДС.Договор.абс_ТипПредметаСделки),
	               |	ЕСТЬNULL(втДоходыНДС.Договор.абс_НаименованиеПредметаСделки, втРасходыНДС.Договор.абс_НаименованиеПредметаСделки),
	               |	ЕСТЬNULL(втДоходыНДС.Организация, втРасходыНДС.Организация),
	               |	ВЫБОР
	               |		КОГДА втДоходыНДС.ДоходСумма > 0
	               |			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыКонтролируемыхСделок.ПолученДоход)
	               |		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыКонтролируемыхСделок.ОсуществленРасход)
	               |	КОНЕЦ,
	               |	ЕСТЬNULL(втДоходыНДС.Договор.абс_КодСтраныПроисхожденияПредметаСделкиОКСМ, втРасходыНДС.Договор.абс_КодСтраныПроисхожденияПредметаСделкиОКСМ),
	               |	ЕСТЬNULL(втДоходыНДС.Валюта, втРасходыНДС.Валюта),
	               |	ЕСТЬNULL(втДоходыНДС.ДоходНДССумма, втРасходыНДС.РасходСуммаНДС),
	               |	ЕСТЬNULL(втДоходыНДС.Период, втРасходыНДС.Период),
	               |	ЕСТЬNULL(втДоходыНДС.Регистратор, втРасходыНДС.Регистратор),
	               |	втДоходыНДС.ДоходСумма,
	               |	втДоходыНДС.ДоходНДССумма,
	               |	втРасходыНДС.РасходСумма,
	               |	втРасходыНДС.РасходСуммаНДС
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Контрагент";
	Запрос.УстановитьПараметр("Организация", ОбщийСписокОрганизаций);
	Запрос.УстановитьПараметр("НачалоПериода", НачалоГода(Уведомление.ОтчетныйГод));
	Запрос.УстановитьПараметр("КонецПериода", КонецГода(Уведомление.ОтчетныйГод));
	Запрос.УстановитьПараметр("Свойство", ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоКоду("00000000050"));
	Запрос.УстановитьПараметр("Значение", Справочники.ЗначенияПоказателейАнализовНоменклатуры.НайтиПоКоду("000005375"));
	Запрос.УстановитьПараметр("ВидЗависимостиСтраны", Справочники.ЗначенияСвойствОбъектов.НайтиПоКоду("000005375"));	   
	Запрос.УстановитьПараметр("СписокСчетовДоход", СписокСчетовДоход);
	Запрос.УстановитьПараметр("СписокСчетовРасход", СписокСчетовВзаиморасчетовРасходов);
	Запрос.УстановитьПараметр("СчетаНДСДохода",СчетаНДСДохода);
	Запрос.УстановитьПараметр("СписокКорСчетовДоход", СписокКорСчетовДоход);
	Запрос.УстановитьПараметр("СписокКорСчетовРасход", СписокКорСчетовРасход);
	Запрос.УстановитьПараметр("СписокКорСчетовРасходИсключаемые", СписокКорСчетовРасходИсключаемые);
	Запрос.УстановитьПараметр("Счет19", ПланыСчетов.Хозрасчетный.НДСпоПриобретеннымЦенностям);
	Запрос.УстановитьПараметр("Счет58", ПланыСчетов.Хозрасчетный.ПаиИАкции);
	Запрос.УстановитьПараметр("Счет76", ПланыСчетов.Хозрасчетный.ПрочиеРасчетыСРазнымиДебиторамиИКредиторами);
	Запрос.УстановитьПараметр("СписокСтранОффшоров", СписокСтранОффшоров);
	   
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		НС = ТаблицаСделок.Добавить();	
...
Процедура СформироватьКонтролируемыеСделкиУведомления(Уведомление,СписокФилиалов) Экспорт
	//Вставка 231213 Родин Трансфертное ценообразование
	НовыйАлгоритм = глЗначениеПеременной("абс_ТЦОЗаполнениеПоНовомуАлгоритму");
	//Вставка 231213 Родин Трансфертное ценообразование
	...
	Запрос = Новый Запрос();
	Запрос.МенеджерВременныхТаблиц = ПолучитьВременныеТаблицыДляЗаполненияУведомления(Уведомление,ОбщийСписокОрганизаций);
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|	КонтролируемыеСделки.ДоговорКонтрагента.абс_КодПредметаСделкиТНВЭД.Код КАК КодТНВЭД,
	// {{ Киселева #77317447 
	| КонтролируемыеСделки.ДоговорКонтрагента.абс_КодПредметаСделкиОКП.Код КАК КодОКП, " +
		?(ПараметрыУведомления.ОтчетныйГод < Дата(2017,1,1), " КонтролируемыеСделки.ДоговорКонтрагента.абс_КодПредметаСделкиОКВЭД.Код"," КонтролируемыеСделки.ДоговорКонтрагента.ttk_КодПредметаСделкиОКВЭД2.КодКлассификатора") + " КАК КодОКВЭД,
	//| КонтролируемыеСделки.ДоговорКонтрагента.абс_КодПредметаСделкиОКВЭД.Код КАК КодОКВЭД,
	// }} Киселева #77317447 
	//|	КонтролируемыеСделки.ДоговорКонтрагента.абс_КодПредметаСделкиОКВЭД.Код КАК КодОКВЭД,
	|	КонтролируемыеСделки.ПредметСделки КАК ПредметСделки,
	|	КонтролируемыеСделки.ДоговорКонтрагента.абс_НаименованиеПредметаСделки КАК НаименованиеПредметаСделки,
	|	ВЫБОР
	|		КОГДА КонтролируемыеСделки.ДоговорКонтрагента.абс_КодОснованияОтнесенияСделкиККонтролируемой.Код = ""121""
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Строка121СтороныВзаимозависимыПоКодексу,
	|	ВЫБОР
	|		КОГДА КонтролируемыеСделки.ДоговорКонтрагента.абс_КодОснованияОтнесенияСделкиККонтролируемой.Код = ""122""
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Строка122СделкаВОбластиВнешнейТорговли,
	|	ВЫБОР
	|		КОГДА КонтролируемыеСделки.ДоговорКонтрагента.абс_КодОснованияОтнесенияСделкиККонтролируемой.Код = ""123""
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Строка123СделкаСКонтрагентомСЛьготнымНалогообложением,
	|	ВЫБОР
	|		КОГДА КонтролируемыеСделки.ДоговорКонтрагента.абс_КодОснованияОтнесенияСделкиККонтролируемой.Код = ""124""
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Строка124СделкаСНезависимымПосредником,
	|	ВЫБОР
	|		КОГДА КонтролируемыеСделки.ДоговорКонтрагента.абс_ОсобенностиОтнесенияСделкиККонтролируемой.Код = ""131""
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Строка131СуммаДоходовПоСделкамПревышаетПредел,
	|	ВЫБОР
	|		КОГДА КонтролируемыеСделки.ДоговорКонтрагента.абс_ОсобенностиОтнесенияСделкиККонтролируемой.Код = ""132""
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Строка132СделкаСПлательщикомНДПИ,
	|	ВЫБОР
	|		КОГДА КонтролируемыеСделки.ДоговорКонтрагента.абс_ОсобенностиОтнесенияСделкиККонтролируемой.Код = ""133""
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|	ВЫБОР
	|		КОГДА КонтролируемыеСделки.ДоговорКонтрагента.абс_ОсобенностиОтнесенияСделкиККонтролируемой.Код = ""134""
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Строка134СделкаСПлательщикомНалогаНаПрибыль,
	|	ВЫБОР
	|		КОГДА КонтролируемыеСделки.ДоговорКонтрагента.абс_ОсобенностиОтнесенияСделкиККонтролируемой.Код = ""135""
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Строка135СделкаСРезидентомОЭЗ,
	|	КонтролируемыеСделки.ДоговорКонтрагента.абс_КодНаименованияСделки.Код КАК КодНаименованияСделки,
	|	КонтролируемыеСделки.ДоговорКонтрагента.абс_СпособОпределенияЦенКонтролируемыхСделок КАК СпособОпределенияЦеныСделки,
	|	ЕСТЬNULL(ДоговорыУчастниковКонтролируемыхСделок.КодУсловийПоставки, """") КАК КодУсловийПоставки,
	|	КонтролируемыеСделки.ДоговорКонтрагента.абс_ТипПредметаСделки КАК ТипПредметаСделки,
	...
	|	КонтролируемыеСделки.ДоговорКонтрагента.абс_КодСтраныПроисхожденияПредметаСделкиОКСМ КАК СтранаПроисхожденияПредметаСделки,
	|	КонтролируемыеСделки.ПрименяемаяМетодикаДляТрансфертногоЦенообразования,
	|	КонтролируемыеСделки.ДоговорКонтрагента.абс_ОтноситсяКГруппеОднородныхСделок КАК ОтноситсяКГруппеОднородныхСделок
	|ИЗ
	...
	Если  Не НовыйАлгоритм  Тогда
		...
		|	ТипСделки,
		|	ОтноситсяКГруппеОднородныхСделок";
	Иначе
		Запрос.Текст = Запрос.Текст + 
		"
		|	ПрименяемаяМетодикаДляТрансфертногоЦенообразования,
		|   ОтноситсяКГруппеОднородныхСделок,
		|	Контрагент,
		|	ДоговорКонтрагента УБЫВ";
	КонецЕСли;
	Выборка = Запрос.Выполнить().Выбрать();
	
	ДопКлючСделки = Новый Структура("Контрагент, ДоговорКонтрагента");
	
	Если НовыйАлгоритм Тогда
		КлючСделки = Новый Структура("ПрименяемаяМетодикаДляТрансфертногоЦенообразования,ОтноситсяКГруппеОднородныхСделок");
	Иначе
		КлючСделки = Новый Структура("Организация,Контрагент, ДоговорКонтрагента, ПредметСделки");
	КонецЕсли;
	//Вставка 231213 Родин Трансфертное ценообразование
	...
	Пока Выборка.Следующий() Цикл
		Если ДокументСделки = Неопределено ИЛИ
			НЕ КлючСделкиСовпадает(Выборка, КлючСделки) или (Не Выборка.ОтноситсяКГруппеОднородныхСделок и НовыйАлгоритм и НЕ КлючСделкиСовпадает(Выборка, ДопКлючСделки)) Тогда
			
			ЗаполнитьЗначенияСвойств(КлючСделки, Выборка);
			ЗаполнитьЗначенияСвойств(ДопКлючСделки, Выборка);
			...
			Если ДокументСделки <> Неопределено Тогда
				...
				Если ДокументСделки.Организация.абс_СтатусОрганизации <> Перечисления.абс_СтатусОрганизации.ГоловнаяОрганизация Тогда
					ДокументСделки.ОрганизацияФилиал = "";
					ДокументСделки.Организация = Справочники.Организации.НайтиПоНаименованию("КТТК");
				КонецЕсли;
				
				Если  ДокументСделки.ДоговорКонтрагента.абс_ОтноситсяКГруппеОднородныхСделок Тогда
					ДокументСделки.ДоговорКонтрагента = "";
					ДокументСделки.Контрагент         = "";
					ДокументСделки.ГруппаОднородныхСделок = Истина;
				КонецЕсли;
			...
			КонецЕсли;
			//Вставка 231213 Родин Трансфертное ценообразование
			//Отбор = Новый Структура("Организация,Контрагент, ДоговорКонтрагента, ПредметСделки, Используется",
			...
			ДокументСделки.КодСтороныСделки =  ДокументСделки.ДоговорКонтрагента.абс_КодСтороныСделки.Код;
	...
	Если ДокументСделки <> Неопределено Тогда
		...
		Если ДокументСделки.Организация.абс_СтатусОрганизации <> Перечисления.абс_СтатусОрганизации.ГоловнаяОрганизация Тогда
			ДокументСделки.ОрганизацияФилиал = Выборка.Организация;
			ДокументСделки.Организация = Справочники.Организации.НайтиПоНаименованию("КТТК");
		КонецЕсли;
...
Функция ПолучитьВременныеТаблицыДляЗаполненияУведомления(Уведомление,ОбщийСписокОрганизаций) Экспорт
	...
	Запрос = Новый Запрос();
	Запрос.МенеджерВременныхТаблиц = ПолучитьВременныеТаблицыДляЗаполненияСделок(Уведомление,ОбщийСписокОрганизаций);
	Запрос.Параметры.Вставить("Уведомление", Уведомление);
	Запрос.Параметры.Вставить("Организация", ПараметрыУведомления.Организация);
	Запрос.Параметры.Вставить("ОбщийСписокОрганизаций", ОбщийСписокОрганизаций);
	Запрос.Параметры.Вставить("ОтчетныйГод", ПараметрыУведомления.ОтчетныйГод);
	...
	|	КонтролируемыеСделкиОрганизаций.ДоговорКонтрагента.абс_ПрименяемаяМетодикаДляТрансфертногоЦенообразования КАК ПрименяемаяМетодикаДляТрансфертногоЦенообразования
	...
	|	И КонтролируемыеСделкиОрганизаций.Организация В(&ОбщийСписокОрганизаций)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Контрагент,
	|	ДоговорКонтрагента,
...
Функция ПолучитьТекстЗапросаПоКонтролируемымСделкам() Экспорт
	...
	|	КонтролируемаяСделкаСделки.ДоговорКонтрагента.Владелец КАК Контрагент,
	|	КонтролируемаяСделкаСделки.ДоговорКонтрагента.Номер КАК Строка060НомерДоговора,
	|	КонтролируемаяСделкаСделки.ДоговорКонтрагента.Дата КАК Строка065ДатаДоговора,
	...
	|	КонтролируемаяСделкаСделки.ДоговорКонтрагента.Владелец.ЮрФизЛицо КАК ТипКонтрагента
	|ПОМЕСТИТЬ Листы1Б
	|ИЗ
...
Функция ПолучитьОсновныеСведенияУведомленияДляВыгрузки(Уведомление) Экспорт
	...
	РеквизитыУведомления = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Уведомление, 
		"Организация, НомерКорректировки, КодМестаПредставления, ОтчетныйГод, КодФормыРеорганизации, ИННРеорганизованнойОрганизации, КППРеорганизованнойОрганизации");
	...
	ОсновныеСведения.Вставить("ОКАТО", ПараметрыОрганизации.КодПоОКАТО);
	...
	Если ОрганизацияФизЛицо Тогда
		...
		|	ЕСТЬNULL(ГражданствоФизЛица.Страна, ЗНАЧЕНИЕ(Перечисление.СтатусыНалогоплательщиковПоНДФЛ.Резидент)) КАК СтатусНалогоплательщикаПоНДФЛ,
		...
		|	АдресПоПрописке.ТипДома,
		|	АдресПоПрописке.ТипКорпуса,
		|	АдресПоПрописке.ТипКвартиры
		|ИЗ
		...
		Если ДанныеФизЛица.Следующий() Тогда
			...
			Если ДанныеФизЛица.Страна = Справочники.КлассификаторСтранМира.Россия
				ИЛИ НЕ ЗначениеЗаполнено(ДанныеФизЛица.Страна) Тогда
...
Процедура ЗаполнитьТаблицуСделокЛиста1А(Лист1А, ТаблицаСделок, СписокПолейСделки, СтруктураКлючевыхПолей)
	...
	Для Каждого Сделка Из ТаблицаСделок Цикл    
		Если (Сделка.Количество <> 0) ИЛИ (Сделка.Стоимость <> 0) Тогда
			...
			//АБС ВСТАВКА  170314 Родин
			СделкаДокумента.КодОКВЭД = СделкаДокумента.ДоговорКонтрагента.абс_КодПредметаСделкиОКВЭД;
			СделкаДокумента.Контрагент = СделкаДокумента.ДоговорКонтрагента.Владелец;
			//АБС ВСТАВКА  170314 Родин
			
			СделкаДокумента.ЕдиницаИзмерения = Справочники.КлассификаторЕдиницИзмерения.НайтиПоКоду("796");
...
Функция ПолучитьВременныеТаблицыДляЗаполненияСделок(Уведомление,ОбщийСписокОрганизаций)
	...
	Запрос.УстановитьПараметр("СписокОрганизаций", ОбщийСписокОрганизаций);
...
Процедура ЗаполнитьРублевыеСуммыСделок(РублевыеСуммыСделок, СделкиДокумента)
	...
	Для Каждого ДокументСделки Из РублевыеСуммыСделок Цикл
		...
		Если СтрокиДокумента.Количество() > 0 Тогда
			...
			РаспределениеСуммВсего = ttk_ОбщегоНазначения.РаспределитьПропорционально(ВсегоВРублях, СуммыВсегоВВалюте);
			РаспределениеСуммНДС   = ttk_ОбщегоНазначения.РаспределитьПропорционально(СуммаНДСВРублях, СуммыНДСВВалюте);
...
Процедура ДобавитьСделкиРеализацииОтгруженныхТоваров(Уведомление, МенеджерВременныхТаблиц, ТаблицаСделок);
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(, , Регистратор, , , , , ) КАК ХозрасчетныйОбороты
	|		ПО РеализацияОтгруженныхТоваров.Ссылка = ХозрасчетныйОбороты.Регистратор
	|ГДЕ
...
Процедура ДобавитьСделкиПоступленияТоваровУслуг(Уведомление, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(, , Регистратор, , , , , ) КАК ХозрасчетныйОбороты
	|		ПО ПоступлениеТоваровУслуг.Ссылка = ХозрасчетныйОбороты.Регистратор
	|ГДЕ
...
Процедура ДобавитьСделкиПоступленияДопРасходов(Уведомление, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(, , Регистратор, , , , , ) КАК ХозрасчетныйОбороты
	|		ПО ПоступлениеДопРасходов.Ссылка = ХозрасчетныйОбороты.Регистратор
	|ГДЕ
...
Процедура ДобавитьСделкиРеализацииУслугПоПереработке(Уведомление, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(, , Регистратор, , , , , ) КАК ХозрасчетныйОбороты
	|		ПО РеализацияУслугПоПереработке.Ссылка = ХозрасчетныйОбороты.Регистратор
	|ГДЕ
...
Процедура ДобавитьСделкиАктаОбОказанииУслуг(Уведомление, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(, , Регистратор, , , , , ) КАК ХозрасчетныйОбороты
	|		ПО Акт.Ссылка = ХозрасчетныйОбороты.Регистратор
	|ГДЕ
...
Процедура ДобавитьСделкиПоступленияНМА(Уведомление, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(, , Регистратор, , , , , ) КАК ХозрасчетныйОбороты
	|		ПО ПоступлениеНМА.Ссылка = ХозрасчетныйОбороты.Регистратор
	|ГДЕ
...
Процедура ДобавитьСделкиПередачиОС(Уведомление, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(, , Регистратор, , , , , ) КАК ХозрасчетныйОбороты
	|		ПО ПередачаОС.Ссылка = ХозрасчетныйОбороты.Регистратор
	|ГДЕ
...
Процедура ДобавитьСделкиПередачиНМА(Уведомление, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(, , Регистратор, , , , , ) КАК ХозрасчетныйОбороты
	|		ПО ПередачаНМА.Ссылка = ХозрасчетныйОбороты.Регистратор
	|ГДЕ
...
Процедура ДобавитьСделкиОтчетаКомитентуОПродажах(Уведомление, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(, , Регистратор, , , , , ) КАК ХозрасчетныйОбороты
	|		ПО ОтчетКомитентуОПродажах.Ссылка = ХозрасчетныйОбороты.Регистратор
	|ГДЕ
...
Процедура ДобавитьСделкиОтчетаКомиссионераОПродажах(Уведомление, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(, , Регистратор, , , , , ) КАК ХозрасчетныйОбороты
	|		ПО ОтчетКомиссионераОПродажах.Ссылка = ХозрасчетныйОбороты.Регистратор
	|ГДЕ
...
Процедура ДобавитьСделкиВозвратаТоваровПоставщику(Уведомление, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(, , Регистратор, , , , , ) КАК ХозрасчетныйОбороты
	|		ПО ВозвратТоваровПоставщикуТовары.Ссылка = ХозрасчетныйОбороты.Регистратор
	|ГДЕ
