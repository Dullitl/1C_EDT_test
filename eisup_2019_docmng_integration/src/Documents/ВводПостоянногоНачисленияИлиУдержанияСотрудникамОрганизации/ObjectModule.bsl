...
Функция СформироватьЗапросПоНачисления(ВыборкаПоШапкеДокумента)
	...
	Если Плановые = "ПлановыеНачисленияРаботниковОрганизаций" Тогда
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений."+Плановые+" КАК СуществующиеДвижения
	...
	|			И ВводПостоянногоНачисленияИлиУдержанияСотрудникамОрганизацииОсновныеНачисления.Ссылка.Организация = СуществующиеДвижения.Организация
	|			И ВводПостоянногоНачисленияИлиУдержанияСотрудникамОрганизацииОсновныеНачисления.Сотрудник"+РаботникСоединения+" = СуществующиеДвижения."+Работник+"
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВводПостоянногоНачисленияИлиУдержанияСотрудникамОрганизации.ОсновныеНачисления КАК ПовторяющиесяСтроки
	|		ПО ВводПостоянногоНачисленияИлиУдержанияСотрудникамОрганизацииОсновныеНачисления.Ссылка = ПовторяющиесяСтроки.Ссылка
	|			И ВводПостоянногоНачисленияИлиУдержанияСотрудникамОрганизацииОсновныеНачисления.НомерСтроки < ПовторяющиесяСтроки.НомерСтроки
	|			И ВводПостоянногоНачисленияИлиУдержанияСотрудникамОрганизацииОсновныеНачисления.Сотрудник = ПовторяющиесяСтроки.Сотрудник
	...
	|			ИЗ
	|				Документ.ВводПостоянногоНачисленияИлиУдержанияСотрудникамОрганизации.ОсновныеНачисления КАК ТЧРаботники
	|					ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений."+Плановые+" КАК НачисленияРаботников
	|					ПО ТЧРаботники.Сотрудник"+РаботникСоединения+" = НачисленияРаботников."+ Работник + "
	|						И (НачисленияРаботников.Период < &ДатаДействия)
	|						И (НачисленияРаботников.Организация < ТЧРаботники.Ссылка.Организация)
	|			ГДЕ
	...
	|				ПО ДатыПоследнихДвиженийНачисленийРаботников.Сотрудник"+РаботникСоединения+" = ПлановыеНачисленияРаботников."+ Работник + "
	...
	|		ПО ВводПостоянногоНачисленияИлиУдержанияСотрудникамОрганизацииОсновныеНачисления.Сотрудник = НачисленияДоНазначения.Сотрудник
	|			И ВводПостоянногоНачисленияИлиУдержанияСотрудникамОрганизацииОсновныеНачисления.Ссылка.Организация = НачисленияДоНазначения.Организация
...
//  Отказ						- флаг отказа в проведении.
//
Процедура ПроверитьЗаполнениеСтрокиНачисления(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок, ПроверкаШтатногоРасписания, СоответствиеВалютныеСпособыРасчета)
	...
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.КонфликтнаяСтрокаНомер) Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке("Один и тот же отрудник " + ВыборкаПоСтрокамДокумента.СотрудникНаименование + " введен дважды (см. строку " + ВыборкаПоСтрокамДокумента.КонфликтнаяСтрокаНомер + ")!", Отказ, Заголовок); 
	КонецЕсли;
	...
	Если ВыборкаПоШапкеДокумента.Следующий() Тогда
		...
		Если НЕ Отказ Тогда
			...
			Пока ВыборкаПоНачислениям.Следующий() Цикл
				...
				ПроверитьЗаполнениеСтрокиНачисления(ВыборкаПоШапкеДокумента, ВыборкаПоНачислениям, Отказ, Заголовок, ПроверкаШтатногоРасписания, СоответствиеВалютныеСпособыРасчета);
...
мСписокРасчетовПоИсполнительнымЛистам = Новый Массив;
