...
Функция ПечатьАктаСверкиТМЦ()
	
	ТабДок = Новый ТабличныйДокумент;
	ТабДок.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_абс_АктСверкиТМЦ_АктСверки";
	...
	//
	Если НЕ РежимПоПереданным тогда
	...
	ТекстЗаголовка = "остатков ТМЦ на дату " +  Формат(ДатаОкончания, "ДФ=dd.MM.yyyy") + Символы.ПС
					+ "между " + НазваниеОрганизации + Символы.ПС + "и " + НаименованиеКонтрагента;
	Если ЗначениеЗаполнено(ДоговорКонтрагента) и РежимПоПереданным Тогда
	    ТекстЗаголовка = ТекстЗаголовка + Символы.ПС + "по договору " + СокрЛП(ДоговорКонтрагента.Наименование);
	КонецЕсли;
	
	//АБС+ по заявке №4582
	//Если РежимПоПереданным тогда
	//	ТекстЗаголовка = ТекстЗаголовка + Символы.ПС + "с назначением использования: "+НазначениеИспользования.Наименование;
	//КонецЕсли;
	//АБС-
	...
	ФИОПредставителя = "" + ОбщегоНазначения.ФамилияИнициалыФизЛица(СокрЛП(ПредставительОрганизации));
	
	ФИОПредставителяКонтрагента = "" + ОбщегоНазначения.ФамилияИнициалыФизЛица(СокрЛП(ПредставительКонтрагента));
	...
	СтрЗаголовокТаблица = "Мы, нижеподписавшиеся, "
				...
				+ "составили настоящий акт сверки в том, что состояние остатков ТМЦ по данным учета следующее:";
	...
	Для Инд = 0 По МаксИндекс Цикл
		ОбластьОбороты.Параметры.НомерСтроки = Инд+1;
		...
		Если Инд < ПоДаннымОрганизации.Количество() Тогда
			...
			ОбластьОбороты.Параметры.Номенклатура		= СтрокаОрганизации.Номенклатура;
			ОбластьОбороты.Параметры.Назначение         = СтрокаОрганизации.абс_НазначениеИспользования;
			ОбластьОбороты.Параметры.КодНоменклатуры    = СтрокаОрганизации.Номенклатура.Код;
			ОбластьОбороты.Параметры.Счет				= СтрокаОрганизации.Счет;
			ОбластьОбороты.Параметры.ЕдИзм				= СтрокаОрганизации.Номенклатура.ЕдиницаХраненияОстатков.Наименование;
			ОбластьОбороты.Параметры.Количество		 	= СтрокаОрганизации.Количество;
			ОбластьОбороты.Параметры.Сумма		 		= СтрокаОрганизации.Сумма;
		...
		Если Инд < ПоДаннымКонтрагента.Количество() Тогда
			...
			ОбластьОбороты.Параметры.КоличествоКонтрагент	= СтрокаОрганизации.Количество;
			ОбластьОбороты.Параметры.СуммаКонтрагент		= СтрокаОрганизации.Сумма;
		КонецЕсли;
	...
	ОбластьКонОстатки.Параметры.ИтогКоличество =ПоДаннымОрганизации.Итог("Количество"); 
	ОбластьКонОстатки.Параметры.ИтогСумма = ПоДаннымОрганизации.Итог("Сумма");
	ОбластьКонОстатки.Параметры.ИтогКоличествоКонтрагент = ПоДаннымКонтрагента.Итог("Количество");
	ОбластьКонОстатки.Параметры.ИтогСуммаКонтрагент = ПоДаннымКонтрагента.Итог("Сумма"); 
	...
	ОбластьПодвал.Параметры.Должность = ?(ДолжностьПредставителяОрганизации<>"",ДолжностьПредставителяОрганизации,"_______________________");
...
Функция ПечатьАктаСверкиТМЦПоДоговорам()
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	ПоДаннымОрганизации.Номенклатура КАК Номенклатура,
	               |	ПоДаннымОрганизации.абс_НазначениеИспользования КАК Назначение,
	               |	ПоДаннымОрганизации.Номенклатура.Код КАК КодНоменклатуры,
	               |	ПоДаннымОрганизации.Счет КАК Счет,
	               |	ПоДаннымОрганизации.Номенклатура.ЕдиницаХраненияОстатков.Наименование КАК ЕдИзм,
	               |	ПоДаннымОрганизации.Количество,
	               |	ПоДаннымОрганизации.Сумма,
	               |	0 КАК КоличествоКонтрагент,
	               |	0 КАК СуммаКонтрагент,
	               |	ПоДаннымОрганизации.ДоговорКонтрагента КАК ДоговорКонтрагента
	               |ПОМЕСТИТЬ вр
	               |ИЗ
	               |	Документ.абс_АктСверкиТМЦ.ПоДаннымОрганизации КАК ПоДаннымОрганизации
	               |ГДЕ
	               |	ПоДаннымОрганизации.Ссылка = &Ссылка
	               |
	               |ОБЪЕДИНИТЬ ВСЕ
	               |
	               |ВЫБРАТЬ
	               |	ПоДаннымКонтрагента.Номенклатура,
	               |	ЗНАЧЕНИЕ(Справочник.абс_НазначенияИспользования.ПустаяСсылка),
	               |	ПоДаннымКонтрагента.Номенклатура.Код,
	               |	ПоДаннымКонтрагента.Счет,
	               |	ПоДаннымКонтрагента.Номенклатура.ЕдиницаХраненияОстатков.Наименование,
	               |	0,
	               |	0,
	               |	ПоДаннымКонтрагента.Количество,
	               |	ПоДаннымКонтрагента.Сумма,
	               |	ПоДаннымКонтрагента.ДоговорКонтрагента
	               |ИЗ
	               |	Документ.абс_АктСверкиТМЦ.ПоДаннымКонтрагента КАК ПоДаннымКонтрагента
	               |ГДЕ
	               |	ПоДаннымКонтрагента.Ссылка = &Ссылка
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	ДоговорКонтрагента,
	               |	Номенклатура,
	               ...
	               |	вр.Счет КАК Счет,
	               |	вр.ЕдИзм,
	               |	СУММА(вр.Количество) КАК Количество,
	               |	СУММА(вр.Сумма) КАК Сумма,
	               |	СУММА(вр.КоличествоКонтрагент) КАК КоличествоКонтрагент,
	               |	СУММА(вр.СуммаКонтрагент) КАК СуммаКонтрагент,
	               |	вр.ДоговорКонтрагента КАК ДоговорКонтрагента
	               |ИЗ
	               |	вр КАК вр
	               |
	               |СГРУППИРОВАТЬ ПО
	...
	Результат = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам,"ДоговорКонтрагента");
	НомерСтроки = 0;	
	Пока Результат.Следующий() Цикл
		ОбработкаПрерыванияПользователя();
		НомерСтроки = НомерСтроки + 1;
		ОбластьОбороты.Параметры.НомерСтроки = НомерСтроки;
		ОбластьОбороты.Параметры.Заполнить(Результат);
		ОбластьОбороты.Параметры.Номенклатура = Результат.ДоговорКонтрагента;
		ОбластьОбороты.Параметры.Назначение = "    Х    ";
		ОбластьОбороты.Параметры.ЕдИзм = "    Х    ";
		Для н = 2 по 11 Цикл
			ОбластьОбороты.Область(1, н, 1, н).Шрифт = Макет.ПолучитьОбласть("Цвет0").Область(1, 1, 1, 1).Шрифт;	
		КонецЦикла;	
		
		ТабДок.Вывести(ОбластьОбороты);
		Рез = Результат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам,,"Все");
		Пока Рез.Следующий() Цикл
			ОбработкаПрерыванияПользователя();
			НомерСтроки = НомерСтроки + 1;
			ОбластьОбороты.Параметры.НомерСтроки = НомерСтроки;
			ОбластьОбороты.Параметры.Заполнить(Рез);
			Для н = 2 по 11 Цикл
				ОбластьОбороты.Область(1, н, 1, н).Шрифт = Макет.ПолучитьОбласть("Цвет1").Область(1, 1, 1, 1).Шрифт;	
			КонецЦикла;
			ТабДок.Вывести(ОбластьОбороты);
		КонецЦикла;
	КонецЦикла;	
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ИмяМакета = "АктСверки" Тогда
		...
		ТабДокумент = ПечатьАктаСверкиТМЦ();
		
	ИначеЕсли ИмяМакета = "АктСверки1" Тогда	
		
		ТабДокумент = ПечатьАктаСверкиТМЦПоДоговорам();
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("АктСверки,АктСверки1","Акт сверки ТМЦ","Акт сверки ТМЦ по договорам");
...
Процедура ЗаполнитьПоДаннымБухгалтерскогоУчета(ФильтрСписокСчетов = неопределено) Экспорт
	Запрос = Новый Запрос;     
	Дата = ?(НЕ ЗначениеЗаполнено(ДатаОкончания), Неопределено, КонецДня(ДатаОкончания)+1);
	...
	Если ЗначениеЗаполнено(НазначениеИспользования) Тогда
		Запрос.УстановитьПараметр("абс_НазначениеПлатежа", НазначениеИспользования);
	КонецЕсли;	
	Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
		Запрос.УстановитьПараметр("ДоговорКонтрагента", ДоговорКонтрагента);
	Иначе	
	...
	Если РежимПоПереданным тогда  
		...
		Запрос.Текст = "ВЫБРАТЬ
		               |	ПартииТоваровПереданныеБухгалтерскийУчетОстатки.Номенклатура КАК Номенклатура,
					   |	ПартииТоваровПереданныеБухгалтерскийУчетОстатки.абс_НазначениеПлатежа КАК абс_НазначениеИспользования,
					   //абс вставка №40658 начало
					   |	ПартииТоваровПереданныеБухгалтерскийУчетОстатки.ХарактеристикаНоменклатуры КАК ХарактеристикаНоменклатуры,
					   |	ПартииТоваровПереданныеБухгалтерскийУчетОстатки.СерияНоменклатуры КАК СерияНоменклатуры,
					   //\\абс вставка №40658 конец
		               |	СУММА(ПартииТоваровПереданныеБухгалтерскийУчетОстатки.КоличествоОстаток) КАК Количество,
		               |	СУММА(ПартииТоваровПереданныеБухгалтерскийУчетОстатки.СтоимостьОстаток) КАК Сумма,
		               |	ПартииТоваровПереданныеБухгалтерскийУчетОстатки.СчетУчета КАК Счет
		               |ИЗ
					   ...
					   Иначе
					   Запрос.Текст = Запрос.Текст + "
		               |			ДоговорКонтрагента.Владелец = &Контрагент И ";					   
				   	   КонецЕсли;
				       Запрос.Текст = Запрос.Текст + "
		               |				Организация = &Организация";
					   Если ЗначениеЗаполнено(НазначениеИспользования) Тогда
                       Запрос.Текст = Запрос.Текст + " 
		               |				И абс_НазначениеПлатежа = &абс_НазначениеПлатежа";
					   КонецЕсли;
					   Запрос.Текст = Запрос.Текст + "
		               |				И СчетУчета В ИЕРАРХИИ (&ФильтрСписокСчетов)) КАК ПартииТоваровПереданныеБухгалтерскийУчетОстатки
		               |
		               |СГРУППИРОВАТЬ ПО
		               |	ПартииТоваровПереданныеБухгалтерскийУчетОстатки.Номенклатура,
					   |	ПартииТоваровПереданныеБухгалтерскийУчетОстатки.абс_НазначениеПлатежа,
					   //абс вставка №40658 начало
	...
	Иначе
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	ПартииТоваровНаСкладахБухгалтерскийУчетОстатки.СчетУчета КАК Счет
		               |ИЗ
		               |	РегистрНакопления.ПартииТоваровНаСкладахБухгалтерскийУчет.Остатки(
		               |			&ДатаКон,
		               |			Организация = &Организация
		               |				И СчетУчета В ИЕРАРХИИ (&ФильтрСписокСчетов)
	...
	//абс вставка №40658 начало
	СтруктураЗаполнения = ПолучитьФорму("Форма").ОткрытьМодально();
	Если не СтруктураЗаполнения.ХарактеристикаНоменклатуры и не СтруктураЗаполнения.СерияНоменклатуры Тогда
		ВремТаб.Свернуть("Номенклатура,Счет"+?(ВремТаб.Колонки.Найти("абс_НазначениеИспользования")=Неопределено,"",",абс_НазначениеИспользования"),"Количество,Сумма");	
		ВремТаб.Сортировать("Счет,Номенклатура");
	ИначеЕсли СтруктураЗаполнения.ХарактеристикаНоменклатуры и не СтруктураЗаполнения.СерияНоменклатуры Тогда
		ВремТаб.Свернуть("Номенклатура,Счет,ХарактеристикаНоменклатуры"+?(ВремТаб.Колонки.Найти("абс_НазначениеИспользования")=Неопределено,"",",абс_НазначениеИспользования"),"Количество,Сумма");	
		ВремТаб.Сортировать("Счет,Номенклатура,ХарактеристикаНоменклатуры");	
	ИначеЕсли не СтруктураЗаполнения.ХарактеристикаНоменклатуры и СтруктураЗаполнения.СерияНоменклатуры Тогда
		ВремТаб.Свернуть("Номенклатура,Счет,СерияНоменклатуры"+?(ВремТаб.Колонки.Найти("абс_НазначениеИспользования")=Неопределено,"",",абс_НазначениеИспользования"),"Количество,Сумма");	
		ВремТаб.Сортировать("Счет,Номенклатура,СерияНоменклатуры");		
	КонецЕсли;	
	//\\абс вставка №40658 конец 	
	
	ПоДаннымОрганизации.Очистить();
	ПоДаннымОрганизации.Загрузить(ВремТаб);
	
КонецПроцедуры

Процедура ЗаполнитьПоДаннымБухгалтерскогоУчета3(ФильтрСписокСчетов = неопределено) Экспорт
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ДатаКон", ?(НЕ ЗначениеЗаполнено(ДатаОкончания), Неопределено, КонецДня(ДатаОкончания)));
	Запрос.УстановитьПараметр("Организация",   Организация);
	Запрос.УстановитьПараметр("Валюта",        ?(НЕ ЗначениеЗаполнено(ВалютаДокумента) или (ВалютаДокумента = мВалютаРегламентированногоУчета), неопределено,ВалютаДокумента));
	Запрос.УстановитьПараметр("ФильтрСписокСчетов", ФильтрСписокСчетов);
	...
	АналитикаСчетов = новый Массив();
				   
	Если РежимПоПереданным тогда 
		АналитикаСчетов.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Номенклатура);
		АналитикаСчетов.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты);
		АналитикаСчетов.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры);
		Запрос.УстановитьПараметр("Субконто2",Контрагент);
		Запрос.УстановитьПараметр("Субконто3",ДоговорКонтрагента);
	Иначе
		АналитикаСчетов.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Номенклатура);
		АналитикаСчетов.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Склады);
		Запрос.УстановитьПараметр("Субконто2",Склад);
        Запрос.Текст = СтрЗаменить(Запрос.Текст,"Субконто3 = &Субконто3","ИСТИНА");
	КонецЕсли;
	
	Запрос.УстановитьПараметр("АналитикаСчетов",    АналитикаСчетов);
...
Процедура ЗаполнитьПоДаннымБухгалтерскогоУчета2(ФильтрСписокСчетов = неопределено) Экспорт
