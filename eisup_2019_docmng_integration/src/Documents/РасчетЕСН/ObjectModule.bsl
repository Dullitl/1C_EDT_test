...
Функция ПолучитьОграничениеНаСотрудников(Отказ)
	...
	Если ЗначениеЗаполнено(ТекстЗапроса) Тогда
		...
		Если МассивПодразделений.Количество() = 0 Тогда
			...
			ОбщегоНазначения.ВывестиИнформациюОбОшибке("Для ответственного не задан список подразделений для расчета!", Отказ, "Расчет ЕСН");
		КонецЕсли;
	...
	|ИЗ
	|	ВТ_Работники КАК РаботникиОрганизаций
	...
	|			И (Договора.ФизЛицо = РаботникиОрганизаций.Физлицо)
	|ГДЕ
	|	РаботникиОрганизаций.Физлицо В
	|			(ВЫБРАТЬ
	|				ВТ_Физлица.Физлицо
	|			ИЗ
	|				ВТ_Физлица)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Работники КАК РаботникиОрганизаций
	|		ПО ДатыПоследнихДвижений.Период = РаботникиОрганизаций.Период
	|			И ДатыПоследнихДвижений.Физлицо = РаботникиОрганизаций.Физлицо
	|
	|СГРУППИРОВАТЬ ПО
	...
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Работники КАК РаботникиОрганизаций
	|				ПО ПоследниеДаты.Физлицо = РаботникиОрганизаций.Физлицо
	...
	|						ИНАЧЕ ИСТИНА
	|					КОНЕЦ)
	|					И (ДоговорНаВыполнениеРаботСФизЛицом.ПодразделениеОрганизации В (&парамМассивПодразделений))
	|		
	|		ОБЪЕДИНИТЬ
	|		
	|		ВЫБРАТЬ
	...
	|					ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Работники КАК РаботникиОрганизаций
	|					ПО Договора.ФизЛицо = РаботникиОрганизаций.Физлицо
	|			ГДЕ
	...
	|				ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Работники КАК РаботникиОрганизаций
	|				ПО ДатыПоследнихДоговоров.Физлицо = РаботникиОрганизаций.Физлицо
...
Функция ПолучитьДанныеРасчетаЕСН(ГоловнаяОрганизация, ОрганичениеНаСотрудников, Отказ)
	...
	|ИЗ
	|	РегистрСведений.УчетнаяПолитикаОрганизаций.СрезПоследних(&парамНачалоГода, Организация = &парамГоловнаяОрганизация) КАК УчетнаяПолитикаПоЕСНСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок = "")
	...
	ИначеЕсли ВыборкаПоШапкеДокумента.ПериодРегистрации >= ПроведениеРасчетов.ДатаЗаменыЕСНСтраховымиВзносами() Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке("Уплата ЕСН с 1 января 2010 года отменена!", Отказ);
	ИначеЕсли ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ОбособленноеПодразделение) и ПустаяСтрока(Заголовок) Тогда
		...
		Если ВыборкаПоШапкеДокумента.НеУказанВидСтавокЕСН Тогда
			...
			СтрокаСообщения = ОбщегоНазначения.ПреобразоватьСтрокуИнтерфейса("организации");
