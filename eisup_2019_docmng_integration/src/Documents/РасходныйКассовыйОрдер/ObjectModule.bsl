...
Функция ПечатьРКО()
	...
	//ОбластьМакета.Параметры.СуммаПрописьюПолучил     = ОбщегоНазначения.СформироватьСуммуПрописью(Шапка.Сумма, Шапка.Валюта); //абсо
	ОбластьМакета.Параметры.ОрганизацияПоОКПО = СведенияОбОрганизации.КодПоОКПО;
	ОбластьМакета.Параметры.ДатаДокумента     = Шапка.ДатаДокумента;
	ОбластьМакета.Параметры.ДатаДокумента     = Шапка.ДатаДокумента;
	ОбластьМакета.Параметры.ДатаДокументаПодпись     ="";
	...
	Если ТекстПоДокументу="" И ЗначениеЗаполнено(Контрагент) И (ВидОперации=Перечисления.ВидыОперацийРКО.ВыдачаДенежныхСредствПодотчетнику
		...
		Попытка
			...
			ДокФизЛица = абс_СерверныеФункции.ПаспортныеДанные(Контрагент.Код);
			Если ЗначениеЗаполнено(ДокФизЛица.Вид) Тогда
				ТекстПоДокументу = Строка(ДокФизЛица.Вид) + " серия " +
				ДокФизЛица.Серия       + ", номер " +
				ДокФизЛица.Номер       + ", выданый " +
				ДокФизЛица.ДатаВыдачи  + " " +
				ДокФизЛица.Выдан;
			КонецЕсли;
		Исключение
	...
	КонецЕсли;
	//АБС
	Если ТекстПоДокументу="" И (ВидОперации=Перечисления.ВидыОперацийРКО.РасходДенежныхСредствПрочее) Тогда
		Если СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.РасчетыСПерсоналомПоОплатеТруда Тогда
			Если ЗначениеЗаполнено(СубконтоДт1) Тогда
			Попытка
			//ДокФизЛица = ОбщегоНазначения.ПаспортныеДанные(Контрагент, Дата);
			ДокФизЛица = абс_СерверныеФункции.ПаспортныеДанные(СубконтоДт1.Код);
			Если ЗначениеЗаполнено(ДокФизЛица.Вид) Тогда
				ТекстПоДокументу = Строка(ДокФизЛица.Вид) + " серия " +
				ДокФизЛица.Серия       + ", номер " +
				ДокФизЛица.Номер       + ", выданый " +
		...
		Исключение
			...
			ДокФизЛица = ОбщегоНазначения.ПаспортныеДанные(СубконтоДт1, Дата);
			Если ЗначениеЗаполнено(ДокФизЛица) Тогда
				ТекстПоДокументу = Строка(ДокФизЛица.Вид) + " серия " +
				ДокФизЛица.Серия       + ", номер " +
				ДокФизЛица.Номер       + ", выданый " +
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект, Строка(ВидОперации)));
...
Процедура АвтозаполнениеПлатежныхВедомостей() Экспорт
	...
	Иначе
		...
		|	СУММА(ЗарплатаКВыплате.Сумма) КАК СуммаПлатежа
		|ИЗ
		...
		|		ПО ЗарплатаКВыплате.Ссылка = РасходныйКассовыйОрдерВыплатаЗаработнойПлаты.Ведомость
		|ГДЕ
...
Функция ОпределитьКодыСчетСубсчет() 
	...
	Если КассаВВалюте Тогда
		СчетКт = абс_СчетКассы.Код;//ПланыСчетов.Хозрасчетный.КассаОрганизацииВал.Код;
		РасчетыВВалюте = Истина;
	Иначе
		СчетКт = абс_СчетКассы.Код;//ПланыСчетов.Хозрасчетный.КассаОрганизации.Код;
		РасчетыВВалюте = Ложь;
	КонецЕсли;
...
Процедура УстановитьОснование() Экспорт
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаДепонентов Тогда
		Основание = "Выплата депонированной заработной платы"	
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыРаботнику Тогда
		Основание = "Выплата заработной платы"	
	КонецЕсли;
...
Процедура ПроверитьСуммуКВыплатеРаботнику(Отказ,Заголовок)
	...
	Если Не Выборка.Следующий() Или Выборка.СуммаВыплачено <> СуммаДокумента Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не совпадает сумма документа и сумма по ведомости на выплату заработной платы.");
		Отказ=Истина;
	КонецЕсли;
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, Отказ, Заголовок,СтруктураШапкиДокумента)
	...
	Если РасчетыВВалюте Тогда
		СчетКт = абс_СчетКассы;//ПланыСчетов.Хозрасчетный.КассаОрганизацииВал;
	Иначе
		СчетКт = абс_СчетКассы;//ПланыСчетов.Хозрасчетный.КассаОрганизации;
	КонецЕсли;
	...
	Если ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.Сумма <> 0 Тогда
				...
				Проводка.СчетКт      = абс_СчетКассы;//ПланыСчетов.Хозрасчетный.КассаОрганизации;
				Проводка.СубконтоКт.СтатьиДвиженияДенежныхСредств = Выборка.СтатьяДвиженияДенежныхСредств;
				Если абс_СчетКассы = ПланыСчетов.Хозрасчетный.НайтиПоКоду("50.02") Тогда
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,2,абс_КассаККМ); 	
				КонецЕсли;
			КонецЕсли;
	...
	Иначе
		...
		Если ЕстьРасчетыСКонтрагентами Тогда
			...
			//Абс-софт...		
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ВыдачаДенежныхСредствПодотчетнику И СписокПодотчетныхЛиц Тогда
			
			Для каждого Строка Из РасшифровкаПлатежаПодотчетникам Цикл
				...
				КонецЕсли;
				Проводка.СубконтоДт.РаботникиОрганизации = Строка.ФизЛицо;
				Проводка.СубконтоДт.ВидыПодотчетныхСумм  = Строка.ВидПодотчетнойСуммы;
				
				Проводка.СчетКт = абс_СчетКассы;//ПланыСчетов.Хозрасчетный.КассаОрганизации;
				Проводка.Сумма  = Строка.Сумма;			
				Проводка.СубконтоКт.СтатьиДвиженияДенежныхСредств = Строка.СтатьяДвиженияДенежныхСредств;
				Если абс_СчетКассы = ПланыСчетов.Хозрасчетный.НайтиПоКоду("50.02") Тогда
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,2,абс_КассаККМ);	
				КонецЕсли;
				
			КонецЦикла;	
			//+++ T#7723272 2017.05.10 Гущина
			Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете тогда 
				Для каждого Строка Из РасшифровкаПлатежаПодотчетникам Цикл
					...
					ПроводкаНУ.СчетДт = ?(РасчетыВВалюте,ПланыСчетов.Налоговый.НайтиПоКоду("71.21"),ПланыСчетов.Налоговый.НайтиПоКоду("71.01"));
					ПроводкаНУ.СубконтоДт.РаботникиОрганизации = Строка.ФизЛицо;
					ПроводкаНУ.СубконтоДт.ВидыПодотчетныхСумм  = Строка.ВидПодотчетнойСуммы;
		...
		Иначе
			...
			Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийРКО.ВыдачаДенежныхСредствПодотчетнику Тогда
				...
				Проводка.СубконтоДт.ВидыПодотчетныхСумм = СтруктураШапкиДокумента.ВидПодотчетнойСуммы;
			...
			Если РасчетыВВалюте Тогда
				...
				Проводка.СчетКт = абс_СчетКассы;//ПланыСчетов.Хозрасчетный.КассаОрганизацииВал;
			...
			Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете тогда 
				...
				Если ЗначениеЗаполнено(СчетУчетаРасчетовСКонтрагентомНУ)
					И СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийРКО.РасходДенежныхСредствПрочее Тогда  //+++--- T#7723272  2017.05.10 Гущина. Было "И", заменила на "Если"
			...
			ИначеЕсли	СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийРКО.ВыдачаДенежныхСредствПодотчетнику Тогда
				...
				ПроводкаНУ.Содержание  = "Выдача средств подотчетнику";
				ПроводкаНУ.СчетДт  = ?(РасчетыВВалюте,ПланыСчетов.Налоговый.НайтиПоКоду("71.21"),ПланыСчетов.Налоговый.НайтиПоКоду("71.01"));
				ПроводкаНУ.СубконтоДт.РаботникиОрганизации = СтруктураШапкиДокумента.Контрагент;
				ПроводкаНУ.СубконтоДт.ВидыПодотчетныхСумм = СтруктураШапкиДокумента.ВидПодотчетнойСуммы;
				ПроводкаНУ.Сумма = Проводка.Сумма;
			ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийРКО.РасчетыПоКредитамИЗаймамСРаботниками Тогда
				ПроводкаНУ=ПроводкиНУ.Добавить();
				ПроводкаНУ.Период      = СтруктураШапкиДокумента.Дата;
				ПроводкаНУ.Активность  = Истина;
				ПроводкаНУ.Организация = СтруктураШапкиДокумента.Организация;
				ПроводкаНУ.Содержание  = "Расчеты по предоставленным займам";
				ПроводкаНУ.СчетДт =  БухгалтерскийУчет.ПреобразоватьСчетаБУвСчетНУ(Новый Структура("СчетБУ",  ПланыСчетов.Хозрасчетный.РасчетыПоПредоставленнымЗаймам), "НУ",  Дата);
				БухгалтерскийУчет.УстановитьСубконто(ПроводкаНУ.СчетДт, ПроводкаНУ.СубконтоДт, 1, СтруктураШапкиДокумента.Контрагент);
				ПроводкаНУ.Сумма = Проводка.Сумма;
			КонецЕсли;
			//--- T#7723272 2017.05.10 Гущина	
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеДокументаУпр(Отказ, Режим, Заголовок)
	...
	Если ВидОперации=Перечисления.ВидыОперацийРКО.ВзносНаличнымиВБанк
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не совпадают валюты банковского счета и кассы", Отказ, Заголовок);
	КонецЕсли;
		
	Если ВидОперации=Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям Тогда
		Если ВыплатаЗаработнойПлаты.Количество() = 0 Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не выбрана ни одна ведомость на выплату зарплаты", Отказ, Заголовок);
		ИначеЕсли ВыплатаЗаработнойПлаты.Итог("СуммаПлатежа") <> СуммаДокумента Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не совпадают сумма документа и сумма по ведомостям на выплату зарплаты", Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
	...
	Если ОтражатьВУправленческомУчете 
		...
		Если ВидОперации <> Перечисления.ВидыОперацийРКО.ВзносНаличнымиВБанк 
			...
			//АБС-Е ЗАКОММЕНТИРОВАЛ 25.01.12 чтобы без проблем проводить РКО! 
		...
		Если СвободныйОстаток < СуммаДокумента Тогда
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения, Ложь, Заголовок);
	...
	Если ВидОперации=Перечисления.ВидыОперацийРКО.РасчетыПоКредитамИЗаймамСРаботниками И
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Организация, выбранная в документе, не совпадает с организацией, выбранной в договоре займа.", Отказ, Заголовок);
	КонецЕсли;
	...
	Если ВидОперации=Перечисления.ВидыОперацийРКО.ВыдачаДенежныхСредствКассеККМ 
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Выдача денежных средств в кассу ККМ производится только в валюте регламентированного учета.", Отказ, Заголовок);
	КонецЕсли;
	...
	Если Организация.ОтражатьВРегламентированномУчете
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Проводить наличные расчеты с нерезидентами по данной организации запрещено.", Отказ, Заголовок);
	КонецЕсли;
	...
	Если ОтражатьВБухгалтерскомУчете Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Пока ВыборкаПоВыплатаДепонентов.Следующий() Цикл
					...
					Если Не ЕстьФизЛицо Тогда
						ttk_ОбщегоНазначения.СообщитьОбОшибке(НачалоСообщения + "не указан сотрудник!", Отказ, Заголовок);
					КонецЕсли;
					
					Если Не ЕстьВедомость Тогда
						ttk_ОбщегоНазначения.СообщитьОбОшибке(НачалоСообщения + "не указана платежная ведомость!", Отказ, Заголовок);
					ИначеЕсли НЕ ОшибкаОрганизации и ВыборкаПоВыплатаДепонентов.Организация <> Организация Тогда
						ttk_ОбщегоНазначения.СообщитьОбОшибке(НачалоСообщения + "платежная ведомость " + ВыборкаПоВыплатаДепонентов.Номер + " от " + ВыборкаПоВыплатаДепонентов.Дата + " оформлена по другой организации!", Отказ, Заголовок);
					...
					Если ЕстьВедомость И ЕстьФизЛицо И ВыборкаПоВыплатаДепонентов.ОстатокДепонента < ВыборкаПоВыплатаДепонентов.СуммаКВыплате Тогда
						ttk_ОбщегоНазначения.СообщитьОбОшибке(НачалоСообщения + "невыданный остаток депонента (" + Формат(ВыборкаПоВыплатаДепонентов.ОстатокДепонента,"ЧДЦ=2") + ") меньше суммы, указанной в документе!", Отказ, Заголовок);
					КонецЕсли;
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям Тогда 
			...
			Пока РезультатЗапроса.Следующий() Цикл
				...
				Пока ВложеннаяВыборка.Следующий() Цикл
					...
					Если НЕ ОшибкаОрганизации и ВложеннаяВыборка.Организация <> Организация Тогда
						...
						ttk_ОбщегоНазначения.СообщитьОбОшибке("Документ ""Зарплата к выплате"" " + ВложеннаяВыборка.Номер + " от " + ВложеннаяВыборка.Дата + " оформлен по другой организации!", Отказ, Заголовок);
						ОшибкаОрганизации = Истина;
					...
					Если ВложеннаяВыборка.ВыплаченностьЗарплаты = Перечисления.ВыплаченностьЗарплаты.НеВыплачено Тогда
						...
						ttk_ОбщегоНазначения.СообщитьОбОшибке("В строке № "+ ВложеннаяВыборка.НомерСтроки +" документа ""Зарплата к выплате"" " + ВложеннаяВыборка.Номер + " от " + ВложеннаяВыборка.Дата + " указана отметка  ""Не выплачено""", Отказ, Заголовок);
			...
			Пока Выборка.Следующий() Цикл
				Если Выборка.ОформленПозже Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Документ ""Зарплата к выплате"" " + РезультатЗапроса.Номер + " от " + РезультатЗапроса.Дата + " оформлен позже, чем этот РКО!", Отказ, Заголовок);
				КонецЕсли;
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыРаботнику Тогда 
			...
			Если РасчетныйДокумент.Организация <> Организация Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Не совпадают организации в расходном кассовом ордере и платежной ведомости", Отказ, Заголовок);
			КонецЕсли; 
			
			Если РасчетныйДокумент.СпособВыплаты = Перечисления.СпособыВыплатыЗарплаты.ЧерезБанк Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("В документе ""Зарплата к выплате"" установлен способ выплаты через банк!", Отказ, Заголовок);
			КонецЕсли;
			
			Если КонецДня(Дата) < РасчетныйДокумент.Дата Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Указанная платежная ведомость оформлена позже, чем этот РКО!",Отказ,Заголовок);
			КонецЕсли;
		...
		Если ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаДепонентов Тогда
			...
			Если ВыплатаДепонентов.Итог("СуммаКВыплате") <> РасшифровкаПлатежа.Итог("СуммаПлатежа") Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Сумма выплачиваемых депонентов не совпадает с расшифровкой платежа.",Отказ,Заголовок);
			КонецЕсли;
		КонецЕсли;
		...
		Если ВидОперации=Перечисления.ВидыОперацийРКО.РасходДенежныхСредствПрочее И СчетУчетаРасчетовСКонтрагентом.Пустая() Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указан счет учета.");
		КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ЗарплатаКВыплатеОрганизаций")	Тогда
		...
		//АБС+++ Гончаров 24.05.2013
		ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыРаботнику;
		РасчетныйДокумент = Основание;
		СтатьяДвиженияДенежныхСредств = Константы.абс_СтатьяДДСВыплатаЗП.Получить();
		//\\АБС--- Гончаров 24.05.2013
	...
	// АБС ВСТАВКА 22911	
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.Абс_АктПриёмкиВыполненныхРабот") Тогда
		
		ВидОперации = Перечисления.ВидыОперацийРКО.РасходДенежныхСредствПрочее;
		Организация = Основание.Организация;
		Ответственный = глЗначениеПеременной("глТекущийПользователь");
		ВалютаВзаиморасчетовРаботника = Константы.ВалютаРегламентированногоУчета.Получить();
		ВалютаДокумента = Константы.ВалютаРегламентированногоУчета.Получить();
		СуммаДокумента = Основание.Сотрудники[0].СуммаЗаРаботу; 
		СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.РасчетыСПерсоналомПоОплатеТруда;
		СчетУчетаРасчетовСКонтрагентомНУ = ПланыСчетов.Налоговый.РасчетыСПерсоналомПоОплатеТруда;
		СубконтоДт1 = Основание.Сотрудники[0].Сотрудник.ФизЛицо;
		СубконтоНУДт1 = Основание.Сотрудники[0].Сотрудник.ФизЛицо;
		ОтражатьВБухгалтерскомУчете = Истина;
		ОтражатьВНалоговомУчете = Истина;
		ОтражатьВУправленческомУчете = Истина;
		ЭтотОбъект.Основание = "Оплата по документу: " + Основание; 
		СтатьяДвиженияДенежныхСредств = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("ТД104");
		Подразделение = Основание.Сотрудники[0].ДоговорПодряда.абс_ЦФО;
		Комментарий = "Оплата по документу: " + Основание;
		
		СтрокаПлатеж=РасшифровкаПлатежа.Добавить();
		СтрокаПлатеж.СтатьяДвиженияДенежныхСредств = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("ТД104");
		СтрокаПлатеж.СуммаВзаиморасчетов = Основание.Сотрудники[0].СуммаЗаРаботу; 
		СтрокаПлатеж.СуммаПлатежа = Основание.Сотрудники[0].СуммаЗаРаботу; 
		
	// АБС ВСТАВКА 22911 КОНЕЦ	
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	// АБС ВСТАВКА
	Если Не Отказ И ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.Абс_АктПриёмкиВыполненныхРабот") Тогда 
		абс_БизнесПроцессыПривелегированный.СменитьСтатусАктаРКО(ДокументОснование, Ссылка, Истина, Отказ);
	КонецЕсли;
	// АБС ВСТАВКА КОНЕЦ 
...
Процедура ПриЗаписи(Отказ)
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.Абс_АктПриёмкиВыполненныхРабот") Тогда 
		абс_БизнесПроцессыПривелегированный.СменитьСтатусАктаРКО(ДокументОснование, Ссылка, Ложь, Отказ);
	КонецЕсли;
	// АБС ВСТАВКА КОНЕЦ  
	...
	Если Константы.абс_СинхронизацияВыплатыЗарплаты.Получить() 
		...
		И (ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыРаботнику ИЛИ ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям) Тогда  //АБС ВСТАВКА Манжела 06.03.2014 
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//АБС ВСТАВКА 48952  31.10.2014 12:52:00  Шамов
	Если ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыРаботнику Тогда
		ВыплатаЗаработнойПлаты.Очистить();
	КонецЕсли;
	//АБС ВСТАВКА 48952 КОНЕЦ

	//АБС ВСТАВКА №10497 НАЧАЛО
	Если НЕ ЗначениеЗаполнено(абс_СчетКассы) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Поле ""Счет кассы"" обязательно для заполнения!", Отказ);
        Возврат;
	КонецЕсли;
	//\\АБС ВСТАВКА №10497 КОНЕЦ
