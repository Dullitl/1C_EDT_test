...
//АБС	
	мТаблицаПериодов = Новый ТаблицаЗначений;
	мТаблицаПериодов.Колонки.Добавить("НомерМесяца",Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	мТаблицаПериодов.Колонки.Добавить("Месяц",Новый ОписаниеТипов("Дата"));
	ТекДата = '19970101';
	ТекДата = НачалоМесяца(ТекДата);
	НомерМесяца = 1;
	Пока НачалоМесяца(ТекДата)<=НачалоМесяца(ТекущаяДата()) Цикл
		
		НоваяЗапись = мТаблицаПериодов.Добавить();
	...
	СчетРезервыНаПереоценку = ПланыСчетов.Международный.Служебный;
	БухИтогиР.РассчитатьИтоги("Международный", "КонечныйОстатокДт,КонечныйОстатокКт", "Сумма", "Счет, Субконто1, Субконто2", Дата, Дата, , ПланыСчетов.Международный.Служебный, МассивСубконто, , , "Организация", Организация);
		    	
	// Start КТТК Ермолов Е.Л.  22.09.2015 000034449
	ЗапросКорректировкиДаты = Новый Запрос("ВЫБРАТЬ
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.Период,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.Регистратор,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.НомерСтроки,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.Активность,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.ОсновноеСредство,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.Организация,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.ДатаПринятияКУчету,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.МестонахождениеОбъекта,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.МОЛ,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.Состояние,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.СчетУчета,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.СрокПолезногоИспользования,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.НачислятьАмортизацию,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.МетодНачисленияАмортизации,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.СчетНачисленияАмортизации,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.ПервоначальнаяСтоимость,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.СправедливаяСтоимость,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.ЛиквидационнаяСтоимость,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.СчетЗатрат,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.Субконто1,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.Субконто2,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.Субконто3,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.ПредполагаемыйОбъемПродукции,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.СуммаНачисленнойАмортизации,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.УбытокОтОбесценения,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.КоэффициентУскорения,
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.СчетСниженияСтоимости
	                                        |ИЗ
	                                        |	РегистрСведений.ОсновныеСредстваМеждународныйУчет.СрезПервых(, ) КАК ОсновныеСредстваМеждународныйУчетСрезПервых
	                                        |ГДЕ
	                                        |	ОсновныеСредстваМеждународныйУчетСрезПервых.ОсновноеСредство = &ОсновноеСредство");
	...
	Пока ВыборкаОС.Следующий() Цикл
		...
		// Start КТТК Ермолов Е.Л.  22.09.2015 000034449
		ЗапросКорректировкиДаты.УстановитьПараметр("ОсновноеСредство",ВыборкаОС.ОсновноеСредство);
		ВыборкаКорректировка = ЗапросКорректировкиДаты.Выполнить().Выбрать();
		Если ВыборкаКорректировка.Следующий() Тогда 
			ДатаУстановки = ВыборкаКорректировка.ДатаПринятияКУчету;	
		Иначе
			ДатаУстановки             = НачалоМесяца(ВыборкаОС.ДатаПринятияКУчету);
		КонецЕсли;
		// Stop КТТК Ермолов Е.Л.  22.09.2015
		...
		КонецЕсли;
		// Start КТТК Ермолов Е.Л.  23.09.2015  000034449
		//СтрокаТЧ = мТаблицаПериодов.Найти(ДатаУстановки,"Месяц");
		СтрокаТЧ = мТаблицаПериодов.Найти(НачалоМесяца(ДатаУстановки),"Месяц");
		// Stop КТТК Ермолов Е.Л.  23.09.2015
		Если СтрокаТЧ<>Неопределено Тогда
		...
		Если ОставшеесяЧислоМесяцев<=0 Тогда
			Сообщить(""+ВыборкаОС.ОсновноеСредство.Код+"; оставшийся срок ;"+ОставшеесяЧислоМесяцев+"; срок использования ;"+ВыборкаОС.СрокПолезногоИспользования);
			Продолжить;
		КонецЕсли;
		...
		Если (ЗначениеЗаполнено(СчетЗатрат)) 
			...
			Движение = Движения.абс_СтоимостьОСМеждународный.Добавить();
			...
			Движение.ОсновноеСредство = ВыборкаОС.ОсновноеСредство;
			Движение.Организация = Организация;
			// Start КТТК Ермолов Е.Л. 000034449 22.09.2015 
			Если ЗначениеЗаполнено(ВыборкаКорректировка.ДатаПринятияКУчету) Тогда 
				Движение.ДатаПринятияКУчету = ВыборкаКорректировка.ДатаПринятияКУчету;	
			Иначе
				Движение.ДатаПринятияКУчету = НачалоМесяца(ВыборкаОС.ДатаПринятияКУчету);
			КонецЕсли;
			// Stop КТТК Ермолов Е.Л.  22.09.2015
//			Движение.ДатаПринятияКУчету = ВыборкаОС.ДатаПринятияКУчету;
			...
			// Start КТТК Ермолов Е.Л. 000034449 22.09.2015 
     		Если ЗначениеЗаполнено(ВыборкаКорректировка.СрокПолезногоИспользования) Тогда 
				Движение.СрокПолезногоИспользования = ВыборкаКорректировка.СрокПолезногоИспользования;
			Иначе
			...
			Если СчетЗатрат = Счета.ОсновноеПроизводствоНеОблагаемоеЕНВД  Тогда
