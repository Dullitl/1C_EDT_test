...
Процедура ЗаполнитьКомплектующие() Экспорт
	...
	Пока Выборка.Следующий() Цикл
        ...
        ЗаполнитьСчетаУчетаВСтрокеТабЧастиРегл(НоваяСтрока, "Комплектующие", ОтражатьВБухгалтерскомУчете, ОтражатьВНалоговомУчете);
...
Функция ПечатьКомплектацияНоменклатуры()
    //АБС ВСТАВКА 55381 НАЧАЛО
	Если ВидОперации <> Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияНоменклатуры И ВидОперации <> Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияОборудования Тогда
	//\\АБС ВСТАВКА 55381 КОНЕЦ	
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, ?(ВидКомплектации = Перечисления.ВидыКомплектации.Сборка, "Комплектация", "Разукомплектация")+" номенклатуры");
	...
	Для каждого ВыборкаСтрокКомплектующие Из ЗапросКомплектующие Цикл
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаСтрокКомплектующие.Номенклатура) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("В одной из строк не заполнено значение комплектующей - строка при печати пропущена.");
			Продолжить;
		КонецЕсли;
...
//
Процедура ЗаполнитьСчетаУчетаВШапке(ЗаполнятьБУ=Неопределено, ЗаполнятьНУ=Неопределено) Экспорт
	...
	Если ЗаполнятьНУ = Неопределено Тогда
		ЗаполнятьНУ = ОтражатьВБухгалтерскомУчете;
	КонецЕсли;
	
	СчетаУчетаВДокументах.ЗаполнитьСчетаУчетаШапки(ЭтотОбъект, ЗаполнятьБУ, ЗаполнятьНУ,,,"Номенклатура",Истина);
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	   ...
	   ИЛИ ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.ПоступлениеОтПереработчика Тогда
		...
		Если ВидКомплектации = Перечисления.ВидыКомплектации.Разборка Тогда
			Если СтатьяЗатрат.ВидЗатрат <> Перечисления.ВидыЗатрат.Материальные Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Укажите материальную статью затрат (закладка ""Учет затрат"")!", Отказ, Заголовок);
			КонецЕсли;
			...
			Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Укажите производственную статью затрат (закладка ""Учет затрат"")!", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтруктураШапкиДокумента.Номенклатура) Тогда
		...
		Если СтруктураШапкиДокумента.Услуга Или СтруктураШапкиДокумента.Набор Или СтруктураШапкиДокумента.Комплект Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Комплектуемая номенклатура не может быть услугой, набором-пакетом или набором-комплектом!", Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
	Если ЗначениеЗаполнено(СтруктураШапкиДокумента.Склад) Тогда
		...
		Если СтруктураШапкиДокумента.ВидСклада = Перечисления.ВидыСкладов.НТТ Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Комплектация номенклатуры не может проводится на НТТ!", Отказ, Заголовок);
		КонецЕсли;
    КонецЕсли;
...
Процедура ПроверитьЗаполнениеТабличнойЧастиКомплектующих(ТаблицаПоКомплектующим, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если Комплектующие.Количество() = 0 Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не заполнена табличная часть ""Комплектующие"".", Отказ, Заголовок);
	КонецЕсли;
	...
	Если ВидКомплектации = Перечисления.ВидыКомплектации.Разборка Тогда
		...
		СтруктураОбязательныхПолей = Новый Структура("Номенклатура, Количество, ДоляСтоимости,СтатусПартии, СтатьяОборотов");
	Иначе
	...
	//АБС ВСТАВКА 55381 НАЧАЛО
	Если ВидОперации <> Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияНоменклатуры И ВидОперации <> Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияОборудования
	   И ВидКомплектации = Перечисления.ВидыКомплектации.Сборка
	   //\\АБС ВСТАВКА 55381 КОНЕЦ
	Тогда
		СтруктураОбязательныхПолей.Вставить("СтатьяЗатрат");
	...
	Для каждого Строка из Комплектующие Цикл
		...
		СтрокаНачалаСообщенияОбОшибке = Нстр("ru = 'В строке номер """+ СокрЛП(Строка.НомерСтроки) + """ табличной части "" Комплектующие "": '");
		...
		//Если СокрЛП(СтрокаТаблицы.СтатьяОборотов.Код) = "КВпоПост" Тогда
		//	СтрокаНачалаСообщенияОбОшибке = "В строке номер """+ СокрЛП(СтрокаТаблицы.НомерСтроки) +
		//	""" табличной части """ + "Товары" + """: ";
		//	Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.абс_ТЭО) ИЛИ НЕ ЗначениеЗаполнено(СтрокаТаблицы.абс_КВ) Тогда
		//		ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не заполнены аналитики " +
		//		"ТЭО и КВ - они обязательны при указанной статье!", Отказ, Заголовок);				
		//	КонецЕсли;
		//	Если НЕ (СчетУчетаБУ.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ОборудованиеКУстановке) ИЛИ СчетУчетаБУ.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ВложенияВоВнеоборотныеАктивы))
		//		ИЛИ НЕ (СтрокаТаблицы.СчетУчетаБУ.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ОборудованиеКУстановке) ИЛИ СтрокаТаблицы.СчетУчетаБУ.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ВложенияВоВнеоборотныеАктивы))
		//		Тогда
		//	Иначе
		//		ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + Нстр("ru = 'Необходимо выбрать при установленной статье оборотов только счета группы 08 или 07'") , Отказ, Заголовок);
		//	КонецЕсли;				
		//КонецЕсли;
		//Если (СчетУчетаБУ.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ОборудованиеКУстановке) ИЛИ СчетУчетаБУ.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ВложенияВоВнеоборотныеАктивы))
		//		ИЛИ (СтрокаТаблицы.СчетУчетаБУ.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ОборудованиеКУстановке) ИЛИ СтрокаТаблицы.СчетУчетаБУ.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ВложенияВоВнеоборотныеАктивы))
		//		Тогда
		//	Если НЕ СокрЛП(СтрокаТаблицы.СтатьяОборотов.Код) = "КВпоПост" Тогда
		//		ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + Нстр("ru = 'Необходимо выбрать при установленных счетах группы 08 или 07 статью Единая статья КВ'") , Отказ, Заголовок);				
		//	КонецЕсли;
		//КонецЕсли;
	КонецЦикла;
	//Крамаренко Д.М.
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоКомплектующим, ТаблицаПоКомплектам, Отказ, Заголовок);
	...
	Если ttk_ОбщегоНазначения.абс_НеВыполнятьДвиженияВСтатусе(СтруктураШапкиДокумента) или не ОтражатьВБухгалтерскомУчете или Отказ Тогда
		...
		// Горелик Р.В. 26.09.2018 [CHG495\495] +++
		Если ПроверкаДляКомплектацииНоменклатуры() = 0 Тогда 
			Возврат;
		Иначе 
			Счетчик = 2;
		КонецЕсли;
	...
	// Горелик Р.В. 26.09.2018 [CHG495\495] ---
		...
		//АБС ИЗМЕНЕНИЕ 36867  09.01.2014 16:07:55  Шамов
	...
	// Горелик Р.В. 26.09.2018 [CHG495\495] ---
		...
		ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоКомплектующим, ТаблицаПоКомплектам, Отказ, Заголовок, Счетчик);
	...
	// Горелик Р.В. 26.09.2018 [CHG495\495] ---
		...
		Если ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияОборудования Тогда
	...
	Процедура ВнестиИзмененияВРегистры()
		...
		Для каждого СтрокаДвижение Из ДвиженияПоРегПартииТоваровБух Цикл
			...
			ВнестиИзмененияВРегистрыПоСтоимости(СтрокаДвижение); //АБС Коломиец
			
			Если СтрокаДвижение.ВидДвижения = ВидДвиженияНакопления.Приход Тогда
				СтрокаДвижение.ДокументОприходования = Ссылка;
				СтрокаДвижение.КодОперации = Перечисления.КодыОперацийПартииТоваров.ПоступлениеОборудования;
				
				//АБС НАЧАЛО Задача №08759
				Если ТипЗнч(ЗаказРезерв) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда 
		        	СтрокаДвижение.Заказ = ЗаказРезерв;
				КонецЕсли;
		...
		//АБС НАЧАЛО Задача №08759
        ДвиженияПоРегПартииТоваровУ = РегистрыНакопления.ПартииТоваровНаСкладах.СоздатьНаборЗаписей();
		ДвиженияПоРегПартииТоваровУ.Отбор.Регистратор.Значение = Ссылка;
		ДвиженияПоРегПартииТоваровУ.Отбор.Регистратор.Использование = Истина;
		ДвиженияПоРегПартииТоваровУ.Отбор.Регистратор.ВидСравнения = ВидСравнения.Равно;
		ДвиженияПоРегПартииТоваровУ.Прочитать();
		
		Для каждого СтрокаДвижение Из ДвиженияПоРегПартииТоваровУ Цикл
			Если СтрокаДвижение.ВидДвижения = ВидДвиженияНакопления.Приход Тогда
				//АБС НАЧАЛО Задача №08759
				Если ТипЗнч(ЗаказРезерв) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда 
		        	СтрокаДвижение.Заказ = ЗаказРезерв;
			...
			КонецЕсли;
		КонецЦикла;
		ДвиженияПоРегПартииТоваровУ.Записать();
  		//\\АБС КОНЕЦ Задача №08759
		
		//АБС Коломиец+		
		Если абс_ФиксированнаяСтоимость Тогда
			ДвиженияПоРегПартииТоваровУУ = РегистрыНакопления.ПартииТоваровНаСкладах.СоздатьНаборЗаписей();
			ДвиженияПоРегПартииТоваровУУ.Отбор.Регистратор.Значение = Ссылка;
			ДвиженияПоРегПартииТоваровУУ.Отбор.Регистратор.Использование = Истина;
			ДвиженияПоРегПартииТоваровУУ.Отбор.Регистратор.ВидСравнения = ВидСравнения.Равно;
			ДвиженияПоРегПартииТоваровУУ.Прочитать();
			
			Для каждого СтрокаДвижение Из ДвиженияПоРегПартииТоваровУУ Цикл
				ВнестиИзмененияВРегистрыПоСтоимости(СтрокаДвижение); 
				Если СтрокаДвижение.ВидДвижения = ВидДвиженияНакопления.Приход Тогда
					СтрокаДвижение.ДокументОприходования = Ссылка;
					СтрокаДвижение.КодОперации = Перечисления.КодыОперацийПартииТоваров.ПоступлениеОборудования;
			...
			КонецЦикла;
			ДвиженияПоРегПартииТоваровУУ.Записать();
			...
			Для каждого СтрокаДвижение Из ДвиженияБУ Цикл
				ВнестиИзмененияВРегистрыПоСтоимостиБУНУ(СтрокаДвижение); //АБС Коломиец
			КонецЦикла;
			ДвиженияБУ.Записать();
			
			ДвиженияНУ = РегистрыБухгалтерии.Налоговый.СоздатьНаборЗаписей();
			ДвиженияНУ.Отбор.Регистратор.Значение = Ссылка;
			ДвиженияНУ.Отбор.Регистратор.Использование = Истина;
			ДвиженияНУ.Отбор.Регистратор.ВидСравнения = ВидСравнения.Равно;
			ДвиженияНУ.Прочитать();
			Для каждого СтрокаДвижение Из ДвиженияНУ Цикл
				ВнестиИзмененияВРегистрыПоСтоимостиБУНУ(СтрокаДвижение); 
			КонецЦикла;
			ДвиженияНУ.Записать();	
		КонецЕсли;
		//АБС Коломиец-
...
Процедура ВнестиИзмененияВРегистрыПоСтоимости(СтрокаДвижение) //АБС Коломиец
	
	Если СтрокаДвижение.ВидДвижения = ВидДвиженияНакопления.Приход Тогда
		Если абс_ФиксированнаяСтоимость Тогда
			СтоимостьДок = 0;					
			Если ВидКомплектации = Перечисления.ВидыКомплектации.Разборка Тогда   //Разборка - комплектующие
				Для Каждого СтрокаДокумента из Комплектующие Цикл 
					Если СтрокаДвижение.Номенклатура.ВестиПартионныйУчетПоСериям Тогда
						Если  (СтрокаДокумента.Номенклатура = СтрокаДвижение.Номенклатура) 
							И (СтрокаДокумента.ХарактеристикаНоменклатуры = СтрокаДвижение.ХарактеристикаНоменклатуры) 
							И (СтрокаДокумента.СерияНоменклатуры = СтрокаДвижение.СерияНоменклатуры) 
							И (СтрокаДокумента.Количество = СтрокаДвижение.Количество)Тогда
							СтоимостьДок  = СтоимостьДок  + СтрокаДокумента.абс_СтоимостьСписания;
						Иначе
							Продолжить;
						КонецЕсли;
					Иначе
					...
					КонецЕсли;
				КонецЦикла;
				СтрокаДвижение.Стоимость = СтоимостьДок;						
			ИначеЕсли ВидКомплектации = Перечисления.ВидыКомплектации.Сборка Тогда   //Сборка - комплект
				СтрокаДвижение.Стоимость = Комплектующие.Итог("абс_СтоимостьСписания") * СтрокаДвижение.Количество;					
			КонецЕсли; 					
		КонецЕсли;
	...
	Иначе
		Если абс_ФиксированнаяСтоимость Тогда
			...
			Если ВидКомплектации = Перечисления.ВидыКомплектации.Сборка Тогда   //Сборка - комплектующие
				Для Каждого СтрокаДокумента из Комплектующие Цикл 
					Если СтрокаДвижение.Номенклатура.ВестиПартионныйУчетПоСериям Тогда
						Если  (СтрокаДокумента.Номенклатура = СтрокаДвижение.Номенклатура) 
							И (СтрокаДокумента.ХарактеристикаНоменклатуры = СтрокаДвижение.ХарактеристикаНоменклатуры) 
				...
				КонецЦикла;
				СтрокаДвижение.Стоимость = СтоимостьДок;						
			ИначеЕсли ВидКомплектации = Перечисления.ВидыКомплектации.Разборка Тогда   //Разборка - комплект
				СтрокаДвижение.Стоимость = Комплектующие.Итог("абс_СтоимостьСписания") * СтрокаДвижение.Количество;					
			КонецЕсли; 					
		КонецЕсли;
	КонецЕсли;
...
Процедура ВнестиИзмененияВРегистрыПоСтоимостиБУНУ(СтрокаДвижение) //АБС Коломиец
	
	СтоимостьДок = 0;		
	Если ВидКомплектации = Перечисления.ВидыКомплектации.Разборка Тогда   //Разборка - комплектующие
		Для Каждого СтрокаДокумента из Комплектующие Цикл 
			Если  (СтрокаДокумента.Номенклатура = СтрокаДвижение.СубконтоДт.Номенклатура)  
				И (СтрокаДокумента.Количество = СтрокаДвижение.КоличествоДт)Тогда
				СтоимостьДок  = СтоимостьДок  + СтрокаДокумента.абс_СтоимостьСписания;
			КонецЕсли;
		КонецЦикла;
		СтрокаДвижение.Сумма = СтоимостьДок;
		//СтрокаДвижение.Сумма = Комплектующие.Итог("абс_СтоимостьСписания") * СтрокаДвижение.КоличествоКт;						
	ИначеЕсли ВидКомплектации = Перечисления.ВидыКомплектации.Сборка Тогда   //Сборка - комплект
		Для Каждого СтрокаДокумента из Комплектующие Цикл 
			Если  (СтрокаДокумента.Номенклатура = СтрокаДвижение.СубконтоКт.Номенклатура)  
				И (СтрокаДокумента.Количество = СтрокаДвижение.КоличествоКт)Тогда
				СтоимостьДок  = СтоимостьДок  + СтрокаДокумента.абс_СтоимостьСписания;
			КонецЕсли;
		КонецЦикла;
		СтрокаДвижение.Сумма = СтоимостьДок;
...
Процедура СформироватьДвиженияКомплектующиеРасход_ТоварыНаСкладах(РежимПроведения, СтруктураШапкиДокумента,ТаблицаПоКомплектующим, Отказ, Заголовок)
	...
	Если НЕ Отказ Тогда
		...
		Если НЕ СтруктураШапкиДокумента.ВидСклада = Перечисления.ВидыСкладов.Розничный Тогда
			ТЗ = ТаблицаПоКомплектующим.Скопировать();
			ТЗ.Колонки.СчетУчетаБУ.Имя = "СчетУчета";
			СтруктТаблицДокумента = Новый Структура;
			СтруктТаблицДокумента.Вставить("ТаблицаПоКомплектующим", ТЗ);		
		Иначе
			СтруктТаблицДокумента = Новый Структура;
...
Процедура СформироватьДвиженияКомплектующиеРасход_ТоварыВРезервеНаСкладах(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоКомплектующим, Отказ, Заголовок)
	...
	Если ТаблицаПоКомплектующимИзРезерва.Количество() > 0 Тогда
		...
		Если Не Отказ Тогда
			...
			ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "Склад",           Склад);
			ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "ДокументРезерва", ЗаказРезерв);
...
Процедура СформироватьДвиженияКомплектующиеРасход_ТоварыПереданные(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоКомплектующим, Отказ, Заголовок)
	...
	ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "Сделка",      		  ЗаказВыпуска);
	ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "СтатусПередачи",     Перечисления.СтатусыПолученияПередачиТоваров.ВПереработку);
...
Процедура СформироватьДвиженияКомплектыПриход_ТоварыНаСкладах(РежимПроведения, СтруктураШапкиДокумента,ТаблицаПоКомплектам, Отказ, Заголовок);
	...
	//Бобылев А.А 21.05.2018 СППР00-00000088
	Если НЕ СтруктураШапкиДокумента.ВидСклада = Перечисления.ВидыСкладов.Розничный Тогда
		ТЗ = ТаблицаПоКомплектам.Скопировать();
		ТЗ.Колонки.СчетУчетаБУ.Имя = "СчетУчета";
		СтруктТаблицДокумента = Новый Структура;
		СтруктТаблицДокумента.Вставить("ТаблицаПоКомплектам", ТЗ);		
	Иначе
		СтруктТаблицДокумента = Новый Структура;
	...
	Если НЕ СтруктураШапкиДокумента.ВидСклада = Перечисления.ВидыСкладов.Розничный Тогда
		ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "СчетУчета",СчетУчетаБУ);
	КонецЕсли;
	//Исаенко С.П 
	ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "Склад",    Склад);
	ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "Качество", Справочники.Качество.Новый);
...
Процедура СформироватьДвиженияПоЗаказамНаПроизводство(ТаблицаПоТоварам, ИмяТабличнойЧасти, СтруктураШапкиДокумента, РежимПроведения, Отказ, Заголовок)
	...
	Если НЕ Отказ Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоПродукцииДляРазмещения, ТаблицаДвиженийПоРазмещению);
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоКомплектующим, ТаблицаПоКомплектам, Отказ, Заголовок, Счетчик)
	...
	Если ВидКомплектации = Перечисления.ВидыКомплектации.Сборка Тогда
		...
		Если ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияНоменклатуры
			ИЛИ ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияОборудования
		...
		Если ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияНоменклатуры ИЛИ  ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияОборудования Тогда
	...
	ИначеЕсли ВидКомплектации = Перечисления.ВидыКомплектации.Разборка Тогда
		...
		Если ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияНоменклатуры ИЛИ
			ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияОборудования Тогда
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.ПоступлениеОтПереработчика И Счетчик = 1 Тогда
...
Процедура ДвиженияПоТоварамОрганизаций(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоКомплектующим, ТаблицаПоКомплектам, Отказ, Заголовок)
	...
	Если ВидКомплектации = Перечисления.ВидыКомплектации.Сборка Тогда
		...
		// По комплектующим
		Если ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.ВыпускПродукции
		   ИЛИ ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияНоменклатуры
		   ИЛИ ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияОборудования Тогда
...
Процедура СформироватьДвиженияПоРегиструСписанныеТовары(
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияНоменклатуры ИЛИ 
		ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияОборудования Тогда
	...
	ИначеЕсли ЗначениеЗаполнено(СтруктураШапкиДокумента.ЗаказРезерв)
	   И (
		...
		(СтруктураШапкиДокумента.ВидОперации <> Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияНоменклатуры
		 И СтруктураШапкиДокумента.ВидОперации <> Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияОборудования )
	...
	Иначе
		...
		// АБС ВСТАВКА 50947
		НоваяСтрока.ДопустимыйСтатус4 = Перечисления.СтатусыПартийТоваров.Оборудование;
		//\\ АБС ВСТАВКА
	КонецЕсли;
	...
	Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияНоменклатуры 
		ИЛИ СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияОборудования
		Тогда
	...
	Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияНоменклатуры
		ИЛИ
		СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияОборудования
		Тогда
	...
	//АБС НАЧАЛО Задача №08759
	Если НЕ СтрокаТаблицыПоТоварам.Заказ.Пустая() Тогда 
		НоваяСтрока.ЗаказПартии                 = СтрокаТаблицыПоТоварам.Заказ;
		НоваяСтрока.ЗаказСписания               = СтрокаТаблицыПоТоварам.Заказ;
...
Процедура ДвиженияПоСписаннымТоварам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоКомплектующим, ТаблицаПоКомплектам, Отказ, Заголовок)
	...
	ИначеЕсли ВидКомплектации = Перечисления.ВидыКомплектации.Разборка Тогда
		...
		Для каждого Строка Из ТаблицаПоТоварам Цикл
			...
			// АБС ИЗМЕНЕНО Фролов 4904
			// В долю стоимости будем вводить стоимость рузукомплектованного оборудования
			// поэтому делаем большую точность при расчете пропорции
			КоличествоКомплекта = Окр(СтрокаКомплекта.Количество*КоэффициентСтроки, 3);
			//КоличествоКомплекта = Окр(СтрокаКомплекта.Количество*КоэффициентСтроки, 12);
			// АБС ИЗМЕНЕНО Фролов 4904 КОНЕЦ
			...
			// АБС ИЗМЕНЕНО Фролов 4904
			// Количество устанавливаем указанное в документе
...
Функция ПодготовитьТаблицуКомплектацииДляРасширеннойАналитики(СтруктураШапкиДокумента, ТаблицаПоКомплектующим,ТаблицаПоКомплектам)
	...
	ТаблицаКомплектации.Колонки.Добавить("СчетУчетаНУ");
	...
	ТаблицаКомплектации.Колонки.Добавить("КорСчетУчетаНУ");
	...
	Если СтруктураШапкиДокумента.ВидКомплектации = Перечисления.ВидыКомплектации.Сборка Тогда
		...
		Для Каждого ТекущаяСтрока Из ТаблицаПоКомплектующим Цикл
			...
			НоваяСтрока.КорСчетУчетаНУ = СтруктураШапкиДокумента.СчетУчетаНУ;
	...
	Иначе
		...
		Для Каждого ТекущаяСтрока Из ТаблицаПоКомплектующим Цикл
			...
			НоваяСтрока.КорСчетУчетаНУ = ТекущаяСтрока.СчетУчетаНУ;
			...
			//АБС ИЗМЕНЕНИЕ 55381  НАЧАЛО
			Если (СтруктураШапкиДокумента.ВидОперации <> Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияНоменклатуры) И  (СтруктураШапкиДокумента.ВидОперации <> Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияОборудования) Тогда
			//\\АБС ИЗМЕНЕНИЕ 55381  КОНЕЦ	
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//Горелик Р.В 24.10.2018 [CHG495/495]+++
	Если СчетУчетаБУ.Пустая() Тогда
		Сообщить("Необходимо указать счета учета!");
		Отказ = Истина;
	КонецЕсли;
	ПроверкаЗаполненияСчетов = ЭтотОбъект.Комплектующие;
	Для Каждого Стр ИЗ ПроверкаЗаполненияСчетов Цикл
		Если Стр.СчетУчетаБУ.Пустая() Тогда
			Сообщить("Необходимо указать счета учета в табличной части ""Исходные комплектующие""!");
			Отказ = Истина;
		КонецЕсли;
	КонецЦикла;
	//Горелик Р.В 24.10.2018 [CHG495/495]---
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	//+++ Колесников А.С. 16.10.2018 [CHG448 / 498]
...
// Горелик Р.В. 26.09.2018 [CHG495\495] +++
Функция ПроверкаДляКомплектацииНоменклатуры ()
	Счетчик = 0;
	Если ТипЗнч(Этотобъект.Ссылка) = Тип("ДокументСсылка.КомплектацияНоменклатуры") Тогда
		Если Этотобъект.Ссылка.абс_Статус = Перечисления.абс_СтатусыПервичныхДокументов.Подготовка ИЛИ Этотобъект.Ссылка.абс_Статус = Перечисления.абс_СтатусыПервичныхДокументов.Отказ ИЛИ Этотобъект.Ссылка.абс_Статус = Перечисления.абс_СтатусыПервичныхДокументов.Отмена Тогда
			//Счетчик = 0;
			Возврат Счетчик;
		Иначе
			Счетчик = 1;
