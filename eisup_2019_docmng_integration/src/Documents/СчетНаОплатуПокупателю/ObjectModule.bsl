...
//АБС ВСТАВКА
//Процедура обеспечивает заполнение счета на основании заявки
Процедура ЗаполнитьПоЗаявке(ДокументОснование) Экспорт
	...
	ЗаполнитьТабличнуюЧасть(ДокументОснование,"Товары");
	ЗаполнитьТабличнуюЧасть(ДокументОснование,"Услуги");
	
КонецПроцедуры

Процедура ЗаполнитьТабличнуюЧасть(ДокументОснование,ИмяТЧ)
	
	ЭтотОбъект[ИмяТЧ].Очистить();
	Для каждого строкаОснование из ДокументОснование[ИмяТЧ] Цикл
...
//TTK Вставка
Процедура ЗаполнитьПоАктОбОказанииПроизводственныхУслуг(ДокументОснование) Экспорт
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.Событие") Тогда
		...
		//АБС ВСТАВКА 11.01.11
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.абс_РегистрацияЗаявкиНаРеализацию") Тогда
		
		// Заполнение по реализации
		ЗаполнитьПоЗаявке(Основание);
		
		//\\АБС КОНЕЦ ВСТАВКИ 11.01.11
		
		//ТТК Вставка 09.11.11
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.АктОбОказанииПроизводственныхУслуг") Тогда
		
		АктОбОказанииУслуг = Основание;
		ЗаполнитьПоАктОбОказанииПроизводственныхУслуг(Основание);
		УправлениеЗапасами.ЗаполнитьСоставНабораПоОснованию(ЭтотОбъект, Основание);
       //ТТК КонецВставка  09.11.11
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ ОбменДанными.Загрузка  Тогда
		...
		Если ЭтотОбъект.ДоговорКонтрагента.абс_СтатусДоговора<>Перечисления.абсСтатусыДоговоров.Исполнение и 
			ЭтотОбъект.ДоговорКонтрагента.абс_СтатусДоговора<>Перечисления.абсСтатусыДоговоров.Корректировка и
			ЭтотОбъект.ДоговорКонтрагента.абс_СтатусДоговора<>Перечисления.абсСтатусыДоговоров.СогласованиеОФКПослеКоррректировки  и
			ЭтотОбъект.ДоговорКонтрагента.абс_СтатусДоговора<>Перечисления.абсСтатусыДоговоров.ОтказПослеКорректировки Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Запрещено оформлять счета по неактивным договорам");
			Отказ = Истина;
		КонецЕсли;
		//абс Родин Лимитный контроль 
