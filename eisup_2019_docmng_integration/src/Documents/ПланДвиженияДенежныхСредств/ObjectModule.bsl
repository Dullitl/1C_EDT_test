...
Процедура ДвиженияПоРегистрамУпр()
	...
	Иначе
		...
		Если КурсДок = 0 
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке("При пересчете в валюту упр. учета обнаружен нулевой курс.");
			Возврат;
		Иначе	
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ЗаполнениеДокументов.ПроверитьЗаполнениеТабличнойЧасти(ЭтотОбъект, "ДвиженияДенежныхСредств", Новый Структура("ПриходРасход,СтатьяДвиженияДенежныхСредств",
													"Не заполнен вид движения","Не заполнена статья движения денежных средств"),Отказ, Заголовок);
