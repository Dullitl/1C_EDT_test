	...
	Для Каждого СтрокаТЧ Из Материалы Цикл
		Если СтрокаТЧ.ТипСтоимости = Перечисления.ВидыНормативнойСтоимостиПроизводства.Фиксированная Тогда
			Если ОтражатьВУправленческомУчете И НЕ ЗначениеЗаполнено(СтрокаТЧ.Сумма) Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указана сумма. (Строка № " + СтрокаТЧ.НомерСтроки + ")", Отказ, Заголовок);
			КонецЕсли;
			Если ОтражатьВБухгалтерскомУчете И НЕ ЗначениеЗаполнено(СтрокаТЧ.СуммаРегл) Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указана сумма регл. (Строка № " + СтрокаТЧ.НомерСтроки + ")", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоМатериалам, Отказ, Заголовок)
	...
	//ДатаНачалаУчетаПоПартиямМатериалов = Константы.абс_УчетПоПартиямВПеремещенииМатериаловВЭксплуатации.Получить();
...
//АБС Формирование движений по регистрам ПартииМатериаловВЭксплуатации управленческого, бухгалтерского и налогового учекта
Процедура абсДвиженияПоРегистрамПартийМатериаловВЭксплуатации(СтруктураШапкиДокумента, ТаблицаПоМатериалам, Отказ, Заголовок)

	//АБС+ Тупиков 14871, АБС Коломиец 17531 изменения
	
	СтруктураРазделовУчета = Новый Структура("БУ, НУ", "БухгалтерскийУчет", "НалоговыйУчет");
	//АБС ВСТАВКА 35878  10.12.2013 13:25:00  Шамов
	СтруктураРазделовУчета.Вставить("УУ", "");
	//АБС ВСТАВКА 35878 КОНЕЦ 
	СтруктураОбозначенийРазделовУчетавШапке = Новый Структура("УУ, БУ, НУ", "ОтражатьВУправленческомУчете", 
			"ОтражатьВБухгалтерскомУчете", "ОтражатьВНалоговомУчете");
	Для каждого РазделУчета Из СтруктураРазделовУчета Цикл
	
		Если СтруктураШапкиДокумента[СтруктураОбозначенийРазделовУчетавШапке[РазделУчета.Ключ]] <> Истина Тогда
			Продолжить;
		КонецЕсли;
		
		ЭтоНУ = РазделУчета.Ключ = "НУ";
		//АБС ВСТАВКА 35878  10.12.2013 13:25:00  Шамов
		ЭтоУУ = РазделУчета.Ключ = "УУ";
		//АБС ВСТАВКА 35878 КОНЕЦ 
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
		Запрос.Текст = "ВЫБРАТЬ
		               |	ПеремещениеМатериаловВЭксплуатацииМатериалы.ПодразделениеОрганизацииПолучатель КАК ПодразделениеПолучатель,
		               ...
		               |	ПеремещениеМатериаловВЭксплуатацииМатериалы.СерияНоменклатуры,
		               |	ПеремещениеМатериаловВЭксплуатацииМатериалы.ДокументПередачи,
		               ...
		               |	ПеремещениеМатериаловВЭксплуатацииМатериалы.НазначениеИспользования.СпособПогашенияСтоимости КАК СпособПогашенияСтоимости,
		               |	ПеремещениеМатериаловВЭксплуатацииМатериалы.НазначениеИспользования.СрокПолезногоИспользования КАК СрокПолезногоИспользования,
		               ...
		               |	ПеремещениеМатериаловВЭксплуатацииМатериалы.Сумма,
		               |	ВЫБОР
		               |		КОГДА ПеремещениеМатериаловВЭксплуатацииМатериалы.ПодразделениеОрганизацииПолучатель <> &Подразделение
		               |				ИЛИ ПеремещениеМатериаловВЭксплуатацииМатериалы.НазначениеИспользования <> ПеремещениеМатериаловВЭксплуатацииМатериалы.НазначениеИспользования
		               |				ИЛИ ПеремещениеМатериаловВЭксплуатацииМатериалы.ФизЛицо <> ПеремещениеМатериаловВЭксплуатацииМатериалы.ФизЛицоПолучатель
		               |				ИЛИ ПеремещениеМатериаловВЭксплуатацииМатериалы.СчетУчетаБУ <> ПеремещениеМатериаловВЭксплуатацииМатериалы.СчетУчетаПолучательБУ
		               |			ТОГДА ИСТИНА
		               |		ИНАЧЕ ЛОЖЬ
		               |	КОНЕЦ КАК Переносить,
		               |	ПеремещениеМатериаловВЭксплуатацииМатериалы.НомерСтроки,
		               |	ПеремещениеМатериаловВЭксплуатацииМатериалы.Ссылка.Организация,
		               |	ПеремещениеМатериаловВЭксплуатацииМатериалы.СчетУчетаБУ КАК СчетУчета,
		               |	ПеремещениеМатериаловВЭксплуатацииМатериалы.СчетУчетаПолучательБУ КАК СчетУчетаПолучатель
		               |ПОМЕСТИТЬ Материалы
		               |ИЗ
		               |	Документ.ПеремещениеМатериаловВЭксплуатации.Материалы КАК ПеремещениеМатериаловВЭксплуатацииМатериалы
		               |ГДЕ
		               |	ПеремещениеМатериаловВЭксплуатацииМатериалы.Ссылка = &Ссылка
		               |;
		               |
		               |////////////////////////////////////////////////////////////////////////////////
		               |ВЫБРАТЬ
		               ...
		               |////////////////////////////////////////////////////////////////////////////////
		               |ВЫБРАТЬ
		               |	МатериалыСвернуто.Номенклатура,
		               |	МатериалыСвернуто.ХарактеристикаНоменклатуры,
		               |	МатериалыСвернуто.СерияНоменклатуры,
		               |	ПартииМатериаловВЭксплуатацииОстатки.ДокументПередачи КАК ДокументПередачи,
		               |	МатериалыСвернуто.ФизЛицо,
		               |	МатериалыСвернуто.Организация,
		               |	МатериалыСвернуто.СчетУчета,
		               |	МатериалыСвернуто.НазначениеИспользования,
		               ...
		               |	&Подразделение,
		               |	МатериалыСвернуто.ДокументПередачи КАК ДокументПередачиТЧ
		               |ИЗ
		               |	МатериалыСвернуто КАК МатериалыСвернуто
		               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ПартииМатериаловВЭксплуатации" + РазделУчета.Значение + ".Остатки(
		               |				&Период,
		               |				Подразделение = &Подразделение
		               |					И Организация = &Организация) КАК ПартииМатериаловВЭксплуатацииОстатки
		               |		ПО МатериалыСвернуто.Номенклатура = ПартииМатериаловВЭксплуатацииОстатки.Номенклатура
		               |			И МатериалыСвернуто.ХарактеристикаНоменклатуры = ПартииМатериаловВЭксплуатацииОстатки.ХарактеристикаНоменклатуры
		               |			И МатериалыСвернуто.СерияНоменклатуры = ПартииМатериаловВЭксплуатацииОстатки.СерияНоменклатуры
		               |			И МатериалыСвернуто.ФизЛицо = ПартииМатериаловВЭксплуатацииОстатки.ФизЛицо
		               |			И МатериалыСвернуто.НазначениеИспользования = ПартииМатериаловВЭксплуатацииОстатки.НазначениеИспользования
		               |			И МатериалыСвернуто.СчетУчета = ПартииМатериаловВЭксплуатацииОстатки.СчетУчета
		               |			И (ВЫБОР
		               |				КОГДА МатериалыСвернуто.ДокументПередачи = ЗНАЧЕНИЕ(Документ.ПередачаМатериаловВЭксплуатацию.ПустаяСсылка)
		               |						ИЛИ МатериалыСвернуто.ДокументПередачи = НЕОПРЕДЕЛЕНО
		               |						ИЛИ МатериалыСвернуто.ДокументПередачи ЕСТЬ NULL 
		               |					ТОГДА ИСТИНА
		               |				ИНАЧЕ МатериалыСвернуто.ДокументПередачи = ПартииМатериаловВЭксплуатацииОстатки.ДокументПередачи
		               |			КОНЕЦ)";
					   					   
		Если РазделУчета.Ключ = "НУ" Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "СчетУчетаБУ", "СчетУчетаНУ");
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "СчетУчетаПолучательБУ", "СчетУчетаПолучательНУ");
		КонецЕсли; 
		//АБС ВСТАВКА 35878  10.12.2013 13:25:00  Шамов
		Если ЭтоУУ Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "И МатериалыСвернуто.СчетУчета = ПартииМатериаловВЭксплуатацииОстатки.СчетУчета", "И ИСТИНА");
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "И Организация = &Организация", "И ИСТИНА");
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПеремещениеМатериаловВЭксплуатацииМатериалы.ПодразделениеОрганизацииПолучатель", "ПеремещениеМатериаловВЭксплуатацииМатериалы.ПодразделениеПолучатель");
		КонецЕсли;
		//АБС ВСТАВКА 35878 КОНЕЦ 
		Запрос.УстановитьПараметр("Ссылка",			СтруктураШапкиДокумента.Ссылка);
		Запрос.УстановитьПараметр("Организация",	СтруктураШапкиДокумента.Организация);
		//АБС ИЗМЕНЕНИЕ 35878  10.12.2013 14:05:16  Шамов
		...
		//НайденныеСтроки = ОстаткиПоРегистру.НайтиСтроки(Новый Структура("НедостаточноКоличестваПоРегистру", Истина));
		//
		//СтруктураИзмерений = Новый Структура("Номенклатура, ХарактеристикаНоменклатуры, СерияНоменклатуры, НазначениеИспользования, СчетУчета, ФизЛицо, Подразделение, Организация");
		//
		//Если НайденныеСтроки.Количество() > 0 Тогда //Недостаточно остатков
		//
		//	Для каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		//	
		//		ЗаполнитьЗначенияСвойств(СтруктураИзмерений, НайденнаяСтрока);
		//		СтрокаСообщения = "Для заданного набора ";
		//		
		//		Для каждого Измерение Из СтруктураИзмерений Цикл
		//		
		//			Если Не ЗначениеЗаполнено(Измерение.Значение) Тогда
		//				Продолжить;
		//			КонецЕсли; 
		...
		//Достаточно остатков по регистру
		Запрос.Текст = "ВЫБРАТЬ
		               |	Материалы.ПодразделениеПолучатель,
		               |	Материалы.Номенклатура,
		               |	Материалы.ХарактеристикаНоменклатуры,
		               |	Материалы.СерияНоменклатуры,
		               |	Материалы.ДокументПередачи,
		               |	Материалы.ФизЛицо,
		               |	Материалы.ФизЛицоПолучатель,
		               |	Материалы.НазначениеИспользования,
		               |	Материалы.СпособПогашенияСтоимости,
		               |	Материалы.СрокПолезногоИспользования,
		               |	Материалы.НазначениеИспользованияПолучатель,
		               |	Материалы.Количество,
		               |	Материалы.Сумма,
		               |	Материалы.Переносить,
		               |	Материалы.НомерСтроки КАК НомерСтроки,
		               |	&Подразделение,
		               |	Материалы.Организация,
		               |	Материалы.СчетУчета,
		               |	Материалы.СчетУчетаПолучатель
		               |ИЗ
		               |	Материалы КАК Материалы
		               |
		               |УПОРЯДОЧИТЬ ПО
		...
		//СтруктураЗаписи используется для создания прихода копией основных полей из сторно (расхода)
		СтруктураЗаписи = Новый Структура;
		Для каждого Колонка Из НаборДвижений.ВыгрузитьКолонки().Колонки Цикл
			СтруктураЗаписи.Вставить(Колонка.Имя);
		КонецЦикла;
		
		//Предполагается что все ресурсы кроме количества распределяются пропроционально количеству
		...
		//Перебираем строки которые нужно перенести
		Пока Выборка.Следующий() Цикл
		
			Если Не Выборка.Переносить
			...
			КонецЕсли;
			//АБС ВСТАВКА 35878  10.12.2013 13:56:56  Шамов
			Если ЭтоУУ Тогда
			...
			Для Каждого Остаток Из ОстатокСтроки Цикл
				...
				КонецЕсли; 
				// Start КТТК Ермолов Е.Л.  03.12.2015 000035644
				//Заполним приход на основаниии расхода
				ЗаполнитьЗначенияСвойств(СтруктураЗаписи, НовоеДвижение);
				НовоеДвижение = НаборДвижений.Добавить();
				ЗаполнитьЗначенияСвойств(НовоеДвижение, СтруктураЗаписи,, "ВидДвижения, СписаниеПартий");
				//НовоеДвижение.Период		   = СтруктураШапкиДокумента.Дата;
				//НовоеДвижение.Регистратор	   = СтруктураШапкиДокумента.Ссылка;
				//НовоеДвижение.Активность	   = Истина;
				
				СтруктураЗамены = Новый Структура("Подразделение, ФизЛицо, НазначениеИспользования, СчетУчета");
				//АБС ВСТАВКА 35878  10.12.2013 13:56:56  Шамов
				Если ЭтоУУ Тогда
					СтруктураЗамены.Удалить("СчетУчета");
				КонецЕсли;
				//АБС ВСТАВКА 35878 КОНЕЦ
				
				Для каждого ЭлементЗамены Из СтруктураЗамены Цикл
					НовоеДвижение[ЭлементЗамены.Ключ] = Выборка[ЭлементЗамены.Ключ + "Получатель"];
				КонецЦикла; 			
				// Stop КТТК Ермолов Е.Л.  03.12.2015
			КонецЦикла;
			...
			Если КоличествоОсталосьСписать > 0 Тогда			
				...
				СтрокаСообщения = СтрокаСообщения + " недостаточно остатков по регистру ""Партии материалов в эксплуатации " + РазделУчета.Значение + """""";
				//АБС ИЗМЕНЕНИЕ 35878  10.12.2013 13:27:05  Шамов
				//ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения, Отказ);
				Если ЭтоУУ ТОгда
					//нет партий в уу - проведение не отменяем
					//ибо бардак
					ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения);
					//!!! приходуем списанное количество по партиям УУ!!!
					Попытка
						ЗаполнитьЗначенияСвойств(СтруктураЗаписи, НовоеДвижение);
						НовоеДвижение = НаборДвижений.Добавить();
						ЗаполнитьЗначенияСвойств(НовоеДвижение, СтруктураЗаписи,, "ВидДвижения, СписаниеПартий");
						СтруктураЗамены = Новый Структура("Подразделение, ФизЛицо, НазначениеИспользования");
						Для каждого ЭлементЗамены Из СтруктураЗамены Цикл
							НовоеДвижение[ЭлементЗамены.Ключ] = Выборка[ЭлементЗамены.Ключ + "Получатель"];
						КонецЦикла; 			
					Исключение
			...
			//ЗаполнитьЗначенияСвойств(НовоеДвижение, Выборка);
			//ЗаполнитьЗначенияСвойств(СтруктураИзмерений, Выборка);
			//Остаток = ОстаткиПоРегистру.НайтиСтроки(СтруктураИзмерений)[0];
			//
			//НовоеДвижение.ДокументПередачи = Остаток.ДокументПередачи;
			//
			////Распределим сумму
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоМатериалам, Отказ, Заголовок)
	...
	//АБС
	ТабМат.Колонки.Удалить("МестоЭксплуатацииПоСооружению");
	ТабМат.Колонки["МестоЭксплуатацииПоСооружениюПолучатель"].Имя = "МестоЭксплуатацииПоСооружению";

	ТабМат.Колонки.Удалить("ФункциональнаяПринадлежность");
	ТабМат.Колонки["ФункциональнаяПринадлежностьПолучатель"].Имя = "ФункциональнаяПринадлежность";

	
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТабМат, ТаблицаДвижений);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	СтруктураПолей.Вставить( "ДокументПередачи", "ДокументПередачи");
	 		//АБС
	СтруктураПолей.Вставить( "МестоЭксплуатацииПоСооружению",        "МестоЭксплуатацииПоСооружению");
	СтруктураПолей.Вставить( "МестоЭксплуатацииПоСооружениюПолучатель",        "МестоЭксплуатацииПоСооружениюПолучатель");
	СтруктураПолей.Вставить( "ФункциональнаяПринадлежность",        "ФункциональнаяПринадлежность");
	СтруктураПолей.Вставить( "ФункциональнаяПринадлежностьПолучатель",        "ФункциональнаяПринадлежностьПолучатель");
	//\\АБС
