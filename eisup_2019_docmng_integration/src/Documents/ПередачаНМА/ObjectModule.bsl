...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если НЕ Перечисления.ВидыДоговоровКонтрагентов.СПокупателем = СтруктураШапкиДокумента.ВидДоговора Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("В документе ""Передача НМА"" могут использоваться только договоры c покупателем.",Отказ, Заголовок);
	КонецЕсли;
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	СтруктураПараметровЗачетАванса = БухгалтерскийУчетРасчетовСКонтрагентами.ПодготовкаСтруктурыПараметровДляЗачетаАванса(Ссылка, мВалютаРегламентированногоУчета, Заголовок,?(СуммаВключаетНДС, Сумма, Сумма + СуммаНДС));
