...
Процедура ПроверкаРеквизитов(Отказ, Заголовок, СтруктураШапкиДокумента) Экспорт
	...
	Для Каждого СтрокаТЧ Из ТаблицаМатериалов Цикл
		Если СтрокаТЧ.ХарактерЗатрат = Перечисления.ХарактерЗатрат.ПроизводственныеРасходы
		   И СтрокаТЧ.ВидЗатрат = Перечисления.ВидыЗатрат.Материальные Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя использовать материальные производственные статьи затрат """ + СтрокаТЧ.СтатьяЗатрат + """ (строка № " + СтрокаТЧ.НомерСтроки + " табличной части ""Затраты"")", Отказ, Заголовок);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(СтрокаТЧ.ХарактерЗатрат) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указан Хар-р затрат в статье затрат """ + СтрокаТЧ.СтатьяЗатрат + """ (строка № " + СтрокаТЧ.НомерСтроки + " табличной части ""Затраты"")", Отказ, Заголовок);
		КонецЕсли;
		Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете И СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
			Если СтрокаТЧ.Сумма = 0 И СтрокаТЧ.СуммаРегл = 0 Тогда
				Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете И СтрокаТЧ.СуммаНал = 0 Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указана сумма для статьи затрат """ + СтрокаТЧ.СтатьяЗатрат + """ (строка № " + СтрокаТЧ.НомерСтроки + " табличной части ""Затраты"")", Отказ, Заголовок);
				Иначе
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указана сумма для статьи затрат """ + СтрокаТЧ.СтатьяЗатрат + """ (строка № " + СтрокаТЧ.НомерСтроки + " табличной части ""Затраты"")", Отказ, Заголовок);
				КонецЕсли;
			КонецЕсли;
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		Если Не СписСчетовБух.НайтиПоЗначению(Счет) = Неопределено Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя указывать счет " + Счет + " в качестве корреспонденции! (счет бух. учета)", Отказ, Заголовок);
		КонецЕсли;
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			...
			Если Не СписСчетовНал.НайтиПоЗначению(СчетНУ) = Неопределено Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя указывать счета " + СчетНУ + " в качестве корреспонденции! (счет нал. учета)", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСубконтоРегл(Отказ, Заголовок, СтруктураШапкиДокумента)
	...
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
		// Допускается не указывать счет кредита, но если указали - должно быть все корректно
		Если ЗначениеЗаполнено(СчетНУ) Тогда
			СтруктураПараметров = Новый Структура("СчетНУ, СубконтоНУ1, СубконтоНУ2, СубконтоНУ3",
				"Не выбран счет кредита (НУ)",
				"Не указано субконто 1 кредита счета (НУ)",
				"Не указано субконто 2 кредита счета (НУ)",
				"Не указано субконто 3 кредита счета (НУ)");
	...
	Для Каждого СтрокаТабличнойЧасти Из Затраты Цикл
		...
		Если ОтражатьВБухгалтерскомУчете Тогда
			...
			Если ВидОперации = Перечисления.ВидыОперацийПрочиеЗатраты.Отражение Тогда
				...
				// АБС ВСТАВКА Фролов 20110919 Добавлена возможность получения подразделения из табличной части документа
				//УправлениеЗатратами.ЗаполнитьСчетИСубконтоУчетаЗатрат(НоваяПроводка, СтрокаТабличнойЧасти, СтруктураШапкиДокумента);
				УправлениеЗатратами.ЗаполнитьСчетИСубконтоУчетаЗатрат(НоваяПроводка, СтрокаТабличнойЧасти, СтруктураШапкиДокумента,,,Истина,"абс_ПодразделениеОрганизации");
		...
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			...
			Если СтрокаТабличнойЧасти.СуммаНал <> 0 Тогда
				...
				Если ВидОперации = Перечисления.ВидыОперацийПрочиеЗатраты.Отражение Тогда
					...
					УправлениеЗатратами.ЗаполнитьСчетИСубконтоУчетаЗатрат(НоваяПроводкаНУ, СтрокаТабличнойЧасти, СтруктураШапкиДокумента,"Налоговый",,Истина,"абс_ПодразделениеОрганизации");				
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// АБС ВСТАВКА Фролов Проведение сторно затрат 20111118
	Если ЗначениеЗаполнено(абс_КодОперацииНЗП) Тогда
		СтруктураШапкиДокумента.Вставить("КодОперацииНЗП", абс_КодОперацииНЗП);
	КонецЕсли;
	// АБС ВСТАВКА Фролов Проведение сторно затрат 20111118 КОНЕЦ
	...
	//	Если БухгалтерскийУчет.ЭтоСубсчет(Счет,ПланыСчетов.Хозрасчетный.РасчетыСРазнымиДебиторамиИКредиторами) Тогда	
	//		СообщениеПриОтказе = "";
	...
	// АБС Фролов для проведения по бюджетировани
