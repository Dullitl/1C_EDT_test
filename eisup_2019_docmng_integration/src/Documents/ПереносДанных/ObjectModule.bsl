...
Процедура ПередЗаписью(Отказ)
	...
	// нач АБС ВСТАВКА Нарыжный 15.04.2014 Филиализация
	Если абс_ТипПереносаОстатков = Перечисления.абс_ТипыПереносаОстатков.ВводОстатков И Не ЗначениеЗаполнено(абс_КорректировкаЗаписейРегистров) Тогда
		НовыйДокументКорректировка = Документы.КорректировкаЗаписейРегистров.СоздатьДокумент();
		НовыйДокументКорректировка.абс_Заблокирована = Истина;
		НовыйДокументКорректировка.Дата = ЭтотОбъект.Дата;
		//НовыйДокументКорректировка.абс_ОрганизацияИсточник = ЭтотОбъект.абс_ДЗО;
		НовыйДокументКорректировка.Ответственный = глЗначениеПеременной("глТекущийПользователь");
		НовыйДокументКорректировка.Записать();
		НовыйДокументКорректировка.УстановитьНовыйНомер();
		ЭтотОбъект.абс_КорректировкаЗаписейРегистров = НовыйДокументКорректировка.Ссылка;		
	КонецЕсли;	
	// кон АБС ВСТАВКА Нарыжный 15.04.2014 Филиализация
...
Процедура ПриЗаписи(Отказ)
	...
	НаборЗаписейСоответствиеОбъектовИнформационнойБазы = ЭтотОбъект.Движения.абс_СоответствиеОбъектовИнформационнойБазы;
	НаборЗаписейСоответствиеОбъектовИнформационнойБазы.Очистить();
	Для  Каждого СтрокаСоответствиеОбъектов Из ЭтотОбъект.абс_СоответствиеОбъектов Цикл
		Если Не ЗначениеЗаполнено(СтрокаСоответствиеОбъектов.Приемник) Тогда 
			Продолжить;
		КонецЕсли;	
		Движение = НаборЗаписейСоответствиеОбъектовИнформационнойБазы.Добавить();
		//Движение.Период = ЭтотОбъект.Дата;
		Движение.Источник = СтрокаСоответствиеОбъектов.Источник;
		Движение.Приемник = СтрокаСоответствиеОбъектов.Приемник;
		Движение.ДЗО = ЭтотОбъект.абс_ДЗО;
		Движение.Филиал = ЭтотОбъект.абс_Филиал;
	КонецЦикла;
	НаборЗаписейСоответствиеОбъектовИнформационнойБазы.Записать();
КонецПроцедуры
// кон АБС ВСТАВКА Нарыжный 15.04.2014 Филиализация
////////////////////////////////////////////////////////////////////////////////
