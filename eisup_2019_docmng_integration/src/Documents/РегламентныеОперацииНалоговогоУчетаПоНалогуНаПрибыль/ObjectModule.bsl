...
Процедура НормированиеРасходовПоПроценту(СтруктураШапкиДокумента, ВидЗатрат, Норма, Процент)
	...
	Запрос.УстановитьПараметр("СтатьиЗатрат", Неопределено);
	...
	Для Каждого Строка из ТаблицаРасходов Цикл
		...
		Если Строка.МожноСписать > 0 Тогда
			...
			Проводка.СчетКт       = Строка.СчетУчета;
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "СтатьиЗатрат", Строка.СтатьяЗатрат);
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Подразделения", Строка.Подразделение);//абсо
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "ВидыДеятельности", Справочники.абс_ВидыДеятельностиКТТК.НайтиПоНаименованию(СокрЛП(Строка.Подразделение.Наименование)));//абсо
...
Процедура НормированиеРасходовПоРаботникуОрганизации(СтруктураШапкиДокумента, ВидЗатрат)
	...
	Запрос.УстановитьПараметр("КонГраница",   Новый Граница(КонецПериода, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("Организация",  СтруктураШапкиДокумента.Организация);
	Запрос.УстановитьПараметр("СтатьиЗатрат", ТаблицаРасходов.ВыгрузитьКолонку("СтатьяЗатрат"));
	...
	Для Каждого Расход из ТаблицаРасходов Цикл
		...
		Пока Выборка.НайтиСледующий(СтруктураПоиска) Цикл
			...
			Расход.ОборотКтПР   = Расход.ОборотКтПР + ?(Выборка.ОборотКтПР = Null, 0, Выборка.ОборотКтПР);
			Расход.ОборотКт     = Расход.ОборотКт   + ?(Выборка.ОборотКт = Null, 0, Выборка.ОборотКт);
		...
		Пока Выборка.НайтиСледующий(СтруктураПоиска) Цикл
			...
			НоваяСтрока.СчетЗатратНУ = Выборка.Счет;
			...
			МожноСписать = Выборка.СуммаПриход - Выборка.ОборотКт; // Остаток суммы, который может быть списан.
			СуммаСписания = Мин(МожноСписать, ВсегоМожноСписать);
			
			НоваяСтрока.СуммаНал = СуммаСписания;
			НоваяСтрока.ВременнаяРазница = 0;
			НоваяСтрока.ПостояннаяРазница = МожноСписать - СуммаСписания - Выборка.ОборотКтПР;
...
Процедура ДвиженияПоРегистрам(СтруктураШапкиДокумента, Отказ, Заголовок, мУчетнаяПолитикаРегл)
	...
	Если  СтруктураШапкиДокумента.НормированиеРасходовНаДобровольноеСтрахованиеИВозмещениеПроцентов Тогда
		...
		Если СтруктураШапкиДокумента.Дата < Дата("20090101") Тогда
			НормированиеРасходовПоПроценту(СтруктураШапкиДокумента, Перечисления.ВидыРасходовНУ.ДобровольноеЛичноеСтрахование, ОпределитьНормуПоРасходамНаОплатуТруда(СтруктураШапкиДокумента, 3, Ложь), 3);
			НормированиеРасходовПоПроценту(СтруктураШапкиДокумента, Перечисления.ВидыРасходовНУ.РасходыНаВозмещениеЗатратРаботниковПоУплатеПроцентов, ОпределитьНормуПоРасходамНаОплатуТруда(СтруктураШапкиДокумента, 0, Ложь), 0);
		Иначе
			НормированиеРасходовПоПроценту(СтруктураШапкиДокумента, Перечисления.ВидыРасходовНУ.ДобровольноеЛичноеСтрахование, ОпределитьНормуПоРасходамНаОплатуТруда(СтруктураШапкиДокумента, 6, Ложь), 6);
			Если СтруктураШапкиДокумента.Дата < Дата("20110101") Тогда
			НормированиеРасходовПоПроценту(СтруктураШапкиДокумента, Перечисления.ВидыРасходовНУ.РасходыНаВозмещениеЗатратРаботниковПоУплатеПроцентов, ОпределитьНормуПоРасходамНаОплатуТруда(СтруктураШапкиДокумента, 3, Ложь), 3);
			КонецЕсли;
		КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ)
	...
	Если Проверить_ДокументВыполняетУдаленныеОперации() Тогда
		ТекстСообщения = "Документ содержит проводки, которые теперь формируются другими регламентными документами."+Символы.ПС+Символы.Таб+
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ИмяМакета = "НормированиеРасходов" Тогда
		//АБС ВСТАВКА 47730  09.10.2014 10:33:26  Хазеев
		...
		ВремОрганизация = Новый СписокЗначений;
		ВремОрганизация.Добавить(Организация);
		БухгалтерскийУчет.ПечатьСправкиРасчета(Отчеты.СправкаРасчетНормированияРасходов, Новый Структура("Организация, Дата", ВремОрганизация, Дата));
		//АБС ВСТАВКА 47730 КОНЕЦ 
	КонецЕсли;
