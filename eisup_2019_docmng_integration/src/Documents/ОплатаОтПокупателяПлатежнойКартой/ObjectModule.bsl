...
Процедура ДвиженияПоРегистрамРегл(Режим, Отказ, Заголовок, СтруктураШапкиДокумента)
	...
	Иначе
		ttk_ОбщегоНазначения.СообщитьОбОшибке("На выбранном счете расчетов с эквайрером отсутствует аналитика по контрагентам!",Отказ, Заголовок);
	КонецЕсли;
	...
	Иначе
		ttk_ОбщегоНазначения.СообщитьОбОшибке("На выбранном счете расчетов с эквайрером отсутствует аналитика по договорам!",Отказ, Заголовок);
	КонецЕсли;	
...
Процедура ПроверитьЗаполнениеДокументаУпр(Отказ, Заголовок)
	...
	Если ВидОплаты.ТипОплаты <> Перечисления.ТипыОплатЧекаККМ.ПлатежнаяКарта Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("В документе выбран вид оплаты неверного типа!", Отказ, Заголовок);
	КонецЕсли;
