...
Процедура ПроверитьПравильностьЗаполненияДокумента() Экспорт
	...
	Для Каждого ДанныеСтроки Из СоставСмены Цикл
		...
		Если ДанныеСтроки.Сотрудник.Пустая() Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(НСтр(НачалоСообщения + "не указан сотрудник'"), Ложь, ЗаголовокСообщения, СтатусСообщения.Информация);
			Продолжить;
		КонецЕсли;
		...
		Если ПустаяСтрока(ДанныеСтроки.Примечание) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(НСтр(НачалоСообщения + "не указана причина неявки'"), Ложь, ЗаголовокСообщения, СтатусСообщения.Информация);
		КонецЕсли;
		
		Если ДанныеСтроки.ЗамещающийСотрудник.Пустая() Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(НСтр(НачалоСообщения + "не указан замещающий сотрудник'"), Ложь, ЗаголовокСообщения, СтатусСообщения.Информация);
		Иначе
			// проверим, не замещает ли сотрудник сам себя
			Если ДанныеСтроки.ЗамещающийСотрудник = ДанныеСтроки.Сотрудник Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(НСтр(НачалоСообщения + "указано, что сотрудник замещает сам себя'"), Ложь, ЗаголовокСообщения, СтатусСообщения.Информация);
			Иначе
				...
				Если ЗамещающийСотрудник = Неопределено Тогда
					...
					ttk_ОбщегоНазначения.СообщитьОбОшибке(НСтр(НачалоСообщения + "не отмечена явка указанного замещающего сотрудника'"), Ложь, ЗаголовокСообщения, СтатусСообщения.Информация);
					
				ИначеЕсли НЕ ЗамещающийСотрудник.Явка Тогда
					
					ttk_ОбщегоНазначения.СообщитьОбОшибке(НСтр(НачалоСообщения + "не отмечена явка указанного замещающего сотрудника (строка "+ЗамещающийСотрудник.НомерСтроки+", "+ЗамещающийСотрудник.Сотрудник+")'"), Ложь, ЗаголовокСообщения, СтатусСообщения.Информация);
	...
	Если НЕ Ответственный.ФизЛицо.Пустая() Тогда
		...
		Если СоставСмены.Найти(Ответственный.ФизЛицо, "Сотрудник") = Неопределено Тогда
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru = 'Ответственный не указан в составе смены'"), Ложь, ЗаголовокСообщения, СтатусСообщения.Информация);
		КонецЕсли;
...
Функция МожноЗаписатьДокумент(Заголовок)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщениеОбОшибке, Отказ, Заголовок);
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ДатаСмены = ?(ЗначениеЗаполнено(Дата), Дата, ОбщегоНазначенияЗК.ПолучитьРабочуюДату());
