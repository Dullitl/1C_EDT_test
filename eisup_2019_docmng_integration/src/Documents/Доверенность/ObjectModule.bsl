...
Функция ПечатьДоверенности(НазваниеМакета)
	...
	//АБС задача 
	Попытка
		ПаспортФизЛица        = абс_СерверныеФункции.ПаспортныеДанные(Шапка.ФизЛицо.Код);
	Исключение
	КонецПопытки;
	//АБС задача 
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
    ...
    Если ИмяМакета = "М2" Тогда
		ТабДокумент = ПечатьДоверенности(ИмяМакета);
	ИначеЕсли ИмяМакета = "М2а" Тогда
		ТабДокумент = ПечатьДоверенности(ИмяМакета);
...
Процедура ЗаполнитьТоварыПоОстаткам() Экспорт
	...
	|ИЗ
	|	РегистрНакопления.ЗаказыПоставщикам.Остатки(&ДатаОстатков, ДоговорКонтрагента = &Договор И ЗаказПоставщику = &Сделка) КАК Остатки
	|
	|ГДЕ
