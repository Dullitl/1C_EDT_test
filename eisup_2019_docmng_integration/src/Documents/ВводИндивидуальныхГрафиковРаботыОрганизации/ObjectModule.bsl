...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяМакета) Тогда
		...
		|	ГрафикРаботы.Сотрудник.Наименование КАК Наименование
		|ИЗ
	...
	Если ТабДокумент = Неопределено Тогда
		Возврат Неопределено;
	Иначе 	
		Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,Метаданные().Синоним + "(" + НРег(ОписаниеПечатнойФормы) + ")"));
	КонецЕсли;
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	ВводИндивидуальныхГрафиковРаботыОрганизацииПереопределяемый.ВнестиПечатныеФормы(СтруктураМакетов);
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ)
	...
	КонецЕсли;
	 ...
	 Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ГоловнаяОрганизация) Тогда
		СписокСообщений.Добавить(Перечисления.ВидыСообщений.Ошибка, ОбщегоНазначения.ПреобразоватьСтрокуИнтерфейса("Не указана организация, по которой вводится индивидуальный график!"));
		Отказ = Истина;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиГрафикРаботы(ВыборкаПоСтрокамДокумента, Отказ)
	...
	Если ВыборкаПоСтрокамДокумента.ОшибкаНеСоответствиеСотрудникаИОрганизации Тогда
		СписокСообщений.Добавить(Перечисления.ВидыСообщений.Ошибка, СтрокаНачалаСообщенияОбОшибке + ОбщегоНазначения.ПреобразоватьСтрокуИнтерфейса("указанный сотрудник оформлен на другую организацию!"));
		Отказ = Истина;
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если СписокСообщений.Количество() > 0 Тогда
		...
		#Иначе
			Для каждого ЭлементСписка Из СписокСообщений Цикл
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ЭлементСписка.Представление, Отказ, Заголовок);
			КонецЦикла; 
		#КонецЕсли 
