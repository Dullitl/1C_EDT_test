...
// +++ввв 06.09.2016 г.
	...
	Запрос.Текст="ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ ПЕРВЫЕ 1
	             ...
	             |			ИНАЧЕ """"
	             |		КОНЕЦ, ОтветственныеЛицаСрезПоследних.ФизическоеЛицо.Наименование) КАК ФИОПолное
	             |ИЗ
	             |	РегистрСведений.ОтветственныеЛица.СрезПоследних(&ДатаДока, СтруктурнаяЕдиница = &Касса) КАК ОтветственныеЛицаСрезПоследних
	             |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц.СрезПоследних(&ДатаДока, ФизЛицо ССЫЛКА Справочник.ФизическиеЛица) КАК ФИОФизЛицСрезПоследних
	             |		ПО ОтветственныеЛицаСрезПоследних.ФизическоеЛицо = ФИОФизЛицСрезПоследних.ФизЛицо";
	Запрос.УстановитьПараметр("Касса", ЭтотОбъект.Касса);
	Запрос.УстановитьПараметр("ДатаДока", ЭтотОбъект.Дата);
	Кассиры=Запрос.Выполнить().Выбрать();
	Если Кассиры.Следующий() Тогда
		Кассир = Кассиры.ФИОПолное;
	КонецЕсли;
// ---ввв 06.09.2016 г.
...
Функция ОпределитьКодыСчетСубсчет() 
	...
	Если КассаВВалюте Тогда
		СчетДт = абс_СчетКассы.Код;//ПланыСчетов.Хозрасчетный.КассаОрганизацииВал.Код;
		РасчетыВВалюте = Истина;
	Иначе
		СчетДт = абс_СчетКассы.Код;//ПланыСчетов.Хозрасчетный.КассаОрганизации.Код;
		РасчетыВВалюте = Ложь;
	КонецЕсли;
...
Процедура ДвиженияПоРегистрамРегл(СтруктураШапкиДокумента, Режим, Отказ, Заголовок)
	...
	ПараметрыУчетнойПолитики = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(СтруктураШапкиДокумента.Дата, СтруктураШапкиДокумента.Организация);
	...
	Если РасчетыВВалюте Тогда
		СчетДт = абс_СчетКассы;//ПланыСчетов.Хозрасчетный.КассаОрганизацииВал;
	Иначе
		СчетДт = абс_СчетКассы;//ПланыСчетов.Хозрасчетный.КассаОрганизации;
	КонецЕсли;
	...
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПКО.ПриходДенежныхСредствРозничнаяВыручка Тогда
		...
		Если СтруктураШапкиДокумента.ВыручкаСНТТ Тогда
			...
			Если СуммаПослеРаспределения > 0 Тогда
				...
				Если СчетДт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("50.02") Тогда
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,2,абс_КассаККМ);
				КонецЕсли;
	...
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПКО.РасчетыПоКредитамИЗаймамСРаботниками Тогда
		...
		Если НЕ СтруктураДолг.Проценты = 0 Тогда
			...
			Проводка.Сумма = СтруктураДолг.Проценты;
	...
	Иначе
		...
		Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПКО.ВозвратДенежныхСредствПодотчетником Тогда
			...
			Проводка.СубконтоКт.РаботникиОрганизации = СтруктураШапкиДокумента.Контрагент;
			Проводка.СубконтоКт.ВидыПодотчетныхСумм = СтруктураШапкиДокумента.ВидПодотчетнойСуммы;
	...
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете тогда
		...
		Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПКО.ВозвратДенежныхСредствПодотчетником Тогда
			...
			ПроводкаНУ.СчетКт  = ?(РасчетыВВалюте,ПланыСчетов.Налоговый.НайтиПоКоду("71.21"),ПланыСчетов.Налоговый.НайтиПоКоду("71.01"));
			ПроводкаНУ.СубконтоКт.РаботникиОрганизации = СтруктураШапкиДокумента.Контрагент;
			ПроводкаНУ.СубконтоКт.ВидыПодотчетныхСумм = СтруктураШапкиДокумента.ВидПодотчетнойСуммы;
			ПроводкаНУ.Сумма = Проводка.Сумма;
			
		ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПКО.ПриходДенежныхСредствПрочее 
			И СчетУчетаРасчетовСКонтрагентом.Родитель = ПланыСчетов.Хозрасчетный.РасчетыСПерсоналомПоПрочимОперациям Тогда //73й
			
			ПроводкаНУ=ПроводкиНУ.Добавить();
			ПроводкаНУ.Период      = СтруктураШапкиДокумента.Дата;
			ПроводкаНУ.Активность  = Истина;
			ПроводкаНУ.Организация = СтруктураШапкиДокумента.Организация;
			ПроводкаНУ.СчетКт =  БухгалтерскийУчет.ПреобразоватьСчетаБУвСчетНУ(Новый Структура("СчетБУ",  СчетУчетаРасчетовСКонтрагентом), "НУ",  Дата);
         	ПроводкаНУ.Содержание = "Прочие поступления";
			
			БухгалтерскийУчет.УстановитьСубконто(ПроводкаНУ.СчетКт,ПроводкаНУ.СубконтоКт,1,СтруктураШапкиДокумента.СубконтоКт1);
			БухгалтерскийУчет.УстановитьСубконто(ПроводкаНУ.СчетКт,ПроводкаНУ.СубконтоКт,2,СтруктураШапкиДокумента.СубконтоКт2);
			БухгалтерскийУчет.УстановитьСубконто(ПроводкаНУ.СчетКт,ПроводкаНУ.СубконтоКт,3,СтруктураШапкиДокумента.СубконтоКт3);
			ПроводкаНУ.Сумма = Проводка.Сумма;
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеДокументаУпр(Отказ, Заголовок)
	...
	Если (НЕ ОтраженоВОперУчете) И (НЕ Оплачено) И ОтражатьВУправленческомУчете Тогда
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не выбрано правило проведения (""Отражено в опер. учете"",""Оплачено"")", Отказ, Заголовок);
	...
	Если (ОтражатьВУправленческомУчете И (ОтраженоВОперУчете ИЛИ (ОтражатьВБухгалтерскомУчете И Оплачено)))
		...
		ИначеЕсли (ВидОперации=Перечисления.ВидыОперацийПКО.РасчетыПоКредитамИЗаймамСРаботниками	
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указана сумма взаиморасчетов с физическим лицом.",Отказ, Заголовок);
	...
	Если ВидОперации = Перечисления.ВидыОперацийПКО.ПриходДенежныхСредствРозничнаяВыручка 
		...
		Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Прием розничной выручки производится только в валюте регламентированного учета.", Отказ, Заголовок);
	КонецЕсли;
	...
	Если ВидОперации = Перечисления.ВидыОперацийПКО.РасчетыПоКредитамИЗаймамСРаботниками 
		...
		Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Организации в документе и в договоре займа не совпадают.", Отказ, Заголовок);
