...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ДатаНачала > ДатаОкончания И ЗначениеЗаполнено(ДатаОкончания) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Дата начала действия скидки не может быть больше даты окончания действия скидки" , Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеТабличнойЧастиЦеновыеГруппы(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ЦеновыеГруппы.Количество() = 0 Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Необходимо заполнить перечень ценовых групп!", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеТабличнойЧастиБонусы(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если Бонусы.Количество() = 0 Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Необходимо заполнить перечень бонусов!", Отказ, Заголовок);
	КонецЕсли;
		...
		Если (ЗначениеЗаполнено(СтрокаБонус.Номенклатура)
		 ...
		 Или (ЗначениеЗаполнено(СтрокаБонус.СпецПредложение)
		   И СтрокаБонус.СпецПредложение.Услуга) Тогда
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "содержится услуга. Услуг здесь быть не должно!", Отказ, Заголовок);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СтрокаБонус Из Бонусы Цикл
		Если СтрокаБонус.Номенклатура.Комплект Тогда
			Если СоставСтрокиНеЗаполнен(СтрокаБонус.КлючСтроки - 1) Тогда
				...
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не задан состав!", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если Не РассчитыватьАвтоматическиеСкидки(ТекВид) Тогда
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("В учетной политике не указано использование скидок этого вида ("+ НаименованиеВида + ")."
	...
	Если ВидОперации = Перечисления.ВидыОперацийУстановкаСкидокНоменклатуры.ПоНоменклатуре
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Необходимо заполнить перечень товаров или установить флажок ""Для всей номенклатуры""!", Отказ, Заголовок);
	КонецЕсли;

	Если ПолучателиСкидки.Количество() = 0 И Не ДляВсехПолучателей Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Необходимо заполнить перечень получателей скидки или установить флажок ""Для всех получателей""!", Отказ, Заголовок);
	КонецЕсли;
