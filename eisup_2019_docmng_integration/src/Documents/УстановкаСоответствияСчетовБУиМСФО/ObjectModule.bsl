...
Процедура АБС_ЗаполнитьПоУмолчанию(Включать02Счет = Истина) Экспорт
	ЗагружатьТолько01_02Счета = Истина;
	Макет = ПолучитьМакет("ТиповоеСоответствие2");
	Для й = 2 по 10000 Цикл
		
		КодСчетаБУ = СокрЛП(Макет.Область(й,2,й,2).Текст);
		ВидДвижения = СокрЛП(Макет.Область(й,3,й,3).Текст);
		СчетМежд = СокрЛП(Макет.Область(й,7,й,7).Текст);
		//Если ЗагружатьТолько01_02Счета Тогда
		////Будем устанавливать соответствие только с 0-вым счетом!
		//СчетМежд = "000";
		//КонецЕсли;
		
		РеквизитПредставление = СокрЛП(Макет.Область(й,11,й,11).Текст);
		Реквизит = СокрЛП(Макет.Область(й,13,й,13).Текст);
		Значение = СокрЛП(Макет.Область(й,12,й,12).Текст);
		
		Если ПустаяСтрока(КодСчетаБУ) Тогда
			прервать;
		КонецЕсли;
		
		НоваяСтрока = ЭтотОбъект.СоответствиеСчетовБУиМСФО.Добавить();
		НоваяСтрока.Учитывается=Истина;
		НоваяСтрока.ВидДвижения = ПолучитьПеречисление(ВидДвижения,"ВидыДвиженийБухгалтерии");
		НоваяСтрока.СчетХозрасчетный=ПланыСчетов.Хозрасчетный.НайтиПоКоду(КодСчетаБУ);
		НоваяСтрока.СчетМеждународный=ПланыСчетов.Международный.НайтиПоКоду(счетМежд);
		Если Не ПустаяСтрока(Реквизит) Тогда
				НаименованиеПеречисления =  СокрЛП(Макет.Область(й,14,й,14).Текст);
				НоваяСтрока.Реквизит=Реквизит;
				НоваяСтрока.РеквизитПредставление=РеквизитПредставление;
				Если ПустаяСтрока(НаименованиеПеречисления) или (НаименованиеПеречисления="Ошибка") Тогда
					сообщить("В строке "+НоваяСтрока.НомерСтроки+" неверно заполнено значение реквизита!",СтатусСообщения.ОченьВажное);
				Иначе
					НоваяСтрока.Значение=ПолучитьПеречисление(Значение,НаименованиеПеречисления);
				КонецЕсли;
		КонецЕсли;
		
		НоваяСтрока.СубконтоМежд1=Неопределено;
		НоваяСтрока.СубконтоМежд2=Неопределено;
		НоваяСтрока.СубконтоМежд3=Неопределено;
		НоваяСтрока.СубконтоХозр1=Неопределено;
		НоваяСтрока.СубконтоХозр2=Неопределено;
		НоваяСтрока.СубконтоХозр3=Неопределено;
	КонецЦикла;
	
	//Выборка = Справочники.ЦелиИспользования.Выбрать();
	...
	//			Сообщить("В справочнике ""Цели использования"" обнаружен непредопределенный элемент с кодом "+Выборка.Код);
	//			Сообщить("Данный элемент необходимо удалить из информационной базы");
	//			продолжить;
	//		КонецЕсли;
	//		//Константы.РедактированиеЦелиИспользования.Установить(1);
	//		Объект = Выборка.ПолучитьОбъект();
	//		Объект.УстановитьПометкуУдаления(Истина);
	//		//Константы.РедактированиеЦелиИспользования.Установить(0);
	//		Сообщить("В справочнике ""Цели использования"" обнаружен непредопределенный элемент с кодом "+Выборка.Код);
	//		Сообщить("Данный элемент необходимо удалить из информационной базы");
	//		продолжить;
	//	КонецЕсли;	
	...
	//	НоваяСтрока = ЭтотОбъект.СоответствиеСчетовБУиМСФО.Добавить();
	//	счетМежд = Справочники.ЦелиИспользования.ПолучитьИмяПредопределенного(Выборка.Ссылка);
	//	НоваяСтрока.Учитывается=Истина;
	//	НоваяСтрока.ВидДвижения = Перечисления.ВидыДвиженийБухгалтерии.Дебет;
	//	НоваяСтрока.СчетХозрасчетный=ПланыСчетов.Хозрасчетный.НайтиПоКоду("01.01");
	//	СчетМУ = Неопределено;
	//	Попытка
	//		СчетМУ = ПланыСчетов.Международный[СчетМежд];
	//	Исключение
	//	КонецПопытки;
	...
	//	КонецЕсли;
	//	
	//	НоваяСтрока.СчетМеждународный=СчетМУ;
	//	НоваяСтрока.Реквизит="Субконто1.ЦельИспользования";
	//	НоваяСтрока.РеквизитПредставление="Основные средства/Цель использования";
	...
	//	НоваяСтрока.СубконтоХозр3=Неопределено;
		...
		//НоваяСтрока.Учитывается=Истина;
		//НоваяСтрока.ВидДвижения = Перечисления.ВидыДвиженийБухгалтерии.Дебет;
		//НоваяСтрока.СчетХозрасчетный=ПланыСчетов.Хозрасчетный.НайтиПоКоду("01.03");
		//НоваяСтрока.СчетМеждународный=ПланыСчетов.Международный[СчетМежд];
		//НоваяСтрока.Реквизит="Субконто1.ЦельИспользования";
		//НоваяСтрока.РеквизитПредставление="Основные средства/Цель использования";
		//НоваяСтрока.Значение=Выборка.Ссылка;
		//
		...
		//АБС
		//Если Включать02Счет Тогда
		//	//амортизация!
		//	НоваяСтрока = ЭтотОбъект.СоответствиеСчетовБУиМСФО.Добавить();
		//	счетМежд = "Амортизация"+Справочники.ЦелиИспользования.ПолучитьИмяПредопределенного(Выборка.Ссылка);
		//	НоваяСтрока.Учитывается=Истина;
		//	НоваяСтрока.ВидДвижения = Перечисления.ВидыДвиженийБухгалтерии.Кредит;
		//	НоваяСтрока.СчетХозрасчетный=ПланыСчетов.Хозрасчетный.НайтиПоКоду("02.01");
		//	НоваяСтрока.СчетМеждународный=ПланыСчетов.Международный[счетМежд];
		//	НоваяСтрока.Реквизит="Субконто1.ЦельИспользования";
		//	НоваяСтрока.РеквизитПредставление="Основные средства/Цель использования";
		//	НоваяСтрока.Значение=Выборка.Ссылка;
		...
		//	//НоваяСтрока.Учитывается=Истина;
		//	//НоваяСтрока.ВидДвижения = Перечисления.ВидыДвиженийБухгалтерии.Кредит;
		//	//НоваяСтрока.СчетХозрасчетный=ПланыСчетов.Хозрасчетный.НайтиПоКоду("02.03");
		//	//НоваяСтрока.СчетМеждународный=ПланыСчетов.Международный[счетМежд];
		//	//НоваяСтрока.Реквизит="Субконто1.ЦельИспользования";
		//	//НоваяСтрока.РеквизитПредставление="Основные средства/Цель использования";
		//	//НоваяСтрока.Значение=Выборка.Ссылка;
