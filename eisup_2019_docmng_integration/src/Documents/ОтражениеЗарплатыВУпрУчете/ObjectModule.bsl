...
Функция ПодготовитьВременныеТаблицы(АктуальныйПериодРегистрации)
	...
	Если мВалютаРегламентированногоУчета <> мВалютаУправленческогоУчета Тогда
		...
		Если Не ЗначениеЗаполнено(СрезКурсаВалюты.Курс) ИЛИ Не ЗначениеЗаполнено(СрезКурсаВалюты.Кратность) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не задан курс " + мВалютаУправленческогоУчета + " для расчетов с персоналом на " + ПредставлениеПериода(НачалоМесяца(ПериодРегистрации), КонецМесяца(ПериодРегистрации))+ "!");
	...
	|ИЗ
	|	РегистрСведений.УчетнаяПолитикаОрганизаций.СрезПоследних(&парамКонецПериодаРегистрации, ) КАК УчетнаяПолитикаНалоговыйУчет
	|ГДЕ
	|	ВЫБОР
	|		КОГДА УчетнаяПолитикаНалоговыйУчет.СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.Упрощенная)
	|			ТОГДА ИСТИНА
	...
	|ИЗ
	|	РегистрСведений.УчетнаяПолитикаОрганизаций.СрезПоследних(&парамКонецПериодаРегистрации, ) КАК УчетнаяПолитикаНалоговыйУчетСрезПоследних
	|ГДЕ
	|	УчетнаяПолитикаНалоговыйУчетСрезПоследних.УплачиватьДобровольныеВзносыВФСС";
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаОрганизаций.СрезПоследних(&парамКонецПериодаРегистрации, ) КАК УчетнаяПолитикаНалоговыйУчет
...
Процедура ПроверитьЗаполнениеШапки(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ПериодРегистрации) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не задан период за который выполняется отражение начислений учете!", Отказ);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиДохода(ВыборкаПоДоходам, Отказ)
	...
	Если ВыборкаПоДоходам.Подразделение.Пустая() Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не указано подразделение!", Отказ);
	КонецЕсли;
	
	Если ВыборкаПоДоходам.СтатьяЗатрат = Справочники.СтатьиЗатрат.ПустаяСсылка() Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не указана статья затрат!", Отказ);
	КонецЕсли;
	...
	Если ВыборкаПоДоходам.ХарактерЗатрат = Перечисления.ХарактерЗатрат.ВложенияВоВнеоборотныеАктивы и 
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не задан объект строительства!", Отказ);
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоДоходам,ТаблицаЗатрат);
