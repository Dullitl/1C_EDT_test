...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ТипЗнч(ТабДокумент) = Тип("Массив") Тогда
		Для К = 0 По ТабДокумент.ВГраница() Цикл
			УниверсальныеМеханизмы.НапечататьДокумент( ТабДокумент[К], КоличествоЭкземпляров, НаПринтер, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект), ТабДокумент[К]);
		КонецЦикла;
	Иначе
			   ...
			   ИЛИ НЕ ЗначениеЗаполнено(СтрокаСпец.ХарактеристикаНоменклатуры)) Тогда
				...
				Иначе
					...
					КолВыпуск = (КолВыпускБезНаработки   * СтрокаТЧ.Коэффициент)
							  / (СтрокаСпец.Количество * СтрокаСпец.ЕдиницаИзмерения.Коэффициент);
				КонецЕсли;
...
Процедура ДобавитьСтрокуВТехОперации(ТаблицаТехнологическиеОперации, СтрокаВыпуска, ТехОпер)
	...
	Для Каждого СтрокаТехОпер Из ТехОпер Цикл
		...
		МассивСуммУпр = ttk_ОбщегоНазначения.РаспределитьПропорционально( СуммаУпр, МассивКоэф, 2);
		МассивСуммБух = ttk_ОбщегоНазначения.РаспределитьПропорционально( СуммаБух, МассивКоэф, 2);
		МассивСуммВал = ttk_ОбщегоНазначения.РаспределитьПропорционально( СтрокаТехОпер.Количество * СтрокаТехОпер.ТехОперация.Расценка, МассивКоэф, 2);
...
Процедура ПроверкаРеквизитов(СтруктураШапкиДокумента, Отказ, Заголовок) Экспорт
		...
		Если СтрокаТаблицы.ВидВыпуска = Перечисления.ВидыВыпуска.Выпуск ИЛИ Не ИспользоватьНаработку Тогда
			...
			Если Не СтрокаТаблицы.Номенклатура.Услуга Тогда
				Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.Качество)  Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указано качество в строке № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Продукция"")", Отказ, Заголовок);
				Иначе
					 Если СтрокаТаблицы.Качество <> Справочники.Качество.Новый Тогда
						Если ЗначениеЗаполнено(СтрокаТаблицы.ЗаказВыпуска) Тогда
							ttk_ОбщегоНазначения.СообщитьОбОшибке("В строке № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Продукция"") указан заказ-выпуска для бракованной продукции. Данная продукция не может быть выпущена под заказ.", Отказ, Заголовок);
						КонецЕсли;
						Если ЗначениеЗаполнено(СтрокаТаблицы.ЗаказРезерв) Тогда
							ttk_ОбщегоНазначения.СообщитьОбОшибке("В строке № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Продукция"") указан документ-резерва для бракованной продукции. Данная продукция не может быть зарезервирована.", Отказ, Заголовок);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			...
			Если СтруктураШапкиДокумента.ИспользоватьНаправленияВыпуска И НЕ ЗначениеЗаполнено(СтрокаТаблицы.НаправлениеВыпуска) Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указано направление выпуска в строке № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Продукция"")", Отказ, Заголовок);
			КонецЕсли;
			Если СтруктураШапкиДокумента.ИспользоватьНаправленияВыпуска И СтрокаТаблицы.НаправлениеВыпуска = Перечисления.НаправленияВыпуска.НаЗатраты Тогда
				Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.СтатьяЗатратПолучатель) Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указана статья затрат в строке № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Получатели"")", Отказ, Заголовок);
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ПодразделениеПолучатель) И (СтруктураШапкиДокумента.ОтражатьВУправленческомУчете) Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указано подразделение в строке № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Получатели"")", Отказ, Заголовок);	
				КонецЕсли;
				Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете И НЕ ЗначениеЗаполнено(СтрокаТаблицы.ПодразделениеОрганизацииПолучатель) Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указано подразделение организации в строке № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Получатели"")", Отказ, Заголовок);	
				КонецЕсли;
				Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете И НЕ ЗначениеЗаполнено(СтрокаТаблицы.СчетЗатратПолучатель) Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указан счет затрат в строке № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Получатели"")", Отказ, Заголовок);	
				КонецЕсли;
				Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете И НЕ ЗначениеЗаполнено(СтрокаТаблицы.СчетЗатратПолучательНУ) Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указан счет затрат НУ в строке № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Получатели"")", Отказ, Заголовок);	
				КонецЕсли;
				Если СтрокаТаблицы.СтатьяЗатратПолучатель.СтатусМатериальныхЗатрат = Перечисления.СтатусыМатериальныхЗатратНаПроизводство.ВозвратныеОтходы Тогда
				    ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя указывать статьи затрат со статусом ""Возвратные отходы"". Строка № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Получатели"")", Отказ, Заголовок);
				КонецЕсли;
				Если СтрокаТаблицы.СтатьяЗатратПолучатель.СтатусМатериальныхЗатрат = Перечисления.СтатусыМатериальныхЗатратНаПроизводство.ПринятыеВПереработку Тогда
				    ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя указывать статьи затрат со статусом ""Принятые в переработку"". Строка № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Получатели"")", Отказ, Заголовок);
				КонецЕсли;
			КонецЕсли;
			...
			Если СтруктураШапкиДокумента.ИспользоватьНаправленияВыпуска И СтрокаТаблицы.НаправлениеВыпуска = Перечисления.НаправленияВыпуска.НаЗатратыСписок Тогда
				...
				Для Каждого СтрокаНаправления Из НаправленияСписания Цикл
					...
					Если СтрокаНаправления.Коэффициент = 0 Тогда
						ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указан коэффициент в направлениях списания для строки № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Продукция"")", Отказ, Заголовок);
					КонецЕсли;
					Если НЕ ЗначениеЗаполнено(СтрокаНаправления.СтатьяЗатрат) Тогда
						ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указана статья затрат в направлениях списания для строки № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Продукция"")", Отказ, Заголовок);
					КонецЕсли;
					Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете И НЕ ЗначениеЗаполнено(СтрокаНаправления.Подразделение) Тогда
						ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указано подразделение в направлениях списания для строки № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Продукция"")", Отказ, Заголовок);
					КонецЕсли;
					Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете И НЕ ЗначениеЗаполнено(СтрокаНаправления.ПодразделениеОрганизации) Тогда
						ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указано подразделение организации в направлениях списания для строки № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Продукция"")", Отказ, Заголовок);
					КонецЕсли;
					Если СтрокаНаправления.СтатьяЗатрат.СтатусМатериальныхЗатрат = Перечисления.СтатусыМатериальныхЗатратНаПроизводство.ВозвратныеОтходы Тогда
					    ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя в направлениях списания указывать статьи затрат со статусом ""Возвратные отходы"". Строка № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Продукция"")", Отказ, Заголовок);
					КонецЕсли;
					Если СтрокаНаправления.СтатьяЗатрат.СтатусМатериальныхЗатрат = Перечисления.СтатусыМатериальныхЗатратНаПроизводство.ПринятыеВПереработку Тогда
					    ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя в направлениях списания указывать статьи затрат со статусом ""Принятые в переработку"". Строка № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Продукция"")", Отказ, Заголовок);
					КонецЕсли;
					Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете И НЕ ЗначениеЗаполнено(СтрокаНаправления.СчетЗатрат) Тогда
						ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указан счет затрат в направлениях списания для строки № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Продукция"")", Отказ, Заголовок);	
					КонецЕсли;
					Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете И НЕ ЗначениеЗаполнено(СтрокаНаправления.СчетЗатратНУ) Тогда
						ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указан счет затрат НУ в направлениях списания для строки № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Продукция"")", Отказ, Заголовок);	
					КонецЕсли;
				КонецЦикла;
				Если ЕстьНаправленияСписания = Ложь Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указаны получатели для строки № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Продукция"")", Отказ, Заголовок);	
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
		...
		Для Каждого СтрокаТЧ Из ПрочиеЗатраты Цикл
			Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете И СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
				Если СтрокаТЧ.Сумма = 0 И СтрокаТЧ.СуммаРегл = 0 Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указана сумма для статьи затрат """ + СтрокаТЧ.СтатьяЗатрат + """ (строка № " + СтрокаТЧ.НомерСтроки + " табличной части ""Прочие затраты"")", Отказ, Заголовок);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Иначе
		Если ИспользоватьМатериалы И ЗначениеЗаполнено(СтатьяЗатрат) Тогда
			Если Не СтатьяЗатрат.ВидЗатрат = Перечисления.ВидыЗатрат.Материальные Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Укажите материальную статью затрат (закладка ""Материалы"")!", Отказ, Заголовок);
			КонецЕсли;
			Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете И Не СтатьяЗатрат.ХарактерЗатрат = Перечисления.ХарактерЗатрат.ПроизводственныеРасходы Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Укажите производственную статью затрат (закладка ""Материалы"")!", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если Не СтруктураШапкиДокумента.ИспользоватьНаправленияВыпуска Тогда
		...
		Если Строки.Количество() > 0 тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Документом отражается выпуск услуг, установите использование направления выпуска!", Отказ, Заголовок);
		КонецЕсли;
	...
	Если ИспользоватьПрочиеЗатраты И Не АвтораспределениеПрочихЗатрат Тогда
		...
		Для Каждого СтрокаТЧ Из РаспределениеПрочихЗатрат Цикл
			Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете И СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
				Если СтрокаТЧ.Сумма = 0 И СтрокаТЧ.СуммаРегл = 0 Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указана сумма для статьи затрат """ + СтрокаТЧ.СтатьяЗатрат + """ (строка № " + СтрокаТЧ.НомерСтроки + " табличной части ""Распределение прочих затрат"")", Отказ, Заголовок);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Процедура ДвиженияПоТабличнойЧастиПродукцияУпр(ТаблицаПоПродукции, СтруктураШапкиДокумента, Отказ, Заголовок, РежимПроведения)
	...
	Если НЕ Отказ Тогда
		...
		ttk_ОбщегоНазначения.ПереименоватьКолонкуТаблицыЗначений(ТаблицаПоПродукцииДляРазмещения, ТабИмен, "ЗаказПокупателя");
		ttk_ОбщегоНазначения.ПереименоватьКолонкуТаблицыЗначений(ТаблицаПоПродукцииДляРазмещения, ТабИмен, "ЗаказНаПроизводство", "ЗаказПоставщику");
		ttk_ОбщегоНазначения.ПереименоватьКолонкуТаблицыЗначений(ТаблицаПоПродукцииДляРазмещения, ТабИмен, "ЗаказРезерв",         "ЗаказПокупателя");
	...
	Если ИспользоватьНаправленияВыпуска И ИспользоватьЛимитыОтпускаМатериалов Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоТоварамЛимиты, ТаблицаДвижений);
	...
	Если ОтражатьВБухгалтерскомУчете Тогда
		...
		Пока ВыборкаПоРаботники.Следующий() Цикл 
			...
			Если ВыборкаПоРаботники.СуммаКНачислению <> 0 Тогда
				...
				МассивСуммКНачислению = ttk_ОбщегоНазначения.РаспределитьПропорционально(ВыборкаПоРаботники.СуммаКНачислению,МассивСуммЗаРаботы);
	...
	Если ИспользоватьТехнологическиеОперации Тогда
		...
		Если ОтражатьВУправленческомУчете Тогда
			...
			Если ТехнологическиеОперации.Итог("Сумма") <> Исполнители.Итог("СуммаКНачислению") Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Не совпадают итоговые суммы к начислению по управленческому учету по табличным частям ""Тех. операции"" и ""Исполнители""!", Отказ, Заголовок);
			КонецЕсли;
			Если ТехнологическиеОперации.Итог("Сумма") <> РаспределениеТехнологическихОпераций.Итог("Сумма") Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Не совпадают итоговые суммы по управленческому учету по табличным частям ""Тех. операции"" и ""Распределение тех.операций""!", , Заголовок);
			КонецЕсли;
		КонецЕсли;
		
		Если ОтражатьВБухгалтерскомУчете Тогда
			Если ТехнологическиеОперации.Итог("СуммаРегл") <> Исполнители.Итог("СуммаКНачислениюРегл") Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Не совпадают итоговые суммы к начислению по регламентированному учету по табличным частям ""Тех. операции"" и ""Исполнители""!", Отказ, Заголовок);
			КонецЕсли;
			Если ТехнологическиеОперации.Итог("СуммаРегл") <> РаспределениеТехнологическихОпераций.Итог("СуммаРегл") Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Не совпадают итоговые суммы по регламентированному учету по табличным частям ""Тех. операции"" и ""Распределение тех.операций""!", , Заголовок);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
