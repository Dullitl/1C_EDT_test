...
Процедура ЗаполнитьМатериалыПоСпецификации() Экспорт
	...
	СтруктураДопКолонок = Новый Структура("Заказ, ВидВыпуска, НоменклатурнаяГруппа, СчетЗатрат, СчетЗатратНУ");
	...
	ТабличнаяЧастьМатериалы.Свернуть("Номенклатура, ХарактеристикаНоменклатуры, ЕдиницаИзмерения, Коэффициент, СтатьяЗатрат, Заказ, ВидВыпуска, НоменклатурнаяГруппа,
									|СчетЗатрат, СчетЗатратНУ", "Количество");
...
Процедура ЗаполнитьСписокРеквизитовЗависимыхОтТиповУчетаРегл()
	...
	мСтрокаРеквизитыНалУчета = "Материалы.СчетЗатратНУ, РаспределениеМатериалов.СчетЗатратНУ,
							   |ВозвратныеОтходы.СчетЗатратНУ, РаспределениеВозвратныхОтходов.СчетЗатратНУ,
...
Процедура ПроверкаРеквизитов(Отказ, Заголовок, СтруктураШапкиДокумента) Экспорт
	...
	РеквизитыТабРаспМат = "Номенклатура, Количество, СтатьяЗатрат, Продукция, СчетЗатрат, СчетЗатратНУ";
	РеквизитыТабРаспределенияОтходов = "Номенклатура, СтатьяЗатрат, Количество, Продукция, СчетЗатрат, СчетЗатратНУ";
	...
	Если ДатаНачалаПериода > Дата Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Дата начала периода не может быть больше даты документа!", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПолучитьТаблицуЗатратРегл(ТабЗатраты)
	...
	ИначеЕсли ОтражатьВНалоговомУчете Тогда
		СуффиксИмениРегистра = "НалоговыйУчет";
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуЗатрат(ТабЗатраты, РезультатЗапроса)
	...
	Если ОтражатьВБухгалтерскомУчете Тогда
		СтрокаРекв = СтрокаРекв + ", СчетУчета, СчетУчетаНУ";
	КонецЕсли;
					...
					Пока ОбходЗатрата.Следующий() Цикл
						...
						Пока ОбходХар.Следующий() Цикл
							...
							Пока Обход.Следующий() Цикл
								...
								ОбходПодробно = Обход.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
								Пока ОбходПодробно.Следующий() Цикл
									Если ОтражатьВНалоговомУчете И ЗначениеЗаполнено(ОбходПодробно.СчетУчетаНУ) Тогда
										НоваяСтрока.СчетУчетаНУ 		= ОбходПодробно.СчетУчетаНУ;
										Прервать;
									КонецЕсли;
								КонецЦикла;
...
Процедура ПолучитьТаблицуНормативовРегл(ТабПотребностей)
	...
	ИначеЕсли ОтражатьВНалоговомУчете Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "%Суффикс%", "НалоговыйУчет");
		Запрос.УстановитьПараметр("Подразделение", ПодразделениеОрганизации);
...
Функция РассчитатьНормативноеКоличество(ТабЗатраты, ТабВыпуска, ТабНорматив)
	...
	Для Каждого СтрокаЗатр Из ТабЗатраты Цикл
		...
		МассивКол = ttk_ОбщегоНазначения.РаспределитьПропорционально( СтрокаЗатр.КолВход, МассивКоэф, 3);
		...
		Для Каждого СтрокаНорматива Из СтрокиЗатратыПоНормативу Цикл
			Если МассивКол[Индекс] > 0 Тогда
				...
				МассивКолКРаспр = ttk_ОбщегоНазначения.РаспределитьПропорционально(МассивКол[Индекс], МассивКоэфКРаспр, 3);
				...
				Для Каждого СтрокаВыпуска Из СтрокиВыпуска Цикл
					...
					ОбщегоНазначения.КопироватьСтрокуТаблицыЗначений( НоваяСтрокаЗатрНаВыпуск, СтрокаЗатр,
						...
						?(ОтражатьВБухгалтерскомУчете,",СчетУчета, СчетУчетаНУ", ""));
...
Процедура ЗаполнитьДокументЗатратамиНаВыпуск(ТабЗатратНаВыпуск)
	...
	Для Каждого СтрокаРаспр Из ТабЗатратНаВыпуск Цикл
		...
		МассивРаспрЗатр = ttk_ОбщегоНазначения.РаспределитьПропорционально(СтрокаРаспр.КолКРаспр, МассивКоэфЗатр, 3);
		...
		Для Каждого СерияЗатрат Из СтрокаРаспр.ТабСерийЗатрат Цикл
			...
			МассивРаспрПрод = ttk_ОбщегоНазначения.РаспределитьПропорционально(МассивРаспрЗатр[ИндексЗатр], МассивКоэфПрод, 3);
			...
			Для Каждого СерияПрод Из СтрокаРаспр.ТабСерийПрод Цикл
				...
				Если Не МассивРаспрПрод[ИндексПрод] = 0 Тогда
					...
					Если ОтражатьВБухгалтерскомУчете Тогда
						...
						НоваяСтрока.СчетЗатратНУ           = СтрокаРаспр.СчетУчетаНУ;
					КонецЕсли;
