...
// {{KM WARE Лазаревский К.В. Заявка №35397 21.10.2015 начало
// для ыыода ОС без перв.стоимости
//		                        |,ПервоначальнаяСтоимость,
// }}KM WARE Лазаревский К.В. Заявка №35397 21.10.2015 окончание								
		                        |";
...
// {{KM WARE Лазаревский К.В. Заявка №35397 <Дата dd.mm.yyyy> начало
// для ввода ОС без первоначальной стоимости
// {{
		//ОбязательныеРеквизиты = ОбязательныеРеквизиты + ", , ПервоначальнаяСтоимостьБУ";
// }}
// }}KM WARE Лазаревский К.В. Заявка №35397 <Дата dd.mm.yyyy> окончание
							  
	КонецЕсли;

	// Обязательные реквизиты для отражения в НУ.
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
		 //абс н (убираем проверку для ОС, списанных в НУ)
// {{KM WARE Лазаревский К.В. Заявка №35397 <Дата dd.mm.yyyy> начало
// для ввода ОС без первоначальной стоимости
// {{
...
// {{KM WARE Лазаревский К.В. Заявка № 21.10.2015 начало
// для ыыода ОС без перв.стоимости
//			ОбязательныеРеквизиты = ОбязательныеРеквизиты + ", ТекущаяСтоимость";
// }}KM WARE Лазаревский К.В. Заявка № 21.10.2015 окончание
...
// Для ввода ОС с нулевой стоимостью
// {{
...
// }}
// }}KM WARE Лазаревский К.В. Заявка № <Дата dd.mm.yyyy> окончание
					ОбязательныеРеквизиты = ОбязательныеРеквизиты + " КоэффициентУскорения";
...
// Для ввода ОС с нулевой стоимостью
// {{
					//ОбязательныеРеквизиты = ОбязательныеРеквизиты + ", ТекущаяСтоимостьБУ";
// }}
// }}KM WARE Лазаревский К.В. Заявка № <Дата dd.mm.yyyy> окончание
			ОбязательныеРеквизиты = ОбязательныеРеквизиты + ", СчетУчетаБУ";
...
// }}
// }}KM WARE Лазаревский К.В. Заявка № <Дата dd.mm.yyyy> окончание
					ОбязательныеРеквизиты = ОбязательныеРеквизиты + ", КоэффициентУскоренияБУ";
...
// Для ввода ОС с нулевой стоимостью
// {{
					//ОбязательныеРеквизиты = ОбязательныеРеквизиты + ", СрокПолезногоИспользованияНУ";
// }}
// }}KM WARE Лазаревский К.В. Заявка № <Дата dd.mm.yyyy> окончание
...
// }}Новый код:
		Врем = Ложь;
		ОшибкаПроведенияПоСтроке(СтрокаОС, СтрокаСообщения, "(НУ)", Врем , Заголовок);
...
Процедура ДвижениеПоРегиструМестонахождениеОСБух( ТабОС, СтруктураШапкиДокумента)
	...
	Для Каждого СтрокаОС Из ТабОС Цикл
		...
		НоваяСтрока.Обособленное 	 = СтруктураШапкиДокумента.абс_Обособленное;
		//\\АБС ВСТАВКА №9946 КОНЕЦ
		//// Гущина С. 7720159 нач
		//Если  ЗначениеЗаполнено(СтруктураШапкиДокумента.абс_Обособленное)     // проверка на пустую т.к. с пустой не даст записать ОС
		//	И СтрокаОС.ОсновноеСредство.абс_ОбособленноеПодразделение <>  СтруктураШапкиДокумента.абс_Обособленное
		//	Тогда 
		//	РедактируемоеОС = СтрокаОС.ОсновноеСредство.ПолучитьОбъект();
		//	РедактируемоеОС.абс_ОбособленноеПодразделение =  СтруктураШапкиДокумента.абс_Обособленное;
		//	Попытка 
		//		РедактируемоеОС.Записать();
		//	Исключение
		//	КонецПопытки;
		//КонецЕсли;
		//// Гущина С. 7720159 кон
...
Процедура ФормированиеПроводокНал( ТабОС, СтруктураШапкиДокумента)
	...
	Для Каждого СтрокаОС Из ТабОС Цикл
		...
		Если НЕ (СтрокаОС.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ВключениеВРасходыПриПринятииКУчету) Тогда
			...
			//Вставка №16069
			Если ЗначениеЗаполнено(СтрокаОС.абс_АмортизационнаяПремия) Тогда
				...
				Проводка.Сумма       = СтрокаОС.абс_АмортизационнаяПремия;
...
Процедура ФормированиеПроводокПоРазницам( ТабОС, СтруктураШапкиДокумента)
	...
	Если ЕстьПередачаОС <> Неопределено Тогда
		ОрганизацияИсточник = абс_БизнесПроцессыПривелегированный.ПолучитьОрганизациюИсточник(Новый Структура("СсылкаНаОбъект",Ссылка)); 	
	КонецЕсли;
	...
	Для Каждого СтрокаОС Из ТабОС Цикл
			 ...
			 //Если СтрокаОС.СчетУчетаНУ = СчетНУ1012 Тогда
            ...
            Если ТекущаяСтоимостьПР <> 0 Тогда
				...
				//Авизо
				Если ЕстьПередачаОС <> Неопределено Тогда
					Проводка.СчетКт      =  ПланыСчетов.Налоговый.НайтиПоКоду("79.01");
					Проводка.ВидУчетаКт   = Проводка.ВидУчетаДт;
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "ОбособленныеПодразделения", ОрганизацияИсточник);
				КонецЕсли;
				//Авизо
		   ...
		   НакопленнаяАмортизацияВР = СтрокаОС.НакопленнаяАмортизацияБУ - СтрокаОС.НакопленнаяАмортизацияНУ - СтрокаОС.НакопленнаяАмортизацияПР;
            ...
            Если НакопленнаяАмортизацияПР <> 0 Тогда
				...
				//Авизо
				Если ЕстьПередачаОС <> Неопределено Тогда
					Проводка.СчетДт      =  ПланыСчетов.Налоговый.НайтиПоКоду("79.01");
					Проводка.ВидУчетаДт   = Проводка.ВидУчетаКт;
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "ОбособленныеПодразделения", ОрганизацияИсточник);
				КонецЕсли;
				//Авизо
...
Процедура ДвиженияПоРегистрам( ТабОС, СтруктураШапкиДокумента, Заголовок)
	...
	//АБС
	//ДвижениеПоРегиструМестоЭксплуатацииОСПоСооружению(ТабОС);	
	//АБС
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		Если ВидОперации = Перечисления.абс_ВидыОперацийВводОстОС.ПоступлениеПоАвизо Тогда
			ФормированиеПроводокБухАвизо(ТабОС, СтруктураШапкиДокумента);
		Иначе
	...
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
		...
		//АБС НАЧАЛО Заявка на модификацию 07.13
		Если ВидОперации = Перечисления.абс_ВидыОперацийВводОстОС.ПоступлениеПоАвизо Тогда	
			ФормированиеПроводокНалАвизо(ТабОС, СтруктураШапкиДокумента);
		Иначе 
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	вр_абс_АмортизационнаяПремия = ОС.Итог("абс_АмортизационнаяПремия");
	Если вр_абс_АмортизационнаяПремия > 0 И Не ЗначениеЗаполнено(СчетУчетаЗатратПоАмортизационнойПремии) Тогда
		Текст = "На закладке ""дополнительно"" не указан счет затрат по амортизационной премии и аналитика!";
		ttk_ОбщегоНазначения.СообщитьОбОшибке(Текст,Отказ);
	КонецЕсли;	
	мУдалятьДвижения = НЕ ЭтоНовый();
// {{KM WARE Лазаревский К.В. Заявка №33804 27.07.2015 начало
// Для АВИЗО - контроль на заполнение метода амортизации.
	Если Не ЭтоНовый() Тогда
		СтруктураАвизо = абс_БизнесПроцессыПривелегированный.ЕстьПередачаОС(Новый Структура("СсылкаНаОбъект",Ссылка));
		Если СтруктураАвизо <> Неопределено Тогда
			Для Каждого Строка Из ОС Цикл
				Если НЕ ЗначениеЗаполнено(Строка.МетодНачисленияАмортизацииНУ) Тогда
					Строка.МетодНачисленияАмортизацииНУ = Перечисления.МетодыНачисленияАмортизации.Нелинейный;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;	
// }}KM WARE Лазаревский К.В. Заявка №33804 27.07.2015 окончание	
...
//АБС НАЧАЛО Заявка на модификацию 07.13
Процедура ФормированиеПроводокБухАвизо( ТабОС, СтруктураШапкиДокумента)
		...
		Если Не СтрокаОС.СписатьНаЗатратыПриВводеРегл Тогда
			...
			Если ЗначениеЗаполнено(СтрокаОС.СчетУчетаБУ) Тогда
				...
				Если НЕ (СуммаПроводки = 0) Тогда
					...
					Проводка.СчетКт = СтрокаОС.СчетБУ;
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт,1, СтрокаОС.СубконтоБУ);
			...
			Если ЗначениеЗаполнено( СтрокаОС.СчетАмортизацииБУ) Тогда
			   ...
			   Если НЕ (СуммаПроводки = 0) Тогда
					...
					Проводка.СчетДт = СтрокаОС.СчетБУ;
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт,1, СтрокаОС.СубконтоБУ);
...
Процедура ФормированиеПроводокНалАвизо( ТабОС, СтруктураШапкиДокумента)
		...
		Если НЕ (СтрокаОС.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ВключениеВРасходыПриПринятииКУчету) Тогда
            ...
            Если ЗначениеЗаполнено(СтрокаОС.СчетУчетаНУ) Тогда
				...
				Если НЕ (СуммаПроводки = 0) Тогда
					...
					Проводка.СчетКт      = СтрокаОС.СчетНУ;
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 1, СтрокаОС.СубконтоНУ);
			...
			Если ЗначениеЗаполнено(СтрокаОС.СчетАмортизацииНУ) Тогда
			   ...
			   Если НЕ (СуммаПроводки = 0) Тогда
					...
					Проводка.СчетДт      = СтрокаОС.СчетНУ;
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 1, СтрокаОС.СубконтоНУ);
...
//АБС
Процедура ДвижениеПоРегиструМестоЭксплуатацииОСПоСооружению(ТабОС)
	//НаборДвижений   = Движения.МестоЭксплуатацииОСПоСооружению;
	//ТаблицаДвижений = НаборДвижений.Выгрузить();
	//
	//Для Каждого СтрокаОС Из ТабОС Цикл
	//	
	...
	//	НоваяСтрока.Активность       = Истина;
	//	НоваяСтрока.ОсновноеСредство = СтрокаОС.ОсновноеСредство;
	//	НоваяСтрока.МестоЭксплуатацииПоСооружению = СтрокаОС.МестоЭксплуатацииПоСооружению;
	//	//АБС
	//	Если Не СтрокаОС.ФункциональнаяПринадлежность = Неопределено Тогда
	//		НоваяСтрока.ФункциональнаяПринадлежность = СтрокаОС.ФункциональнаяПринадлежность;
	//	Иначе 
	//		НоваяСтрока.ФункциональнаяПринадлежность = Справочники.КлассификаторСетейМСС.ПустаяСсылка();
	//	КонецЕсли;
	//	
	//	//\\
	//	//АБС Перезаполнение МестаЭксплуатацииПоСооружению
	//	Если Не СтрокаОС.ОсновноеСредство.МестоЭксплуатацииПоСооружению = СтрокаОС.МестоЭксплуатацииПоСооружению Тогда 
	//		РедактируемоеОС = СтрокаОС.ОсновноеСредство.ПолучитьОбъект();
	//		РедактируемоеОС.МестоЭксплуатацииПоСооружению = СтрокаОС.МестоЭксплуатацииПоСооружению;
	//		Попытка 
	//			РедактируемоеОС.Записать();
	//		Исключение
	//		КонецПопытки;
