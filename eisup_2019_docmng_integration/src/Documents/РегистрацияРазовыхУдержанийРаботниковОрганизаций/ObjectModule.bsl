...
Процедура Автозаполнение(ВременнаяТаблица = Неопределено, Реквизиты = Неопределено, ОсновноеНачисление = Истина, ДатаАктуальности, НомерСтроки = Неопределено) Экспорт
	...
	|			ИЗ
	|				ВТСотрудники КАК ВременнаяТаблица
	|				ЛЕВОЕ СОЕДИНЕНИЕ (
	|					ВЫБРАТЬ
	...
	|					ГДЕ
	|						&ПарамДатаОкончания >= РаботникиОрганизации.Период
	|						И РаботникиОрганизации.Сотрудник.ФизЛицо В (&ПарамФизЛица)
	|						И РаботникиОрганизации.ПричинаИзмененияСостояния = Значение(Перечисление.ПричиныИзмененияСостояния.ПриемНаРаботу)
	|					СГРУППИРОВАТЬ ПО
	|						РаботникиОрганизации.Сотрудник) КАК Сотрудники
	|				ПО ВременнаяТаблица.ФизЛицо = Сотрудники.ФизЛицо) КАК ВременнаяТаблица
	|		ПО Истина
	|ГДЕ
	...
	Иначе
		...
		Если Выборка.Следующий() Тогда
			...
			Удержания[НомерСтроки].ВидРасчета = Выборка.ВидРасчета;
			Удержания[НомерСтроки].ДатаНачала = Выборка.ДатаНачала;
			Удержания[НомерСтроки].ДатаОкончания = Выборка.ДатаОкончания;
			Удержания[НомерСтроки].Авторасчет = Выборка.Авторасчет;
			Удержания[НомерСтроки].Физлицо = Выборка.Физлицо;
			Для СчКолонки = 1 По 6 Цикл
				Удержания[НомерСтроки]["Показатель"+СчКолонки] = Выборка["Показатель"+СчКолонки];
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок = "")
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.Организация) Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке(ОбщегоНазначения.ПреобразоватьСтрокуИнтерфейса("Не указана организация, в которой выполняются удержания!"), Отказ, Заголовок);
	КонецЕсли;
	
	//  ПериодРегистрации
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ПериодРегистрации) Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке("Не указан месяц, в котором выполняются удержания!", Отказ, Заголовок);
	КонецЕсли;
	...
	Если ВыборкаПоШапкеДокумента.ОрганизацияПерерасчета <> null 
		...
		ОбщегоНазначения.ВывестиИнформациюОбОшибке(ОбщегоНазначения.ПреобразоватьСтрокуИнтерфейса("Организация, заданная для документа, должна совпадать с организацией исправляемого документа!"), Отказ, Заголовок);
	КонецЕсли;
