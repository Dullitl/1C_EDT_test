...
Функция ПечатьОприходованияТоваров()
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, "Оприходование товаров");
...
Процедура ПроверитьЗаполнениеТабличнойЧастиТовары(ТаблицаПоТоварам, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	СтруктураОбязательныхПолей = Новый Структура("Номенклатура, Количество, Сумма, Качество, СтатьяОборотов");
	//Крамаренко Д.М.
	//НФС 2018
	   ...
	   И СтруктураШапкиДокумента.ПоддержкаПБУ18 Тогда
		...
		Для каждого СтрокаТаблицы из ТаблицаПоТоварам Цикл
			...
			Если СтрокаТаблицы.СуммаРегл <> СтрокаТаблицы.СуммаНУ+СтрокаТаблицы.СуммаВР+СтрокаТаблицы.СуммаПР Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не выполняется правило. " +
	...
	Для Каждого Строка из Товары Цикл
		...
		Если СокрЛП(Строка.СтатьяОборотов.Код) = "КВпоПост" Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Если абс_СтатусДокумента = Перечисления.абс_СтатусыПервичныхДокументов.ДокументыПринятыБухгалтером или абс_СтатусДокумента = Перечисления.абс_СтатусыПервичныхДокументов.Завершен Тогда
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, Отказ, Заголовок)
	...
	Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		...
		Попытка
			Если НаборДвижений = Движения.ТоварыНаСкладах Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи =  РежимЗаписиДокумента.Проведение Тогда
		...
		// Не делаем движения в статусах обработки инициатором
		Если абс_СтатусДокумента = Перечисления.абс_СтатусыПервичныхДокументов.Подготовка
			ИЛИ абс_СтатусДокумента = Перечисления.абс_СтатусыПервичныхДокументов.СогласованиеОФК
			ИЛИ абс_СтатусДокумента = Перечисления.абс_СтатусыПервичныхДокументов.УточнениеБухгалтерией
			ИЛИ абс_СтатусДокумента = Перечисления.абс_СтатусыПервичныхДокументов.УточнениеОФК
			ИЛИ абс_СтатусДокумента = Перечисления.абс_СтатусыПервичныхДокументов.ОбработкаБухгалтером
			ИЛИ абс_СтатусДокумента = Перечисления.абс_СтатусыПервичныхДокументов.Отказ 
			ИЛИ абс_СтатусДокумента = Перечисления.абс_СтатусыПервичныхДокументов.Отмена Тогда 
	...
	Если ЗначениеЗаполнено(абс_Авизо) И абс_Авизо.Статус <> Перечисления.абс_СтатусыАвизо.Отказ Тогда
		Если абс_Авизо.ВидОперации = Перечисления.абс_ВидыОперацийАвизо.ПередачаМатериалы10_07 Тогда	
			...
			Для Каждого Проводка ИЗ ДвиженияБУ Цикл 
				Проводка.СубконтоДт.Контрагенты = абс_Авизо.ДоговорКонтрагентаПриемникШапка.Владелец;
				
				
				Если Проводка.СчетДт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры)<>НЕОПРЕДЕЛЕНО Тогда
					
					Проводка.СубконтоДт.Договоры = абс_Авизо.ДоговорКонтрагентаПриемникШапка;
...
// АБС ВСТАВКА Согласование первичных документов
Процедура ЗаписатьНовыйСтатус(НовыйСтатус, врКомментарий = Неопределено) Экспорт
	...
	Запись.Комментарий 				= врКомментарий;  	
