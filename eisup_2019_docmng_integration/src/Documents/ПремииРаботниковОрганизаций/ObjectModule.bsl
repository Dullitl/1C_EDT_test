...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,Метаданные().Синоним + ИмяФормы));
...
Процедура Автозаполнение(ВременнаяТаблица = Неопределено, Реквизиты = Неопределено, ОсновноеНачисление = Истина, ДатаАктуальности, НомерСтроки = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ПарамПериодРегистрации",?(ДатаАктуальности = '0001.01.01',ДатаАктуальности,ПериодРегистрации));
	Запрос.УстановитьПараметр("ПарамСотрудники",ВременнаяТаблица.ВыгрузитьКолонку("Сотрудник"));
	Запрос.УстановитьПараметр("ПарамОрганизация",Организация);
	Запрос.УстановитьПараметр("ПарамДатаНачала",?(ДатаНачалаБП = '0001.01.01',ДатаНачалаБП,НачалоМесяца(ПериодРегистрации)));
	Запрос.УстановитьПараметр("ПарамДатаОкончания",?(ДатаОкончанияБП = '0001.01.01',ДатаОкончанияБП,КонецМесяца(ПериодРегистрации)));
	...
	|		ПО Начисления.ТарифныйРазряд" + Сч +" = ТарифныеРазряды" + Сч +".ТарифныйРазряд
	...
	|	Показатели.Сотрудник.ФизЛицо КАК ФизЛицо,
	|	СУММА(ВЫБОР
	...
	|СГРУППИРОВАТЬ ПО
	|	Показатели.Сотрудник
	|УПОРЯДОЧИТЬ ПО Показатели.Сотрудник.Наименование";
	...
	Иначе
		...
		Если Выборка.Следующий() Тогда
			...
			Начисления[НомерСтроки].Авторасчет = Выборка.Авторасчет;
			Начисления[НомерСтроки].Сотрудник = Выборка.Сотрудник;
			Начисления[НомерСтроки].Физлицо = Выборка.Физлицо;
			Для СчКолонки = 1 По 6 Цикл
				Начисления[НомерСтроки]["Показатель"+СчКолонки] = Выборка["Показатель"+СчКолонки];
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Функция ПечатьТ11()
	...
	Если ВыборкаДляШапки.Следующий() Тогда
		...
		ВыводитьДатуОзнакомленияСПриказом = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(глЗначениеПеременной("глТекущийПользователь"), "ПроставлятьАвтоматическиДатуОзнакомленияВКадровыхПриказах");
		Если Не ВыводитьДатуОзнакомленияСПриказом Тогда
			ОбластьМакетаПодвал.Параметры.ДатаДок = ФормированиеПечатныхФорм.ПустаяДатаВПечатнойФорме();
		КонецЕсли;
	...
	Пока ВыборкаРаботники.Следующий() Цикл
		...
		ОбластьМакета.Параметры.ПредставлениеВеличины = ОбщегоНазначения.СформироватьСуммуПрописью(Результат,Валюта);
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок)
	...
	Если Не ЕстьВидРасчета Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке("Не указан вид премии!", Отказ, Заголовок);
	КонецЕсли;

	//  Число месяцев
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ЧислоМесяцев) Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке("Не указано число месяцев за которые начисляется премия!", Отказ, Заголовок);
	КонецЕсли;
	...
	Если ЕстьВидРасчета Тогда
		...
		Если ВыборкаПоШапкеДокумента.ВидПремии = Перечисления.ВидыПремии.НеПремия Тогда
			ОбщегоНазначения.ВывестиИнформациюОбОшибке("В качестве премии указан вид расчета, не описанный как премия!", Отказ, Заголовок);
		...
		Иначе
			...
			Если ВыборкаПоШапкеДокумента.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.Процентом Тогда
				...
				Если Не ЕстьДатаНачалаБП Тогда
					ОбщегоНазначения.ВывестиИнформациюОбОшибке("Не указана дата начала базового периода!", Отказ, Заголовок);
				КонецЕсли;
				
				Если Не ЕстьДатаОкончанияБП Тогда
					ОбщегоНазначения.ВывестиИнформациюОбОшибке("Не указана дата окончания базового периода!", Отказ, Заголовок);
				КонецЕсли;
		...
		Если ВыборкаПоШапкеДокумента.ВидПремии = Перечисления.ВидыПремии.ГодоваяПремия Тогда
			...
			Если ЕстьДатаНачалаБП И ЕстьДатаОкончанияБП Тогда
				...
				Если Год(ВыборкаПоШапкеДокумента.БазовыйПериодНачало) <> Год(ВыборкаПоШапкеДокумента.БазовыйПериодКонец) Тогда
					...
					ОбщегоНазначения.ВывестиИнформациюОбОшибке("Для годовой премии дата начала и окончания базового периода должны быть в пределах одного года!", Отказ, Заголовок)
...
Процедура ДобавитьСтрокуВДвиженияПоНДФЛ(ВыборкаПоШапкеДокумента, ВыборкаПоТЧ)
	...
	ЗаполнитьЗначенияСвойств(Движение,ВыборкаПоШапкеДокумента); // ВидРасчета, ОбособленноеПодразделение
