...
Процедура ЗаполнитьДокумент(ОшибкаЗаполнения = Ложь, Сообщать = Истина, СтрокаСообщения = "", ОтменитьПроведение = Ложь, УдалятьНезаполненные = Ложь) Экспорт
	...
	Иначе
		...
		СписокСчетовФактур = ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(Дерево_НДСДляВосстановления.Строки.ВыгрузитьКолонку("СчетФактура"),Истина);
		...
		Если УдалятьНезаполненные Тогда
			УдалитьНезаполненныеСтроки(ТаблицаВосстановления, "ВидЦенности, Поставщик, СчетФактура, СтавкаНДС, СчетУчетаНДС");
		КонецЕсли;
...
Процедура ДвиженияПоРегистрамРегл(СтруктураШапкиДокумента, ТаблицаВосстановления)
	...
	Для Каждого Строка Из ТаблицаВосстановления Цикл
		...
		Если (Строка.НДС <> 0) Тогда
			...
			//абсо \/
			Если ПроводкиСторно Тогда

				СтрокаДвиженияПроводка.СчетДт		= ПланыСчетов.Хозрасчетный.НДС; //68.02
				БухгалтерскийУчет.УстановитьСубконто(СтрокаДвиженияПроводка.СчетДт, СтрокаДвиженияПроводка.СубконтоДт, "ВидыПлатежейВГосБюджет", Перечисления.ВидыПлатежейВГосБюджет.Налог);
				
				СтрокаДвиженияПроводка.СчетКт		= Строка.СчетУчетаНДС; //19.XX
				БухгалтерскийУчет.УстановитьСубконто(СтрокаДвиженияПроводка.СчетКт, СтрокаДвиженияПроводка.СубконтоКт, "Контрагенты", Строка.Поставщик);
				
				//АБС+ Тупиков 14523
				Если Строка.СчетУчетаНДС = ПланыСчетов.Хозрасчетный.НДСпоАвансамИПредоплатам Тогда //76.АВ
					БухгалтерскийУчет.УстановитьСубконто(СтрокаДвиженияПроводка.СчетКт, СтрокаДвиженияПроводка.СубконтоКт, "СФВыданные", Строка.СчетФактура);
				Иначе
					БухгалтерскийУчет.УстановитьСубконто(СтрокаДвиженияПроводка.СчетКт, СтрокаДвиженияПроводка.СубконтоКт, "СФПолученные", Строка.СчетФактура);
				КонецЕсли;
...
Процедура ДвиженияРегистровПодсистемыНДС(СтруктураШапкиДокумента, ТаблицаВосстановления) Экспорт
	...
	Если СтруктураШапкиДокумента.ОтразитьВКнигеПродаж Тогда
		...
		Если абс_ДатаУчетаПоСчетамУчетаВНДСПродажи <> '00010101000000' И СтруктураШапкиДокумента.Дата >= абс_ДатаУчетаПоСчетамУчетаВНДСПродажи  Тогда
			СоответствиеКолонок = Новый Структура("Покупатель, СуммаБезНДС, НДС, абс_СчетУчета", "Поставщик", "СуммаБезНДСКнигаПродаж", "НДСКнигаПродаж", "СчетУчетаНДС"); 
		Иначе // АБС Коломиец 13843-
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаВосстановления, ТаблицаДвижений_НДСЗаписиКнигиПродаж);
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаВосстановления, ТаблицаДвижений_НДСЗаписиКнигиПокупок);
	...
	ПараметрыОтбора.Вставить("ВидЦенности", Перечисления.ВидыЦенностей.АвансыПолученные); 
	ТаблицаВосстановленияНДСсАвансов = ТаблицаВосстановления.Скопировать(ПараметрыОтбора);
	ТаблицаВосстановленияНДСсАвансов.Колонки.Добавить("Покупатель");
	ТаблицаВосстановленияНДСсАвансов.Колонки.Добавить("ВалютаАванса");
	ТаблицаВосстановленияНДСсАвансов.Колонки.Добавить("ВалютнаяСуммаСНДС");
	
	Если ТаблицаВосстановленияНДСсАвансов.Количество() > 0 Тогда 
		Для Каждого СтрокаВосстановления Из ТаблицаВосстановленияНДСсАвансов Цикл
			СтрокаВосстановления.СуммаБезНДС = - СтрокаВосстановления.СуммаБезНДС;
			СтрокаВосстановления.НДС = - СтрокаВосстановления.НДС;
			СтрокаВосстановления.Покупатель = СтрокаВосстановления.Поставщик;
			СтрокаВосстановления.ВалютаАванса = Справочники.Валюты.НайтиПоКоду("643");
			СтрокаВосстановления.ВалютнаяСуммаСНДС = СтрокаВосстановления.СуммаБезНДС + СтрокаВосстановления.НДС;
		КонецЦикла;
		
		ТаблицаДвижений_НДСсАвансов = Движения.НДСсАвансов.Выгрузить();
		ТаблицаДвижений_НДСсАвансов.Очистить();
		
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаВосстановленияНДСсАвансов, ТаблицаДвижений_НДСсАвансов);
		ТаблицаДвижений_НДСсАвансов.ЗаполнитьЗначения(СтруктураШапкиДокумента.Дата, "ДатаСобытия");
		
		Если Не ТаблицаДвижений_НДСсАвансов.Количество() = 0 Тогда
	...
	Для Каждого Строка Из ТаблицаВосстановления Цикл
		Если Строка.ВидЦенности = Перечисления.ВидыЦенностей.АвансыПолученные Тогда
			ТаблицаВосстановления.Удалить(Строка);
		КонецЕсли;
	КонецЦикла;
	//АБС- Тупиков	
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаВосстановления, ТаблицаДвижений_НДСПредъявленный);
