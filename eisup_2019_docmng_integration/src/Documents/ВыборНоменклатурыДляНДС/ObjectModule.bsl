...
Процедура ПроверкаДублейСтрок(Отказ, Заголовок)
	...
	Пока Обход.Следующий() Цикл
		...
		КонецЦикла;
		ttk_ОбщегоНазначения.СообщитьОбОшибке( "Номенклатура: " + Обход.Номенклатура + " повторяется в строках " + ТекстНомераСтрок, Отказ, Заголовок);
	КонецЦикла;
...
Процедура ПроверкаРеквизитовТЧ(Отказ, Заголовок)
	...
	Для Каждого Строка Из ТаблицаНоменклатурныхГрупп Цикл
		Если НЕ ЗначениеЗаполнено(Строка.НоменклатурнаяГруппа) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не задана номенклатурная группа у номенклатуры: " + Строка.Номенклатура + " (строка № " + Строка.НомерСтроки + ")", , Заголовок);
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если НЕ ЗначениеЗаполнено(Организация) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указана организация!", Отказ, Заголовок);
	КонецЕсли;
