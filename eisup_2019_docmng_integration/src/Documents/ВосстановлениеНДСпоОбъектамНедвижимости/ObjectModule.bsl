...
Процедура ПроверитьЗаполнениеТабличныхЧастей(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Для Каждого СтрокаТаблицыОС Из ОбъектыНедвижимости Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицыОС.ОбъектНедвижимости) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не заполнено значение реквизита ""Объект недвижимости""!", Отказ, Заголовок);
		КонецЕсли;
		Если СтрокаТаблицыОС.ИспользуетсяДляОперацийНеОблагаемыхНДС Тогда
			...
			Если СтрокаТаблицыОС.ДатаНачалаИспользованияДляОпераций = '00010101' Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не заполнено значение реквизита ""Дата начала использования для операций, не облагаемых НДС""!", Отказ, Заголовок);
			КонецЕсли;
			Если СтрокаТаблицыОС.ДатаНачалаИспользованияДляОпераций = 0 Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не заполнено значение реквизита ""Стоимость объекта недвижимости (БУ)""!", Отказ, Заголовок);
			КонецЕсли;
			Если СтрокаТаблицыОС.ДоляВыручкиНеОблагаемаяНДС = 0 Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не заполнено значение реквизита ""Доля выручки, не облагаемая НДС (%)""!", Отказ, Заголовок);
			КонецЕсли;
		...
		Если СтруктураШапкиДокумента.ОтражатьВосстановлениеВКнигеПродаж И СтрокаТаблицыОС.ИспользуетсяДляОперацийНеОблагаемыхНДС Тогда
			...
			Для Каждого СтрокаСчетФактура Из СтрокиСчетаФактуры Цикл
				...
				Если НЕ ЗначениеЗаполнено(СтрокаСчетФактура.СчетФактура) Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не заполнено значение реквизита ""Счет-фактура""", Отказ, Заголовок);
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(СтрокаСчетФактура.ВидЦенности) Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не заполнено значение реквизита ""Вид ценности""", Отказ, Заголовок);
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(СтрокаСчетФактура.КодОперацииДляДекларации) Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не заполнено значение реквизита ""Код операции для декларации""", Отказ, Заголовок);
				КонецЕсли;
