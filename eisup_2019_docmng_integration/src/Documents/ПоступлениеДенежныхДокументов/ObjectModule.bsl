...
//  Табличный документ - печатная форма приходного ордера
//
Функция ПечатьПоступлениеДенежныхДокументов()
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, "Приходный ордер");
...
Процедура ДвиженияПоРегистрамУпр(СтруктураШапкиДокумента, ТаблицаПоДенежнымДокументам,Отказ, Заголовок)
	...
	Если ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхДокументов.ПрочееПоступление Тогда
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхДокументов.ПоступлениеОтПодотчетногоЛица Тогда
		...
		СтрокаДвижений.РасчетныйДокумент   = ?(ЗначениеЗаполнено(РасчетныйДокумент), РасчетныйДокумент, Ссылка);
...
Процедура ДвиженияПоРегистрамРегл(СтруктураШапкиДокумента, ТаблицаПоДенежнымДокументам, РежимПроведения, Отказ, Заголовок)
	...
	Для каждого СтрокаТаблицы Из ТаблицаПоДенежнымДокументам Цикл
		...
		Если ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхДокументов.ПоступлениеОтПоставщика Тогда
			...
			//АБС для счета 60.01 
			Если Проводка.СчетКт = ПланыСчетов.Хозрасчетный.РасчетыСПоставщиками Тогда
				...
				Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ
				                      ...
				                      |			&Дата,
				                      |			Счет В (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыПоАвансамВыданным)),
				                      |			,
				                      |			Организация В (&Организация)
				                      |				И Субконто1 В (&Контрагент)
				                      |				И Субконто2 В (&ДоговорКонтрагента)) КАК ХозрасчетныйОстатки");
				...
				Если ЗначениеЗаполнено(Результат.Сумма) Тогда
				
					Проводка = Проводки.Добавить();
					Проводка.Период      = ДатаДока;
					Проводка.Организация = СтруктураШапкиДокумента.Организация;
					Проводка.Содержание  = "Зачет аванса поставщику";
					Проводка.Сумма       = Мин(Результат.Сумма,СтрокаТаблицы.СуммаБУ); 
					Проводка.СчетДт      = СтруктураШапкиДокумента.СчетУчетаРасчетовСКонтрагентом; 				
					Проводка.СчетКт      = ПланыСчетов.Хозрасчетный.РасчетыПоАвансамВыданным;
					БухгалтерскийУчет.УстановитьСубконто(
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхДокументов.ПоступлениеОтПодотчетногоЛица Тогда
			...
			КонецЕсли;
			БухгалтерскийУчет.УстановитьСубконто(
				Проводка.СчетКт, Проводка.СубконтоКт, "РаботникиОрганизации", СтруктураШапкиДокумента.Контрагент, Истина);
