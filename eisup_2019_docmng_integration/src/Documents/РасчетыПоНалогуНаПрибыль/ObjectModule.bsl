...
Процедура СформироватьПроводкиПоНалогуНаПрибыль(СуммаНалога, КоррСчет, Субконто = Неопределено, ПоДебету, СодержаниеПроводки, СуммаЕНВД = Неопределено, Субконто68_4 = Неопределено)
	
	Счет99 = ПланыСчетов.Хозрасчетный.ПрибылиИУбыткиОблагаемыеЕНВД;
	
	Если Субконто68_4 <> Неопределено Тогда
		
		Счет68_04Д = ПланыСчетов.Хозрасчетный.РасчетыСБюджетом;
		Счет68_04К = ПланыСчетов.Хозрасчетный.РасчетыСБюджетом;
	Иначе
		Если ВидОперацииРасчетаРазниц = 0 Тогда
			Счет68_04Д = ПланыСчетов.Хозрасчетный.РасчетНалогаНаПрибыль;
			Счет68_04К = ПланыСчетов.Хозрасчетный.РасчетНалогаНаПрибыль;
		ИначеЕсли ВидОперацииРасчетаРазниц = 1 Тогда
			Счет68_04Д = ПланыСчетов.Хозрасчетный.Вспомогательный;
			Счет68_04К = ПланыСчетов.Хозрасчетный.Вспомогательный;
		Иначе
			Счет68_04Д = ПланыСчетов.Хозрасчетный.УбытокПодлежащийПокрытию;
			Счет68_04К = ПланыСчетов.Хозрасчетный.ПрибыльПодлежащаяРаспределению;
		КонецЕсли;
...
Процедура РассчитатьОНАПоУбыткамТекущегоПериода(СтрокаВидАктиваОбязательства)
    ...
    //АБС ВСТАВКА №46531 НАЧАЛО «1 октября 2014 г.», Пополитов
	Если абс_НеИспользоватьУбытокПрошлогоПериода Тогда
		СуммаКт09 = 0; 	
	КонецЕсли;	
	//\\АБС ВСТАВКА №46531 КОНЕЦ		
...
Процедура СформироватьПроводкиПоНалогуНаПрибыльРасчетыСБюджетом(СтруктураШапкиДокумента)
	...
	Если СуммаНалога <> 0 И ВсеСтавки <> 0 Тогда
		...
		Если СтавкаРФ <> 0 Тогда
			...
			Проводка.Период       = Дата;
			Проводка.Организация  = Организация;
			Проводка.Сумма        = СуммаНалога * СтавкаРФ / ВсеСтавки;
			Проводка.Содержание   = "Налог на прибыль";
			Проводка.НомерЖурнала = "";
			Проводка.СчетДт = Счет68_04Дебет;
			Проводка.СчетКт = Счет68_04Кредит;
...
Процедура ОбработкаПроведения(Отказ)
	...
	#Если Клиент Тогда
		абс_БСПОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени(Справочники.абс_БСПКлючевыеОперации.РасчетПоНалогуНаПрибыль_Проведение);
	#Иначе
	...
	#Иначе
		абс_БСПОценкаПроизводительностиКлиентСервер.ЗакончитьЗамерВремени(Справочники.абс_БСПКлючевыеОперации.РасчетПоНалогуНаПрибыль_Проведение, ВремяНачала);
	#КонецЕсли
	//АБС ВСТАВКА 38320 КОНЕЦ 
