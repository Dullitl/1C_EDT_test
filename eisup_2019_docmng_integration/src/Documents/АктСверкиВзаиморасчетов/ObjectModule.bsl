...
Функция ПечатьАктаСверки()
	...
	// {{ТТК Сладков А. Заявка № 07.07.2017 начало
	
	
	// }}ТТК Сладков А. Заявка № 07.07.2017 окончание
	...
	ПредставлениеАдреса = УправлениеКонтактнойИнформацией.ПолучитьПредставлениеАдреса(Результат);
	ОбластьПодвал.Параметры.ПредставлениеАдреса = "Подписанный и заверенный печатью второй экземпляр акта сверки, выслать по адресу : " + 
													ПредставлениеАдреса;
...
Функция ПечатьАктаСверкиПечатьПодпись()
	
	ТабДок = Новый ТабличныйДокумент;
	ТабДок.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_АктСверкиВзаиморасчетов_АктСверки";
	Макет  = ПолучитьМакет("АктСверкиПечатьПодпись");
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("АктСверки,АктСверкиПечатьПодпись","Акт сверки", "Акт сверки (с печатью и подписью)");
...
Процедура ЗаполнитьПоДаннымБухгалтерскогоУчета(ФильтрСписокСчетов = неопределено) Экспорт
	...
	//АБС ИЗМЕНЕНИЕ 43724  26.05.2014 18:31:05  Шамов
	|				И НЕ(ХозрасчетныйОбороты.КорСубконто1 ЕСТЬ NULL)
	|				И НЕ(ХозрасчетныйОбороты.КорСубконто2 ЕСТЬ NULL)
	//АБС ИЗМЕНЕНИЕ 43724 КОНЕЦ
	|				ТОГДА ВЫБОР
	|						КОГДА &ДоговорКонтрагента = НЕОПРЕДЕЛЕНО
	...
	Пока Выборка.Следующий() Цикл
		...
		//АБС_Бреев. договора должны заполняться в таб части
	...
	Если ВалютаДокумента = мВалютаРегламентированногоУчета Тогда
		...
		НаборДвижений.Свернуть("Дата,ДоговорКонтрагента,ДокументРегистратор,Представление,Валюта","Дебет, Кредит, ВалютнаяСумма");
	...
	Иначе
		...
		НаборДвижений.Свернуть("Дата,ДоговорКонтрагента,ДокументРегистратор,Представление","Дебет, Кредит");	
		
	КонецЕсли; 
	
	НаборДвижений.Сортировать("Дата,ДоговорКонтрагента,ДокументРегистратор,Представление");	
...
// Заполнение таблицы "По данным организации" по информации из бухгалтерского учета
Процедура ЗаполнитьПоДаннымБухгалтерскогоУчетаАгентскийДоговор(ФильтрСписокСчетов = неопределено) Экспорт
	...
	СтруктураПараметров.Вставить("ДатаНачала",    ?(НЕ ЗначениеЗаполнено(ДатаНачала),    Неопределено, ДатаНачала));
	СтруктураПараметров.Вставить("ДатаОкончания", ?(НЕ ЗначениеЗаполнено(ДатаОкончания), Неопределено, Новый Граница(КонецДня(ДатаОкончания),ВидГраницы.Включая)));
	СтруктураПараметров.Вставить("Организация",   Организация);
	СтруктураПараметров.Вставить("Контрагент",    Контрагент);
	СтруктураПараметров.Вставить("Валюта",        ?(НЕ ЗначениеЗаполнено(ВалютаДокумента) или (ВалютаДокумента = мВалютаРегламентированногоУчета), неопределено,ВалютаДокумента));
	...
	|			Счет В ИЕРАРХИИ (&ФильтрСписокСчетов),
	|			&АналитикаРасчетов,
	|			Субконто2.абс_Агент = &Организация
	|				И ВЫБОР
	|					КОГДА &Валюта = НЕОПРЕДЕЛЕНО
	|						ТОГДА ИСТИНА
	|					ИНАЧЕ Валюта = &Валюта
	...
	Выборка = абс_Привилегированный.ВыполнитьЗапрос(Текст,СтруктураПараметров).Выбрать();
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.НомерВходящегоДокумента = Неопределено тогда
			Если Представление = "Продажа" И ТипЗнч(Выборка.ДокументРегистратор) <> Тип("ДокументСсылка.ОперацияБух") Тогда
				Представление = Представление + " (" + ?(НЕ ЗначениеЗаполнено(Выборка.НомерВходящегоДокумента),Выборка.Номер, Выборка.НомерВходящегоДокумента) + " от " + Формат(Выборка.Дата, "ДФ=dd.MM.yyyy") + ")";
			Иначе
				Представление = Представление + " ("+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy") + ")";
			КонецЕсли;
		Иначе
	...
	Если ЗначениеЗаполнено(ДатаНачала) Тогда
		...
		ОстатокНаНачало = абс_Привилегированный.ВыполнитьЗапрос(Текст,СтруктураПараметров).Выгрузить().Итог("ОстатокНаНачало");
	...
	СтруктураПараметров.Вставить("ДатаОкончания", ?(ЗначениеЗаполнено(ДатаОкончания), 
		Новый Граница(КонецДня(ДатаОкончания), ВидГраницы.Включая), 
		Неопределено));
	Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	ОстатокНаКонец = абс_Привилегированный.ВыполнитьЗапрос(Текст,СтруктураПараметров).Выгрузить().Итог("ОстатокНаКонец");
