...
Функция ПолучитьЗарплатуПоХарактеруВыплаты(ФизЛица = Неопределено)
	...
	Запрос.УстановитьПараметр("парамПоВсемПодразделениям",	Подразделение.Пустая());
...
Функция ПолучитьФизлицаРаботниковПодразделенияНаДату(Подразделение = Неопределено, ДатаКадровая, ФизЛица = Неопределено, ВключаяУволенных = Ложь)
		...
		Если НЕ ВключаяУволенных Тогда
...
Процедура Рассчитать(ФизЛица = Неопределено) Экспорт
	...
	// обновляем суммы в табличной части
	Для Каждого СтрокаЗарплаты Из Зарплата Цикл
		Если СтрокаЗарплаты.Авторасчет Тогда
			СтрокаТаблицЗарплат = ТаблицаЗарплат.Найти(СтрокаЗарплаты.Физлицо, "ФизЛицо");
			Если СтрокаТаблицЗарплат <> Неопределено Тогда
				СтрокаЗарплаты.Сумма = СтрокаТаблицЗарплат.Сумма;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,"Зарплата к выплате "));
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ ПометкаУдаления И РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда
		Если ПолучитьДокументыВведенныеНаОсновании().Количество()  > 0 Тогда
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке("По документу уже созданы платежные документы, отмена проведения невозможна!", Отказ, Заголовок);
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Для Каждого СтрокаЗарплаты Из Зарплата Цикл
		...
		Если НЕ Отказ Тогда
			...
			// ресурсы
			Движение.СуммаУпр		= СтрокаЗарплаты.Сумма;
