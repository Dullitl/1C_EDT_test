...
Процедура ЗаполнитьСписокРеквизитовЗависимыхОтТиповУчетаРегл()
	...
	мСтрокаРеквизитыБухУчета = "ПодразделениеОрганизации, НадписьПодразделениеОрганизации,
		|Материалы.СчетЗатрат, Материалы.СуммаРегл,
		|Получатели.СчетЗатратПолучатель, Получатели.ПодразделениеОрганизацииПолучатель,
		|Получатели.СтатьяЗатратПолучатель";

	мСтрокаРеквизитыНалУчета = "Материалы.СчетЗатратНУ, 
		|Материалы.СуммаНал,
		|Получатели.СчетЗатратПолучательНУ";
...
Функция ПроверкаРеквизитов(Отказ, Заголовок, СтруктураШапкиДокумента) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаМатериалов Цикл
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ХарактерЗатрат) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указан характер затрат в статье затрат (строка № " + СтрокаТаблицы.НомерСтроки + " табличной части ""Материалы"")", Отказ, Заголовок);
		ИначеЕсли СтрокаТаблицы.ХарактерЗатрат <> Перечисления.ХарактерЗатрат.ПроизводственныеРасходы Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя указывать статью затрат с хар-ром затрат """+СтрокаТаблицы.ХарактерЗатрат+""". Статья затрат """ + СтрокаТаблицы.СтатьяЗатрат + """ (строка № " + СтрокаТаблицы.НомерСтроки + " табличной части ""Материалы"")", Отказ, Заголовок);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ХарактерЗатратПолучатель) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указан характер затрат в статье затрат (строка № " + СтрокаТаблицы.НомерСтроки + " табличной части ""Получатели"")", Отказ, Заголовок);
		ИначеЕсли СтрокаТаблицы.ХарактерЗатратПолучатель <> Перечисления.ХарактерЗатрат.ПроизводственныеРасходы Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Нельзя указывать статью затрат с хар-ром затрат """+СтрокаТаблицы.ХарактерЗатратПолучатель+""". Статья затрат """ + СтрокаТаблицы.СтатьяЗатратПолучатель + """ (строка № " + СтрокаТаблицы.НомерСтроки + " табличной части ""Получатели"")", Отказ, Заголовок);
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.СчетЗатратПолучатель) И (СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указан счет затрат (БУ) в строке № " + СтрокаТаблицы.НомерСтроки + " (таб. часть ""Получатели"")", Отказ, Заголовок);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.СчетЗатратПолучательНУ) И (СтруктураШапкиДокумента.ОтражатьВНалоговомУчете) Тогда
