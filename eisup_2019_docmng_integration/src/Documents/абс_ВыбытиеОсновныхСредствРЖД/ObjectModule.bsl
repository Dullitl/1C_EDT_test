...
Процедура ОбработкаПроведения(Отказ)
	...
	Для Каждого ТекСтрокаОсновныеСредства Из ОсновныеСредства Цикл
		СрезСведенийМежд = РегистрыСведений.абс_ОсновныеСредстваРЖД.ПолучитьПоследнее(Дата, Новый Структура("ОсновноеСредство", ТекСтрокаОсновныеСредства.ОсновноеСредство));
		//АБС
		...
		//БухИтоги.РассчитатьИтоги("абс_МеждународныйОСРЖД", "НачальныйОстатокДт,НачальныйОстатокКт", "Сумма", "Счет,Субконто1", НачалоМесяца(ПериодНачало), НачалоМесяца(ПериодНачало), , МассивСчетов, ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства, , ,"Организация", Организация);
		...
		ДвижениеРС = Движения.абс_ОсновныеСредстваРЖД.Добавить();
		...
		Если СуммаДт <> 0 Тогда
			...
			Движение.СчетДт = ПланыСчетов.абс_МеждународныйОСРЖД.ВыбытиеОсновныхСредств; 	
			...
			Если не Движение.СчетДт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства,) = Неопределено Тогда
				Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства] = ТекСтрокаОсновныеСредства.ОсновноеСредство;
			КонецЕсли;
			...
			Если не Движение.СчетКт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства,) = Неопределено Тогда
				Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства] = ТекСтрокаОсновныеСредства.ОсновноеСредство;
			КонецЕсли;
			
			Если  ТипЗнч(ТекСтрокаОсновныеСредства.Документ) = Тип("ДокументСсылка.СписаниеОС") Тогда
				Если не Движение.СчетДт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ОбособленныеПодразделения,) = Неопределено Тогда
					Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ОбособленныеПодразделения] = ТекСтрокаОсновныеСредства.Документ.СубконтоБУ;
				КонецЕсли;
				
				Если не Движение.СчетКт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ОбособленныеПодразделения,) = Неопределено Тогда
					Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ОбособленныеПодразделения] = ТекСтрокаОсновныеСредства.Документ.СубконтоБУ;
				КонецЕсли;
			КонецЕсли
		...
		Если СуммаКт <> 0 Тогда
			...
			Движение.СчетКт = ПланыСчетов.абс_МеждународныйОСРЖД.ВыбытиеОсновныхСредств; //201	
			Движение.Организация = Организация;
			Если ТипЗнч(ТекСтрокаОсновныеСредства.Документ) <> Тип("ДокументСсылка.абс_ЧастичнаяЛиквидацияОС") Тогда
		...
		СчетаОС.Добавить(ПланыСчетов.абс_МеждународныйОСРЖД.ОсновныеСредства); //101
		...
		Если ОбщаяСуммаКорректировки <> 0  Тогда // Если были операции, связанные с выбытием.
			...
			СуммаДт = 0;//БухИтоги.ПолучитьИтог("СуммаКонечныйОстатокДт", "Счет,Субконто1", ПланыСчетов.абс_МеждународныйОСРЖД.ВыбытиеОсновныхСредств, ТекСтрокаОсновныеСредства.ОсновноеСредство) - БухИтоги.ПолучитьИтог("СуммаКонечныйОстатокКт", "Счет,Субконто1", ПланыСчетов.абс_МеждународныйОСРЖД.ВыбытиеОсновныхСредств, ТекСтрокаОсновныеСредства.ОсновноеСредство);
			Если (СуммаДт + ОбщаяСуммаКорректировки) <> 0 Тогда // Если есть остаток по 201 с учетом предыдущей корретировки, то списываем его
				...
				Если ТипЗнч(ТекСтрокаОсновныеСредства.Документ) = Тип("ДокументСсылка.абс_ЧастичнаяЛиквидацияОС")
					...
					Структура =  абс_МеждународныйУчетОСРЖД.ПреобразоватьСчетаБУвСчетМСФОРЖД(ТекСтрокаОсновныеСредства.Документ.СчетСписанияБУ);
					Движение.СчетДт =Структура.Счет;
					
				ИначеЕсли ТипЗнч(ТекСтрокаОсновныеСредства.Документ) = Тип("ДокументСсылка.ПередачаОС")	 Тогда
					Структура =  абс_МеждународныйУчетОСРЖД.ПреобразоватьСчетаБУвСчетМСФОРЖД(ТекСтрокаОсновныеСредства.Документ.ОС[0].СчетРасходовБУ);
					Движение.СчетДт =Структура.Счет;
					
				КонецЕсли;
				
				Движение.СчетКт = ПланыСчетов.абс_МеждународныйОСРЖД.ВыбытиеОсновныхСредств;//ОсновныеСредстваДляПродажи; //201
				...
				Если ТипЗнч(ТекСтрокаОсновныеСредства.Документ) = Тип("ДокументСсылка.абс_ЧастичнаяЛиквидацияОС")
					ИЛИ ТипЗнч(ТекСтрокаОсновныеСредства.Документ) = Тип("ДокументСсылка.СписаниеОС") Тогда
					Если не Движение.СчетДт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы,) = Неопределено Тогда
						Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы] = ТекСтрокаОсновныеСредства.Документ.СубконтоБУ;
					КонецЕсли;
				...
				Если ТипЗнч(ТекСтрокаОсновныеСредства.Документ) = Тип("ДокументСсылка.ПередачаОС") Тогда
					Если не Движение.СчетДт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы,) = Неопределено Тогда
						Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы] = ТекСтрокаОсновныеСредства.Документ.ОС[0].СубконтоБУ;
					КонецЕсли;
					
					Если не Движение.СчетКт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы,) = Неопределено Тогда
						Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы] = ТекСтрокаОсновныеСредства.Документ.ОС[0].СубконтоБУ;
					КонецЕсли;
				КонецЕсли;
	...
	Для Каждого ТекСтрокаОсновныеСредства Из КопияТЗ Цикл
		...
		Для Каждого ЗаписьБУ Из НаборПроводокБУ Цикл
			Если Не ЗаписьБУ.СчетДт.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ОсновныеСредства) И
				...
				СтруктураДт = абс_МеждународныйУчетОСРЖД.ПреобразоватьСчетаБУвСчетМСФОРЖД(ЗаписьБУ.СчетДт);
				СтруктураКт = абс_МеждународныйУчетОСРЖД.ПреобразоватьСчетаБУвСчетМСФОРЖД(ЗаписьБУ.СчетКт);
				Если СтруктураДт.Счет=Неопределено Или СтруктураКт.Счет=Неопределено Тогда
					Продолжить;
				КонецЕсли;
				Если ЗначениеЗаполнено(СтруктураДт.Счет) И ЗначениеЗаполнено(СтруктураКт.Счет) Тогда
					...
					Если не Движение.СчетДт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты,) = Неопределено Тогда
						Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты] = ЗаписьБУ.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты];
					КонецЕсли;
					Если не Движение.СчетКт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты,) = Неопределено Тогда
						Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты] = ЗаписьБУ.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты];
					КонецЕсли;
					//Контрагенты
					
					//Договоры
					Если не Движение.СчетДт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры,) = Неопределено Тогда
						Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры] = ЗаписьБУ.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры];
					КонецЕсли;
					Если не Движение.СчетКт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры,) = Неопределено Тогда
						Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры] = ЗаписьБУ.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры];
					КонецЕсли;	   
					//Договоры
					
					//Номенклатурные группы
					Если не Движение.СчетДт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы,) = Неопределено Тогда
						Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы] = ЗаписьБУ.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы];
					КонецЕсли;
					Если не Движение.СчетКт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы,) = Неопределено Тогда
						Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы] = ЗаписьБУ.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы];
					КонецЕсли;	   
					//Номенклатурные группы
					
					//Прочие доходы и расходы
					Если не Движение.СчетДт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы,) = Неопределено Тогда
						Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы] = ЗаписьБУ.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы];
					КонецЕсли;
					Если не Движение.СчетКт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы,) = Неопределено Тогда
						Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы] = ЗаписьБУ.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы];
					КонецЕсли;	   
					//Прочие доходы и расходы
					//ВидыПлатежейВБюджетИФонды
					Если не Движение.СчетДт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыПлатежейВГосБюджет,) = Неопределено Тогда
						Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыПлатежейВГосБюджет] = ЗаписьБУ.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыПлатежейВГосБюджет];
					КонецЕсли;
					Если не Движение.СчетКт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыПлатежейВГосБюджет,) = Неопределено Тогда
						Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыПлатежейВГосБюджет] = ЗаписьБУ.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыПлатежейВГосБюджет];
					КонецЕсли;	   
					//ВидыПлатежейВБюджетИФонды
					
					
					//СтавкиНДС
					Если не Движение.СчетДт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтавкиНДС,) = Неопределено Тогда
						Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтавкиНДС] = ЗаписьБУ.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтавкиНДС];
					КонецЕсли;
					Если не Движение.СчетКт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтавкиНДС,) = Неопределено Тогда
						Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтавкиНДС] = ЗаписьБУ.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтавкиНДС];
					КонецЕсли;	   
					//ВидыПлатежейВБюджетИФонды
				Иначе
	...
	МассивДвиженийУдалить = Новый Массив;
	Для Каждого Движение Из Движения.абс_МеждународныйОСРЖД Цикл
		Если Не ЗначениеЗаполнено(Движение.СчетДт) Или Не ЗначениеЗаполнено(Движение.СчетКт) Тогда
			МассивДвиженийУдалить.Добавить(Движение);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого Движение Из МассивДвиженийУдалить Цикл
		Движения.абс_МеждународныйОСРЖД.Удалить(Движение);
	КонецЦикла;
	
	//АБС ВСТАВКА 36048  11.03.2014 17:10:03  Глебов
	ЗаполнитьСтоимостьОСРЖД();
	//АБС ВСТАВКА 36048 КОНЕЦ 
	
	Движения.абс_ОсновныеСредстваРЖД.Записать();
	Движения.абс_МеждународныйОСРЖД.Записать();
...
Функция ПолучитьОстаточнуюСтоимостьБУ()
	...
	//АБС ИЗМЕНЕНИЕ 36048  31.01.2014 12:45:24  Пугачев
	Запрос.УстановитьПараметр("КонПериода",Новый Граница(ЭтотОбъект.МоментВремени(), ВидГраницы.Исключая));
	//АБС ИЗМЕНЕНИЕ 36048 КОНЕЦ 
...
Функция ПолучитьОстаткиМСФО()
	//АБС ИЗМЕНЕНИЕ  71572
	МассивДт = Новый Массив;
	МассивДт.Добавить(ПланыСчетов.абс_МеждународныйОСРЖД.ОсновныеСредстваВОрганизации);
	//МассивДт.Добавить(ПланыСчетов.Хозрасчетный.ВыбытиеОС);
	//МассивДт.Добавить(ПланыСчетов.Хозрасчетный.ПрочиеРасходыНеОблагаемыеЕНВД);
	
	// МассивКт = Новый Массив;
	МассивДт.Добавить(ПланыСчетов.абс_МеждународныйОСРЖД.АмортизацияОсновныхСредствНа01);
	//МассивКт.Добавить(ПланыСчетов.Хозрасчетный.ВыбытиеОС);
	...
	Запрос.Текст ="ВЫБРАТЬ
	              ...
	              |	СУММА(ВЫБОР
	              |			КОГДА МеждународныйОстатки.Счет = ЗНАЧЕНИЕ(ПланСчетов.абс_МеждународныйОСРЖД.ОсновныеСредстваВОрганизации)
	              |				ТОГДА МеждународныйОстатки.СуммаОстаток
	              |			ИНАЧЕ 0
	              |		КОНЕЦ) КАК Сумма01Кон,
	              |	СУММА(ВЫБОР
	              |			КОГДА МеждународныйОстатки.Счет = ЗНАЧЕНИЕ(ПланСчетов.абс_МеждународныйОСРЖД.АмортизацияОсновныхСредствНа01)
	              |				ТОГДА -МеждународныйОстатки.СуммаОстаток
	              |			ИНАЧЕ 0
...
//АБС ВСТАВКА 36048  11.03.2014 16:21:47  Глебов
Процедура ЗаполнитьОС() Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	РегистрБухгалтерии.абс_МеждународныйОСРЖД.Остатки(&КонецПериода, Счет = ЗНАЧЕНИЕ(ПланСчетов.абс_МеждународныйОСРЖД.ОсновныеСредстваВОрганизации), , ) КАК МеждународныйОстатки
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	...
	Пока Выборка.Следующий() Цикл
		...
		//АБС
		СрезСведенийМежд = РегистрыСведений.абс_ОсновныеСредстваРЖД.ПолучитьПоследнее(Дата, Новый Структура("ОсновноеСредство", СтрокаТП.ОсновноеСредство));
...
Процедура ЗаполнитьСтоимостьОСРЖД()
	...
	Для каждого Движение Из Движения.абс_МеждународныйОСРЖД Цикл		
		...
		КонецЕсли;
	КонецЦикла;	
	Движения.абс_СтоимостьОСРЖД.Записать();
КонецПроцедуры
