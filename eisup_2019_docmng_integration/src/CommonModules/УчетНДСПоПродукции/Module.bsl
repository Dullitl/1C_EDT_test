		...
		Для каждого СтрокаПоСчетамФактурам из МассивСтрокПоСчетамФактурам Цикл
			...
			Для каждого СтрокаПоЗатратамНаВыпуск из МассивСтрокПоЗатратамНаВыпуск Цикл
				...
				Если СтрокаПоЗатратамНаВыпуск.ВидЗатрат = Перечисления.ВидыЗатрат.Материальные Тогда
					Показатель = СтрокаПоСчетамФактурам.КоличествоОстаток;
				Иначе
				 	Показатель = СтрокаПоСчетамФактурам.СтоимостьОстаток;
				КонецЕсли;
...
Процедура КорректировкаДвиженийПоВыпускуПродукцииНДС(
	...
	Пока ВыборкаПоНДСПродукция.Следующий() Цикл
		...
		МассивСумм = ttk_ОбщегоНазначения.РаспределитьПропорционально(ВыборкаПоНДСПродукция.СуммаЗатрат, МассивКоэф, 2);
		МассивНДС  = ttk_ОбщегоНазначения.РаспределитьПропорционально(ВыборкаПоНДСПродукция.СуммаЗатратНДС, МассивКоэф, 2);
