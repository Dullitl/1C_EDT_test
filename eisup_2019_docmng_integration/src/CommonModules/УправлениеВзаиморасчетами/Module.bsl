	    ...
	    Если (СтрокаТаблицы.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоЗаказам
		  ИЛИ СтрокаТаблицы.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоСчетам) 
		   И НЕ ЗначениеЗаполнено(СтрокаТаблицы.Сделка) Тогда
				...
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "выбран договор, по которому взаиморасчеты ведутся по заказам. " +
	...
	Если ЕстьЗаказыВТабличныхЧастях Тогда
		...
		Для каждого ТабЧасть Из СтруктураПараметров.СтруктураТабличныхЧастей Цикл
			...
			Иначе
				...
				ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТекТабЧасть, ТаблицаСделок);
			КонецЕсли;
...
Процедура ЗаполнитьТаблицуДокументовРасчетовСКонтрагентом(ДокументОбъект, СтруктураПараметров) Экспорт
	...
	КонецЕсли; 
	СделкиДокумента = ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(ТаблицаСделок.ВыгрузитьКолонку("Сделка"));
...
Процедура ДополнитьСтруктуруПараметровДляЗаполненияТаблицыДокументовРасчетов(ДокументОбъект, СтруктураПараметров) Экспорт
	...
	//***** Гетц. Разнесение платежей. (06.11.13)
	ИспользоватьРазнесениеПлатежей = БухгалтерскийУчетРасчетовСКонтрагентами.ПолучитьИспользованиеРаспределенияПлатежей(ДокументОбъект, СтруктураПараметров.ДоговорКонтрагента);
	...
	Иначе
		СтруктураПараметров.Вставить("Дата", ОбщегоНазначенияЗК.ПолучитьРабочуюДату());	
	КонецЕсли;
...
Функция СформироватьТаблицуСделокДляПроведенияПоВзаиморасчетам(СтруктураПараметров, СтруктураШапкиДокумента) 
	...
	ТаблицаСделок.Колонки.Добавить(ИмяКолонкиСделка, Новый ОписаниеТипов("ДокументСсылка.ЗаказПокупателя, ДокументСсылка.ЗаказПоставщику, 
		|ДокументСсылка.СчетНаОплатуПокупателю, ДокументСсылка.СчетНаОплатуПоставщика, ДокументСсылка.абс_СчетНаОплату, Неопределено"), ИмяКолонкиСделка);
	...
	Если СтруктураШапкиДокумента.Свойство("ВидДокумента") Тогда  		
		...
		Если СтруктураШапкиДокумента.ВидДокумента = "КорректировкаРеализации" Тогда
			ДопЧисловыеКолонки = "врСуммаВзаиморасчетов, врСуммаРегл, врСуммаУпр";
		КонецЕсли;
		//\\АБС ВСТАВКА КОНЕЦ
	КонецЕсли;
	
	ЧисловыеКолонки = ОбщиеЧисловыеКолонки + ?(ДопЧисловыеКолонки = "","",", " + ДопЧисловыеКолонки);
	...
	ТаблицаСделок.Свернуть(ИмяКолонкиСделка, ОбщиеЧисловыеКолонки + ?(ДопЧисловыеКолонки = "","",", " + ДопЧисловыеКолонки));
...
Функция ПодготовитьТаблицуПоВзаиморасчетам(ДокументОбъект, СтруктураПараметров, СтруктураШапкиДокумента, ТаблицаСделокДокумента, Отказ, Заголовок)
	...
	Если СтруктураШапкиДокумента.ВедениеВзаиморасчетов <> Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоЗаказам Тогда
		...
		КонецЕсли;
		ТаблицаСделок.Свернуть("Сделка", "СуммаВзаиморасчетов, СуммаРегл, СуммаУпр"+ДопЧисловыеКолонки);
	КонецЕсли;
	...
	//АБС ВСТАВКА НАЧАЛО
	ТаблицаПоВзаиморасчетам.Колонки.Добавить("врСуммаВзаиморасчетов", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	ТаблицаПоВзаиморасчетам.Колонки.Добавить("врСуммаРегл", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	ТаблицаПоВзаиморасчетам.Колонки.Добавить("врСуммаУпр", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	//\\АБС ВСТАВКА КОНЕЦ
	...
	Если СтруктураШапкиДокумента.ВидДокумента <> "КорректировкаПоступления"
		...
		Для каждого СтрокаТабЧасти Из ДокументОбъект.ДокументыРасчетовСКонтрагентом Цикл
			...
			//АБС ВСТАВКА НАЧАЛО								
			СтрокаВзаиморасчетов.врСуммаУпр = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СтрокаВзаиморасчетов.врСуммаВзаиморасчетов, 
			                                СтруктураШапкиДокумента.ВалютаВзаиморасчетов, СтруктураШапкиДокумента.ВалютаУправленческогоУчета,
			                                СтруктураШапкиДокумента.КурсВзаиморасчетов, СтруктураШапкиДокумента.КурсВалютыУправленческогоУчета, 
			                                СтруктураШапкиДокумента.КратностьВзаиморасчетов, СтруктураШапкиДокумента.КратностьВалютыУправленческогоУчета);										
			//\\АБС ВСТАВКА КОНЕЦ								
	...
	Если СтруктураШапкиДокумента.ВедениеВзаиморасчетов <> Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоЗаказам Тогда
		...
		Если ВсегоСуммаВзаиморасчетов <> ВсегоСуммаПредоплаты Тогда
			...
			СтрокаВзаиморасчетов.Сделка                        = СделкаДляВзаиморасчетов;
			СтрокаВзаиморасчетов.ДокументРасчетовСКонтрагентом = ДокументРасчетовСКонтрагентом;//ДокументОбъект.Ссылка;
	...
	//абсо - +// внесено в один цикл со стандартным - см. ниже
	//Множитель = ?(ЭтоВозврат, -1, 1);
	//Для каждого СтрокаВзаиморасчетов Из ТаблицаПоВзаиморасчетам Цикл
	//	//АБС
	//	Если ТипЗнч(ДокументОбъект)<>  Тип("ДокументОбъект.РеализацияТоваровУслуг")
	//		И ТипЗнч(ДокументОбъект)<>  Тип("ДокументОбъект.ПоступлениеТоваровУслуг") Тогда
	//		//АБС	
	...
	//			ttk_ОбщегоНазначения.СообщитьОбОшибке("Указана сумма предоплаты (сделка: " + ПредставлениеСделки + ") больше возможной по документу на " 
	...
	Если СтруктураШапкиДокумента.ВидДокумента <> "КорректировкаПоступления"
		...
		//абсо - для сохранения совместимости +
		И СтруктураШапкиДокумента.ВидДокумента <> "РеализацияТоваровУслуг"
		И СтруктураШапкиДокумента.ВидДокумента <> "ПоступлениеТоваровУслуг"
		Тогда
	   ...
	   И ВалютаРегламентированногоУчета <> ВалютаДокумента Тогда
		...
		Если ОтражатьВБухгалтерскомУчете 
		 Или ОтражатьВНалоговомУчете Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(ДополнениеКСообщению + " Валюта документа ("+СокрЛП(ВалютаДокумента)
	...
	ИначеЕсли ВалютаДокумента = ВалютаРегламентированногоУчета Тогда
		...
		Если (Не ВалютаВзаиморасчетов = ВалютаРегламентированногоУчета) 
		   И Не(РасчетыВУсловныхЕдиницах) Тогда
			...
			Если ОтражатьВБухгалтерскомУчете 
			 Или ОтражатьВНалоговомУчете Тогда 
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ДополнениеКСообщению + " Валюта расчетов по договору """ + СокрЛП(ДоговорКонтрагента) 
	...
	Иначе
		...
		Если РасчетыВУсловныхЕдиницах
		   И ЭтоДокументОплаты Тогда 
			...
			Если ОтражатьВБухгалтерскомУчете 
			 Или ОтражатьВНалоговомУчете Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ДополнениеКСообщению + " Документ оплаты при расчете по договору (" + СокрЛП(ДоговорКонтрагента) 
...
Процедура ОтражениеЗадолженностиВРегистреОперативныхРасчетовПоДокументам(СтруктураШапкиДокумента, ТаблицаВзаиморасчетов, ВидРасчетовСКонтрагентом, ВидДвижения, Движения, Отказ, Заголовок) Экспорт
	...
	ВестиПоДокументамРасчетовСКонтрагентом = СтруктураШапкиДокумента.ДоговорКонтрагента.ВестиПоДокументамРасчетовСКонтрагентом; //***** АБС. Гетц. 02.12.13. заявка №000022702
	...
	УпрУчет            = Ссылка.ОтражатьВУправленческомУчете;
