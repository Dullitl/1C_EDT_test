...
Функция КорректныйПорядокНазначенияPLU(ПорядокНазначения) Экспорт
	...
	Если ПорядокНазначения = ПерПорядки.ПоПорядку Тогда
		Результат = Истина;
	ИначеЕсли ПорядокНазначения = ПерПорядки.ПоКодуНоменклатуры Тогда
		...
		|		РегистрСведений.ТоварыНаККМ КАК РегТовары2
		|		ПО РегТовары1.Код <> РегТовары2.Код
		|		 И РегТовары1.Номенклатура = РегТовары2.Номенклатура
		|		 И РегТовары1.КассаККМ = РегТовары2.КассаККМ");
		...
		Если ТипШтрихкода = ПланыВидовХарактеристик.ТипыШтрихкодов.EAN13 Тогда
			Если (ДлинаКода <> 13)
			 ...
			 Или КонтрольныйСимволEAN(Лев(Штрихкод,12), 13) <> Прав(Штрихкод, 1) Тогда
				Возврат Ложь;
			КонецЕсли;
		ИначеЕсли ТипШтрихкода = ПланыВидовХарактеристик.ТипыШтрихкодов.ITF14 Тогда
			Если (ДлинаКода <> 14)
			 Или НЕ(ТолькоЦифры(Штрихкод)) // штрихкод должен состоять из цифр
			 Или КонтрольныйСимволEAN(Лев(Штрихкод,13), 14) <> Прав(Штрихкод, 1) Тогда
				Возврат Ложь;
			КонецЕсли;
...
Процедура ИмпортироватьОбработки(ПроцессОбновления, ВидТО = Неопределено, ИмпортироватьССайта = Ложь, АдресИПараметрыСервера = Неопределено) Экспорт
	...
	Если Не ПроцессОбновления Тогда
		Для Каждого ФайлОО Из СписокФайлов Цикл
			...
			Исключение
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Ошибка при обработке файла: " + ФайлОО.Имя + Символы.ПС
				...
				Если ООИзСправочника = Неопределено Тогда
					...
					Попытка
						Если ОбъектСправочника.ПолучитьФорму().ЗагрузитьОбработкуОбслуживания(ФайлОО.ПолноеИмя) Тогда
							...
							ttk_ОбщегоНазначения.СообщитьОбОшибке("Элемент справочника """ + ОбъектСправочника.Наименование + """ (" + СокрЛП(Строка(ОбъектСправочника.Код)) + ") добавлен.", , , СтатусСообщения.Информация);
							ДобавленоЗаписей = ДобавленоЗаписей + 1;
						КонецЕсли;
					Исключение
						ttk_ОбщегоНазначения.СообщитьОбОшибке("Не удалось записать элемент с кодом " + ОбъектСправочника.Код, , , СтатусСообщения.Внимание);
					КонецПопытки;
				Иначе
					...
					Если ВерсияИзСпр < ВерсияИзФайла Тогда // Если версия ОО из файла равна или меньше версии в справочнике, то не обновляемся
						...
						Исключение
							ttk_ОбщегоНазначения.СообщитьОбОшибке("Не удалось заблокировать для изменения элемент с кодом " + ОбъектСправочника.Код + "
						...
						Попытка
							Если ОбъектСправочника.ПолучитьФорму().ЗагрузитьОбработкуОбслуживания(ФайлОО.ПолноеИмя) Тогда
								ОбъектСправочника.Записать();
								ttk_ОбщегоНазначения.СообщитьОбОшибке("Элемент справочника """ + ОбъектСправочника.Наименование + """ (" + СокрЛП(Строка(ОбъектСправочника.Код)) + ") обновлен.", , , СтатусСообщения.Информация);
			...
			Иначе
				Если Не ПроцессОбновления Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Файл " + ФайлОО.Имя + " не загружен." + Символы.ПС 
...
Процедура ЭкспортироватьОбработки(ВидТО = Неопределено) Экспорт
	...
	Для Каждого ЭлементСправочника Из ЭлементыСправочника Цикл
		...
		Попытка
			...
			КонецЕсли;
		Исключение
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не удалось записать " + ЭлементСправочника.ИмяФайла 
