	...
	Пока ВыборкаКлючейДокументов.Следующий() Цикл
		...
		Если НовыйДокумент.ОтражатьВБухгалтерскомУчете Тогда
			...
			Для Каждого ДанныеТЧ Из ТабличныеЧастиСоСчетами Цикл
				НовыйДокумент.ЗаполнитьСчетаУчетаВТабЧасти(НовыйДокумент[ДанныеТЧ.Ключ], ДанныеТЧ.Ключ, НовыйДокумент.ОтражатьВБухгалтерскомУчете, НовыйДокумент.ОтражатьВНалоговомУчете);
			КонецЦикла;
	...
	Пока ВыборкаКлючейДокументов.Следующий() Цикл
		...
		Иначе
			...
			Если ЗначениеЗаполнено(НовыйДокумент.Организация) И НЕ НовыйДокумент.УчитыватьНДС Тогда
				НовыйДокумент.УчитыватьНДС = НЕ НалоговыйУчетУСН.ПрименениеУСН(НовыйДокумент.Организация,НовыйДокумент.Дата);
			КонецЕсли;
