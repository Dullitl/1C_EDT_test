...
Процедура ОпределениеДополнительныхПараметровТаблицыПартийДляПодсистемыУчетаНДС(СтруктураШапкиДокумента, ТаблицаЗначений) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
		...
		СтрокаТаблицы.ВидЦенности       = ?(ЗначениеЗаполнено(СтрокаТаблицы.ВидЦенности),СтрокаТаблицы.ВидЦенности,ОпределитьВидЦенности(Ценность, СчетУчетаЦенности, ЭтоДопРасходы, ЭтоТаможенныйНДС, УчетАгентскогоНДС, ВидАгентскогоДоговора, ?(ЕстьКолонкаУслуга, СтрокаТаблицы.Услуга, Неопределено), СтруктураШапкиДокумента, СпособСтроительства, ВидыЦенностейПоСчетамУчета));
...
Функция СинхронизацияПометкиНаУдалениеУСчетаФактуры(ДокументОбъект, ВидСчетаФактуры = "СчетФактураВыданный", Отказ = Ложь) Экспорт
	...
	Если ДокументОбъект.ПометкаУдаления <> ?(Результат.Количество() > 0, Результат[0].ПометкаУдаления,ложь) Тогда
		...
		СчетФактура = УчетНДС.НайтиПодчиненныйСчетФактуру(ДокументОбъект.Ссылка, ВидСчетаФактуры, СтруктураОтбора, ДокументОбъект.Ссылка);
...
Процедура СинхронизацияПроведенияУСчетаФактуры(ДокументОбъект, ВидСчетаФактуры = "СчетФактураВыданный", Отказ = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СчетФактура)
		...
		// АБС ВСТАВКА Фролов 20111118 Зачем перезаписывать СФ полученный при записи документа - непонятно
		//Если ДокументОбъект.Проведен И (Не СчетФактура.Проведен Или Не ВидСчетаФактуры = "СчетФактураВыданный") Тогда
		Если ДокументОбъект.Проведен И (Не СчетФактура.Проведен Или Ложь) Тогда
		//Если ДокументОбъект.Проведен И (Не СчетФактура.Проведен Или Не ВидСчетаФактуры = "СчетФактураВыданный") Тогда
		// АБС ВСТАВКА Фролов 20111118 Зачем перезаписывать СФ полученный при записи документа - непонятно КОНЕЦ
...
//АБС ВСТАВКА 36164  11.12.2013 15:24:04  Стрельцов
...
Процедура ПолучитьПараметрыСчетаФактуры(СчетФактура, мВалютаРегламентированногоУчета, Результат) Экспорт
	...
	Результат = Новый Структура("Организация, Контрагент, Договор, СуммаДокумента, ВалютаДокумента, 
		...
		//АБС ВСТАВКА 36164  11.12.2013 14:38:23  Стрельцов
		|РеквизитыОснований");
		//\\АБС ВСТАВКА 36164 КОНЕЦ 
	...
	//АБС ВСТАВКА 36164  11.12.2013 14:38:48  Стрельцов
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.СчетФактураВыданный") 
		...
		ИЛИ СчетФактура.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаСуммовуюРазницу) Тогда
		// Реквизиты заново не определяются, получаются из счета-фактуры
		Результат.Вставить("Организация"	, СчетФактура.Организация);
		Результат.Вставить("Контрагент"		, СчетФактура.Контрагент);
		Результат.Вставить("Договор"		, СчетФактура.ДоговорКонтрагента);
	...
	//АБС ВСТАВКА 36164  11.12.2013 15:35:08  Стрельцов
	Корректировка = Ложь;
	//\\АБС ВСТАВКА 36164 КОНЕЦ 
	...
	Для каждого ТипОснования Из ТипыОснований Цикл
		...
		//АБС ВСТАВКА 36164  11.12.2013 15:46:33  Стрельцов
		...
		ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.КорректировкаПоступления") Тогда
			...
			ТекстЗапроса =
			//АБС ИЗМЕНЕНИЕ 36164  11.12.2013 15:42:13  Стрельцов
			"ВЫБРАТЬ
		...
		ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
			...
			Запрос.УстановитьПараметр("КорректировочныйСчетФактура", СчетФактура.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.Корректировочный);
		...
		ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.ПоступлениеДопРасходов") Тогда
			...
			|	ПоступлениеДопРасходов.ВалютаДокумента";
// +++ввв 18.10.2016 г.			
		ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах") Тогда
			
			Запрос.УстановитьПараметр("ДокументОснование_ОтчетКомиссионераОПродажах", ДокументыОснования);
			...
			|	Таблица.Ссылка.ДоговорКонтрагента КАК Договор,
			|	СУММА(Таблица.Сумма + ВЫБОР
			|			КОГДА Таблица.Ссылка.ЗатратыСуммаВключаетНДС
			|				ТОГДА 0
			|			ИНАЧЕ Таблица.СуммаНДС
			|		КОНЕЦ) КАК СуммаДокумента,
			|	Таблица.Ссылка.ВалютаДокумента КАК ВалютаДокумента,
			|	СУММА(Таблица.СуммаНДС) КАК СуммаНДСДокумента,
			|	СУММА(ВЫБОР
			|			КОГДА Таблица.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС)
			|				ТОГДА 0
			|			ИНАЧЕ 1
			|		КОНЕЦ) КАК ЕстьНДС
			|ИЗ
			|	Документ.ОтчетКомиссионераОПродажах.ввв_ПТУ КАК Таблица
			|ГДЕ
			|	Таблица.Ссылка В (&ДокументОснование_ОтчетКомиссионераОПродажах)
			|
			|СГРУППИРОВАТЬ ПО
			|	Таблица.Ссылка,
			|	Таблица.Ссылка.Организация,
...
// ---ввв 18.10.2016 г.
	...
	Иначе
		...
		Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);  //АБС ИЗМЕНЕНИЕ 36164  11.12.2013 15:49:28  Стрельцов
		...
		Пока Выборка.Следующий() Цикл
			...
			//АБС ВСТАВКА 36164  11.12.2013 15:50:04  Стрельцов
			...
			Если Корректировка Тогда
				...
				Иначе
					ЭтоКорректировочный = СчетФактура.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.Корректировочный;						
				КонецЕсли;
...
// +++ввв 18.10.2016 г.
	...
	//	ИгнорироватьТЧ.Добавить("Товары");
	//	ИгнорироватьТЧ.Добавить("СерийныеНомера");
	//	ИгнорироватьТЧ.Добавить("ДокументыРасчетовСКонтрагентом");
	//	ИгнорироватьТЧ.Добавить("Покупатели");
	//	ИгнорироватьТЧ.Добавить("kmw_Услуги");
	//	ИгнорироватьТЧ.Добавить("kmw_ЗатратыУслуги");
	//	ИгнорироватьТЧ.Добавить("kmw_Маппинг");
	//	ИгнорироватьТЧ.Добавить("ввв_РТУ");
	//	ИгнорироватьТЧ.Добавить("Ф_Округление");
	//	ПостфиксСумм = "";
	//	ИмяРеквизитаСтавкаНДС = "Ссылка.СтавкаНДС";
	//ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") И НЕ ЭтоПолученныйСФ
	Если ТипДокументаОснования = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") И НЕ ЭтоПолученныйСФ
// ---ввв 18.10.2016 г.
...
Функция ОпределитьНаличиеСчетовФактурПолученных(НачПериода = Неопределено, КонПериода = Неопределено, Организация, Фильтр = Неопределено, ВсеКромеФильтра = Ложь, НаличиеСчетаФактуры = Неопределено, СчетФактураПроведен = Неопределено, ДатаСФНеБолее = Неопределено) Экспорт
	...
	УсловиеЗапроса = УсловиеЗапроса
	               + ?(СчетФактураПроведен = Неопределено, "", ?(НЕ ЗначениеЗаполнено(УсловиеЗапроса), "", " И ") + "
					|	ВЫБОР
					|		КОГДА НДСПредъявленныйОбороты.СчетФактура ССЫЛКА Документ.СчетФактураВыданный
					|			ТОГДА ВЫРАЗИТЬ(НДСПредъявленныйОбороты.СчетФактура КАК Документ.СчетФактураВыданный).Проведен
	...
	УсловиеЗапроса = УсловиеЗапроса
	               + ?(НЕ ЗначениеЗаполнено(ДатаСФНеБолее), "", ?(НЕ ЗначениеЗаполнено(УсловиеЗапроса), "", " И ") + "
					|	ВЫБОР
					|		КОГДА НДСПредъявленныйОбороты.СчетФактура ССЫЛКА Документ.СчетФактураВыданный
					|			ТОГДА ВЫРАЗИТЬ(НДСПредъявленныйОбороты.СчетФактура КАК Документ.СчетФактураВыданный).Дата
					|		КОГДА НДСПредъявленныйОбороты.СчетФактура ССЫЛКА Документ.ГТДИмпорт
	...
	|			КОГДА НДСПредъявленныйОбороты.СчетФактура ССЫЛКА Документ.СчетФактураВыданный
	|				ТОГДА ВЫРАЗИТЬ(НДСПредъявленныйОбороты.СчетФактура КАК Документ.СчетФактураВыданный).ДокументОснование
	...
	КонецЕсли; 
	Запрос.Текст = Запрос.Текст + " УПОРЯДОЧИТЬ ПО СчетФактураДата";
	...
	Если ТипЗнч(ДокОбъект.Ссылка) = Тип("ДокументСсылка.АвансовыйОтчет") Тогда 
		...
		Для Каждого СтрокаТаблицы Из ДокОбъект.Ссылка.Товары Цикл
			Если ЗначениеЗаполнено(СтрокаТаблицы.СчетФактура) Тогда
				СчетаФактуры.Добавить(СтрокаТаблицы.СчетФактура);
			КонецЕсли;
		КонецЦикла;
		...
		ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(СчетаФактуры);
	...
	Для Каждого СчетФактура Из СчетаФактуры Цикл
		...
		Если ДокОбъект.Ссылка.Дата <> ДокОбъект.Дата
		   И НачалоДня(ДокОбъект.Дата) <> НачалоДня(СчетФактура.Дата) Тогда
			Если Сообщать Тогда
				ОбщегоНазначения.Сообщение("Изменена дата документа-основания. Возможно требуется внести изменения в счет-фактуру.", СтатусСообщения.Внимание);
			КонецЕсли;
		...
		Если ТипЗнч(ДокОбъект.Ссылка) = Тип("ДокументСсылка.АвансовыйОтчет") Тогда
			СуммаДокумента = 0;
			СтрокиПоСчетуФактуре = ДокОбъект.Товары.НайтиСтроки(Новый Структура("СчетФактура", СчетФактура));
			Для Каждого СтрокаТаблицы Из СтрокиПоСчетуФактуре Цикл
				СуммаДокумента = СтрокаТаблицы.Сумма + ?(ДокОбъект.СуммаВключаетНДС, 0, СтрокаТаблицы.СуммаНДС) + СуммаДокумента; //АБС  + СуммаДокумента
			КонецЦикла;
			СтрокиПоСчетуФактуре = ДокОбъект.Прочее.НайтиСтроки(Новый Структура("СчетФактура", СчетФактура));
			Для Каждого СтрокаТаблицы Из СтрокиПоСчетуФактуре Цикл
				СуммаДокумента = СтрокаТаблицы.Сумма + ?(ДокОбъект.СуммаВключаетНДС, 0, СтрокаТаблицы.СуммаНДС) + СуммаДокумента; //АБС  + СуммаДокумента
			КонецЦикла;
			
			Если СчетФактура.СуммаДокумента <> СуммаДокумента Тогда
				Если Сообщать Тогда
				...
				Флаг = Ложь;
			КонецЕсли;	
		ИначеЕсли МетаРекв.Найти("СуммаДокумента") <> Неопределено И ДокОбъект.Ссылка.СуммаДокумента <> ДокОбъект.СуммаДокумента Тогда
			Если Сообщать Тогда
				Текст = Текст + ?(ПустаяСтрока(Текст),"", Символы.ПС) + " - Изменилась сумма документа";
			КонецЕсли;
		...
		Если Сообщать И НЕ ПустаяСтрока(Текст) 
			и СчетФактура.Метаданные().ТабличныеЧасти.Найти("ДокументыОснования") <> Неопределено Тогда
			...
			Если СчетФактура.ДокументыОснования.Количество() = 1 
				Или СтруктураРеквизиты.Количество() = 1 И СтруктураРеквизиты.Свойство("СуммаДокумента") Тогда
				...
				Попытка
					...
					Для Каждого Элемент Из СтруктураРеквизиты Цикл
						...
						// Если изменена организация, то надо обновить номер документа
							СчетФактураОбъект.Номер = "";
						КонецЕсли;							

					КонецЦикла;
					СчетФактураОбъект.мОбновлятьРеквизитыПриЗаписи = Ложь;
					//Бобылев А.А. 22.02.2018 СППР 00000012
					СчетФактураОбъект.Записать(РежимЗаписиДокумента.Проведение);
					//Бобылев А.А. -------------------------
				...
				Если флУдалосьПерезаполнить Тогда
					...
					Если СтруктураРеквизиты.Свойство("Организация") Тогда
						Текст = Текст + Символы.ПС+"Изменен номер счета-фактуры в связи с изменением организации";
					КонецЕсли;
				Иначе
			...
			ОбщегоНазначения.Сообщение(Текст, СтатусСообщения.Внимание);
		КонецЕсли;
...
Процедура ОтразитьПредположениеСтавки0(СтруктураШапкиДокумента, ТаблицаСписания, СтруктураПараметров, Заголовок) Экспорт
	...
	Если  ТипЗнч(СтруктураШапкиДокумента.Ссылка) = Тип("ДокументСсылка.РеализацияОтгруженныхТоваров")
		...
		Возврат;
	ИначеЕсли ТипЗнч(СтруктураШапкиДокумента.Ссылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
...
Процедура ДвижениеНДСпоТаблицеДокументов(ТаблицаДокументов,ФормироватьПроводки = Истина) Экспорт
	...
	Для Каждого Строка из ТаблицаДокументов Цикл
		...
		Если ТаблицаСписания.Количество()>0 Тогда
			...
			СтруктураПараметров.ДвиженияПартииТоваровНаСкладахБух.Прочитать();
			ТаблицаДвижений = СтруктураПараметров.ДвиженияПартииТоваровНаСкладахБух.Выгрузить();
			ЗагрузитьВТаблицуЗначенийСписанныеПартии(ТаблицаДвижений,СтруктураПараметров.ТаблицаДвиженийПартииТоваровНаСкладахБух);
			
			СтруктураПараметров.ДвиженияПартииТоваровПереданныеБух.Прочитать();
			ТаблицаДвижений = СтруктураПараметров.ДвиженияПартииТоваровПереданныеБух.Выгрузить();
			ЗагрузитьВТаблицуЗначенийСписанныеПартии(ТаблицаДвижений,СтруктураПараметров.ТаблицаДвиженийПартииТоваровПереданныеБух);
			
			СтруктураПараметров.ДвиженияНезавершенноеПроизводствоБух.Прочитать();
			ТаблицаДвижений = СтруктураПараметров.ДвиженияНезавершенноеПроизводствоБух.Выгрузить();
			ЗагрузитьВТаблицуЗначенийСписанныеПартии(ТаблицаДвижений,СтруктураПараметров.ТаблицаДвиженийНезавершенноеПроизводствоБух);
			
			СтруктураПараметров.ДвиженияБракВПроизводствеБух.Прочитать();
			ТаблицаДвижений = СтруктураПараметров.ДвиженияБракВПроизводствеБух.Выгрузить();
			ЗагрузитьВТаблицуЗначенийСписанныеПартии(ТаблицаДвижений,СтруктураПараметров.ТаблицаДвиженийБракВПроизводствеБух);
			
			СтруктураПараметров.ДвиженияЗатратыБух.Прочитать();
			ТаблицаДвижений = СтруктураПараметров.ДвиженияЗатратыБух.Выгрузить();
			ЗагрузитьВТаблицуЗначенийСписанныеПартии(ТаблицаДвижений,СтруктураПараметров.ТаблицаДвиженийЗатратыБух);
			
			СтруктураПараметров.ДвиженияПартииМатериаловВЭксплуатацииБух.Прочитать();
			ТаблицаДвижений = СтруктураПараметров.ДвиженияПартииМатериаловВЭксплуатацииБух.Выгрузить();
			ЗагрузитьВТаблицуЗначенийСписанныеПартии(ТаблицаДвижений,СтруктураПараметров.ТаблицаДвиженийПартииМатериаловВЭксплуатацииБух);
...
Процедура СформироватьДвиженияПоНДС(СтруктураПараметров, ТаблицаСписания, МассивНоменклатуры, Организация = Неопределено)
	...
	СтруктураШапкиДокумента = УчетНДС.ПолучитьСтруктуруШапкиДокументаПоСсылке(СтруктураПараметров.Регистратор);
	...
	Если Отказ тогда
		Возврат;
	КонецЕсли;
	Если СтруктураШапкиДокумента.ОтгрузкаБезПереходаПраваСобственности И НЕ СтруктураШапкиДокумента.НачислятьНДСПоОтгрузке Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьДвиженияПоРегиструНДСпоОСиНМА_ПеремещениеОборудования(СтруктураШапкиДокумента, СтруктураПараметров, Отказ)
	...
	Строка = "ВозвратОтКомиссионера, ВозвратОтПереработчика, ВозвратПоставщику, ВыпускПоОперацииСтоимость, ВыпускПродукцииФиксНаСклад,
...
Функция ПолучитьТаблицуАвансовыйОтчет(ДокументОбъект, Ошибка)
	...
	Перем СтруктураШапкиДокумента, ТаблицаПоТоварам, мВалютаРегламентированногоУчета;
	...
	Если СтруктураШапкиДокумента.ВедениеУчетаПоПроектам И НЕ ЗначениеЗаполнено(ДокументОбъект.Проект) Тогда
	...
	СтруктураПолейПрочее = Новый Структура;
	СтруктураПолейПрочее.Вставить("ВидДокВходящий"          , "ВидДокВходящий");
	СтруктураПолейПрочее.Вставить("НомерВходящегоДокумента" , "НомерВходящегоДокумента");
	СтруктураПолейПрочее.Вставить("ДатаВходящегоДокумента"  , "ДатаВходящегоДокумента");  	
	СтруктураПолейПрочее.Вставить("Содержание",          "Содержание");
	СтруктураПолейПрочее.Вставить("Номенклатура",        "Номенклатура");
	СтруктураПолейПрочее.Вставить("Услуга",              "Номенклатура.Услуга");
	СтруктураПолейПрочее.Вставить("Набор" ,              "Номенклатура.Набор");
	СтруктураПолейПрочее.Вставить("Комплект",            "Номенклатура.Комплект");
	СтруктураПолейПрочее.Вставить("Заказ",               "Заказ");
	СтруктураПолейПрочее.Вставить("СтатьяЗатрат",        "СтатьяЗатрат");
	СтруктураПолейПрочее.Вставить("ХарактерЗатрат",      "СтатьяЗатрат.ХарактерЗатрат");
	СтруктураПолейПрочее.Вставить("ОбъектСтроительства", "ОбъектСтроительства");
	СтруктураПолейПрочее.Вставить("СпособСтроительства", "СпособСтроительства");
	СтруктураПолейПрочее.Вставить("Количество",          "Количество");
	СтруктураПолейПрочее.Вставить("Сумма",               "Сумма");
	СтруктураПолейПрочее.Вставить("СтавкаНДС",           "СтавкаНДС");
	СтруктураПолейПрочее.Вставить("СуммаНДС",            "СуммаНДС");
	СтруктураПолейПрочее.Вставить("НДС", 				   "СуммаНДС");
	СтруктураПолейПрочее.Вставить("Поставщик",           "Поставщик"); 
	СтруктураПолейПрочее.Вставить("СчетФактура",         "СчетФактура"); 
	СтруктураПолейПрочее.Вставить("Подразделение",       "Подразделение");
	СтруктураПолейПрочее.Вставить("СчетУчетаНДС",        "СчетУчетаНДС"); 
	СтруктураПолейПрочее.Вставить("СпособРаспределенияЗатратНаВыпуск", "СпособРаспределенияЗатратНаВыпуск");
	СтруктураПолейПрочее.Вставить("ПроектЗатрат", "Проект");	
	
	РезультатЗапросаПоТоварам = УправлениеЗапасами.СформироватьЗапросПоТабличнойЧасти(ДокументОбъект, "Товары", СтруктураПолей, СтруктураСложныхПолей);
	РезультатЗапросаПоПрочее = УправлениеЗапасами.СформироватьЗапросПоТабличнойЧасти(ДокументОбъект, "Прочее", СтруктураПолейПрочее,СтруктураСложныхПолей);
	
	ТаблицаПоТоварам = ДокументОбъект.ПодготовитьТаблицуТоваров(РезультатЗапросаПоТоварам, СтруктураШапкиДокумента, Истина);
	ТаблицаПоПрочее  = ДокументОбъект.ПодготовитьТаблицуПрочие(РезультатЗапросаПоПрочее, СтруктураШапкиДокумента);
	
	Для каждого Строка Из ТаблицаПоПрочее Цикл
		ЗаполнитьЗначенияСвойств(ТаблицаПоТоварам.Добавить(),Строка);
	КонецЦикла;	
...
Функция ПолучитьТаблицуГТДИмпорт(ДокументОбъект, Ошибка)
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоТоварамРубли, ТаблицаПоТоварамВалюта);
...
Функция ПолучитьТаблицуРеализацияТоваровУслуг(ДокументОбъект, Ошибка)
	...
	//АБС+ Тупиков 
	//закомментировано из-за того, что в реквизите СуммаНДС таб. части Товары неотрицательное, а СуммаНДС таб.части Услуги может быть отрицательное
	//соответственно неверно загружалось в таблицу значений в случае отрицательной суммы
	
	//ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоУслугам, ТаблицаПоТоварам);
	
	//Возврат ТаблицаПоТоварам;

	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоТоварам, ТаблицаПоУслугам);
...
Процедура ЗаполнитьСписокКодовВидовОпераций(ЧастьЖурнала, СписокВыбора, ИспользоватьКоды2015 = Ложь, ИспользоватьДополнительныеКоды = Ложь) Экспорт
	...
	Если ЧастьЖурнала = Перечисления.ЧастиЖурналаУчетаСчетовФактур.ПолученныеСчетаФактуры Тогда
		...
		СписокКодовВидовОперации.Добавить("04", "04 - товары, работы, услуги комитента на основе договора комиссии (агентского договора), в случае, если агент совершает действия от своего имени");
		СписокКодовВидовОперации.Добавить("05", "05 - авансы за товары, работы, услуги комитента на основе договора комиссии (агентского договора), в случае, если агент совершает действия от своего имени");
		//АБС ВСТАВКА
		СписокКодовВидовОперации.Добавить("06", "06 - налоговый агент, статья 161 НК");
		СписокКодовВидовОперации.Добавить("08", "08 - строительно-монтажные работы, пп.3 пункт 1 статья 146 НК");
		//АБС ВСТАВКА КОНЕЦ
		...
		СписокКодовВидовОперации.Добавить("13", "13 - капитальное строительство, модернизация (реконструкция) объектов недвижимости подрядными организациями (застройщиками, тех. заказчиками)");
		
		Если ИспользоватьКоды2015 Тогда
			Если ИспользоватьДополнительныеКоды Тогда
				СписокКодовВидовОперации.Добавить("16", "16 - Возврат от покупателя-неплательщика НДС");
				СписокКодовВидовОперации.Добавить("17", "17 - Возврат от покупателя-физического лица");
				СписокКодовВидовОперации.Добавить("18", "18 - Изменение стоимости отгруженных товаров (работ, услуг) в сторону уменьшения");
				СписокКодовВидовОперации.Добавить("19", "19 - Ввоз товаров из Евразийского экономического союза");
				СписокКодовВидовОперации.Добавить("20", "20 - Ввоз импортных товаров на территорию РФ");
				СписокКодовВидовОперации.Добавить("22", "22 - Возврат, зачет авансовых платежей, п.5 статьи 171, п.6 статьи 172 НК");
				СписокКодовВидовОперации.Добавить("23", "23 - Командировочные и представительские расходы по бланку строгой отчетности, п.7 статьи 171 НК");
				СписокКодовВидовОперации.Добавить("24", "24 - Подтверждение ставки 0% после истечения 180 дней");
				СписокКодовВидовОперации.Добавить("25", "25 - Подтверждение ставки 0%");
			КонецЕсли;
			СписокКодовВидовОперации.Добавить("27", "27 - Сводный комиссионный счет-фактура, п.3.1 статьи 169 НК");
			СписокКодовВидовОперации.Добавить("28", "28 - Сводный комиссионный счет-фактура на аванс, п.3.1 статьи 169 НК");
		КонецЕсли;
	...
	ИначеЕсли ЧастьЖурнала = Перечисления.ЧастиЖурналаУчетаСчетовФактур.ВыставленныеСчетаФактуры Тогда
		...
		Если ИспользоватьКоды2015 Тогда
			Если ИспользоватьДополнительныеКоды Тогда
				СписокКодовВидовОперации.Добавить("18", "18 - Изменение стоимости полученных товаров (работ, услуг) в сторону уменьшения");
				СписокКодовВидовОперации.Добавить("21", "21 - Восстановление НДС, п.8 статьи 145, п.3 статьи 170, статья 171.1 НК, а также при операциях, облагаемых по ставке 0%");
				СписокКодовВидовОперации.Добавить("26", "26 - Реализация товаров, работ, услуг лицам, не явл. плательщиками НДС, освобожденными от уплаты НДС");
			КонецЕсли;
			СписокКодовВидовОперации.Добавить("27", "27 - Сводный комиссионный счет-фактура, п.3.1 статьи 169 НК");
			СписокКодовВидовОперации.Добавить("28", "28 - Сводный комиссионный счет-фактура на аванс, п.3.1 статьи 169 НК");
		КонецЕсли;
...
Процедура УстановитьПараметрыЗапросаКнигиПокупок(Запрос, СтруктураПараметров)
	...
	абс_УчетНДС.УстановитьПараметрыЗапросаКнигиПокупок(Запрос, СтруктураПараметров);
...
Функция ПолучитьЗаписиКнигиПокупок(СписокСчетовФактур, СтруктураПараметров) Экспорт
	...
	Возврат абс_УчетНДС.ПолучитьЗаписиКнигиПокупок(СписокСчетовФактур, СтруктураПараметров);
...
// +++ввв 08.06.2016 г.
	...
	|ГДЕ
	|	(СчетаФактуры.СчетФактураДокумент = ВТ_ДО.СчетФактура ИЛИ СчетаФактуры.СчетФактура ЕСТЬ NULL) И
...
// ---ввв 08.06.2016 г.
	...
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
// +++ввв 21.07.2016 г.
// {{KM WARE Семенов И.С. Заявка № 15.01.2016 начало
// оптимизация
// 
// удалить
...
//	|	ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.СчетФактураДокумент.Дата, ЗаписиКнигиПокупок.СчетФактура.Дата) КАК kmw_СчетФактураДокумент_Дата,
	//|	подстрока(ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.СчетФактураДокумент.Номер, ЗаписиКнигиПокупок.СчетФактура.Номер),1,30) КАК kmw_СчетФактураДокумент_Номер,
	//|	подстрока(ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.СчетФактураДокумент.КодВидаОперации, ЗаписиКнигиПокупок.СчетФактура.КодВидаОперации),1,2) КАК kmw_СчетФактураДокумент_КодВидаОперации,
	//|	выбор 
...
// }}KM WARE Семенов И.С. Заявка № 15.01.2016 окончание	
// ---ввв 21.07.2016 г.
...
// +++ввв 23.06.2016 г.
	|			ЕСТЬNULL(ВЫРАЗИТЬ(ЗаписиКнигиПокупок.СчетФактура КАК Документ.СчетФактураВыданный).ДокументОснование, ЗаписиКнигиПокупок.СчетФактура)
// ---ввв 23.06.2016 г.
	|		КОГДА ЗаписиКнигиПокупок.СчетФактура ССЫЛКА Документ.СчетФактураПолученный ТОГДА
// +++ввв 23.06.2016 г.
	|			ЕСТЬNULL(ВЫРАЗИТЬ(ЗаписиКнигиПокупок.СчетФактура КАК Документ.СчетФактураПолученный).ДокументОснование, ЗаписиКнигиПокупок.СчетФактура)
// ---ввв 23.06.2016 г.
...
// +++ввв 23.06.2016 г.
	КлассификаторСлучаев=абс_УчетНДС.ПолучитьКлассификаторСлучаев(Запрос,""+Перечисления.ttk_ТипыРазделовНДС.Покупки);
// ---ввв 23.06.2016 г.
...
// ---ввв 21.07.2016 г.
	...
	|	ЗаписиКнигиПокупок.ПорядокОтраженияПоДатам,
	|	ЗаписиКнигиПокупок.Организация КАК Организация,
	|	ЗаписиКнигиПокупок.ДатаОплаты КАК ДатаОплаты,
	|	ЗаписиКнигиПокупок.СчетФактураДата КАК СчетФактураДата,
	|	ЗаписиКнигиПокупок.ДатаОприходования КАК ДатаОприходования,
	...
	|	ЗаписиКнигиПокупок.ИсправленныйСчетФактура КАК ИсправленныйСчетФактура,
	|	ЗаписиКнигиПокупок.КодВидаОперации КАК КодВидаОперации,
// +++ввв над этим работает классификатор начало
	|	ЗаписиКнигиПокупок.КодВидаОперации КАК КодВидаОперацииКласс,
...
// +++ввв 14.06.2016 г.
	...
	|	ВЫБОР
	|		КОГДА ЗаписиКнигиПокупок.КодВидаОперации=""04"" И
...
// +++ввв 23.06.2016 г.
	//|		И ВТ_КС.ИспользоватьДокументРасчетовКакСчетФактуру = ЗаписиКнигиПокупок.ФлагИспользоватьДокументРасчетовКакСчетФактуру
// ---ввв 23.06.2016 г.
	//|ГДЕ
// Натянем на итоговую наблицу ещё один запрос с фильтром
//++Готовцев КТТК 20.07.2018

//	|	(ВЫБОР
//	|		КОГДА (ЗаписиКнигиПокупок.СуммаБезНДС20+ЗаписиКнигиПокупок.СуммаБезНДС18+
//	|			  ЗаписиКнигиПокупок.СуммаБезНДС10+ЗаписиКнигиПокупок.НДС0+ЗаписиКнигиПокупок.СуммаСовсемБезНДС+
//	|			  ЗаписиКнигиПокупок.НДС20+ЗаписиКнигиПокупок.НДС18+ЗаписиКнигиПокупок.НДС10=0) ТОГДА ЛОЖЬ
//	|		ИНАЧЕ ИСТИНА
//	|		КОНЕЦ)
//	| И 
//--Готовцев КТТК 20.07.2018
	//
	//| (ВЫБОР
	//|		КОГДА (ЗаписиКнигиПокупок.СуммаБезНДС20<>0 И ЗаписиКнигиПокупок.НДС20=0) ТОГДА ЛОЖЬ
	//|		ИНАЧЕ ИСТИНА
	//|		КОНЕЦ)
	//|	И (ВЫБОР
	//|		КОГДА (ЗаписиКнигиПокупок.СуммаБезНДС18<>0 И ЗаписиКнигиПокупок.НДС18=0) ТОГДА ЛОЖЬ
	//|		ИНАЧЕ ИСТИНА
	//|		КОНЕЦ)
	//|	И (ВЫБОР
	//|		КОГДА (ЗаписиКнигиПокупок.СуммаБезНДС10<>0 И ЗаписиКнигиПокупок.НДС10=0) ТОГДА ЛОЖЬ
	//|		ИНАЧЕ ИСТИНА
	//|		КОНЕЦ)
//++Готовцев КТТК 13.07.2018
	//|   И (ЗаписиКнигиПокупок.СуммаБезНДС20+ЗаписиКнигиПокупок.СуммаБезНДС18+
	//|			  ЗаписиКнигиПокупок.СуммаБезНДС10+ЗаписиКнигиПокупок.НДС0+ЗаписиКнигиПокупок.СуммаСовсемБезНДС+
	//|			  ЗаписиКнигиПокупок.НДС20+ЗаписиКнигиПокупок.НДС18+ЗаписиКнигиПокупок.НДС10)<> 0
	//| и ПОДСТРОКА(&ФлагОшибка, 1, 1) = """"
//--Готовцев КТТК 13.07.2018
	...
	|	ИсправленныйСчетФактура";
		...
		КонецЕсли;	
		// +++ввв 23.06.2016 г.
		абс_УчетНДС.ПрименитьКлассификаторСлучаев(КлассификаторСлучаев, Запрос,""+Перечисления.ttk_ТипыРазделовНДС.Покупки);
		// ---ввв 23.06.2016 г.
		Если йЭтап=1 Тогда
			...
			|ГДЕ","
			|ГДЕ
			|	ВТ_КС.Ошибка <= ЗаписиКнигиПокупок.ДатаСчетаФактуры И");
...
Процедура ПолучитьСчетаФактурыДокументыКнигаПокупок(Запрос)
	...
	абс_УчетНДС.ПолучитьСчетаФактурыДокументыКнигаПокупок(Запрос);
...
Функция ПроверитьНаличиеДопЛистовКнигиПродаж(СтруктураПараметров) Экспорт
	...
	ПрименяетсяОсвобождениеОтУплатыНДС = ПрименяетсяОсвобождениеОтУплатыНДС(
...
Функция ПолучитьЗаписиДополнительныхЛистовКнигиПродаж(СписокСчетовФактур, СтруктураПараметров) Экспорт
	...
	|	ВЫБОР
	|		КОГДА НДСЗаписиКнигиПродаж.КодВидаОперации="""" ИЛИ НДСЗаписиКнигиПродаж.КодВидаОперации ЕСТЬ NULL ТОГДА
	|			ВЫБОР
	|				КОГДА НДСЗаписиКнигиПродаж.ВидЦенности В (&ВидыЦенностей_Аванс) ТОГДА ""02""
	|				КОГДА (НДСЗаписиКнигиПродаж.Регистратор ССЫЛКА Документ.КорректировкаРеализации) ИЛИ
	|					  (НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.ДокументРасчетовСКонтрагентом) ИЛИ
	|					  (НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.РеализацияТоваровУслуг) ИЛИ
	|					  (НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.ОтражениеРеализацииТоваровИУслугНДС) ИЛИ
	|					  (НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.СчетФактураВыданный)
	|					ТОГДА
	|					ВЫБОР
	|						КОГДА (НДСЗаписиКнигиПродаж.Покупатель В (&АгрегатныеКонтры)) ТОГДА ""26""
	|						ИНАЧЕ ""01""
	|					КОНЕЦ
// +++ввв 19.10.2016 г.
	|				КОГДА НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.КорректировкаПоступления
	|					ТОГДА ""18""
// ---ввв 19.10.2016 г.
	|				ИНАЧЕ НДСЗаписиКнигиПродаж.КодВидаОперации
	|			КОНЕЦ
	|		ИНАЧЕ НДСЗаписиКнигиПродаж.КодВидаОперации
	|	КОНЕЦ КАК КодВидаОперации,
// +++ввв 14.06.2016 г.
	|	ВЫБОР
	|		КОГДА (НДСЗаписиКнигиПродаж.КодВидаОперации="""" ИЛИ НДСЗаписиКнигиПродаж.КодВидаОперации ЕСТЬ NULL) И НДСЗаписиКнигиПродаж.ВидЦенности В (&ВидыЦенностей_Аванс) ТОГДА ""02""
// +++ввв 19.10.2016 г.
	|		КОГДА (НДСЗаписиКнигиПродаж.КодВидаОперации="""" ИЛИ НДСЗаписиКнигиПродаж.КодВидаОперации ЕСТЬ NULL)
	|			  И НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.КорректировкаПоступления
	|					ТОГДА ""18""
// ---ввв 19.10.2016 г.
	|		ИНАЧЕ НДСЗаписиКнигиПродаж.КодВидаОперации
	|	КОНЕЦ КАК КодВидаОперацииРеальный
// ---ввв 14.06.2016 г.
...
// ---ввв 08.06.2016 г.
	...
	|				КОНЕЦ
//	|		КОГДА ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.СчетФактураДокумент, ЕСТЬNULL(СчетаФактурыДоп.СчетФактура, ЗаписиКнигиПродаж.СчетФактура)) ССЫЛКА Документ.ДокументРасчетовСКонтрагентом
//	|			ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.СчетФактураДокумент, ЕСТЬNULL(СчетаФактурыДоп.СчетФактура, ЗаписиКнигиПродаж.СчетФактура)) КАК Документ.ДокументРасчетовСКонтрагентом).ttk_Номер
...
// ---ввв 15.06.2016 г.
	|				КОНЕЦ
//	|		КОГДА ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.СчетФактураДокумент, ЕСТЬNULL(СчетаФактурыДоп.СчетФактура, ЗаписиКнигиПродаж.СчетФактура)) ССЫЛКА Документ.ДокументРасчетовСКонтрагентом
//	|			ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.СчетФактураДокумент, ЕСТЬNULL(СчетаФактурыДоп.СчетФактура, ЗаписиКнигиПродаж.СчетФактура)) КАК Документ.ДокументРасчетовСКонтрагентом).ttk_Дата
...
// ---ввв 08.06.2016 г.
	...
	// {{KM WARE Малыгин П.К. Заявка № 20.01.2016 запрос от Сидорова начало
	// 
	// {{Старый код:
	//|	ВЫБОР
	...
	// }}KM WARE Малыгин П.К. Заявка № 20.01.2016 запрос от Сидорова окончание 
	|	ВЫБОР
...
// +++ввв 17.06.2016 г.
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ЗаписиКнигиПродаж.КодВидаОперацииРеальный, """") = """"
	|			ТОГДА ВЫБОР
	...
	|		ИНАЧЕ ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.КодВидаОперации, ЕСТЬNULL(СчетаФактурыДоп.СчетФактура.КодВидаОперации, ""01""))
	|	КОНЕЦ КАК КодВидаОперации,
// ---ввв 17.06.2016 г.
// +++ввв 19.10.2016 г.
	|	ВЫБОР
...
// +++ввв 24.06.2016 г.
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.СчетФактураДокумент, ЕСТЬNULL(СчетаФактурыДоп.СчетФактура,ЗаписиКнигиПродаж.СчетФактура)) ССЫЛКА Документ.СчетФактураВыданный
	|			ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.СчетФактураДокумент, ЕСТЬNULL(СчетаФактурыДоп.СчетФактура,ЗаписиКнигиПродаж.СчетФактура)) КАК Документ.СчетФактураВыданный).Исправление
	|		КОГДА ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.СчетФактураДокумент, ЕСТЬNULL(СчетаФактурыДоп.СчетФактура,ЗаписиКнигиПродаж.СчетФактура)) ССЫЛКА Документ.СчетФактураПолученный
...
// +++ввв 24.06.2016 г.
	|";
	Если НЕ СтруктураПараметров.СформироватьОтчетПоСтандартнойФорме И СтруктураПараметров.ОтбиратьПоКонтрагенту 
		И СтруктураПараметров.КонтрагентДляОтбора.ЭтоГруппа Тогда
	...
	КлассификаторСлучаев=абс_УчетНДС.ПолучитьКлассификаторСлучаев(Запрос,""+Перечисления.ttk_ТипыРазделовНДС.Продажи);
	Для йЭтап=СтруктураПараметров.ЗаполнятьРаздел89СУровнемПроверки По 3 Цикл
		Запрос.Текст="
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
// +++ввв 21.07.2016 г.
	// в итоге требуется удалить начало
	//|	ЗаписиКнигиПродаж.kmw_СчетФактура_абс_ДатаСФ,
	//|	ЗаписиКнигиПродаж.kmw_СчетФактура_Дата,
//	|	ЗаписиКнигиПродаж.kmw_СчетФактураДокумент_абс_ДатаСФ,
//	|	ЗаписиКнигиПродаж.kmw_СчетФактураДокумент_Дата,
//	|	ЗаписиКнигиПродаж.kmwСчетФактураДата,
//	|	ЗаписиКнигиПродаж.kmwСчетФактураНомер,
//	|	ЗаписиКнигиПродаж.kmwСчетФактураДокументДатаВходящегоДокумента,
//	|	ЗаписиКнигиПродаж.kmwСчетФактураДокументНомерВходящегоДокумента,
//	|	ЗаписиКнигиПродаж.kmwСчетФактураДокументДатаПлатежноРасчетногоДокумента,
//	|	ЗаписиКнигиПродаж.kmwСчетФактураДокументНомерПлатежноРасчетногоДокумента,
	// в итоге требуется удалить конец
// ---ввв 21.07.2016 г.
	|	ЗаписиКнигиПродаж.Организация,
	|	ЗаписиКнигиПродаж.НалоговыйПериод,
	|	ЗаписиКнигиПродаж.КонецНалоговогоПериода,
	|	ЗаписиКнигиПродаж.НаАванс,
	|	ЗаписиКнигиПродаж.НаСуммовуюРазницу,
	|	ЗаписиКнигиПродаж.СчетФактураДата,
...
// ---ввв над этим работает классификатор конец
	...
	|	ЗаписиКнигиПродаж.ДатаОформления,
	|	ЗаписиКнигиПродаж.СторнирующаяЗаписьДопЛиста,
	...
	|ГДЕ
	|	(ВЫБОР
	|		КОГДА (ЗаписиКнигиПродаж.СуммаБезНДС20+ЗаписиКнигиПродаж.СуммаБезНДС18+ЗаписиКнигиПродаж.СуммаБезНДС10+
	|			  ЗаписиКнигиПродаж.НДС0+ЗаписиКнигиПродаж.СуммаСовсемБезНДС+ЗаписиКнигиПродаж.НДС20+
	|			  ЗаписиКнигиПродаж.НДС18+ЗаписиКнигиПродаж.НДС10=0) ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|		КОНЕЦ)
	|	И (ВЫБОР
	|		КОГДА (ЗаписиКнигиПродаж.СуммаБезНДС20<>0 И ЗаписиКнигиПродаж.НДС20=0) ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|		КОНЕЦ)
	|	И (ВЫБОР
	|		КОГДА (ЗаписиКнигиПродаж.СуммаБезНДС18<>0 И ЗаписиКнигиПродаж.НДС18=0) ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|		КОНЕЦ)
	|	И (ВЫБОР
	|		КОГДА (ЗаписиКнигиПродаж.СуммаБезНДС10<>0 И ЗаписиКнигиПродаж.НДС10=0) ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|		КОНЕЦ)
	|УПОРЯДОЧИТЬ ПО
	|	НалоговыйПериод,
	...
	|	МАКСИМУМ(Покупатель),
	|	МАКСИМУМ(ДатаОплаты),
	|	СУММА(ВсегоПродаж) КАК ВсегоПродаж,	
	|	СУММА(СуммаБезНДС20),
	|	СУММА(НДС20),
	|	СУММА(СуммаБезНДС18),
	|	СУММА(НДС18),
	...
	|";
		...
		Если СтруктураПараметров.ГруппироватьПоКонтрагентам И 
			 Не СтруктураПараметров.СформироватьОтчетПоСтандартнойФорме Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПО
	              	|	НалоговыйПериод,
	              	|	ДатаОформления,", "ПО
	              	|	НалоговыйПериод,
		...
		абс_УчетНДС.ПрименитьКлассификаторСлучаев(КлассификаторСлучаев, Запрос,""+Перечисления.ttk_ТипыРазделовНДС.Продажи);
		Если йЭтап=1 Тогда
		// поиск ошибок Классификации
			Запрос.УстановитьПараметр("ФлагОшибка","Ошибка учёта");
	    	Запрос.Текст=СтрЗаменить(Запрос.Текст,"
	|ГДЕ","
	|ГДЕ
	|	ВТ_КС.Ошибка <= ЗаписиКнигиПродаж.ДатаСчетаФактуры И");
		ИначеЕсли йЭтап=2 Тогда
	    // поиск несогласованных случаев
			Запрос.УстановитьПараметр("ФлагОшибка","Не согласовано");
	    	Запрос.Текст=СтрЗаменить(Запрос.Текст,"
	...
	|	НЕ ВТ_КС.Согласован И");
		Иначе
		// вывод всей информации без проверок
			Запрос.УстановитьПараметр("ФлагОшибка","");
		КонецЕсли;
		РезультатЗапроса=Запрос.Выполнить();
		Если НЕ РезультатЗапроса.Пустой() Тогда
...
Процедура ПолучитьСчетаФактурыДокументыКнигаПродаж(Запрос)
	...
	абс_УчетНДС.ПолучитьСчетаФактурыДокументыКнигаПродаж(Запрос);
...
Функция ПолучитьЗаписиКнигиПродаж(СписокСчетовФактур, СтруктураПараметров) Экспорт
	...
	Возврат абс_УчетНДС.ПолучитьЗаписиКнигиПродаж(СписокСчетовФактур, СтруктураПараметров);
	
КонецФункции

Процедура УстановитьПараметрыЗапросаКнигиПродаж(Запрос, СтруктураПараметров)
	
	абс_УчетНДС.УстановитьПараметрыЗапросаКнигиПродаж(Запрос, СтруктураПараметров);
...
// {{KM WARE Семенов И.С. Заявка № 32103 20.07.2015 начало		
	если типзнч(СтруктураПараметров.Организация)=тип("СправочникСсылка.Организации") тогда
		м=новый массив;
		м.Добавить(СтруктураПараметров.Организация);
	иначе
		м=СтруктураПараметров.Организация;
	конецесли;
// }}KM WARE Семенов И.С. Заявка № 32103 20.07.2015 окончание
	...
	Запрос.УстановитьПараметр("ПравилаПостановления735", 
			ВерсияПостановленияНДС1137(СтруктураПараметров.НалоговыйПериод) = 3);
...
// дата и номер для 04 кода лежит в НомерСчетаФактурыПродавца
|		ИНАЧЕ """"
	//ЖурналУчетаСчетовФактурСрезПоследних.ДатаСчетаФактуры
...
Функция СобратьДанныеПоОтчетКомиссионераОПродажах(ДокОснование, Ссылка)
	...
	Если ТаблицаШапка.Количество() = 1 Тогда
		...
		ДанныеДляПечати.Вставить("Грузополучатель", Шапка.Грузополучатель);
	Иначе
		ДанныеДляПечати.Вставить("Покупатель",      ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(
			ТаблицаШапка.ВыгрузитьКолонку("Покупатель")));
		ДанныеДляПечати.Вставить("Грузополучатель", ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(
...
Функция СобратьДанныеПоРеализацияТоваровУслуг(ДокОснование, Ссылка)
	...
	|	Ложь			КАК ЗаРуководителяПоПриказу
	|ИЗ
...
Функция СобратьДанныеСФнаАвансИСуммовуюРазницу(Ссылка)
	...
	Если Не (Ссылка.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаАванс
		или Ссылка.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаАвансКомитента
	...
	НадписьНаименование = ?(Ссылка.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаАванс
							ИЛИ Ссылка.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаАвансКомитента, мНадписьПредоплата, "Суммы, связанные с расчетами по оплате (ст. 162 НК РФ)");
	
	Запрос = Новый Запрос;
 	Запрос.УстановитьПараметр("Ссылка", Ссылка);
 	Запрос.УстановитьПараметр("ВалютаРегламентированногоУчета", Константы.ВалютаРегламентированногоУчета.Получить());
	НаАванс = Ссылка.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаАванс;
	...
	Если НаАванс И ТаблицаШапка.Количество() > 1 Тогда
		...
		Покупатель = ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(ТаблицаШапка.ВыгрузитьКолонку("Покупатель"));
	Иначе
		Поставщик = Шапка.Поставщик;
	...
	Если Ссылка.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаАванс
		ИЛИ Ссылка.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаАвансКомитента Тогда
...
Процедура ВывестиСчетФактуруВТабличныйДокумент(ТабДокумент, Макет, ДанныеДляПечати) Экспорт
	...
	НужныСведенияОПодразделенииПокупателя = ДанныеДляПечати.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаРеализацию
		ИЛИ ДанныеДляПечати.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаАванс;
	...
	Иначе
		...
		Иначе
			...
			Если ДанныеДляПечати.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НалоговыйАгент
	...
	Если ДанныеДляПечати.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаАванс
		ИЛИ ДанныеДляПечати.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаАвансКомитента Тогда
	...
	Для Каждого Строчка Из ВыборкаСтрокТовары Цикл
		...
		Если ДанныеДляПечати.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаРеализацию И 
	...
	Для Каждого Строчка Из ВыборкаСтрокТовары Цикл
		...
		Если ДанныеДляПечати.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаРеализацию Тогда
			ОбластьМакета.Параметры.Стоимость = Строчка.СуммаБезНДС;
			ОбластьМакета.Параметры.Цена   	  = Строчка.Цена;
		Иначе
			Если ДанныеДляПечати.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НалоговыйАгент 
				ИЛИ ДанныеДляПечати.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаСуммовуюРазницу Тогда
				ОбластьМакета.Параметры.Стоимость = Строчка.СуммаБезНДС;
			Иначе
		...
		Если ДанныеДляПечати.ВидСчетаФактуры <> Перечисления.НДСВидСчетаФактуры.НаАванс 
			И ДанныеДляПечати.ВидСчетаФактуры <> Перечисления.НДСВидСчетаФактуры.НаАвансКомитента Тогда
	...
	Если ДанныеДляПечати.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НалоговыйАгент Тогда
...
Функция ПодготовитьДанныеДляПечатиУниверсальногоПередаточногоДокумента(ДанныеДокумента)
	...
	ДанныеСчетаФактуры.Вставить("ВидСчетаФактуры", 	  Перечисления.НДСВидСчетаФактуры.НаРеализацию);
...
Функция СобратьДанныеДляПечатиКорректировочногоСчетаФактуры(ТекущееОснование, Ссылка) Экспорт
	...
	Иначе
		...
		ДанныеДокумента.Вставить("Номер",                            ОбщегоНазначения.ПолучитьНомерНаПечать(Ссылка));
		ДанныеДокумента.Вставить("ВидСчетаФактуры",                  Перечисления.НДСВидСчетаФактуры.Корректировочный);
...
Процедура ВывестиКорректировочныйСчетФактуруВТабличныйДокумент(ТабДокумент, Макет, ДанныеДляПечати) Экспорт
	...
	Если НЕ (ДанныеДляПечати.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НалоговыйАгент
		...
		Для Каждого ИтогПоКонтрагенту Из ИтогПоОрганизации.Строки Цикл
			...
			Для Каждого ИтогПоДокументу Из ИтогПоКонтрагенту.Строки Цикл
				Для Каждого ИтогПоКоду Из ИтогПоДокументу.Строки Цикл
					Для Каждого ИтогПоИсправлению Из ИтогПоКоду.Строки Цикл
...
// +++ввв 12.07.2016 г.
									Если ДетальнаяЗапись.ВсегоПокупок=0 И ДетальнаяЗапись.НДС=0 Тогда
// +++ввв 12.05.2017 г.
										Если НЕ ЕстьОшибкаВСтроке(ДетальнаяЗапись) Тогда
											Продолжить;
										КонецЕсли;
// ---ввв 12.05.2017 г.
									КонецЕсли;
// ---ввв 12.07.2016 г.
							...
							Иначе	
// +++ввв 12.07.2016 г.
								Если ЗаписьКниги.ВсегоПокупок=0 И ЗаписьКниги.НДС=0 Тогда
// +++ввв 12.05.2017 г.
										Если НЕ ЕстьОшибкаВСтроке(ЗаписьКниги) Тогда
											Продолжить;
										КонецЕсли;
// ---ввв 12.05.2017 г.
								КонецЕсли;
...
// ---ввв 12.07.2016 г.
							...
							Если НЕ СтруктураПараметров.ЗаполнениеДокумента Тогда 
								СтруктураСекций.СекцияСтрока.Область(1, , СтруктураСекций.СекцияСтрока.ВысотаТаблицы).Имя = "Строка_" + Счетчик;
								ТабличныйДокумент.Вывести(СтруктураСекций.СекцияСтрока);
							КонецЕсли;
...
// +++ввв 12.05.2017 г.
Функция ЕстьОшибкаВСтроке(ЗаписьКниги)
	Если ЗаписьКниги.Строки.Количество() > 0 Тогда 
		ПараметрыЗаполнения = ЗаписьКниги.Строки[0];
	Иначе
		ПараметрыЗаполнения = ЗаписьКниги;
	КонецЕсли;
	Если ПараметрыЗаполнения.Владелец().Колонки.Найти("ОшибкаУчета")=Неопределено Тогда
		Возврат Ложь;
	Иначе
		Возврат НЕ ПустаяСтрока(ПараметрыЗаполнения.ОшибкаУчета);
	КонецЕсли;
КонецФункции
// ---ввв 12.05.2017 г.
...
// +++ввв 18.10.2016 г.
// +++ввв 12.04.2017 г.
	Если ТипЗнч(ПараметрыЗаполнения.СчетФактураДокумент)=Тип("ДокументСсылка.ГТДИмпорт") Тогда
		йС = 0;
		йКурс =  ПараметрыЗаполнения.СчетФактураДокумент.КурсДокумента / ?(ПараметрыЗаполнения.СчетФактураДокумент.КратностьДокумента=0,1,ПараметрыЗаполнения.СчетФактураДокумент.КратностьДокумента);
		Для каждого йСтр Из ПараметрыЗаполнения.СчетФактураДокумент.Разделы Цикл
			йС = йС + Окр(йСтр.ТаможеннаяСтоимость*?(йСтр.ТаможеннаяСтоимостьВВалютеРеглУчета,1,йКурс),2,РежимОкругления.Окр15как20);
		КонецЦикла;
		ЗаписьКниги.ВсегоПокупок=йС;
		ПараметрыЗаполнения.ВсегоПокупок=йС;
		Если ЗначениеЗаполнено(ПараметрыЗаполнения.СчетФактураДокумент.НомерГТД) Тогда
			ЗаписьКниги.НомерСчетаФактуры = ПараметрыЗаполнения.СчетФактураДокумент.НомерГТД.Код;
			ПараметрыЗаполнения.НомерСчетаФактуры = ПараметрыЗаполнения.СчетФактураДокумент.НомерГТД.Код;
		Иначе
			ЗаписьКниги.НомерСчетаФактуры = ПараметрыЗаполнения.СчетФактураДокумент.Номер;
			ПараметрыЗаполнения.НомерСчетаФактуры = ПараметрыЗаполнения.СчетФактураДокумент.Номер;
		КонецЕсли;
// +++ввв 18.04.2017 г.
		//ЗаписьКниги.ДатаСчетаФактуры = ПараметрыЗаполнения.СчетФактураДокумент.Дата;
		//ПараметрыЗаполнения.ДатаСчетаФактуры = ПараметрыЗаполнения.СчетФактураДокумент.Дата;
// ---ввв 18.04.2017 г.
	ИначеЕсли ПараметрыЗаполнения.Владелец().Колонки.Найти("ВсегоПокупокВСчетеФактуре")<>Неопределено И
// ---ввв 12.04.2017 г.
		 (ТипЗнч(ПараметрыЗаполнения.СчетФактураДокумент)=Тип("ДокументСсылка.СчетФактураВыданный") ИЛИ
		 ТипЗнч(ПараметрыЗаполнения.СчетФактураДокумент)=Тип("ДокументСсылка.СчетФактураПолученный")) Тогда
// ---ввв 18.10.2016 г.
		Если (ЗначениеЗаполнено(ПараметрыЗаполнения.Валюта) И ПараметрыЗаполнения.ВалютаКод<>"643") ИЛИ ЗаписьКниги.ВсегоПокупокВСчетеФактуре=0 Тогда
		Иначе
			ЗаписьКниги.ВсегоПокупок=ЗаписьКниги.ВсегоПокупокВСчетеФактуре;
			ПараметрыЗаполнения.ВсегоПокупок=ЗаписьКниги.ВсегоПокупокВСчетеФактуре;
		КонецЕсли;
	КонецЕсли;
// ---ввв 10.08.2016 г.
...
// +++ввв 18.04.2017 г.	
	Если ТипЗнч(ПараметрыЗаполнения.СчетФактураДокумент)=Тип("ДокументСсылка.ГТДИмпорт") Тогда
		Если ЗначениеЗаполнено(ПараметрыЗаполнения.СчетФактураДокумент.ttk_ДатаГТД) Тогда
			ЗаписьКниги.ДатаСчетаФактуры = ПараметрыЗаполнения.СчетФактураДокумент.ttk_ДатаГТД;
			ПараметрыЗаполнения.ДатаСчетаФактуры = ПараметрыЗаполнения.СчетФактураДокумент.ttk_ДатаГТД;
			ВариантыПредставленияСФ = ОпределитьДатуИНомерСФ(ПараметрыЗаполнения, СтруктураПараметров);
		Иначе
			ВариантыПредставленияСФ = Новый Структура("ДатаНомер,НомерДата,Дата,Номер", "", "", "", "");
			ВариантыПредставленияСФ.ДатаНомер = СокрЛП(ЗаписьКниги.НомерСчетаФактуры);
			ВариантыПредставленияСФ.НомерДата = СокрЛП(ЗаписьКниги.НомерСчетаФактуры);
			ВариантыПредставленияСФ.Номер = СокрЛП(ЗаписьКниги.НомерСчетаФактуры);
		КонецЕсли;
	Иначе
// ---ввв 18.04.2017 г.
		ВариантыПредставленияСФ = ОпределитьДатуИНомерСФ(ПараметрыЗаполнения, СтруктураПараметров);
...
// +++ввв 01.06.2016 г.
	...
	Иначе
		йС=Символы.ВК+Символы.ПС+ПараметрыЗаполнения.ОшибкаУчета;
		Если ПустаяСтрока(ПараметрыЗаполнения.ОшибкаУчета) Тогда
			йС="";
		КонецЕсли;
		СоставСтроки.Вставить("НомерДата", ВариантыПредставленияСФ.НомерДата+йС);
	КонецЕсли;
// ---ввв 01.06.2016 г.
...
// +++ввв 12.04.2017 г.
	Если ТипЗнч(ПараметрыЗаполнения.СчетФактураДокумент)=Тип("ДокументСсылка.ГТДИмпорт") Тогда
// для документа ГТД импорт	нет надобности вводить номера декларации так как она указывается в начале
	Иначе
// ---ввв 12.04.2017 г.
	// ГТД и страна происхождения
// +++ввв 27.03.2017 г.
	СчетаФактуры = ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(СчетаФактуры);
// ---ввв 27.03.2017 г.
...
// {{KM WARE Семенов И.С. Заявка № 17.07.2015 начало
// не проставлялиись для кодов 22
	если ТекстОплатыСНомером="" тогда
		если ЗаписьКниги.Строки.Количество()>0 тогда
						
			// {{KM WARE Малыгин П.К. Заявка № 20.01.2016 начало
			// Переносим вызов в привелегированный модуль
			// {{Старый код:
			//абс_ОпределитьНомерДатуДокументаОплаты(ЗаписьКниги.Строки[0],ТекстОплатыСНомером,ТекстОплаты);
			// }}Новый код:
			СтруктураЗК = Новый Структура;
			СписокКолонок  = ЗаписьКниги.Владелец().Колонки; 
			Для Каждого Колонка из СписокКолонок Цикл
			СтруктураЗК.Вставить(Колонка.Имя, ПараметрыЗаполнения[Колонка.Имя]);	
			КонецЦикла;	
			
			абс_БизнесПроцессыПривелегированный.абс_ОпределитьНомерДатуДокументаОплаты(СтруктураЗК,ТекстОплатыСНомером,ТекстОплаты);
			
			ЗаполнитьЗначенияСвойств(ПараметрыЗаполнения,СтруктураЗК);
			// }}KM WARE Малыгин П.К. Заявка № 20.01.2016 окончание 
			
			
			если НЕ (ТекстОплатыСНомером="") тогда
				СтруктураДокПдтвУпл = Новый Структура("НомДокПдтвУпл,ДатаДокПдтвУпл");
// +++ввв 22.03.2016 г.
				йинд=Найти(ТекстОплатыСНомером,";");
				йтосн=ТекстОплатыСНомером;
				Если йинд>0 Тогда
					йтосн=Лев(ТекстОплатыСНомером,йинд-1);
				КонецЕсли;
//				СтруктураДокПдтвУпл.НомДокПдтвУпл = ТекстОплатыСНомером;
				СтруктураДокПдтвУпл.НомДокПдтвУпл = йтосн;
// ---ввв 22.03.2016 г.
				СтруктураДокПдтвУпл.ДатаДокПдтвУпл = ТекстОплаты;
				МассивДокПдтвУпл.Добавить(СтруктураДокПдтвУпл);
			конецесли;
		конецесли;
	конецесли;
// }}KM WARE Семенов И.С. Заявка № 17.07.2015 окончание

// +++ввв 14.06.2016 г.
// {{KM WARE Малыгин П.К. Заявка 34043 № 16.10.2015 начало
	// Проставление для кода "04"
//		если ЗаписьКниги.Строки.Количество()>0 тогда
//			kmwr_ОпределитьСчетФактуруИПрочее(ЗаписьКниги.Строки[0],ТекстОплатыСНомером,СоставСтроки);
//		конецесли;
...
// +++ввв 27.03.2017 г.
	СчетаФактурыДокументы = ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(СчетаФактурыДокументы);
// ---ввв 27.03.2017 .
...
// ---ввв 27.03.2017 г.
	...
	ИначеЕсли СтруктураПараметров.ЗаполнениеДокумента
		...
		Если МассивСвПрод.Количество() = 0
			И НЕ ПустаяСтрока(ПараметрыЗаполнения.ПродавецИНН) Тогда
			СтруктураРеквизитовКонтрагента = ПолучитьСтруктуруРеквизитовКонтрагента(ПараметрыЗаполнения.ПродавецИНН, ПараметрыЗаполнения.ПродавецКПП);
		...
		Если НЕ ПустаяСтрока(ПосредникИНН) Тогда
			СоставСтроки.Вставить("СвПосредн", 
				ПолучитьСтруктуруРеквизитовКонтрагента(ПосредникИНН, ПосредникКПП, Посредник));
		КонецЕсли;
				...
				Для каждого ИтогПоДоговору Из ИтогПоДокументу.Строки Цикл
					Для каждого ИтогПоКоду Из ИтогПоДоговору.Строки Цикл
						Для каждого ЗаписьКниги Из ИтогПоКоду.Строки Цикл
							// +++ввв 16.03.2016 г.
							// +++ввв 07.06.2016 г.
							//++Готовцев КТТК 16.07.2018
							ЕСли  (ЗаписьКниги.ВсегоПокупок+ЗаписьКниги.НДС+ЗаписьКниги.НДС0+ЗаписьКниги.НДС10+ЗаписьКниги.НДС18+ЗаписьКниги.НДС20
								+ ЗаписьКниги.СуммаБезНДС10+ЗаписьКниги.СуммаБезНДС18+ЗаписьКниги.СуммаБезНДС20+ЗаписьКниги.СуммаСовсемБезНДС) = 0  ТОгда
								Продолжить;
							КонецЕсли;
...
// +++ввв 27.07.2016 г.		
	ПолучитьДополнительнуюИнформациюПоСФДляКнигиПродажДатыОплаты(СтруктураПараметров, СписокСчетовФактур);
// ---ввв 27.07.2016 г.
...
// +++ввв 15.07.2016 г.
									Если ЗаписьКнигиДетальная.СуммаБезНДС18+ЗаписьКнигиДетальная.СуммаБезНДС10+
										 ЗаписьКнигиДетальная.НДС18+ЗаписьКнигиДетальная.НДС10+ЗаписьКнигиДетальная.НДС0+
										 ЗаписьКнигиДетальная.СуммаСовсемБезНДС=0 Тогда
// +++ввв 12.05.2017 г.
										Если НЕ ЕстьОшибкаВСтроке(ЗаписьКнигиДетальная) Тогда
											Продолжить;
										КонецЕсли;
// ---ввв 12.05.2017 г.
									КонецЕсли;
...
// +++ввв 15.07.2016 г.
								Если ЗаписьКниги.СуммаБезНДС18+ЗаписьКниги.СуммаБезНДС10+ЗаписьКниги.НДС18+
									 ЗаписьКниги.НДС10+ЗаписьКниги.НДС0+ЗаписьКниги.СуммаСовсемБезНДС=0 Тогда
// +++ввв 12.05.2017 г.
									Если НЕ ЕстьОшибкаВСтроке(ЗаписьКниги) Тогда
										Продолжить;
									КонецЕсли;
...
// ---ввв 15.07.2016 г.
	...
	Иначе
		...
		Для Каждого НалоговыеПериоды Из ИтогПоОрганизации.Строки Цикл
			Для Каждого СтрокаПоПорядокОтражения Из НалоговыеПериоды.Строки Цикл
				Для Каждого ИтогПоДокументу Из СтрокаПоПорядокОтражения.Строки Цикл
// {{KM WARE Семенов И.С. Заявка № 33343 14.07.2015 начало
// в процедуре абс_УчетНДс.ПолучитьЗаписиКнигиПродаж добавили еще одну группировку 
					Для Каждого Исправление Из ИтогПоДокументу.Строки Цикл
// }}KM WARE Семенов И.С. Заявка № 33343 14.07.2015 окончание
					Для Каждого ЗаписьКниги Из Исправление.Строки Цикл
...
Процедура ЗаполнитьСтрокуКнигиПродаж(СтруктураПараметров, Секция, ЗаписьКниги, НомерСтроки)
	...
	Если ЗначениеЗаполнено(РасшифровкаДанныхПокупателя.Покупатель) Тогда
		...
		// {{KM WARE Малыгин П.К. Заявка № 25.01.2016 начало
		//Если контаргент "ИП", то выводим только ИНН
		Если ТипЗнч(РасшифровкаДанныхПокупателя.Контрагент) <> Тип("СправочникСсылка.Контрагенты") Тогда 
		...
		ИначеЕсли РасшифровкаДанныхПокупателя.Контрагент.ОКОПФ = Справочники.КлассификаторОКОПФ.НайтиПоНаименованию("ИП") Тогда	
		СоставСтроки.Вставить("Графа8", Строка(РасшифровкаДанныхПокупателя.Контрагент.ИНН));	
		Иначе	
			// }}KM WARE Малыгин П.К. Заявка № 25.01.2016 окончание
...
// +++ввв 26.07.2016 г.
	Если ЗначениеЗаполнено(РасшифровкаДанныхПокупателя.СчетФактура) Тогда
		СтрокаДополнительнойИнформации = СтруктураПараметров.СоответствиеДатОплатыСчетуФактуре[Строка(РасшифровкаДанныхПокупателя.СчетФактура.УникальныйИдентификатор())];
		Если СтрокаДополнительнойИнформации<>Неопределено Тогда
			Если СтрокаДополнительнойИнформации.Количество() > 0 Тогда
				Для Каждого ТекущаяДатаОплаты Из СтрокаДополнительнойИнформации Цикл
					Если СписокДатОплат.НайтиПоЗначению(Формат(ТекущаяДатаОплаты, "ДФ=dd.MM.yyyy")) = Неопределено тогда
						ТекстОплаты = ТекстОплаты + ?(ПустаяСтрока(ТекстОплаты), "", ","+Символы.ПС) + Формат(ТекущаяДатаОплаты, "ДФ=dd.MM.yyyy");
						СписокДатОплат.Добавить(Формат(ТекущаяДатаОплаты, "ДФ=dd.MM.yyyy"));
					КонецЕсли;
...
// ---ввв 26.07.2016 г.
// +++ввв 25.07.2016 г.	 
// {{KM WARE Семенов И.С. Заявка № 33343 13.07.2015 начало
// добавил процедуру из внешнего отчета, что бы заполнялась графа 11 раздела 9
//	если ТекстОплатыСНомером="" тогда
//// {{KM WARE Семенов И.С. Заявка № 35030 23.10.2015 начало
//// было
////	абс_ОпределитьНомерДатуДокументаОплаты(ЗаписьКниги,ТекстОплатыСНомером,ТекстОплаты);
//// {{KM WARE Малыгин П.К. Заявка № 20.01.2016 начало
//// Переносим вызов в привелегированный модуль
//// {{Старый код:
////абс_ОпределитьНомерДатуДокументаОплаты(ПараметрыЗаполнения,ТекстОплатыСНомером,ТекстОплаты);
//// }}Новый код:
//СтруктураЗК = Новый Структура;
//СписокКолонок  = ЗаписьКниги.Владелец().Колонки; 
//Для Каждого Колонка из СписокКолонок Цикл
...
//	
//		
//// }}KM WARE Семенов И.С. Заявка № 35030 23.10.2015 окончание
//	конецесли;
...
КонецПроцедуры
// +++ввв 27.07.2016 г.
Процедура ПолучитьДополнительнуюИнформациюПоСФДляКнигиПродажДатыОплаты(СтруктураПараметров, СписокСчетовФактур
// +++ввв 03.08.2016 г.
...
// ---ввв 03.08.2016 г.
	...
	// Определить частичные оплаты
	СтруктураПараметров.СоответствиеДатОплатыСчетуФактуре = Новый Соответствие;
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
...
// ---ввв 03.08.2016 г.
	...
	Для Каждого СтрокаСФ Из ДатыОплатСФ.Строки Цикл
		Если Не ЗначениеЗаполнено(СтрокаСФ.СчетФактура) Тогда
			Продолжить;
		КонецЕсли;
		Если СтрокаСФ.Строки.Количество()=0 Тогда
			Продолжить;
		КонецЕсли;
		СтруктураПараметров.СоответствиеДатОплатыСчетуФактуре.Вставить(Строка(СтрокаСФ.СчетФактура.УникальныйИдентификатор()),СтрокаСФ.Строки.ВыгрузитьКолонку("ДатаОплаты"));
	КонецЦикла; 

КонецПроцедуры // ПолучитьДополнительнуюИнформациюПоСФДляКнигиПродажДатыОплаты()
// ---ввв 26.07.2016 г.
...
// +++ввв 03.08.2016 г.		
	ПолучитьДополнительнуюИнформациюПоСФДляКнигиПродажДатыОплаты(СтруктураПараметров, СписокСчетовФактур,Истина);
// ---ввв 03.08.2016 г.
...
// +++ввв 15.03.2016 г.
// +++ввв 20.02.2016 г.
//						Если ЗаписьКниги.ВсегоПродаж+ЗаписьКниги.СуммаБезНДС10+ЗаписьКниги.СуммаБезНДС18+ЗаписьКниги.СуммаБезНДС20+ЗаписьКниги.СуммаСовсемБезНДС = 0 Тогда
// +++ввв 09.03.2016 г.
//						Если ЗаписьКниги.СуммаБезНДС10+ЗаписьКниги.СуммаБезНДС18+ЗаписьКниги.СуммаБезНДС20+ЗаписьКниги.СуммаСовсемБезНДС = 0 Тогда
						Если ЗаписьКниги.СуммаБезНДС10+ЗаписьКниги.СуммаБезНДС18+ЗаписьКниги.СуммаБезНДС20+
							 ЗаписьКниги.СуммаСовсемБезНДС+ЗаписьКниги.НДС0+
							 ЗаписьКниги.НДС10+ЗаписьКниги.НДС18+ЗаписьКниги.НДС20 = 0 Тогда
// ---ввв 09.03.2016 г.
// ---ввв 20.02.2016 г.
// +++ввв 12.05.2017 г.
							Если НЕ ЕстьОшибкаВСтроке(ЗаписьКниги) Тогда
...
// ---ввв 15.03.2016 г.
// +++ввв 16.06.2016 г.
						Если ЗаписьКниги.СуммаБезНДС10<>0 И ЗаписьКниги.НДС10=0 Тогда
							Продолжить;
						КонецЕсли;
						Если ЗаписьКниги.СуммаБезНДС18<>0 И ЗаписьКниги.НДС18=0 Тогда
							Продолжить;
						КонецЕсли;
						Если ЗаписьКниги.СуммаБезНДС20<>0 И ЗаписьКниги.НДС20=0 Тогда
							Продолжить;
						КонецЕсли;
// ---ввв 16.06.2016 г.
						Счетчик = Счетчик + 1;
						...
						// {{KM WARE Малыгин П.К. Заявка № 7718454 05.02.2016 начало
						// Строки с нулевыми суммами выводиться не должны
						ЗаполнитьСтрокуКнигиПродаж(СтруктураПараметров, ПараметрыСтроки, ЗаписьКниги, Счетчик);
						// }}KM WARE Малыгин П.К. Заявка № 7718454 05.02.2016 окончание
...
// ---ввв 25.07.2016 г.
	...
	// {{Старый код:
	//	Возврат абс_УчетНДС.ОпределитьДатуИНомерСФ(ЗаписьКниги, СтруктураПараметров);
	// }}Новый код:
	СтруктураЗК = Новый Структура;
	СписокКолонок  = ЗаписьКниги.Владелец().Колонки; 
	Для Каждого Колонка из СписокКолонок Цикл
		СтруктураЗК.Вставить(Колонка.Имя, ЗаписьКниги[Колонка.Имя]);	
	КонецЦикла;	
	
	Возврат абс_БизнесПроцессыПривелегированный.ОпределитьДатуИНомерСФ(СтруктураЗК, СтруктураПараметров.Организация);
...
//			КонецЕсли;
//		КонецЕсли;
//		Если ФлагОсобый Тогда
//			ВариантыПредставленияКоррСчетаФактуры.Номер = СокрЛП(ЗаписьКниги.СчетФактураДокумент.Номер);
//			ВариантыПредставленияКоррСчетаФактуры.Дата = Формат(ЗаписьКниги.СчетФактураДокумент.Дата, "ДФ=dd.MM.yyyy");
//		Иначе
		...
		ВариантыПредставленияКоррСчетаФактуры.Дата = Формат(ЗаписьКниги.ДатаКорректировки, "ДФ=dd.MM.yyyy");
// ---ввв 12.05.2016 г.
		ВариантыПредставленияКоррСчетаФактуры.НомерДата = "" + ВариантыПредставленияКоррСчетаФактуры.Номер 
														  + ";" + ВариантыПредставленияКоррСчетаФактуры.Дата;
// ---ввв 14.04.2016 г.
	КонецЕсли;
...
Процедура ОбработатьКорректировочныеСчетаФактуры() Экспорт
	...
	Пока Выборка.Следующий() Цикл
	    ...
	    Попытка
			...
			СчетФактура.ВидСчетаФактуры				= Перечисления.НДСВидСчетаФактуры.Корректировочный;
...
Функция СобратьДанныеДляПечатиСчетФактурыВыданногоПоДокументу(ДокументОснование, Ссылка, УчетнаяПолитика)
	...
	Если Ссылка.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаАванс
		ИЛИ Ссылка.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаАвансКомитента
		ИЛИ Ссылка.ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаСуммовуюРазницу Тогда
...
Процедура НеИспользуется_абс_ОпределитьНомерДатуДокументаОплаты(ЗаписьКниги,ТекстОплатыСНомером,ТекстОплаты)
// +++ввв 21.07.2016 г.
...
КонецПроцедуры
// }}KM WARE Семенов И.С. Заявка № 33343 13.07.2015 окончание

// Заполнение данных для операций комисии - Вид операции 04
// {{KM WARE Малыгин П.К. Заявка № 34043 16.10.2015 начало
Процедура kmwr_ОпределитьСчетФактуруИПрочее(ЗаписьКниги,ТекстОплатыСНомером,СоставСтроки)
	Если ЗаписьКниги.КодВидаОперации<>"04" Тогда
	Возврат;
	КонецЕсли;
	...
	Запрос.Текст = 
	                     "ВЫБРАТЬ
	                     |	СчетФактураПолученныйДокументыОснования.Ссылка.абс_Продавец,
	                     ...
	                     |	СчетФактураПолученныйДокументыОснования.Ссылка.Контрагент.ИНН,
	                     |	СчетФактураПолученныйДокументыОснования.Ссылка.Контрагент.КПП,
	                     |	СчетФактураПолученныйДокументыОснования.Ссылка,
	                     |	СчетФактураПолученныйДокументыОснования.Ссылка.Номер,
	                     |	СчетФактураПолученныйДокументыОснования.Ссылка.Дата,
	                     |	СчетФактураПолученныйДокументыОснования.Ссылка.Контрагент.НаименованиеПолное,
	                     |	СчетФактураПолученныйДокументыОснования.Ссылка.абс_Продавец.НаименованиеПолное,
	                     |	СчетФактураПолученныйДокументыОснования.Ссылка.абс_Продавец.ИНН,
	                     |	СчетФактураПолученныйДокументыОснования.Ссылка.абс_Продавец.КПП
	                     |ИЗ
	                     |	Документ.СчетФактураПолученный.ДокументыОснования КАК СчетФактураПолученныйДокументыОснования
	                     |ГДЕ
	                     |	СчетФактураПолученныйДокументыОснования.ДокументОснование = &ДокументОснование
	                     |	И СчетФактураПолученныйДокументыОснования.Ссылка.КодВидаОперации = ""04""";
	 
	 Запрос.УстановитьПараметр("ДокументОснование",ЗаписьКниги.СчетФактураДокумент);
	 
	 РезультатТЗ = Запрос.Выполнить().Выгрузить();
	 
	 Результат = Запрос.Выполнить().Выбрать();
	 Если Результат.Следующий() Тогда
	ЗаписьКниги.Посредник = Результат.КонтрагентНаименованиеПолное; 	 
	ЗаписьКниги.Продавец = Результат.абс_ПродавецНаименованиеПолное;
	ЗаписьКниги.ДатаДокументаОплаты = ЗаписьКниги.СчетФактураДокумент.Дата;
	ЗаписьКниги.НомерДокументаОплаты = ЗаписьКниги.СчетФактураДокумент.Номер;
	ЗаписьКниги.СчетФактура =  Результат.Ссылка;
	ЗаписьКниги.СчетФактураДокумент = Результат.Ссылка;
	ЗаписьКниги.СчетФактураДокументРасшифровка = Результат.Ссылка;

	//ЗаписьКниги.kmwСчетФактураНомер	= Результат.Номер;
	//ЗаписьКниги.kmwСчетФактураДата =   Результат.Дата;
	ЗаписьКниги.СчетФактураДата	=   Результат.Дата;
	
	ПродавецИНН = Результат.абс_ПродавецИНН;
	ПродавецКПП = Результат.абс_ПродавецИНН;	
	ПосредникИНН =  Результат.КонтрагентИНН;
	ПосредникКПП =  Результат.КонтрагентКПП;
	
	ЗаписьКниги.ПродавецИНН=ПродавецИНН;
    ЗаписьКниги.ПродавецКПП=ПродавецКПП;
	ЗаписьКниги.ПосредникИНН= ПосредникИНН;
	ЗаписьКниги.ПосредникКПП=ПосредникКПП;
	
	ТекстОплатыСНомером = ЗаписьКниги.НомерДокументаОплаты+";"+Формат(ЗаписьКниги.ДатаДокументаОплаты,"ДФ=dd.MM.yyyy");
	
	СоставСтроки.СчетФактура = Результат.Ссылка;
   	СоставСтроки.НомерДата = Результат.НомерВходящегоДокумента+";"+Формат(Результат.ДатаВходящегоДокумента,"ДФ=dd.MM.yyyy");	
КонецЕсли;

КонецПроцедуры // kmwr_ОпределитьСчетФактуруИПрочее() }}KM WARE Малыгин П.К. Заявка № 34043 16.10.2015 окончание
...
Процедура ЗаполнитьСписокКодовВидовОпераций2016(ЧастьЖурнала, СписокКодовОпераций, Период) Экспорт
...
Процедура ttk_ОбработкаЗаписейРегистраНДСПриЗаписи(Источник, Отказ, Замещение) Экспорт
КонецПроцедуры
