...
Процедура ЗачиститьНаборЗаписейОтРегламентныхОпераций(НаборЗаписей, МассивРегОпераций, ИмяРеквизита = "ВидРегламентнойОперации") Экспорт
	...
	Если НаборЗаписей = Неопределено или МассивРегОпераций = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьСубконто(Счет, Субконто, ИмяСубконто, ЗначениеСубконто, Сообщать = Ложь, Заголовок = "", ВидыСубконтоСчета = Неопределено) Экспорт
	...
	ИначеЕсли Сообщать Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Неверное значение """ + ЗначениеСубконто + """ для вида субконто <" + ВидСубк + ">",, Заголовок);
	КонецЕсли;
...
// АБС ИЗМЕНЕНО Списание РБП по группам
Процедура РасчетИСписаниеРПБ(СтруктураШапкиДокумента, Отказ, ВидРБП, ВидУчета, мПроводки, ЗаписьРасчетаРБП, ГруппаРБП = Неопределено) Экспорт
	...
	Если ВидУчета = "БУ" Тогда
		...
		// АБС ВСТАВКА расчеты по имущественному страхованию
		Массив.Добавить(ПланыСчетов.Хозрасчетный.ПлатежиПоИмущественномуСтрахованию);
		// АБС ВСТАВКА КОНЕЦ
		...
		|						ИНАЧЕ ЛОЖЬ
		|					КОНЕЦ)) КАК ХозрасчетныйОстатки " + ?(ЗначениеЗаполнено(ГруппаРБП), "
		|ГДЕ
		|	(ВЫРАЗИТЬ(ХозрасчетныйОстатки.Субконто1 КАК Справочник.РасходыБудущихПериодов) В ИЕРАРХИИ (&ГруппаРБП)
		|			ИЛИ ВЫРАЗИТЬ(ХозрасчетныйОстатки.Субконто2 КАК Справочник.РасходыБудущихПериодов) В ИЕРАРХИИ (&ГруппаРБП)
		|			ИЛИ ВЫРАЗИТЬ(ХозрасчетныйОстатки.Субконто3 КАК Справочник.РасходыБудущихПериодов) В ИЕРАРХИИ (&ГруппаРБП))
		| " , "") + "
		|СГРУППИРОВАТЬ ПО
		...
		//|// АБС ВСТАВКА Списание РБП по группам 
		//|" + ?(ГруппаРБП = Неопределено, "" , "
		//|				ГДЕ Субконто1 В ИЕРАРХИИ (&ГруппаРБП) ИЛИ Субконто2 В ИЕРАРХИИ (&ГруппаРБП) ИЛИ Субконто3 В ИЕРАРХИИ (&ГруппаРБП)") + "
		//|СГРУППИРОВАТЬ ПО
		//|	ХозрасчетныйОстатки.Субконто1,
		//|	ХозрасчетныйОстатки.Субконто2,
		//|	ХозрасчетныйОстатки.Субконто3,
		...
		// АБС ВСТАВКА Списание РБП по группам	
		Запрос.УстановитьПараметр("ГруппаРБП", ГруппаРБП);
		//Запрос.УстановитьПараметр("НеИспользоватьОтборПоРБП", НЕ ЗначениеЗаполнено(ГруппаРБП));		
		// АБС ВСТАВКА Списание РБП по группам КОНЕЦ
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ПроизводственныеРасходы Тогда
				//АБС для 25 счета установим вид деятельности
			...
			// Гущина 2016.10.25   T#7742262 +++
			//СтрокаЗаписьРасчетаРБП.КонОстаток = Выборка.Остаток - СуммаСписания;    // было  
			СтрокаЗаписьРасчетаРБП.КонОстаток = Выборка.Остаток - СтрокаЗаписьРасчетаРБП.Сумма; // с учетом округления   
			// Гущина 2016.10.25   T#7742262 ---	
		КонецЦикла;
	...
	Если ВидУчета = "НУ" Тогда
		...
		|							ТОГДА ВЫРАЗИТЬ(Субконто2 КАК Справочник.РасходыБудущихПериодов).ВидРБП В (&ВидРБП)
		|						ИНАЧЕ ЛОЖЬ
		|					КОНЕЦ)) КАК НалоговыйОстатки " + ?(ЗначениеЗаполнено(ГруппаРБП), "
		|ГДЕ
		|	(ВЫРАЗИТЬ(НалоговыйОстатки.Субконто1 КАК Справочник.РасходыБудущихПериодов) В ИЕРАРХИИ (&ГруппаРБП)
		|			ИЛИ ВЫРАЗИТЬ(НалоговыйОстатки.Субконто2 КАК Справочник.РасходыБудущихПериодов) В ИЕРАРХИИ (&ГруппаРБП))
		|", "") + "
		|СГРУППИРОВАТЬ ПО
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ПроизводственныеРасходыНУ Тогда
				//АБС заполним вид деятельности в НУ
				УстановитьСубконто(ПроводкаНУ.СчетДт, ПроводкаНУ.СубконтоДт, 3, РБП.СубконтоБУ3);
				//\\АБС
			...
			// РБП списанные на 84.05 проводим по ВР в Дт ПВ
			Если РБП.абс_ЭтоРБПДо2011Года И ПроводкаНУ.ВидУчетаДт = Перечисления.ВидыУчетаПоПБУ18.ВР Тогда		
				
				ПроводкаНУ.СчетДт = ПланыСчетов.Налоговый.ПоступлениеИВыбытиеИмуществаРаботУслугПрав;
				
				ПроводкаНУ.СубконтоДт.Очистить();
				
			КонецЕсли;
			// АБС ВСТАВКА Фролов 20111014 КОНЕЦ
...
Процедура ОбработатьВыборСубконто(Элемент, СтандартнаяОбработка, Организация, СписокПараметров = Неопределено, ОграничениеТипаЭлемента = Неопределено) Экспорт
	...
	ИначеЕсли ТипЭлемента = Тип("ДокументСсылка.ДокументРасчетовСКонтрагентом") Тогда
		...
		Возврат;
	// АБС ВСТАВКА начало	
    ИначеЕсли ТипЭлемента = Тип("СправочникСсылка.НоменклатурныеГруппы") Тогда

		СтандартнаяОбработка = Ложь;
		//ФормаВыбора          = Справочники.НоменклатурныеГруппы.ПолучитьФормуВыбора("абс_ФормаВыбора",Элемент, "Отчет");
		ФормаВыбора          = Справочники.НоменклатурныеГруппы.ПолучитьФормуВыбора(,Элемент,);
		
		ФормаВыбора.СправочникСписок.Отбор.абс_ИспользуетсяДляОтчетов.Значение      = Истина;
		ФормаВыбора.СправочникСписок.Отбор.абс_ИспользуетсяДляОтчетов.Использование = Истина;
		ФормаВыбора.СправочникСписок.Отбор.абс_ИспользуетсяДляОтчетов.ВидСравнения  = ВидСравнения.Равно;
		ФормаВыбора.ЭлементыФормы.СправочникСписок.НастройкаОтбора.абс_ИспользуетсяДляОтчетов.Доступность = Ложь;
	ИначеЕсли ТипЭлемента = Тип("СправочникСсылка.СтатьиЗатрат") Тогда

		СтандартнаяОбработка = Ложь;
		
	//	ФормаВыбора          = Справочники.СтатьиЗатрат.ПолучитьФормуВыбора("абс_ФормаВыбора",Элемент, "Отчет");
...
Функция ПолучитьМассивСчетов(ИмяПланаСчетов, Знач СчетаСтрокой = "", ИсключитьГруппы = Ложь) Экспорт
	...
	Пока Не ПустаяСтрока(СчетаСтрокой) Цикл
		...
		Если НЕ ЗначениеЗаполнено(Счет) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не найден счет с кодом """ + Код + """ (план счетов - " + ИмяПланаСчетов + ")");
			Продолжить;
		КонецЕсли;
...
Процедура УстановитьСубконтоПоВидуСубконто(Счет, Субконто, ВидСубконто, ЗначениеСубконто) Экспорт
	...
	Иначе
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Неверное значение """ + ЗначениеСубконто + """ для вида субконто <" + ВидСубконто + ">");
	КонецЕсли;
...
Процедура СвернутьНаборЗаписейРегистраБухгалтерии(НаборЗаписейРегистраБухгалтерии) Экспорт
	...
	СтрокаГруппировка = "Период, Регистратор, Активность, Организация, "+
		...
		МассивСчетовРасчетовСПерсоналом.Добавить(ПланыСчетов.Хозрасчетный.РасходыНаОплатуТрудаБудущихПериодов);
	                                                                                                                               //АБС настройка для своднго учета 
	ttk_ОбщегоНазначения.СвернутьНаборЗаписейРегистраБухгалтерии(НаборЗаписейРегистраБухгалтерии,СтрокаГруппировка,СтрокаСуммирование, Константы.ВестиРеглУчетРасчетовСПерсоналомСводно.Получить(),МассивСчетовРасчетовСПерсоналом);
КонецПроцедуры
