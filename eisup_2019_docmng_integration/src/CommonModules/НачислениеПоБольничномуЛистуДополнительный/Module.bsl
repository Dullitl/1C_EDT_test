...
Процедура ПроверитьЗаполнениеСтрокиОтраженияВУчете(ВыборкаПоДоходам, Отказ, ПроверятьСочетание, Заголовок = "") Экспорт 
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоДоходам.ПериодДействияНачало) Тогда
		ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указана дата начала периода начисления!", Отказ, Заголовок);
	Иначе
		// Дата окончания
		Если НЕ ЗначениеЗаполнено(ВыборкаПоДоходам.ПериодДействияКонец) Тогда
			ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указана дата окончания периода начисления!", Отказ, Заголовок);
		Иначе
			Если ВыборкаПоДоходам.ПериодДействияКонец < ВыборкаПоДоходам.ПериодДействияНачало Тогда
				ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "дата окончания периода начисления не должна быть меньше даты начала!", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьСтрокуОтраженияВУчете(ДокументОбъект, ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, СтруктураПараметровПеременныхВеденияУчета) Экспорт 
	...
	СтрокаСвойствИзСтрокиДокумента = "ПериодДействияНачало,ПериодДействияКонец,Результат," 
							+ "СчетДт,СчетКт,СпособРаспределенияЗатрат,РасходыУСН,"
...
Функция СформироватьЗапросПоОтражениеВУчете(ВыборкаПоШапкеДокумента) Экспорт 
	...
	|	СтрокаОтраженияВУчете.ОтражениеВУСН КАК РасходыУСН,
...
Функция ПолучитьСтруктураПараметровПеременныхВеденияУчета(Организация, ПериодРегистрации) Экспорт
	...
	ИП_ОСН = Организация.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо и Не УСН; //индивидуальный предприниматель на общей системе налогообложения
	
	ЗаполнятьДанныеНУ = Не (УСН или ИП_ОСН); // или ПрограммаБухучета = Перечисления.ПрограммыБухгалтерскогоУчета.ДругаяПрограммаБухучета);
	
	ЗаполнятьСчетаНУ  = Истина; //ПрограммаБухучета = Перечисления.ПрограммыБухгалтерскогоУчета.Бухгалтерия77
						//или ПрограммаБухучета = Перечисления.ПрограммыБухгалтерскогоУчета.БухгалтерияПредприятия16;
						
	УчетПоПодразделениямНаСчетах = Ложь; //ПрограммаБухучета = Перечисления.ПрограммыБухгалтерскогоУчета.БухгалтерияПредприятияКорп
						//или ПрограммаБухучета = Перечисления.ПрограммыБухгалтерскогоУчета.БухгалтерияАвтономногоУчреждения;
	
	СтруктураПараметров = Новый Структура("ЗаполнятьДанныеНУ, ЗаполнятьСчетаНУ, УчетПоПодразделениямНаСчетах", ЗаполнятьДанныеНУ, ЗаполнятьСчетаНУ, УчетПоПодразделениямНаСчетах);
...
Функция ПолучитьДанныеУчета2009(ВыборкаПоШапкеДокумента, ЧитатьРасчетнуюБазу) Экспорт 
	...
	Если КоличествоСторноЗаписей <> 0 Тогда
		...
		|			БУОсновныеНачисления.РасходыУСН КАК РасходыУСН,
	...
	Если ЧитатьРасчетнуюБазу Тогда
		...
		|	БазаСреднегоЗаработка.РасходыУСНРазрез,
		...
		|	БазаСреднегоЗаработка.РасходыУСНРазрез КАК РасходыУСНРазрез,
		|	БазаСреднегоЗаработка.РезультатБаза КАК РезультатБаза,
		|	ЕСТЬNULL(ВЫБОР
		...
		|	ОтражениеВУчетеСторно.РасходыУСН,
