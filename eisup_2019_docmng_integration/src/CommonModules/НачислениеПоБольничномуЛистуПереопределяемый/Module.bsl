...
Процедура АвтозаполнениеПроводок2009(ДокументОбъект, ВыборкаПоШапкеДокумента = Неопределено, ФормироватьДвижения = Истина)
	...
	СтруктураПараметровПеременныхВеденияУчета = НачислениеПоБольничномуЛистуДополнительный.ПолучитьСтруктураПараметровПеременныхВеденияУчета(ВыборкаПоШапкеДокумента.ОбособленноеПодразделение, ВыборкаПоШапкеДокумента.ПериодРегистрации);
...
Процедура АвтозаполнениеПроводок2010(ДокументОбъект, ВыборкаПоШапкеДокумента = Неопределено, ФормироватьДвижения = Истина)
	...
	НаборЗаписей.Свернуть("КодДоходаЕСН,ОблагаетсяЕНВД,СчетДт,СубконтоДт1,СубконтоДт2,СубконтоДт3,СчетКт,СубконтоКт1,СубконтоКт2,СубконтоКт3,СчетДтНУ,СчетКтНУ,СубконтоДтНУ1,СубконтоДтНУ2,СубконтоДтНУ3,СубконтоКтНУ1,СубконтоКтНУ2,СубконтоКтНУ3,СпособРаспределенияЗатрат,ОтражениеВУСН,ПериодДействияНачало, ПериодДействияКонец, Сторно,ПодразделениеДт,ПодразделениеКт,ВидНачисленийОплатыТрудаПоСтатье255НК","Результат,ДополнительныйРезультат,СуммаНУ,СуммаВР,СуммаПР");
	...
	Если ВыборкаПоШапкеДокумента.ПерерассчитываемыйДокумент.Пустая() 
		...
		Если ВыборкаПоШапкеДокумента.ДатаНачалаСобытия < мДатаЗакона255ФЗ И ВыборкаПоШапкеДокумента.ДатаОкончания >= мДатаЗакона255ФЗ И ВыборкаПоШапкеДокумента.ПричинаНетрудоспособности <> Перечисления.ПричиныНетрудоспособности.ТравмаНаПроизводстве Тогда
			Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ПричинаНетрудоспособностиПереходногоПериода) Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указана причина нетрудоспособности для переходного периода!", Отказ, Заголовок);
			ИначеЕсли ВыборкаПоШапкеДокумента.ПричинаНетрудоспособностиПереходногоПериода = Перечисления.ПричиныНетрудоспособности.ПоУходуЗаРебенком И НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.СлучайУходаЗаБольнымРебенкомПереходногоПериода) Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указан случай ухода за ребенком для переходного периода!", Отказ, Заголовок);
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ПроцентОплатыПереходногоПериода) Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указан процент оплаты для переходного периода!", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПереписатьПрочиеДвиженияПриПерерасчете(ДокументОбъект, ВыборкаПоШапкеДокумента) Экспорт 
	...
	Если ВыборкаПоШапкеДокумента.ОтражатьВУчетеЭтимДокументом Тогда
		...
		Пока ВыборкаПоОтражениеВУчете.Следующий() Цикл
			НачислениеПоБольничномуЛистуДополнительный.ДобавитьСтрокуОтраженияВУчете(ДокументОбъект, ВыборкаПоШапкеДокумента, ВыборкаПоОтражениеВУчете);
		КонецЦикла;
	КонецЕсли;
...
Функция ПолучитьСтруктуруПечатныхФорм(ДокументОбъект) Экспорт
	...
	Если НачислениеПоБольничномуЛистуДополнительный.ОпределитьПереходныйПериод(ДатаНачалаСобытия, ДатаНачала, ДатаОкончания, ПричинаНетрудоспособности) Тогда
		...
		Возврат Новый Структура("РасчетСреднегоЗаработка, РасчетСреднегоЗаработкаПП, ОборотЛисткаНетрудоспособности", "Расчет среднего заработка", "Расчет среднего заработка переходного периода", "Оборот листка нетрудоспособности переходного периода");
	КонецЕсли;
...
Функция ПолучитьТекстДополнительнойТаблицыДокумента(Запрос, СобиратьДополнительныеДанные) Экспорт
	...
	Если СобиратьДополнительныеДанные Тогда
		...
		|		КОГДА СпособыОтраженияЗарплатыВРеглУчете.РасходыУСН <> ЗНАЧЕНИЕ(Справочник.СпособыОтраженияЗарплатыВРеглУчете.ПустаяСсылка)
		|			ТОГДА СпособыОтраженияЗарплатыВРеглУчете.СпособРаспределенияЗатрат
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК СпособРаспределенияЗатрат,
		|	ВЫБОР
		|		КОГДА УчетОсновногоЗаработкаСрезПоследних.СпособОтраженияВБухучетеБольничногоЗаСчетРаботодателя <> ЗНАЧЕНИЕ(Справочник.СпособыОтраженияЗарплатыВРеглУчете.ПустаяСсылка) 
		|			ТОГДА УчетОсновногоЗаработкаСрезПоследних.СпособОтраженияВБухучетеБольничногоЗаСчетРаботодателя.РасходыУСН
		|		КОГДА УчетПодразделенияОрганизацииСрезПоследних.СпособОтраженияВБухучетеБольничногоЗаСчетРаботодателя <> ЗНАЧЕНИЕ(Справочник.СпособыОтраженияЗарплатыВРеглУчете.ПустаяСсылка) 
		|			ТОГДА УчетПодразделенияОрганизацииСрезПоследних.СпособОтраженияВБухучетеБольничногоЗаСчетРаботодателя.РасходыУСН
		|		КОГДА СпособыОтраженияЗарплатыВРеглУчете.РасходыУСН <> ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.ПустаяСсылка)
		|			ТОГДА СпособыОтраженияЗарплатыВРеглУчете.РасходыУСН
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь, ДокументОбъект) Экспорт
	...
	Если ИмяМакета = "РасчетСреднегоЗаработка" Тогда
		Если ДатаНачалаСобытия < ДатаЗакона255ФЗ И ДатаОкончания >= ДатаЗакона255ФЗ И ПричинаНетрудоспособности <> Перечисления.ПричиныНетрудоспособности.ТравмаНаПроизводстве Тогда
			...
			ТабДокумент = ФормированиеПечатныхФорм.ПечатьРасчетаСреднегоЗаработка(Ссылка, ДатаНачалаСобытия, ПериодРасчетаСреднегоЗаработка, "с " + Формат(ДатаНачала,"ДФ=dd.MM.yyyy") + " по " + Формат(ДатаОкончания,"ДФ=dd.MM.yyyy"), Перечисления.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработкуФСС, ?(ДатаНачалаСобытия < ДатаЗакона255ФЗ, Неопределено, Ложь),СписокСоставляющихСреднегоЗаработка);
			Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ДокументОбъект, ДокументОбъект.Метаданные().Синоним) + " (расчет среднего заработка)");
		КонецЕсли;
	ИначеЕсли ИмяМакета = "ОборотЛисткаНетрудоспособности" Тогда                                                                                                         
		Если ДатаНачалаСобытия < ДатаЗакона255ФЗ И ДатаОкончания >= ДатаЗакона255ФЗ И ПричинаНетрудоспособности <> Перечисления.ПричиныНетрудоспособности.ТравмаНаПроизводстве Тогда
			...
			Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ДокументОбъект, ДокументОбъект.Метаданные().Синоним) + " (оборот листка нетрудоспособности)");
		КонецЕсли;
	ИначеЕсли ИмяМакета = "РасчетСреднегоЗаработкаПП" Тогда
		...
		ТабДокумент = ФормированиеПечатныхФорм.ПечатьРасчетаСреднегоЗаработка(Ссылка, ДатаНачалаСобытия, ПериодРасчетаСреднегоЗаработка, "с " + Формат(ДатаНачала,"ДФ=dd.MM.yyyy") + " по " + Формат(ДатаОкончания,"ДФ=dd.MM.yyyy"), Перечисления.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработкуФСС,  Ложь,СписокСоставляющихСреднегоЗаработка);
		Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ДокументОбъект, ДокументОбъект.Метаданные().Синоним) + " (расчет среднего заработка)");
	КонецЕсли;
...
Функция ПечатьОборотаБольничного(ДокументОбъект) Экспорт
	...
	Запрос.УстановитьПараметр("ДатаПредельнойБазы",	 ?(мДатаЗакона213ФЗ > ДатаНачалаСобытия, мДатаЗакона213ФЗ, ДатаНачалаСобытия));
	Запрос.УстановитьПараметр("ДатаЗакона255ФЗ",	 мДатаЗакона255ФЗ);
	...
	ИначеЕсли ПериодРегистрации < мДатаЗакона213ФЗ Тогда
	...
	|	ВЫБОР
	|		КОГДА &УСН
	|			ТОГДА БУОсновныеНачисления.РасходыУСН
	...
	Если ЗначениеЗаполнено(ДатаНепрерывногоСтажа) Тогда
		ОбщегоНазначения.РазобратьРазностьДат(ДатаНачалаСобытия	, ДатаНепрерывногоСтажа, ЛетСтажа, МесяцевСтажа, ДнейСтажа);
	КонецЕсли;
	...
	Если ПериодРегистрации < мДатаЗакона213ФЗ Тогда
		...
		КонецЕсли;
		ПечатьРасчетаСреднегоЗаработкаОтчет = ФормированиеПечатныхФорм.ПечатьРасчетаСреднегоЗаработка(Ссылка, ДатаНачалаСобытия	, ПериодРасчетаСреднегоЗаработка, "", Перечисления.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработкуФСС, ?(ДатаНачалаСобытия < мДатаЗакона255ФЗ, Неопределено, Ложь), СписокСоставляющихСреднегоЗаработка, Ложь, Ложь, СтруктураПоказателей);
	ИначеЕсли ДатаНачалаОплаты < мДатаЗакона213ФЗ Тогда // переходный период 2009-2010 годов
		
		СписокСоставляющихСреднегоЗаработка = Новый Массив(2); 
		СписокСоставляющихСреднегоЗаработка[0] = ПланыВидовРасчета.СреднийЗаработок.ПоЗаработкуПособий;
		СписокСоставляющихСреднегоЗаработка[1] = ПланыВидовРасчета.СреднийЗаработок.ПоПремиямПособий;
		Начало = ФормированиеПечатныхФорм.ПечатьРасчетаСреднегоЗаработка(Ссылка, ДатаНачалаСобытия	, ПериодРасчетаСреднегоЗаработка, "", Перечисления.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработкуФСС, Ложь, СписокСоставляющихСреднегоЗаработка, Ложь, Ложь, СтруктураПоказателей);
		...
		СписокСоставляющихСреднегоЗаработка[0] = ПланыВидовРасчета.СреднийЗаработок.ПоЗаработкуПособийС2010;
		СписокСоставляющихСреднегоЗаработка[1] = ПланыВидовРасчета.СреднийЗаработок.ПоПремиямПособийС2010;
		Продолжение = ФормированиеПечатныхФорм.ПечатьРасчетаСреднегоЗаработка(Ссылка, ДатаНачалаСобытия	, ПериодРасчетаСреднегоЗаработка, "", Перечисления.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработкуФСС, Ложь, СписокСоставляющихСреднегоЗаработка, Ложь, Ложь, СтруктураПоказателей);
	...
	ИначеЕсли ДатаНачалаСобытия < мДатаЗакона213ФЗ Тогда  // продолжение переходящего б/л, полностью в 2010 году
		СписокСоставляющихСреднегоЗаработка = Новый Массив(2); 
		СписокСоставляющихСреднегоЗаработка[0] = ПланыВидовРасчета.СреднийЗаработок.ПоЗаработкуПособийС2010;
		СписокСоставляющихСреднегоЗаработка[1] = ПланыВидовРасчета.СреднийЗаработок.ПоПремиямПособийС2010;
		ПечатьРасчетаСреднегоЗаработкаОтчет = ФормированиеПечатныхФорм.ПечатьРасчетаСреднегоЗаработка(Ссылка, ДатаНачалаСобытия	, ПериодРасчетаСреднегоЗаработка, "", Перечисления.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработкуФСС, Ложь, СписокСоставляющихСреднегоЗаработка, Ложь, Ложь, СтруктураПоказателей);
	Иначе	
		ПечатьРасчетаСреднегоЗаработкаОтчет = ФормированиеПечатныхФорм.ПечатьРасчетаСреднегоЗаработка(Ссылка, ДатаНачалаСобытия	, ПериодРасчетаСреднегоЗаработка, "", Перечисления.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработкуФСС, Ложь, СписокСоставляющихСреднегоЗаработка, Ложь, Ложь, СтруктураПоказателей);
	КонецЕсли;
...
Процедура ОбновитьИнформационнуюНадписьСреднийЗаработок(ДокументОбъект, ФормаДокумента) Экспорт                
	...
	ДатаОкончания				= ДокументОбъект.ДатаОкончания;
	ПричинаНетрудоспособности	= ДокументОбъект.ПричинаНетрудоспособности;
	Дата1 = ДокументОбъект.ПериодРасчетаСреднегоЗаработкаНачало;
	Дата2 = ДокументОбъект.ПериодРасчетаСреднегоЗаработкаОкончание;
...
Процедура ОбновитьРазмерыПособия(ДокументОбъект, ПоддержкаПереходногоПериода = Истина,  ТолькоПереходныйПериод = Ложь) Экспорт 
	...
	Если ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.ТравмаНаПроизводстве Тогда
		...
		ДокументОбъект.ПроцентОплаты = 100;
		ДокументОбъект.ОграничениеПособия = Перечисления.ВидыОграниченияПособия.БезОграничений;
	
	ИначеЕсли ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.ПоБеременностиИРодам Тогда	
		
		Если Не ТолькоПереходныйПериод Тогда
			ДокументОбъект.ПроцентОплаты = 100;
		КонецЕсли;
		...
		Если Не ТолькоПереходныйПериод И ДатаНачалаСобытия >= мДатаЗакона255ФЗ Тогда
			
			ДатаСтажа = ПроведениеРасчетов.ПолучитьНачалоСтажаДляБольничногоЛиста(ДатаНачалаСобытия, Физлицо, мГоловнаяОрганизация);
			Если ЗначениеЗаполнено(ДатаСтажа) Тогда
				...
				Иначе
					ДокументОбъект.ОграничениеПособия = Перечисления.ВидыОграниченияПособия.ОграничениеВРазмереММОТ;
				КонецЕсли;
			Иначе
				ДокументОбъект.ОграничениеПособия = Перечисления.ВидыОграниченияПособия.ОграничениеВРазмереММОТ;
			КонецЕсли;
			
		ИначеЕсли ПоддержкаПереходногоПериода И мЭтоПереходныйПериодПоЗакону255ФЗ	Тогда
			
			Если ДокументОбъект.ОграничениеПособия = Перечисления.ВидыОграниченияПособия.БезОграничений или ДокументОбъект.ОграничениеПособия.Пустая() Тогда
				ДокументОбъект.ОграничениеПособия = Перечисления.ВидыОграниченияПособия.ОбщееОграничение
			КонецЕсли;
		...
		ИначеЕсли Не ТолькоПереходныйПериод И ДокументОбъект.ОграничениеПособия = Перечисления.ВидыОграниченияПособия.БезОграничений или ДокументОбъект.ОграничениеПособия.Пустая() Тогда
			ДокументОбъект.ОграничениеПособия = Перечисления.ВидыОграниченияПособия.ОбщееОграничение
		КонецЕсли;
	...
	ИначеЕсли Не ТолькоПереходныйПериод И ДатаНачалаСобытия >= мДатаЗакона255ФЗ Тогда
		...
		Если ЗначениеЗаполнено(ДатаСтажа) Тогда
			...
			Иначе
				ДокументОбъект.ОграничениеПособия = Перечисления.ВидыОграниченияПособия.ОграничениеВРазмереММОТ;
			КонецЕсли;
			ДокументОбъект.ПроцентОплаты = ?(УжеНеРаботает, 60,?(Лет >= 8, 100,?(Лет >= 5, 80, 60)));
			
		Иначе
			
			ДокументОбъект.ОграничениеПособия = Перечисления.ВидыОграниченияПособия.ОграничениеВРазмереММОТ;
			ДокументОбъект.ПроцентОплаты = 60;
		...
		Если ПоддержкаПереходногоПериода И мЭтоПереходныйПериодПоЗакону255ФЗ Тогда
			...
			Если ЗначениеЗаполнено(ДатаСтажа) Тогда
				...
				Иначе
					ДокументОбъект.ОграничениеПособияПереходногоПериода = Перечисления.ВидыОграниченияПособия.ОграничениеВРазмереММОТ;
				КонецЕсли;
				ДокументОбъект.ПроцентОплатыПереходногоПериода = ?(УжеНеРаботает, 60,?(Лет >= 8, 100,?(Лет >= 5, 80, 60)));
			Иначе
				ДокументОбъект.ОграничениеПособияПереходногоПериода = Перечисления.ВидыОграниченияПособия.ОграничениеВРазмереММОТ;
