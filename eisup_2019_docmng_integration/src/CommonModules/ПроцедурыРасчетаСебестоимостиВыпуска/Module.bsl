...
Функция ПолучитьХарактерЗатратПоСчетуЗатратМежд(
	...
	ИначеЕсли ТипЗнч(СчетЗатрат) <> Тип("ПланСчетовСсылка.Международный") Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Счет затрат " + СчетЗатрат + " не соответствует плану счетов: ""Международный"".
			|Функция <ПолучитьХарактерЗатратПоСчетуЗатратМежд>");
		ПравильныйСчетЗатрат = Ложь;
...
Функция СформироватьТекстЗапросаПоСписаниюНаСобственныеНужды()
	...
	|						И ((СпособыРаспределенияСтатейЗатрат.ХарактерРаспределенияЗатрат <> &НеУчитыватьПодразделение 
	|							//ДляУпрУчета И СпособыРаспределенияСтатейЗатрат.Подразделение = СписанныеТовары.Подразделение)
	|							//ДляРеглУчета И СпособыРаспределенияСтатейЗатрат.Подразделение = СписанныеТовары.ПодразделениеОрганизации)
	|							ИЛИ 
	|							СпособыРаспределенияСтатейЗатрат.ХарактерРаспределенияЗатрат = &НеУчитыватьПодразделение)
	|						
	|				ГДЕ
	|					СписанныеТовары.Период МЕЖДУ &НачДата И &КонДата
	|					//ДляУпрУчета И СписанныеТовары.ОтражатьВУправленческомУчете
	...
	|				И ((СпособыРаспределенияСтатейЗатрат.ХарактерРаспределенияЗатрат <> &НеУчитыватьПодразделение 
	|					И СпособыРаспределенияСтатейЗатрат.Подразделение = ВыпускПродукции.ПодразделениеПолучатель)
	|					ИЛИ 
	|					СпособыРаспределенияСтатейЗатрат.ХарактерРаспределенияЗатрат = &НеУчитыватьПодразделение)
	|		ГДЕ
	|			ВыпускПродукции.Период МЕЖДУ &НачДата И &КонДата
...
Процедура ОпределитьНоменклатуруИсключаемуюИзБазыРаспределения(СтруктураШапкиДокумента) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаИсключаемойНоменклатуры, ТаблицаДвижений);
	...
	Если ТаблицаДвиженийКорректировка.Количество() > 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаДвиженийКорректировка, ТаблицаДвижений);
...
Процедура РаспределитьЗатратыНаВыпускНаработка(
	...
	Пока ВыборкаПоЗатратам.Следующий() Цикл
		...
		МассивКоличество = ttk_ОбщегоНазначения.РаспределитьПропорционально(КоличествоРасход, МассивКоэф, 3);
		МассивСумма      = ttk_ОбщегоНазначения.РаспределитьПропорционально(СуммаРасход, МассивКоэф, 2);
...
Функция ПолучитьСодержаниеПроводкиПоКодуОперации(КодОперацииВыпускПродукции, СоответствиеКодОперацииСодержание) Экспорт
	...
	Если Содержание = Неопределено Тогда
		Содержание = "";
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не определено содержание проводки для кода операции: """ + КодОперацииВыпускПродукции + """.
...
Процедура СформироватьПроводкуПоСписаниюЗатратНа40Счет(
	...
	Иначе
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Списание затрат на 40 счет используется только при отражении документа в бухгалтерском или налоговом учете!
			|Процедура <СформироватьПроводкуПоСписаниюЗатратНа40Счет>");
...
Процедура СформироватьПроводкуПоВыпускуПродукции(
	...
	Иначе
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не сформирована проводка по коду операции: """ + СтрокаВыборки.КодОперации + """.
			|Процедура <СформироватьПроводкуПоВыпускуПродукции>");
...
Процедура ДосписатьПоНаправлениюВыпуска(
	...
	ИначеЕсли СтрокаВыборки.КодОперации = Перечисления.КодыОперацийВыпускПродукции.ВыпускПродукцииФиксВнутрУслугиНаПостЗатраты
		...
		Иначе
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не определен характер затрат по коду операции""" + СтрокаВыборки.КодОперации + """.
				|Процедура <ДосписатьПоНаправлениюВыпуска>");
		КонецЕсли;
		...
		Иначе
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не обработан характер затрат """ + ХарактерЗатрат + """.
			|Процедура <ДосписатьПоНаправлениюВыпуска>");
	...
	Иначе
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не обработан код операции """ + СтрокаВыборки.КодОперации + """.
			|Процедура <ДосписатьПоНаправлениюВыпуска>");
...
Процедура СформироватьДвиженияПоРегиструНезавершенноеПроизводствоРаспределениеЗатрат(
	...
	Если ВидДвижения <> ВидДвиженияНакопления.Приход
	   И ВидДвижения <> ВидДвиженияНакопления.Расход Тогда
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указан вид движения!
					|Процедура <СформироватьДвиженияПоРегиструНезавершенноеПроизводствоРаспределениеЗатрат>");
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не определен код операции для характера затрат: """ + ХарактерЗатрат + """.
					|Процедура <СформироватьДвиженияПоРегиструНезавершенноеПроизводствоРаспределениеЗатрат>");
...
Процедура СформироватьПроводкуПоРаспределениюЗатрат(
	...
	Иначе
		СодержаниеПроводки = "";
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не определено содержание операции для характера затрат: """ + ХарактерЗатрат + """.
					|Процедура <СформироватьПроводкуПоРаспределениюЗатрат>");
...
Процедура РаспределитьПрямыеПроизводственныеЗатратыПоБазеРаспределения(
	...
	Пока ВыборкаПоЗатратам.Следующий() Цикл
		...
		МассивКоличество = ttk_ОбщегоНазначения.РаспределитьПропорционально(КоличествоОстаток, МассивКоэф, 3);
		МассивСумма      = ttk_ОбщегоНазначения.РаспределитьПропорционально(СуммаОстаток, МассивКоэф, 2);
		...
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			...
			МассивПостояннаяРазница = ttk_ОбщегоНазначения.РаспределитьПропорционально(ПостояннаяРазницаОстаток, МассивКоэф, 2);
			МассивВременнаяРазница  = ttk_ОбщегоНазначения.РаспределитьПропорционально(ВременнаяРазницаОстаток, МассивКоэф, 2);
			МассивСуммаНУ           = ttk_ОбщегоНазначения.РаспределитьПропорционально(СуммаОстатокНУ, МассивКоэф, 2);
		КонецЕсли;
...
Процедура РаспределитьЗатратыНаВыпускПродукции(
	...
	Если ВсегоБазаРаспределения <> 0 Тогда
		...
		МассивСумма = ttk_ОбщегоНазначения.РаспределитьПропорционально(СуммаНаВыпуск, МассивКоэф, 2);
			
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
				
			МассивПостояннаяРазница = ttk_ОбщегоНазначения.РаспределитьПропорционально(ПостояннаяРазницаНаВыпуск, МассивКоэф, 2);
			МассивВременнаяРазница  = ttk_ОбщегоНазначения.РаспределитьПропорционально(ВременнаяРазницаНаВыпуск, МассивКоэф, 2);
			
			ОбщаяСуммаНаВыпуск = СуммаНаВыпуск + ПостояннаяРазницаНаВыпуск + ВременнаяРазницаНаВыпуск;
			МассивОбщаяСумма   = ttk_ОбщегоНазначения.РаспределитьПропорционально(ОбщаяСуммаНаВыпуск, МассивКоэф, 2);
...
Процедура РаспределитьПроизводственныеЗатратыПоБазеРаспределения(
	...
	Пока ВыборкаПоЗатратам.Следующий() Цикл
		...
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			...
			МассивПостояннаяРазница = ttk_ОбщегоНазначения.РаспределитьПропорционально(ПостояннаяРазницаОстаток, МассивКоэф, 2);
			МассивВременнаяРазница  = ttk_ОбщегоНазначения.РаспределитьПропорционально(ВременнаяРазницаОстаток, МассивКоэф, 2);
			МассивСуммаНУ           = ttk_ОбщегоНазначения.РаспределитьПропорционально(СуммаОстаток, МассивКоэфНУ, 2);
...
Процедура РаспределитьЗатратыВНезавершенноеПроизводство(
	...
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
		...
		МассивОбщаяСумма = ttk_ОбщегоНазначения.РаспределитьПропорционально(ОбщаяСумма, МассивКоэф, 2);
...
Процедура СформироватьПроводкуРаспределениеПоПродажам(
	...
	Если ХарактерЗатрат = Перечисления.ХарактерЗатрат.ИздержкиОбращения Тогда
...
Функция СформироватьТекстЗапросаПоСебестоимостиРеализацииИОстаткамТоваровРасширеннаяАналитика()
	...
	|ИЗ (
	|	ВЫБРАТЬ
	|		УчетЗатрат.СтоимостьОстаток КАК ОстатокТоваров,
	...
	|					РегистрАналитикаВидаУчета.Ссылка
	|				ИЗ
	|					АналитикаВидаУчета КАК РегистрАналитикаВидаУчета
	|				)
	|		) КАК УчетЗатрат
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	|		0 КАК ОстатокТоваров,
	|		СУММА(УчетЗатрат.Стоимость) КАК СтоимостьРеализованныхТоваров
	|	ИЗ
	...
	|		ПО
	|			УчетЗатрат.КорАналитикаВидаУчета = РегистрАналитикаУчетаПрочихЗатрат.Ссылка
	|			
	|	ГДЕ
...
Функция СформироватьТекстЗапросаПоСебестоимостиРеализацииИОстаткамТоваров()
	...
	|	ВЫБРАТЬ
	|		0 КАК ОстатокТоваров,
	|		РегистрБухгалтерии.СуммаОборот КАК СтоимостьРеализованныхТоваров
	|	ИЗ	
...
//	Запрос - Запрос
//
Процедура УстановитьПараметрыЗапросаПоСебестоимостиРеализацииИОстаткамТоваров(
	СтруктураШапкиДокумента,
	Запрос
	)
...
Функция ПолучитьКоэффициентСписанияТранспортныхРасходов(
	...
	Если СтруктураШапкиДокумента.ИспользоватьРасширеннуюАналитику Тогда
		ТекстЗапросаСКомментариями = СформироватьТекстЗапросаПоСебестоимостиРеализацииИОстаткамТоваровРасширеннаяАналитика();
	Иначе
...
Процедура РаспределитьТранспортныеРасходы(
	...
	МассивСумма = ttk_ОбщегоНазначения.РаспределитьПропорционально(СуммаОстаток, МассивКоэфСумма, 2);
		
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			
		МассивОбщаяСумма        = ttk_ОбщегоНазначения.РаспределитьПропорционально(ОбщаяСуммаОстаток, МассивКоэфОбщаяСумма, 2);
		МассивПостояннаяРазница = ttk_ОбщегоНазначения.РаспределитьПропорционально(ПостояннаяРазницаОстаток, МассивКоэфПостояннаяРазница, 2);
		МассивВременнаяРазница  = ttk_ОбщегоНазначения.РаспределитьПропорционально(ВременнаяРазницаОстаток, МассивКоэфВременнаяРазница, 2);
...
Функция ПолучитьМассивВыполняемыхДействийРасширеннаяАналитика(ПериодРегистрации, Организация, ОтражатьВУправленческомУчете)
	...
	Если Не ОтражатьВУправленческомУчете Тогда
		УчетнаяПолитикаРегл = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(КонецМесяца(ПериодРегистрации), Организация,истина);
	...
	Если Не ОтражатьВУправленческомУчете Тогда
		МассивДействий.Добавить(Перечисления.ВыполняемыеОперацииРасчетСебестоимостиВыпуска.СписаниеКосвенныхРасходовНаРБП);
	КонецЕсли;
...
Функция ПолучитьМассивВыполняемыхДействийТрадиционныйРежим(ПериодРегистрации, Организация, ОтражатьВУправленческомУчете, СообщатьОбОшибке)
	...
	Если ОтражатьВУправленческомУчете Тогда
		УчетнаяПолитика = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиУпр(ПериодРегистрации, СообщатьОбОшибке);
	Иначе
		УчетнаяПолитика = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(ПериодРегистрации, Организация, СообщатьОбОшибке);
	КонецЕсли;
