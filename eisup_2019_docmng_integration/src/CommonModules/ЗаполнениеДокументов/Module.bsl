...
Процедура ЗаполнитьШапкуДокументаПоОснованию(ДокументОбъект, ДокументОснование) Экспорт
	...
	Если МетаданныеДокумента.Реквизиты.Найти("абс_Продавец") <> Неопределено
	   И МетаданныеДокументаОснования.Реквизиты.Найти("абс_Продавец") <> Неопределено Тогда
		ДокументОбъект.абс_Продавец = ДокументОснование.абс_Продавец;
	КонецЕсли;
...
Процедура ПриИзмененииЗначенияДоговора(ДокументОбъект, СтараяВалютаВзаиморасчетов, ВалютаРегламентированногоУчета,
	...
	Иначе
		...
		// АБС ВСТАВКА Фролов 
		Если (ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПередачаТоваров") ИЛИ
			  ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.ПередачаТоваров")) И 
			 ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийПередачаТоваров.абс_ОборудованиеВПереработку Тогда 
			 
		// АБС ВСТАВКА Фролов КОНЕЦ	 
			 
		ИначеЕсли СписокТиповОбъектовПоступлениеОборудования.НайтиПоЗначению(ТипЗнч(ДокументОбъект)) <> Неопределено
		  ...
		  ИЛИ ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийПередачаТоваров.абс_ОборудованиеВПереработку
...
//  ОтражатьВНалоговомУчете         - признак формирования очереди по нал. учету,
//
// Возвращаемое значение:
//	структура очередей
//
...
Функция ПолучитьСледующийДокумент(СтруктураОчередей, ДатаОкончания, МаксимальноеКоличествоДокументовВВыборке, Организации)
	СтруктураВозврата = Новый Структура("Ссылка, Дата, ОтражатьВУправленческомУчете, ОтражатьВБухгалтерскомУчете, ОтражатьВНалоговомУчете, Организация",Неопределено,Неопределено,Ложь,Ложь,Ложь,Справочники.Организации.ПустаяСсылка());
...
//  Бух            							- признак установки ГП по бух. учету
//  Нал            							- признак установки ГП по нал. учету
//  НДС        			    				- признак установки ГП по НДС. учету
...
//  ОтражатьВНалоговомУчете   		- признак проведения по партиям нал.,
...
Процедура ВыполнитьВосстановление (ДатаКонцаОбработки,
	...
	Если НЕ (ОтражатьВУправленческомУчете
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не выбран ни один из учетов!");
		Возврат;
	...
	Пока ЕстьДокументы Цикл
		...
		# Если Сервер Тогда
			...
			СтруктураНаборовЗаписей.Вставить("ЕстьСтрокиОтражатьВНалоговомУчете"      , ОтражатьВБухгалтерскомУчете);
		...
		СтруктраУчетов.Вставить("Нал", Бух);
		...
		Если ДокументСсылка.Проведен Тогда
			...
			Иначе
				...
				Если (Бух ИЛИ Упр) Тогда
					...
					Если (ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг"))  Тогда 
						ttk_ОбщегоНазначения.РеализацияСотрудникуТТК(ДокументСсылка.ПолучитьОбъект());
					КонецЕсли;
			...
			УправлениеПроизводствомДвиженияПоРегистрам.ПровестиДокументПоПроизводственнымРегистрам(ДокументСсылка, Упр, Бух, Бух, ПроведениеОстановлено);
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |ГДЕ
				   |	ПодтверждениеНулевойСтавкиНДС.Организация В (&Организации) 
...
Процедура ПриИзмененииРозничнаяТорговляОблагаетсяЕНВД(ДокументОбъект,
	...
	Если ЗначениеЗаполнено(ПараметрыУчетнойПолитикиРегл) Тогда
		НовыйРозничнаяТорговляОблагаетсяЕНВД = ПараметрыУчетнойПолитикиРегл.РозничнаяТорговляОблагаетсяЕНВД;
	Иначе
		НовыйРозничнаяТорговляОблагаетсяЕНВД = Ложь;
	КонецЕсли;
	...
	Если НадоПересчитывать Тогда
		...
		ТекстВопроса = ТекстВопроса + " изменилось отражение операции по деятельности, облагаемой/не облагаемой ЕНВД.";
		ТекстВопроса = ТекстВопроса + Символы.ПС + "Пересчитать документ?";
		Ответ        = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура ИзменитьЦеныВалюту(ДокументОбъект, СпособЗаполненияЦен, СтруктураРеквизитовДокумента, 
	...
	Для Каждого ТЧОбъекта Из МетаданныеДокумента.ТабличныеЧасти Цикл
		...
		Если (СтруктураНеобрабатываемыхТабличныхЧастей = Неопределено
		      ...
		      ИЛИ МетаданныеТабЧастиДокумента.Реквизиты.Найти("СуммаНДС") <> Неопределено) Тогда
...
Процедура ПриПроведенииПроверитьЗаполнениеПериодаРегистрации(Источник, Отказ, РежимПроведения) Экспорт
	Если НачалоМесяца(Источник.ПериодРегистрации) <> НачалоМесяца(Источник.Дата) Тогда
		Заголовок = "";
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Период регистрации не соответствует дате документа!", Отказ, Заголовок);
	КонецЕсли;
КонецПроцедуры
