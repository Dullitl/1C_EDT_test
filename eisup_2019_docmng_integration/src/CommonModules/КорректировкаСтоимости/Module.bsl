...
Процедура РазделитьДокументыКорректировкаСтоимостиСписания(МассивДокументов) Экспорт
	...
	СписокРегистровНУ = Новый Структура();
	...
	Для Каждого ТекущийДокумент Из МассивДокументов Цикл
		...
		Если НЕ ВидУчетаДокумента = Неопределено Тогда
			Если (ВидУчетаДокумента = "Бух") ИЛИ (ВидУчетаДокумента = "Нал") ИЛИ (ВидУчетаДокумента = "Меж") Тогда 
		...
		Для Каждого Движение ИЗ МетаданнныеДокумента.Движения Цикл
			...
			ИначеЕсли ОбщегоНазначения.РегистрПринадлежитВидамУчета(Движение.Имя, Новый Структура("ОтражатьВНалоговомУчете",Истина)) Тогда
				СписокРегистровНУ.Вставить(Движение.Имя, ВидРегистра);
		...
		СформироватьДвиженияПоВидуУчета(ТекущийДокумент, "Нал", СписокРегистровНУ, ТаблицаДокументовДвижения, Истина);
		...
		Если ТаблицаДокументовДвижения.Количество() > 0 Тогда
			...
			ИначеЕсли ТаблицаДокументовДвижения[0].ВидУчета = "Нал" Тогда
				ДокументОбъект.ОтражатьВУправленческомУчете = Ложь;
				ДокументОбъект.ОтражатьВБухгалтерскомУчете = Ложь;
			...
			Для Каждого СозданныйДокумент Из ТаблицаДокументовДвижения Цикл
				...
				ИначеЕсли СозданныйДокумент.Документ.ОтражатьВБухгалтерскомУчете ИЛИ СозданныйДокумент.Документ.ОтражатьВНалоговомУчете Тогда
...
Функция ПолучитьВидУчетаДокумента(ДокументСсылка)
	...
	Если ДокументСсылка.ОтражатьВНалоговомУчете Тогда
		КоличествоВидовУчета = КоличествоВидовУчета + 1;
		ВидУчетаДокумента = "Нал";
	КонецЕсли;	
	...
	Иначе
		...
		Иначе
			...
			ИначеЕсли ВидУчета = "Нал" Тогда
				НовыйДокументОбъект.ОтражатьВНалоговомУчете = Истина;	
