...
Функция РасчетАмортизацииБухРегл(ДатаРасчета, Организация, ОсновноеСредство = Неопределено, ВыдаватьСообщения = Истина) Экспорт
	...
	Запрос.УстановитьПараметр("НачалоГода",	      Новый Граница(НачалоГода(ДатаРасчета), ВидГраницы.Включая));
	Запрос.УстановитьПараметр("НачалоПериода",    Новый Граница(НачалоМесяца(ДатаРасчета), ВидГраницы.Включая));
	Запрос.УстановитьПараметр("КонецПериода",     Новый Граница(КонецМесяца(ДатаРасчета), ВидГраницы.Исключая));
	Запрос.УстановитьПараметр("Период",           ДатаРасчета); //это конец месяца
	...
	|	ЛОЖЬ КАК ТекМесяцДляОС
	|ПОМЕСТИТЬ СписокОС_ДоГруппировки
	|ИЗ
	...
	|						ИЗ
	|							СнятыеСУчета))) КАК НачислениеАмортизации
	|ГДЕ
	...
	|	НачислениеАмортизации.НачислятьАмортизациюВТекущемМесяце
	|ИЗ
	|	РегистрСведений.НачислениеАмортизацииОСБухгалтерскийУчет.СрезПоследних(
	|			,
	|			Организация = &Организация
	...
	|ГДЕ
	|	НачислениеАмортизации.НачислятьАмортизациюВТекущемМесяце
	|	И НАЧАЛОПЕРИОДА(НачислениеАмортизации.Период, МЕСЯЦ) = НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство
	|;
	...
	|	МИНИМУМ(СписокОС.ТекМесяцДляОС) КАК ТекМесяцДляОС
	|ПОМЕСТИТЬ СписокОС
	|ИЗ
	|	СписокОС_ДоГруппировки КАК СписокОС
	|
	|СГРУППИРОВАТЬ ПО
	|	СписокОС.ОсновноеСредство
	|;
	...
	|	ЕСТЬNULL(ПервоначальныеСведения.ПервоначальнаяСтоимость, 0) КАК ПервоначальнаяСтоимость,
	...
	|					ГДЕ
	|						(НЕ СписокОС.ТекМесяцДляОС))) КАК ПервоначальныеСведения
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ЕСТЬNULL(ПервоначальныеСведения.ПервоначальнаяСтоимость, 0),
	|	ПервоначальныеСведения.ПараметрВыработки
	|ИЗ
	|	РегистрСведений.ПервоначальныеСведенияОСБухгалтерскийУчет.СрезПоследних(
	|			,
	|			Организация = &Организация
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						СписокОС.ТекМесяцДляОС)) КАК ПервоначальныеСведения
	|ГДЕ
	|	НАЧАЛОПЕРИОДА(ПервоначальныеСведения.Период, МЕСЯЦ) = НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|						ПервоначальныеСведения.ПараметрВыработки
	|					ИЗ
	|						ПервоначальныеСведенияОбОсновныхСредствахОрганизацийСрезПоследних КАК ПервоначальныеСведения)
	|				И ОсновноеСредство В
	|					(ВЫБРАТЬ
	|						СписокОС.ОсновноеСредство
	|					ИЗ
	|						СписокОС)) КАК ВыработкаОСОбороты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ЕСТЬNULL(ПараметрыАмортизации.КоэффициентАмортизации, 0) КАК КоэффициентАмортизации,
	|	ЕСТЬNULL(ПараметрыАмортизации.КоэффициентУскорения, 0) КАК КоэффициентУскорения,
	|	ПараметрыАмортизации.Период КАК Период
	|ПОМЕСТИТЬ ПараметрыАмортизацииБухгалтерскийУчетСрезПоследних
	|ИЗ
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						(НЕ СписокОС.ТекМесяцДляОС))) КАК ПараметрыАмортизации
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ПараметрыАмортизации.ОсновноеСредство,
	|	ЕСТЬNULL(ПараметрыАмортизации.СрокИспользованияДляВычисленияАмортизации, 0),
	|	ЕСТЬNULL(ПараметрыАмортизации.СтоимостьДляВычисленияАмортизации, 0),
	|	ЕСТЬNULL(ПараметрыАмортизации.ОбъемПродукцииРаботДляВычисленияАмортизации, 0),
	|	ЕСТЬNULL(ПараметрыАмортизации.КоэффициентАмортизации, 0),
	|	ЕСТЬNULL(ПараметрыАмортизации.КоэффициентУскорения, 0),
	|	ПараметрыАмортизации.Период
	|ИЗ
	|	РегистрСведений.ПараметрыАмортизацииОСБухгалтерскийУчет.СрезПоследних(
	|			,
	|			Организация = &Организация
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						СписокОС.ТекМесяцДляОС)) КАК ПараметрыАмортизации
	|ГДЕ
	|	НАЧАЛОПЕРИОДА(ПараметрыАмортизации.Период, МЕСЯЦ) = НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|	И ПараметрыАмортизации.ПрименитьВТекущемМесяце
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						(НЕ СписокОС.ТекМесяцДляОС))) КАК ГрафикиАмортизации
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ГрафикиАмортизации.ОсновноеСредство,
	|	ГрафикиАмортизации.ГрафикАмортизации
	|ИЗ
	|	РегистрСведений.ГрафикиАмортизацииОСБухгалтерскийУчет.СрезПоследних(
	|			,
	|			Организация = &Организация
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						СписокОС.ТекМесяцДляОС)) КАК ГрафикиАмортизации
	|ГДЕ
	|	НАЧАЛОПЕРИОДА(ГрафикиАмортизации.Период, МЕСЯЦ) = НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ СпособыОтраженияРасходовПоАмортизацииСрезПоследних
	|ИЗ
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						(НЕ СписокОС.ТекМесяцДляОС))) КАК СпособыОтраженияРасходов
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	СпособыОтраженияРасходов.ОсновноеСредство,
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						СписокОС.ТекМесяцДляОС)) КАК СпособыОтраженияРасходов
	|ГДЕ
	|	НАЧАЛОПЕРИОДА(СпособыОтраженияРасходов.Период, МЕСЯЦ) = НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	АмортизацияБухгалтерский.ОсновноеСредство КАК ОсновноеСредство,
	|	ЕСТЬNULL(АмортизацияБухгалтерский.СтоимостьНачальныйОстаток, 0) КАК СтоимостьНачальныйОстаток,
	|	ЕСТЬNULL(АмортизацияБухгалтерский.АмортизацияНачальныйОстаток, 0) КАК АмортизацияНачальныйОстаток,
	|	ЕСТЬNULL(АмортизацияБухгалтерский.АмортизацияКонечныйОстаток, 0) КАК АмортизацияКонечныйОстаток,
	|	ЕСТЬNULL(АмортизацияБухгалтерский.АмортизацияОборот, 0) КАК АмортизацияОборот,
	|	ЕСТЬNULL(АмортизацияБухгалтерский.СтоимостьКонечныйОстаток, 0) КАК БалансоваяСтоимость
	|ПОМЕСТИТЬ АмортизацияБухгалтерскийУчетОстаткиИОбороты
	|ИЗ
	|	РегистрНакопления.СтоимостьОСБухгалтерскийУчет.ОстаткиИОбороты(
	|			&НачалоПериода,
	|			&КонецПериода,
	|			,
	|			,
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						(НЕ СписокОС.ТекМесяцДляОС))) КАК АмортизацияБухгалтерский
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	АмортизацияБухгалтерский.ОсновноеСредство,
	|	ЕСТЬNULL(АмортизацияБухгалтерский.СтоимостьОборот, 0),
	|	ЕСТЬNULL(АмортизацияБухгалтерский.АмортизацияОборот, 0),
	|	ЕСТЬNULL(АмортизацияБухгалтерский.АмортизацияКонечныйОстаток, 0),
	|	ЕСТЬNULL(АмортизацияБухгалтерский.АмортизацияОборот, 0),
	|	ЕСТЬNULL(АмортизацияБухгалтерский.СтоимостьКонечныйОстаток, 0)
	|ИЗ
	|	РегистрНакопления.СтоимостьОСБухгалтерскийУчет.ОстаткиИОбороты(
	|			&НачалоПериода,
	|			&КонецПериода,
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						СписокОС.ТекМесяцДляОС)) КАК АмортизацияБухгалтерский
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|						СписокОС.ОсновноеСредство
	|					ИЗ
	|						СписокОС)) КАК СчетаБухгалтерскогоУчетаОсновныхСредствСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|						СписокОС.ОсновноеСредство
	|					ИЗ
	|						СписокОС)) КАК АмортизацияНаНачалоГода
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ ОсновныеСредстваСписанныеНаЗатратыОрганизацииОстатки
	|ИЗ
	|	РегистрНакопления.ОССписанныеНаЗатратыОрганизаций.Остатки(
	|			&КонецПериода,
	|			Организация = &Организация
	|				И ОсновноеСредство В
	|					(ВЫБРАТЬ
	|						СписокОС.ОсновноеСредство
	|					ИЗ
	|						СписокОС)) КАК ОсновныеСредстваСписанныеНаЗатратыОрганизацииОстатки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СписокОС.ОсновноеСредство
	|ПОМЕСТИТЬ СписокОС_Итог
	|ИЗ
	|	СписокОС КАК СписокОС
	|
	|СГРУППИРОВАТЬ ПО
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПараметрыАмортизацииБухгалтерскийУчетСрезПоследних КАК ПараметрыАмортизацииБухгалтерскийУчетСрезПоследних
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ СпособыОтраженияРасходовПоАмортизацииСрезПоследних КАК СпособыОтраженияРасходовПоАмортизацииСрезПоследних
	|		ПО СписокОС.ОсновноеСредство = СпособыОтраженияРасходовПоАмортизацииСрезПоследних.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ АмортизацияБухгалтерскийУчетОстаткиИОбороты КАК АмортизацияБухгалтерскийУчетОстаткиИОбороты
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ОсновныеСредстваСписанныеНаЗатратыОрганизацииОстатки КАК ОсновныеСредстваСписанныеНаЗатратыОрганизацииОстатки
	|		ПО СписокОС.ОсновноеСредство = ОсновныеСредстваСписанныеНаЗатратыОрганизацииОстатки.ОсновноеСредство";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст,"И &ВременныйПараметр",УсловиеПоОС);
	...
	Пока ВыборкаПоОС.Следующий() Цикл
		...
		Если НЕ ВыборкаПоОС.АмортизацияОборот = 0 Тогда
			Если ВыдаватьСообщения = Истина и не (ОсновноеСредство = Неопределено) Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + " амортизация в этом месяце уже начислялась");
			КонецЕсли; 
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаПоОС.СпособНачисленияАмортизации) Тогда
			Если ВыдаватьСообщения = Истина Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + " не указан способ амортизации");
			КонецЕсли; 
			Продолжить;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаПоОС.СтоимостьДляВычисленияАмортизации) Тогда
			Если ВыдаватьСообщения = Истина Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + " стоимость равна 0");
			КонецЕсли; 
			Продолжить;
		КонецЕсли;
		...
		Если (ВыборкаПоОС.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.Линейный 
		 ...
		 ИЛИ ВыборкаПоОС.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.УменьшаемогоОстатка)
		   И НЕ ЗначениеЗаполнено(ВыборкаПоОС.СрокИспользованияДляВычисленияАмортизации) Тогда
			Если ВыдаватьСообщения = Истина Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + " срок использования равен 0");
			КонецЕсли; 
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли ВыборкаПоОС.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.УменьшаемогоОстатка Тогда
			...
			Если НЕ ЗначениеЗаполнено(ВыборкаПоОС.КоэффициентУскорения) Тогда
				...
				Если ВыдаватьСообщения = Истина Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + " коэффициент ускорения не заполнен");
				КонецЕсли;
			КонецЕсли;
		...
		ИначеЕсли ВыборкаПоОС.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.ПоСуммеЧиселЛетИспользования Тогда
			...
			Если ВыборкаПоОС.Период = NULL Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + " не определилась дата начала использования (изменения параметров амортизации)!");
				Продолжить;
			КонецЕсли; 
		...
		Если КоэффициентПериода = 0 Тогда
			Если ВыдаватьСообщения = Истина Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + " коэффициент амортизации равен 0");
			КонецЕсли;
		КонецЕсли;
		...
		Если СуммаАмортизации = 0 Тогда
			Если ВыдаватьСообщения = Истина Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + " амортизация равна 0");
			КонецЕсли;
		КонецЕсли;
...
Функция РасчетАмортизацииНалогРегл(Объект,ДатаРасчета,Организация,ОсновноеСредство = Неопределено, ВыдаватьСообщения = Истина) Экспорт
	...
	ТаблицаАмортизации.Колонки.Добавить("СтоимостьДляВычисления",    				  Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));	
	// АБС ВСТАВКА Фролов 9721 КОНЕЦ

	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("Организация",                  Организация);
	Запрос.УстановитьПараметр("НачалоПериодаУчетнойПолитики", КонецМесяца(ДатаРасчета));
	Запрос.УстановитьПараметр("НачалоПериода",                Новый Граница(НачалоМесяца(ДатаРасчета), ВидГраницы.Включая));
	Запрос.УстановитьПараметр("КонецПериода",                 Новый Граница(КонецМесяца(ДатаРасчета), ВидГраницы.Исключая));
	...
	"ВЫБРАТЬ
	|	НачислениеАмортизации.ОсновноеСредство КАК ОсновноеСредство,
	|	НачислениеАмортизации.ОсновноеСредство.ГруппаОС КАК ГруппаОС,
	|	НачислениеАмортизации.ОсновноеСредство.АмортизационнаяГруппа КАК АмортизационнаяГруппа,
	|	ЛОЖЬ КАК ТекМесяцДляОС
	|ПОМЕСТИТЬ СписокОС_ДоГруппировки
	|ИЗ
	...
	|			&НачалоПериода,
	|			Организация = &Организация
	|				И &ВременныйПараметр) КАК НачислениеАмортизации
	|ГДЕ
	|	НачислениеАмортизации.НачислятьАмортизацию
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|ВЫБРАТЬ
	|	НачислениеАмортизации.ОсновноеСредство,
	|	НачислениеАмортизации.ОсновноеСредство.ГруппаОС,
	|	НачислениеАмортизации.ОсновноеСредство.АмортизационнаяГруппа,
	|	НачислениеАмортизации.НачислятьАмортизациюВТекущемМесяце
	|ИЗ
	|	РегистрСведений.НачислениеАмортизацииОСНалоговыйУчет.СрезПоследних(
	|			,
	...
	|	ЕСТЬNULL(ПараметрыАмортизации.СтоимостьДляВычисленияАмортизации, 0) КАК СтоимостьДляВычисленияАмортизации,
	|	ЕСТЬNULL(ПараметрыАмортизации.СрокИспользованияДляВычисленияАмортизации, 0) КАК СрокИспользованияДляВычисленияАмортизации
	|ПОМЕСТИТЬ ПараметрыАмортизацииНалоговыйУчетСрезПоследних
	|ИЗ
	...
	|ВЫБРАТЬ
	|	ПараметрыАмортизации.ОсновноеСредство,
	|	ЕСТЬNULL(ПараметрыАмортизации.СрокПолезногоИспользования, 0),
	|	ЕСТЬNULL(ПараметрыАмортизации.СтоимостьДляВычисленияАмортизации, 0),
	|	ЕСТЬNULL(ПараметрыАмортизации.СрокИспользованияДляВычисленияАмортизации, 0)
	|ИЗ
	|	РегистрСведений.ПараметрыАмортизацииОСНалоговыйУчет.СрезПоследних(
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПараметрыАмортизацииОСНалоговыйУчет.ОсновноеСредство КАК ОсновноеСредство,
	|	ИСТИНА КАК ЧастичнаяПродажа
	|ПОМЕСТИТЬ ПараметрыАмортизацииНалоговыйУчетЧастичнаяПродажа
	|ИЗ
	|	РегистрСведений.ПараметрыАмортизацииОСНалоговыйУчет КАК ПараметрыАмортизацииОСНалоговыйУчет
	|ГДЕ
	|	ПараметрыАмортизацииОСНалоговыйУчет.Период МЕЖДУ НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ) И КОНЕЦПЕРИОДА(&Период, МЕСЯЦ)
	|	И ПараметрыАмортизацииОСНалоговыйУчет.Организация = &Организация
	|	И ПараметрыАмортизацииОСНалоговыйУчет.ОсновноеСредство В
	|			(ВЫБРАТЬ
	|				СписокОС.ОсновноеСредство
	|			ИЗ
	|				СписокОС)
	|	И ПараметрыАмортизацииОСНалоговыйУчет.Регистратор.абс_ЧастичнаяПродажа
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	АмортизацияНалоговый.ОсновноеСредство КАК ОсновноеСредство,
	|	ЕСТЬNULL(АмортизацияНалоговый.АмортизацияПриход, 0) - ЕСТЬNULL(АмортизацияНалоговый.АмортизацияРасход, 0) КАК АмортизацияВМесяце,
	|	ЕСТЬNULL(АмортизацияНалоговый.СтоимостьНачальныйОстаток, 0) КАК СтоимостьНаНачалоМесяца,
	|	ЕСТЬNULL(АмортизацияНалоговый.СтоимостьКонечныйОстаток, 0) КАК СтоимостьНаКонецМесяца,
	|	ЕСТЬNULL(АмортизацияНалоговый.АмортизацияНачальныйОстаток, 0) КАК АмортизацияНачальныйОстаток,
	|	ЕСТЬNULL(АмортизацияНалоговый.АмортизацияКонечныйОстаток, 0) КАК АмортизацияКонечныйОстаток
	|ПОМЕСТИТЬ АмортизацияНалоговыйУчетОстаткиИОбороты
	|ИЗ
	|	РегистрНакопления.СтоимостьОСНалоговыйУчет.ОстаткиИОбороты(
	|			&НачалоПериода,
	|			&КонецПериода,
	|			,
	|			,
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						(НЕ СписокОС.ТекМесяцДляОС))) КАК АмортизацияНалоговый
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	АмортизацияНалоговый.ОсновноеСредство,
	|	ЕСТЬNULL(АмортизацияНалоговый.АмортизацияПриход, 0) - ЕСТЬNULL(АмортизацияНалоговый.АмортизацияРасход, 0),
	|	ЕСТЬNULL(АмортизацияНалоговый.СтоимостьОборот, 0),
	|	ЕСТЬNULL(АмортизацияНалоговый.СтоимостьКонечныйОстаток, 0),
	|	ЕСТЬNULL(АмортизацияНалоговый.АмортизацияОборот, 0),
	|	ЕСТЬNULL(АмортизацияНалоговый.АмортизацияКонечныйОстаток, 0)
	|ИЗ
	|	РегистрНакопления.СтоимостьОСНалоговыйУчет.ОстаткиИОбороты(
	|			&НачалоПериода,
	|			&КонецПериода,
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						СписокОС.ТекМесяцДляОС)) КАК АмортизацияНалоговый
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СчетаНУ.ОсновноеСредство КАК ОсновноеСредство,
	|	СчетаНУ.СчетНачисленияАмортизации КАК СчетНачисленияАмортизации,
	|	СчетаНУ.СчетУчета КАК СчетУчета
	|ПОМЕСТИТЬ СчетаНалоговогоУчетаОсновныхСредствСрезПоследних
	|ИЗ
	...
	|						СписокОС.ОсновноеСредство
	|					ИЗ
	|						СписокОС)) КАК СчетаНУ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Коэффициент.ОсновноеСредство КАК ОсновноеСредство,
	|	ЕСТЬNULL(Коэффициент.СпециальныйКоэффициент, 0) КАК СпециальныйКоэффициент
	|ПОМЕСТИТЬ СпециальныйКоэффициентДляАмортизацииНалоговыйУчетСрезПоследних
	|ИЗ
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						(НЕ СписокОС.ТекМесяцДляОС))) КАК Коэффициент
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	Коэффициент.ОсновноеСредство,
	|	ЕСТЬNULL(Коэффициент.СпециальныйКоэффициент, 0)
	|ИЗ
	|	РегистрСведений.НачислениеАмортизацииОССпециальныйКоэффициентНалоговыйУчет.СрезПоследних(
	|			,
	|			Организация = &Организация
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						СписокОС.ТекМесяцДляОС)) КАК Коэффициент
	|ГДЕ
	|	НАЧАЛОПЕРИОДА(Коэффициент.Период, МЕСЯЦ) = НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НачислениеАмортизации.ОсновноеСредство КАК ОсновноеСредство,
	|	НачислениеАмортизации.ПризнакНачисленияПоБазовойСтоимости КАК ПризнакНачисленияПоБазовойСтоимости,
	|	ЕСТЬNULL(НачислениеАмортизации.СуммаНакопленнойАмортизации, 0) КАК СуммаНакопленнойАмортизации,
	|	ЕСТЬNULL(НачислениеАмортизации.ФактическийСрокполезногоИспользования, 0) КАК ФактическийСрокполезногоИспользования
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						(НЕ СписокОС.ТекМесяцДляОС))) КАК НачислениеАмортизации
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	НачислениеАмортизации.ОсновноеСредство,
	|	НачислениеАмортизации.ПризнакНачисленияПоБазовойСтоимости,
	|	ЕСТЬNULL(НачислениеАмортизации.СуммаНакопленнойАмортизации, 0),
	|	ЕСТЬNULL(НачислениеАмортизации.ФактическийСрокполезногоИспользования, 0)
	|ИЗ
	|	РегистрСведений.НачислениеАмортизацииОСПоБазовойСтоимостиНалоговыйУчет.СрезПоследних(
	|			,
	|			Организация = &Организация
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						СписокОС.ТекМесяцДляОС)) КАК НачислениеАмортизации
	|ГДЕ
	|	НАЧАЛОПЕРИОДА(НачислениеАмортизации.Период, МЕСЯЦ) = НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						(НЕ СписокОС.ТекМесяцДляОС))) КАК НаправленияАмортизации
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	НаправленияАмортизации.ОсновноеСредство,
	|	НаправленияАмортизации.СпособыОтраженияРасходовПоАмортизации
	|ИЗ
	|	РегистрСведений.СпособыОтраженияРасходовПоАмортизацииОСНалоговыйУчет.СрезПоследних(
	|			,
	|			Организация = &Организация
	...
	|						СписокОС КАК СписокОС
	|					ГДЕ
	|						СписокОС.ТекМесяцДляОС)) КАК НаправленияАмортизации
	|ГДЕ
	|	НАЧАЛОПЕРИОДА(НаправленияАмортизации.Период, МЕСЯЦ) = НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СписокОС.ОсновноеСредство КАК ОсновноеСредство
	|ПОМЕСТИТЬ СписокОССОстаточнойСтоимостью
	|ИЗ
	|	СписокОС КАК СписокОС
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПервоначальныеСведенияОбОсновныхСредствахОрганизацийСрезПоследних КАК ПервоначальныеСведенияОбОсновныхСредствахОрганизацийСрезПоследних
	|		ПО СписокОС.ОсновноеСредство = ПервоначальныеСведенияОбОсновныхСредствахОрганизацийСрезПоследних.ОсновноеСредство
	|ГДЕ
	|	(НЕ СписокОС.ОсновноеСредство В
	|				(ВЫБРАТЬ
	|					СписокОС.ОсновноеСредство
	|				ИЗ
	|					СписокОС
	|				ГДЕ
	|					(СписокОС.АмортизационнаяГруппа В (ЗНАЧЕНИЕ(Перечисление.АмортизационныеГруппы.ВосьмаяГруппа), ЗНАЧЕНИЕ(Перечисление.АмортизационныеГруппы.ДевятаяГруппа), ЗНАЧЕНИЕ(Перечисление.АмортизационныеГруппы.ДесятаяГруппа))
	|							И СписокОС.ГруппаОС В (ЗНАЧЕНИЕ(Перечисление.ГруппыОС.Здания), ЗНАЧЕНИЕ(Перечисление.ГруппыОС.Сооружения))
	...
	|				ТОГДА ПервоначальныеСведенияОбОсновныхСредствахОрганизацийСрезПоследних.МетодНачисленияАмортизации = ЗНАЧЕНИЕ(Перечисление.МетодыНачисленияАмортизации.Нелинейный)
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВЫРАЗИТЬ(АмортизацияОСНачалоПериода.Субконто1 КАК Справочник.ОсновныеСредства) КАК ОсновноеСредство,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СписокОССОстаточнойСтоимостью КАК СписокОССОстаточнойСтоимостью
	...
	|	ЕСТЬNULL(ПараметрыАмортизацииНалоговыйУчетСрезПоследних.СтоимостьДляВычисленияАмортизации, 0) КАК СтоимостьДляВычисленияАмортизации,
	|	ЕСТЬNULL(ПараметрыАмортизацииНалоговыйУчетСрезПоследних.СрокИспользованияДляВычисленияАмортизации, 0) КАК СрокИспользованияДляВычисленияАмортизации,
	...
	|					(ВЫБРАТЬ
	|						СписокОССОстаточнойСтоимостью.ОсновноеСредство
	|					ИЗ
	...
	|	ВЫБОР
	|		КОГДА ГОД(&Период) < 2009
	|			ТОГДА ПервоначальныеСведенияОбОсновныхСредствахОрганизацийСрезПоследних.МетодНачисленияАмортизации
	|		ИНАЧЕ ВЫБОР
	|				КОГДА СписокОС.АмортизационнаяГруппа В (ЗНАЧЕНИЕ(Перечисление.АмортизационныеГруппы.ВосьмаяГруппа), ЗНАЧЕНИЕ(Перечисление.АмортизационныеГруппы.ДевятаяГруппа), ЗНАЧЕНИЕ(Перечисление.АмортизационныеГруппы.ДесятаяГруппа))
	|							И СписокОС.ГруппаОС В (ЗНАЧЕНИЕ(Перечисление.ГруппыОС.Здания), ЗНАЧЕНИЕ(Перечисление.ГруппыОС.Сооружения))
	|						ИЛИ СписокОС.АмортизационнаяГруппа = ЗНАЧЕНИЕ(Перечисление.АмортизационныеГруппы.ОтдельнаяГруппа)
	...
	|	ЕСТЬNULL(ПараметрыАмортизацииНалоговыйУчетЧастичнаяПродажа.ЧастичнаяПродажа, ЛОЖЬ) КАК ЧастичнаяПродажа,
	...
	|		ПО СписокОС.ОсновноеСредство = ПервоначальныеСведенияОбОсновныхСредствахОрганизацийСрезПоследних.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПараметрыАмортизацииНалоговыйУчетСрезПоследних КАК ПараметрыАмортизацииНалоговыйУчетСрезПоследних
	|		ПО СписокОС.ОсновноеСредство = ПараметрыАмортизацииНалоговыйУчетСрезПоследних.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ АмортизацияНалоговыйУчетОстаткиИОбороты КАК АмортизацияНалоговыйУчетОстаткиИОбороты
	|		ПО СписокОС.ОсновноеСредство = АмортизацияНалоговыйУчетОстаткиИОбороты.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ СчетаНалоговогоУчетаОсновныхСредствСрезПоследних КАК СчетаНалоговогоУчетаОсновныхСредствСрезПоследних
	|		ПО СписокОС.ОсновноеСредство = СчетаНалоговогоУчетаОсновныхСредствСрезПоследних.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ СпециальныйКоэффициентДляАмортизацииНалоговыйУчетСрезПоследних КАК СпециальныйКоэффициентДляАмортизацииНалоговыйУчетСрезПоследних
	|		ПО СписокОС.ОсновноеСредство = СпециальныйКоэффициентДляАмортизацииНалоговыйУчетСрезПоследних.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ НачислениеАмортизацииПоБазовойСтоимостиНалоговыйУчетСрезПоследних КАК НачислениеАмортизацииПоБазовойСтоимостиНалоговыйУчетСрезПоследних
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПараметрыАмортизацииНалоговыйУчетЧастичнаяПродажа КАК ПараметрыАмортизацииНалоговыйУчетЧастичнаяПродажа
	|		ПО СписокОС.ОсновноеСредство = ПараметрыАмортизацииНалоговыйУчетЧастичнаяПродажа.ОсновноеСредство";
	...
	Пока ВыборкаПоОС.Следующий() Цикл
		...
		//АБС меняем <>0 на >0, т.к. если производилось частичное списание, то амортизацию надо начислить
		//Если НЕ ВыборкаПоОС.АмортизацияВМесяце = 0 Тогда
		//	Продолжить; // по указанному ОС в этом месяце уже начисляли амортизацию 
		//КонецЕсли;
		
		//АБС ВСТАВКА №42310 НАЧАЛО «21 мая 2014 г.», Пополитов
		//Если (ВыборкаПоОС.АмортизацияВМесяце > 0) И
		//	(ВыборкаПоОС.АмортизацияВМесяце <> Null)Тогда
		//	Продолжить; // по указанному ОС в этом месяце уже начисляли амортизацию 
		//КонецЕсли;
		////АБС -  		
		
		Если ?(ВыборкаПоОС.АмортизацияВМесяце = Null, 0, ВыборкаПоОС.АмортизацияВМесяце) <> 0 Тогда
			Продолжить; // по указанному ОС в этом месяце уже начисляли амортизацию 
		КонецЕсли;
        ...
        //АБС Коломиец 16760+
		Если ВыборкаПоОС.ЧастичнаяПродажа Тогда
			Продолжить; // по указанному ОС в этом месяце уже начисляли амортизацию 
		КонецЕсли;
		//АБС Коломиец 16760-
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаПоОС.МетодНачисленияАмортизации) Тогда
			Если ВыдаватьСообщения = Истина Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + " не указан метод амортизации");
			КонецЕсли;
			Продолжить;
		КонецЕсли;
		 ...
		 ИЛИ ВыборкаПоОС.МетодНачисленияАмортизации = Перечисления.МетодыНачисленияАмортизации.Линейный Тогда
			...
			Если НЕ ЗначениеЗаполнено(ВыборкаПоОС.СрокПолезногоИспользования) Тогда
				Если ВыдаватьСообщения Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + " не указан срок полезного использования");
				КонецЕсли; 
				Продолжить;
			КонецЕсли;
		...
		Иначе
			...
			Если ВыборкаПоОС.МетодНачисленияАмортизации = Перечисления.МетодыНачисленияАмортизации.Нелинейный
			   И НЕ ЗначениеЗаполнено(ВыборкаПоОС.АмортизационнаяГруппа) Тогда
				Если ВыдаватьСообщения Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + "не указана амортизационная группа");
				КонецЕсли; 
				Продолжить;
			КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаПоОС.СчетНачисленияАмортизации) Тогда
			Если ВыдаватьСообщения Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + " не указан счет для начисления амортизации");
			КонецЕсли; 
			Продолжить;
		КонецЕсли;
		...
		//АБС
		СтоимостьДляВычисления = ?(ВыборкаПоОС.СтоимостьДляВычисленияАмортизации>0,ВыборкаПоОС.СтоимостьДляВычисленияАмортизации, СтоимостьДляВычисления);
		СрокИспользованияДляВычисления  = ?(ВыборкаПоОС.СрокИспользованияДляВычисленияАмортизации>0,ВыборкаПоОС.СрокИспользованияДляВычисленияАмортизации, СрокИспользованияДляВычисления);
		//\\АБС	
		...
		// АБС ВСТАВКА Фролов 9721
		СтрокаАмортизации.СтоимостьДляВычисления					 = СтоимостьДляВычисления;
		// АБС ВСТАВКА Фролов 9721 КОНЕЦ		
...
// конец параметры зависящие от "применить в текущем месяце"
	...
	Пока ВыборкаПоОС.Следующий() Цикл
		...
		Если ПервоначальнаяСтоимость = NULL Тогда
			Если ВыдаватьСообщения = Истина Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + " первоначальная стоимость равна 0");
			КонецЕсли;
...
Функция ПолучитьРаспределениеАмортизацииПоНаправлениямУпр(ДокОбъект, Отказ, Заголовок, ТабАмортизации, СтруктураШапкиДокумента) Экспорт
	...
	Для Каждого СтрокаТЗ Из ТабАмортизации Цикл
		...
		МассивСумм = ttk_ОбщегоНазначения.РаспределитьПропорционально( СтрокаТЗ.Сумма, МассивКоэф, 2);
...
Функция ПолучитьРаспределениеАмортизацииПоНаправлениямРегл(ДокОбъект, Отказ, Заголовок, ТабАмортизации, СтруктураШапкиДокумента, ТипУчета, НомерЖурнала, ТекстПроводки = Неопределено) Экспорт
	...
	Для Каждого СтрокаТЗ Из ТабАмортизации Цикл
		...
		Если ТипУчета = "Нал"  И СтруктураШапкиДокумента.ПоддержкаПБУ18 Тогда
			...
			МассивСуммПР = ttk_ОбщегоНазначения.РаспределитьПропорционально( СтрокаТЗ.СуммаПР, МассивКоэф, 2);
			МассивСуммБУ = ttk_ОбщегоНазначения.РаспределитьПропорционально( СтрокаТЗ.СуммаБУ, МассивКоэф, 2);
		КонецЕсли;
		...
		Для Каждого СтрокаНапр Из СтрокаТЗ.НаправлениеАмортизации.Способы Цикл
				...
				//АБС ВСТАВКА Навценя 27.02.2015
				НоваяСтрока.ПодразделениеОрганизации = абс_ВнеоборотныеАктивыВызовСервера.ПолучитьПодразделениеОрганизации(
																				НоваяСтрока.Подразделение, 
																				НоваяСтрока.ПодразделениеОрганизации, 
																				СтрокаТЗ.НаправлениеАмортизации, 
...
Процедура ПроверитьДубли(Таблица, ПредставлениеТаблицы, ИмяКолонки, ПредставлениеКолонки, Отказ, Заголовок) Экспорт
	...
	Если ?(ТаблицаДублей.Количество() > 0, ТаблицаДублей.Итог("КоличествоПовторений") / ТаблицаДублей.Количество(), 0) > 1 Тогда
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщенияОбОшибке, Отказ, Заголовок);
...
Функция РасчетАмортизацииНМА_НИОКР_БухРегл(ДатаРасчета, Организация, НематериальныйАктив = Неопределено, ВыдаватьСообщения = Истина) Экспорт
	...
	Пока ВыборкаПоНМА.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаПоНМА.СтоимостьНачальныйОстаток) Тогда
			Если ВыдаватьСообщения = Истина Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + " текущая балансовая стоимость равна 0");
			КонецЕсли; 
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли ВыборкаПоНМА.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииНМА.ПропорциональноОбъемуПродукции Тогда
			...
			Если НЕ ЗначениеЗаполнено(ВыборкаПоНМА.ОбъемПродукцииРаботДляВычисленияАмортизации) Тогда
				Если ВыдаватьСообщения = Истина Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + " объем продукции для вычисления не определен");
				КонецЕсли; 
				Продолжить;
			КонецЕсли;
...
Функция РасчетАмортизацииНМА_НИОКР_НалогРегл(Объект,ДатаРасчета,Организация,НематериальныйАктив = Неопределено, ВыдаватьСообщения = Истина) Экспорт
	...
	Для каждого ВыборкаПоНМА из ТаблицаПоНМА Цикл
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаПоНМА.МетодНачисленияАмортизации) Тогда
			...
			Если ВыдаватьСообщения = Истина Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + " не указан метод начисления амортизации");
			КонецЕсли;
					...
					Если (ВыборкаПоНМА.СтоимостьНаКонецМесяца-ВыборкаПоНМА.АмортизацияКонечныйОстаток-СуммаАмортизации)<= (ВыборкаПоНМА.СтоимостьНаКонецМесяца*0.2) тогда
						...
						Если Объект.Движения.Найти("НачислениеАмортизацииНМАПоБазовойСтоимостиНалоговыйУчет")=Неопределено тогда
							...
							Если НематериальныйАктив =Неопределено и ВыдаватьСообщения = Истина тогда
								ttk_ОбщегоНазначения.СообщитьОбОшибке("Нал. расчет амортизации: нематериальный актив " + ВыборкаПоНМА.НематериальныйАктив + " должнен быть переведен на начисление амортизации по базовой стоимости.");
							КонецеслИ;
...
Процедура НачалоВыбораЗначенияСпособовОтраженияРасходовПоАмортизации(ЭлементФормы, Организация = Неопределено, СтандартнаяОбработка) Экспорт
	...
	// ВСТАВКА нач  07.06.2016 Гущина T#7731159  нач
    // было Если НЕ Организация = Неопределено Тогда ... , перенесла в ИначеЕсли
	Если   НЕ Организация = Неопределено 
		И  НЕ Организация.ГоловнаяОрганизация = Справочники.Организации.ПустаяСсылка()Тогда
		...
		конецЕсли;
	ИначеЕсли НЕ Организация = Неопределено Тогда
		// ВСТАВКА кон  07.06.2016 Гущина 
		
		// Оставим в выборе только записи с организацией из документа и те, в которых организация вообще не указана
		СписокОрганизаций = Новый СписокЗначений;
		СписокОрганизаций.Добавить(Организация);
		//АБС ВСТАВКА Филиализация 21.08.2014 Пугачев
		Если ПараметрыСеанса.абс_НастройкиСистемы.ДатаНачалаПримененияСпособовОтраженияКТТК<ТекущаяДата() Тогда
			СписокОрганизаций.Добавить(ПараметрыСеанса.абс_НастройкиСистемы.ОрганизацияКТТК);
		КонецЕсли;
		//\\АБС ВСТАВКА Филиализация 21.08.2014 Пугачев
...
Функция ПолучитьФормуВыбораЛьготыПоНалогуНаИмущество(ВыбранноеЗначение) Экспорт
	...
	Макет = Отчеты.РегламентированныйОтчетИмущество.ПолучитьМакет("Списки");
