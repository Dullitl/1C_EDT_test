...
Функция ИспользоватьЗаказВТабличнойЧасти(ДокументВидОперации, ДоговорКонтрагента, ВидОперации = "Реализация") Экспорт
	...
	Иначе
		ttk_ОбщегоНазначения.СообщитьОбОшибке("ИспользоватьЗаказВТабличнойЧасти: передан не верный вид операции документа.");
	...
	Если УказаниеЗаказов Тогда
		Если ВидОперации = "Реализация" Тогда
			...
			ИначеЕсли (ДокументВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ПродажаКомиссия)
			  ИЛИ (ДокументВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.Брак)
			  ИЛИ (ДокументВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ОтгрузкаБезПереходаПраваСобственности) Тогда
		...
		ИначеЕсли ВидОперации = "Поступление" Тогда
			Если ДокументВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ПокупкаКомиссия
				ИЛИ ДокументВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Оборудование Тогда
				Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
					ВедениеВзаиморасчетов = ДоговорКонтрагента.ВедениеВзаиморасчетов;
...
Процедура ПроверитьРеквизитыЗаказов(ДокументОбъект, ИмяТабличнойЧасти, ИмяРевизитаЗаказ, 
	...
	//АБС_Стрельцов+ добавлено: 22.11.2012
	//условие, касающиеся Проектного Учета
	//если, в какой-либо строке ТЧ указан проект и заказ, принадлежщий проекту, то
	//проверка по этой строке отключается...
	//-----------------------------------------------------------------------------
	МетаТаблЧасти = Метаданные.НайтиПоТипу(Тип(ДокументОбъект[ИмяТабличнойЧасти]));
	РеквПроект = МетаТаблЧасти.Реквизиты.Найти("абс_Проект");
	ЕстьПроектТЧ = ((НЕ РеквПроект = Неопределено) И (РеквПроект.Тип.СодержитТип(Тип("СправочникССылка.Проекты"))));	
	//-----------------------------------------------------------------------------
	//АБС_Стрельцов-
	...
	Для Каждого СтрокаТаблицы Из ДокументОбъект[ИмяТабличнойЧасти] Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы[ИмяРевизитаЗаказ]) И 
			...
			//АБС_Стрельцов+++ добавлено: 22.11.2012
			//--------------------------------------
			Если ЗначениеЗаполнено(СтрокаТаблицы[ИмяРевизитаЗаказ]) И ТипЗнч(СтрокаТаблицы[ИмяРевизитаЗаказ]) = Тип("ДокументСсылка.ЗаказПокупателя") 
				И ЕстьПроектТЧ Тогда
				
				ЕСли ЗначениеЗаполнено(СтрокаТаблицы.абс_Проект.абс_ЗаказПокупателя) И ЗначениеЗаполнено(СтрокаТаблицы[ИмяРевизитаЗаказ])
					И СтрокаТаблицы.абс_Проект.абс_ЗаказПокупателя = СтрокаТаблицы[ИмяРевизитаЗаказ] Тогда
			...
			Если ДокЗаказ.Организация <> ДокументОбъект.Организация Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "выбран заказ с неверным реквизитом ""Организация"". " +
			...
			Если ДокЗаказ.Контрагент <> ДокументОбъект.Контрагент Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "выбран заказ с неверным реквизитом ""Контрагент"". " +
			...
			Если ДокЗаказ.ДоговорКонтрагента <> ДокументОбъект.ДоговорКонтрагента Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "выбран заказ с неверным реквизитом ""Договор контрагента"". " +
			...
			Если ДокЗаказ.Дата > ДокументОбъект.Дата Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "дата и время Заказа больше даты и времени документа!", Отказ, Заголовок);
			КонецЕсли;
