...
//   Дерево значений
//
Функция ДеревоДляПоказаСписка() Экспорт

	Запрос = Новый Запрос;
					   
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	СреднийЗаработок.Ссылка КАК Ссылка,
	|	ВЫБОР
	|		КОГДА СреднийЗаработок.Ссылка В (&Список)
	|			ТОГДА 1
	|		КОГДА СреднийЗаработок.Ссылка В (&Отпуска)
	|			ТОГДА 2
	|		КОГДА СреднийЗаработок.Ссылка В (&СписокДо2010)
	|			ТОГДА 3
	|		КОГДА СреднийЗаработок.Ссылка В (&СписокСтарый)
	|			ТОГДА 5
	|		КОГДА СреднийЗаработок.Ссылка В (&ОтпускаСтарый)
	|			ТОГДА 4
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ПорядокВывода,
	|	ВЫБОР
	|		КОГДА СреднийЗаработок.Ссылка В (&Заработок)
	|			ТОГДА 1
	|		КОГДА СреднийЗаработок.Ссылка В (&Премии)
	|			ТОГДА 2
	|		КОГДА СреднийЗаработок.Ссылка В (&ГодовыеПремии)
	|			ТОГДА 3
	|		ИНАЧЕ 4
	|	КОНЕЦ КАК ДопУпорядочивание
	|ИЗ
	|	ПланВидовРасчета.СреднийЗаработок КАК СреднийЗаработок
	|ГДЕ
	|	ВЫБОР
	|		КОГДА СреднийЗаработок.Ссылка В (&Список)
	|			ТОГДА 1
