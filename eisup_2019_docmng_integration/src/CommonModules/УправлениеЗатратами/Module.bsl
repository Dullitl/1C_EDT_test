...
Функция ПолучитьХарактерЗатратПоСчетуЗатрат(СчетЗатрат, СтатьяЗатрат, ИмяПланСчетов = "Хозрасчетный") экспорт
	...
	ИначеЕсли ТипЗнч(СчетЗатрат) <> Тип("ПланСчетовСсылка."+ИмяПланСчетов) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Счет затрат " + СчетЗатрат + " не соответствует плану счетов: """ + ИмяПланСчетов + """.
...
Процедура ЗаполнитьСчетИСубконтоУчетаЗатрат(
	...
	// АБС ВСТАВКА 20110919 Фролов
	// Добавлена возможность получения подразделения из ТЧ документа
	,ИмяРеквизитаПодразделениеОрганизацииТЧ = "ПодразделениеОрганизации" 
	) Экспорт
	...
	Если ПроизводственныеРасходы Тогда
		...
		Если ПолучитьПодразделениеИзСтрокиТаблицы Тогда
			// АБС ВСТАВКА 20110919 Фролов
			// Добавлена возможность получения подразделения из ТЧ документа
			ПодразделениеОрганизации = СтрокаТабличнойЧасти[ИмяРеквизитаПодразделениеОрганизацииТЧ];
		Иначе
...
//
Процедура УстановитьВидимостьЯчеекАналитикиЗатрат(ХарактерЗатрат, Продукция, СчетЗатрат = НЕОПРЕДЕЛЕНО, СчетЗатратНУ = НЕОПРЕДЕЛЕНО, ОформлениеСтроки, ОтражатьВУправленческомУчете, ОтражатьВБухгалтерскомУчете, ОтражатьВНалоговомУчете, врСтатусДокумента = Неопределено) Экспорт
	...
	ПрочиеРасходыНУ         = Истина;//(ХарактерЗатратНУ = Перечисления.ХарактерЗатрат.Прочие);
	...
	Если ЗначениеЗаполнено(СчетЗатрат) Тогда// И ПрочиеРасходыБУ  Тогда
		КоличествоСубконто = СчетЗатрат.ВидыСубконто.Количество();
	Иначе
		КоличествоСубконто = 0;
	КонецЕсли;
	Если ЗначениеЗаполнено(СчетЗатратНУ)Тогда // и ПрочиеРасходыНУ Тогда
	...
	Если КоличествоСубконто = КоличествоСубконтоНУ 
	  // и ПрочиеРасходыБУ
	   И ЗначениеЗаполнено(СчетЗатрат)
	  // и ПрочиеРасходыНУ 
	...
	//АБС - настройка видимости аналитики в таб. части 
	Если НЕ ОформлениеСтроки.Ячейки.Найти("СчетЗатрат") = НЕОПРЕДЕЛЕНО Тогда
		ЯчейкаСчета = "СчетЗатрат";
	ИначеЕсли НЕ ОформлениеСтроки.Ячейки.Найти("СчетБУ") = НЕОПРЕДЕЛЕНО Тогда
		ЯчейкаСчета = "СчетБУ";
	КонецЕсли;	
	Если НЕ ОформлениеСтроки.Ячейки.Найти("ВидСубконто3") = НЕОПРЕДЕЛЕНО И ЗначениеЗаполнено(ЯчейкаСчета) Тогда		
		Если ОформлениеСтроки.Ячейки[ЯчейкаСчета].Значение = ПланыСчетов.Хозрасчетный.ОбщепроизводственныеРасходыНеОблагаемыеЕНВД Тогда			
			ОформлениеСтроки.Ячейки.ВидСубконто1.Видимость = Истина;
			ОформлениеСтроки.Ячейки.ВидСубконто1.Значение = СчетЗатрат.ВидыСубконто.Получить(0).ВидСубконто;
			ОформлениеСтроки.Ячейки.ВидСубконто1.ТолькоПросмотр = Истина;
			ОформлениеСтроки.Ячейки.ВидСубконто1.Текст = СчетЗатрат.ВидыСубконто.Получить(0).ВидСубконто;
			ОформлениеСтроки.Ячейки.Субконто1.Видимость = Истина;
			ОформлениеСтроки.Ячейки.Субконто1.ТолькоПросмотр = Ложь;
			
			ОформлениеСтроки.Ячейки.ВидСубконто2.Видимость = Истина;
			ОформлениеСтроки.Ячейки.ВидСубконто2.Значение = СчетЗатрат.ВидыСубконто.Получить(1).ВидСубконто;
			ОформлениеСтроки.Ячейки.ВидСубконто2.ТолькоПросмотр = Истина;
			ОформлениеСтроки.Ячейки.ВидСубконто2.Текст = СчетЗатрат.ВидыСубконто.Получить(1).ВидСубконто;
			ОформлениеСтроки.Ячейки.Субконто2.Видимость = Истина;                            
			ОформлениеСтроки.Ячейки.Субконто2.ТолькоПросмотр = Ложь;
			
			ОформлениеСтроки.Ячейки.ВидСубконто3.Видимость = Истина;
			ОформлениеСтроки.Ячейки.ВидСубконто3.Значение = СчетЗатрат.ВидыСубконто.Получить(2).ВидСубконто;
			ОформлениеСтроки.Ячейки.ВидСубконто3.ТолькоПросмотр = Истина;
			ОформлениеСтроки.Ячейки.ВидСубконто3.Текст = СчетЗатрат.ВидыСубконто.Получить(2).ВидСубконто;
			ОформлениеСтроки.Ячейки.Субконто3.Видимость = Истина;
			ОформлениеСтроки.Ячейки.Субконто3.ТолькоПросмотр = Ложь;
		КонецЕсли;
	КонецЕсли;
	//АБС\\
	...
	//АБС ВСТАВКА №8519,35863 НАЧАЛО
	Если НЕ врСтатусДокумента = Неопределено и мРолиПользователя.Найти(Справочники.РолиИсполнителей.ЗаполнениеСубконтоВПодготовке) = Неопределено Тогда
		Если врСтатусДокумента = Перечисления.абс_СтатусыПервичныхДокументов.Подготовка 
			или врСтатусДокумента = Перечисления.абс_СтатусыПервичныхДокументов.СогласованиеОФК Тогда
			
				Если НЕ ОформлениеСтроки.Ячейки.Найти("ВидАналитики") = НЕОПРЕДЕЛЕНО Тогда
					ОформлениеСтроки.Ячейки.ВидАналитики.Видимость     = ЛОЖЬ;
				КонецЕсли;
				Если НЕ ОформлениеСтроки.Ячейки.Найти("ВидАналитикиДоп") = НЕОПРЕДЕЛЕНО Тогда
					ОформлениеСтроки.Ячейки.ВидАналитикиДоп.Видимость  = ЛОЖЬ;  
				КонецЕсли;	
				Если НЕ ОформлениеСтроки.Ячейки.Найти("ВидСубконто1") = НЕОПРЕДЕЛЕНО Тогда
					ОформлениеСтроки.Ячейки.ВидСубконто1.Видимость  = ЛОЖЬ;  
				КонецЕсли;	
				Если НЕ ОформлениеСтроки.Ячейки.Найти("ВидСубконто2") = НЕОПРЕДЕЛЕНО Тогда
					ОформлениеСтроки.Ячейки.ВидСубконто2.Видимость  = ЛОЖЬ;  
				КонецЕсли;	
				Если НЕ ОформлениеСтроки.Ячейки.Найти("ВидСубконто3") = НЕОПРЕДЕЛЕНО Тогда
					ОформлениеСтроки.Ячейки.ВидСубконто3.Видимость  = ЛОЖЬ;  
				КонецЕсли;	
				Если НЕ ОформлениеСтроки.Ячейки.Найти("ВидСубконтоНУ1") = НЕОПРЕДЕЛЕНО Тогда
					ОформлениеСтроки.Ячейки.ВидСубконтоНУ1.Видимость  = ЛОЖЬ;  
				КонецЕсли;		
				Если НЕ ОформлениеСтроки.Ячейки.Найти("ВидСубконтоНУ2") = НЕОПРЕДЕЛЕНО Тогда
				...
				Если НЕ ОформлениеСтроки.Ячейки.Найти("Аналитика") = НЕОПРЕДЕЛЕНО Тогда
					ОформлениеСтроки.Ячейки.Аналитика.Видимость  = ЛОЖЬ;  
				КонецЕсли;
				Если НЕ ОформлениеСтроки.Ячейки.Найти("НоменклатурнаяГруппа") = НЕОПРЕДЕЛЕНО Тогда
					ОформлениеСтроки.Ячейки.НоменклатурнаяГруппа.Видимость  = ЛОЖЬ;  
				КонецЕсли;
				Если НЕ ОформлениеСтроки.Ячейки.Найти("НоменклатурнаяГруппаДоп") = НЕОПРЕДЕЛЕНО Тогда
					ОформлениеСтроки.Ячейки.НоменклатурнаяГруппаДоп.Видимость  = ЛОЖЬ;  
				КонецЕсли;
				Если НЕ ОформлениеСтроки.Ячейки.Найти("СпособРаспределенияЗатратНаВыпуск") = НЕОПРЕДЕЛЕНО Тогда
					ОформлениеСтроки.Ячейки.СпособРаспределенияЗатратНаВыпуск.Видимость  = ЛОЖЬ;  
				КонецЕсли;
				Если НЕ ОформлениеСтроки.Ячейки.Найти("Субконто1") = НЕОПРЕДЕЛЕНО Тогда
					ОформлениеСтроки.Ячейки.Субконто1.Видимость  = ЛОЖЬ;  
				КонецЕсли;	
				Если НЕ ОформлениеСтроки.Ячейки.Найти("Субконто2") = НЕОПРЕДЕЛЕНО Тогда
					ОформлениеСтроки.Ячейки.Субконто2.Видимость  = ЛОЖЬ;  
				КонецЕсли;	
				Если НЕ ОформлениеСтроки.Ячейки.Найти("Субконто3") = НЕОПРЕДЕЛЕНО Тогда
				...
				КонецЕсли;	
				Если НЕ ОформлениеСтроки.Ячейки.Найти("ХарактеристикаПродукции") = НЕОПРЕДЕЛЕНО Тогда
					ОформлениеСтроки.Ячейки.ХарактеристикаПродукции.Видимость  = ЛОЖЬ;  
				КонецЕсли;	
				Если НЕ ОформлениеСтроки.Ячейки.Найти("СерияПродукции") = НЕОПРЕДЕЛЕНО Тогда
...
Процедура ПроверитьПодразделениеОрганизации(ДокументОбъект, Отказ, Заголовок) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументОбъект.Организация)
	   ...
	   //АБС ВСТАВКА 54493  16.12.2014 18:32:50  Шамов
	   ...
	   И Не ДокументОбъект.ПодразделениеОрганизации.Владелец = ДокументОбъект.Организация.Родитель Тогда
	   //АБС ВСТАВКА 54493 КОНЕЦ 

		ttk_ОбщегоНазначения.СообщитьОбОшибке("Указанное в документе подразделение не соответствует организации!", Отказ, Заголовок);
...
Процедура ПроверитьПодразделениеОрганизацииВСтрокахТабЧасти(ДокументОбъект, ТабЧасть, ИмяТабЧасти, ИмяРеквизитаТЧ = "ПодразделениеОрганизации", Отказ, Заголовок) Экспорт
	...
	Для Каждого СтрокаТабличнойЧасти из ТабЧасть Цикл
		   ...
		   И Не СтрокаТабличнойЧасти[ИмяРеквизитаТЧ].Владелец = ДокументОбъект.Организация Тогда
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Указанное подразделение не соответствует организации! Строка № " + СтрокаТабличнойЧасти.НомерСтроки + " (таб. часть """ + ИмяТабЧасти + """)", Отказ, Заголовок);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьЗаполнениеСубконто(ДокОбъект, СтруктураПараметров, ИмяТабЧасти = "", Отказ, Заголовок) Экспорт
	...
	Если ПустаяСтрока(ИмяТабЧасти) Тогда
		...
		Если ДокОбъект[ИмяСчета].Пустая() Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(СтруктураПараметров[ИмяСчета], Отказ, Заголовок);
			Возврат;
		КонецЕсли;
		...
		Для К = 0 По ДокОбъект[ИмяСчета].ВидыСубконто.Количество() - 1 Цикл
			Если Не ДокОбъект[ИмяСчета].ВидыСубконто[К].ТолькоОбороты
				...
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтруктураПараметров[ИмяСубконто[К]], Отказ, Заголовок);
			КонецЕсли;
		КонецЦикла;
	...
	Иначе
		...
		Для Каждого СтрокаТЧ Из ДокОбъект[ИмяТабЧасти] Цикл
			...
			Если СтрокаТЧ[ИмяСчета].Пустая() Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(СтруктураПараметров[ИмяСчета] + " (строка № " + СтрокаТЧ.НомерСтроки + ")", Отказ, Заголовок);
				Продолжить;
			КонецЕсли;
			...
			Для К = 0 По СтрокаТЧ[ИмяСчета].ВидыСубконто.Количество() - 1 Цикл
				Если Не СтрокаТЧ[ИмяСчета].ВидыСубконто[К].ТолькоОбороты
					...
					ttk_ОбщегоНазначения.СообщитьОбОшибке(СтруктураПараметров[ИмяСубконто[К]] + " (строка № " + СтрокаТЧ.НомерСтроки + ")", Отказ, Заголовок);
				КонецЕсли;
			КонецЦикла;
