...
Процедура СформироватьДвиженияПоНаправлениямСписания(
	...
	Если СтрокаВыборки.КодОперации = Перечисления.КодыОперацийПартииМатериаловВЭксплуатации.ПеремещениеВЭксплуатации Тогда
		...
		// АБС ВСТАВКА Фролов 20110620 
		Попытка 
			
			СтруктураАналитики.Вставить("ДокументДвижения", СтрокаВыборки.ДокументДвижения);
			
		Исключение
			
		КонецПопытки;
		// АБС ВСТАВКА Фролов 20110620 КОНЕЦ
...
Процедура ВывестиСообщениеОПревышенииКоличества(
	...
	Если ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВУправленческомУчете Тогда
		СтрокаГруппы = "Количество по упр. учету, списываемое из эксплуатации, превышает остаток:";
		
	ИначеЕсли ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВБухгалтерскомУчете
		ИЛИ ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВРегламентированномУчете
	Тогда
		СтрокаГруппы = "Количество по бух. учету, списываемое из эксплуатации, превышает остаток:";
		
	ИначеЕсли ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВНалоговомУчете Тогда
		СтрокаГруппы = "Количество по нал. учету, списываемое из эксплуатации, превышает остаток:";
...
Процедура РассчитатьСуммуСписанияИзЭксплуатации(
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого СтрокаТаблицы Из МассивСтрок Цикл
			...
			//АБС ИЗМЕНЕНИЕ 38191  04.02.2014 15:23:09  Шамов
			Попытка
			//АБС ИЗМЕНЕНИЕ 38191 КОНЕЦ
			...
			//АБС ИЗМЕНЕНИЕ 38191  04.02.2014 15:23:09  Шамов
			Исключение
				//вероятно, в поле документа передачи затесалось Неопределено
				//исправление в таблицах регистров пока не согласовали
				//а документы проводить нужно
			КонецПопытки;
			//АБС ИЗМЕНЕНИЕ 38191 КОНЕЦ
...
Процедура СформироватьДвиженияПоМатериаламВЭксплуатации(
	...
	Для Каждого ВидОтраженияВУчете Из Перечисления.ВидыОтраженияВУчете Цикл
		...
		//АБС Коломиец 15198+
		Если СтруктураШапкиДокумента.Свойство("абс_ПерепроводитьДокументыСписания")
			И СтруктураШапкиДокумента.абс_ПерепроводитьДокументыСписания Тогда
			...
			ДокументыДляУдаленияДвиженияПартий = РезультатЗапроса.Выгрузить();
			ДокументыДляУдаленияДвиженияПартий.Свернуть("ДокументДвижения");
			
			Для Каждого Документ Из ДокументыДляУдаленияДвиженияПартий Цикл
				
				Если ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВУправленческомУчете Тогда
					НаборЗаписейПартии = РегистрыНакопления.ПартииМатериаловВЭксплуатации.СоздатьНаборЗаписей();
				ИначеЕсли ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВБухгалтерскомУчете Тогда
					НаборЗаписейПартии = РегистрыНакопления.ПартииМатериаловВЭксплуатацииБухгалтерскийУчет.СоздатьНаборЗаписей();
				ИначеЕсли ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВНалоговомУчете Тогда
					НаборЗаписейПартии = РегистрыНакопления.ПартииМатериаловВЭксплуатацииНалоговыйУчет.СоздатьНаборЗаписей();	
				КонецЕсли;
				
				НаборЗаписейПартии.Отбор.Регистратор.Значение = Документ.ДокументДвижения;
		...
		УправлениеЗатратамиДвиженияПоРегистрам.ЗаписатьНаборыЗаписейРегистров(СтруктураНаборыЗаписей,,Истина);
...
Процедура СформироватьПроводкуПоРегиструБухгалтерииПогашениеСтоимости(
	...
	Если ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВБухгалтерскомУчете Тогда
		...
		СчетУчета = СтрокаВыборки.СчетУчетаБУ;                                                      //АБС 21.03.2011                     //АБС Коломиец 14558
		ЭтоИнвентарь = (СчетУчета = ПланыСчетов.Хозрасчетный.ИнвентарьИХозяйственныеПринадлежности) или (СчетУчета.Код = "10.12") ИЛИ (СчетУчета.Код = "10.13");
			
	ИначеЕсли ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВНалоговомУчете Тогда
		ИмяПланСчетов = "Налоговый";
		СчетЗатрат = СтрокаВыборки.СчетЗатратНУ;
		СчетУчета = СтрокаВыборки.СчетУчетаНУ;                                                  //АБС 21.03.2011                     //АБС Коломиец 14558
		ЭтоИнвентарь = (СчетУчета = ПланыСчетов.Налоговый.ИнвентарьИХозяйственныеПринадлежности) или (СчетУчета.Код = "10.12") ИЛИ (СчетУчета.Код = "10.13");
...
Функция СформироватьЗапросПоСпособамОтраженияРасходов(
	...
	//АБС ВСТАВКА Навценя 27.02.2015
           ...
           |	ВЫБОР
           |		КОГДА СпособыОтраженияРасходов.Ссылка.Организация = &Организация
           |			ТОГДА СпособыОтраженияРасходов.ПодразделениеОрганизации
           |		ИНАЧЕ ЕСТЬNULL(СоответствиеПодразделений.ПодразделениеОрганизации, СпособыОтраженияРасходов.ПодразделениеОрганизации)
	...
	//АБС ВСТАВКА Навценя 27.02.2015
	       ...
	       |			ПО СпособыОтраженияРасходов.Подразделение = СоответствиеПодразделений.Подразделение
...
Функция СформироватьЗапросПоСписаннымПартиямРасширеннаяАналитика(
	...
	|	ПО
	|		РегистрКорАналитикаУчетаПартий.НазначениеИспользования.СпособОтраженияРасходов = СпособыОтраженияРасходов.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ПартииМатериалов.АналитикаВидаУчета,
	|	ПартииМатериалов.АналитикаУчетаЗатрат,
...
Функция СформироватьЗапросПоСписаннымПартиям(
	...
	|		) КАК СпособыОтраженияРасходов
	|	ПО
	|		ПартииМатериалов.НазначениеИспользования.СпособОтраженияРасходов = СпособыОтраженияРасходов.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	//ДляУпрУчета ПартииМатериалов.Подразделение,
	|	//ДляРеглУчета ПартииМатериалов.Организация,
		...
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			...
			Иначе
				Если Выборка.НеУчитываемыеВНУ Тогда
					ПогашеннаяПостояннаяРазница = - Выборка.СтоимостьНал;
