...
Функция ПолучитьИнтернетПочтовыйПрофиль(УчетнаяЗапись) Экспорт
	...
	КонецЕсли; 
	Если Не УчетнаяЗапись.НеИспользоватьPOP3 Тогда
...
Процедура ОткрытьПисьмоПочтовогоКлиентаОперационнойСистемы(СтруктураПараметров) Экспорт
	...
	Исключение
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Вы можете ввести в текст текущего письма только " + ДопустимаяДлинаТекста + " символов, а у вас - " + СтрДлина(СокрЛП(ТекстСообщения)));
...
Процедура ОтправитьПисьма(Письма, СоответствиеТекстовЭлектронныхПисем, ТекущийПользователь, Знач ПоказыватьОкноВыполнения = Истина, ТекстОшибок = "") Экспорт
	...
	Для каждого Письмо Из Письма Цикл
		...
		Если ТипЗнч(Письмо.Значение) <> Тип("ДокументОбъект.ЭлектронноеПисьмо") Тогда
			...
			Исключение
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, (СокрЛП(Письмо) + " исключено из списка отправки."));
				Продолжить;;
			КонецПопытки;
		Иначе
...
Функция ОткрытьФайлДляРедактированияВнешнимХТМЛРедактором(ИмяФайла) Экспорт
	...
	Если НЕ Файл.Существует() ИЛИ Файл.ЭтоКаталог() Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Ошибка открытия файла. Файл не найден на диске.");
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ИмяПриложения) ИЛИ ТипЗнч(ИмяПриложения) <> Тип("Строка") Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("У пользователя не указана командная строка для запуска внешнего HTML редактора текстов.",, "Редактирование невозможно.");
		Возврат Ложь;
	КонецЕсли;
	
	ИмяПриложения = Врег(ИмяПриложения);
	Если Найти(ИмяПриложения, "[FILENAME]") = 0 ИЛИ СтрЧислоВхождений(ИмяПриложения, "[FILENAME]") > 1 Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("В настройках пользователя некорректно задана командная строка для запуска внешнего HTML редактора текстов.
	...
	Исключение
		ttk_ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, "Невозможно запустить внешнее приложение.");
		Возврат Ложь;
	КонецПопытки;
