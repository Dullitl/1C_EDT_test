...
Функция ПолучитьТаблицуДвиженийПоРегиструПараметрыАмортизацииОС(ТаблицаЗатрат,СтруктураКлючиАналитики,ДатаДокумента, ВидОтраженияВУчете)
	...
	Если ТаблицаСписания.Количество() > 0 Тогда
		...
		|	ТаблицаКорректировкиСтоимостиОС.Стоимость + ЕСТЬNULL(ПараметрыАмортизацииОС.СтоимостьДляВычисленияАмортизации,0) КАК СтоимостьДляВычисленияАмортизации,
...
Процедура СформироватьДвиженияПоАналитическимРегистрам(
	...
	Для Каждого ЭлементСоответствия Из СтруктруаНаборовЗаписей Цикл
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТекущаяТаблицаДвижений, ТаблицаДвижений);
...
Процедура ДобавитьПроводку(СтруктураПараметров, ИмяРегистра, Движение,КэшПоИерархииСчетов, Сумма, ВидУчета = Неопределено) Экспорт
	...
	Если (НЕ ВидУчета = Неопределено)
	  И (ИмяРегистра = "Налоговый") Тогда
	...
	Если Проводка.СчетКт.Количественный Тогда
		Если ВидУчета <> Перечисления.ВидыУчетаПоПБУ18.ПР
	 	  И ВидУчета <> Перечисления.ВидыУчетаПоПБУ18.ВР Тогда
	...
	Если СовпадаетАналитикаДтКт(Проводка)
	  ...
	  ИЛИ ((НЕ ЗначениеЗаполнено(Проводка.СчетДт)) ИЛИ (НЕ ЗначениеЗаполнено(Проводка.СчетКт))) Тогда
	...
	Для Каждого Движение Из ТаблицаСписания Цикл
		...
		Если СтруктураПараметров.ОтражатьВНалоговомУчете Тогда
			Если НЕ Движение.СтоимостьНУ = 0 Тогда
				ДобавитьПроводку(СтруктураПараметров,"Налоговый",Движение,КэшПоИерархииСчетов, Движение.СтоимостьНУ, Перечисления.ВидыУчетаПоПБУ18.НУ);
			КонецЕсли;	
			...
			Если Движение.СтоимостьНУ = 0 И Движение.ПостояннаяРазница = 0 Тогда
				ДобавитьПроводку(СтруктураПараметров,"Налоговый",Движение, КэшПоИерархииСчетов, 0, Перечисления.ВидыУчетаПоПБУ18.НУ);
			КонецЕсли;	
		КонецЕсли;	
		  ...
		  И СтруктураПараметров.ОтражатьВНалоговомУчете Тогда
		  	Если СтруктураПараметров.ПоддержкаПБУ18 Тогда
			  	Если Движение.Стоимость - Движение.СтоимостьНУ - Движение.ПостояннаяРазница <> 0 Тогда
			  		ДобавитьПроводку(СтруктураПараметров,"Налоговый",Движение, КэшПоИерархииСчетов, Движение.Стоимость - Движение.СтоимостьНУ - Движение.ПостояннаяРазница, Перечисления.ВидыУчетаПоПБУ18.ВР);
				КонецЕсли;
			КонецЕсли;	
		КонецЕсли;		
	КонецЦикла;
...
Процедура РаспределитьТЗРСоСчета16(СтруктураШапкиДокумента) Экспорт
	...
	|				ИЛИ НаправленияСписания.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПриобретениеТоваров)
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ОтклонениеВСтоимостиТоваров)
	|		ИНАЧЕ ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ОтклонениеВСтоимостиМатериалов)
	|	КОНЕЦ КАК СчетУчетаОтклонений,
...
Процедура ВключитьНДСВЗатраты(СтруктураШапкиДокумента) Экспорт
	...
	СтруктураПараметров = Новый Структура("Дата, Ссылка, Организация, Движения", СтруктураШапкиДокумента.Дата, СтруктураШапкиДокумента.Ссылка, СтруктураШапкиДокумента.Организация, СтруктураНаборыЗаписей);
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	СУММА(ВЫБОР
						  //Для Включения и Исключения НДС из стоимости нужно формировать движения с разным знаком
	                      ...
	                      |				ТОГДА НДСПартииТоваровОбороты.НДСПриход - НДСПартииТоваровОбороты.НДСРасход
	                      |			ИНАЧЕ ВЫБОР
	                      ...
	                      |						ТОГДА НДСПартииТоваровОбороты.НДСРасход - НДСПартииТоваровОбороты.НДСПриход
	                      |				КОНЕЦ
	                      |		КОНЕЦ) КАК НДСПриход
	                      |ИЗ
						  //Обрабатываем только перемещения и комплектации
	                      |	РегистрНакопления.НДСПартииТоваров.Обороты(&НачДата, &КонДата, Регистратор, Организация = &Организация) КАК НДСПартииТоваровОбороты
	                      |ГДЕ
						  ...
						  //Если измерения СчетФактура не заполнено, то НДС уже был принят к вычету и спишется на расходы. Включать в себестоимость его не нужно
	                      |	И НДСПартииТоваровОбороты.СчетФактура <> НЕОПРЕДЕЛЕНО
	                      |
	                      |СГРУППИРОВАТЬ ПО
	                      |	НДСПартииТоваровОбороты.Склад,
	                      |	НДСПартииТоваровОбороты.ХарактеристикаНоменклатуры,
	...
	ТаблицаВключенияВСтоимость.Колонки.Добавить("СчетУчетаНУ");
	ТаблицаВключенияВСтоимость.Колонки.Добавить("КорСчетУчетаНУ");
	
	ТаблицаВключенияВСтоимость.ЗаполнитьЗначения(ПланыСчетов.Налоговый.ПоступлениеИВыбытиеИмуществаРаботУслугПрав,"СчетУчетаНУ");
	
	Для Каждого ТекущаяСтрока Из ТаблицаВключенияВСтоимость Цикл
		ТекущаяСтрока.КорСчетУчетаНУ = РасширеннаяАналитикаУчета.ПолучитьСчетУчетаНУ(ТекущаяСтрока.КорСчетУчета);
		АналитикаВидаУчета = РасширеннаяАналитикаУчета.ПолучитьЗначениеКлючаАналитики(
			СтруктураШапкиДокумента,
			Новый Структура("Организация, СчетУчета, СчетУчетаНУ, Склад, РазделУчета", 
				ТекущаяСтрока.Организация, 
				ТекущаяСтрока.КорСчетУчета,
				ТекущаяСтрока.КорСчетУчетаНУ,
				ТекущаяСтрока.Склад,
		...
		Если НЕ Результат.Пустой() Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ИмяРегистраУчетаЗатрат = "УчетЗатратРегл" Тогда
					...
					ТаблицаДляФормированияДвижений[Выборка.НомерСтроки].ПостояннаяРазница = ТаблицаДляФормированияДвижений[Выборка.НомерСтроки].КоличествоНУ * Выборка.ЦенаПР;
					ТаблицаДляФормированияДвижений[Выборка.НомерСтроки].ПостояннаяРазницаПолучатель = ТаблицаДляФормированияДвижений[Выборка.НомерСтроки].ПостояннаяРазница;
				КонецЕсли;	
			КонецЦикла;	
		КонецЕсли;			
	КонецЦикла;
	...
	Если НЕ СтруктураНаборовЗаписейПоВидамУчета.Свойство("УчетЗатратРегл") = Неопределено 
	  И ТаблицаДляФормированияПроводок.Количество() > 0	Тогда
		...
		Если Не Источник.мСтруктураШапкиДокумента.Свойство("ПоддержкаПБУ18") Тогда
			ПараметрыУчетнойПолитикиРегл = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(Источник.мСтруктураШапкиДокумента.Дата, Источник.мСтруктураШапкиДокумента.Организация, Ложь);
			Если ЗначениеЗаполнено(ПараметрыУчетнойПолитикиРегл) Тогда
