...
//АБС Попов
Процедура УстановкаПараметровОтбораПоОрганизации() Экспорт
	Если глЗначениеПеременной("абс_ИспользоватьПараметрОтбораПоОрганизации")Тогда
		...
		ВыборкаОрганизаций = Запрос.Выполнить().Выгрузить();
		Если ВыборкаОрганизаций.Количество()>0 Тогда
			
				ПараметрыСеанса.абс_ИспользоватьПараметрОтбораПоОрганизации = Истина;
				ПараметрыСеанса.абс_ПараметрОтбораПоОрганизации = новый ФиксированныйМассив(ВыборкаОрганизаций.ВыгрузитьКолонку("ОбъектДоступа"));
			
		Иначе
			ПараметрыСеанса.абс_ИспользоватьПараметрОтбораПоОрганизации = Ложь;
		КонецЕсли;
...
//АБС ВСТАВКА 37344  15.01.2014 17:12:16  Шамов
Процедура УстановкаНастроекУниверсальногоОбмена() Экспорт

	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	абс_НастройкаУниверсальногоОбмена.ОбъектОбмена
		|ИЗ
		|	РегистрСведений.абс_НастройкаУниверсальногоОбмена КАК абс_НастройкаУниверсальногоОбмена
		|ГДЕ
		|	абс_НастройкаУниверсальногоОбмена.Выгружать_ЕИСУП_ИСУЗК";

	ВыборкаОбъектыОбмена = Запрос.Выполнить().Выгрузить();

	Если ВыборкаОбъектыОбмена.Количество()>0 Тогда
		
		ПараметрыСеанса.абс_НастройкиУниверсальногоОбмена = новый ФиксированныйМассив(ВыборкаОбъектыОбмена.ВыгрузитьКолонку("ОбъектОбмена"));
		
	Иначе
		
		ПараметрыСеанса.абс_НастройкиУниверсальногоОбмена = новый ФиксированныйМассив(Новый Массив());
...
Процедура УстановитьПараметрГраницыЗапретаИзмененияДанных() Экспорт
    ...
    ТекстЗапросаПоОбщейДатеЗапретаРедактирования = "ВЫБРАТЬ
                                                   ...
                                                   |	ЕСТЬNULL(ГраницыЗапретаИзмененияДанных.абс_ГраницаЗапретаВВодаПервичныхДокументов, ДАТАВРЕМЯ(1, 1, 1)) КАК ГраницаППД,
                                                   |	ЕСТЬNULL(ГраницыЗапретаИзмененияДанных.абс_ГраницаЗапретаОбработкиБухгалтером, ДАТАВРЕМЯ(1, 1, 1)) КАК ГраницаБухгалтерии
                                                   |ИЗ
    ...
    ТекстЗапросаПоПриоритетамЗаписей = "ВЫБРАТЬ
                                       ...
                                       |	Границы.абс_ГраницаЗапретаВВодаПервичныхДокументов КАК ГраницаППД,
                                       |	Границы.абс_ГраницаЗапретаОбработкиБухгалтером КАК ГраницаБухгалтерии
                                       |ИЗ
	...
	Запрос.Текст = "ВЫБРАТЬ  РАЗРЕШЕННЫЕ
				   ...
				   |	МИНИМУМ(ЕСТЬNULL(ТаблицаГраницыЗапрета.ГраницаППД, ОбщаяДатаЗапрета.ОбщаяДата)) КАК ГраницаППД,
				   |	МИНИМУМ(ЕСТЬNULL(ТаблицаГраницыЗапрета.ГраницаБухгалтерии, ОбщаяДатаЗапрета.ОбщаяДата)) КАК ГраницаБухгалтерии				   
                   |ИЗ
...
Процедура ЗаписатьНаборПрав(НаборПрав, Отказ, ШапкаОшибки)
	...
	Попытка
		НаборПрав.Записать();
	Исключение
		ttk_ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, ШапкаОшибки);
		Отказ = Истина;
	КонецПопытки;
...
Процедура ЗаписатьПраваДоступаПользователей(ТаблицаНабораПрав, СтруктураОтбора, Отказ = Ложь, ШапкаОшибки = "") Экспорт
	...
	Если НЕ УправлениеДопПравамиПользователей.ЕстьПравоАдминистрированияПользователей() Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Нарушение прав доступа!",, ШапкаОшибки);
		Возврат;
	КонецЕсли;	
	...
	ИначеЕсли НаборПрав.Отбор.Пользователь.Использование Тогда
		...
		Если НЕ ЗначениеЗаполнено(НаборПрав.Отбор.Пользователь.Значение) Тогда
			Отказ = Истина;
			ttk_ОбщегоНазначения.СообщитьОбОшибке(ШапкаОшибки);
			Возврат;
		КонецЕсли;
...
//
Процедура ЗаписатьНаборЗаписейНаСервере(ИмяРегистра, Регистратор, ТаблицаДвижений = Неопределено, ТипРегистра = "РегистрНакопления", Отказ) Экспорт
	...
	Если ТипРегистра = "РегистрНакопления" Тогда
		...
		Если ТаблицаДвижений <> Неопределено Тогда
			...
			ttk_ОбщегоНазначения.ВыполнитьДвижениеПоРегистру(Набор);		
		КонецЕсли;
...
Процедура ПодготовитьСтруктуруДляЗаполненияШапкиДокумента(СтруктураПараметров, ТипОперации, ОбъектКопирования) Экспорт
	...
	ДатаДокумента = ?(НЕ ЗначениеЗаполнено(СтруктураПараметров.Дата),ОбщегоНазначенияЗК.ПолучитьРабочуюДату(),СтруктураПараметров.Дата);
...
//АБС ВСТАВКА №4647 НАЧАЛО
Процедура ИзменитьДатуДокумента(ДокументСсылка,НоваяДата, КонтрольИзменения = Истина) Экспорт
	
	Отказ = Ложь;
	Документ = ДокументСсылка.ПолучитьОбъект();
	
	Если КонтрольИзменения Тогда
		Отказ = КонтрольИзмененияДаты(Документ,Отказ);	
	КонецЕсли;	
...
Функция КонтрольИзмененияДаты(Документ,Отказ)
		
	//Контроль изменения   
	Если Документ.Проведен Тогда
		Отказ = Истина;
		Сообщить("Менять дату можно только у не проведенного документа!",СтатусСообщения.Информация);
	КонецЕсли;
	
	Если НЕ Документ.Метаданные().Реквизиты.Найти("абс_Статус") = Неопределено Тогда	
		Если НЕ Документ.абс_Статус = Перечисления.абс_СтатусыПервичныхДокументов.Подготовка Тогда
			Отказ = Истина;
			Сообщить("Менять дату можно только у документа в статусе ""Подготовка""!",СтатусСообщения.Информация);
		КонецЕсли;
	Иначе
		Сообщить("Документа не имеет реквизит статус!",СтатусСообщения.Информация);
		Отказ = Истина;
	КонецЕсли;	
	
	Если НЕ Документ.Движения.Найти("Хозрасчетный") = Неопределено Тогда 		
		Документ.Движения["Хозрасчетный"].Прочитать();               		
		Если Документ.Движения["Хозрасчетный"].Количество() > 0 Тогда  
			Отказ = Истина;
			Сообщить("У документа не должно быть движений по регистру ""Хозрасчетный""!",СтатусСообщения.Информация);
		КонецЕсли;		
	Иначе
		Сообщить("Документа не делает движений по регистру ""Хозрасчетный""!",СтатусСообщения.Информация);
		Отказ = Истина;
	КонецЕсли;	
...
КонецФункции  
//\\АБС ВСТАВКА №4647 КОНЕЦ

//АБС ВСТАВКА №10498 НАЧАЛО
Процедура ИзменитьБулевоЗначение(ДокументСсылка,Реквизит) Экспорт 
	
	Если ДокументСсылка.ПометкаУдаления Тогда
		Сообщить("Документ помечен на удаления!",СтатусСообщения.Важное);
		Возврат;		
	КонецЕсли;
	
	Если не ДокументСсылка.Метаданные().Реквизиты.Найти(Реквизит) = Неопределено Тогда
		
		Документ = ДокументСсылка.ПолучитьОбъект();
		Документ.ОбменДанными.Загрузка = Истина;
		Документ[Реквизит] = ?(Документ[Реквизит],Ложь,Истина);
		Документ.Записать(РежимЗаписиДокумента.Запись);
		
	КонецЕсли;	
		
КонецПроцедуры
//\\АБС ВСТАВКА №10498 КОНЕЦ
...
Функция ОбеспечитьРегламентноеЗадание(МетаданныеРегламентноеЗадание, Идентификатор, Сообщение = "", Заголовок = "") Экспорт
	...
	Если НЕ ПустаяСтрока(Идентификатор) Тогда
		...
		Исключение
			ttk_ОбщегоНазначения.СообщитьОбОшибке(Сообщение + ?(ПустаяСтрока(Сообщение), "", ": ") + ОписаниеОшибки(), , Заголовок);
			Возврат Ложь;
		КонецПопытки;
...
Процедура УстановитьПараметрСоответствиеДатыЗапретаППД(
// +++ввв 01.04.2016 г.	
	ИмяОбъекта=Неопределено, ФлагТрассировки=Ложь
// ---ввв 01.04.2016 г.	
	) Экспорт
	
// +++ввв 01.04.2016 г.
	Если НЕ ФлагТрассировки Тогда
// ---ввв 01.04.2016 г.
		Если НЕ Константы.абс_ПроверятьДатуЗапретаППД.Получить() Тогда		
			ПараметрыСеанса.абс_СоответствиеДатыЗапертаППД = Новый ХранилищеЗначения(Неопределено, Новый СжатиеДанных(0));	
		...
		РолиПользователя  = абс_БизнесПроцессы.ПолучитьСписокДоступныхРолейПользователя(глЗначениеПеременной("глТекущийПользователь"));
		
		Если НЕ РолиПользователя.Найти(Справочники.РолиИсполнителей.РедактированиеППДБезКонтроляДатыЗапрета) = Неопределено	Тогда
			ПараметрыСеанса.абс_СоответствиеДатыЗапертаППД = Новый ХранилищеЗначения(Неопределено, Новый СжатиеДанных(0));	
...
// ---ввв 01.04.2016 г.
	КонецЕсли;
// +++ввв 01.04.2016 г.
	//АБС ВСТАВКА 000030395  31.12.2014 12:56:00  Шамов
	//настройки сеанса загружаются позже, поэтому так
	ОрганизацияКТТК = Справочники.Организации.НайтиПоНаименованию("КТТК", Истина);
	//дополнительная проверка по организация = КТТК и роль = НалоговыйКонтроль
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ
	                      ...
	                      |	И РолиИИсполнители.Роль = &НалоговыйКонтроль
	                      |
	                      |СГРУППИРОВАТЬ ПО
	                      |	РолиИИсполнители.Роль");
	Запрос.УстановитьПараметр("ТекПользователь", глЗначениеПеременной("глТекущийПользователь"));
	Запрос.УстановитьПараметр("Организация", ОрганизацияКТТК);
	Запрос.УстановитьПараметр("НалоговыйКонтроль", Справочники.РолиИсполнителей.НалоговыйКонтроль);
	РезультатЗапроса = Запрос.Выполнить();
	...
	//АБС ВСТАВКА 54865  21.01.2015 16:09:35  Шамов
	//"ВЫБРАТЬ
	//|	абс_ГраницаЗапретаППД.Организация,
	//|	абс_ГраницаЗапретаППД.ТипДокумента,
	//|	абс_ГраницаЗапретаППД.Статус,
	//|	абс_ГраницаЗапретаППД.СтатусНовый,
	//|	ВЫБОР
	//|		КОГДА &РольБухгалтерДоступна
	//|			ТОГДА КОНЕЦПЕРИОДА(абс_ГраницаЗапретаППД.ДатаБухалтерии, ДЕНЬ)
	//|		ИНАЧЕ КОНЕЦПЕРИОДА(абс_ГраницаЗапретаППД.ДатаИнициатора, ДЕНЬ)
	//|	КОНЕЦ КАК ДатаЗапрета
	//|ИЗ
	//|	РегистрСведений.абс_ГраницаЗапретаППД КАК абс_ГраницаЗапретаППД");
	"ВЫБРАТЬ
	|	абс_ГраницаЗапретаППД.Организация,
	|	абс_ГраницаЗапретаППД.ТипДокумента,
	|	абс_ГраницаЗапретаППД.Статус,
	|	абс_ГраницаЗапретаППД.СтатусНовый,
	|	ВЫБОР
	|		КОГДА абс_ГраницаЗапретаППД_Исключение.Организация ЕСТЬ NULL 
	|			ТОГДА ВЫБОР
	|					КОГДА &РольБухгалтерДоступна
	|						ТОГДА КОНЕЦПЕРИОДА(абс_ГраницаЗапретаППД.ДатаБухалтерии, ДЕНЬ)
	|					ИНАЧЕ КОНЕЦПЕРИОДА(абс_ГраницаЗапретаППД.ДатаИнициатора, ДЕНЬ)
	...
	|ИЗ
	|	РегистрСведений.абс_ГраницаЗапретаППД КАК абс_ГраницаЗапретаППД
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.абс_ГраницаЗапретаППД_Исключение КАК абс_ГраницаЗапретаППД_Исключение
	|		ПО абс_ГраницаЗапретаППД.Организация = абс_ГраницаЗапретаППД_Исключение.Организация
	|			И (абс_ГраницаЗапретаППД_Исключение.Пользователь = &ТекПользователь)");
	//АБС ВСТАВКА 54865 КОНЕЦ 
	
	ЗапросСоответствия.УстановитьПараметр("РольБухгалтерДоступна", ПараметрыСеанса.абс_РольБухгалтерииДоступна);
	//АБС ВСТАВКА 54865  21.01.2015 16:09:35  Шамов
	ЗапросСоответствия.УстановитьПараметр("ТекПользователь", глЗначениеПеременной("глТекущийПользователь"));
	//АБС ВСТАВКА 54865 КОНЕЦ 
	
	Если ИмяОбъекта=Неопределено Тогда
	ИначеЕсли ИмяОбъекта="" Тогда
	Иначе
		ЗапросСоответствия.УстановитьПараметр("ТипДока",СокрЛП(ИмяОбъекта));
		ЗапросСоответствия.Текст=ЗапросСоответствия.Текст+"
	|ГДЕ
	|	абс_ГраницаЗапретаППД.ТипДокумента=&ТипДока";
	КонецЕсли;
	
	ТаблицаСоответствия = ЗапросСоответствия.Выполнить().Выгрузить();
	
	ПараметрыСеанса.абс_СоответствиеДатыЗапертаППД = Новый ХранилищеЗначения(ТаблицаСоответствия, Новый СжатиеДанных(0));	

КонецПроцедуры
// АБС ВСТАВКА Дата запрета по статусам ППД КОНЕЦ

// АБС ВСТАВКА Гетц 05.02.2014. Проектный учет
Процедура УстановкаНастроекГраницыЗапретаПроектныйУчет() Экспорт 
	
	Если НЕ глЗначениеПеременной("абс_ПроектныйУчет") Тогда		
		ПараметрыСеанса.абс_СоответствиеДатыЗапертаПроектныйУчет = Новый ХранилищеЗначения(Неопределено, Новый СжатиеДанных(0));	
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	абс_КонтрольДатыЗапретаПроектныйУчет.Организация,
	|	абс_КонтрольДатыЗапретаПроектныйУчет.ТипДокумента,
	|	КОНЕЦПЕРИОДА(абс_КонтрольДатыЗапретаПроектныйУчет.ДатаЗапрета, ДЕНЬ) КАК ДатаЗапрета
	|ИЗ
	|	РегистрСведений.абс_КонтрольДатыЗапретаПроектныйУчет КАК абс_КонтрольДатыЗапретаПроектныйУчет";
	
	ТаблицаСоответствия = Запрос.Выполнить().Выгрузить();
	
	ПараметрыСеанса.абс_СоответствиеДатыЗапертаПроектныйУчет = Новый ХранилищеЗначения(ТаблицаСоответствия, Новый СжатиеДанных(0));
КонецПроцедуры
// АБС ВСТАВКА Гетц 05.02.2014. Проектный учет

// АБС ВСТАВКА Ограничение видимости аналитики по ЦФО
Процедура УстановитьПараметрЭтоДЗО() Экспорт
	
	ЗапросГруппы = Новый Запрос();
	
	ЗапросГруппы.Текст = 
	"ВЫБРАТЬ
	|	МИНИМУМ(ГруппыПользователейПользователиГруппы.Ссылка.абс_ЭтоДЗО) КАК абс_ЭтоДЗО
	|ИЗ
	...
	Если Выборка.Следующий() Тогда
		ПараметрыСеанса.абс_ПользовательДЗО = ?(Выборка.абс_ЭтоДЗО = NULL, Ложь, Выборка.абс_ЭтоДЗО);
	Иначе
		ПараметрыСеанса.абс_ПользовательДЗО = Ложь;
	КонецЕсли;
		
КонецПроцедуры
// АБС ВСТАВКА Ограничение видимости аналитики по ЦФО КОНЕЦ

//АБС ВСТАВКА 39249 28.02.2014 18:24:43  Шамов
...
Функция ДокументПоДоступнойОрганизации(ДокументСсылка, Организация) Экспорт
	Если ПеременнаяСодержитСвойство(ДокументСсылка, "Организация") Тогда
		Возврат ДокументСсылка.Организация = Организация;
	Иначе
		Возврат ЛОЖЬ;
	КонецЕсли;
КонецФункции
//АБС ВСТАВКА 39249 КОНЕЦ

//АБС ВСТАВКА №45268 НАЧАЛО «6 августа 2014 г.», Пополитов
Функция абс_ПрочитатьОшибкиСервера() Экспорт
	
	Попытка
		ТекстСообщенияСервера = ПараметрыСеанса.абс_АдресОшибокСервера;
	Исключение
		ТекстСообщенияСервера = "";	
	КонецПопытки;	
	
	Если не ТекстСообщенияСервера = "" Тогда
		Попытка
			абс_ЗапомнитьАдресОшибокСервера("");
		Исключение
			//
		КонецПопытки;	
	КонецЕсли;
	
	Возврат ТекстСообщенияСервера;
	
КонецФункции

Процедура абс_ЗапомнитьАдресОшибокСервера(Значение) Экспорт
	...
	Если СокрЛП(Значение) = "" Тогда
		ПараметрыСеанса.абс_АдресОшибокСервера = Значение;
	Иначе
		ПараметрыСеанса.абс_АдресОшибокСервера = ТекстСообщенияСервера + "#" + Значение;	
	КонецЕсли;

КонецПроцедуры
//\\АБС ВСТАВКА №45268 КОНЕЦ

//АБС ВСТАВКА   05.11.2014 17:44:37  Шамов
Функция ПревышенЛимитКонтрагента(СтруктураПараметров, СообщениеОбОшибке = "") Экспорт
	
	//получим организации КТТК + филиалы
	мОрганизации = Новый Массив;
	мОрганизации.Добавить(ПараметрыСеанса.абс_НастройкиСистемы.ОрганизацияКТТК);
	...
	Запрос.Текст = 
		...
		|	Справочник.Организации КАК Организации
		|ГДЕ
		|	Организации.ГоловнаяОрганизация В(&ГоловнаяОрганизация)
		|	И Организации.абс_СтатусОрганизации = ЗНАЧЕНИЕ(перечисление.абс_статусорганизации.филиал)";

	Запрос.УстановитьПараметр("ГоловнаяОрганизация", мОрганизации);

	Результат = Запрос.Выполнить();
	ВыборкаДетальныеЗаписи = Результат.Выбрать();
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		мОрганизации.Добавить(ВыборкаДетальныеЗаписи.Ссылка);
	КонецЦикла;
	
	//получим сумму счетов на оплату в статусах старше "Утверждение руководителем ЦФК"
    мСтатусы = Документы.абс_СчетНаОплату.ПолучитьСтатусыСтаршеУтвержденияРуководителемЦФК();
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СУММА(абс_СчетНаОплату.СуммаПлатежа) КАК СуммаСчетов
		|ИЗ
		|	Документ.абс_СчетНаОплату КАК абс_СчетНаОплату
		|ГДЕ
		|	абс_СчетНаОплату.Организация В(&мОрганизации)
		|	И абс_СчетНаОплату.Контрагент = &Контрагент
		|	И абс_СчетНаОплату.ДатаОплаты МЕЖДУ НАЧАЛОПЕРИОДА(&ДатаОплаты, МЕСЯЦ) И КОНЕЦПЕРИОДА(&ДатаОплаты, МЕСЯЦ)
		|	И абс_СчетНаОплату.СтатусСчета В(&мСтатусыСчетов)";

	Запрос.УстановитьПараметр("ДатаОплаты", СтруктураПараметров.ПериодПлатежа);
	Запрос.УстановитьПараметр("Контрагент", СтруктураПараметров.Контрагент);
	Запрос.УстановитьПараметр("мОрганизации", мОрганизации);
	Запрос.УстановитьПараметр("мСтатусыСчетов", мСтатусы);

	Результат = Запрос.Выполнить();

	ОбщаяСуммаСчетов = СтруктураПараметров.СуммаПлатежа;
	ВыборкаДетальныеЗаписи = Результат.Выбрать();
	ВыборкаДетальныеЗаписи.Следующий();
	Если ВыборкаДетальныеЗаписи.СуммаСчетов <> NULL Тогда
		ОбщаяСуммаСчетов = ОбщаяСуммаСчетов + ВыборкаДетальныеЗаписи.СуммаСчетов;
	КонецЕсли;
		
	//сравним ОбщаяСуммаСчетов с данными регистра лимитов
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	абс_ЛимитыПоКонтрагентам.ЛимитМесячный КАК Лимит,
		|	1 КАК Приоритет
		|ИЗ
		...
		|	РегистрСведений.абс_ЛимитыПоКонтрагентам КАК абс_ЛимитыПоКонтрагентам
		|ГДЕ
		|	абс_ЛимитыПоКонтрагентам.Контрагент = ЗНАЧЕНИЕ(Справочник.Контрагенты.Пустаяссылка)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Приоритет";
    ...
    Если НЕ Результат.Пустой() Тогда
		ВыборкаДетальныеЗаписи = Результат.Выбрать();
		ВыборкаДетальныеЗаписи.Следующий();
		Если ОбщаяСуммаСчетов > ВыборкаДетальныеЗаписи.Лимит Тогда
			СообщениеОбОшибке = "Превышен лимит оплат по контрагенту """ + СтруктураПараметров.Контрагент + """ за " + Формат(СтруктураПараметров.ПериодПлатежа, "ДФ='MMMM yyyy'") + "!" + Символы.ПС + 
				"Сумма оплат = " + Формат(ОбщаяСуммаСчетов, "ЧЦ=19; ЧДЦ=2") + "; Лимит = " + Формат(ВыборкаДетальныеЗаписи.Лимит, "ЧЦ=19; ЧДЦ=2");
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
	//нет ошибок	
...
//	Организация 	- организация, указанная в документе
//
Процедура ttk_РегистрацияОтложенногоПроведения(ДокументСсылка, Организация) Экспорт
	
	Набор = РегистрыСведений.ttk_ДокументыТребующиеДопроведения.СоздатьНаборЗаписей();
	Набор.Отбор.Регистратор.Установить(ДокументСсылка);
	Запись 				= Набор.Добавить();
	Запись.Организация 	= Организация;
	Запись.Период 		= ДокументСсылка.Дата;
...
Процедура ttk_УдалениеОтложенногоПроведения(ДокументСсылка, Организация) Экспорт
