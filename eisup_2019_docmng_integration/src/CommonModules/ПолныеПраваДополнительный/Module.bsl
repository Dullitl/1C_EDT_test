...
Процедура УстановитьРеквизитыИЗаписатьСотрудника(Выборка, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			# Иначе
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ОбщегоНазначения.ПолучитьПричинуОшибки(ИнформацияОбОшибке()).Описание);
			# КонецЕсли
			Отказ = Истина;
			Возврат;
		КонецПопытки;
...
Процедура ЗаписатьТекущиеКадровыеДанныеСотрудника(Отказ, Замещение, БезРегистратора, Регистратор) Экспорт
	...
	ФизлицаВторичныеДанныеПереопределяемый.УстановитьРеквизитыИЗаписатьФизлицо(Выборка, Отказ);
