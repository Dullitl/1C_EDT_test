...
Функция СформироватьТекстЗапросаПоБазамРаспределения()
	...
	|	МАКСИМУМ(БазаНал), МАКСИМУМ(БазаПриходРегл), МАКСИМУМ(БазаОстатокНЗПНал)
	|ПО
	|	БазаРаспределенияЗатрат.НоменклатурнаяГруппа,
	|	БазаРаспределенияЗатрат.Заказ,
	|	Продукция,
								...
								Пока ОбходПоПодразделению.Следующий() Цикл
									...
									Пока ОбходПоПодразделениюОрганизации.Следующий() Цикл
										...
										Пока ОбходПоСчетуЗатрат.Следующий() Цикл
											...
											Пока ОбходПоСчетуЗатратНУ.Следующий() Цикл
												
												Если ЗначениеЗаполнено(НоваяСтрока.СчетЗатрат) И НоваяСтрока.СчетЗатрат <> ОбходПоСчетуЗатратНУ.СчетЗатрат
												 ...
												 ИЛИ ЗначениеЗаполнено(НоваяСтрока.ПодразделениеОрганизации) И НоваяСтрока.ПодразделениеОрганизации <> ОбходПоСчетуЗатратНУ.ПодразделениеОрганизации Тогда
												...
												Пока Обход.Следующий() Цикл
													...
													// Налоговый учет.
													ИначеЕсли Обход.ИндексУчета = 1 Тогда
														НоваяСтрока.ПодразделениеОрганизации = Обход.ПодразделениеОрганизации;
														НоваяСтрока.ВариантВыпускаПродукции = Обход.ВариантВыпускаПродукции;
														НоваяСтрока.ВидВыпуска = Обход.ВидВыпуска;
														НоваяСтрока.СчетЗатратНУ = Обход.СчетЗатратНУ;
														НоваяСтрока.БазаНал = Обход.БазаНал;
														НоваяСтрока.БазаПриходНал = Обход.БазаПриходНал;
														НоваяСтрока.БазаОстатокНЗПНал = Обход.БазаОстатокНЗПНал;
