...
Функция ПолучитьРегистрыОтложенногоПроведения() Экспорт
	...
	МассивРегистров.Добавить("РегистрНакопления.ПродажиСебестоимость");
...
Функция ВыполнитьДопроведениеДокументов(СтруктураПараметров)
	...
	ttk_ОбщегоНазначения.СообщитьОбОшибке("При допроведении документов произошли ошибки!",,Заголовок);
	Для каждого Элемент из мСоответствиеОшибокПриДопроведении цикл
		СтрокаСообщения = "" + ttk_ОбщегоНазначения.ПредставлениеДокументаПриПроведении(Элемент.Ключ) + 
									Символы.ПС+"Допроведение документа не выполнено: " + Элемент.Значение; 
		ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения,,Заголовок,,Элемент.Ключ);
	КонецЦикла;
	Если мДопроведениеПрекращено Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Получено 1000 сообщений об ошибках. После этого допроведение было прекращено",, Заголовок);
	КонецЕсли;
	Возврат Ложь;
КонецФункции
...
Процедура ВыполнитьДопроведениеДокументовПоСпискуОрганизаций(ТаблицаОрганизаций, ПараметрыДопроведения) Экспорт
	...
	Для Каждого СтрокаТаблицы ИЗ ТаблицаОрганизаций Цикл
		...
		Иначе
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения, , ЗаголовокСообщ);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
//Параметры: СтруктураШапкиДокумента - структура шапки документа
//				ДокументСсылка - ссылка на документ, передается если на момент вызова не известна СтруктураШапкиДокумента
Функция ttk_ИспользуетсяОтложенноеПроведение(СтруктураШапкиДокумента = Неопределено, ДокументСсылка = Неопределено) Экспорт
	...
	Запрос.Текст ="ВЫБРАТЬ
	|	ttk_НастройкаОтложенногоПроведенияДокументовСрезПоследних.ОтложенноеПроведениеВключено
	|ИЗ
	|	РегистрСведений.ttk_НастройкаОтложенногоПроведенияДокументов.СрезПоследних(
	|			&Дата,
	|			Организация = &Организация
	|				И ВидДокумента = &ВидДокумента) КАК ttk_НастройкаОтложенногоПроведенияДокументовСрезПоследних";
	...
	Если Выборка.Следующий() Тогда
		...
		Возврат Выборка.ОтложенноеПроведениеВключено;
...
// Функция возвращает признак того, что документ используется в отложенном проведении
//
Функция ttk_ДокументПодерживаетОтложенноеПроведение(ДокументОбъект, СтруктураШапкиДокумента = Неопределено) Экспорт
	//Используется ли вообще отложенное проведение по дате и организации, указанным в документе
	Если НЕ ИспользуетсяОтложенноеПроведение(СтруктураШапкиДокумента,ДокументОбъект) Тогда
		Возврат Ложь;
	КонецЕсли;

	Возврат ДокументОбъект.Метаданные().Движения.Содержит(РегистрыСведений.ttk_ДокументыТребующиеДопроведения.СоздатьНаборЗаписей().Метаданные());
...
//Возвращаемое значение: булево (Истина - проведен полностью, Ложь - не проведен полностью)
//Вызывается из процедуры НастройкаПравДоступа.ПроверитьВозможностьОтраженияВРеглУчете
Функция ttk_ПроверитьДокументПроведенПолностью(ДокументСсылка) Экспорт
	
	Если не ДокументСсылка.Проведен Тогда
		Возврат Ложь;
	КонецЕсли;
	Если НЕ ttk_ДокументПодерживаетОтложенноеПроведение(ДокументСсылка) Тогда
		Возврат Истина;
	КонецЕсли;
	...
	//Вызовем функцию проверки статуса для таблицы документов
	РезультатЗапроса = ttk_ПроверитьДокументыПроведеныПолностью(МассивДокументов, новый ОписаниеТипов(МассивТипов));
	//В результате всегда будет одна строка
	ВыборкаРезультат = РезультатЗапроса.Выбрать();
	ВыборкаРезультат.Следующий();
	Возврат ВыборкаРезультат.ПроведенПолностью;
...
//Возвращаемое значение: выборка из результата запроса с полями Документ (ссылка), Проведен (булево), ПометкаУдаления (булево) и ПроведенПолностью (булево)
//Используется в журнале ОтложенноеПроведениеДокументов и в функции ПроверитьДокументПроведенПолностью
Функция ttk_ПроверитьДокументыПроведеныПолностью(МассивДокументов, ОписаниеТиповДокументов) Экспорт
	//Создадим таблицу значений со списком документов
	ТаблицаДокументов = новый ТаблицаЗначений;
	ТаблицаДокументов.Колонки.Добавить("Документ",ОписаниеТиповДокументов);
	Для каждого Элемент из МассивДокументов цикл
	...
	|ИЗ ВременнаяТаблицаДокументов
	|ЛЕВОЕ СОЕДИНЕНИЕ 
	|	РегистрСведений.ttk_ДокументыТребующиеДопроведения 			КАК ДокументыТребующиеДопроведения
	|	ПО ДокументыТребующиеДопроведения.Регистратор = ВременнаяТаблицаДокументов.Документ";
	Результат = Запрос.Выполнить();
	Возврат Результат;
КонецФункции
