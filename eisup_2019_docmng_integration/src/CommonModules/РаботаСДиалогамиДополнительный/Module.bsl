...
Функция ПолучитьПредставлениеСпособаОтраженияНачисленияВУчетах(СпособОтраженияВБухучете, НетБазовыхРасчетов) Экспорт
	...
	Иначе
		...
		|	СпособыОтраженияЗарплатыВРеглУчете.РасходыУСН,
		...
		|		СпособыОтраженияЗарплатыВРеглУчете.РасходыУСН КАК РасходыУСН
		|	ИЗ
		...
		Если ПустаяСтрока(Выборка.СчетКт) и Выборка.РасходыУСН.Пустая() Тогда
			РасшифровкаТекст = РасшифровкаТекст + "определяется по расчетной базе";
		Иначе
			РасшифровкаТекст = РасшифровкаТекст + ?(Выборка.РасходыУСН = Перечисления.ОтражениеВУСН.Принимаются,"","не ") + "принимается как расходы";
		КонецЕсли;
	КонецЕсли;
