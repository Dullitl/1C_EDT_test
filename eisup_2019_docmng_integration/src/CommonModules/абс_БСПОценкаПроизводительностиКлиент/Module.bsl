...
Процедура ЗаписатьРезультатыАвтоНеГлобальный(ПередЗавершением = Ложь) Экспорт
	Если НЕ ОценкаПроизводительностиЗамерВремени = Неопределено Тогда
		...
		Иначе		
			...
			НовыйПериодЗаписи = абс_БСПОценкаПроизводительностиВызовСервераПолныеПрава.ЗафиксироватьДлительностьКлючевыхОпераций(Замеры);		 
			...
			КонецЦикла;			
		КонецЕсли;
		ПодключитьОбработчикОжидания("абс_БСПЗаписатьРезультатыАвто", НовыйПериодЗаписи, Истина);
...
Процедура ЗакончитьЗамерВремени(АвтоЗамер = Истина)
	...
	ВремяОкончания = абс_БСПОценкаПроизводительностиКлиентСервер.ЗначениеТаймера();
		
	Если АвтоЗамер И ТипЗнч(ВремяОкончания) = Тип("Число") Тогда
		ВремяОкончания = ВремяОкончания - 0.100;					
	КонецЕсли;	
    ДатаОкончания = абс_БСПОценкаПроизводительностиКлиентСервер.ЗначениеТаймера(Ложь);	
	Замеры = ОценкаПроизводительностиЗамерВремени["Замеры"];
