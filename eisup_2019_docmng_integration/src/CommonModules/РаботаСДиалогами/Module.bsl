...
Процедура ПриИзмененииОрганизацияЯвляетсяПлательщикомЕНВД(ДокументОбъект,
	...
	ИначеЕсли МетаданныеДокумента.Реквизиты.Найти("ЗаказПокупателя") <> Неопределено Тогда
	...
	ПараметрыУчетнойПолитикиРегл = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(Дата, НоваяОрганизация, Ложь);
	...
	Иначе
		НовыйОрганизацияЯвляетсяПлательщикомЕНВД = Ложь;
	КонецЕсли;
...
Процедура НачалоВыбораЗначенияСделки(ДокументОбъект, ФормаДокумента, ЭлементФормы, ДоговорКонтрагента, Сделка,
	...
	Если (СтрокаТипаЗаказа = "ЗаказПоставщику") или (СтрокаТипаЗаказа = "ЗаказПокупателя") Тогда
		...
		// Овчинников А.В.  23.01.12 
		// В нашем случае в качестве сделки будет использоваться  только <абс_СчетНаОплату>, дублирование излишне. 
		//ТипДокументаСчет = ?(СтрокаТипаЗаказа = "ЗаказПоставщику","СчетНаОплатуПоставщика","СчетНаОплатуПокупателю"); 
		ТипДокументаСчет = ?(СтрокаТипаЗаказа = "ЗаказПоставщику","абс_СчетНаОплату","СчетНаОплатуПокупателю"); 
		// Овчинников А.В.  23.01.12  
	...
	// Овчинников А.В.  23.01.12     
	// В форме выбора документа <абс_СчетНаОплату>, наименование отбора отличается от типового 
	// 
	...
	Если ТипДокументаСделки = "абс_СчетНаОплату" Тогда 
		...
		Если НЕ ДокументОбъект.Метаданные().Реквизиты.Найти("абс_ЗакупочныйЗаказ") = Неопределено Тогда
			Если ЗначениеЗаполнено(ДокументОбъект.абс_ЗакупочныйЗаказ) Тогда
				ФормаВыбора.Отбор.ЗакупочныйЗаказ.Значение = ДокументОбъект.абс_ЗакупочныйЗаказ;
				ФормаВыбора.Отбор.ЗакупочныйЗаказ.Использование = Истина;
			КонецЕсли;
		КонецЕсли;	
	Иначе         
		ФормаВыбора.Отбор.ДокументыПоДоговоруКонтрагента.Значение      = ДоговорКонтрагента; 
...
Процедура НачалоВыбораЗначенияДокументаРасчетовСКонтрагентом(ДокументОбъект, ФормаДокумента, ЭлементФормы, ДоговорКонтрагента, Сделка,
	                          СтандартнаяОбработка, СтрокаВидаДвижения,абс_Дата = '00010101') Экспорт
	...
	// {{KM WARE Агапов Н.А. Заявка №34608 12.10.2015 начало
	//Если (ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПлатежноеПоручениеВходящее") Или ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РеализацияТоваровУслуг")) И БухгалтерскийУчетРасчетовСКонтрагентами.ПолучитьИспользованиеРаспределенияПлатежей(ДокументОбъект, ДокументОбъект.ДоговорКонтрагента) Тогда 
	...
	//КонецЕсли;
	Если (ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПлатежноеПоручениеВходящее") Или ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РеализацияТоваровУслуг")) И БухгалтерскийУчетРасчетовСКонтрагентами.ПолучитьИспользованиеРаспределенияПлатежей(ДокументОбъект, ДокументОбъект.ДоговорКонтрагента) 
		или ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.КорректировкаДолга") Тогда 
		ВестиПоДокументамРасчетовСКонтрагентом = Истина;
	Иначе 
		ВестиПоДокументамРасчетовСКонтрагентом = ДоговорКонтрагента.ВестиПоДокументамРасчетовСКонтрагентом;
	КонецЕсли;
	
	// }}KM WARE Агапов Н.А. Заявка №34608 12.10.2015 окончание
	//***** Гетц\\
	Если ВестиПоДокументамРасчетовСКонтрагентом Тогда //***** Гетц. Разнесение платежей. 26.10.12// ДоговорКонтрагента.ВестиПоДокументамРасчетовСКонтрагентом Тогда
...
// ---ввв 23.12.2015 г.				
		...
		Иначе
			...
			ИначеЕсли ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПоставщиком
				...
				СписокТипов.Добавить(Метаданные.Документы["ПлатежноеТребованиеПолученное"].Имя,         Метаданные.Документы["ПлатежноеТребованиеПолученное"].Представление());
				СписокТипов.Добавить(Метаданные.Документы["ПлатежныйОрдерСписаниеДенежныхСредств"].Имя, Метаданные.Документы["ПлатежныйОрдерСписаниеДенежныхСредств"].Представление());
                //АБС ВСТАВКА №35267 НАЧАЛО
				СписокТипов.Добавить(Метаданные.Документы["ПоступлениеТоваровУслуг"].Имя,               Метаданные.Документы["ПоступлениеТоваровУслуг"].Представление());
				//\\АБС ВСТАВКА №35267 КОНЕЦ
		...
		// {{ТТК Лапин А. Заявка №000032543 19.10.2015 начало
        ...
        // }}ТТК Лапин А. Заявка №000032543 19.10.2015 окончание
	...
	Если ТипДокументаРасчетов = "ДокументРасчетовСКонтрагентом" Тогда
		...
		//АБС
		Если ЗначениеЗаполнено(абс_Дата) Тогда
			СтруктураОтбора.Вставить("Дата",               абс_Дата);	
		Иначе
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ДокументОбъект.Ссылка) Тогда 
			ФормаВыбора.Отбор.Дата.ЗначениеПо = ДокументОбъект.Дата;
		Иначе
			ФормаВыбора.Отбор.Дата.ЗначениеПо = абс_Дата;
		КонецЕсли;
		
		//АБС ВСТАВКА №15420 НАЧАЛО
...
Процедура НачалоВыбораЗначенияСправочникаСоставногоТипа(Объект, ФормаОбъекта, ЭлементФормы, 
	...
	Иначе
		...
		Для Каждого ЭлементМассива Из МассивТипов Цикл
			...
			Если НЕ ОграничениеТипов = Неопределено Тогда
			 	Если (ТипЗнч(ОграничениеТипов) = Тип("Массив") И ОграничениеТипов.Найти(ЭлементМассива) = Неопределено)
			 	 ИЛИ (ТипЗнч(ОграничениеТипов) = Тип("ОписаниеТипов") И ОграничениеТипов.Типы().Найти(ЭлементМассива) = Неопределено) Тогда
					// Тип не вписывается в полученные ограничения
					Продолжить;
				КонецЕсли; 
			КонецЕсли; 
...
КонецПроцедуры // НачалоВыбораЗначенияСправочникаСоставногоТипа()

// Романова 30-06-16
//
Процедура НачалоВыбораЗначенияНазначениеИспользования(ЭлементФормы, Организация = Неопределено, Номенклатура = Неопределено, СтандартнаяОбработка) Экспорт

	СтандартнаяОбработка = Ложь;
	ФормаВыбора          = Справочники.НазначенияИспользования.ПолучитьФормуВыбора(,ЭлементФормы,);
	...
	Если НЕ Организация = Неопределено Тогда
		...
		СписокСпособовОтраженияРасходовПоАмортизации = Новый СписокЗначений;
		
		Запрос = Новый Запрос;
		//Запрос.УстановитьПараметр("абс_Статус", Перечисления.абс_СтатусыСпособыОтраженияАмортизации.Использование);
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |ГДЕ
		               |	СпособыОтраженияРасходовПоАмортизации.Организация В(&Организация)
		               //|	И СпособыОтраженияРасходовПоАмортизации.абс_Статус = &абс_Статус
					   |	";
					   
		Выборка = Запрос.Выполнить().Выбрать();
		
		Пока Выборка.Следующий() Цикл 
			СписокСпособовОтраженияРасходовПоАмортизации.Добавить(Выборка.Ссылка);
		КонецЦикла;			
		
		ФормаВыбора.СправочникСписок.Отбор.СпособОтраженияРасходов.ВидСравнения = ВидСравнения.ВСписке;
		ФормаВыбора.СправочникСписок.Отбор.СпособОтраженияРасходов.Значение         = СписокСпособовОтраженияРасходовПоАмортизации;
...
// Процедура изменяет возможность сортировки для колонок динамического списка обычной формы
//
// Параметры
//  ЭлементФормыТабличноеПоле - элемент управления, связанный с динамическим списком.
//  СписокИменКолонок 		  - список имен колонок, для которых нужно изменить возможность сортировки.
//						        Допустимо передавать Массив, СписокЗначений или строку имен разделенных запятыми.
//  ДоступностьСортировки 	  - устанавливаемый режим, Истина - по колонкам разрешена сортировка, Ложь - запрещена.
//                              Допустимо передавать любое значение, для которого возможно преобразование к типу Булево.
//  Примечание: сортировку можно разрешить только по колонкам, связанным с реквизитами не составного типа.
Процедура УстановитьДоступностьСортировкиКолонок(ЭлементФормыТабличноеПоле, СписокИменКолонок, ДоступностьСортировки = Истина) Экспорт
	
	ТипСпискаИменКолонок = ТипЗнч(СписокИменКолонок);
	
	Если 	  ТипСпискаИменКолонок = Тип("СписокЗначений") Тогда
		мИменКолонок = СписокИменКолонок.ВыгрузитьЗначения();
	ИначеЕсли ТипСпискаИменКолонок = Тип("Массив") Тогда
		мИменКолонок = СписокИменКолонок;
	ИначеЕсли ТипСпискаИменКолонок = Тип("Строка") Тогда
		мИменКолонок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СписокИменКолонок, ",");
	Иначе
		Возврат;
	КонецЕсли;
	
	Попытка
		НастройкаПорядка = ЭлементФормыТабличноеПоле.НастройкаПорядка;
		ДоступностьСортировки = Булево(ДоступностьСортировки);
	Исключение
		Возврат;
	КонецПопытки;
	
	Для Каждого ИмяКолонки Из мИменКолонок Цикл
		ЭлементПорядка = НастройкаПорядка.Найти(СокрЛП(ИмяКолонки));
		Если ЭлементПорядка <> Неопределено Тогда
			ЭлементПорядка.Доступность = ДоступностьСортировки;
		КонецЕсли;
	КонецЦикла;

КонецПроцедуры // УстановитьДоступностьСортировки()
...
//
Функция ПолучитьТекстСчетаФактуры(СчетФактура, ДокументОснование = Неопределено, Гиперссылка = Неопределено) Экспорт
	...
	Иначе
		Если ДокументОснование <> Неопределено 
			И (СчетФактура = ДокументОснование.Ссылка 
			...
			Гиперссылка = Ложь;
			Если ДокументОснование.Метаданные().Реквизиты.Найти("ИспользоватьДокументРасчетовКакСчетФактуру") <> Неопределено
		...
		Иначе // Если ПодразделениеОрганизации заполнено.
			...
			Если НЕ Субконто.Владелец = Организация Тогда
				...
				Иначе // Выбрана не основная организация. ПодразделениеОрганизации нужно очистить.
					...
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Значение в поле ""Подразделение"" не соответствует выбранной организации и было очищено.");
				КонецЕсли;
...
Функция ЗаписатьНовыйОбъектВФорме(ФормаОбъекта) Экспорт
	...
	Если ФормаОбъекта.ЭтоНовый() Тогда
		...
		Иначе
			...
			Исключение
				Сообщить(ttk_ОбщегоНазначения.СформироватьТекстСообщения(ОписаниеОшибки()));
				Возврат Ложь;
			КонецПопытки;
		КонецЕсли;
	Иначе
...
Процедура УдалитьКнопкуЭД(Кнопки, ИспользоватьОбменЭД = Неопределено) Экспорт
	...
	Если Кнопки.Найти("ЭД") <> Неопределено Тогда
		Кнопки.Удалить(Кнопки.ЭД);
	КонецЕсли;
	
	Если Кнопки.Найти("РазделительЭД") <> Неопределено Тогда
...
Функция ПроверитьМодифицированностьДокументаОснования(Документ) Экспорт
	...
	Если ЗначениеЗаполнено(Документ) Тогда
		...
		Если ФормаДокумента.Модифицированность() Тогда
			ОчиститьСообщения();
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Документ " + СокрЛП(Документ) + " был изменен! Сначала следует записать документ!");
			Результат = Истина;
		КонецЕсли;
	КонецЕсли;
...
//
Функция ПроверитьМодифицированностьДокумента(Документ)
...
//
Функция ПроверитьМодифицированностьОбъекта(Объект)
...
Процедура ОткрытьСвойстваДокумента(ДокументОбъект, ФормаДокумента) Экспорт
	...
	Форма.НазначениеСвойств      = ttk_ОбщегоНазначения.ПолучитьСписокНазначенийСвойствКатегорийОбъектовПоСсылке(ДокументОбъект.Ссылка);
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ ОБРАБОТКИ СВОЙСТВ И КАТЕГОРИЙ СПРАВОЧНИКОВ

// Процедура выполняет открытие формы работы со свойствами справочника
//
// Параметры:
//  СправочникСсылка - ссылка на элемент справочника,
//  ЭтаФорма - форма-владелец формы работы со свойствами справочника
//
Процедура ОткрытьСвойстваСправочника(СправочникСсылка, ЭтаФорма) Экспорт
	...
	Форма.ОбъектОтбораЗначений   = СправочникСсылка;
	Форма.ОбъектОтбораНазначений = СправочникСсылка;
...
// Процедура перезаполняет свойства и значения элемента справочника на форме
//
// Параметры:
//  СправочникСсылка - ссылка на элемент справочника,
//  ЗначенияСвойствОбъекта - обработка-объект "ЗначенияСвойствОбъекта" (реквизит формы)
//  РодительВладелец - передается в двух случаях: 
//					   1) родитель элемента СправочникСсылка, нужно передавать если процедура вызывается
//					   из формы нового, еще не записанного элемента иерархического справочника 
//                     чтобы прочитались свойства, назначенные элементам вверх по иерархии.
//					   2) владелец элемента СправочникСсылка, нужно передавать если назначение задается 
//					   не для элементов самого справочника, а для элементов справочника-владельца, 
//					   например Номенклатура+ХарактеристикиНоменклатуры.
//
Процедура ОбновитьСвойстваСправочника(СправочникСсылка, ЗначенияСвойствОбъекта, РодительВладелец = Неопределено) Экспорт
	
	Если СправочникСсылка = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если РодительВладелец <> Неопределено Тогда
		Если ТипЗнч(СправочникСсылка) <> ТипЗнч(РодительВладелец) Тогда
			ОбъектОтбораНазначений = РодительВладелец;
		Иначе
			Если СправочникСсылка.Пустая() Тогда
				ОбъектОтбораНазначений = РодительВладелец;
			Иначе
				ОбъектОтбораНазначений = СправочникСсылка;
			КонецЕсли;
		КонецЕсли;
	Иначе
		ОбъектОтбораНазначений = СправочникСсылка;
	...
	ЗначенияСвойствОбъекта.ОбъектОтбораЗначений   = СправочникСсылка;
	ЗначенияСвойствОбъекта.ОбъектОтбораНазначений = ОбъектОтбораНазначений;
...
//
//
Процедура ВидимостьРеквизитовПоТипуУчета(ДокументОбъект, ФормаДокумента, Знач РеквизитыУпрУчета, Знач РеквизитыБухУчета, Знач РеквизитыНалУчета = "", Знач РеквизитыМеждУчета = "") Экспорт
	...
	Если НЕ ПустаяСтрока(СтрРеквНал) Тогда
	...
	Иначе
		ОтражатьВНалоговомУчетеНП = ДокументОбъект.ОтражатьВНалоговомУчете;
	Конецесли;
	...
	Пока Не ПустаяСтрока(СтрРеквНал) Цикл
		...
		Иначе
			ФормаДокумента.ЭлементыФормы[Лев(ИмяРекв, Поз - 1)].Колонки[Сред(ИмяРекв, Поз + 1)].Видимость = ОтражатьВНалоговомУчетеНП;
		КонецЕсли;
...
Процедура ПриВыбореСчета(Счет, Субконто1, НадписьСубконто1, Субконто2, НадписьСубконто2, Субконто3, НадписьСубконто3, ПолеСчет = НЕОПРЕДЕЛЕНО, ТолькоПросмотр = ЛОЖЬ) Экспорт
	...
	Для Сч = 1 По ЧислоАктивныхСубконто Цикл
		...
		//АБС ВСТАВКА   02.02.2015 21:36:08  Шамов
		//ЭлементыСубконто["Субконто" + Сч].ВыбиратьТип = Ложь;
		Если ТипЗнч(ЭлементыСубконто["Субконто" + Сч]) = Тип("ПолеВвода") Тогда
			ЭлементыСубконто["Субконто" + Сч].ВыбиратьТип = (ТипСубк.Типы().Количество() > 1);
			ЭлементыСубконто["Субконто" + Сч].КнопкаОчистки = (ТипСубк.Типы().Количество() > 1);	
		Иначе
			ЭлементыСубконто["Субконто" + Сч].ЭлементУправления.ВыбиратьТип = (ТипСубк.Типы().Количество() > 1);
		КонецЕсли; 
		//АБС ВСТАВКА  КОНЕЦ 
...
Процедура УстановитьВидимостьТекста(Документ, Ид, ВключитьВидимость, Параметр = "inline") Экспорт
	...
	Если Документ.readyState <> "complete" Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Справка: справка еще не загружена");
		Возврат;
	КонецЕсли;
	...
	Если Элемент = Неопределено Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Справка: не найден элемент " + Ид);
		Возврат;
	КонецЕсли;
...
Процедура ВывестиОкноСПредупреждениемОбИзменении(НазваниеФормы, КлючФормы) Экспорт 
	
	Если ХранилищеОбщихНастроек.Загрузить(НазваниеФормы, КлючФормы) = Истина Тогда
		Возврат;
	КонецЕсли;
	...
	// Версия 1.3.33
	// Предупреждение выдается при установке в документе "Передача ОС" с 01.01.13 при установке флага восстановления амортизационной премии
	Если  КлючФормы = "ПоказыватьВосстановлениеАмортПремии2013" Тогда
		СсылкаНаИТС   = "http://its.1c.ru/bmk/bp20/vosst_amor_prem_2013";
		НазваниеНаИТС   = "Восстановление амортизационной премии в 2013 году";
		ОписаниеПредупреждения = НСтр("ru = 'Начиная с 2013 г., амортизационная премия восстанавливается только при реализации ОС взаимозависимому лицу. Теперь восстановление амортизационной премии не увеличивает базу по налогу на прибыль.'");
	КонецЕсли;	
	// Предупреждение выдается при установке в документе "Передача ОС" с 01.01.13 при установке флага перехода права собственности после гос.регистрации
	Если  КлючФормы = "ПоказыватьПродажаНедвижимости2013" Тогда
		СсылкаНаИТС   = "http://its.1c.ru/bmk/bp20/real_estate_sales_2013";
		НазваниеНаИТС = "Продажа недвижимости в 2013 году";
		ОписаниеПредупреждения = НСтр("ru = 'С 2013 года в налоговом учете доходы и расходы от реализации недвижимого имущества, признаются в периоде передачи объекта, независимо от даты перехода права собственности.'");
	КонецЕсли;	
	// Предупреждение выдается при установке открытии документа "Подготовка передачи ОС" с 01.01.13
	Если  КлючФормы = "ПоказыватьПодготовкаКПередачеОС2013" Тогда
		СсылкаНаИТС   = "http://its.1c.ru/bmk/bp20/real_estate_sales_2013";
		НазваниеНаИТС = "Продажа недвижимости в 2013 году";
		ОписаниеПредупреждения = НСтр("ru = 'С 2013 г. документ ""Подготовка к передаче ОС"" не используется для передачи недвижимого имущества. Вместо него создается документ ""Передача ОС"", в котором на закладке ""Прочее"" устанавливается флаг ""Право собственности переходит после государственной регистрации""'");
	КонецЕсли;
	// Предупреждение выдается при установке группы ОС в элементе справочника ОС
	Если  КлючФормы = "ПоказыватьВлияниеГруппыОСНаНалогНаИмущество" Тогда
	    ОписаниеПредупреждения = НСтр("ru = 'В базу по налогу на имущество не включается движимое имущество, принятое к учету после 1 января 2013 года. Движимое имущество определяется по реквизиту ""Группа учета ОС""'");
	КонецЕсли;
	
	// Версия 1.3.48
	// Предупреждение выдается при открытии формы документа "Расчет налога на прибыль" с 01.01.2014
	Если  КлючФормы = "ПоказыватьОкруглениеНалогов2014" Тогда
	    ОписаниеПредупреждения = НСтр("ru = 'С 2014 года налог на прибыль исчисляется в полных рублях. 
		|С 2014 года при выполнении регламентной операции ""Расчет налога на прибыль"" сальдо счета 68.04.2 ""Расчет налога на прибыль"" переносится на счет 99.09 ""Прочие прибыли и убытки""'");
	КонецЕсли;
	// Предупреждение выдается в документе "Платежное поручение исходящее" с 01.01.2014 при изменении даты, флага "Перечисление в бюджет", если не установлена дата начала применения приказа №107н
	Если КлючФормы = "ВключитьНовыеПравилаОформленияПлатежейВБюджет2014" Тогда
		ОписаниеПредупреждения = НСтр("ru = 'Изменились правила заполнения платежных поручений в бюджет (приказ Минфина №107н от 12.11.2013).
		|
		...
		|
		|В Настройках параметров учета следует указать, с какой даты будет применяться приказ (меню ""Настройка параметров учета"", закладка ""Денежные средства"").
		|
		|Проверьте правильность настроек и правильность оформления платежного поручения.
		|'");
	КонецЕсли;	
	// Предупреждение выдается в документе "Платежное поручение исходящее" с 01.01.2014 при изменении даты, флага "Перечисление в бюджет", если установлена дата начала применения приказа №107н
	Если КлючФормы = "ПоказыватьНовыеПравилаОформленияПлатежейВБюджет2014" Тогда
		ОписаниеПредупреждения = НСтр("ru = 'В 2014 году действуют новые правила заполнения платежных поручений в бюджет.
		|
		|Проверьте правильность оформления платежного поручения.
		|'");
	КонецЕсли;
	
	
	Если ОписаниеПредупреждения <> "" Тогда 
		ФормаПредупреждения = ПолучитьОбщуюФорму("ФормаПредупреждение");
		ФормаПредупреждения.КлючФормы = КлючФормы;
		ФормаПредупреждения.НазваниеФормы = НазваниеФормы;
		ФормаПредупреждения.ОписаниеПредупреждения = ОписаниеПредупреждения;
		
		Если НазваниеНаИТС <> "" Тогда 
			ФормаПредупреждения.НазваниеНаИТС = НазваниеНаИТС;
		КонецЕсли;
		Если СсылкаНаИТС <> "" Тогда 
			ФормаПредупреждения.ССылкаНаИТС = СсылкаНаИТС;
		КонецЕсли;
...
КонецФункции // ПользовательСогласенЗаписать
...
//  ФлагИзмененияКурса             - валюта регламентированного учета
//
Процедура ПриИзмененииЗначенияВхДатыДокумента(ДокументОбъект, ВалютаРегламентированногоУчета, ФлагИзмененияКурса = Неопределено) Экспорт
	...
	ЕстьДатаВхДокумента			= (МетаданныеДокумента.Реквизиты.Найти("ДатаВходящегоДокумента") <> Неопределено);

	ДатаДокумента = ?(ЕстьДатаВхДокумента, ДокументОбъект.ДатаВходящегоДокумента, ДокументОбъект.Дата);
	...
	Если ПроверкаПоВалютеВзаиморасчетов
	   ...
	   И ЕстьКратностьДокумента Тогда
		...
		СтруктураВалюты               = МодульВалютногоУчета.ПолучитьКурсВалюты(ВалютаВзаиморасчетов, ДатаДокумента);
		НовыйКурсВзаиморасчетов       = СтруктураВалюты.Курс;
		НоваяКратностьВзаиморасчетов  = СтруктураВалюты.Кратность;
		...
		СтруктураВалюты               = МодульВалютногоУчета.ПолучитьКурсВалюты(ВалютаДокумента, ДатаДокумента);
		НовыйКурсДокумента            = СтруктураВалюты.Курс;
		НоваяКратностьДокумента       = СтруктураВалюты.Кратность;
...
//                          которые выполняют приход, и наоборот.
//
Процедура абс_НачалоВыбораЗначенияДокументаРасчетовСКонтрагентом(ДокументОбъект, ФормаДокумента, ЭлементФормы, ДоговорКонтрагента, Сделка,
	                          СтандартнаяОбработка, СтрокаВидаДвижения,абс_Дата = '00010101') Экспорт
	...
	//Если ДоговорКонтрагента.ВестиПоДокументамРасчетовСКонтрагентом Тогда
		...
		КонецЕсли;
		СписокТипов.Добавить(Метаданные.Документы["ДокументРасчетовСКонтрагентом"].Имя,                 Метаданные.Документы["ДокументРасчетовСКонтрагентом"].Представление());
		СписокТипов.Добавить(Метаданные.Документы["абс_Платежка"].Имя,                 Метаданные.Документы["абс_Платежка"].Представление());
		// Пользователь выбирает тип расчетного документа
...
Процедура ЖурналДокументовСписокПередНачаломДобавления(СписокВидовДокументов, Отказ, Копирование, Форма, Организация = Неопределено) Экспорт
	...
	Если ВыбранныйЭлемент <> НеОпределено Тогда
		РаботаСДиалогами.СоздатьПоВидуДокументаСтрокой(ВыбранныйЭлемент.Значение, Организация);
	КонецЕсли;
