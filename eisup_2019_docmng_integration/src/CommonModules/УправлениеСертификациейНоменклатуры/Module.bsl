	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Пока Обход.Следующий() Цикл
			...
			И Не Обход.СертификатВнутр = Перечисления.СостоянияСертификацииНоменклатуры.Есть Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Номенклатура """ + Обход.Номенклатура + """ серия: """ + Обход.Серия + """" + Символы.ПС + "    Нет внутренней сертификации", Отказ, Заголовок);
			КонецЕсли;
			...
			И Не Обход.СертификатВнешн = Перечисления.СостоянияСертификацииНоменклатуры.Есть Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Номенклатура: """ + Обход.Номенклатура + """ серия: """ + Обход.Серия + """" + Символы.ПС + "    Нет внешней сертификации", Отказ, Заголовок);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
//
Функция ПолучитьАттестатАккредитации(КонтрагентСсылка, Дата) Экспорт
	...
	Для каждого запись Из Набор Цикл
		Если (запись.Период <= Дата) и (запись.ДатаОкончания >= Дата) Тогда
			Возврат "Аттестат аккредитации МЗ РФ" + Символы.ПС + "№ " + СокрЛП(запись.НомерАттестата) + " от "
			+ Формат(запись.период,"ДЛФ = Д") + " до " + Формат(запись.ДатаОкончания,"ДЛФ = Д");
		КонецЕсли;
	КонецЦикла;
	Возврат "НЕТ ИНФОРМАЦИИ" + Символы.ПС + "ОБ АТТЕСТАТЕ АККРЕДИТАЦИИ!";
КонецФункции
