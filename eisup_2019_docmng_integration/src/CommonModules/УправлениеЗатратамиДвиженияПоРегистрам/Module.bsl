...
Процедура РаспределитьСуммуЗатратПоПроектамРасширеннаяАналитика(
	...
	Если ВсегоБазаРаспределения <> 0 Тогда
		...
		МассивКоличество = ttk_ОбщегоНазначения.РаспределитьПропорционально(СтрокаВыборки.КоличествоУпр, МассивКоэф, 3);
		МассивСтоимость = ttk_ОбщегоНазначения.РаспределитьПропорционально(СтрокаВыборки.СуммаУпр, МассивКоэф, 2);
...
Процедура ЗаписатьНаборыЗаписейРегистров(
	...
	// АБС Фролов Изменено
	, абс_ПогашениеСтоимости = Ложь) Экспорт
	...
	Для Каждого СтрокаСтруктуры Из СтруктураНаборыЗаписей Цикл
		...
		// АБС ВСТАВКА Фролов 
		Если абс_ПогашениеСтоимости И Найти(СтрокаСтруктуры.Ключ, "ПартииМатериаловВЭксплуатации") > 0 Тогда
			Продолжить;
		КонецЕсли;
	...
	// АБС ВСТАВКА Фролов
	// Перенесем записи в регистре в движения по регистратору = Документ движения.
	Если НЕ абс_ПогашениеСтоимости Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого СтрокаСтруктуры Из СтруктураНаборыЗаписей Цикл
		
		Если НЕ Найти(СтрокаСтруктуры.Ключ, "ПартииМатериаловВЭксплуатации") > 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		ТабДокументовДвижения = НаборЗаписей.Выгрузить();
		
		ТабДокументовДвижения.Свернуть("ДокументДвижения");
		
		Для Каждого ТекСтрокаДокументаДвижения Из ТабДокументовДвижения Цикл
			
			НаборПоДокументуДвижения = РегистрыНакопления[СтрокаСтруктуры.Ключ].СоздатьНаборЗаписей();
			
			НаборПоДокументуДвижения.Отбор.Регистратор.Установить(ТекСтрокаДокументаДвижения.ДокументДвижения);
			
			Для Каждого СтрокаОсновногоНабора Из НаборЗаписей Цикл
				
				Если НЕ СтрокаОсновногоНабора.ДокументДвижения = ТекСтрокаДокументаДвижения.ДокументДвижения Тогда
					Продолжить;
				КонецЕсли;
				
				ЗаписьПоДокументуДвижения = НаборПоДокументуДвижения.Добавить();
				
				ЗаполнитьЗначенияСвойств(ЗаписьПоДокументуДвижения, СтрокаОсновногоНабора);
				
				ЗаписьПоДокументуДвижения.Регистратор = ТекСтрокаДокументаДвижения.ДокументДвижения;
				
			КонецЦикла;
			
			НаборПоДокументуДвижения.Записать();
...
Функция СформироватьТекстЗапросаПоЗатратамПрочиеЗатраты()
	...
	// АБС ВСТАВКА Фролов 20110919 Загрузка затрат по филиалам
	// В запрос добавлена возможность получения подразделения из табличной части документа Прочие Затраты
	
	ТекстЗапроса = "ВЫБРАТЬ
	               |	ВЫБОР
	               |		КОГДА ТабличнаяЧасть.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПрочиеЗатраты.Списание)
	               ...
	               |	ТабличнаяЧасть.Ссылка.Организация КАК Организация,
	               |	ВЫБОР
	               |		КОГДА (НЕ ТабличнаяЧасть.абс_Подразделение = ЗНАЧЕНИЕ(Справочник.Подразделения.ПустаяСсылка))
	               |			ТОГДА ТабличнаяЧасть.абс_Подразделение
	               |		ИНАЧЕ ТабличнаяЧасть.Ссылка.Подразделение
	               |	КОНЕЦ КАК Подразделение,
	               |	ВЫБОР
	               |		КОГДА (НЕ ТабличнаяЧасть.абс_ПодразделениеОрганизации = ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка))
	               |			ТОГДА ТабличнаяЧасть.абс_ПодразделениеОрганизации
	               |		ИНАЧЕ ТабличнаяЧасть.Ссылка.ПодразделениеОрганизации
...
Процедура СформироватьДвиженияПоРегистрамУчетЗатрат(
	...
	Тогда
		...
		Если ХарактерЗатратБУ = Перечисления.ХарактерЗатрат.ЗаготовлениеИПриобретениеМЦ
...
Функция СформироватьТекстЗапросаПоТаблицеЗатрат()
	...
	//АБС ВСТАВКА Филиализация  26.09.2014 16:23:39  Пугачев
	ПрименятьСпособыКТТК = ?(ТипЗнч(ПараметрыСеанса.абс_НастройкиСистемы.ДатаНачалаПримененияСпособовОтраженияКТТК)=Тип("Дата"), ПараметрыСеанса.абс_НастройкиСистемы.ДатаНачалаПримененияСпособовОтраженияКТТК<ТекущаяДата(), Ложь);
	...
	//АБС ВСТАВКА Филиализация  26.09.2014 16:23:39  Пугачев
	|	ИНАЧЕ";
	Если ПрименятьСпособыКТТК Тогда
		ТекстЗапроса = ТекстЗапроса+"
		|		ВЫБОР КОГДА ТаблицаЗатрат.ПодразделениеОрганизации.Владелец=ТаблицаЗатрат.Организация ТОГДА
		|			ТаблицаЗатрат.ПодразделениеОрганизации 
		|		КОГДА ТаблицаЗатрат.ПодразделениеОрганизации.Владелец=&ОрганизацияКТТК ТОГДА
	...
	Если ПрименятьСпособыКТТК Тогда
		ТекстЗапроса = ТекстЗапроса+"
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеПодразделенийИПодразделенийОрганизаций КАК СоответствиеПодразделений
		|	ПО ТаблицаЗатрат.Подразделение = СоответствиеПодразделений.Подразделение
		|		И ТаблицаЗатрат.Организация = СоответствиеПодразделений.Организация";
	КонецЕсли;
	ТекстЗапроса = ТекстЗапроса+"
	|
	//АБС ВСТАВКА Филиализация КОНЕЦ 
...
Функция СформироватьЗапросПоТаблицеЗатрат(
	...
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	//АБС ВСТАВКА Филиализация  26.09.2014 16:25:19  Пугачев
	...
	Иначе
		Запрос.УстановитьПараметр("НДСВключенВСтоимость", Ложь);	
	КонецЕсли;
	...
	Иначе
		...
		Тогда
			...
			Если СтруктураШапкиДокумента.ПоддержкаПБУ18	Тогда
				...
				// Если временная разница не указана, то рассчитаем разницу по данным учетов.
				Если ВременнаяРазница = 0
				   И СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете
				   И (СтоимостьБУ <> 0 ИЛИ СтоимостьНУ <> 0)
				Тогда
					Если СтоимостьБУ > (ПостояннаяРазница + СтоимостьНУ) Тогда
						ВременнаяРазница = Макс(ВременнаяРазница, СтоимостьБУ - ПостояннаяРазница - СтоимостьНУ);
					Иначе
						ВременнаяРазница = Мин(ВременнаяРазница, СтоимостьБУ - ПостояннаяРазница - СтоимостьНУ);
					КонецЕсли;
				КонецЕсли;
		...
		Иначе
			...
			УправлениеПроизводствомДвиженияПоРегистрам.ПолучитьКодОперацииНезавершенноеПроизводство(СтруктураШапкиДокумента);
		КонецЕсли;
...
Функция ДополнитьСтруктуруШапкиДокументаНастройкиУчетаЗатрат(
	...
	Если ИсходнаяСтруктураШапкиДокумента.Свойство("ПериодРегистрации") Тогда
	...
	Тогда
		...
		УчетнаяПолитика = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиУпр(СтруктураШапкиДокумента.Дата, Ложь);
