...
Процедура СформироватьСписокЗадач() Экспорт
	...
	Пока ВыборкаЗапросаВидовЗадач.Следующий() Цикл
		...
		ИначеЕсли ВыборкаЗапросаВидовЗадач.ВидЗадачиПользователей = Справочники.ВидыЗадачПользователей.РасчетЕСН Тогда
			Если Год(АктуальныйПериодРегистрации) < 2010 Тогда
				ДобавитьЗадачиРасчетЕСН(АктуальныйПериодРегистрации);
			КонецЕсли;
		...
		ИначеЕсли ВыборкаЗапросаВидовЗадач.ВидЗадачиПользователей = Справочники.ВидыЗадачПользователей.РасчетСтраховыхВзносов Тогда
			Если Год(АктуальныйПериодРегистрации) >= 2010 Тогда
				ДобавитьЗадачиРасчетСтраховыхВзносов(АктуальныйПериодРегистрации);
			КонецЕсли;
			
		ИначеЕсли ВыборкаЗапросаВидовЗадач.ВидЗадачиПользователей = Справочники.ВидыЗадачПользователей.СведенияОСтавкахЕСНиПФР Тогда
			ДобавитьЗадачиСведенияОСтавкахЕСНиПФР(ВремяФормированияЗадач);
	...
	КонецЦикла;
	// Start КТТК Ермолов Е.Л.  09.06.2016 Чистка задач бизнес-процессов и уведомлений пользователям
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|	И абсЗадачаДоговора.Дата < ДОБАВИТЬКДАТЕ(&АктуальныйПериодРегистрации, МЕСЯЦ, -3)";
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Выборка.Ссылка.ПолучитьОбъект().Удалить();
	КонецЦикла;
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	абс_ОтправкаУведомленийБП.БП,
	               |	абс_ОтправкаУведомленийБП.ТочкаМаршрута,
	               |	абс_ОтправкаУведомленийБП.ДокументСсылка,
	               |	абс_ОтправкаУведомленийБП.ВидБП,
	               |	абс_ОтправкаУведомленийБП.Статус,
	               |	абс_ОтправкаУведомленийБП.СтатусНовый,
	               |	абс_ОтправкаУведомленийБП.УведомлениеОтправлено,
	               |	абс_ОтправкаУведомленийБП.ПериодРегистрацииУведомления,
	               |	абс_ОтправкаУведомленийБП.ПричинаИзмененияСтатуса,
	               |	абс_ОтправкаУведомленийБП.Ответственный,
	               |	абс_ОтправкаУведомленийБП.ОтправкаОтветственным,
	               |	абс_ОтправкаУведомленийБП.ОтправкаИнициатору
	               |ИЗ
	               |	РегистрСведений.абс_ОтправкаУведомленийБП КАК абс_ОтправкаУведомленийБП
	               |ГДЕ
	               |	абс_ОтправкаУведомленийБП.Период < ДОБАВИТЬКДАТЕ(&АктуальныйПериодРегистрации, МЕСЯЦ, -3)
	               |	И НЕ(абс_ОтправкаУведомленийБП.ПричинаИзмененияСтатуса ПОДОБНО ""%(Метка ППД)%""
	               |				ИЛИ абс_ОтправкаУведомленийБП.ПричинаИзмененияСтатуса ПОДОБНО ""%(АВИЗО)%"")";
				   //|	абс_ОтправкаУведомленийБП.УведомлениеОтправлено
				   //|	И абс_ОтправкаУведомленийБП.ПериодРегистрацииУведомления < ДОБАВИТЬКДАТЕ(&АктуальныйПериодРегистрации, МЕСЯЦ, -3)";
	Выборка = Запрос.Выполнить().Выбрать();
	
	Письмо = РегистрыСведений.абс_ОтправкаУведомленийБП.СоздатьМенеджерЗаписи();
			//ВыборкаОтправленных.ПолучитьМенеджерЗаписи().Удалить();
	Пока Выборка.Следующий() Цикл
		Письмо.Период = Выборка.Период;
		Письмо.БП = Выборка.БП;
		Письмо.ТочкаМаршрута  = Выборка.ТочкаМаршрута;
		Письмо.ДокументСсылка = Выборка.ДокументСсылка;
		Письмо.ВидБП = Выборка.ВидБП;
	    Письмо.Статус = Выборка.Статус;
		Письмо.СтатусНовый = Выборка.СтатусНовый;
		Письмо.Прочитать();
		Если Письмо.Выбран() Тогда 
			Письмо.Удалить();
		КонецЕсли;
	КонецЦикла; 
	...
	// Stop КТТК Ермолов Е.Л.  09.06.2016
...
Процедура ДобавитьЗадачиНамеченныеСобытияПоПерсоналу(ВремяФормированияЗадач, МассивОрганизаций, ЕстьЗадачиВозвратНаРаботу, ЕстьЗадачиКадровоеПеремещение, ЕстьЗадачиОценкаИспытательногоСрока, ЕстьЗадачиУвольнение)
	
	Если НЕ ЕстьЗадачиВозвратНаРаботу И НЕ ЕстьЗадачиКадровоеПеремещение И НЕ ЕстьЗадачиОценкаИспытательногоСрока И НЕ ЕстьЗадачиУвольнение Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьЗадачиРасчетЕСН(АктуальныйПериодРегистрации)
...
Процедура ДобавитьЗадачиРасчетСтраховыхВзносов(АктуальныйПериодРегистрации)
...
Процедура ДобавитьЗадачиСведенияОСтавкахЕСНиПФР(ВремяФормированияЗадач)
	
	Если Месяц(ВремяФормированияЗадач) = 12 Тогда
		// проверим заполнение ставок ЕСН на следующий год
		ГодСтавок = Год(ДобавитьМесяц(ВремяФормированияЗадач, 1));
	Иначе
		// проверим заполнение ставок на текущий год
		ГодСтавок = Год(ВремяФормированияЗадач);
	КонецЕсли;
	
	Если ГодСтавок >= 2010 Тогда
		Возврат;
	КонецЕсли;
	...
	Если ЗаполнятьСведенияОСтавках Тогда
