...
Процедура ОшибкаНетОстатка(ТекстСообщения, Остаток, Требуется, ЕИ, Отказ, Заголовок="") Экспорт
	...
	ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + Символы.ПС + Символы.Таб +
	...
	Для каждого СтрокаОбъекта Из ДеревоПолейЗапросаПоШапке.Строки Цикл
		...
		ИначеЕсли Объект = "Константы" Тогда
			Для Каждого СтрокаПоля Из СтрокаОбъекта.Строки Цикл
				...
				Если СтрокаПоля.Поле = "КурсВалютыУправленческогоУчета" Тогда
					...
					СтрокиЗапроса = СтрокиЗапроса + ",
						|	КурсыВалютСрезПоследних.Курс КАК КурсВалютыУправленческогоУчета";
						
					//Кроме курса валюты, нужно получить и кратность валюты	
					СтрокиЗапроса = СтрокиЗапроса + ",
						|	КурсыВалютСрезПоследних.Кратность КАК КратностьВалютыУправленческогоУчета";
            ...
            Если НуженКурсВалютыУпрУчета Тогда
				СтрокиЗапроса = СтрокиЗапроса + ",
					| 	ВЫБОР КОГДА КурсыВалютСрезПоследних.Период ЕСТЬ NULL 
					|		ТОГДА ИСТИНА
					|		ИНАЧЕ ЛОЖЬ
		...
		Иначе
			...
			Для Каждого СтрокаПоля Из СтрокаОбъекта.Строки Цикл
			    ...
			    Если СтрокаПоля.Поле = "ОбособленныйУчетТоваровПоЗаказамПокупателей" Тогда 
			     СтрокиЗапроса = СтрокиЗапроса + ",
			     | ИСТИНА"+
			     ?(НЕ ЗначениеЗаполнено(СтрокаПоля.Псевдоним), "", " КАК " + СокрЛП(СтрокаПоля.Псевдоним));
			    Иначе
...
Процедура ПроверитьЧтоНетТоваров(ДокументОбъект, ИмяТабличнойЧасти, ТаблицаЗначений, 
	...
	// АБС ВСТАВКА Фролов для Цвили
	РазрешенаМалоценка = Ложь;
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РеализацияТоваровУслуг") И ИмяТабличнойЧасти = "Услуги" Тогда
		
		РазрешенаМалоценка = Истина;
	КонецЕсли;
	// АБС ВСТАВКА Фролов для Цвили КОНЕЦ
	...
	Для Каждого СтрокаТаблицы Из ТаблицаЗначений Цикл		
		...
		Если СтрокаТаблицы.Услуга=ложь И ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура) Тогда  // СтрокаТаблицы.Услуга может иметь значение Неопределено в случае если не заполнена номенклатура
			...
			// АБС ВСТАВКА Фролов для Цвилий
			Если РазрешенаМалоценка И СтрокаТаблицы.Номенклатура.ВидНоменклатуры = Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Основные средства менее 40 тыс.руб.", Истина) Тогда
				Продолжить;
			КонецЕсли;
			// АБС ВСТАВКА Фролов для Цвилий КОНЕЦ
...
Процедура ПроверитьЧтоСкладВШапкеОптовый(СтруктураШапкиДокумента, Отказ, Заголовок, ИмяСклада = "Склад", ИмяВидаСклада = "ВидСклада") Экспорт
	...
	Если ВидСклада <> Перечисления.ВидыСкладов.Оптовый Тогда
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("В документе можно указывать только оптовый склад!", Отказ, Заголовок);
...
Процедура ПроверитьЧтоВСерииЗаполненНомерГТД(ДокументОбъект, ИмяТабличнойЧасти, ТаблицаЗначений, Отказ, Заголовок) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
		...
		Если СтрокаТаблицы.УчетПоСериям <> истина  //в значении СтрокаТаблицы.УчетПоСериям может быть значение null, поэтому в условии используется сравнение
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.НомерГТД) Тогда
			...
			КонецЕсли;
			ttk_ОбщегоНазначения.СообщитьОбОшибке("В строке номер """     + СокрЛП(СтрокаТаблицы.НомерСтроки) +
			       """ табличной части """ + ПредставлениеТабличнойЧасти + """: в серии не заполнен номер ГТД для импортного товара!", Ложь, Заголовок);
...
Функция СформироватьЗапросПоТабличнойЧасти(ДокументОбъект, ИмяТабличнойЧасти, СтруктураПолей,
	...
	Для Каждого Реквизит Из СтруктураПолей Цикл
		...
		Если СокрЛП(Реквизит.Ключ) = "ОбособленныйУчетТоваровПоЗаказамПокупателей" Тогда 
			ДокументРезерва = "Док."+СтрЗаменить(Реквизит.Значение,".ДоговорКонтрагента.ОбособленныйУчетТоваровПоЗаказамПокупателей","");
			
			ТекстЗапроса  = ТекстЗапроса + ",
			|" + "ВЫБОР КОГДА " +ДокументРезерва+ " Ссылка Документ.ЗаказПокупателя И " +ДокументРезерва+" <> Значение(Документ.ЗаказПокупателя.ПустаяСсылка) ТОГДА ИСТИНА ИНАЧЕ ЛОЖЬ КОНЕЦ" + 
			" КАК " + СокрЛП(Реквизит.Ключ);
		Иначе
			ТекстЗапроса  = ТекстЗапроса + ",
			|Док." + Реквизит.Значение + 
...
Функция ПолучитьТаблицуОстатковПоСериям(СкладДоговор, Организация, МассивНоменклатуры, Комиссионер = Неопределено, НТТ = Ложь, ТаблицаРезервовПоСериям=неопределено,КонДата=неопределено) Экспорт
	...
	СтруктураУП = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиУпр(ТекущаяДата(), Ложь);
...
Процедура СообщитьОНедостаткеТовара(Требуется, Свободно = 0, ВРезерве = 0, КПередаче = 0, КПолучению = 0, Отказ, Заголовок="", Склад = "", ЗаказПоставщику = "", Номенклатура = "", Характеристика = "", Серия = "", Качество = "", ЕИ = "", СтрокаСообщения = "") Экспорт
	...
	Если КПередаче > 0 Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("К передаче " + КПередаче + " " + ЕИ, , Заголовок, СтатусСообщения.Информация);
	КонецЕсли;
	Если ВРезерве > 0 Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Зарезервировано " + ВРезерве + " " + ЕИ, , Заголовок, СтатусСообщения.Информация);
	КонецЕсли;
	Если КПолучению > 0 Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("К получению "+ КПолучению + " "+ ЕИ, , Заголовок, СтатусСообщения.Информация);
	КонецЕсли;
		...
		Для Каждого СтрокаКомплект Из НайденныеКомплекты Цикл
			...
			Для Каждого СтрокаКомплектующая Из НайденныеКомплектующие Цикл
				...
				Для Сч = 1 По ТаблицаРезультат.Колонки.Количество() Цикл
					...
					ИначеЕсли ИмяКолонки = "СчетУчетаНУ" Тогда
						НоваяСтрока.Установить(Индекс, СтрокаКомплектующая.СчетУчетаНУ);
						
					ИначеЕсли ИмяКолонки = "ПринятыеСчетУчетаБУ" Тогда
						НоваяСтрока.Установить(Индекс, СтрокаКомплектующая.ПринятыеСчетУчетаБУ);
						
					ИначеЕсли ИмяКолонки = "ПринятыеСчетУчетаНУ" Тогда
						НоваяСтрока.Установить(Индекс, СтрокаКомплектующая.ПринятыеСчетУчетаНУ);
		...
		Иначе
			...
			Если ТекДокументПродажи <> Неопределено Тогда
				...
				Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СчетУчетаБУ", ДокументОбъект.Метаданные(), "СоставНабора") Тогда
					ДокументОбъект.ЗаполнитьСчетаУчетаВСтрокеТабЧастиРегл(НоваяСтрока, "СоставНабора", ДокументОбъект.ОтражатьВБухгалтерскомУчете, ДокументОбъект.ОтражатьВНалоговомУчете);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗарегистрироватьДокументВПоследовательностяхПартионногоУчета(ДокументОбъект, Дата, Организация = Неопределено, Упр=Ложь, Бух = Ложь, Нал = Ложь, СпособВеденияПартионногоУчетаПоОрганизации=Неопределено) Экспорт
...
Процедура КонтрольЛимитовВозвратнойТарыПоставщика(ДокументОбъект, СтруктураШапкиДокумента, Отказ, Заголовок) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения + Символы.ПС + Символы.Таб +
...
Функция ОпределитьДоступностьРеквизитаДокументПоступленияВВозвратах(ДокументОбъект) Экспорт
	Если ДокументОбъект.ОтражатьВУправленческомУчете Тогда
		УчетнаяПолитикаУпр = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиУпр(ДокументОбъект.Дата, Ложь);
	...
	Если ДокументОбъект.ОтражатьВБухгалтерскомУчете Тогда
		УчетнаяПолитикаРегл = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(ДокументОбъект.Дата, ДокументОбъект.Организация, Ложь);
			  ...
			  ИЛИ РежимРА = Перечисления.РежимыИспользованияРасширеннойАналитики.РегламентированныйУчетСДополнительнойАналитикой Тогда
				Если (ТипЗНЧ(Источник.Ссылка) = Тип("ДокументСсылка.ОтражениеЗарплатыВУпрУчете")
				  ...
				  И Источник.Проведен Тогда  
					...
					Заголовок = ttk_ОбщегоНазначения.ПредставлениеДокументаПриПроведении(Источник);
