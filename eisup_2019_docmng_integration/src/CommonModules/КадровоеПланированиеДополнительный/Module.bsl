...
Функция ОбновитьРабочиеМестаУпр(Подразделение, Период = Неопределено) Экспорт
...
Процедура ОбновитьДанныеПодразделения(ЭтотОбъект, ЭтаФорма) Экспорт
	...
	Если ДанныеСтроки = Неопределено Тогда
		...
		РабочиеМеста.Загрузить(КадровоеПланированиеДополнительный.ОбновитьРабочиеМестаУпр(Неопределено, ?(ЭтаФорма.РежимПериода, Неопределено, ЭтаФорма.ДатаСреза)).Выгрузить());
		ЭлементыФормы.РабочиеМеста.Колонки.Подразделение.Видимость	= ЭтаФорма.РабочиеМеста.Итог("ЕстьИерархия") > 0;
		Возврат;
	КонецЕсли;
	                      
	РабочиеМеста.Загрузить(КадровоеПланированиеДополнительный.ОбновитьРабочиеМестаУпр(ДанныеСтроки.Ссылка, ?(ЭтаФорма.РежимПериода, Неопределено, ЭтаФорма.ДатаСреза)).Выгрузить());
