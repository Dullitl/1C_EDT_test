...
Функция СохранитьФайлНаДиске(Хранилище, ИмяФайла, ТолькоЧтение, СпособПерезаписи, ВопросОПерезаписи = Истина, ИмяСправочника = "ХранилищеДополнительнойИнформации") Экспорт
	...
	Попытка
		...
		Если ТипЗнч(ДвоичныеДанные) = Тип("ИнтернетПочтовоеСообщение") Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Это вложение не может быть сохранено!");
			Возврат Ложь;
		КонецЕсли;
...
//
Процедура ОткрытьФайлИзХранилища(ИмяФайла, Хранилище, СпособПерезаписи = "", ВопросОПерезаписи = Истина)
...
Процедура ОткрытьФормуСпискаФайловИИзображений(СтруктураДляСпискаИзображений, СтруктураДляСпискаДополнительныхФайлов, ОбязательныеОтборы, ФормаВладелец, ИмяСправочника = "ХранилищеДополнительнойИнформации") Экспорт
	...
	ФормаФайлов = Справочники[ИмяСправочника].ПолучитьФорму("ФормаСписка", ФормаВладелец);
	...
	Если СтруктураДляСпискаДополнительныхФайлов.Свойство("ОтборОбъектИспользование") Тогда
		ФормаФайлов.СправочникСписок.Отбор.Объект.Использование = СтруктураДляСпискаДополнительныхФайлов.ОтборОбъектИспользование;
		ФормаФайлов.СправочникСписок.Отбор.Объект.Значение      = СтруктураДляСпискаДополнительныхФайлов.ОтборОбъектЗначение;
	КонецЕсли;
	Если СтруктураДляСпискаДополнительныхФайлов.Свойство("ОтборКлючСтроки") Тогда
		ФормаФайлов.СправочникСписок.Отбор.абс_КлючСтроки.Использование = Истина;
		ФормаФайлов.СправочникСписок.Отбор.абс_КлючСтроки.Значение = СтруктураДляСпискаДополнительныхФайлов.ОтборКлючСтроки;
	КонецЕсли;	
	Если СтруктураДляСпискаДополнительныхФайлов.Свойство("ДоступностьОтбораОбъекта") Тогда
		ФормаФайлов.ЭлементыФормы.СправочникСписок.НастройкаОтбора.Объект.Доступность = СтруктураДляСпискаДополнительныхФайлов.ДоступностьОтбораОбъекта;
	КонецЕсли; 
	Если СтруктураДляСпискаДополнительныхФайлов.Свойство("ВидимостьКолонкиОбъекта") Тогда
		ФормаФайлов.ЭлементыФормы.СправочникСписок.Колонки.Объект.Видимость = СтруктураДляСпискаДополнительныхФайлов.ВидимостьКолонкиОбъекта;
	КонецЕсли; 
	...
	//АБС ВСТАВКА №30131 НАЧАЛО «24 декабря 2014 г.», Пополитов
	// Start КТТК Ермолов Е.Л.  27.07.2015 000033858
//	Если Тип(ФормаВладелец) = Тип("Форма") и ТипЗнч(ФормаВладелец.Ссылка) = Тип("ДокументСсылка.абс_РаспоряжениеНаЗакупку") Тогда
    Попытка
	    Если Тип(ФормаВладелец) = Тип("Форма") и ТипЗнч(ФормаВладелец.Ссылка) = Тип("ДокументСсылка.абс_РаспоряжениеНаЗакупку") Тогда
		// Stop КТТК Ермолов Е.Л.  27.07.2015	
			мРолиПользователя = абс_БизнесПроцессы.ПолучитьСписокДоступныхРолейПользователя(глЗначениеПеременной("глТекущийПользователь"),ФормаВладелец.Ссылка.Организация);	
			мРолиПользователяБезОтбораПоОрганизации = абс_БизнесПроцессы.ПолучитьСписокДоступныхРолейПользователя();
			Если мРолиПользователя.Найти(Справочники.РолиИсполнителей.СогласованиеДирКСП) = Неопределено
				и мРолиПользователяБезОтбораПоОрганизации.Найти(Справочники.РолиИсполнителей.СуперПользователь) = Неопределено Тогда
				ФормаФайлов.ЭлементыФормы.ДействияФормы.Кнопки.ЗагрузитьФайл.Доступность = Ложь;	
			КонецЕсли;	
		КонецЕсли;	
	Исключение
	КонецПопытки;
	//\\АБС ВСТАВКА №30131 КОНЕЦ   	
	
	ФормаФайлов.Открыть();

КонецПроцедуры

// {{ТТК Сладков А. Заявка №7754431  02.11.2016 начало
// Процедура открывает форму файлов и изображений по объекту отбора
//
Процедура ОткрытьФормуСпискаФайловИИзображенийМодально(СтруктураДляСпискаИзображений, СтруктураДляСпискаДополнительныхФайлов, ОбязательныеОтборы, ФормаВладелец, ИмяСправочника = "ХранилищеДополнительнойИнформации") Экспорт
...
//	Если Тип(ФормаВладелец) = Тип("Форма") и ТипЗнч(ФормаВладелец.Ссылка) = Тип("ДокументСсылка.абс_РаспоряжениеНаЗакупку") Тогда
	...
	ФормаФайлов.ОткрытьМодально();

КонецПроцедуры
// }}ТТК Сладков А. Заявка №7754431  02.11.2016 окончание
