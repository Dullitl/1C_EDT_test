...
Процедура УправлениеПометкамиКнопокЗаголовковКоманднойПанели(ФормаОтчета, Знач ПоказыватьЗаголовок) Экспорт
	...
	//АБС ВСТАВКА №000028252 НАЧАЛО «14 ноября 2014 г.», Пополитов
	Структура = Новый Структура;
	ОбъектМетаданные = ФормаОтчета.ЭтотОбъект.Метаданные();
	Структура.Вставить("Заголовок",ОбъектМетаданные.Синоним);    
	Структура.Вставить("ИмяОбъекта",ОбъектМетаданные.Имя);  
	Структура.Вставить("ТипОбъекта","Отчет");         
	Структура.Вставить("Картинка",БиблиотекаКартинок.Отчет);
 	абс_СлужебныеФункции.абс_ИсторияОткрытияОбъектовЗапись(ЗначениеВСтрокуВнутр(Структура));
 	//\\АБС ВСТАВКА №000028252 КОНЕЦ	
