...
Функция ПериодЗаписи() Экспорт
	ТекущийПериод = Константы.абс_БСПОценкаПроизводительностиПериодЗаписи.Получить();
...
Процедура ЭкспортОценкиПроизводительности(КаталогиЭкспорта) Экспорт
	...
	Если Не абс_БСПОценкаПроизводительностиВызовСервераПовтИсп.ВыполнятьЗамерыПроизводительности() Тогда
	    Возврат;	
	КонецЕсли;
	...
	|ИЗ 
	|	РегистрСведений.абс_БСПЗамерыВремени КАК Замеры";
...
Функция УстановитьАвтоматическоеОбновлениеКлючевыхОперацийВРИБ() Экспорт
	//Справочники.абс_БСПКлючевыеОперации.УстановитьОбновлениеПредопределенныхДанных(ОбновлениеПредопределенныхДанных.ОбновлятьАвтоматически);
КонецФункции
...
Процедура ЗафиксироватьДлительностьКлючевойОперации(
	...
	Запись = РегистрыСведений.абс_БСПЗамерыВремени.СоздатьМенеджерЗаписи();
...
Процедура СформироватьЗапросРасчетаAPDEX(Запрос)
	...
	ПустаяВыборка = 
	"ВЫБРАТЬ	
	|	ЗНАЧЕНИЕ(Справочник.абс_БСПКлючевыеОперации.ПустаяСсылка),
	|	0";
	...
	|	И КлючевыеОперации.Ссылка <> ЗНАЧЕНИЕ(Справочник.абс_БСПКлючевыеОперации.ОбщаяПроизводительностьСистемы)";
	...
	|	Справочник.абс_БСПКлючевыеОперации КАК КлючевыеОперации
	|ГДЕ
	|	КлючевыеОперации.Ссылка <> ЗНАЧЕНИЕ(Справочник.абс_БСПКлючевыеОперации.ОбщаяПроизводительностьСистемы)
	|	И КлючевыеОперации.ПометкаУдаления = ЛОЖЬ";
...
Процедура ВыгрузитьРезультаты(КаталогиЭкспорта, ВыборкаАпдекс, МассивыЗамеров)
	...
	Для Каждого КлючВыполнятьКаталог Из КаталогиЭкспорта Цикл		
		...
		Если Ключ = абс_БСПОценкаПроизводительностиКлиентСервер.ЛокальныйКаталогЭкспортаКлючЗадания() Тогда
...
Функция ЗаполнитьКэшКлючевыхОпераций()
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	абс_БСПКлючевыеОперации.Ссылка,
	                      |	абс_БСПКлючевыеОперации.НачальныйСтатус,
	                      |	абс_БСПКлючевыеОперации.КонечныйСтатус
	                      |ИЗ
	                      |	Справочник.абс_БСПКлючевыеОперации КАК абс_БСПКлючевыеОперации
	                      |ГДЕ
	                      |	НЕ абс_БСПКлючевыеОперации.Предопределенный
	                      |	И НЕ абс_БСПКлючевыеОперации.ПометкаУдаления");
	
	ТаблицаПереходов = Запрос.Выполнить().Выгрузить();
	ОперацииПереходаКСтатусу = ТаблицаПереходов.Скопировать();
	
	СтрокиДляУдаленияТП = Новый Массив;
	СтрокиДляУдаленияОП = Новый Массив;
	ПоследнийИндекс = ТаблицаПереходов.Количество()-1;
	
	Для Сч = 0 По ПоследнийИндекс Цикл
		
		СтрокаТП = ТаблицаПереходов[Сч];
		СтрокаОП = ОперацииПереходаКСтатусу[Сч];
		
		Если СтрокаТП.КонечныйСтатус=Неопределено Или СтрокаТП.КонечныйСтатус=NULL Или Не ЗначениеЗаполнено(СтрокаТП.КонечныйСтатус) Тогда
			СтрокиДляУдаленияТП.Добавить(СтрокаТП);
			СтрокиДляУдаленияОП.Добавить(СтрокаОП);
		ИначеЕсли СтрокаТП.НачальныйСтатус=Неопределено Или СтрокаТП.НачальныйСтатус=NULL Или Не ЗначениеЗаполнено(СтрокаТП.НачальныйСтатус) Тогда
			СтрокиДляУдаленияТП.Добавить(СтрокаТП);
		Иначе
			СтрокиДляУдаленияОП.Добавить(СтрокаОП);
		КонецЕсли;
	...
	Для Каждого СтрокаТП Из СтрокиДляУдаленияТП Цикл
		ТаблицаПереходов.Удалить(СтрокаТП);
	КонецЦикла;
	
	Для Каждого СтрокаОП Из СтрокиДляУдаленияОП Цикл
		ОперацииПереходаКСтатусу.Удалить(СтрокаОП);
	КонецЦикла;
	ОперацииПереходаКСтатусу.Колонки.Удалить(ОперацииПереходаКСтатусу.Колонки.НачальныйСтатус);
	
	ТаблицаПереходов.Индексы.Добавить("НачальныйСтатус, КонечныйСтатус");
	ОперацииПереходаКСтатусу.Индексы.Добавить("КонечныйСтатус");	
	
	ДанныеКэша = Новый Структура;
	ДанныеКэша.Вставить("ТаблицаПереходов", ТаблицаПереходов);
	ДанныеКэша.Вставить("ОперацииПереходаКСтатусу", ОперацииПереходаКСтатусу);
	
	ХранилищеКэша = Новый ХранилищеЗначения(ДанныеКэша);
	ПараметрыСеанса.абс_БСПКэшКлючевыхОпераций = ХранилищеКэша;
	
	Возврат ХранилищеКэша;
	
КонецФункции

Функция ПолучитьКэшКлючевыхОпераций() Экспорт
	
	Попытка
		ХранилищеКэша = ПараметрыСеанса.абс_БСПКэшКлючевыхОпераций;
	Исключение
		ХранилищеКэша = ЗаполнитьКэшКлючевыхОпераций();
	КонецПопытки;
