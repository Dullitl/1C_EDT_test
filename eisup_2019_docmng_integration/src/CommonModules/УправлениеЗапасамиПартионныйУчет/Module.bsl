...
Функция ПолучитьПараметрУчетнойПолитикиПартионногоУчета(ИмяПараметра, Учет, СтруктураПараметров) Экспорт
	...
	Иначе	
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не задан параметр учетной политики """ +ИмяПараметра+ """ "+Учет+" учета"+"!");
...
Процедура СписаниеПартий(СтрокаДокумента, СтруктураПараметров, РегистрУчета)
	...
	Если СтрокаДереваПартий <> Неопределено Тогда
		...
		Для Каждого СтрокаПартииРаспределения ИЗ СтрокаДереваПартий.Строки Цикл
			...
			// АБС ВСТАВКА Фролов 
			Если Найти(ИмяРегистра, "ПартииТоваровПереданные") > 0 Тогда
				Движение.СерияНоменклатуры = СтрокаПартии.СерияНоменклатуры;
					
				Попытка
					Движение.абс_ЦФО = СтрокаПартии.абс_ЦФО;
					Движение.абс_НазначениеПлатежа = СтрокаПартии.абс_НазначениеПлатежа;
				Исключение
					
				КонецПопытки;
				
				//АБС НАЧАЛО Задача №08759
    			Движение.абс_ЗаказПроект = СтрокаПартии.абс_ЗаказПроект;
	...
	// Товара не хватило
	Если (КоличествоОсталосьПогасить > 0) Тогда
		//Горелик Р.В. 01.10.2018 [CHG572\639] +++ 
		//Отключение вывода сообщения по использованию отключенного регистра
		Если НЕ ИмяРегистра = "ПартииТоваровНаСкладахУпр" Тогда
...
Процедура СообщитьОНехваткеПартии(СтрокаДокумента, СтруктураПараметров, РегистрУчета, КоличествоОсталосьПогасить)
	//Бобылев А.А. 29.05.2018 СППР 00-00000112
	Если ТипЗнч(СтрокаДокумента.Регистратор) <> Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
...
Процедура ПроведениеКорректировкиРеализации(Ссылка, СтруктураПараметров, ТаблицаСписания)
	...
	Для Каждого СтрокаТЧ из ТаблицаПоТоварам Цикл
		...
		ИначеЕсли СтрокаТЧ.СуммаБУ <> 0 Тогда
			...
			ТаблицаПартийБУ.ЗагрузитьКолонку(ttk_ОбщегоНазначения.РаспределитьПропорционально(СтрокаТЧ.СуммаБУ, МассивКоэфф, , Ложь), 	"СуммаВыручкиБУ");
			ТаблицаПартийБУ.ЗагрузитьКолонку(ttk_ОбщегоНазначения.РаспределитьПропорционально(СтрокаТЧ.НДС, МассивКоэфф, , Ложь), 		"СуммаНДСВыручкиБУ");
			...
			Для Каждого Партия Из ТаблицаПартийБУ Цикл
				...
				Если (НЕ БУ_Тек) Тогда
					...
					Если Партия.Комиссионный Тогда
						// {{ТТК Сладков А. Заявка №7739561  13.07.2016 ++
						
						// Убираем проводки из этого модуля потому что они дублируються в самом докуемнте "Корректировка реализации"
						// и происходит задвоение проводок (по списанию товара проводкио оставляем, они не дублируются)
					...
					Если ОтразитьВУчетеМПЗ Тогда
						Документы.КорректировкаРеализации.СформироватьПроводкиПоТовару(СтруктураШапкиДокумента, ПроводкиБУ, Партия, СтруктураШапкиДокумента.ДокументРеализацииДата, "БУ", Ложь, Ложь, Истина);
					КонецЕсли;
					Если Партия.Комиссионный Тогда
						// {{ТТК Сладков А. Заявка №7739561  13.07.2016 ++
				...
				Если (БУ_Тек) Тогда
					...
					Если ОтразитьВУчетеМПЗ Тогда						
						Документы.КорректировкаРеализации.СформироватьПроводкиПоТовару(СтруктураШапкиДокумента, ПроводкиБУ, Партия, СтруктураШапкиДокумента.ДокументРеализацииДата, "БУ", ПрошлыйПериод, Ложь, Ложь);
					КонецЕсли;
					Если Партия.Комиссионный Тогда
						// {{ТТК Сладков А. Заявка №7739561  13.07.2016 ++
					...
					Иначе
						// {{ТТК Сладков А. Заявка №7739561  13.07.2016 ++
						//Документы.КорректировкаРеализации.СформироватьПроводкиПоСобственнойВыручке(СтруктураШапкиДокумента, ПроводкиБУ, Партия, СтруктураШапкиДокумента.Дата, "БУ", ПрошлыйПериод, Ложь, Ложь);
						//Документы.КорректировкаРеализации.СформироватьПроводкиПоНДС(СтруктураШапкиДокумента, ПроводкиБУ, Партия, СтруктураШапкиДокумента.Дата, ПрошлыйПериод);
						// {{ТТК Сладков А. Заявка №7739561  13.07.2016 --
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		...
		Если ТаблицаПартийНУ <> Неопределено И ТаблицаПартийНУ.Количество() > 0 И СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			...
			Для Каждого Партия Из ТаблицаПартийНУ Цикл
				Если (НЕ НУ_Тек) Тогда
					...
					//Возврат корректировки со счета К
					Если ОтразитьВУчетеМПЗ Тогда
						Документы.КорректировкаРеализации.СформироватьПроводкиПоТовару(СтруктураШапкиДокумента, ПроводкиНУ, Партия, СтруктураШапкиДокумента.ДокументРеализацииДата, "НУ", Ложь, Ложь, Истина);
						Если БУ_Тек Тогда
							Документы.КорректировкаРеализации.СформироватьПроводкиПоТовару(СтруктураШапкиДокумента, ПроводкиНУ, Партия, СтруктураШапкиДокумента.ДокументРеализацииДата, "ПР", ПрошлыйПериод, Ложь, Ложь);
						КонецЕсли;
					КонецЕсли;
					Если НЕ Партия.Комиссионный Тогда
						// {{ТТК Сладков А. Заявка №7739561  13.07.2016 ++
				...
				Если (НУ_Тек) Тогда
					//Коррктировка текущего периода
					Если ОтразитьВУчетеМПЗ Тогда
						Документы.КорректировкаРеализации.СформироватьПроводкиПоТовару(СтруктураШапкиДокумента, ПроводкиНУ, Партия, СтруктураШапкиДокумента.ДокументРеализацииДата, "НУ", ПрошлыйПериод, Ложь, Ложь);
						Если НЕ БУ_Тек Тогда
							Документы.КорректировкаРеализации.СформироватьПроводкиПоТовару(СтруктураШапкиДокумента, ПроводкиНУ, Партия, СтруктураШапкиДокумента.ДокументРеализацииДата, "ПР", ПрошлыйПериод, Ложь, Ложь);
						КонецЕсли;
					КонецЕсли;
					Если НЕ Партия.Комиссионный Тогда
						...
						Если НЕ БУ_Тек Тогда
							// {{ТТК Сладков А. Заявка №7739561  13.07.2016 ++
							//Документы.КорректировкаРеализации.СформироватьПроводкиПоСобственнойВыручке(СтруктураШапкиДокумента, ПроводкиНУ, Партия, СтруктураШапкиДокумента.Дата, "ПР", ПрошлыйПериод, Ложь, Ложь);
							// {{ТТК Сладков А. Заявка №7739561  13.07.2016 --
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
				Если (БУ_Тек) И (НЕ НУ_Тек) Тогда
					//Коррктировка текущего периода
					Если НЕ Партия.Комиссионный Тогда
						// {{ТТК Сладков А. Заявка №7739561  13.07.2016 начало
						//Документы.КорректировкаРеализации.СформироватьПроводкиПоСобственнойВыручке(СтруктураШапкиДокумента, ПроводкиНУ, Партия, СтруктураШапкиДокумента.Дата, "ПР", ПрошлыйПериод, Ложь, Ложь);
						// }}ТТК Сладков А. Заявка №7739561  13.07.2016 окончание
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
	...
	Если НЕ ЗначениеЗаполнено(Параметры.СчетРасчетовСКомитентом) тогда
		...
		Если НЕ ЗначениеЗаполнено(Параметры.СчетРасчетовСКомитентом) тогда
			Если НЕ ЗначениеЗаполнено(Партия) тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("При списании комиссионных товаров не определена партия оприходования.
				|Невозможно определить счет расчетов с комитентом.",Отказ,Заголовок);
			Иначе	
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Невозможно определить счет расчетов с комитентом при списании комиссионных товаров по партии """+Строка(Партия)+"""",Отказ,Заголовок);
			КонецЕслИ;
		Иначе
			Если НЕ ЗначениеЗаполнено(Партия) тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("При списании комиссионных товаров не определена партия оприходования.
			...
			Иначе	
				ttk_ОбщегоНазначения.СообщитьОбОшибке("В партии комиссионных товаров """+Строка(Партия)+""" не указан счет учета расчетов с комитентом.
...
Процедура ДополнитьТаблицуПартийДляКорректировки(ТаблицаПартий, СтрокаТЧ)
	...
	Если ТаблицаПартий.Колонки.Найти("СуммаБУ") = Неопределено Тогда
		...
		Если ТаблицаПартий.Колонки.Найти("СуммаВыручкиБУ") = Неопределено Тогда
			ТаблицаПартий.ЗагрузитьКолонку(ttk_ОбщегоНазначения.РаспределитьПропорционально(СтрокаТЧ.СуммаБУ, МассивКоэфф, , Ложь), "СуммаБУ");
		Иначе
	...
	Если ТаблицаПартий.Колонки.Найти("НДС") = Неопределено Тогда
		...
		Если ТаблицаПартий.Колонки.Найти("СуммаНДСВыручкиБУ") = Неопределено Тогда
			ТаблицаПартий.ЗагрузитьКолонку(ttk_ОбщегоНазначения.РаспределитьПропорционально(СтрокаТЧ.НДС, МассивКоэфф, , Ложь), "НДС");
		Иначе
	...
	Если ТаблицаПартий.Колонки.Найти("СуммаВал") = Неопределено Тогда
		...
		ТаблицаПартий.ЗагрузитьКолонку(ttk_ОбщегоНазначения.РаспределитьПропорционально(СтрокаТЧ.СуммаВал, МассивКоэфф, , Ложь), "СуммаВал");
	КонецЕсли;
	Если ТаблицаПартий.Колонки.Найти("НДСВал") = Неопределено Тогда
		ТаблицаПартий.Колонки.Добавить("НДСВал");
		ТаблицаПартий.ЗагрузитьКолонку(ttk_ОбщегоНазначения.РаспределитьПропорционально(СтрокаТЧ.НДСВал, МассивКоэфф, , Ложь), "НДСВал");
	КонецЕсли;
	Если ТаблицаПартий.Колонки.Найти("НДСИтоговый") = Неопределено Тогда
		ТаблицаПартий.Колонки.Добавить("НДСИтоговый");
		ТаблицаПартий.ЗагрузитьКолонку(ttk_ОбщегоНазначения.РаспределитьПропорционально(СтрокаТЧ.НДСИтоговый, МассивКоэфф, , Ложь), "НДСИтоговый");
	КонецЕсли;
	Если ТаблицаПартий.Колонки.Найти("НДСДоИзм") = Неопределено Тогда
		ТаблицаПартий.Колонки.Добавить("НДСДоИзм");
		ТаблицаПартий.ЗагрузитьКолонку(ttk_ОбщегоНазначения.РаспределитьПропорционально(СтрокаТЧ.НДСДоИзм, МассивКоэфф, , Ложь), "НДСДоИзм");
	КонецЕсли;
	Если ТаблицаПартий.Колонки.Найти("НДСДоКорр") = Неопределено Тогда
		ТаблицаПартий.Колонки.Добавить("НДСДоКорр");
		ТаблицаПартий.ЗагрузитьКолонку(ttk_ОбщегоНазначения.РаспределитьПропорционально(СтрокаТЧ.НДСДоКорр, МассивКоэфф, , Ложь), "НДСДоКорр");
	КонецЕсли;
	...
	КонецЕсли;
	Если ТаблицаПартий.Колонки.Найти("абс_СубконтоБУ2") = Неопределено Тогда
		ТаблицаПартий.Колонки.Добавить("абс_СубконтоБУ2");
		ТаблицаПартий.ЗаполнитьЗначения(СтрокаТЧ.абс_СубконтоБУ2, "абс_СубконтоБУ2");
	КонецЕсли;
...
Процедура ПоступлениеПереданныхУпр(СтрокаДокумента, СтруктураПараметров, Строка)
	...
	// АБС ВСТАВКА Фролов
	Попытка
		Движение.абс_ЦФО				= СтрокаДокумента.абс_ЦФО;
		Движение.абс_НазначениеПлатежа	= СтрокаДокумента.абс_НазначениеПлатежа;
	Исключение
	...
	// АБС ВСТАВКА Фролов КОНЕЦ
	 ...
	 //АБС НАЧАЛО Задача №08759
 	Движение.абс_ЗаказПроект    = Строка.Заказ;
 	//\\АБС КОНЕЦ Задача №08759
...
Процедура ПоступлениеПереданныхНал(СтрокаДокумента, СтруктураПараметров, Строка)
	...
	//АБС ВСТАВКА №42343 НАЧАЛО «30 мая 2014 г.», Пополитов
 	Движение.СерияНоменклатуры		= Строка.СерияНоменклатуры;
 	//\\АБС ВСТАВКА №42343 КОНЕЦ   
...
Процедура ПоступлениеВозвратОтПокупателя(СтрокаДокумента, СтруктураПараметров)
	...
	Если (КоличествоОсталосьПогасить > 0) Тогда
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрУчет + " учет. Документ " + СтрокаДокумента.Регистратор + ", строка :" + СтрокаДокумента.НомерСтроки + Символы.ПС
...
Процедура СписаниеПартийОрдерныйСклад(СтрокаДокумента, СтруктураПараметров)
	...
	Если (КоличествоОсталосьПогасить > 0) Тогда
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрУчет + " учет. Документ " + СтрокаДокумента.Регистратор + Символы.ПС
...
Процедура ВыполнитьДопДвиженияПоРезультатуСписания(СтруктураПараметров, ТаблицаСписания)
	...
	Если СтруктураПараметров.ТипЗначенияРегистратора = Тип("ДокументСсылка.ПередачаТоваров") Тогда //АБС 16099+
		Для Каждого СтрокаСписания Из СтруктураПараметров.ТаблицаСписанныхПартий Цикл  
			Если СтрокаСписания.КодОперацииПартииТоваров = Перечисления.КодыОперацийПартииТоваров.ВозвратОтПереработчика Тогда  
				СтрокаСписания.КодОперацииПартииТоваров = Перечисления.КодыОперацийПартииТоваров.ПередачаВПереработку;
				СтрокаСписания.ДокументПередачи         = СтрокаСписания.Регистратор;
				текДоговор                              = СтрокаСписания.ДоговорКонтрагента;
				СтрокаСписания.ДоговорКонтрагента       = СтрокаСписания.Регистратор.ДоговорКонтрагента;
				Если СтруктураПараметров.ЕстьСтрокиОтражатьВБухгалтерскомУчете Тогда
				СтрокаСписания.КорСубконтоБУ3           = текДоговор;
				СтрокаСписания.КорСубконтоБУ2           = текДоговор.Владелец;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;  
	КонецЕсли;//АБС 16099-
	...
	Для Каждого СтрокаСписания Из СтруктураПараметров.ТаблицаСписанныхПартий Цикл
		...
		Если СтрокаСписания.ВидОперации = "Списание" Тогда
			...
			Если ФормироватьПроводки Тогда
				Если СтруктураПараметров.ТипЗначенияРегистратора = Тип("ДокументСсылка.ПередачаТоваров") Тогда //АБС 16099+
					Для Каждого текСтрокаСписания Из СтруктураПараметров.ТаблицаСписанныхПартий Цикл  
						Если текСтрокаСписания.КодОперации = Перечисления.КодыОперацийПартииТоваров.ВозвратОтПереработчика Тогда  
							текДоговор                              = СтрокаСписания.ДоговорКонтрагента;
							текСтрокаСписания.ДоговорКонтрагента    = СтрокаСписания.КорСубконтоБУ3;
							текСтрокаСписания.КорСубконтоБУ3        = текДоговор;
							текСтрокаСписания.КорСубконтоБУ2        = текДоговор.Владелец;
						КонецЕсли;
					КонецЦикла;  
				КонецЕсли;//АБС 16099-
...
Функция ПодготовитьСтруктуруТаблицыСписания(СтруктураПараметров, ТаблицаСписания)
	...
	СписокКодовОпераций.ЗагрузитьЗначения(ttk_ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(ТаблицаСписания.ВыгрузитьКолонку("КодОперацииПартииТоваров")));
...
Процедура ВыполнитьСписание(СтруктураПараметров, ТаблицаСписания, МоментКон, Останавливаться = Ложь, ПроведениеОстановлено = Ложь )
	...
	СтруктураПараметров.ТаблицаСписанныхПартий.Колонки.Добавить("абс_СубконтоБУ2");    //АБС Коломиец
	...
	Для Каждого СтрокаТаблицыСписания Из ТаблицаСписания Цикл
		...
		Для Каждого ТекущаяСтрокаСписанныхПартий Из ТаблицаСписанныхПартийПоТекущейСтроке Цикл
			...
			ТекущаяСтрокаСписанныхПартий.абс_СубконтоБУ2 = СтрокаДокумента.абс_СубконтоБУ2;    //АБС Коломиец
		КонецЦикла;	
	КонецЦикла; 
	...
	Если Не ОбрабатываемыйДокумент = Неопределено Тогда
		...
		Если Останавливаться И СтруктураПараметров.Отказ тогда
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Проведение по партиям остановлено ",СтруктураПараметров.Отказ);
...
Процедура СдвигГраницыПоследовательностиРасчетыПоРеализацииОрганизацииНазад(СтруктураПараметров, ТаблицаСписания)
	...
	// Включен сдвиг последовательности в разрезе договоров
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	РасчетыПоРеализацииОрганизации.МоментВремени,
	|	РасчетыПоРеализацииОрганизации.Договор
	|ИЗ
	...
	|	И РасчетыПоРеализацииОрганизации.Договор = &Договор";
	
	Запрос.УстановитьПараметр("Организация", ТаблицаСписания[0].Организация);
	Запрос.УстановитьПараметр("Договор", ТаблицаСписания[0].ДоговорКонтрагента);
	...
	Если Выборка.Следующий() Тогда
		...
		Если МоментВремениДокумента.Сравнить(Выборка.МоментВремени) = -1 Тогда
			Последовательности.РасчетыПоРеализацииОрганизации.УстановитьГраницу(МоментВремениДокумента, Новый Структура("Организация, Договор", ТаблицаСписания[0].Организация, ТаблицаСписания[0].ДоговорКонтрагента));
		КонецЕсли;
	КонецЕсли;
...
Процедура ДвижениеПартийТоваров(ОбрабатываемыйДокумент, ТаблицаСписания = Неопределено,
	...
	//АБС ВСТАВКА №28395 НАЧАЛО «22 октября 2014 г.», Пополитов
	СтруктураПараметров.Вставить("абс_ЗаполнятьНалоговыеРегистрыИзОстатковБухгалтерских",?(ОбрабатываемыйДокумент.Метаданные().Реквизиты.Найти("абс_ЗаполнятьНалоговыеРегистрыИзОстатковБухгалтерских") = Неопределено,Ложь,ОбрабатываемыйДокумент.абс_ЗаполнятьНалоговыеРегистрыИзОстатковБухгалтерских)); 	
 	//\\АБС ВСТАВКА №28395 КОНЕЦ     	
...
Функция ПолучитьДеревоПартийПереданныеУпр(МоментКон, Регистратор, ДоговорКонтрагента, СпособОценкиМПЗ, СтратегияСтатусПартии, БюджетнаяАналитика = Неопределено)
	...
	|	// АБС ВСТАВКА Фролов
	|	ПартииТоваровПереданные.абс_ЦФО,
	|	ПартииТоваровПереданные.абс_НазначениеПлатежа,
	|	ПартииТоваровПереданные.СерияНоменклатуры,
	|	// АБС ВСТАВКА Фролов КОНЕЦ
	 //АБС НАЧАЛО Задача №08759
    | ПартииТоваровПереданные.абс_ЗаказПроект,
  	//\\АБС КОНЕЦ Задача №08759
	|	ВЫБОР                                                           
	...
	|	//  АБС ВСТАВКА Фролов Списание партий переданных по ЦФО и назначению использования
	| 	И абс_ЦФО = &абс_ЦФО И абс_НазначениеПлатежа = &абс_НазначениеПлатежа
	...
	//АБС ВСТАВКА №42518 НАЧАЛО «16 мая 2014 г.», Пополитов
 	|			И Выбор Когда СписанныеТовары.Номенклатура.ВестиПартионныйУчетПоСериям Тогда СписанныеТовары.СерияНоменклатуры = ПартииТоваровПереданные.СерияНоменклатуры Иначе Истина Конец
 	//\\АБС ВСТАВКА №42518 КОНЕЦ    	
	//АБС НАЧАЛО Задача №08759
  	|   И (ВЫБОР КОГДА СписанныеТовары.ЗаказСписания = НЕОПРЕДЕЛЕНО ТОГДА Значение(Документ.ЗаказПокупателя.ПустаяСсылка) ИНАЧЕ СписанныеТовары.ЗаказСписания  КОНЕЦ = ПартииТоваровПереданные.абс_ЗаказПроект)
  	//\\АБС КОНЕЦ Задача №08759
	...
	//АБС ИЗМЕНЕНИЕ 36268  09.12.2013 12:49:30  Шамов
	//|								И СписанныеТовары.ЗаказПартии = НЕОПРЕДЕЛЕНО
	////АБС Шамов	
	//|							ИЛИ ПартииТоваровПереданные.ДокументПередачи.Заказ = &ЗаказПокупателяПустаяСсылка
	//|								И СписанныеТовары.ЗаказПартии = НЕОПРЕДЕЛЕНО
	//|							ИЛИ ПартииТоваровПереданные.ДокументПередачи.Заказ = НЕОПРЕДЕЛЕНО
	////\\АБС Шамов
	|								И (СписанныеТовары.ЗаказПартии = НЕОПРЕДЕЛЕНО 
	|									ИЛИ СписанныеТовары.ЗаказПартии = &ЗаказПокупателяПустаяСсылка)		
	|							ИЛИ ПартииТоваровПереданные.ДокументПередачи.Заказ = &ЗаказПокупателяПустаяСсылка
	|								И (СписанныеТовары.ЗаказПартии = НЕОПРЕДЕЛЕНО 
	|									ИЛИ СписанныеТовары.ЗаказПартии = &ЗаказПоставщикуПустаяСсылка)
	|							ИЛИ ПартииТоваровПереданные.ДокументПередачи.Заказ = НЕОПРЕДЕЛЕНО
	//АБС ИЗМЕНЕНИЕ 36268 КОНЕЦ	
	|				ИНАЧЕ ИСТИНА
	|			КОНЕЦ)
	|ГДЕ
	|	СписанныеТовары.Регистратор = &Ссылка
	...
	//АБС Шамов	
	Запрос.УстановитьПараметр("ЗаказПокупателяПустаяСсылка", Документы.ЗаказПокупателя.ПустаяСсылка());
	//\\АБС Шамов
	// АБС ВСТАВКА Фролов
	Если ТипЗнч(Регистратор) = Тип("ДокументСсылка.ПолучениеУслугПоПереработке") Тогда
		Запрос.УстановитьПараметр("абс_ЦФО", Регистратор.абс__ЦФО);
	Иначе
		Запрос.УстановитьПараметр("абс_ЦФО", Регистратор.абс_ЦФО);
	КонецЕсли;
	
	Запрос.УстановитьПараметр("абс_НазначениеПлатежа", Регистратор.абс_НазначениеПлатежа);
...
Функция ПолучитьДеревоПартийПереданныеБух(МоментКон, Регистратор, Организация, ДоговорКонтрагента, СпособОценкиМПЗ, СтратегияСтатусПартии)
	...
	|						ИЛИ СписанныеТовары.КодОперацииПартииТоваров = &ВозвратОтПереработчика
	|					ТОГДА СписанныеТовары.ЗаказПартии = ПартииТоваровПереданные.ДокументПередачи.Заказ
	//АБС ВСТАВКА №29836 НАЧАЛО «11 декабря 2014 г.», Пополитов
	//|							ИЛИ ПартииТоваровПереданные.ДокументПередачи.Заказ = &ЗаказПоставщикуПустаяСсылка
	//							//АБС+ Тупиков 17650
	//|							ИЛИ ПартииТоваровПереданные.ДокументПередачи.Заказ = НЕОПРЕДЕЛЕНО
	//							//АБС- Тупиков	 	
	//|								И СписанныеТовары.ЗаказПартии = НЕОПРЕДЕЛЕНО
	|							ИЛИ (ПартииТоваровПереданные.ДокументПередачи.Заказ В (&жМассивТипов) И СписанныеТовары.ЗаказПартии В (&жМассивТипов))
	//\\АБС ВСТАВКА №29836 КОНЕЦ  		
	|				ИНАЧЕ ИСТИНА
	|			КОНЕЦ)
	|ГДЕ
	|	СписанныеТовары.Регистратор = &Ссылка
	...
	Запрос.УстановитьПараметр("ЗаказПоставщикуПустаяСсылка", Документы.ЗаказПоставщику.ПустаяСсылка());
	//АБС ВСТАВКА №29836 НАЧАЛО «11 декабря 2014 г.», Пополитов
	абс_УстановитьПараметр_жМассивТипов(Запрос);
	//\\АБС ВСТАВКА №29836 КОНЕЦ
	
	// АБС ВСТАВКА Фролов
	
	//Бреев. 12.07.2011. проверка есть ли реквизит с двумя подчеркиваниями.
	Если Регистратор.Метаданные().Реквизиты.Найти("абс__ЦФО")<>Неопределено тогда
		пер_абс_ЦФО = Регистратор.абс__ЦФО;
	Иначе                                                        
		пер_абс_ЦФО = Регистратор.абс_ЦФО;
	КонецЕсли;
	
	Запрос.УстановитьПараметр("абс_ЦФО", пер_абс_ЦФО);
	Запрос.УстановитьПараметр("абс_НазначениеПлатежа", Регистратор.абс_НазначениеПлатежа);
...
Процедура ЗаполнитьЗапросПартийНаСкладахНал(Запрос, ВестиПартионныйУчетПоСкладам, СтратегияСтатусПартии, СпособОценкиМПЗ)
    ПараметрыЗапроса_ДатаОприходования = ПолучитьДанныеДляЗапроса_ДокументОприходованияДата(СпособОценкиМПЗ,"ПартииТоваровНаСкладах");
	...
	|ИЗ
	|	РегистрСведений.СписанныеТовары КАК СписанныеТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПартииТоваровНаСкладахНалоговыйУчет.Остатки(
	|		&Дат,
	|		Организация = &Организация
	|		    И Номенклатура В
	|		        (ВЫБРАТЬ
...
Процедура ЗаполнитьЗапросПартийНаСкладахДляЗакрытияЗаказовПокупателейНал(Запрос)
	...
	Запрос.Текст ="ВЫБРАТЬ
				  ...
				  |ИЗ
				  |	РегистрСведений.СписанныеТовары КАК СписанныеТовары
				  |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПартииТоваровНаСкладахНалоговыйУчет.Остатки(&Дат, ) КАК ПартииТоваровНаСкладах
				  |		ПО СписанныеТовары.Номенклатура = ПартииТоваровНаСкладах.Номенклатура
				  |			И СписанныеТовары.Номенклатура = ПартииТоваровНаСкладах.Номенклатура
				  |			И СписанныеТовары.ХарактеристикаНоменклатуры = ПартииТоваровНаСкладах.ХарактеристикаНоменклатуры
				  |			И СписанныеТовары.ЗаказПартии = ПартииТоваровНаСкладах.Заказ
...
Функция ПолучитьДеревоПартийНаСкладахНал(МоментКон, Организация, СтруктураПараметров)
	...
	Иначе
		// Общий случай списания
		ЗаполнитьЗапросПартийНаСкладахНал(Запрос, ВестиПартионныйУчетПоСкладам, СтратегияСтатусПартии, СпособОценкиМПЗ);
		Если НЕ СтруктураПараметров.ИспользоватьУказаниеСерийНоменклатурыПриРезервировании тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст,"ИЛИ ПартииТоваровНаСкладах.СерияНоменклатуры = &ПустаяСерияНоменклатуры",
			"ИЛИ ПартииТоваровНаСкладах.СерияНоменклатуры = &ПустаяСерияНоменклатуры
			|ИЛИ СписанныеТовары.КодОперацииПартииТоваров = &КодРезервирование");
...
Функция ПолучитьДеревоПартийПереданныеНал(МоментКон, Регистратор, Организация, ДоговорКонтрагента, СпособОценкиМПЗ, СтратегияСтатусПартии)
	...
	|ИЗ
	|	РегистрСведений.СписанныеТовары КАК СписанныеТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПартииТоваровПереданныеНалоговыйУчет.Остатки(&Дат,	
	|																					Номенклатура В
	|																					    (ВЫБРАТЬ
	|																					        РегистрСведений.СписанныеТовары.Номенклатура
	|																					    ИЗ
	...
	// АБС ВСТАВКА Фролов
	//Бреев. 27.07.2011. проверка есть ли реквизит с двумя подчеркиваниями.
	Если Регистратор.Метаданные().Реквизиты.Найти("абс__ЦФО")<>Неопределено тогда
		пер_абс_ЦФО = Регистратор.абс__ЦФО;
	Иначе                                                        
		пер_абс_ЦФО = Регистратор.абс_ЦФО;
...
Процедура ПолучитьОстаткиПартийПереданныхРегл(СтруктураПараметров, МоментКон, Организация, ДоговорКонтрагента)
	...
	Если СтруктураПараметров.ЕстьСтрокиОтражатьВНалоговомУчете Тогда
		...
		//АБС ВСТАВКА №28395 НАЧАЛО «22 октября 2014 г.», Пополитов
		Если СтруктураПараметров.абс_ЗаполнятьНалоговыеРегистрыИзОстатковБухгалтерских Тогда
			МассивСчетовНУ = Новый Массив;        
			абс_ЗаполнитьМассивСчетовНУ(СтруктураПараметров.ДеревоПартииТоваровПереданныеНал,МассивСчетовНУ);
			МассивСчетовНУ = ?(ЗначениеЗаполнено(МассивСчетовНУ),МассивСчетовНУ,Неопределено);
			абс_НовоеДерево = СтруктураПараметров.ДеревоПартииТоваровПереданныеБух.Скопировать();
			СтруктураПараметров.Вставить("ДеревоПартииТоваровПереданныеНал",абс_НовоеДерево);
			СтруктураПараметров.ДеревоПартииТоваровПереданныеНал.Колонки.Добавить("СчетУчетаНУ",Новый ОписаниеТипов("ПланСчетовСсылка.Налоговый"));
			абс_ИзменитьСчетБУнаНУ(СтруктураПараметров.ДеревоПартииТоваровПереданныеНал,МассивСчетовНУ);
			СтруктураПараметров.ДеревоПартииТоваровПереданныеНал.Колонки.Удалить("СчетУчета");
			СтруктураПараметров.ДеревоПартииТоваровПереданныеНал.Колонки.Добавить("СчетУчета",Новый ОписаниеТипов("ПланСчетовСсылка.Налоговый"));
			абс_ЗаполнитьСчетУчета(СтруктураПараметров.ДеревоПартииТоваровПереданныеНал);
			СтруктураПараметров.ДеревоПартииТоваровПереданныеНал.Колонки.Удалить("СчетУчетаНУ");
		КонецЕсли;
		//\\АБС ВСТАВКА №28395 КОНЕЦ 		
		...
		Для Каждого Измерение ИЗ Метаданные.РегистрыНакопления.ПартииТоваровПереданныеНалоговыйУчет.Измерения Цикл
			...
			ТаблицаОстатковПартий.Колонки.Добавить(Измерение.Имя);
		КонецЦикла;
		ТаблицаОстатковПартий.Колонки.Добавить("Количество",Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(20,3)));
		ТаблицаОстатковПартий.Колонки.Добавить("Стоимость",Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(20,2)));
		ТаблицаОстатковПартий.Колонки.Добавить("ПостояннаяРазница",Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(20,2)));
		ТаблицаОстатковПартий.Колонки.Добавить("ВременнаяРазница",Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(20,2)));
...
//АБС ВСТАВКА №28395 НАЧАЛО «22 октября 2014 г.», Пополитов
Функция абс_ИзменитьСчетБУнаНУ(Дерево,МассивСчетовНУ) Экспорт
	
	Для Каждого Выборка Из Дерево.Строки Цикл
		Если ЗначениеЗаполнено(Выборка.СчетУчета) Тогда
			Выборка.СчетУчетаНУ = БухгалтерскийУчет.ПреобразоватьСчетаБУвСчетНУ(Новый Структура("СчетБУ",Выборка.СчетУчета),,,МассивСчетовНУ);	
			Если не ЗначениеЗаполнено(Выборка.СчетУчетаНУ) Тогда
				Выборка.СчетУчетаНУ = БухгалтерскийУчет.ПреобразоватьСчетаБУвСчетНУ(Новый Структура("СчетБУ",Выборка.СчетУчета));	
			КонецЕсли;	
		КонецЕсли;
		абс_ИзменитьСчетБУнаНУ(Выборка,МассивСчетовНУ);
	КонецЦикла;	
	
КонецФункции	

Функция абс_ЗаполнитьМассивСчетовНУ(Дерево,МассивСчетовНУ) Экспорт
	
	Для Каждого Выборка Из Дерево.Строки Цикл
		Если ЗначениеЗаполнено(Выборка.СчетУчета) Тогда
			МассивСчетовНУ.Добавить(Выборка.СчетУчета);	
		КонецЕсли;
		абс_ЗаполнитьМассивСчетовНУ(Выборка,МассивСчетовНУ);
	КонецЦикла;	
	
КонецФункции

Функция абс_ЗаполнитьСчетУчета(Дерево) Экспорт
	
	Для Каждого Выборка Из Дерево.Строки Цикл
		Выборка.СчетУчета = Выборка.СчетУчетаНУ; 
		абс_ЗаполнитьСчетУчета(Выборка);
	КонецЦикла;	
...
Процедура ОпределитьСоставКолонокТаблицаСписанныхПартийОбщихДляВсехВидовУчета(СтруктураПараметров, СписокИспользуемыхОпераций, СтруктураКолонок) Экспорт
	...
	// АБС ВСТАВКА Фролов 
	СтруктураКолонок.Вставить("абс_ЦФО");
	СтруктураКолонок.Вставить("абс_НазначениеПлатежа");
	// АБС ВСТАВКА Фролов КОНЕЦ
...
Процедура ВыполнитьПриходПоРегистрамПартий_ВводНачальныхОстатковУСН(СтруктураШапкиДокумента, ТаблицаПартийНаСкладах, ТаблицаПартийПереданных, ТаблицаСтаройРегистрацииВПоследовательности)
	...
	Если ТаблицаПартийНаСкладах.Количество()>0 Тогда
		...
		ТаблицыДанныхДокумента = ttk_ОбщегоНазначения.ЗагрузитьТаблицыДокументаВСтруктуру(Движения.ПартииТоваровНаСкладахНалоговыйУчет, новый Структура("ТаблицаПартий",ТаблицаПартийНаСкладах));
		ttk_ОбщегоНазначения.ЗаписатьТаблицыДокументаВРегистр(Движения.ПартииТоваровНаСкладахНалоговыйУчет, ВидДвиженияНакопления.Приход, ТаблицыДанныхДокумента, СтруктураШапкиДокумента.Дата);	
	...
	Если ТаблицаПартийПереданных.Количество()>0 Тогда
		...
		ТаблицыДанныхДокумента = ttk_ОбщегоНазначения.ЗагрузитьТаблицыДокументаВСтруктуру(Движения.ПартииТоваровПереданныеНалоговыйУчет, новый Структура("ТаблицаПартий",ТаблицаПартийПереданных));
		ttk_ОбщегоНазначения.ЗаписатьТаблицыДокументаВРегистр(Движения.ПартииТоваровПереданныеНалоговыйУчет, ВидДвиженияНакопления.Приход, ТаблицыДанныхДокумента, СтруктураШапкиДокумента.Дата);	
...
Функция ОпределитьСтатусПартииПриходаУпр(СтруктураШапкиДокумента) Экспорт
	...
	Если ТипДокумента = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") 
		Или ТипДокумента = Тип("ДокументСсылка.абс_НеотфактурованнаяПоставка") Тогда //АБС Кряковкин Добавлено условие для Неотфактфактурированной поставки
		...
		ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ОтветХранение Тогда

			Статус = Перечисления.СтатусыПартийТоваров.ОтветственноеХранение; 
...
Процедура ЗаполнитьПоляПартииТоваровНаСкладахПриПриходе(ТаблицаТЧ, ТаблицаДвижений, СтатусПартии, ВестиПартионныйУчетПоСкладам, СпособОценкиМПЗ, СтруктураШапкиДокумента, ВидУчета,флФормированиеРазниц=ложь)
	...
	Для каждого СтрокаДвижений Из ТаблицаДвижений Цикл
		...
		Если (ТипДокумента = Тип("ДокументСсылка.ПоступлениеТоваровУслуг"))
		  И	(СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ВПереработку 
		    ИЛИ СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Оборудование) Тогда
			  ...
			  Если ТаблицаТЧ.Колонки.Найти("Заказ") <> Неопределено Тогда
				  СтрокаДвижений.Заказ = СтрокаТЧ.Заказ;
			  КонецЕсли;
			  //\\АБС КОНЕЦ Задача №08759
		Иначе
			...
			КонецЕсли;
			//АБС НАЧАЛО Задача №08759
   			//Ведем учет по заказам в любом случае
            //Если Заказ <> неопределено И ОбособленныйУчет=истина И ЗначениеЗаполнено(Заказ) И ТипЗнч(Заказ)=Тип("ДокументСсылка.ЗаказПокупателя") Тогда
   			Если Заказ <> неопределено И ЗначениеЗаполнено(Заказ) И ТипЗнч(Заказ)=Тип("ДокументСсылка.ЗаказПокупателя") Тогда
      		//\\АБС КОНЕЦ Задача №08759
		...
		Если Не СпособОценкиМПЗ = Перечисления.СпособыОценки.ПоСредней 
		 ...
		 //// АБС. Гетц. 31.10.13
		 //ИЛИ 
		 ////АБС ВСТАВКА Только если включена проверка заполнения по преоктносу учету  15.04.2014 10:39:10  Гетц
		 //(РегистрыСведений.абс_КонтрольАналитикПроектныйУчет.НеобходимаПроверкаДляОрганизации(СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.Дата).КонтрольЗаполнения И
		 ////АБС ВСТАВКА  КОНЕЦ		 
		 //(СтатусПартии = Перечисления.СтатусыПартийТоваров.Купленный
		 //ИЛИ (ТаблицаТЧ.Колонки.Найти("СтатусПартии") <> Неопределено И СтрокаТЧ.СтатусПартии = Перечисления.СтатусыПартийТоваров.Купленный))) // для товаров всегда учитывается по документам оприходования (добавлено)
		 //// АБС. Гетц. 31.10.13 \\
		Тогда 
...
Процедура ВыполнитьПриходПоРегистрамПартийПоВидуУчета(СтруктураШапкиДокумента, УчетнаяПолитика, КодОперацииПартииТоваров, Отказ, СтруктТаблицДокумента, Движения, ВидУчета)
	...
	ТаблицыДанныхДокумента = ttk_ОбщегоНазначения.ЗагрузитьТаблицыДокументаВСтруктуру(Движения["ПартииТоваровНаСкладах"+СуффиксРегистра], СтруктТаблицДокумента);
	Если ВидУчета = "УправленческийУчет" Тогда
		ВестиПартионныйУчетПоСкладам = глЗначениеПеременной("ПараметрыПартионногоУчета").ВестиПартионныйУчетПоСкладам;
		ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "Организация", УправлениеЗапасами.ПолучитьОрганизациюВСоответствииСоСпособомВеденияПартионногоУчетаПоОрганизациям(СтруктураШапкиДокумента.Организация,СтруктураШапкиДокумента.СпособВеденияПартионногоУчетаПоОрганизации));
	Иначе
	...
	Если ТипДокумента <> Тип("ДокументСсылка.ОприходованиеМатериаловИзПроизводства") Тогда	//Документ ОприходованиеМатериаловИзПроизводства содержит код операции в табличной части											
		ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "КодОперации", КодОперацииПартииТоваров, СписокТаблицДокумента);
	КонецЕсли;
	...
	Если СтруктураШапкиДокумента.Свойство("ВидПоступления")
	  ...
	  И (ВидУчета = "УправленческийУчет") Тогда
		...
		Если НЕ ВестиПартионныйУчетПоСкладам Тогда
			ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "Склад", Неопределено, СписокТаблицДокументаСторно);  
		КонецЕсли;	
		  
		ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "СтатусПартии", Перечисления.СтатусыПартийТоваров.ПоОрдеру, СписокТаблицДокументаСторно);
				
		ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "Стоимость",    0, СписокТаблицДокументаСторно);
				
		ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "КодОперации",  Перечисления.КодыОперацийПартииТоваров.ПоступлениеПоОрдеру, СписокТаблицДокументаСторно);
		
		ttk_ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "Качество", Справочники.Качество.Новый, СписокТаблицДокументаСторно);

	КонецЕсли;
		
	ttk_ОбщегоНазначения.ЗаписатьТаблицыДокументаВРегистр(Движения["ПартииТоваровНаСкладах"+СуффиксРегистра], ВидДвиженияНакопления.Приход, ТаблицыДанныхДокумента, СтруктураШапкиДокумента.Дата);	
		...
		Если ВидУчета="УправленческийУчет" И ИмяТаблицы="ТаблицаПоТоварам" Тогда
			...
			Пока Инд<ТаблицаДвижений.Количество() Цикл
				Если НЕ ЗначениеЗаполнено(ТаблицаДвижений[Инд].ДокументОприходования) Тогда
					// АБС ИЗМЕНЕНО Фролов Разрешаем доп. расходы без партии
					Инд = Инд + 1;
					//ТаблицаДвижений.Удалить(Инд);
					// АБС ИЗМЕНЕНО КОНЕЦ
				Иначе
...
Процедура ВыполнитьПриходПоРегиструПродажиСебестоимость(СтруктураШапкиДокумента,Движения,СтруктТаблицДокумента)
	...
	Если СтруктураШапкиДокумента.ВидДоговора <> Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(СтруктТаблицДокумента.ТаблицаПоТоварам, ТаблицаДвижений);
...
Процедура ЗаполнитьДвиженияПБУ1802(УчетнаяПолитика,СтруктураШапкиДокумента,Движения, ТаблицаТЧ,КодОперацииПартииТоваров,ПоддержкаПБУ18,флФормироватьДвиженияПоЗатратам,СтатусПартии)
	...
	Если ТаблицаРазницы.Количество()=0 Тогда
		Возврат;
	КонецЕсли;
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаРазницы, ТаблицаДвижений);
    // Недостающие поля.
...
Процедура ВыполнитьПриходПоРегистрамПартийВсеВидыУчета(СтруктураШапкиДокумента, Отказ=Ложь, ТаблицаПоТоварам=Неопределено, ТаблицаПоТаре=Неопределено, ТаблицаПоОборудованию=Неопределено, Упр=Ложь, Бух=Ложь, Нал=Ложь, ТаблицаСтаройРегистрацииВПоследовательности)
	...
	Если Упр Тогда
		ПараметрыУчетнойПолитикиУпр = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиУпр(СтруктураШапкиДокумента.Дата);
	...
	Если Бух ИЛИ Нал Тогда
		ПарамерыУчетнойПолитикиРегл = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(СтруктураШапкиДокумента.Дата, СтруктураШапкиДокумента.Организация);
...
Процедура СформироватьПроводкиВсеВидыУчета(СтруктураШапкиДокумента, Отказ=Ложь, ТаблицаПоТоварам=Неопределено, ТаблицаПоТаре=Неопределено, ТаблицаПоУслугам=неопределено, ТаблицаПоОборудованию=Неопределено, ТаблицаПоОбъектамСтроительства = неопределено, Бух=Ложь, Нал=Ложь)
	...
	Для Каждого ТекущееДвижение Из Движения Цикл
		Если ТекущееДвижение.Значение.Модифицированность() Тогда
			//АБС ВСТАВКА №000031118 НАЧАЛО «4 марта 2015 г.», Пополитов
			Попытка 
			//\\АБС ВСТАВКА №000031118 КОНЕЦ	
				ТекущееДвижение.Значение.Записать(Ложь);
			//АБС ВСТАВКА №000031118 НАЧАЛО «4 марта 2015 г.», Пополитов	
			Исключение
			    Отказ = Истина;
...
Процедура СформироватьПроводкиПоТабличнойЧастиПоВидуУчета(Движения, СтруктураПараметрыПроводок, СтруктураШапкиДокумента, ИмяТЧ, ТаблицаТЧ, ВидУчета)
	...
	Для каждого СтрокаТЧ из ТаблицаТЧ цикл
		...
		Если СтруктураПараметрыПроводок.Свойство("ПриходТМЦ") Тогда
			...
			Если ТипДокумента = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") ИЛИ 
				...
				Если ИмяТЧ = "Услуги" Тогда
					...
					Если ПроизводственныеРасходы Тогда
						...
						Если Найти(Проводка.СчетДт.Код, "25") > 0 Тогда
							...
							//Сторчевой А.Н. 19.04.2017 77101658 {
							//Если субконто вид деятельности не заполнен то пробуем заполнить из верного реквизита
							Если НЕ ВидУчета = "БухгалтерскийУчет" Тогда
								Если Не ЗначениеЗаполнено(Проводка.СубконтоДт[Проводка.СчетДт.ВидыСубконто[2].ВидСубконто]) Тогда
									БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 3, СтрокаТЧ[ИмяКолонкиСубконто+"НУ3"]);	
								КонецЕсли;
							КонецЕсли;
							// } Сторчевой А.Н. 19.04.2017 77101658 
					...
					Иначе
						Если ВидУчета = "БухгалтерскийУчет" Тогда
							ИмяКолонкиСубконто = "Субконто";
							//+++ T#77132362 2017.07.18 Гущина Заполним количество для 50.03
							Если ТипДокумента = Тип("ДокументСсылка.АвансовыйОтчет") и Проводка.СчетДт = ПланыСчетов.Хозрасчетный.ДенежныеДокументы И ЗначениеЗаполнено(СтрокаТЧ.Количество) Тогда
									Проводка.КоличествоДт = БухгалтерскийУчет.КоличествоВПроводку(Проводка.СчетДт, СтрокаТЧ.Количество);
							КонецЕсли;
							//--- T#77132362 2017.07.18 Гущина
						Иначе
							ИмяКолонкиСубконто = "СубконтоНУ";
						КонецЕсли;
				...
				Если ВидУчета = "НалоговыйУчет" И Проводка.СчетДт.ПринадлежитЭлементу(ПланыСчетов.Налоговый.ПрочиеДоходыИРасходы) Тогда   //АБС Коломиец 19314
		...
		Если СтруктураПараметрыПроводок.Свойство("ВыделениеНДС") И СтруктураСуммыВПроводки.НДС > 0 И (НЕ флСчетУчетаЗабалансовый) Тогда
			...
			Если Проводка.СчетДт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("19.БП" ) Тогда
				Попытка
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "РасходыБудущихПериодов", СтрокаТЧ.абс_РасходыБудущихПериодов);
				Исключение
				КонецПопытки;
			КонецЕсли;
	...
	//АБС ВСТАВКА 56815. Корректировка проводок по 15 счету
	//проводки Дт 10,07,08.04 меняем на Дт 15, одновременно добавляем проводку Дт 10 Кт 15
	
	ТипыДокументовЧерез15Счет = абс_ПолучитьТипыДокументовЧерез15Счет();
	
	Если НЕ ТипыДокументовЧерез15Счет.Найти(ТипДокумента) = Неопределено И ИмяТЧ = "Товары" Тогда
		ИзменениеПроизведено = Ложь;
		КоличествоПроводок = Проводки.Количество()-1;
		Для СчетчикПроводок = 0 по КоличествоПроводок Цикл
			Проводка = Проводки[СчетчикПроводок];
			КодСчета = Строка(Проводка.СчетДт);
			// АБС ИЗМЕНЕНО 20110209
			Если Лев(КодСчета,2) = "10" Тогда
				// Только 10 счет
				//или Лев(КодСчета,2) = "07" или Лев(КодСчета,2) = "41" или КодСчета = "08.04" Тогда
				// АБС ИЗМЕНЕНО 20110209 КОНЕЦ
				Номенклатура = Проводка.СубконтоДт.Номенклатура;
				Склад = Проводка.СубконтоДт.Склады;
				Если НЕ Номенклатура.абс_ЯвляетсяТЗР Тогда
					НоваяПроводка = Проводки.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяПроводка,Проводка);
					НоваяПроводка.СубконтоДт.Очистить();
					НоваяПроводка.СубконтоКт.Очистить();
				КонецЕсли;
				
				Если ВидУчета = "БухгалтерскийУчет" Тогда
					Счет15 = ПланыСчетов.Хозрасчетный.НайтиПоКоду("15.01");
				Иначе
					Счет15 = ПланыСчетов.Налоговый.НайтиПоКоду("15.01");
				КонецЕсли;
			
				ПРоводка.СчетДт = Счет15;
								
				Проводка.СубконтоДт.Очистить();
				Проводка.СубконтоДт.Номенклатура = Номенклатура;
				Проводка.КоличествоДт = 0;
				
				Если НЕ Номенклатура.абс_ЯвляетсяТЗР Тогда
					НоваяПроводка.СчетКт = Счет15;
					
					Если ВидУчета = "БухгалтерскийУчет" Тогда
						// В случае использования счета 15, очищаем валюту, и валютную сумму
						НоваяПроводка.ВалютаКт = Справочники.Валюты.ПустаяСсылка();
						НоваяПроводка.ВалютнаяСуммаКт = 0;

					КонецЕсли;
					
					НоваяПроводка.СубконтоКт.Номенклатура = Номенклатура;
					НоваяПроводка.СубконтоДт.Номенклатура = Номенклатура;
					НоваяПроводка.СубконтоДт.Склады = Склад;
					
                    НоваяПроводка.КоличествоКт = 0;
					
				КонецЕсли;
				ИзменениеПроизведено = Истина;
			КонецЕсли;			
		КонецЦикла;
	...
	ИначеЕсли  ТипДокумента = Тип("ДокументСсылка.АвансовыйОтчет") Тогда
		...
		Если ВидУчета="БухгалтерскийУчет" Тогда
			Если ИмяТабЧасти = "Товары" или ИмяТабЧасти = "Услуги" Тогда
		        ...
		        СтруктураАналитикаКт.Вставить("ВидыПодотчетныхСумм","ВидПодотчетнойСуммы");//абсо
			ИначеЕсли ИмяТабЧасти = "Тара" Тогда
			...
			КонецЕсли;
		Иначе
			//+++ T#7723272 2017.05.05 Гущина
			Если ИмяТабЧасти = "Товары" или ИмяТабЧасти = "Услуги" Тогда
				СтруктураПриходТМЦ.Вставить("СчетКт", ?(СтруктураШапкиДокумента.ВалютаДокумента = мВалютаРегламентированногоУчета,ПланыСчетов.Налоговый.НайтиПоКоду("71.01"),ПланыСчетов.Налоговый.НайтиПоКоду("71.21")));
				СтруктураАналитикаКт.Вставить("РаботникиОрганизации","ФизЛицо");
				СтруктураАналитикаКт.Вставить("ВидыПодотчетныхСумм","ВидПодотчетнойСуммы");
			Иначе 
			//---  T#7723272 2017.05.05 Гущина. далее как было
				СтруктураПриходТМЦ.Вставить("СчетКт", ПланыСчетов.Налоговый.ПоступлениеИВыбытиеИмуществаРаботУслугПрав);
				СтруктураАналитикаКт.Вставить("УсловияПоступленияИВыбытия",Перечисления.УсловияПоступленияИВыбытияИмущества.ЗаПлату);
...
Процедура ВыполнитьРаспределениеДопРасходов(ДатаНачалаВосстановления, ДатаКонцаОбработки, СтОтборОстатков = Неопределено, Документ=Неопределено, ДатаДокумента=Неопределено) Экспорт
	...
	Если Документ<>Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТабПоступлений, ТаблицаДвижений);
...
Функция абс_ПолучитьТипыДокументовЧерез15Счет()
...
//АБС ВСТАВКА №29836 НАЧАЛО «11 декабря 2014 г.», Пополитов
Процедура абс_УстановитьПараметр_жМассивТипов(Запрос) Экспорт
	
	жМассивТипов = Новый Массив;
	жМассивТипов.Добавить(Неопределено);
	жМассивТипов.Добавить(Документы.ЗаказПоставщику.ПустаяСсылка());
	Запрос.УстановитьПараметр("жМассивТипов",жМассивТипов);
