...
Процедура ЗаполнитьКонтактнуюИнформациюПоОпросу(ФизическоеЛицо, Опрос, Вопрос, Раздел, ВидКИ, ТипКИ)
	...
	Если	ТипКИ = Перечисления.ТипыКонтактнойИнформации.Адрес	тогда
		...
		МассивПолей	=	СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаСОтветом.ТиповойОтвет,	"¤");
...
Функция ПолучитьПредставлениеКИ(ТипКИ, ЗначениеКИ) Экспорт
	...
	Если ТипКИ = Перечисления.ТипыКонтактнойИнформации.Адрес тогда
		МассивПолей = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ЗначениеКИ, "¤");
...
Функция ПолучитьОтветы(Выборка, Тип, ФлагКолонки = Ложь)
	...
	Иначе
		...
		Иначе
			...
			Иначе
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Справочник """+Объект.Синоним+""" не имеет ни одного элемента. Рекомендуется заполнить его и сформировать анкету еще раз.");
