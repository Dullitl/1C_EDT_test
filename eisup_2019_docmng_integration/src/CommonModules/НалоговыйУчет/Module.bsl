		...
		ИначеЕсли ТипЗнч(Субконто.Значение) = Тип("СправочникСсылка.ПрочиеДоходыИРасходы") И Не Субконто.Значение.ЭтоГруппа Тогда
			...
			Если НЕ Субконто.Значение.ПринятиеКналоговомуУчету Тогда
				...
				Если Проводка.СчетДт = ПланыСчетов.Налоговый.ВнереализационныеРасходыРаспределяемые Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Проводка " + Проводка.Содержание + ": неправильно указан счет затрат для не принимаемых расходов (Внереализационные расходы, подлежащие распределению (ЕНВД / не ЕНВД)).
...
//
Функция ОборотыВременныхРазницПоВидуАктивовОбязательств(СтрокаВидАктиваОбязательства, СтруктураДопПараметров) Экспорт
	...
	Массив = Новый Массив;
	Массив.Добавить(ПланыСчетов.Налоговый.КосвенныеОбщепроизводственныеРасходыРаспределяемые);
	Массив.Добавить(ПланыСчетов.Налоговый.КосвенныеОбщехозяйственныеРасходыРаспределяемые);
	Массив.Добавить(ПланыСчетов.Налоговый.ИздержкиОбращенияРаспределяемые);
	Массив.Добавить(ПланыСчетов.Налоговый.КоммерческиеРасходыРаспределяемые);
	Запрос.УстановитьПараметр("Распределяемые",    Массив);
	
	Массив = Новый Массив;
	Массив.Добавить(ПланыСчетов.Налоговый.РасходыПоДеятельностиЕНВД);
	Массив.Добавить(ПланыСчетов.Налоговый.ПустаяСсылка());
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	ВЫБОР
	               |		КОГДА НалоговыйОбороты.КорСубконто1 ССЫЛКА Справочник.ОсновныеСредства
	               |				И НалоговыйОбороты.Счет В ИЕРАРХИИ (&Распределяемые)
	               |			ТОГДА ЕСТЬNULL(НалоговыйОбороты.СуммаОборотДт, 0)
	               |		ИНАЧЕ 0
	               |	КОНЕЦ КАК СуммаЗатрат,
	               |	ВЫБОР
	               |		КОГДА НалоговыйОбороты.КорСубконто1 ССЫЛКА Справочник.РасходыБудущихПериодов
	               |				И НалоговыйОбороты.Счет В ИЕРАРХИИ (&Распределяемые)
	               |			ТОГДА ЕСТЬNULL(НалоговыйОбороты.СуммаОборотДт, 0)
	               |		ИНАЧЕ 0
	               |	КОНЕЦ КАК СуммаРБП,
	               |	ВЫБОР
	               |		КОГДА НалоговыйОбороты.КорСубконто1 ССЫЛКА Справочник.ОсновныеСредства
	               |				И НалоговыйОбороты.Счет В ИЕРАРХИИ (&ЕНВД)
	               |			ТОГДА ЕСТЬNULL(НалоговыйОбороты.СуммаОборотДт, 0)
	               |		ИНАЧЕ 0
...
Функция ОстаткиВременныхРазницПоВидуАктивовОбязательств(СтрокаВидАктиваОбязательства, Организация, НачалоГода, КонецГода) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |					ИЛИ НалоговыйОстатки.СуммаОстатокКт > 0
	...
	Если ФлагОС_НМА = 1 Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "НалоговыйОстатки.Счет КАК Счет,", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "НалоговыйОстатки.Счет,", "");
	КонецЕсли;
...
Процедура ЗаполнитьДанныеНалоговогоУчетаПоБухгалтерскомуУчету(ДанныеНУ, ДанныеБУ, Дата) Экспорт
	...
	Для каждого СтрокаОрганизаций Из ТаблицаОрганизаций Цикл
		ПараметрыУчетнойПолитикиРегл = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(ПериодЗаписей, СтрокаОрганизаций.Организация, Ложь);
	...
	Для Каждого Выборка из ДанныеБУ Цикл
		...
		Если  Не Выборка.СчетДт = ПланыСчетов.Хозрасчетный.ПустаяСсылка() И (Выборка.СчетДт.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.РасчетыСПоставщикамиИПодрядчиками) ИЛИ Выборка.СчетДт.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.РасчетыСПокупателямиИЗаказчиками) ИЛИ Выборка.СчетДт.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.РасчетыСРазнымиДебиторамиИКредиторами)) Тогда
			//АБС ВСТАВКА 31423  16.04.2015 13:45:10  Коломиец
			Если Не Выборка.СчетДт = ПланыСчетов.Хозрасчетный.ПлатежиПоДобровольномуСтрахованиюРаботников И Не Выборка.СчетДт = ПланыСчетов.Хозрасчетный.ПлатежиПоИмущественномуСтрахованию И Не Выборка.СчетДт = ПланыСчетов.Хозрасчетный.ПлатежиПоПрочимВидамСтрахования Тогда
			//АБС ВСТАВКА 31423 КОНЕЦ			
			...
			КонецЕсли; //АБС ВСТАВКА 31423  16.04.2015 13:45:10  Коломиец
		КонецЕсли;
		...
		Если  Не Выборка.СчетКт = ПланыСчетов.Хозрасчетный.ПустаяСсылка() И (Выборка.СчетКт.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.РасчетыСПоставщикамиИПодрядчиками) ИЛИ Выборка.СчетКт.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.РасчетыСПокупателямиИЗаказчиками) ИЛИ Выборка.СчетКт.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.РасчетыСРазнымиДебиторамиИКредиторами)) Тогда
			//АБС ВСТАВКА 31423  16.04.2015 13:45:10  Коломиец
			Если Не Выборка.СчетКт = ПланыСчетов.Хозрасчетный.ПлатежиПоДобровольномуСтрахованиюРаботников И Не Выборка.СчетКт = ПланыСчетов.Хозрасчетный.ПлатежиПоИмущественномуСтрахованию И Не Выборка.СчетКт = ПланыСчетов.Хозрасчетный.ПлатежиПоПрочимВидамСтрахования Тогда
			//АБС ВСТАВКА 31423 КОНЕЦ
