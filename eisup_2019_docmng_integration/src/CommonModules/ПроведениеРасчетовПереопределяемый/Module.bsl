		...
		Если ИсходныеДанные.НайтиСледующий(СтруктураПоиска) Тогда
			...
			Если СтрокаТабличнойЧасти <> Неопределено Тогда
				...
				Если ДополнительныеНачисленияОрганизаций И НеобходимыеДанные.ВычетыКДоходамНДФЛ И СтрокаДвижений.Авторасчет Тогда
					...
					ИначеЕсли СтрокаТабличнойЧасти.КодВычета = ВычетНДФЛКод403 Или СтрокаТабличнойЧасти.КодВычета = ВычетНДФЛМатпомощьПриРожденииРебенка Тогда
					Иначе
			...
			Если ЗаписиДоходыНДФЛ <> Неопределено И 
				...
				Если ОсновныеНачисленияОрганизаций 
					И (ИсходныеДанные.КодДоходаНДФЛ = Справочники.ДоходыНДФЛ.КодДоходаПоУмолчанию Или ИсходныеДанные.КодДоходаНДФЛ = Справочники.ДоходыНДФЛ.Код2012) Тогда
...
Процедура РассчитатьЗаписьРегистраРасчета(СтрокаДвижений,
	...
	Если СтрокаДвижений.Авторасчет Тогда
		...
		Если КомментироватьРасчет Тогда
			...
			ОбщегоНазначения.КомментарийРасчета("Порядок расчета: " + ПроведениеРасчетов.ВизуализироватьФормулуРасчета(ИсходныеДанные.ВидРасчета, "Текст", РегламентированныеУдержания), КомментарийВидаРасчета);
		КонецЕсли;
		...
		Если ПроведениеРасчетов.ПолучитьСписокНачисленийПроцентом().Найти(СпособРасчета) <> НеОпределено Тогда
			...
			Иначе
				Если КомментироватьРасчет Тогда
					КомментарийБазы = ОбщегоНазначения.КомментарийРасчета("Базовые начисления составили всего: " + РезультатБаза, КомментарийВидаРасчета,,,, Ложь);
				...
				Иначе
					...
					Если Не ИсходныеДанные.ВидРасчетаПериодДействияБазовый
						И (ИсходныеДанные.СпособРасчета <> Перечисления.СпособыРасчетаОплатыТруда.Процентом)
						И (ИсходныеДанные.СпособРасчета <> Перечисления.СпособыРасчетаОплатыТруда.ОтСтажаПроцентом)
						Тогда
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПроцентомОтОблагаемыхЕСННачислений Тогда
			...
			Если ИсходныеДанные.ОблагаемыйЕСНДоход = NULL Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + ": Отсутствуют доходы, облагаемые ЕСН ");
			Иначе
				Если КомментироватьРасчет Тогда
					КомментарийБазы = ОбщегоНазначения.КомментарийРасчета("Облагаемые доходы составили всего: " + ИсходныеДанные.ОблагаемыйЕСНДоход, КомментарийВидаРасчета,,,, Ложь);
					РасшифровкаБазы = ИсходныеДанные.Выбрать();
					Пока РасшифровкаБазы.Следующий() Цикл
						Если РасшифровкаБазы.ОблагаемыйЕСНДоход <> 0 Тогда
							ОбщегоНазначения.КомментарийРасчета(РасшифровкаБазы.ВидРасчетаРазрезНаименование + ": " + РасшифровкаБазы.ОблагаемыйЕСНДоход, КомментарийБазы);
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ИсполнительныйЛистПроцентом Тогда
			...
			Иначе
				...
				Результат = Макс(((РезультатБаза - ИсчисленныйНалог) * СтрокаДвижений[ПоказательРазмер]/100) - УжеНачисленоВТекущемПериоде, 0);
				
				Если КомментироватьРасчет Тогда
					КомментарийБазы = ОбщегоНазначения.КомментарийРасчета("Базовые начисления составили всего: " + РезультатБаза, КомментарийВидаРасчета,,,, Ложь);
					ОбщегоНазначения.КомментарийРасчета("Исчислено НДФЛ за текущий период: " + ИсчисленныйНалог, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета("Ранее начислено в текущем периоде: " + УжеНачисленоВТекущемПериоде, КомментарийВидаРасчета);
				КонецЕсли;
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработкуФСС Тогда
			...
			Если ПроведениеРасчетов.РассчитатьРесурсыПоАлгоритму("ПоСреднемуЗаработкуФСС213ФЗ", КомментироватьРасчет, КомментарийВидаРасчета, СтрокаДвижений, ИсходныеДанные) Тогда
				Результат = СтрокаДвижений.Результат
			Иначе	
				...
				Если КомментироватьРасчет Тогда
					...
					ОбщегоНазначения.КомментарийРасчета(?(СтрокаДвижений.ДатаНачалаСобытия < ДатаЗакона255ФЗ И СтрокаДвижений.ВидУчетаВремени = Перечисления.ВидыУчетаВремени.ПоЧасам,"Среднечасовой заработок: ","Среднедневной заработок: ") + ЗаработокЗаЕдиницуВремени, КомментарийВидаРасчета);
				КонецЕсли;
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеПоУходуЗаРебенкомДо1_5Лет Тогда
			...
			Если ИсходныеДанные.ПериодДействияНачало < ДатаЗакона255ФЗ Тогда
				...
				Если КомментироватьРасчет Тогда
					...
					ОбщегоНазначения.КомментарийРасчета("Размер пособия за месяц: " + ИсходныеДанные.ПоУходуЗаРебенкомДоПолутораЛет * Макс(ИсходныеДанные.РайонныйКоэффициентРФОрганизации, 1), КомментарийВидаРасчета);
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеПоУходуЗаРебенкомДо3Лет Тогда
			...
			Если КомментироватьРасчет Тогда
				...
				ОбщегоНазначения.КомментарийРасчета("Оплачиваемые дни отпуска: " + ОплачиваемыеДни, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета("Учтенный районный коэффициент: " + Макс(ИсходныеДанные.РайонныйКоэффициентРФОрганизации, 1), КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета("Размер пособия за месяц: " + ИсходныеДанные.РазмерПособияПоУходуЗаРебенкомДо3Лет * Макс(ИсходныеДанные.РайонныйКоэффициентРФОрганизации, 1), КомментарийВидаРасчета);
			КонецЕсли;
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.СдельныйЗаработок Тогда
			...
			Если ИсходныеДанные.РазмерСдельногоЗаработка = NULL Тогда
				Если ИсходныеДанные.ОшибкаВводаФактическойВыработки Тогда
					ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + ": Данные о фактической выработке введены сводно. Расчет суммы по данному виду расчета невозможен");
				Иначе
			...
			Если ЗначениеЗаполнено(ФормулаРасчета) Тогда //Возможно, что формула пустая, тогда результат вводится руками
				...
				Если РезультатПолучен Тогда
					...
					Если ИсходныеДанные.РасчетИмеетБазовыеНачисления Тогда
						...
						Если КомментироватьРасчет Тогда
							КомментарийБазы = ОбщегоНазначения.КомментарийРасчета("Базовые начисления составили всего: " + РасчетнаяБаза, КомментарийВидаРасчета,,,, Ложь);
					...
					Исключение					
						ОбщегоНазначения.КомментарийРасчета(ИсходныеДанные.СотрудникНаименование + "; " +" При расчете " + ИсходныеДанные.ВидРасчетаНаименование + ОписаниеОшибки(), КомментарийВидаРасчета, ,,Перечисления.ВидыСообщений.Ошибка);
					КонецПопытки;
...
Функция ОплачиваемоеВремя(ИсходныеДанные, СтрокаДвижений, КомментироватьРасчет, КомментарийВладелец, СообщениеОбОшибке = Ложь, ГрафикиБезРабочегоВремени)
	...
	Если КомментироватьРасчет Тогда
		...
		Если УчтеныВнутрисменныеОтклонения Тогда
			ОбщегоНазначения.КомментарийРасчета("При расчете оплачиваемого времени учтены внутрисменные невыходы (в часах) " + ИсходныеДанные.ОтработаноЧасовПоТабелюОтклонение, КомментарийВладелец);
		КонецЕсли;
		...
		Пока ВыборкаБазы.НайтиСледующий(СтруктураПоиска) Цикл
			...
			Иначе
				...
				Если КомментироватьРасчет и ВыборкаБазы.РезультатБаза <> 0 Тогда
					ОбщегоНазначения.КомментарийРасчета("Вид начисления: " + ВыборкаБазы.ВидРасчетаНаименование + ", сумма: " + ВыборкаБазы.РезультатБаза + ?(ВыборкаБазы.СкидкаПоСтраховымВзносам = 0,"",", учтено скидки" + ВыборкаБазы.СкидкаПоСтраховымВзносам), КомментарийРасчетаСреднего);
				КонецЕсли;
			КонецЕсли;
...
Функция ПолучитьДанныеДляРасчета(ИмяРегистра,
	...
	//
	ОблагаемыеЕСНДоходыТекст = 
	"ВЫБРАТЬ
	|	Основной.ФизЛицо КАК ФизЛицо,
	|	Основной.Организация КАК Организация,
	|	НАЧАЛОПЕРИОДА(Основной.БазовыйПериодНачало, МЕСЯЦ) КАК Начало,
	|	НАЧАЛОПЕРИОДА(Основной.БазовыйПериодКонец, МЕСЯЦ) КАК Окончание,
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		РассчитываемыеЗаписи.ФизЛицо КАК ФизЛицо,
	|		РассчитываемыеЗаписи.Организация КАК Организация,
	|		РассчитываемыеЗаписи.ОбособленноеПодразделение КАК ОбособленноеПодразделение,
	|		РассчитываемыеЗаписи.НомерСтроки КАК НомерСтроки,
	|		РассчитываемыеЗаписи.Регистратор КАК Регистратор,
	...
	|				И (НАЧАЛОПЕРИОДА(ДополнительныеНачисленияРаботниковОрганизаций.ПериодРегистрации, МЕСЯЦ) МЕЖДУ РассчитываемыеЗаписи.Начало И РассчитываемыеЗаписи.Окончание)
	|	ГДЕ
	...
	|	
	|	ВЫБРАТЬ
	|		РассчитываемыеЗаписи.ФизЛицо,
	|		РассчитываемыеЗаписи.Организация,
	|		РассчитываемыеЗаписи.ОбособленноеПодразделение,
	|		РассчитываемыеЗаписи.НомерСтроки,
	|		РассчитываемыеЗаписи.Регистратор,
	...
	|			ПО РассчитываемыеЗаписи.Организация = ОсновныеНачисленияРаботниковОрганизаций.Организация
	...
	|				И (НАЧАЛОПЕРИОДА(ОсновныеНачисленияРаботниковОрганизаций.ПериодРегистрации, МЕСЯЦ) МЕЖДУ РассчитываемыеЗаписи.Начало И РассчитываемыеЗаписи.Окончание)
	|	ГДЕ
	|		ОсновныеНачисленияРаботниковОрганизаций.ВидРасчета.КодДоходаЕСН.ВходитВБазуФедеральныйБюджет) КАК Доходы
	|
	|СГРУППИРОВАТЬ ПО
	|	Доходы.ФизЛицо,
	|	Доходы.Организация,
	...
	|			ФактическаяВыработка.Выработка КАК Выработка,
	|			СУММА(ВЫБОР
	...
	// ОтработаноНЕПоВидуВремениДляПоказателейТекст
	//	Данные по отработанному времени введенное табелем НЕ по указанному в начислении виду учета времени
	УпрОтработаноНЕПоВидуВремениДляПоказателейТекст = 
	"ВЫБРАТЬ
	|	Основной.НомерСтроки КАК НомерСтроки,
	|	СУММА(ВЫБОР
	...
	Если ИмяРегистра = "ОсновныеНачисленияРаботниковОрганизаций" Тогда
		...
		Если НеобходимыеДанные.КалендарныеДниПособия Тогда
			...
			ПоСреднемуЗаработкуФСС = Новый Массив(3);
			ПоСреднемуЗаработкуФСС[0] = Перечисления.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработкуФСС;
			ПоСреднемуЗаработкуФСС[1] = Перечисления.СпособыРасчетаОплатыТруда.ПособиеПоУходуЗаРебенкомДо1_5Лет;
			ПоСреднемуЗаработкуФСС[2] = Перечисления.СпособыРасчетаОплатыТруда.ДоплатаДоСреднегоЗаработкаФСС;
			Запрос.УстановитьПараметр("ПоСреднемуЗаработкуФСС", ПоСреднемуЗаработкуФСС);
		...
		Если НеобходимыеДанные.ДанныеДляПрогнозаНДФЛ Тогда
			...
			|	ИЗ РегистрСведений.ГражданствоФизЛиц.СрезПоследних(&парамКонецГода, Физлицо В (ВЫБРАТЬ РАЗЛИЧНЫЕ Физлицо ИЗ ВТСотрудникиСДоначислением)) КАК ГражданствоФизЛицСрезПоследних
	...
	ИначеЕсли 	ИмяРегистра = "УправленческиеНачисления" или 
		...
		Если НеобходимаБаза Тогда
			Если КомментироватьРасчет И ИмяРегистра = "УправленческиеНачисления" Тогда
				ТекстЗапроса = ТекстЗапроса + ",
				|База.РезультатБаза Как РезультатБаза,
				|База.ВидРасчетаРазрез КАК ВидРасчетаРазрез,
				|База.ВидРасчетаРазрез.Наименование КАК ВидРасчетаРазрезНаименование";
		...
		Если НеобходимГрафик Тогда
			...
			Запрос.Текст = УпрОтработаноНЕПоВидуВремениДляПоказателейТекст;
			Запрос.Выполнить();
...
Функция ПолучитьТекстПоказателя(НеобходимыеДанные, ВидПВР, Предопределенный = Ложь, КомментироватьРасчет)		
	...
	Если 	НеобходимыеДанные.СдельныйЗаработок или 
		...
		+ ?(ВидПВР = "ОсновныеНачисленияРаботниковОрганизаций", "
			...
			Если ВозможностьИзменения = Перечисления.ИзменениеПоказателейСхемМотивации.НеИзменяется И ЗапрашиватьПриКадровыхПеремещениях
				...
				Если ВыборкаПоСтрокамДокумента["Показатель" + СчПоказателя] = 0 Тогда
					...
					Если Не ВыборкаПоСтрокамДокумента["Показатель" + СчПоказателя + "Предопределенный"]
						...
						ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + " не указан размер показателя " + ВыборкаПоСтрокамДокумента["Показатель" + СчПоказателя+"Наименование"], Отказ, Заголовок);
					КонецЕсли;
				КонецЕсли;
				Если ЭтоНачисление Тогда
					Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента["ТарифныйРазряд" + СчПоказателя])
						и ТипПоказателя = Перечисления.ТипыПоказателейСхемМотивации.ТарифныйРазряд Тогда
						ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указан размер тарифа " + ВыборкаПоСтрокамДокумента["Показатель" + СчПоказателя+"Наименование"], Отказ, Заголовок);
					КонецЕсли;	
				КонецЕсли;	
	...
	Иначе
		...
		Если ВыборкаПоСтрокамДокумента.КоличествоПоказателей > 0 И ВыборкаПоСтрокамДокумента.Действие <> Перечисления.ВидыДействияСНачислением.Прекратить И ПроведениеРасчетов.СпособРасчетаТребуетРазмер(ВыборкаПоСтрокамДокумента.СпособРасчета) Тогда
			Если ЭтоНачисление И ВыборкаПоСтрокамДокумента.Показатель1 = 0 и Не ВыборкаПоСтрокамДокумента.ТребуетВводаТарифногоРазряда Тогда
				ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указан размер начисления!", Отказ, Заголовок);
			...
			ИначеЕсли Не ЭтоНачисление И ВыборкаПоСтрокамДокумента.Показатель1 = 0 Тогда 
				ОбщегоНазначения.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указан размер удержания!", Отказ, Заголовок);
...
Процедура УстановитьПараметрыВидаРасчета(Параметр, Объект, Элементыформы) Экспорт
	...
	ЭтоПлановоеНачисление = МетаданныеОбъекта.Реквизиты.Найти("ЗачетОтработанногоВремени") <> Неопределено;
		...
		Если ОбъектПоказатели.Найти(Показатель,"Показатель") = Неопределено Тогда
			...
			Если Показатель.ТипПоказателя <> Перечисления.ТипыПоказателейСхемМотивации.ОценочнаяШкалаПроцентная И
				...
				Если (Показатель.ТарифнаяСтавка Или Показатель = Справочники.ПоказателиСхемМотивации.ТарифнаяСтавкаДневная Или
					...
					Если Не ЭтоПлановоеНачисление Тогда
	...
	Элементыформы.СпособРасчетаПредставление.УстановитьТекст(ПроведениеРасчетов.ВизуализироватьФормулуРасчета(Объект, "HTML", МетаданныеОбъекта.Имя = "УдержанияОрганизаций"));
...
Процедура СформироватьРасчетыПоНДФЛПриВыплате(ВыборкаПоШапкеДокумента, ТекстТаблицыВыплаты = Неопределено, НаборЗаписейНДФЛ = Неопределено) Экспорт
