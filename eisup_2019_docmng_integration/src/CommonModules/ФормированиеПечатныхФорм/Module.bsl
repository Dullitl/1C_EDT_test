...
Функция СформироватьАвтоЗначенияРеквизитовПлательщикаПолучателя(Плательщик, СчетПлательщика, Получатель, СчетПолучателя, ВидОперации, ПеречислениеВБюджет = Ложь) Экспорт
	...
	//++КТТК Готовцев 26.02.2018
				//для агрегированного контрагента убираем ИНН и КПП	
	Если ВладелецСчетаПолучателя.ИНН = "9920955555" ТОгда
		РеквизитыВладельцаСчетаПолучателя.ИНН = "";
		РеквизитыВладельцаСчетаПолучателя.КПП = "";
	КонецЕсли;	
	//--КТТК Готовцев 26.02.2018
