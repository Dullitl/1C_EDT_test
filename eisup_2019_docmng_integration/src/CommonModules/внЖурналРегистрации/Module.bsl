...
Функция ПолучитьИсториюИзменений(ДатаНач, ДатаКон, МассивПользователи, МассивКомпьютеры, МассивМетаданных,
	...
	Если СоединениеДоступно И ЗначениеЗаполнено(МассивОбъектовСсылок) И МассивОбъектовСсылок.Количество() > 0 Тогда
		...
		Для Каждого Элемент Из МассивОбъектовСсылок Цикл
			...
			" ПОДСТРОКА(внИзменения.Значение, 1, 300) = &ЭлементМассивОбъектовСсылок" + Формат(Индекс, "ЧРД=; ЧРГ=; ЧН=0; ЧГ=");
			Параметры2.Вставить("ЭлементМассивОбъектовСсылок" + Формат(Индекс, "ЧРД=; ЧРГ=; ЧН=0; ЧГ="), Элемент);
		...
		ТекстВЗапросе2 = ТекстВЗапросе2 + 
		"И(внКэшЖурналаРегистрации.Ссылка В
		|(ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	внИзменения.Изменение
		|ИЗ
		|	РегистрСведений.внИзменения КАК внИзменения
...
Функция ПолучитьНеРегистрируемыеОбъекты() Экспорт
	
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	внНеРегистрируемыеОбъекты.МетаданныеОбъекта КАК МетаданныеОбъекта,
		|	внНеРегистрируемыеОбъекты.ИмяТЧ КАК ИмяТЧ,
		|	внНеРегистрируемыеОбъекты.ИмяРеквизита
		|ИЗ
...
КонецФункции // ПолучитьНеРегистрируемыеОбъекты()
...
Функция ДеСериализоватьОбъект(Данные, ЭлементКэша) Экспорт
	...
	НеРегистрируемое 			= внЖурналРегистрацииПовтИсп.НеРегистрируемыеОбъектыМетаданных(ЭлементКэша.МетаданныеОбъекта);
	РегистрироватьПоРеквизитам	= НеРегистрируемое.Количество() > 0;
	...
	Для каждого Реквизит Из Данные.Реквизиты Цикл
		...
		Если РегистрироватьПоРеквизитам Тогда
			НайденныеСтроки = НеРегистрируемое.НайтиСтроки(Новый Структура("ИмяТЧ,ИмяРеквизита", "", РеквизитИмя));
	...
	Для Каждого Таблица Из Данные.ТабличныеЧасти Цикл
		...
		Если РегистрироватьПоРеквизитам Тогда
			НайденныеСтроки = НеРегистрируемое.НайтиСтроки(Новый Структура("ИмяТЧ,ИмяРеквизита", ТаблицаИмя, ""));
			Если НайденныеСтроки.Количество() > 0 Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		...
		Для Каждого Реквизит Из ТаблицаЗначение.Колонки Цикл
			...
			// Пропускаем регистрацию изменений для не регистрируемых реквизитов
			Если РегистрироватьПоРеквизитам Тогда
				НайденныеСтроки = НеРегистрируемое.НайтиСтроки(Новый Структура("ИмяТЧ,ИмяРеквизита", ТаблицаИмя, РеквизитИмя));
				Если НайденныеСтроки.Количество() > 0 Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
...
Процедура внЖурналРегистрацииПередЗаписью(Источник, Отказ) Экспорт
	...
	Если НЕ Отказ Тогда
		...
		//Сладков ++
		Источник.ДополнительныеСвойства.Вставить("Новый"	, Источник.Ссылка.Пустая());
		//Сладков --
...
Процедура РегистрацияИзмененияОбъекта(Знач Объект, НачальноеЗаполнение = Ложь) Экспорт
	...
	ВремяНачалаЗаписиКлиент = ТекущаяУниверсальнаяДатаВМиллисекундах();
	...
	НеРегистрируемое = внЖурналРегистрацииПовтИсп.НеРегистрируемыеОбъектыМетаданных(ПолноеИмя);	
	Если НеРегистрируемое.Количество() = 1 Тогда
		// В настройках указано, что бы НЕ регистрировались изменения объекта
		Если ПустаяСтрока(НеРегистрируемое[0].ИмяТЧ) И ПустаяСтрока(НеРегистрируемое[0].ИмяРеквизита) Тогда
			Возврат;
		КонецЕсли;		
	КонецЕсли;	 
	РегистрироватьПоРеквизитам	= НеРегистрируемое.Количество() > 0;
	...
	ИначеЕсли ТипОбъекта = 10 Тогда
		...
		Для каждого Реквизит Из ОбъектМетаданные.СтандартныеРеквизиты Цикл
			...
			// Пропускаем регистрацию изменений для не регистрируемых реквизитов
			Если РегистрироватьПоРеквизитам Тогда
				НайденныеСтроки = НеРегистрируемое.НайтиСтроки(Новый Структура("ИмяТЧ,ИмяРеквизита", "", Реквизит.Имя));
				Если НайденныеСтроки.Количество() > 0 Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
	...
	Иначе
		...
		Для каждого Реквизит Из Реквизиты Цикл
			...
			// Пропускаем регистрацию изменений для не регистрируемых реквизитов
			Если РегистрироватьПоРеквизитам Тогда
				ПараметрыОтбора = Новый Структура("ИмяТЧ,ИмяРеквизита", "", РеквизитИмя);
				НайденныеСтрвоки = НеРегистрируемое.НайтиСтроки(ПараметрыОтбора);		
				Если НайденныеСтроки.Количество() > 0 Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
		...
		Если ТипОбъекта = 5 Тогда
			...
			Для Каждого Признак Из ОбъектМетаданные.ПризнакиУчета Цикл
				...
				Если РегистрироватьПоРеквизитам Тогда
					НайденныеСтроки = НеРегистрируемое.НайтиСтроки(Новый Структура("ИмяТЧ,ИмяРеквизита", "ПризнакиУчета", Признак.Имя));
					Если НайденныеСтроки.Количество() > 0 Тогда
						Продолжить;
					КонецЕсли;
				КонецЕсли;
			...
			Для Каждого Элемент Из Массив Цикл
				...
				Если РегистрироватьПоРеквизитам Тогда
					НайденныеСтроки = НеРегистрируемое.НайтиСтроки(Новый Структура("ИмяТЧ,ИмяРеквизита", "ВидыСубконто", Элемент));
					Если НайденныеСтроки.Количество() > 0 Тогда
						Продолжить;
					КонецЕсли;
				КонецЕсли;
		...
		ИначеЕсли ТипОбъекта = 6 Тогда
			...
			Для каждого ИмяВидаРасчета Из Массив Цикл			
				...
				Попытка
					НайденныеСтроки = НеРегистрируемое.НайтиСтроки(Новый Структура("ИмяТЧ,ИмяРеквизита", ИмяВидаРасчета, ""));
					Если НайденныеСтроки.Количество() = 0 Тогда
						Для Каждого Строки Из Объект[ИмяВидаРасчета] Цикл
							
							Если РегистрироватьПоРеквизитам Тогда
								НайденныеСтроки = НеРегистрируемое.НайтиСтроки(Новый Структура("ИмяТЧ,ИмяРеквизита", ИмяВидаРасчета, "ВидРасчета"));
								Если НайденныеСтроки.Количество() > 0 Тогда
									Продолжить;
								КонецЕсли;						
							КонецЕсли;
	...
	Иначе
		...
		// {{ТТК Сладков А. Заявка № 24.05.2016 начало
		ИначеЕсли Объект.ДополнительныеСвойства.Новый = Истина Тогда
			Событие = 1; // изменение
		// {{ТТК Сладков А. Заявка № 24.05.2016 окончание
	...
	ВремяКонцаВыполненияКлиент      = ТекущаяУниверсальнаяДатаВМиллисекундах();
	ВремяЗаписиВМиллисекундахКлиент =  ВремяКонцаВыполненияКлиент-ВремяНачалаЗаписиКлиент;
	// }}ТТК Сладков А. Заявка № 24.05.2016 окончание
	
	// Записываем
	внЖурналРегистрацииСервер.РегистрацияИзмененияОбъекта(ИзмененияТЧ, ИдентификаторОбъекта, ПолучитьУзелРИБ(Объект), ПредставлениеОбъекта, ПолноеИмя, ТипОбъекта, Событие,ВремяЗаписиВМиллисекундахКлиент);
