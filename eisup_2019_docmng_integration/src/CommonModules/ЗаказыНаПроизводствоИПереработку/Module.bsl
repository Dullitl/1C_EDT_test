...
Процедура ВыполнитьПогашениеПотребностиЗаказаНаПроизводствоПоРаспределению(ДокОбъект, ВидДокумента, ИмяРеквизитаЗаказ, СтруктураШапкиДокумента, ИмяТабличнойЧастиПродукция)
	...
	Иначе	
     ...
     Заголовок = ttk_ОбщегоНазначения.ПредставлениеДокументаПриПроведении(ДокОбъект);
	...
	Пока ОбходПоЗаказам.Следующий() Цикл
		...
		Если СообщитьОбОшибке Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрОшибка, Ложь, Заголовок);
		КонецЕсли; 
	...
	ttk_ОбщегоНазначения.ЗаписатьТаблицыДокументаВРегистр(НаборДвижений, ВидДвиженияНакопления.Расход, ТаблицыДанныхДокумента, ДокОбъект.Дата);
...
Процедура ЗаполнитьТабличнуюЧастьПродукцияПоОстаткамЗаказовНаПроизводство(ТабЧасть, ЗаказНаПроизводство, ДатаОстатков, ДопПараметры = Неопределено) Экспорт
	...
	ТекстЗаказ = "Размещение под заказ: " + ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ЗаказНаПроизводство) + " превышает остаток по данному заказу.";
			...
			Пока Обход.Следующий() Цикл
				...
				Иначе
					...
					Если ФлагЕстьОшибка Тогда
						ТекстСтр = ТекстСтр + Символы.ПС + " - По заказу: """ + ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(Обход.ЗаказПокупателя) + """ размещено: " + Обход.Размещено;
					КонецЕсли;
...
Процедура ЗаполнитьТабличнуюЧастьМатериалыПоОстаткам(ТабЧасть, ЗаказНаПроизводство, ДопПараметры = Неопределено) Экспорт
	...
	МассивОстатков    = Новый Массив;
	СоответствиеСтрок = Новый Соответствие;
	ТекстЗаказ = "Резерв (размещение) под заказ: " + ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ЗаказНаПроизводство) + " превышает потребность по данному заказу.";
	ФлагЕстьОшибкаРазмещения = Ложь;
			...
			Пока Обход.Следующий() Цикл
				...
				Иначе
					...
					Если ФлагЕстьОшибка Тогда
						...
						Иначе
							ТекстСтр = ТекстСтр + Символы.ПС + " - Заказ: """ + ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(Обход.Размещение) + """ размещено: " + Обход.КолРезерв;
						КонецЕсли;
					КонецЕсли;
