...
Процедура ОткрытьФормуВыбораПлатежныхВедомостейОрганизации(Элемент, СтандартнаяОбработка, Организация, СпособыВыплатыЗарплаты, Банк = Неопределено) Экспорт
	...
	Если Константы.абс_СинхронизацияВыплатыЗарплаты.Получить() Тогда
		ФормаВыбора.Отбор.абс_Статус.ВидСравнения	= ВидСравнения.Равно;
		ФормаВыбора.Отбор.абс_Статус.Значение		= Перечисления.абс_СтатусыВыплатыЗарплаты.Рассчитан;
		ФормаВыбора.Отбор.абс_Статус.Использование	= Истина;
	КонецЕСли;	
	//\\АБС--- Гончаров 17.04.2013
	
	// КТТК Ермолов Е.Л.  05.11.2014 000028515 Если Банк <> Неопределено Тогда
...
Функция ПодготовитьДанныеПоУправленческимОтпускам(
	...
	Если ВключатьЗапланированныеОтпуска Тогда
		Запрос.УстановитьПараметр("Регистратор",	Регистратор);
		Запрос.УстановитьПараметр("РабочаяДата",	ОбщегоНазначенияЗК.ПолучитьРабочуюДату());
	КонецЕсли;
