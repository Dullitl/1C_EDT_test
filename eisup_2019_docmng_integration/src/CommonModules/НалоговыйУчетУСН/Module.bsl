...
Функция ВидОперацииДДС(СтруктураШапкиДокумента) //
	...
	ИначеЕсли (ВидОперации = Перечисления.ВидыОперацийРКО.ВыдачаДенежныхСредствПодотчетнику) ИЛИ
		...
		(ВидОперации = Перечисления.ВидыОперацийСписаниеБезналичныхДенежныхСредств.ПеречислениеДенежныхСредствПодотчетнику) Тогда
...
Функция ИзменениеСтатусовРасходов(СтруктураШапкиДокумента, ТаблицаРасходов, Сумма = 0, ВидОперации = "Оплата", КонтролироватьСумму = Истина, ДокументОплаты = Неопределено, ВходящаяСтруктура = Неопределено)
	...
	Для Каждого Расход Из ТаблицаРасходов Цикл
		...
		Если ЗначениеЗаполнено(Расход.РеквизитыДокументаОплаты) Тогда
			УчетнаяПолитикаДокумент = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(Расход.РеквизитыДокументаОплаты.Дата, СтруктураШапкиДокумента.Организация, Ложь);
		...
		ИначеЕсли ЗначениеЗаполнено(Расход.РасчетныйДокумент) Тогда
			УчетнаяПолитикаДокумент = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(Расход.РасчетныйДокумент.Дата, СтруктураШапкиДокумента.Организация, Ложь);
			ВыделятьНДСУСНДокумент = ?(НЕ ЗначениеЗаполнено(УчетнаяПолитикаДокумент), ВыделятьНДСУСН, (УчетнаяПолитикаДокумент.ПорядокПризнанияРасходовПоНДС = Перечисления.ПорядокПризнанияРасходовПоНДС.ПоОплатеПоставщику));
		Иначе
...
Процедура ПрочееДДС(Документ) Экспорт
	...
	Если (Документ.Графа4_УСН = 0) И
		...
		Возврат;
	КонецЕсли;
	Если НЕ Документ.ОтражатьВНалоговомУчете Тогда
		Возврат;
	КонецЕсли;
...
//
Процедура ДвижениеВзиморасчетовУСН(ТаблицаДвижений, Организация, Договор = Неопределено, РасчетныйДокумент, Сделка, СуммаДокумента, СуммаЕНВД=0, СуммаКомиссии=0, Работник = Неопределено)
...
Процедура ВыделитьБезналичнуюОплату(СтруктураШапкиДокумента, ТаблицаДвижений, СуммаВзаиморасчетов, СуммаЕНВД, СуммаКомиссии)
	...
	Если (НЕ СтруктураШапкиДокумента.Ссылка.Метаданные().ТабличныеЧасти.Найти("ОплатаПлатежнымиКартами") = НЕОПРЕДЕЛЕНО) И
		...
		Пока Результат.Следующий() Цикл
			...
			ДвижениеВзиморасчетовУСН(ТаблицаДвижений, СтруктураШапкиДокумента.Организация, Результат.ДоговорКонтрагента, СтруктураШапкиДокумента.Ссылка, СтруктураШапкиДокумента.Ссылка, Результат.СуммаОплаты, СуммаЕНВД_БН, СуммаКомиссии_БН);
...
Процедура ПоступлениеРасходовУСН(СтруктураШапкиДокумента, ТаблицаРасходов, ТаблицаДвижений, ВидРасхода, Договор = Неопределено, СтатусПартии, СуммаВключаетНДС = Истина, ВозвратБезУказанияПартии = Ложь, СторноРасхода = 0)  Экспорт //
	...
	Для Каждого Расход из ТаблицаРасходов Цикл
		...
		Если УчетАгентскогоНДС Тогда
			...
			СтрокаДвиженийНДС.СчетУчета 			= ПланыСчетов.Хозрасчетный.НДС;
...
Процедура ЗачетАвансаПоставщикуУСН(СтруктураШапкиДокумента, ТаблицаДвижений, ДвиженияРегистров, Договор, Сделка, ТаблицаРД, СуммаДокумента, ПриходОтражен = Ложь)
	...
	Для Каждого СтрокаРД Из ТаблицаРД Цикл
		...
		КонецЦикла;
		Если СуммаОсталосьПогаситьПоСтроке > 0 Тогда
			ДвижениеВзиморасчетовУСН(ТаблицаДвиженияВзаиморасчетов, СтруктураШапкиДокумента.Организация, Договор, СтруктураШапкиДокумента.Ссылка, СтрокаРД.Сделка, СуммаОсталосьПогаситьПоСтроке);
...
Процедура ЗачетАвансаОтПокупателяУСН(СтруктураШапкиДокумента, ТаблицаДвижений, ДвиженияРегистровВх, ДвиженияРегистров, Договор, Сделка, ТаблицаРД, СуммаДокумента, СуммаЕНВД, СуммаКомиссии, КУДиР)
	...
	Для Каждого СтрокаРД Из ТаблицаРД Цикл
		...
		Если СуммаОсталосьПогаситьПоСтроке > 0 Тогда
			...
			ДвижениеВзиморасчетовУСН(ТаблицаДвиженияВзаиморасчетов, СтруктураШапкиДокумента.Организация, Договор, СтруктураШапкиДокумента.Ссылка, СтрокаРД.Сделка, СуммаВзаиморасчетов, СуммаЕНВДОсталосьПогаситьПоСтроке, СуммаКомиссииОсталосьПогаситьПоСтроке);
		КонецЕсли;
	...
	Для Каждого СтрокаРД Из ТаблицаРД Цикл
		...
		Для Каждого Строка Из НайденныеСтроки Цикл
			...
			Если УдержатьВознаграждение Тогда
				...
				ТаблицаПродаж = ВыполнитьЗапросПоВзаиморасчетамУСН(Новый Граница(СтруктураШапкиДокумента.Ссылка.МоментВремени(), ВидГраницы.Исключая), СтруктураШапкиДокумента.Организация, ТаблицаФильтров, Истина);
		...
		Если СуммаОсталосьПогаситьПоСтроке > 0 Тогда
			...
			ДвижениеВзиморасчетовУСН(ТаблицаДвиженияВзаиморасчетов, СтруктураШапкиДокумента.Организация, Договор, СтруктураШапкиДокумента.Ссылка, СтрокаРД.Сделка, СуммаВзаиморасчетов, СуммаЕНВДОсталосьПогаситьПоСтроке, СуммаКомиссииОсталосьПогаситьПоСтроке);
			Если УдержатьВознаграждение Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Учет УСН: Не достаточно выручки комитента. Удержано из выручки комитента " + Строка(СуммаДокумента - СуммаОсталосьПогасить) + " из " + Строка(СуммаДокумента));
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
//
Процедура ОплатаПоставщикуУСН(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, КУДиР, ВидОперации = "Оплата", ИмяПоляСделка = "Сделка") //
	...
	МассивСделокВЗапрос = Новый Массив;
	Для Каждого ЭлементМассива Из МассивСделок Цикл
		Если (ЗначениеЗаполнено(ЭлементМассива)) И (ИмяПоляСделка = "РасчетныйДокумент") Тогда
			МассивСделокВЗапрос.Добавить(ЭлементМассива);
		КонецЕсли;
	КонецЦикла;
	Если МассивСделокВЗапрос.Количество() > 0 Тогда
		Фильтр = ТаблицаФильтров.Добавить();
		Фильтр.ИмяПоля		= ИмяПоляСделка;
		Фильтр.ЗначениеПоля	= МассивСделокВЗапрос;
		Фильтр.Условие		= "В";
	КонецЕсли;
	...
	Для Каждого СтрокаПлатеж Из ТаблицаОплат Цикл
		...
		КонецЕсли;
		Если (ЗначениеЗаполнено(СтрокаПлатеж.Сделка)) И (ИмяПоляСделка = "РасчетныйДокумент") Тогда
			СтруктураОтбора.Вставить(ИмяПоляСделка, ?(НЕ ЗначениеЗаполнено(СтрокаПлатеж.Сделка), Неопределено, СтрокаПлатеж.Сделка));
		Иначе
			Если ТипЗнч(СтрокаПлатеж.ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
				Если НЕ СтрокаПлатеж.ДоговорКонтрагента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоДоговоруВЦелом Тогда
					Если (ИмяПоляСделка = "Сделка") Тогда
						СтруктураОтбора.Вставить(ИмяПоляСделка, ?(НЕ ЗначениеЗаполнено(СтрокаПлатеж.Сделка), Неопределено, СтрокаПлатеж.Сделка));
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		...
		КонецЦикла;
		Если СуммаОсталосьПогасить > 0 Тогда
			ДвижениеВзиморасчетовУСН(ТаблицаДвиженияВзаиморасчетов, СтруктураШапкиДокумента.Организация, СтрокаПлатеж.ДоговорКонтрагента, СтруктураШапкиДокумента.Ссылка, СтрокаПлатеж.Сделка, СуммаОсталосьПогасить,,,СтрокаПлатеж.Работник);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СтрокаПартии Из ТаблицаДвиженияВзаиморасчетов Цикл
		...
		Для Каждого Строка Из НайденныеСтроки Цикл
			...
			Если ЗначениеЗаполнено(Строка.РасчетныйДокумент) Тогда
				УчетнаяПолитикаДокумент = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(Строка.РасчетныйДокумент.Дата, СтруктураШапкиДокумента.Организация, Ложь);
				ВыделятьНДСУСНДокумент = ?(НЕ ЗначениеЗаполнено(УчетнаяПолитикаДокумент), ВыделятьНДСУСН, (УчетнаяПолитикаДокумент.ПорядокПризнанияРасходовПоНДС = Перечисления.ПорядокПризнанияРасходовПоНДС.ПоОплатеПоставщику));
			Иначе
				ВыделятьНДСУСНДокумент = ВыделятьНДСУСН;
			КонецЕсли;
...
//
Процедура ОплатаОтПокупателяУСН(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, КУДиР, ИмяПоляСделка = "Сделка")
	
	//Подготовка движений регистра взаиморасчетов
	УчетнаяПолитика = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(СтруктураШапкиДокумента.ДатаОплаты, СтруктураШапкиДокумента.Организация);
	...
	Для Каждого СтрокаПлатеж Из ТаблицаОплат Цикл
		...
		Если СуммаОсталосьПогасить > 0 Тогда
			...
			ДвижениеВзиморасчетовУСН(ТаблицаДвиженияВзаиморасчетов, СтруктураШапкиДокумента.Организация, СтрокаПлатеж.ДоговорКонтрагента, СтруктураШапкиДокумента.Ссылка, СтрокаПлатеж.Сделка, СуммаОсталосьПогасить, 0, 0, СтрокаПлатеж.Работник);
		КонецЕсли;
	КонецЦикла;
...
//
Процедура ПереносЗадолженностиУСН(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, ВидДвижения)
	...
	Для Каждого ЭлементМассива Из МассивСделок Цикл
		...
		Если ТипЗнч(ЭлементМассива) <> ТипЗнч(Неопределено) Тогда
		     МассивСделокВЗапрос.Добавить(ЭлементМассива);
		КонецЕсли;
	...
	Для Каждого СтрокаПлатеж Из ТаблицаОплат Цикл
		...
		Если ЗначениеЗаполнено(СтрокаПлатеж.Сделка) Тогда
			СтруктураОтбора.Вставить("РасчетныйДокумент",СтрокаПлатеж.Сделка);
		КонецЕсли;	
		...
		Если  СуммаОсталосьПогасить > 0 Тогда
			ДвижениеВзиморасчетовУСН(ТаблицаДвиженияВзаиморасчетов, СтруктураШапкиДокумента.Организация, СтрокаПлатеж.ДоговорКонтрагента, СтруктураШапкиДокумента.Ссылка, СтрокаПлатеж.Сделка, СуммаОсталосьПогасить, 0, 0);
		КонецЕсли;
	КонецЦикла;
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаВзаиморасчетов,ТаблицаДвиженияВзаиморасчетов);
...
Процедура СписаниеЗадолженностиУСН(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, КУДиР, ВидДвижения)
	...
	Для Каждого СтрокаПлатеж Из ТаблицаОплат Цикл
		...
		Для Каждого Строка Из НайденныеСтроки Цикл
			...
			Если  ВидДвижения = ВидДвиженияНакопления.Приход Тогда
				КУДиР.Графа4 = КУДиР.Графа4 + КЗ*(НоваяСтрока.СуммаВзаиморасчетов)*Курс/Кратность;
			КонецЕсли;
		...
		Если  СуммаОсталосьПогасить > 0 Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Сумма по договору " + СтрокаПлатеж.ДоговорКонтрагента + " превышает остаток по взаиморасчетам");
		КонецЕсли;
	КонецЦикла;
...
Процедура НачислениеЗП(СтруктураШапкиДокумента, ДвиженияРегистров, КУДиР) //
	...
	|	ОтражениеЗарплатыВРеглУчетеОтражениеВУчете.РасходыУСН КАК ОтражениеВУСН
	|ИЗ
	...
	|	БУОсновныеНачисления.РасходыУСН
	|ИЗ
	...
	|	ОтражениеЗарплатыВРеглУчетеОтражениеВУчете.РасходыУСН
	|ИЗ
	|	Документ.ОтражениеЗарплатыВРеглУчете.ОтражениеВУчете КАК ОтражениеЗарплатыВРеглУчетеОтражениеВУчете
	|ГДЕ
	|	ОтражениеЗарплатыВРеглУчетеОтражениеВУчете.Ссылка = &Ссылка
	...
	Для Каждого Расход Из ТаблицаРасходов Цикл
		...
		Если СуммаОсталосьПогасить > 0 Тогда
			Если Расход.ВидРасхода = Перечисления.ВидыРасходовУСН.Зарплата Тогда
				ДвижениеВзиморасчетовУСН(ТаблицаДвиженияВзаиморасчетов, СтруктураШапкиДокумента.Организация, Неопределено, СтруктураШапкиДокумента.Ссылка, Неопределено, СуммаОсталосьПогасить, , ,Расход.ЭлементРасхода);
			Иначе
				ДвижениеВзиморасчетовУСН(ТаблицаДвиженияВзаиморасчетов, СтруктураШапкиДокумента.Организация, Расход.СчетУчета, СтруктураШапкиДокумента.Ссылка, Неопределено, СуммаОсталосьПогасить);
			КонецЕсли;
		КонецЕсли;
...
Процедура ВозвратПоДокументуУСН(СтруктураШапкиДокумента, ТаблицаПоТоварам, ДвиженияРегистров, КУДиР)
	...
	УчетнаяПолитикаРализация = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(СтруктураШапкиДокумента.Сделка.Дата, СтруктураШапкиДокумента.Организация, Ложь);
...
Процедура ОтразитьВУСН(ДокументСсылка, ВидОперации = Неопределено, Параметры = Неопределено, ЭтапПроведения = 0) Экспорт //
	...
	УчетнаяПолитика = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(ДатаОплаты, ДокументСсылка.Организация);
	...
	Если (ВидОперации = "Поступление")
		...
		Иначе
			...
			Если УчетАгентскогоНДС Тогда
				...
				ЗачетАвансаПоставщикуУСН(СтруктураШапкиДокумента, ТаблицаПриходаНДС, ДвиженияРегистров, ПланыСчетов.Хозрасчетный.НДС, Неопределено, ПустаяТаблицаРД, СуммаВзаиморасчетовНДС, (ЭтапПроведения > 0));
		    КонецЕсли;
		КонецЕсли;												   
	...
	ИначеЕсли (ВидОперации = "ВозвратОтПокупателя") Тогда
		...
		Если ДвиженияРегистров = Неопределено Тогда
			...
			ТаблицаПризнанныхРасходов = РегистрыНакопления.КнигаУчетаДоходовИРасходов.СоздатьНаборЗаписей().Выгрузить();
			ТаблицаПризнанныхРасходов.Очистить();
		...
		Если (СуммаЕНВД <> 0) ИЛИ (СуммаКомиссии <> 0) ИЛИ (СуммаВзаиморасчетов <> 0) Тогда
			...
			ДвижениеВзиморасчетовУСН(ДвиженияРегистров.ВзаиморасчетыРасход, СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.ДоговорКонтрагента, СтруктураШапкиДокумента.Ссылка, СтруктураШапкиДокумента.Ссылка, - СуммаВзаиморасчетов, 0, 0);
		КонецЕсли;															
	...
	ИначеЕсли (ВидОперации = "РеализацияУслуг") Тогда
		...
		Если НЕ СуммаУслугПоДокументу = 0 Тогда
			...
			ЗачетАвансаОтПокупателяБезУчетаРасходовУСН(СтруктураШапкиДокумента, ДвиженияРегистров, Договор, Сделка, ТаблицаРД, СуммаВзаиморасчетов, СуммаЕНВД, СуммаКомиссии, КУДиР, ЗачитыватьАванс, УдержатьВознаграждение);
		Конецесли;
	ИначеЕсли (ВидОперации = "Оплата") Тогда	
		Если НЕ МетаданныеДокумента.ТабличныеЧасти.Найти("РасшифровкаПлатежа") = Неопределено Тогда
			Запрос = Новый Запрос;
		...
		Если НЕ МетаданныеДокумента.ТабличныеЧасти.Найти("ОплатаПоставщикам") = Неопределено Тогда
			...
			ТаблицаОплат = Запрос.Выполнить().Выгрузить();
			Если ТаблицаОплат.Количество() > 0 Тогда
				ОплатаПоставщикуУСН(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, КУДиР, "ОплатаАО");
				//КУДиР.Содержание = "Оплата поставщику. ";
			Конецесли;
		КонецЕсли;
	...
	ИначеЕсли (ВидОперации = "ВозвратОтПодотчетника") Тогда	
		...
		НоваяСтрока = ТаблицаОплат.Добавить();
		НоваяСтрока.ДоговорКонтрагента = ДокументСсылка.Контрагент;
		НоваяСтрока.Сделка = ?(НЕ ЗначениеЗаполнено(ДокументСсылка.РасчетныйДокумент), Неопределено, ДокументСсылка.РасчетныйДокумент); //~~
		НоваяСтрока.Сумма = ДокументСсылка.СуммаДокумента;
		НоваяСтрока.КурсВзаиморасчетов =  ЗаполнениеДокументов.КурсДокумента(ДокументСсылка, глЗначениеПеременной("ВалютаРегламентированногоУчета"));
		НоваяСтрока.КратностьВзаиморасчетов =  ЗаполнениеДокументов.КратностьДокумента(ДокументСсылка, глЗначениеПеременной("ВалютаРегламентированногоУчета"));
	...
	ИначеЕсли (ВидОперации = "ПереносЗадолженности") Тогда	
		...
		Если НЕ МетаданныеДокумента.ТабличныеЧасти.Найти("СуммыДолга") = Неопределено Тогда
			...
			ТаблицаОплат = Запрос.Выполнить().Выгрузить();
			Если ТаблицаОплат.Количество() > 0 Тогда
				ПереносЗадолженностиУСН(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, ВидДвиженияНакопления.Приход);
			Конецесли;
	...
	Если НЕ ДвиженияРегистров = Неопределено Тогда
		Если ДвиженияРегистров.ВзаиморасчетыРасход.Количество() > 0 Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ДвиженияРегистров.ВзаиморасчетыРасход, ТаблицаДвижений);
			// Недостающие поля.
		...
		Если ДвиженияРегистров.РасходыУСН.Количество() > 0 Тогда
			...
			ТаблицаДвижений.Очистить();
			// Заполним таблицу движений.
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ДвиженияРегистров.РасходыУСН, ТаблицаДвижений);
			// Недостающие поля.
			ТаблицаДвижений.ЗаполнитьЗначения(ДокументСсылка.Организация, "Организация");
			ТаблицаДвижений.ЗаполнитьЗначения(МоментДокумента, "Период");
			ТаблицаДвижений.ЗаполнитьЗначения(ДокументСсылка, "Регистратор");
...
Процедура ДвиженияУСН(ДокументСсылка, РежимПроведения = Неопределено) Экспорт //
	...
	Если РежимПроведения = Неопределено Тогда
		...
		Если (НЕ ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах")) И
			...
			(НЕ ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПоступлениеТоваровУслугВНТТ")) Тогда
	...
	ИначеЕсли 
		...
		ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РасходныйКассовыйОрдер") Тогда
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПоступлениеТоваровУслугВНТТ") Тогда
		...
		Если ДокументСсылка.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслугВНТТ.ОтПоставщика Тогда
