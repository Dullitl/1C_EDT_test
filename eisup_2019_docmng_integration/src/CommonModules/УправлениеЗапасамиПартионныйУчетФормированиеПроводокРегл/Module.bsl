...
Процедура НачислитьДоходОтРеализации(СтрокаДокумента, СтруктураПараметров, Движение, ТипЗаписи = "Прямая")
	...
	Если Учет="Бух" Тогда
		...
		Для каждого ВидСубконто Из Проводка.СчетКт.ВидыСубконто Цикл
			...
			КонецЕсли;
			//АБС+
			Если ВидСубконто.ВидСубконто.ТипЗначения.СодержитТип(ТипЗнч(СтрокаДокумента.абс_СубконтоБУ2)) Тогда
				Проводка.СубконтоКт.Вставить(ВидСубконто.ВидСубконто, СтрокаДокумента.абс_СубконтоБУ2);
			КонецЕсли;
			//АБС-
		КонецЦикла;
	...
	ИначеЕсли Учет="Нал" Тогда
		...
		Если НЕ ОтноситсяКДеятельностиЕНВДИзСтруктурыПараметров(СтруктураПараметров, СтрокаДокумента.СчетДоходовБУ) Тогда
			...
			Если ТипЗнч(СтруктураПараметров.Регистратор.Ссылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") 
			   И СтруктураПараметров.Регистратор.СчетУчетаРасчетовСКонтрагентом.Родитель = ПланыСчетов.Хозрасчетный.РасчетыСПерсоналомПоПрочимОперациям Тогда //73й
				Проводка.СчетДт =  БухгалтерскийУчет.ПреобразоватьСчетаБУвСчетНУ(Новый Структура("СчетБУ",  СтруктураПараметров.Регистратор.СчетУчетаРасчетовСКонтрагентом), "НУ",  СтруктураПараметров.Регистратор.Дата);
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,1,СтруктураПараметров.Регистратор.абс_ФизЛицо);
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,2,СтруктураПараметров.Регистратор.абс_ВидыУдержаний);			
			Иначе
			//--- T#7723272 2017.05.11 Гущина. Как было:
	...
	Иначе
		Если Учет="Нал" Тогда
			//дополним еще НУ на разницу, т.к. выше проводка без НДС, а про учет разниц в СД речи нет
			//пока так
			Если ТипЗнч(СтруктураПараметров.Регистратор.Ссылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") 
				И СтруктураПараметров.Регистратор.СчетУчетаРасчетовСКонтрагентом.Родитель = ПланыСчетов.Хозрасчетный.РасчетыСПерсоналомПоПрочимОперациям 
				И Движение.СуммаНДСВыручкиБУ > 0 тогда
					Проводка = СтруктураПараметров["Движения"+ИмяРегистра].Добавить();
					Проводка.Период        = СтруктураПараметров.Период;
					Проводка.Регистратор   = СтруктураПараметров.Регистратор;
					Проводка.Активность    = Истина;
					...
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,1,СтруктураПараметров.Регистратор.абс_ФизЛицо);
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,2,СтруктураПараметров.Регистратор.абс_ВидыУдержаний);			
					Проводка.Сумма = Движение.СуммаНДСВыручкиБУ;
			КонецЕсли;
		КонецЕсли;
		//--- T#7723272 2017.05.11 Гущина
	КонецЕсли;
