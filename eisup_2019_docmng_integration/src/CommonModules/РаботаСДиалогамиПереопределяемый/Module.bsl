...
Процедура УстановитьВидимостьЭУБухучета(СтруктураЭУ, ВидимостьНУСчета = Ложь , Организация, Дата) Экспорт
	...
	ПоказыватьСчетаНУ = Истина; //ПрограммаБухучета = Перечисления.ПрограммыБухгалтерскогоУчета.БухгалтерияПредприятия16
		 //или ПрограммаБухучета = Перечисления.ПрограммыБухгалтерскогоУчета.Бухгалтерия77;
	ВестиНУ = Истина; //ПрограммаБухучета <> Перечисления.ПрограммыБухгалтерскогоУчета.ДругаяПрограммаБухучета;	 
	...
	ВидимостьУчетПоПодразделениям = Ложь; //ПрограммаБухучета = Перечисления.ПрограммыБухгалтерскогоУчета.БухгалтерияПредприятияКорп
...
Функция ПолучитьСведенияОВидеРасчетаСхемыМотивации(СведенияОВидахРасчета, ВидРасчета, Режим = "ПлановыеНачисления") Экспорт
	...
	Если СведенияОВидеРасчета = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ВидРасчета) Тогда
			...
			|СГРУППИРОВАТЬ ПО
			|	Показатели.Показатель,
			|	ВЫБОР КОГДА Показатели.Показатель.Валюта = ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) ИЛИ Показатели.Показатель.Валюта = Константы.ВалютаУправленческогоУчета ТОГДА ЛОЖЬ ИНАЧЕ ИСТИНА КОНЕЦ,
			|	ВЫБОР
			|		КОГДА  НЕ Показатели.ЗапрашиватьПриКадровыхПеремещениях
			|			ТОГДА ИСТИНА";
			Если ЭтоОсновноеНачислениеОрганизаций ИЛИ ЭтоУправленческоеНачисление Тогда
