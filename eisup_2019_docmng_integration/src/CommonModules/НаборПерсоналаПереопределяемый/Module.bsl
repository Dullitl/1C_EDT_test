...
Функция ПолучитьСписокЗаявок(ПолеДляУпорядочивания, ДоступныеУчетныеЗаписиДляЧтения) Экспорт
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("ТекущаяДата",	НачалоДня(ОбщегоНазначенияЗК.ПолучитьРабочуюДату()));
...
Процедура ДополнитьМакет(ТекстМакетаОписанияЗаявки, ДанныеСтроки) Экспорт
...
Функция ЯвляетсяОбъектомДополнительногоТипа(Объект) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.Встречи") Тогда
		Возврат	Истина;
	Иначе 
		Возврат	Ложь;
	КонецЕсли;
...
Процедура УстановитьДоступностьДополнительныхКнопок(КнопкиПодменю, КомандыЗаявкиДоступны, НеДоступнаГрупповаяОбработка) Экспорт
...
Процедура ВыполнитьДополнительныеДействия(Кнопка, ЭлементыФормы, ЭтаФорма, ЭтотОбъект) Экспорт
...
Процедура ДополнитьДокHTML(ДокHTML, ДанныеСтроки) Экспорт
