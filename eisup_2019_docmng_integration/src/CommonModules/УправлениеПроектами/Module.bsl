...
Процедура РаспределитьПродажиПоПроектам(ТабДвижений, Строка, ВидРаспределения, Знач ДатаКонМесяца)
	...
	МассивСтоимость 		 = ttk_ОбщегоНазначения.РаспределитьПропорционально(Стоимость, МассивКоэф,,Ложь);
	МассивСтоимостьБезСкидок = ttk_ОбщегоНазначения.РаспределитьПропорционально(СтоимостьБезСкидок, МассивКоэф,,Ложь);
	МассивКоличество		 = ttk_ОбщегоНазначения.РаспределитьПропорционально(Количество, МассивКоэф,,Ложь);	
	
	ЕстьДвиженияПоНДС=НЕ ТабДвижений.Колонки.Найти("НДС")=Неопределено; 

	Если ЕстьДвиженияПоНДС Тогда
		НДС		 			= Строка.НДС;
		МассивНДС			= ttk_ОбщегоНазначения.РаспределитьПропорционально(НДС, МассивКоэф,,Ложь);
	КонецЕсли;
...
Процедура РаспределитьДвиженияДСПоПроектам(ТабДвижений, Строка, ВидРаспределения, Знач ДатаКонМесяца)
	...
	ТабЗапроса = РезультатЗапроса.Выгрузить();
	МассивКоэф 		 = ТабЗапроса.ВыгрузитьКолонку("ДоляРаспределения");
	МассивСумма  = ttk_ОбщегоНазначения.РаспределитьПропорционально(Сумма, МассивКоэф,,Ложь);
	МассивСуммаУпр= ttk_ОбщегоНазначения.РаспределитьПропорционально(СуммаУпр, МассивКоэф,,Ложь);
...
Процедура ОтразитьДвиженияПоПроектам(ИсходнаяТаблица,ТаблицаДвижений,Проект,Дата,ВидДвижения="Продажи",ДокументРасчетов=Неопределено) Экспорт
	...
	Если ВидДвижения="ДенежныеСредстваПоступление" Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПроекты, ТаблицаДвижений);
	...
	Иначе
		...
		Иначе	
			...
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ИсходнаяТаблица, ТаблицаДвижений);
...
Процедура НачалоВыбораПроекта( ФормаДокумента, ЭлементФормы, СтандартнаяОбработка, СтруктураОтбора = Неопределено, ДопПараметры = Неопределено) Экспорт
	...
	//АБС ИЗМЕНЕНИЕ Глебов
	Если ТипЗнч(ФормаВыбора) = Тип("УправляемаяФорма") И ЗначениеЗаполнено(ЭлементФормы.Значение) Тогда
		ФормаВыбора = ПолучитьФорму("Справочник."+ВыбранныйТип+".Форма.ФормаВыбораУправляемая", Новый Структура("ТекущаяСтрока", ЭлементФормы.Значение), ЭлементФормы);              
	КонецЕсли;
	//\\АБС Глебов
...
Процедура РаспределитьЗатратыПоПроектам( ТабДвижений, Строка, Знач Сумма, ВидРаспределения, Знач ДатаКонМесяца) Экспорт
	...
	ТабЗапроса = РезультатЗапроса.Выгрузить();
	МассивКоэф = ТабЗапроса.ВыгрузитьКолонку("ДоляРаспределения");
	МассивСумм = ttk_ОбщегоНазначения.РаспределитьПропорционально( Сумма, МассивКоэф);
