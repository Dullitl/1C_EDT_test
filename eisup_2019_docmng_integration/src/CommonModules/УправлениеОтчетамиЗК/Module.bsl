	...
	ИначеЕсли ТекстЗапроса <> Неопределено тогда
		Если УдалятьПредыдущийЗапрос тогда
			...
			Иначе
				...
				Подзапросы = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ТекстЗапроса, ";");
				Для Каждого Подзапрос Из Подзапросы Цикл
					ПозицияВТ = Найти(Подзапрос, "ПОМЕСТИТЬ РаботникиПредприятия");
					Если ПозицияВТ > 0 Тогда
						ТекстЗапроса = СтрЗаменить(ТекстЗапроса, Подзапрос, РаботникиПредприятия);
						Прервать
					КонецЕсли	
				КонецЦикла	
			КонецЕсли;
...
Процедура СформироватьДиаграмму(ДиаграммаГанта, ПараметрыОтчета) Экспорт
	...
	ПараметрыОтчета.ПостроительОтчета.Параметры.Вставить("ТекущаяДата", КонецДня(ОбщегоНазначенияЗК.ПолучитьРабочуюДату()));
