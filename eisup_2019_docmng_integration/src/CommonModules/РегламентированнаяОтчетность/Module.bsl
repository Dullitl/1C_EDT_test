...
Функция ИДКонфигурации() Экспорт
	...
	ИначеЕсли МетаданныеИмя = "ЗарплатаИУправлениеПерсоналом" ИЛИ МетаданныеИмя = "ЗарплатаИУправлениеПерсоналомБазовая" ИЛИ МетаданныеИмя = "ЗарплатаИУправлениеПерсоналомКОРП" Тогда
		Возврат "ЗУП";
	ИначеЕсли МетаданныеИмя = "УправлениеПроизводственнымПредприятием" ИЛИ
		//АБС ИЗМЕНЕНИЕ СМ ОБНОВЛЕНИЕ ОТЧЕТНОСТИ
		МетаданныеИмя ="УправлениеТелекоммуникационнойКомпаниейКТТК"  Тогда
		//\\//АБС ИЗМЕНЕНИЕ СМ ОБНОВЛЕНИЕ ОТЧЕТНОСТИ
...
// Возвращает код по ОКАТО или ОКТМО организации.
// 
// Параметры:
//  ИсточникСведений - произвольный обладающий реквизитами - источник сведений для определения
//    кода по ОКАТО/ОКТМО
//  ДатаПериодаОтчета - Дата - любая дата из периода отчета.
//
Функция КодТерриториальногоОбразования(ИсточникСведений, ДатаПериодаОтчета, ДатаПодписиОтчета) Экспорт
	
	Результат = "";
	
	НачатоИспользованиеОКТМО = (ДатаПодписиОтчета >= '2014-01-01');
	ГодПериодаОтчета = Год(ДатаПериодаОтчета);
	
	ЕстьРеквизитОКТМО = (ИсточникСведений.Метаданные().Реквизиты.Найти("КодПоОКТМО") <> Неопределено);
	
	Если НачатоИспользованиеОКТМО И ГодПериодаОтчета >= 2013 Тогда
		Результат = ОКТМО(ИсточникСведений);
	Иначе
		Результат = ОКАТО(ИсточникСведений);
	КонецЕсли;
...
Процедура СформироватьСоставКнопкиОчиститьДляРегОтчетов(Форма) Экспорт
	...
	//
	////АБС+
	//Если ИмяСтраницы="Заполнение" Тогда
...
Процедура ПередОткрытиемОсновнойФормыРегламентиованногоОтчета(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Форма.ВладелецФормы <> Неопределено Тогда
		Если Не Форма.ВладелецФормы.ЭтоНовый() Тогда
			...
			ВыбФормаОтчета.ТолькоПросмотр = ТолькоПросмотр;
			ВыбФормаОтчета.Открыть();
...
Функция КодыДляИмениФайлаВФорматеВыгрузки(Параметры) Экспорт
	...
	Если ФорматВыгрузкиСтатистическойОтчетностиВЭлектронномВиде(Параметры["Документ"]["Организация"]) = 2 Тогда // "новый" формат выгрузки
		...
		ТекМес  = Формат(Месяц(ТекДата), "ЧЦ=2");
		ТекДен  = Формат(День(ТекДата),  "ЧЦ=2");
...
Процедура ПриНажатииНаКнопкуОтправкиВУведомленииОКонтролируемыхСделках(Форма) Экспорт
...
Функция ОбновитьПанельСостоянияОтправкиВУведомленииОКонтролируемыхСделках(Форма) Экспорт
	...
	//	ИЛИ НЕ ЗначениеЗаполнено(УчетнаяЗаписьДокументооборота) ИЛИ НЕ УчетнаяЗаписьПредназначенаДляДокументооборотаСФНС(УчетнаяЗаписьДокументооборота) <> Истина Тогда
...
//АБС ВСТАВКА №40966 НАЧАЛО «13 мая 2014 г.», Пополитов
//Не изменённые процедуры релиза 1.3.52.1
...
//\\АБС ВСТАВКА №40966 КОНЕЦ  	

//АБС ВСТАВКА 48889  28.10.2014 14:01:35  Шамов
//Перенесено из 1.3.58.2
#Если ТолстыйКлиентОбычноеПриложение Тогда
...
Функция СписокДоступныхЮридическихЛиц(Знач Текст = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		Иначе
			...
			Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
			               ...
			               |	И Организации.Наименование ПОДОБНО &Текст СПЕЦСИМВОЛ ""~""";
		КонецЕсли;
...
Функция СписокДоступныхИндивидуальныхПредпринимателей(Знач Текст = Неопределено) Экспорт
...
Процедура ПреобразоватьБланкДляExcel(ОбрабатываемыйТабличныйДокумент) Экспорт
	
	Для Каждого Область Из ОбрабатываемыйТабличныйДокумент.Области Цикл
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник Тогда
			ОбластьСодержитЗначение = (Область.СодержитЗначение = Истина); // возможно третье состояние
			Если ОбластьСодержитЗначение Тогда
				Если ТипЗнч(Область.Значение) <> Тип("Строка") Тогда
					СтроковоеПредставлениеЗначения = Формат(Область.Значение, Область.Формат);
					Область.ТипЗначения = ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(0);
					Область.Значение = СтроковоеПредставлениеЗначения;
				КонецЕсли;
				
			Иначе
				// Обработка полей машиночитаемых бланков.
				ТекстОбласти = Область.Текст;
				Если СтрДлина(ТекстОбласти) = 1 И ОбщегоНазначения.ТолькоЦифрыВСтроке(ТекстОбласти) Тогда
					Область.СодержитЗначение = Истина;
					Область.ТипЗначения = ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(0);
					Область.Значение = ТекстОбласти;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
//АБС ВСТАВКА 48889 КОНЕЦ

// Start КТТК Ермолов Е.Л.  27.07.2015 Перенесено из 1.3.67.1
Функция НоваяТаблицаНавигацииПоОшибкам() Экспорт
	
	ТаблицаНавигацииПоОшибкам = Новый ТаблицаЗначений;
	
	ТаблицаНавигацииПоОшибкам.Колонки.Добавить("Раздел");
	ТаблицаНавигацииПоОшибкам.Колонки.Добавить("Графа");
	ТаблицаНавигацииПоОшибкам.Колонки.Добавить("Строка");
	ТаблицаНавигацииПоОшибкам.Колонки.Добавить("Страница");
	ТаблицаНавигацииПоОшибкам.Колонки.Добавить("СтрокаПП");
	ТаблицаНавигацииПоОшибкам.Колонки.Добавить("ИмяЯчейки");
	ТаблицаНавигацииПоОшибкам.Колонки.Добавить("Описание");
	
	Возврат ТаблицаНавигацииПоОшибкам;
...
Функция НовоеДеревоФормИФорматов() Экспорт
	
	ДеревоФормИФорматов = Новый ДеревоЗначений;
	
	ДеревоФормИФорматов.Колонки.Добавить("Код");
	ДеревоФормИФорматов.Колонки.Добавить("ДатаПриказа");
	ДеревоФормИФорматов.Колонки.Добавить("НомерПриказа");
	ДеревоФормИФорматов.Колонки.Добавить("ДатаНачалаДействия");
	ДеревоФормИФорматов.Колонки.Добавить("ДатаОкончанияДействия");
	ДеревоФормИФорматов.Колонки.Добавить("ИмяОбъекта");
	ДеревоФормИФорматов.Колонки.Добавить("Описание");
	
	Возврат ДеревоФормИФорматов;
	
КонецФункции

// Stop КТТК Ермолов Е.Л.  27.07.2015

// +++ввв 10.02.2017 г.
Функция ЦветаРаскраскиПоказателей() Экспорт
	
	ЦветаРаскраски = Новый Структура;
	ЦветаРаскраски.Вставить("_0", Новый Цвет(255, 255, 192)); // ввод вручную (желтый)
	ЦветаРаскраски.Вставить("_1", Новый Цвет(255, 255, 225)); // ввод вручную без автозаполнения (бледно-желтый)
	ЦветаРаскраски.Вставить("_2", Новый Цвет(255, 240, 200)); // автозаполнение с корректировкой (желто-коричневый)
	ЦветаРаскраски.Вставить("_3", Новый Цвет(230, 240, 220)); // автозаполнение (светло-зеленый)
	ЦветаРаскраски.Вставить("_4", Новый Цвет(192, 220, 192)); // авторасчет (зеленый)
	ЦветаРаскраски.Вставить("_5", Новый Цвет(255, 255, 255)); // недоступно для ввода (белый)
