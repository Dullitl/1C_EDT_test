...
Функция ПолучитьЗначениеПоУмолчанию(Пользователь, Настройка) Экспорт
	...
	Если Пользователь = глЗначениеПеременной("глТекущийПользователь") Тогда	
		...
		Если Настройка = "ПоказыватьВДокументахСчетаУчета" 
			//или Настройка = "ОтражатьДокументыВБухгалтерскомУчете" 
			//или Настройка = "ОтражатьДокументыВНалоговомУчете" 
																	Тогда
			мРолиПользователя = абс_БизнесПроцессы.ПолучитьСписокДоступныхРолейПользователя(Пользователь);
			Если не мРолиПользователя.Найти(Справочники.РолиИсполнителей.ЗаполнениеСубконтоВПодготовке) = Неопределено Тогда
				Возврат Истина;	
			КонецЕсли;
		КонецЕсли;
