...
Процедура ПометитьНаУдалениеДополнительнуюИнформациюОбъекта(ОбъектВладелец, Отказ, ШапкаОшибки) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ttk_ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(), Отказ, ШапкаОшибки);
			Возврат;
		КонецПопытки;
	КонецЦикла;
...
Функция ВыполнитьОбработкуПодключаемуюКОтчету(СтрокаКнопки, Отчет, ТабличныйДокумент) Экспорт
	...
	Если ТипЗНЧ(СсылкаНаВнешнююОбработку) = Тип("СправочникСсылка.ВнешниеОбработки") Тогда
		...
		Исключение
			ttk_ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, "Обработка не выполнена!");
		КонецПопытки;
	КонецЕсли;
...
Процедура ОбработатьНажатиеНаДополнительнуюКнопкуЗаполненияТЧ(СтрокаКнопки,ЭтотОбъект) Экспорт
	...
	Если ТипЗНЧ(СсылкаНаВнешнююОбработку) = Тип("СправочникСсылка.ВнешниеОбработки") Тогда
		...
		Исключение
			ttk_ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, "Заполнение ТЧ не произведено!");
		КонецПопытки;
	КонецЕсли;
...
Процедура ОткрытьФормуВыбораПечатныхФормОбъекта(Объект, ФормаВладелец) Экспорт
	...
	Для каждого СтрокаКнопки Из Форма.ЭлементыФормы.ДеревоМакетовПечати.ВыделенныеСтроки Цикл
		...
		Если ТипЗнч(Расшифровка) = Тип("Структура") и Расшифровка.ВидПечатнойФормы = Перечисления.ВидыДополнительныхВнешнихОбработок.ПечатнаяФорма Тогда
			...
			НапечататьДокумент(ТабДокумент, 0, Форма.НаПринтер, ?(ЭтоДокумент, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(Объект), Строка(Объект)), Объект.Ссылка);
		Иначе
...
Процедура ПечатьПоДополнительнойКнопке(УдалитьДеревоМакетов, Объект, Форма = Неопределено, ТекстКнопки) Экспорт
	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") Тогда
		...
		ТабДокумент = НапечататьВнешнююФорму(Объект.Ссылка, Расшифровка);
		НапечататьДокумент(ТабДокумент, , , ?(ЭтоДокумент, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(Объект), Строка(Объект)));
...
Функция НапечататьВнешнююФорму(Ссылка, ИсточникМакета) Экспорт
	...
	Исключение
		ttk_ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, "Не удалось сформировать внешнюю печатную форму!");
	КонецПопытки;
...
Функция НапечататьДокументПоУмолчанию(Объект, КоличествоЭкземпляров = 0, НаПринтер = Ложь) Экспорт
	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") и Расшифровка.ВидПечатнойФормы = Перечисления.ВидыДополнительныхВнешнихОбработок.ПечатнаяФорма Тогда
		...
		ТабДокумент = НапечататьВнешнююФорму(Объект.Ссылка, Расшифровка);
		ЭтоДокумент = Метаданные.Документы.Содержит(Объект.Метаданные());
		НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ?(ЭтоДокумент, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(Объект.Ссылка), Строка(Объект)), Объект.Ссылка);
	Иначе
...
// АБС ИЗМЕНЕНИЕ 49194
// Изменения нужны для обработки групповой печати пакета документов и отправки их по электронной почте
//Функция НапечататьДокумент(ПечДокумент, КоличествоЭкземпляров = 1, НаПринтер = Ложь, Заголовок = "", Ссылка = Неопределено) Экспорт

Функция НапечататьДокумент(ПечДокумент, КоличествоЭкземпляров = 1, НаПринтер = Ложь, Заголовок = "", Ссылка = Неопределено, абс_ПоказатьДиалогПечати = Ложь) Экспорт
//\\ АБС ИЗМЕНЕНИЕ
	...
	Если ТипЗнч(НаПринтер) = Тип("Булево") Тогда
		Если НаПринтер Тогда
			...
			// АБС ИЗМЕНЕНИЕ 49194
			// нужны для обработки групповой печати пакета документов и отправки по электронной почте
			// "абс_ГрупповаяПечатьПакетаДокументов"
			//ПечДокумент.Напечатать();
			Если абс_ПоказатьДиалогПечати Тогда
				ПечДокумент.Напечатать(РежимИспользованияДиалогаПечати.Использовать);
			Иначе
				ПечДокумент.Напечатать(РежимИспользованияДиалогаПечати.НеИспользовать);
			КонецЕсли;			
			//\\ АБС ИЗМЕНЕНИЯ
		Иначе
...
Функция ПросклонятьФамилиюИмяОтчество(Компонента, Знач Фамилия = "", Знач Имя = "", Знач Отчество = "", Падеж = 1, Пол = Неопределено, Результат = "", СообщениеОбОшибке = "") Экспорт
	...
	Попытка
		...
		Если НужноИсправитьФамилию ИЛИ НужноИсправитьИмя ИЛИ НужноИсправитьОтчество Тогда
			...
			МассивФИО = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Результат, " ");
			...
			Если Не ПустаяСтрока(Фамилия) Тогда
				...
				Если НужноИсправитьФамилию Тогда
					...
					Если СтрЧислоВхождений(Фамилия, "-") > 0 Тогда
						...
						МассивФамилия = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ФамилияНовая, "-");
			...
			Если Не ПустаяСтрока(Имя) Тогда
				...
				Если НужноИсправитьИмя Тогда
					...
					Если СтрЧислоВхождений(Имя, "-") > 0 Тогда
						...
						МассивИмя = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ИмяНовое, "-");
			...
			Если Не ПустаяСтрока(Отчество) Тогда
				...
				Если НужноИсправитьОтчество Тогда
					...
					Если СтрЧислоВхождений(Отчество, "-") > 0 Тогда
						...
						МассивОтчество = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ОтчествоНовое, "-");
...
Процедура абс_ОформленияСтрок(Элемент, ОформленияСтрок, ИмяКолонки, ИмяРеквизита, ПутьДанных, жФормат = "ЧДЦ=2") Экспорт
	    		
	Видимость = Элемент.Колонки[ИмяКолонки].Видимость;
	
	Если Видимость Тогда
		
		МассивДокументов = Новый Массив;
		Для каждого ОформленияСтроки Из ОформленияСтрок Цикл
			МассивДокументов.Добавить(ОформленияСтроки.ДанныеСтроки.Ссылка);	
		КонецЦикла;	
		
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("МассивДокументов",МассивДокументов);
		Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		               |	ДанныеЗапроса.Ссылка,
		               |	СУММА(ДанныеЗапроса."+ИмяРеквизита+") КАК "+ИмяРеквизита+"
		               |ИЗ
		               |	"+ПутьДанных+" КАК ДанныеЗапроса
		               |ГДЕ
		               |	ДанныеЗапроса.Ссылка В(&МассивДокументов)
		               |
		               |СГРУППИРОВАТЬ ПО
		               |	ДанныеЗапроса.Ссылка";
		...
		Пока Результат.Следующий() Цикл
			Соответствие.Вставить(Результат.Ссылка,Формат(Результат[ИмяРеквизита],жФормат));	
		КонецЦикла;	
		
		Для каждого ОформленияСтроки Из ОформленияСтрок Цикл
			ТекОбрабатываемыДокумент = ОформленияСтроки.ДанныеСтроки.Ссылка;
			Если не Соответствие.Получить(ТекОбрабатываемыДокумент) = Неопределено Тогда
				ОформленияСтроки.Ячейки[ИмяКолонки].УстановитьТекст(Соответствие[ТекОбрабатываемыДокумент]);	
			КонецЕсли;
		КонецЦикла;	
...
Процедура абс_ОформленияСтрокБезСуммирования(Элемент, ОформленияСтрок, ИмяКолонки, ПутьДанных, ИмяРеквизитаОтбора, ИмяРеквизита, Организация = Неопределено) Экспорт
	...
	Если Видимость Тогда
		...
		Для каждого ОформленияСтроки Из ОформленияСтрок Цикл
			МассивЭлементов.Добавить(ОформленияСтроки.ДанныеСтроки.Ссылка);	
		КонецЦикла;	
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ Различные
		               |	ДанныеЗапроса."+ИмяРеквизитаОтбора+",
		               |	ДанныеЗапроса."+ИмяРеквизита+" КАК "+ИмяРеквизита+"
		               |ИЗ
		               |	"+ПутьДанных+" КАК ДанныеЗапроса
		               |ГДЕ
		               |	ДанныеЗапроса."+ИмяРеквизитаОтбора+" В(&МассивЭлементов)"; 	
		...
		Соответствие = Новый Соответствие;
		Пока Результат.Следующий() Цикл
			Соответствие.Вставить(Результат[ИмяРеквизитаОтбора],Результат[ИмяРеквизита]);	
		КонецЦикла;	
