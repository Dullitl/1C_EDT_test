...
Функция ПроверкаНаДублированиеИдентификатора(Отказ, ЭтотОбъект)
	...
	Если Отказ Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Показатель с таким идентификатором уже существует!", Отказ);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеПоказателя(Отказ, ЭтаФорма)
	...
	Если НЕ ЗначениеЗаполнено(ЭтаФорма.Наименование) Тогда
		Отказ = Истина;
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Необходимо задать наименование показателя!", Отказ);
	КонецЕсли;
		
	Если НЕ ЗначениеЗаполнено(Идентификатор) Тогда
		Отказ = Истина;
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Необходимо задать идентификатор показателя!", Отказ);
	Иначе
		Разделители	= " .,+,-,/,*,?,=,<,>,(,)%!@#$%&*""№:;{}[]?()\|/`~'^_";
		Для НомСимвола = 1 По СтрДлина(Идентификатор) Цикл
			...
			Если НомерСимвола <> 0 Тогда
				Отказ = Истина;
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Идентификатор показателя не должен содержать символ: """+Сред(Разделители,НомерСимвола,1) + """", Отказ);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Если Не Предопределенный Тогда
		...
		Если НЕ ЗначениеЗаполнено(ТипПоказателя) Тогда
			Отказ = Истина;
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Необходимо задать тип показателя!", Отказ);
		КонецЕсли;
		...
		Если ТипПоказателя <> Перечисления.ТипыПоказателейСхемМотивации.Стаж И НЕ ЗначениеЗаполнено(ВозможностьИзменения) И НЕ (ПоказателиСхемМотивацииПереопределяемый.ДопустимыйТипПоказателя(ТипПоказателя)) Тогда
			Отказ = Истина;
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Необходимо задать порядок ввода показателя!", Отказ);
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ВидПоказателя) Тогда
			Отказ = Истина;
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Необходимо задать назначение показателя!", Отказ);
		КонецЕсли;
