...
Процедура ПечатьСправкиПоРегламентированнымОтпускамСотрудника(Сотрудник, ДатаУвольнения = Неопределено, Регистратор = Неопределено) Экспорт
	...
	Если ДатаУвольнения = Неопределено Тогда
		...
		ДатаАктуальности	= ?(СотрудникУволен, Выборка.ДатаУвольнения, ОбщегоНазначенияЗК.ПолучитьРабочуюДату());
	Иначе
...
Функция ПечатьРасчетаСреднегоЗаработка(ДокументСсылка, ДатаСобытия, ПродолжительностьРасчетногоПериода, ОписаниеОплачиваемогоПериода, СпособРасчета, Знач ИспользоватьСреднеЧасовойЗаработок = Неопределено, СписокСоставляющихСреднегоЗаработка = Неопределено, ВыводитьЗаголовок = Истина, ОтражатьДолюВремени = Истина, СтруктураПоказателей = Неопределено) Экспорт
	...
	Если ВыводитьЗаголовок Тогда
		...
		СекцияШапка.Параметры.Представление = ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ДокументСсылка, МетаданныеДокумента.Синоним);
...
Функция РасчетСреднегоЗаработкаПособий2011(ДокументОбъект, ДатаСобытия, ОграничиватьЗаработок = Истина, ВыводитьСамостоятельныйОтчет = Истина, СтруктураПоказателей = Неопределено) Экспорт
	...
	ОбластьМакетаЗаголовокТаблицыСтрахователь = Макет.ПолучитьОбласть("ЗаголовокТаблицыСтрахователь"); 
	ОбластьМакетаСтрокаСтрахователь = Макет.ПолучитьОбласть("СтрокаЗаработкаСтрахователь");
	Если ОграничиватьЗаработок Тогда
	...
	ОбластьМакетаШапка.Параметры.ОписаниеОплачиваемогоПериода = "с " + Формат(ДатаНачала, "ДФ=dd.MM.yyyy") + " по " + Формат(ДатаОкончания,"ДФ=dd.MM.yyyy");
	...
	ОбластьМакетаСередина.Параметры.Средний = Формат(Окр(ВсегоЗаработка / 730, 2), "ЧЦ=15; ЧДЦ=2; ЧН=0.00");
	...
	СтруктураПоказателей = Новый Структура("Средний", ВсегоЗаработка / 730);
