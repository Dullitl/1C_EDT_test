...
Функция ЗафиксироватьВремяОкончания(КлючеваяОперация = Неопределено, ВызовИзОбработчикаОжидания) Экспорт
	...
	ДополнительныйПараметр = Соответствие.Получить("ДополнительныйПараметр");	
	Соответствие.Удалить("ДополнительныйПараметр");	
	...
	//Запись.Успех = НЕ ВызовИзОбработчикаОжидания;
	//Запись.ВремяНачала = ВремяНачала;
	Если ДополнительныйПараметр <> Неопределено Тогда
		Запись.ДополнительныйПараметр = ДополнительныйПараметр;
	КонецЕсли;	
	Попытка
...
//  0 - если замер этой ключевой операции уже начат
//
Функция абс_ЗафиксироватьВремяНачала(КлючеваяОперация, ВызовСКлиента, ДополнительныйПараметр = Неопределено) Экспорт
	
	Если Не абс_ВыполнятьЗамерыПроизводительности() Тогда
		Возврат 0;
	КонецЕсли;
	
	ВремяНачала = ТочноеВремя();
	Соответствие = абс_ПолучитьПараметрСеанса();
	...
	Если Соответствие.Получить(КлючеваяОперация) = Неопределено Тогда
		...
		Соответствие.Вставить(КлючеваяОперация, ПараметрыКО);
		Если ДополнительныйПараметр <> Неопределено Тогда
			Соответствие.Вставить("ДополнительныйПараметр", ДополнительныйПараметр);
		КонецЕсли;			
		абс_УстановитьПараметрСеанса(Соответствие);
	Иначе
		ВремяНачала = 0;
	КонецЕсли;
...
//  0 - если ни один замер не начат
//
Функция абс_ЗафиксироватьВремяОкончания(КлючеваяОперация = Неопределено, ВызовИзОбработчикаОжидания, ПричинаЗавершения = "") Экспорт
	...
	Запись = РегистрыСведений.абс_ЗамерыВремени.СоздатьМенеджерЗаписи(); 
	Запись.ДатаЗамера = ТекущаяДата();
	Запись.КлючеваяОперация = КлючеваяОперация;
	Запись.НомерСеанса = НомерСеансаИнформационнойБазы();
	Запись.ВремяВыполнения = (ВремяОкончания - ВремяНачала - ?(ВызовИзОбработчикаОжидания, 100, 0)) / 1000;
	...
	Если ДополнительныйПараметр <> Неопределено Тогда
		Запись.ДополнительныйПараметр = ДополнительныйПараметр;
	КонецЕсли;
	Запись.ПричинаЗавершения = ПричинаЗавершения;
	Попытка
		Запись.Записать(Ложь);
	Исключение
		ЗаписатьВЖурналРегистрации("ЗафиксироватьВремяОкончания",
...
//  Соответствие - параметр сеанса установлен
//
Функция абс_ПолучитьПараметрСеанса()
	
	УстановитьПривилегированныйРежим(Истина);
	
	Возврат ПараметрыСеанса.абс_ТекущийЗамерВремени.Получить();
...
//  Соответствие - Произвольный тип данных, значение которое будет помещенов в параметр сеанса ТекущийЗамерВремени
//
Процедура абс_УстановитьПараметрСеанса(Соответствие)
	
	УстановитьПривилегированныйРежим(Истина);
	ХранилищеЗначения = Новый ХранилищеЗначения(Соответствие);
	ПараметрыСеанса.абс_ТекущийЗамерВремени = ХранилищеЗначения;
...
//
Функция абс_ВыполнятьЗамерыПроизводительности()
	
	УстановитьПривилегированныйРежим(Истина);
	Возврат Константы.абс_ВыполнятьЗамерыПроизводительности.Получить();
...
Процедура УстановкаПараметровСеанса(Знач ИмяПараметра, УстановленныеПараметры) Экспорт
	//АБС Вставка Попов 28.01.2014
	Если ИмяПараметра <> "ТекущийЗамерВремени" И  ИмяПараметра <> "абс_ТекущийЗамерВремени" Тогда
		Возврат;
	КонецЕсли;
	//АБС Вставка Попов 28.01.2014
	УстановитьПривилегированныйРежим(Истина);
	
	ПараметрыСеанса[ИмяПараметра] = Новый ХранилищеЗначения(Новый Соответствие);
