...
Функция КонтекстПлатежногоДокумента(ИсточникДанных) Экспорт
	...
	Если Не ЗначениеЗаполнено(Контекст.Период) Тогда
		Контекст.Период = НачалоДня(ОбщегоНазначенияЗК.ПолучитьРабочуюДату());
	КонецЕсли;
	
	Контекст.КодТерриторииПоУмолчанию = Справочники.Организации.ПолучитьКодТерритории(ИсточникДанных.Организация, Контекст.Период);
	Контекст.ФизическоеЛицо           = Не ОбщегоНазначения.ЭтоЮрЛицо(ИсточникДанных.Организация);
