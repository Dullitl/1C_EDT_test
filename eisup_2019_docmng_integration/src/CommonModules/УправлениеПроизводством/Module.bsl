...
Функция УчетЗатратПоЗаказамНаПроизводство(Дата, Организация = Неопределено, Учет = "Упр")
	...
	Если Учет = "Упр" Тогда
		УчетнаяПолитика = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиУпр(Дата, Ложь);
		...
		КонецЕсли;
	Иначе
		УчетнаяПолитикаБух = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(Дата, Организация, Ложь);
...
Процедура ДвижениеПоЗаказамНаПроизводство(ДокументОбъект, ТаблицаПоТоварам, ИмяТабличнойЧасти, СтруктураШапкиДокумента, РежимПроведения, Отказ, Заголовок) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПоЗаказам, ТаблицаДвижений);
...
Функция ДокументыВПериоде(Док, Периодичность = "Месяц", СтруктРеквизитов = Неопределено) Экспорт
	...
	Иначе
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Неверное значение параметра ""Периодичность"": " + Периодичность + "!");
...
Процедура ЗаполнитьМатериалыПоИнвентаризацииНЗП(
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РезультатЗапроса, ТаблицаМатериалы);
...
Процедура ЗаполнитьПрочиеЗатратыПоИнвентаризацииНЗП(ДокОбъект, ТабличнаяЧасть) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РезультатЗапроса, ТаблицаПрочиеЗатраты);
...
Процедура ЗаполнитьМатериалыПоСпецификациям(ТабличнаяЧастьМатериалы, ТаблицаПродукция, СтруктураДопКолонок, Отбор, ДатаСпецификации, КоличествоУровнейРазузлования = 1, ПараметрыВыпускаПродукции = Неопределено) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПродукция, ТабличнаяЧастьПродукция);
	...
	Если РезультатРазузлования.ИсходныеКомплектующие <> Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ИсходныеКомплектующие, ТабличнаяЧастьМатериалы);
...
Процедура ЗаполнитьВозвратныеОтходыПоСпецификациям(ТабличнаяЧастьВозвратныеОтходы, ТаблицаПродукция, СтруктураДопКолонок, Отбор, ДатаСпецификации, КоличествоУровнейРазузлования = 1, ПараметрыВыпускаПродукции = Неопределено) Экспорт
	...
	Если РезультатРазузлования.ВозвратныеОтходы <> Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ВозвратныеОтходы, ТабличнаяЧастьВозвратныеОтходы);
...
Процедура ЗаполнитьРаспределениеМатериаловНаПродукцию(СтруктураШапкиДокумента, Материалы, ТаблицаПродукция, РаспределениеМатериалов, СтруктураДопКолонок, Отбор, ПараметрыВыпускаПродукции = Неопределено, ЭтоВозвратныеОтходы = Ложь) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(Потребности, ТаблицаПотребностей);
	...
	Для каждого СтрокаМатериала из ТаблицаМатериалы Цикл
		...
		МассивКоличества = ttk_ОбщегоНазначения.РаспределитьПропорционально(СтрокаМатериала.Количество, МассивКоэф, 3);
	...
	#Если Клиент Тогда
		...
		Для каждого СтрокаМатериалыКонтроль из ТаблицаМатериалыКонтроль Цикл
			...
			Если СтрокаМатериалыКонтроль.Количество > 0 Тогда
				...
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Не удалось распределить " + СтрокаМатериалыКонтроль.Количество + " " + СтрокаМатериалыКонтроль.Номенклатура.ЕдиницаХраненияОстатков + ТекстНазвание + СтрокаМатериалыКонтроль.Номенклатура + """ (строка № " + СтрокаМатериалыКонтроль.НомерСтроки + ")", Ложь, ПредставлениеДокумента);
...
Процедура ЗаполнитьРаспределениеПрочихЗатратНаПродукцию(СтруктураШапкиДокумента, ПрочиеЗатраты, Продукция, РаспределениеПрочихЗатрат) Экспорт
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПродукции, ТаблицаБазыРаспределения);
	КонецЕсли;
	...
	Для Каждого Строка Из ПрочиеЗатраты Цикл
		...
		Если Строка.Сумма = 0 
		   ...
		   И (ЕстьСуммаНал И Строка.СуммаНал = 0 ИЛИ Не ЕстьСуммаНал) Тогда
			Продолжить;
		КонецЕсли;
		...
		МассивСуммУпр = ttk_ОбщегоНазначения.РаспределитьПропорционально(Строка.Сумма,     МассивКоэфУУ, 2);
		Если ЕстьСуммаРегл Тогда
			МассивСуммБух = ttk_ОбщегоНазначения.РаспределитьПропорционально(Строка.СуммаРегл, МассивКоэфБУ, 2);
		Иначе
			МассивСуммБух = Неопределено;
		КонецЕсли;
		Если ЕстьСуммаНал Тогда
			МассивСуммНал = ttk_ОбщегоНазначения.РаспределитьПропорционально(Строка.СуммаНал,  МассивКоэфНУ, 2);
		Иначе
		...
		Пока Обход.Следующий() Цикл
			...
			Если (МассивСуммУпр = Неопределено ИЛИ МассивСуммУпр[К] = 0)
			   ...
			   И (МассивСуммНал = Неопределено ИЛИ МассивСуммНал[К] = 0) Тогда
				Продолжить;
			КонецЕсли;
	...
	Для Каждого Колонка Из ТаблицаРаспределениеПрочихЗатрат.Колонки Цикл
		Если  Колонка.Имя = "НомерСтроки" ИЛИ Колонка.Имя = "Сумма" 
		 ИЛИ Колонка.Имя = "СуммаРегл" ИЛИ Колонка.Имя = "СуммаНал" Тогда
			Продолжить;
		КонецЕсли;
	...
	#Если Клиент Тогда
		...
		Для Каждого Строка Из ТаблицаПрочиеЗатраты Цикл
			...
			КонецЕсли;
			Если Не ПустаяСтрока(ТекстСообщения) Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения, Ложь, ПредставлениеДокумента);
			КонецЕсли;
		КонецЦикла;
	#КонецЕсли
	...
	Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		Если ЗначениеЗаполнено(СтруктураШапкиДокумента.Подразделение)
		   ...
		   И СтруктураШапкиДокумента.Подразделение.ВидПодразделения<>Перечисления.ВидыПодразделений.ВспомогательноеПроизводство Тогда
		    ...
		    ttk_ОбщегоНазначения.СообщитьОбОшибке("В шапке документа должно быть указано подразделение с видом ""Основное производство"" или ""Вспомогательное производство""!", Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете или СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
		Если ЗначениеЗаполнено(СтруктураШапкиДокумента.ПодразделениеОрганизации)
		   ...
		   И СтруктураШапкиДокумента.ПодразделениеОрганизации.ВидПодразделения<>Перечисления.ВидыПодразделений.ВспомогательноеПроизводство Тогда
		    ...
		    ttk_ОбщегоНазначения.СообщитьОбОшибке("В шапке документа должно быть указано подразделение организации с видом ""Основное производство"" или ""Вспомогательное производство""!", Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьЧтоНетСчетовКосвенныхРасходовНУ(ТабЧасть, ИмяТабЧасти, СписокСчетовКосвенныхРасходовНУ, Отказ, Заголовок) Экспорт
	...
	Для Каждого СтрокаТЧ Из ТабЧасть Цикл
		Если СписокСчетовКосвенныхРасходовНУ.НайтиПоЗначению(СтрокаТЧ.СчетЗатратНУ)<>неопределено Тогда
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке("В документе нельзя использовать косвенные счета затрат НУ (таб.часть """ + ИмяТабЧасти + """ строка № " + СтрокаТЧ.НомерСтроки + ")", Отказ, Заголовок);
...
Процедура ПроверитьКоличествоДоделка(ДокументОбъект, ИмяТабличнойЧасти, ТаблицаЗначений, 
	...
	Для Каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
		...
		Если СтрокаТаблицы.Количество<СтрокаТаблицы.КоличествоДоделка Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + " доделка больше количества выпуска. ", Отказ, Заголовок);
		КонецЕсли;
	...
	Для Каждого СтрокаТЧ Из ТабЧасть Цикл
		...
		Если ВидСтатьиЗатрат = "Нематериальные" Тогда
			Если СтатЗатрат.ВидЗатрат = Перечисления.ВидыЗатрат.Материальные Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Укажите статью затрат с нематериальным видом затрат (таб.часть """ + ИмяТабЧасти + """ строка № " + СтрокаТЧ.НомерСтроки + ")", Отказ, Заголовок);
			КонецЕсли;

		ИначеЕсли ВидСтатьиЗатрат = "Материальные" Тогда
			Если Не СтатЗатрат.ВидЗатрат = Перечисления.ВидыЗатрат.Материальные Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Укажите статью затрат с видом затрат ""Материальные"" (таб.часть """ + ИмяТабЧасти + """ строка № " + СтрокаТЧ.НомерСтроки + ")", Отказ, Заголовок);
			КонецЕсли;
		...
		ИначеЕсли ВидСтатьиЗатрат = "Производственные" Тогда
			Если Не СтатЗатрат.ХарактерЗатрат = Перечисления.ХарактерЗатрат.ПроизводственныеРасходы
			   И (Не ЕстьСумма ИЛИ ЕстьСумма И СтрокаТЧ.Сумма <> 0) Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Укажите статью затрат с характером затрат ""Производственные расходы"" (таб.часть """ + ИмяТабЧасти + """ строка № " + СтрокаТЧ.НомерСтроки + ")", Отказ, Заголовок);
			КонецЕсли;
		...
		ИначеЕсли ВидСтатьиЗатрат = "Непроизводственные" Тогда
			Если СтатЗатрат.ХарактерЗатрат = Перечисления.ХарактерЗатрат.ПроизводственныеРасходы
			   И (Не ЕстьСумма ИЛИ ЕстьСумма И СтрокаТЧ.Сумма <> 0) Тогда
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Укажите статью затрат с характером затрат отличным от ""Производственные расходы"" (таб.часть """ + ИмяТабЧасти + """ строка № " + СтрокаТЧ.НомерСтроки + ")", Отказ, Заголовок);
			КонецЕсли;
...
Процедура ПолучитьИспользованиеЛимитовОтпускаМатериалов(ДокументОбъект, ИмяТабличнойЧасти, ТаблицаТоваров) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РезультатЗапроса, ТаблицаТоваров);
