...
Процедура ДобавитьСтрокуВДвиженияПоРегистрамНакопления(ДокументОбъект, ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента) Экспорт 
	...
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.КодДоходаНДФЛ) Тогда
		...
		ЗаполнитьЗначенияСвойств(Движение,ВыборкаПоШапкеДокумента); // ФизЛицо, Организация, ОбособленноеПодразделение, ПодразделениеОрганизации
		ЗаполнитьЗначенияСвойств(Движение,ВыборкаПоСтрокамДокумента); // ВидРасчета
		
		// Свойства
		Движение.Период						= ?(НачалоМесяца(ВыборкаПоШапкеДокумента.ДатаУвольнения) < КонецМесяца(ВыборкаПоШапкеДокумента.ПериодРегистрации) + 1, ВыборкаПоШапкеДокумента.ПериодРегистрации, ВыборкаПоШапкеДокумента.ДатаУвольнения);
...
Функция ПечатьТ61(ДокументОбъект) Экспорт
	...
	Иначе
		...
		КонецЕсли; 
		СуммаНДФЛ =  Окр(ВсегоВыплат * СтавкаНДФЛ/100, 0);
		ПрочиеУдержания = 0;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("Период",			ДокументОбъект.ДатаУвольнения);
	...
	Если Выборка.Следующий() Тогда
		...
		КонечноеСальдо = ?(Выборка.НачальноеСальдо = Null,0,Выборка.НачальноеСальдо) + ВсегоВыплат - ВсегоУдержано;
