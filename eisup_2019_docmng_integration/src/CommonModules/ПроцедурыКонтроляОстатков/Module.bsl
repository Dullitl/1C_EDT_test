...
Процедура ТоварыПереданныеКонтрольОстатков_ЗаказВШапке(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ)
	...
	// АБС ВСТАВКА Проведение в разрезе ЦФО и Назначения использования
	Попытка
	Запрос.УстановитьПараметр("абс_ЦФО"					, СтруктураШапкиДокумента.абс_ЦФО);
	Запрос.УстановитьПараметр("абс_НазначениеПлатежа"	, СтруктураШапкиДокумента.абс_НазначениеПлатежа);
	Исключение
	Запрос.УстановитьПараметр("абс_ЦФО"					, Неопределено);
	Запрос.УстановитьПараметр("абс_НазначениеПлатежа"	, Неопределено);
	КонецПопытки;
	// АБС ВСТАВКА Проведение в разрезе ЦФО и Назначения использования КОНЕЦ
			...
			Пока ВыборкаРегистраторов.Следующий() Цикл
				...
				Если ТипЗнч(НачСсылка) = Тип("ДокументСсылка.КорректировкаЗаписейРегистров") Тогда
					...
					Попытка
						НаборСвободныеОстатки.Записать(Ложь);
					Исключение
						ttk_ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(), ОшибкаПриЗаписиДвижений); 
					КонецПопытки; 
