...
Процедура ПроверитьЗаполнениеРаботникиОрганизации(ЭтотОбъект, Отказ, Заголовок = "")
	...
	Если Год(ЭтотОбъект.Дата) > ЭтотОбъект.НалоговыйПериод Тогда
		МесяцНалоговогоПериода = Дата(ЭтотОбъект.НалоговыйПериод, 12, 1);
	Иначе	
		МесяцНалоговогоПериода = НачалоМесяца(ЭтотОбъект.Дата);
	КонецЕсли;
	...
	Пока ВыборкаПоРаботникиОрганизации.Следующий() Цикл								
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаПоРаботникиОрганизации.ФизЛицо) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(НачалоСообщения + "не указано физическое лицо!", Отказ, Заголовок);
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаПоРаботникиОрганизации.НалогПоСтавке09 + ВыборкаПоРаботникиОрганизации.НалогПоСтавке13 + ВыборкаПоРаботникиОрганизации.НалогПоСтавке35 
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке(НачалоСообщения + "не указаны суммы перерасчета налога!", Отказ, Заголовок);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ВыборкаПоРаботникиОрганизации.ПодразделениеОрганизации) И ВыборкаПоРаботникиОрганизации.ОшибкаПодразделениеНеПринадлежитОрганизации Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(НачалоСообщения + "указано подразделение, принадлежащее другой организации!", Отказ, Заголовок);
		КонецЕсли;
	КонецЦикла;
...
Процедура СформироватьВзаиморасчетыСБюджетом(ЭтотОбъект, ШапкаДокумента)
	...
	Если Год(ШапкаДокумента.Дата) > ШапкаДокумента.НалоговыйПериод Тогда
		МесяцНалоговогоПериода = Дата(ШапкаДокумента.НалоговыйПериод, 12, 1);
	Иначе	
		МесяцНалоговогоПериода = НачалоМесяца(ШапкаДокумента.Дата);
	КонецЕсли; 
