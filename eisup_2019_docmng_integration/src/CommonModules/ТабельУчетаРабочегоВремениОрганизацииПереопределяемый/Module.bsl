...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь, ЭтотОбъект) Экспорт
	...
	Если ИмяМакета = "Т13" Тогда
		...
		Если ТекстОшибки <> "" Тогда
			...
			Возврат Неопределено;
		Иначе
			Форма = УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ttk_ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,ЭтотОбъект.Метаданные().Синоним + " (форма Т-13)"));
			ЗаполнитьЗначенияСвойств(Форма.ЭлементыФормы.ПолеТабличногоДокумента, ТабДокумент);
			Возврат Форма;
		КонецЕсли;
