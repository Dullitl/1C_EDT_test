...
Процедура ЗаполнитьЗапросЭтоНеЮрЛицо(Запрос, ЭтотОбъект) Экспорт
	...
	Если ЭтотОбъект.НалоговыйПериод > 2007 Тогда
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГражданствоФизЛиц.СрезПоследних(&КонецНП, ) КАК Гражданство
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь, ЭтотОбъект) Экспорт
	...
	Если ИмяМакета = "ПечатьФайла" Тогда
		...
		Иначе
			...
			Заголовок = ttk_ОбщегоНазначения.ПредставлениеДокументаПриПроведении(ЭтотОбъект.Ссылка);
		...
		Если Не ТипДанныхЭтоСтрока Или Найти(ТекстФайла,"@@@") = 0 Тогда // с 2005 года
			...
			Если ТипДанныхЭтоСтрока Тогда
				...
				Если Дерево2НДФЛ = Неопределено Тогда
					Отказ = Ложь;
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не удалось разобрать файл с данными!",Отказ, ttk_ОбщегоНазначения.ПредставлениеДокументаПриПроведении(ЭтотОбъект.Ссылка));
					Возврат Неопределено 
				КонецЕсли;
			Иначе
			...
			Если ВерсияФормата = "" Тогда
				Отказ = Ложь;
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указана версия формата! Возможно, нарушена структура файла.",Отказ, ttk_ОбщегоНазначения.ПредставлениеДокументаПриПроведении(ЭтотОбъект.Ссылка));
				Возврат Неопределено 
			КонецЕсли;
			...
			Если ВерсияФормата = "4.00001" Тогда // с 2006 года
				...
				Если ЭтотОбъект.Ответственный.Пустая() Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указано ответственное лицо!", Отказ);
				Иначе	
					...
					Если Не ЗначениеЗаполнено(ВыборкаСведенийОбОтветственном.ФизЛицо) Тогда 
						ttk_ОбщегоНазначения.СообщитьОбОшибке("У ответственного лица не указано физлицо!", Отказ);	
					ИначеЕсли Не ЗначениеЗаполнено(ВыборкаСведенийОбОтветственном.Фамилия) И Не ЗначениеЗаполнено(ВыборкаСведенийОбОтветственном.Имя) Тогда
						ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указано ФИО ответственного лица, либо отсутствует доступ к этим данным!", Отказ);	
					Иначе	
				...
				Если ОбщегоНазначенияПереопределяемый.ЭтоЮрЛицо(ЭтотОбъект.Организация) И ЭтотОбъект.ДолжностьОтветственного.Пустая() Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указана должность ответственного лица!", Отказ);	
				КонецЕсли; 
				...
				Для каждого СтрокаДерева Из Дерево2НДФЛ.Строки Цикл
					Если СтрокаДерева.Имя = "Справка" Тогда
						...
						ЭтотОбъект.ОчиститьПараметрыОбластиТабличногоДокумента(Справка);
		...
		Иначе // устаревший порядок формирования отчетности
			...
			ФорматФайлаСведений = СведенияОДоходахФизлицДополнительный.ЗагрузитьФорматФайлаСведений(ЭтотОбъект.ПолучитьМакет("ФорматНДФЛ2003"));
			...
			Для НомерДокумента = 1 По НаборЗаписейНА.КолДок.Значение Цикл
				...
				Для Каждого Строка ИЗ Справка.ДоходВид.Значение  Цикл
					...
					Если НомерКолонки = 2 Тогда
						...
						ЭтотОбъект.ОчиститьПараметрыОбластиТабличногоДокумента(ОбластьР3СтрокаДоходов);
					КонецЕсли; 
				КонецЦикла; 
				...
				Для Каждого Строка ИЗ Справка.Вычет.Значение  Цикл
					...
					Если НомерКолонки = 4 Тогда
						...
						ЭтотОбъект.ОчиститьПараметрыОбластиТабличногоДокумента(ОбластьР3СтрокаВычеты);
					КонецЕсли; 
				КонецЦикла;		
				// вывод последней неполной строки
				...
				Для Каждого Строка ИЗ Справка.ДоходВидДр.Значение  Цикл
					...
					Если НомерКолонки = 2 Тогда
						...
						ЭтотОбъект.ОчиститьПараметрыОбластиТабличногоДокумента(ОбластьР5СтрокаДоходов);
					КонецЕсли; 
				КонецЦикла; 
				// вывод последней неполной строки
				Если НомерДохода % 2 <> 0 Тогда
				...
				Для Каждого Строка ИЗ Справка.ДохИмущ.Значение  Цикл
					...
					Если НомерКолонки = 3 Тогда
						...
						ЭтотОбъект.ОчиститьПараметрыОбластиТабличногоДокумента(ОбластьР7СтрокаДоходов);
					КонецЕсли; 
				КонецЦикла; 
				// вывод последней неполной строки
