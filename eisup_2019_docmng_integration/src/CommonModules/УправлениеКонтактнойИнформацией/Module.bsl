...
Функция ПолучитьПредставлениеАдреса(Запись) Экспорт
	...
	ДополнитьПредставлениеАдреса(ТекПредставление, СокрЛП(Запись.Поле7), ", "+Запись.ТипДома+" № ");
	ДополнитьПредставлениеАдреса(ТекПредставление, СокрЛП(Запись.Поле8), ", "+Запись.ТипКорпуса+" ");
	ДополнитьПредставлениеАдреса(ТекПредставление, СокрЛП(Запись.Поле9), ", "+Запись.ТипКвартиры+" ");
	//АБС-
...
Процедура ПрочитатьКонтактнуюИнформацию(НаборЗаписей, Ссылка) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	МАКСИМУМ(ВложенныйЗапрос.абс_Синхронизировано) КАК абс_Синхронизировано
	               |ИЗ
	               |	(ВЫБРАТЬ
	               |		&Объект КАК Объект,
	               ...
	               |		РегистрСведенийКонтактнаяИнформация.абс_Синхронизировано
	               |	ИЗ
...
Процедура ЗаписатьКонтактнуюИнформацию(НаборЗаписей, Ссылка, Отказ) Экспорт
	...
	Если НЕ Отказ Тогда
		...
		Если Не НастройкаПравДоступа.СравнитьТаблицыНаборовЗаписей(ПрочитатьНаборЗаписейКонтактнойИнформации(Ссылка), НаборЗаписей.Выгрузить()) Тогда
			...
			Исключение
				ttk_ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, ("Элемент """ + СокрЛП(Ссылка) + """ не записан. Не записана контактная информация."));
...
Функция СведенияОЮрФизЛице(ЮрФизЛицо, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина, БанковскийСчет = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Организации") Тогда
		...
		|					ТОГДА
		|						ВЫБОР КОГДА КонтактнаяИнформация.Вид = &ВидЮридическийАдресФизЛицо 
		|							ТОГДА ВЫРАЗИТЬ(КонтактнаяИнформация.Представление КАК Строка(1000))
		|	  						ИНАЧЕ """"
		|						КОНЕЦ
		|				ИНАЧЕ
		|					ВЫБОР КОГДА КонтактнаяИнформация.Вид = &ВидЮридическийАдрес 
		|						ТОГДА ВЫРАЗИТЬ(КонтактнаяИнформация.Представление КАК Строка(1000))
		|	  					ИНАЧЕ """"
		|					КОНЕЦ
		...
		|	, МАКСИМУМ (ВЫБОР КОГДА Организации.ЮрФизЛицо = ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ФизЛицо)
		|					ТОГДА
		|						ВЫБОР КОГДА КонтактнаяИнформация.Вид = &ВидФактическийАдресФизЛицо 
		|							ТОГДА ВЫРАЗИТЬ(КонтактнаяИнформация.Представление КАК Строка(1000))
		|	  						ИНАЧЕ """"
		|						КОНЕЦ
		|				ИНАЧЕ
		|					ВЫБОР КОГДА КонтактнаяИнформация.Вид = &ВидФактическийАдрес 
		|						ТОГДА ВЫРАЗИТЬ(КонтактнаяИнформация.Представление КАК Строка(1000))
		|	  					ИНАЧЕ """"
		|					КОНЕЦ
		...
		|	И КонтактнаяИнформация.Вид В (&ВидЮридическийАдресФизЛицо, &ВидФактическийАдресФизЛицо, &ВидЮридическийАдрес, &ВидФактическийАдрес, &ВидТелефона)
	...
	ИначеЕсли ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		|	И КонтактнаяИнформация.Вид В (&ВидЮридическийАдрес, &ВидФактическийАдрес, &ВидТелефона)
		|	И КонтактнаяИнформация.Тип В (&ТипАдрес, &ТипТелефон)";
