...
Процедура ПроверитьПриоритет(Отказ)
	...
	Если ДополнительныеСвойства.Свойство(абс_БСПОценкаПроизводительностиКлиентСервер.НеПроверятьПриоритет()) Или Приоритет = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		абс_БСПОценкаПроизводительностиКлиентСервер.ЗаписатьВЖурналРегистрации(
			"Справочник.абс_БСПКлючевыеОперации.МодульОбъекта.ПередЗаписью",
