...
// Таблица значений, выведенная в основную форму элемента справочника
Перем ТаблицаЗагружаемыхРеквизитов Экспорт;

// Первая строка данных табличного документа, из которого производится загрузка
Перем ПерваяСтрокаДанныхТабличногоДокумента Экспорт;
...
КонецПроцедуры // ttk_ОбщегоНазначения.СообщитьОбОшибке()
...
// В процедуре производится перевод данных таблицы "ТаблицаЗагружаемыхРеквизитов"
// в табличную часть "ТЧТаблицаЗагружаемыхРеквизитов"
Процедура абс_ПеревестиТаблицуЗначенийВТабличнуюЧасть() Экспорт
	
	СтрокаЗагружаемыхПолей = "Пометка, ПредставлениеРеквизита, ИмяРеквизита, 
							| МожетБытьПолемПоиска, ПолеПоиска, РежимЗагрузки, 
							| НомерКолонки, Выражение, ЭлементСвязиПоТипу, ШиринаКолонки,
							| ИскатьПо, СвязьПоВладельцу, СвязьПоТипу";
	
	ТЧТаблицаЗагружаемыхРеквизитов.Очистить();
	Для Каждого ТекСтрока Из ТаблицаЗагружаемыхРеквизитов Цикл
		НСтрока = ТЧТаблицаЗагружаемыхРеквизитов.Добавить();
		ЗаполнитьЗначенияСвойств(НСтрока, ТекСтрока, СтрокаЗагружаемыхПолей);
		НСтрока.ДоступноеОписаниеТипов = абс_ПеревестиОписаниеТипов(ТекСтрока.ДоступноеОписаниеТипов);
		НСтрока.ОписаниеТипов = абс_ПеревестиОписаниеТипов(ТекСтрока.ОписаниеТипов);
		Если ТипЗнч(ТекСтрока.ЗначениеПоУмолчанию) = Тип("ОписаниеТипов") Тогда
			НСтрока.ЗначениеПоУмолчаниюОписаниеТипов = абс_ПеревестиОписаниеТипов(ТекСтрока.ЗначениеПоУмолчанию);
		Иначе
			НСтрока.ЗначениеПоУмолчанию = ТекСтрока.ЗначениеПоУмолчанию;
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

// Функция возвращает хранилище значений, в которое записано значение переданной в параметре
// переменной типа "ОписаниеТипов"
// Параметры:
//  СохраняемоеЗначение - ОписаниеТипов, сохраняемое в хранилище значений
//
// Возвращаемое значение:
//  Значение типа "ХранилищеЗначения"
//
Функция абс_ПеревестиОписаниеТипов(СохраняемоеЗначение) Экспорт
	
	Если ТипЗнч(СохраняемоеЗначение) <> Тип("ОписаниеТипов") Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат Новый ХранилищеЗначения(СохраняемоеЗначение, Новый СжатиеДанных(9));
	
КонецФункции

// В процедуре производится перевод данных табличной части "ТЧТаблицаЗагружаемыхРеквизитов"
// в таблицу значений "ТаблицаЗагружаемыхРеквизитов"
//
Процедура абс_ПеревестиТабличнуюЧастьВТаблицуЗначений() Экспорт
	
	// в таблице загружаемых реквизитов, которая заполнена реквизитами загружаемого объекта
	// проставим настройки из табличной части
	Для Каждого ТекСтрока Из ТЧТаблицаЗагружаемыхРеквизитов Цикл
		СтрокаТЗ = ТаблицаЗагружаемыхРеквизитов.Найти(ТекСтрока.ИмяРеквизита, "ИмяРеквизита");
		Если СтрокаТЗ <> Неопределено Тогда
			СтрокаТЗ.Пометка = ТекСтрока.Пометка;
			СтрокаТЗ.РежимЗагрузки = ТекСтрока.РежимЗагрузки;
			СтрокаТЗ.НомерКолонки = ТекСтрока.НомерКолонки;
			СтрокаТЗ.Выражение = ТекСтрока.Выражение;
			СтрокаТЗ.ЭлементСвязиПоТипу = ТекСтрока.ЭлементСвязиПоТипу;
			СтрокаТЗ.ШиринаКолонки = ТекСтрока.ШиринаКолонки;
			// теперь заполним "нетривиальные" реквизиты
			Если СтрокаТЗ.МожетБытьПолемПоиска Тогда
				СтрокаТЗ.ПолеПоиска = ТекСтрока.ПолеПоиска;
			КонецЕсли;
			Если СтрокаТЗ.ДоступноеОписаниеТипов = ТекСтрока.ДоступноеОписаниеТипов.Получить() Тогда
				СтрокаТЗ.ОписаниеТипов = ТекСтрока.ОписаниеТипов.Получить();
				ЗначениеПоУмолчаниюОписаниеТипов = ТекСтрока.ЗначениеПоУмолчаниюОписаниеТипов.Получить();
				Если ЗначениеЗаполнено(ЗначениеПоУмолчаниюОписаниеТипов) Тогда
					СтрокаТЗ.ЗначениеПоУмолчанию = ЗначениеПоУмолчаниюОписаниеТипов;
				Иначе
					СтрокаТЗ.ЗначениеПоУмолчанию = ТекСтрока.ЗначениеПоУмолчанию;
				КонецЕсли;
			КонецЕсли;
			СтрокаТЗ.ИскатьПо = ТекСтрока.ИскатьПо;
			СтрокаТЗ.СвязьПоВладельцу = ТекСтрока.СвязьПоВладельцу;
			СтрокаТЗ.СвязьПоТипу = ТекСтрока.СвязьПоТипу;
		КонецЕсли;
	КонецЦикла;
...
//
Процедура абс_УстановитьИсточник() Экспорт
	...
	Иначе
		Настройка = ПоучитьНастройкуПоУмолчанию(СписокСохраненныхНастроек);
...
Процедура СформироватьСтруктуруКолонок() Экспорт
	...
	Для каждого ЗагружаемыйРеквизит Из ТаблицаЗагружаемыхРеквизитов Цикл
		...
		Для каждого КолонкаЗагружаемыхРеквизитов Из ТаблицаЗагружаемыхРеквизитов.Колонки Цикл
			Если Не Истина И КолонкаЗагружаемыхРеквизитов.Имя = "НомерКолонки" Тогда
...
Процедура ОбновитьДанныеТабличногоДокумента(ТабличныйДокумент, БезВопросов = Ложь) Экспорт
	...
	Если (РежимЗагрузки = 0 или РежимЗагрузки = 2 или РежимЗагрузки = 3) и ТабличныйДокумент.ВысотаТаблицы > 1 И Не БезВопросов Тогда
	...
	МетаданныеИсточника = ПолучитьМетаданныеИсточника();
	Если РежимЗагрузки =0 или РежимЗагрузки =2 или РежимЗагрузки = 3 или МетаданныеИсточника = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
//
Функция ЗагрузитьДанные(ТабличныйДокумент, ТаблицаЗагрузки = Неопределено, ИмяКолонки = "", Организация = Неопределено) Экспорт
	
	Если ИмяКолонки = "" ИЛИ ТаблицаЗагрузки = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	ИначеЕсли РежимЗагрузки = 3 Тогда
		...
		Источник = СсылкаИсточника;
		ТекстВопросаИсточника = " документов: """ + МетаданныеИсточника.Представление() + """";
	...
	ИначеЕсли РежимЗагрузки = 3 Тогда // для документов - только поиск
		...
		Если Не СтрокиПоиска.Количество() = 0 Тогда
			...
			Для каждого СтрокаПоиска Из СтрокиПоиска Цикл
				...
				Если СтрокаПоиска.ИмяРеквизита = "Дата" Тогда
					// по дате ищем с точностью до дня
					ТекстЗапроса = ТекстЗапроса +"
					|НачалоПериода(Документ.Дата, День) = НачалоПериода(&Дата, День)
					|И";					
				ИначеЕсли СтрокаПоиска.ИмяРеквизита = "Номер" Тогда
					// по номеру ищем с учетом префиксов организации и узла
					ТекстЗапроса = ТекстЗапроса +"
					|(Документ.Номер = &НомерСПрефиксом ИЛИ Документ.Номер = &Номер)
					|И";					
				Иначе
					ТекстЗапроса = ТекстЗапроса +"
	...
	//ОчиститьСообщения();
	//Сообщить("Выполняется сопоставление"+ ТекстВопросаИсточника, СтатусСообщения.Информация);
	//Сообщить("Всего: " + КоличествоЭлементов, СтатусСообщения.Информация);
	//Сообщить("---------------------------------------------", СтатусСообщения.БезСтатуса);
	Индикатор = 0;
	Загружено = 0;
	КорректорИндекса = ПерваяСтрокаДанныхТабличногоДокумента;
	
	Для К = КорректорИндекса По ТабличныйДокумент.ВысотаТаблицы Цикл
		...
		ИначеЕсли РежимЗагрузки = 3 Тогда
			...
			Если Не Запрос = Неопределено Тогда
				...
				Для каждого СтрокаПоиска Из СтрокиПоиска Цикл
					...
					Если ПустаяСтрока(ЗначениеРеквизита) Тогда
						СтрокаОшибок = ?(ПустаяСтрока(СтрокаОшибок),"",СтрокаОшибок + ", ") + СтрокаПоиска.ПредставлениеРеквизита;
					Иначе
						Если СтрокаПоиска.ИмяРеквизита = "Номер" Тогда
							// два параметра - один с префиксом, другой без
							ПрефиксОрганизации = "";
							Если Организация <> Неопределено Тогда
								МетаданныеИсточника = Источник.Метаданные();
								Если МетаданныеИсточника.Реквизиты.Найти("Организация") <> Неопределено Тогда
							...
							// теперь дополним нулями
							НулиНомера = "";
							Для Сч = СтрДлина(ПрефиксУзла) + СтрДлина(ПрефиксОрганизации) + СтрДлина(ТекущаяСтрока[СтрокаПоиска.ИмяРеквизита]) + 1 По МетаданныеИсточника.ДлинаНомера Цикл
								НулиНомера = НулиНомера + "0";
							КонецЦикла;
							Запрос.УстановитьПараметр("НомерСПрефиксом", ПрефиксУзла + ПрефиксОрганизации + НулиНомера + ТекущаяСтрока[СтрокаПоиска.ИмяРеквизита]);
							Запрос.УстановитьПараметр("Номер", ТекущаяСтрока[СтрокаПоиска.ИмяРеквизита]);
						ИначеЕсли СтрокаПоиска.ИмяРеквизита = "Дата" Тогда
							// приведем дату к типу "Дата"
							Попытка
								РеквизитДата = Дата(ТекущаяСтрока[СтрокаПоиска.ИмяРеквизита]);
								Запрос.УстановитьПараметр("Дата", РеквизитДата);
							Исключение
								СтрокаОшибок = ?(ПустаяСтрока(СтрокаОшибок),"",СтрокаОшибок + ", ") + СтрокаПоиска.ПредставлениеРеквизита;
							КонецПопытки;
						Иначе
			...
			ОбъектНайден = Не Объект = Неопределено;
			Если Не ОбъектНайден Тогда // для документов НЕДОПУСТИМО создание новых - только ищем существующие
				Индикатор = Индикатор + 1;
				Продолжить;
			КонецЕсли;			
		КонецЕсли;
		...
		Если РежимЗагрузки = 0 Тогда
			Если Не Отказ и ЗаписатьОбъект(Объект, ТекстыЯчеек) Тогда
				...
				// добавим в поле ИмяКолонки таблицы "ТаблицаЗагрузки" в соответствующую строку 
				// ссылку на найденный объект
				ТаблицаЗагрузки[НомерТекущейСтроки - 1][ИмяКолонки] = Объект.Ссылка;
			Иначе
		...
		ИначеЕсли РежимЗагрузки = 3 Тогда // документ
			
			Если Не Отказ  Тогда
				//Сообщить("Найден документ: " + Объект.ссылка, СтатусСообщения.Информация);
				Загружено = Загружено + 1;
				// добавим в поле ИмяКолонки таблицы "ТаблицаЗагрузки" в соответствующую строку 
				// ссылку на найденный объект
				ТаблицаЗагрузки[НомерТекущейСтроки - 1][ИмяКолонки] = Объект.Ссылка;
			Иначе
				Сообщить("Объект не найден. Элемент справочника: " + Объект + ".", СтатусСообщения.Важное);
			КонецЕсли;
	...
	Если РежимЗагрузки = 1  Тогда
		Если ЗаписыватьОбъект и ЗаписатьОбъект(ОбъектИсточника) Тогда
			...
			//Сообщить("Выполнено сопоставление"+ 	ТекстВопросаИсточника, СтатусСообщения.Информация);
			//Сообщить("" +Загружено +" из "+ КоличествоЭлементов + " элементов.", СтатусСообщения.Информация);
			Возврат Истина;
		Иначе
			Сообщить("Сопоставление не выполнено по: " + Объект + ".", СтатусСообщения.Важное);
			Возврат Ложь;
		КонецЕсли;
	...
	ИначеЕсли РежимЗагрузки = 3 Тогда
		//Сообщить("Выполнено сопоставление"+ 	ТекстВопросаИсточника, СтатусСообщения.Информация);
		//Сообщить("" +Загружено +" из "+ КоличествоЭлементов + " документов.", СтатусСообщения.Информация);
		Возврат Истина;
	КонецЕсли;
...
Функция ПолучитьСписокСвязейПоВладельцу(ЗагружаемыйРеквизит) Экспорт
	...
	Для каждого ТипКолонки Из ЗагружаемыйРеквизит.ОписаниеТипов.Типы() Цикл
		Если Справочники.ТипВсеСсылки().СодержитТип(ТипКолонки) Тогда
			Для каждого ТекВладелец Из Метаданные.НайтиПоТипу(ТипКолонки).Владельцы Цикл
...
Процедура ЗаполнитьНастройкиКолонок(Настройки) Экспорт
	...
	ИначеЕсли РежимЗагрузки = 3 Тогда
		...
		ЗаполнитьНастройкиКолонокДокумента();
...
// Заполняет настройки колонок по умолчанию для документа
//
Процедура ЗаполнитьНастройкиКолонокДокумента()
	...
	Если МетаданныеИсточника.ДлинаНомера > 0 Тогда
		...
		ЗагружаемыйРеквизит = ТаблицаЗагружаемыхРеквизитов.Добавить();
		ЗагружаемыйРеквизит.ИмяРеквизита           = "Номер";
		ЗагружаемыйРеквизит.ПредставлениеРеквизита = "Номер";
		ЗагружаемыйРеквизит.МожетБытьПолемПоиска   =  Истина;
		
		Если МетаданныеИсточника.ТипНомера = Метаданные.СвойстваОбъектов.ТипНомераДокумента.Строка Тогда
			ЗагружаемыйРеквизит.ДоступноеОписаниеТипов = Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(МетаданныеИсточника.ДлинаНомера));
		Иначе
			ЗагружаемыйРеквизит.ДоступноеОписаниеТипов = Новый ОписаниеТипов("Число", , , Новый КвалификаторыЧисла(МетаданныеИсточника.ДлинаНомера));
		КонецЕсли;
	...
	// дата
	ЗагружаемыйРеквизит = ТаблицаЗагружаемыхРеквизитов.Добавить();
	ЗагружаемыйРеквизит.ИмяРеквизита           = "Дата";
	ЗагружаемыйРеквизит.ПредставлениеРеквизита = "Дата";
	ЗагружаемыйРеквизит.МожетБытьПолемПоиска   =  Истина;
	ЗагружаемыйРеквизит.ДоступноеОписаниеТипов = Новый ОписаниеТипов("Дата", , , Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя));
...
// добавим создание колонок таблицы значений ТаблицаЗагружаемыхРеквизитов
ТаблицаЗагружаемыхРеквизитов = Новый ТаблицаЗначений;
ОписаниеТипаСтрокаНеограниченнойДлины = Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(0, ДопустимаяДлина.Переменная));
ОписаниеТипаБулево = Новый ОписаниеТипов("Булево");
ОписаниеТипаОписаниеТипов = Новый ОписаниеТипов("ОписаниеТипов");
ТаблицаЗагружаемыхРеквизитов.Колонки.Добавить("Пометка", ОписаниеТипаБулево);
ТаблицаЗагружаемыхРеквизитов.Колонки.Добавить("ПредставлениеРеквизита", ОписаниеТипаСтрокаНеограниченнойДлины);
ТаблицаЗагружаемыхРеквизитов.Колонки.Добавить("ИмяРеквизита", ОписаниеТипаСтрокаНеограниченнойДлины);
ТаблицаЗагружаемыхРеквизитов.Колонки.Добавить("МожетБытьПолемПоиска", ОписаниеТипаБулево);
ТаблицаЗагружаемыхРеквизитов.Колонки.Добавить("ПолеПоиска", ОписаниеТипаБулево);
ТаблицаЗагружаемыхРеквизитов.Колонки.Добавить("ДоступноеОписаниеТипов", ОписаниеТипаОписаниеТипов);
ТаблицаЗагружаемыхРеквизитов.Колонки.Добавить("ОписаниеТипов", ОписаниеТипаОписаниеТипов);
ТаблицаЗагружаемыхРеквизитов.Колонки.Добавить("РежимЗагрузки", ОписаниеТипаСтрокаНеограниченнойДлины);
ТаблицаЗагружаемыхРеквизитов.Колонки.Добавить("НомерКолонки", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(5, 0, ДопустимыйЗнак.Неотрицательный)));
ТаблицаЗагружаемыхРеквизитов.Колонки.Добавить("ЗначениеПоУмолчанию");
ТаблицаЗагружаемыхРеквизитов.Колонки.Добавить("Выражение", ОписаниеТипаСтрокаНеограниченнойДлины);
ТаблицаЗагружаемыхРеквизитов.Колонки.Добавить("ИскатьПо");
ТаблицаЗагружаемыхРеквизитов.Колонки.Добавить("СвязьПоВладельцу");
ТаблицаЗагружаемыхРеквизитов.Колонки.Добавить("СвязьПоТипу");
ТаблицаЗагружаемыхРеквизитов.Колонки.Добавить("ЭлементСвязиПоТипу", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(1, 0, ДопустимыйЗнак.Неотрицательный)));
ТаблицаЗагружаемыхРеквизитов.Колонки.Добавить("ШиринаКолонки", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(3, 0, ДопустимыйЗнак.Неотрицательный)));
