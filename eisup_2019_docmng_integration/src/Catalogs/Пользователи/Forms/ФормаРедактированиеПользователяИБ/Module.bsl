...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПравоАдминистрирования = Пользователи.ПроверитьПравоАдминистрирования();
	...
	//АБС Коломиец 19351+
	Если РольДоступна("абс_АдминистраторДЗО_Филиала") Тогда
		Если ЗначениеЗаполнено(Объект.Ссылка)
			И ЗаполнитьДанныеПользователяИБВРеквизиты() Тогда
		КонецЕсли;		
		Элементы.ГруппаНетПраваАдминистрирования.Видимость = Ложь;
...
&НаКлиенте
Процедура УстановитьОтборПоПользователю()  //АБС Коломиец
	
	ОтборРиИ = РолиИИсполнителиРегистр.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборРиИ.ВидСравнения 	= ВидСравненияКомпоновкиДанных.Равно;
	ОтборРиИ.ЛевоеЗначение 	= Новый ПолеКомпоновкиДанных("Исполнитель");
	ОтборРиИ.ПравоеЗначение = Объект.Ссылка;
	ОтборРиИ.Использование	= Истина;
	
 	ОтборОЛП = ОтветственныеЛицаПодразделенийРегистр.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборОЛП.ВидСравнения 	= ВидСравненияКомпоновкиДанных.Равно;
	ОтборОЛП.ЛевоеЗначение 	= Новый ПолеКомпоновкиДанных("Сотрудник");
	ОтборОЛП.ПравоеЗначение = Объект.абс_Сотрудник;
	ОтборОЛП.Использование	= Истина;

	//АБС_Родин
	ОтборОЛП = абс_ЦФОПользователей1.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборОЛП.ВидСравнения 	= ВидСравненияКомпоновкиДанных.Равно;
	ОтборОЛП.ЛевоеЗначение 	= Новый ПолеКомпоновкиДанных("Пользователь");
	ОтборОЛП.ПравоеЗначение = Объект.Ссылка;
	ОтборОЛП.Использование	= Истина;
	
	ОтборОЛП = ОтветственныеРНПД.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборОЛП.ВидСравнения 	= ВидСравненияКомпоновкиДанных.Равно;
	ОтборОЛП.ЛевоеЗначение 	= Новый ПолеКомпоновкиДанных("Ответственный");
	ОтборОЛП.ПравоеЗначение = Объект.Ссылка;
	ОтборОЛП.Использование	= Истина;  
    //АБС_Родин   	
...
Процедура УстановитьЭлементыПриРассинхронизации(знач РассогласованиеБудетСнято = Ложь)
	...
	ИначеЕсли РассогласованиеДанных = 3 Тогда
		...
		ИмяПолноеИмя = Пользователи.ПолучитьИмяПолноеИмя(Объект.ИдентификаторПользователяИБ);
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПравоАдминистрирования Тогда
		...
		Элементы.СогласованиеРеклассификацииБК.Доступность = ЗначениеЗаполнено(Объект.Ссылка);   		
	Иначе
		Элементы.СогласованиеРеклассификацииБК.Доступность = Ложь;
...
Процедура ЗаписатьПользователяИБ(Отказ, ТекущийОбъект)
	...
	Если ПравоАдминистрирования Тогда
		...
		Если Не Отказ Тогда
			...
			Если РассогласованиеДанных = 1 ИЛИ РассогласованиеДанных = 2 Тогда
				Идентификатор = Пользователи.ЗаписатьПользователяИБ(
...
Функция УдалитьПользователяИБНаСервере()
	...
	Результат = Пользователи.УдалитьПользователяИБ(Объект.ИдентификаторПользователяИБ);
...
Функция ПолучитьПользователяИБ(ЗначениеКопирования = Неопределено)
	...
	Если РольДоступна("абс_АдминистраторДЗО_Филиала") Тогда  //АБС Коломиец 19351
...
Процедура ЗаполнитьСписокРолейПользователяИБ(Знач ПользовательИБ = Неопределено)
	...
	Если НЕ ПравоАдминистрирования 
		И Не РольДоступна("абс_АдминистраторДЗО_Филиала") Тогда  //АБС Коломиец 19351
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ДобавитьОтветственныйПодразделение(Команда)
	
	рег = РегистрыСведений.абс_ОтветственныеЛицаПодразделений.СоздатьМенеджерЗаписи();
	рег.Сотрудник = Объект.абс_Сотрудник;
	Форма = рег.ПолучитьФорму("ФормаЗаписиУправляемая");
...
&НаКлиенте
Процедура СогласованиеРеклассификацииБК1ПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	ТекСтрока.Ответственный = Объект.Ссылка;
	
КонецПроцедуры

&НаСервере
Процедура ЗаписатьРегНаСервере()
    НЗ = ДанныеФормыВЗначение(СогласованиеРеклассификацииБК, Тип("РегистрСведенийНаборЗаписей.абс_СогласованиеРеклассификацииБК"));
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Набор = РегистрыСведений.абс_СогласованиеРеклассификацииБК.СоздатьНаборЗаписей();
	Набор.Отбор.Ответственный.Установить(Объект.Ссылка);
	Набор.Прочитать();           
	ЗначениеВДанныеФормы(Набор,СогласованиеРеклассификацииБК);	

КонецПроцедуры

&НаКлиенте
Процедура СогласованиеРеклассификацииБК1ПослеУдаления(Элемент)
	
	ЗаписатьРегНаСервере();
	
КонецПроцедуры

&НаКлиенте
Процедура СогласованиеРеклассификацииБК1ПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
