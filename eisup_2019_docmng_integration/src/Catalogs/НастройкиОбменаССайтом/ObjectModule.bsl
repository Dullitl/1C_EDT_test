...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ОбменТоварами 
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Должен быть выбран хотя бы один из режимов выгрузки: ""Выгрузка товаров"" или ""Обмен заказами""!", Отказ);
	...
	Если ВыгружатьНаСайт Тогда
		...
		Если Не ЗначениеЗаполнено(HTTPОбменАдресСайта) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указан WEB - сайт для обмена данными!", Отказ);
		КонецЕсли;
		
	Иначе
		
		Если Не ЗначениеЗаполнено(КаталогВыгрузки) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указан каталог для обмена данными!", Отказ);
		КонецЕсли;
	...
	Если ВыгружатьТолькоИзменения Тогда
		...
		Если ОбменТоварами И НЕ ЗначениеЗаполнено(УзелОбменаТоварами) ТОгда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Узел плана обмена для контроля изменений товаров не указан!", Отказ);
		КонецЕсли;
		
		Если ОбменЗаказами И НЕ ЗначениеЗаполнено(УзелОбменаЗаказами) ТОгда
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Узел плана обмена для контроля изменений заказов не указан!", Отказ);
		КонецЕсли;
