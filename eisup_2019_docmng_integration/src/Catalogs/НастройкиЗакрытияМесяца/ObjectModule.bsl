...
Процедура ПередЗаписью(Отказ)
	...
	Если  ДатаНачалаДействияНастройки = Дата('00010101000000') Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указана дата начала действия настройки", Отказ);
		Отказ = истина;
	КонецЕсли;
	Если НЕ (ОтражатьВУправленческомУчете или ОтражатьВБухгалтерскомУчете ИЛИ ОтражатьВНалоговомУчете) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не установлены признаки отражения в учете", Отказ);
	КонецЕсли;
	
	Строка = РегламентныеОперации.Найти(истина,"Использовать");
	Если Строка = Неопределено Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не включена ни одна регламентная операция",Отказ);
	КонецЕсли;
