...
Процедура ЗаполнитьРеквизитыОбъекта(Отказ, ТекстСообщенияОшибки)
	...
	Если ОбщегоНазначенияЗК.ПолучитьРабочуюДату() < ПроведениеРасчетов.ДатаЗаменыЕСНСтраховымиВзносами() И Не ЗначениеЗаполнено(ВидСтавокЕСНиПФР) Тогда
...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ОбменДанными.Загрузка Тогда
		...
		Если Отказ Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщенияОшибки, Отказ);
		КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	//АБС ВСТАВКА Навценя 05.03.2015
