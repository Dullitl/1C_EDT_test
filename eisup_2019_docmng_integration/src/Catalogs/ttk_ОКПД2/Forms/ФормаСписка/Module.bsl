...
Процедура ОбновитьИзРегистраНаСервере()
	лЗапрос = новый Запрос("ВЫБРАТЬ * ИЗ РегистрСведений.ttk_ОКПД2.СрезПоследних УПОРЯДОЧИТЬ ПО Код");
	...
	для каждого лСтрокаТЗ из лтзВыборка цикл
		...
		лЭлемент = Справочники.ttk_ОКПД2.НайтиПоКоду(лТекКод);
		если НЕ ЗначениеЗаполнено(лЭлемент) тогда
			лобЭлемент = Справочники.ttk_ОКПД2.СоздатьЭлемент();
		...
		конецЕсли;
		//.Категория
		если лобЭлемент.Категория <> лСтрокаТЗ.Категория тогда
			лобЭлемент.Категория = лСтрокаТЗ.Категория;
		конецЕсли;
		//.Подкатегория
		если лобЭлемент.Подкатегория <> лСтрокаТЗ.Подкатегория тогда
			лобЭлемент.Подкатегория = лСтрокаТЗ.Подкатегория;
		конецЕсли;
		//.Включает
	...
	лЗапрос2.Текст =
	"ВЫБРАТЬ ttk_ОКПД2.Код
	|ИЗ Справочник.ttk_ОКПД2 КАК ttk_ОКПД2
	|УПОРЯДОЧИТЬ ПО ttk_ОКПД2.Код";
...
Функция ДайРодителяПоКоду(пКод_)
	лРезультат = Справочники.ttk_ОКПД2.ПустаяСсылка();
	...
	пока НЕ ПустаяСтрока(лКодРодителя) цикл
		лРодитель = Справочники.ttk_ОКПД2.НайтиПоКоду(лКодРодителя);
