...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ОбменДанными.Загрузка
	   ...
	   И ПолныеПрава.Склад_СуществуютСсылки(Ссылка) Тогда
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Существуют документы, в которых выбран склад """ + Наименование + """.
	...
	Если Не ОбменДанными.Загрузка
	   ...
	   И НЕ ЗначениеЗаполнено(ТипЦенРозничнойТорговли) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Для склада вида """+ВидСклада+""" необходимо указать тип цен розничной торговли,
		                 |элемент не записан.");
