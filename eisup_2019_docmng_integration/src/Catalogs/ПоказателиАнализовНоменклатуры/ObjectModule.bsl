Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ОбменДанными.Загрузка И НЕ ЭтоГруппа Тогда
		...
		Если НЕ ЗначениеЗаполнено(ВидРезультатаАнализа) Тогда
			ttk_ОбщегоНазначения.СообщитьОбОшибке( "Не указано значение вида результатов анализа.", Отказ);
		КонецЕсли;
...
#ЕСЛИ КЛИЕНТ ТОГДА				
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указано значение показателя в строке № " + ТекЗначение.НомерСтроки, Отказ);
#КОНЕЦЕСЛИ
				КонецЕсли;
			КонецЦикла;
...
#ЕСЛИ КЛИЕНТ ТОГДА				
				ttk_ОбщегоНазначения.СообщитьОбОшибке("Значение начала интервала должно быть меньше конца интервала.", Отказ);
#КОНЕЦЕСЛИ
			КонецЕсли;
