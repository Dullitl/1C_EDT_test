...
Процедура ПриЗаписи(Отказ) 
	...
	Исключение
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Ошибка при записи изменений в настройку обмена """ + Ссылка + """ : " + ОписаниеОшибки(), Отказ);
	КонецПопытки;	
...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ТипНастройки) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не заполнено поле ""Тип настройки""", Отказ);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не заполнено поле ""Узел""", Отказ);
	КонецЕсли;
	
	Если БазоваяПоставка
		И ПроцедурыОбменаДанными.ОпределитьПоУзлуОбменаЭтоРИБ(УзелИнформационнойБазы) Тогда
		
		ttk_ОбщегоНазначения.СообщитьОбОшибке("В базовой версии недоступен обмен через распределенную информационную базу", Отказ);	
	...
	Если ЭтотУзелОбмена = УзелИнформационнойБазы Тогда
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке("В качестве узла обмена выбран узел, соответствующий текущей информационной базе.", Отказ);
	...
	Если OnLineОбмен Тогда
		...
		Если (ПроизводитьПриемСообщений
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не заданы правила обмена данными для выгрузки в базу обмена", Отказ);		
		...
		Если ТипНастройки = Перечисления.ТипыАвтоматическогоОбменаДанными.ОбменЧерезComСоединение Тогда
			...
			Если БазоваяПоставка Тогда
				...
				ttk_ОбщегоНазначения.СообщитьОбОшибке("В базовой версии недоступен Режим обмена через COM - соединение", Отказ);	
			...
			Если ПроизводитьПриемСообщений Тогда
				...
				Если РазмерХранилищаПравилЗагрузки = 0 Тогда
					...
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не заданы правила для выгрузки из базы обмена в текущую базу", Отказ);		
			...
			Если ТипИнформационнойБазыДляПодключения Тогда
				...
				Если НЕ ЗначениеЗаполнено(КаталогИнформационнойБазыДляПодключения) Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не заполнено поле ""Каталог информационной базы для подключения""", Отказ);
				КонецЕсли;
				
			Иначе
				
				Если НЕ ЗначениеЗаполнено(ИмяСервераИнформационнойБазыДляПодключения) Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не заполнено поле ""Сервер 1С:Предприятия""", Отказ);
				КонецЕсли;
				
				Если НЕ ЗначениеЗаполнено(ИмяИнформационнойБазыНаСервереДляПодключения) Тогда
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Не заполнено поле ""Имя информационной базы""", Отказ);
				КонецЕсли;
	...
	Иначе
		...
		Если ТипНастройки = Перечисления.ТипыАвтоматическогоОбменаДанными.ОбменЧерезComСоединение Тогда
			...
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Режим обмена через COM - соединение возможно только для обмена по правилам обмена", Отказ);		
