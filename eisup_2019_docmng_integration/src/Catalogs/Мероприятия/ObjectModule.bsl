...
//  Нет.
//
Функция Печать(Регистратор = Неопределено) Экспорт
	...
	Иначе
		...
		|		ПО ПланируемаяЗанятостьФизлиц.ФизЛицо = ФизическиеЛица.Ссылка
		|ГДЕ
		...
		Иначе
			...
			Для Каждого СоставнаяЧастьМероприятия Из ТаблицаСоставаМероприятия Цикл
				...
				Иначе
					...
					//Формируем строку - представление времени
	...
	Возврат УниверсальныеМеханизмы.НапечататьДокумент(ДокументРезультат,,,?(ЗначениеЗаполнено(Регистратор),Регистратор, "Мероприятие: " + Наименование));
