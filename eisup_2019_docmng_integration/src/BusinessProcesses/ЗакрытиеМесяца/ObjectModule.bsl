...
Функция ПроверитьВозможностьЗапуска() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(НастройкаЗакрытияМесяца) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не указана настройка закрытия месяца.");
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ Результат.Пустой() Тогда
		...
		ttk_ОбщегоНазначения.СообщитьОбОшибке(ТекстПредупреждение);
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ (ОтражатьВУправленческомУчете ИЛИ ОтражатьВБухгалтерскомУчете ИЛИ ОтражатьВНалоговомУчете) Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не установлены признаки отражения в учетах",Отказ);
	КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	Если ПометкаУдаления И Стартован Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Процедура закрытия месяца запущена. Перед пометкой на удаление необходимо отменить запуск процедуры",Отказ);
	Иначе	
		БизнесПроцессРасчетНДС = БизнесПроцессЗакрытиеМесяца.ПолучитьПодчиненныйБизнесПроцессРасчетНДС(Ссылка);
