...
Процедура КонтрольОстатков(ДокументОбъект, ИмяТабличнойЧасти, СтруктураШапкиДокумента, Отказ, Заголовок, РежимПроведения) Экспорт
	...
	//АБС ВКЛЮЧИМ КОНТРОЛЬ ОСТАТКОВ ДЛЯ ВСЕХ РЕЖИМОВ ПРОВЕДЕНИЯ
	...
	СтруктураУП = ttk_ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиУпр(СтруктураШапкиДокумента.Ссылка.Дата);
	...
	|ИЗ
	|	РегистрНакопления.ТоварыКПередачеОрганизаций.Остатки(&ДатаОстатков," + //АБС Добавлен отбор по дате остатков
	...
	|ПОМЕСТИТЬ Остатки
	|ИЗ
	|	РегистрНакопления.ТоварыОрганизаций.Остатки(&ДатаОстатков,"+  //АБС Добавлен отбор по дате остатков
	?(КонтролироватьОстатокПоСкладу , "Склад в (&СписокСкладов) И ","")+"
	|		Номенклатура В (" + ТекстЗапросаСписокНоменклатуры + ")
	|	И Организация = &Организация
	|	И Комиссионер = &Комиссионер) КАК Остатки
	//АБС |ДЛЯ ИЗМЕНЕНИЯ РегистрНакопления.ТоварыОрганизаций.Остатки // Блокирующие чтение таблицы остатков регистра для разрешения коллизий многопользовательской работы
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура
	|"	+ ?(ЕстьХарактеристика, " , ХарактеристикаНоменклатуры", "") + "
	|"	+ ?(ЕстьСерия,          " , СерияНоменклатуры", "") + "
	...
	//АБС
	Запрос.УстановитьПараметр("ДатаОстатков", ДокументОбъект.МоментВремени());
	//\\АБС
