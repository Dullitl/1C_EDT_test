...
Процедура ПередВыполнением(Отказ)
	...
	Если ТочкаМаршрута = БизнесПроцессы.РасчетНДС.ТочкиМаршрута.РегистрацияСчетовФактурНаАванс Тогда
		ПроверитьНаличиеСчетовФактур(Перечисления.НДСВидСчетаФактуры.НаАванс, Отказ, Заголовок);
		Возврат;
	ИначеЕсли ТочкаМаршрута = БизнесПроцессы.РасчетНДС.ТочкиМаршрута.РегистрацияСчетовФактурНаСуммовыеРазницы Тогда
		ПроверитьНаличиеСчетовФактур(Перечисления.НДСВидСчетаФактуры.НаСуммовуюРазницу, Отказ, Заголовок);
		Возврат;
	КонецЕсли;
	...
	Если ОтказУУ ИЛИ ОтказБУ ИЛИ ОтказНУ Тогда
		ttk_ОбщегоНазначения.СообщитьОбОшибке("Не были созданы либо не были проведены все надлежащие документы. Регламентная операция может быть выполнена только в режиме ""без проверок"" (Действия - Выполнить без проверок)",Отказ, Заголовок);			
	ИначеЕсли ТочкаМаршрута = БизнесПроцессы.ЗакрытиеМесяца.ТочкиМаршрута.РассчитатьНДСПоПродукции Тогда
...
Функция СоздатьРегламентныйДокумент(ТипДокумент, Дата, ДатаРегистрации, Организация = Неопределено, ОтражатьВУправленческомУчете = Истина, ОтражатьВБухгалтерскомУчете = Истина, ОтражатьВНалоговомУчете = Истина, Проводить = Истина)
	...
	Если ТипДокумент = "РасчетСебестоимостиВыпуска" ИЛИ
		...
		Исключение
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не удалось заполнить и создать документ: "+ОписаниеОшибки());
			Возврат ложь;
		КонецПопытки;
	...
	ИначеЕсли ТипЗнч(БизнесПроцесс) = Тип("БизнесПроцессСсылка.РасчетНДС") Тогда
		...
		Если ОшибкаЗаполнения И ЗначениеЗаполнено(СтрокаСообщения) Тогда
			...
			#Если Клиент Тогда
				...
				Иначе
					ttk_ОбщегоНазначения.СообщитьОбОшибке("Необходимо выполнить операцию в режиме «Без проверок»");
				КонецЕсли;
			#КонецЕсли
		КонецЕсли;
	КонецЕсли;	
	...
	Если ЗаписатьДокумент Тогда
		Попытка
			...
			Возврат Истина;
		Исключение
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не удалось создать документ: "+ОписаниеОшибки());
			Возврат Ложь;
		КонецПопытки;	
	ИначеЕсли ОткрытьФормуДокумента Тогда
		Попытка
			...
			Возврат Истина;
		Исключение
			ttk_ОбщегоНазначения.СообщитьОбОшибке("Не удалось открыть форму документа: "+ОписаниеОшибки());
			Возврат Ложь;
		КонецПопытки;
	Иначе
		Возврат Ложь;
...
Процедура ПроверитьНаличиеСчетовФактур(ВидСчетаФактуры, Отказ, Заголовок)
	...
	Если РезультатЗапроса.Пустой() Тогда
		ПредставлениеДокумента = """Счет-фактура выданный"" с видом операции " 
			+ ?(ВидСчетаФактуры = Перечисления.НДСВидСчетаФактуры.НаАванс, """На аванс""", """На суммовые разницы""");
